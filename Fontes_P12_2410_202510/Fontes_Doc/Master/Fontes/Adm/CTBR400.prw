...
ElseIf cTipo == "2"		// Razao por Centro de Custo
	If lAnalit 				// Se o relatorio for analitico
		If FunName() <> "CTBC440"
			If FunName() <> "CTBR440" .AND. !((FWIsInCallStack("TOTVS.FRAMEWORK.TREPORTS.CALLTREPORTS") .or.;
								 FWIsInCallStack("TOTVS.FRAMEWORK.TREPORTS.INTEGRATEDPROVIDER.EXECDATA")) .and. lRazCCSV)
...
If !lCtbRazBD .And.;
	...
	(FunName() == 'CTBR400' .Or.;
	 ...
	 FWIsInCallStack("TOTVS.FRAMEWORK.TREPORTS.INTEGRATEDPROVIDER.EXECDATA") .and. !lRazaoSV )) .And.; 
...
If ( Empty(cUserFil) )
	...
	If ( lCtbRazBD == NIL )//RECEBERA MV_CTBRAZB (L)
		If Alltrim(TCGetDb())$"MSSQL7|ORACLE|DB2|INFORMIX|POSTGRES"
			...
			ElseIf FWIsInCallStack("TOTVS.FRAMEWORK.TREPORTS.CALLTREPORTS") .Or.;
...
If ( nTipoRel <> 3 )
	...
	If 	FunName() == "CTBR400" .OR.;
	   	...
	   	(FWIsInCallStack("TOTVS.FRAMEWORK.TREPORTS.CALLTREPORTS") .Or.;
		FWIsInCallStack("TOTVS.FRAMEWORK.TREPORTS.INTEGRATEDPROVIDER.EXECDATA") .and. !lRazaoSV )
		cProcCT2 += "SELECT  @cHistCT2FIM = ''   "+ CRLF
		cProcCT2 += "SELECT @iRecnoTmp = 0 "+ CRLF
		cProcCT2 += "SELECT  @iRecnoTmpAnt = 0 " + CRLF
		cProcCT2 += "SELECT @cHistOri ='' "+ CRLF
