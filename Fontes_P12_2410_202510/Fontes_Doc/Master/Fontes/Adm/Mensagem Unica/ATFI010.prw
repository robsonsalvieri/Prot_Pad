...
Function ATFI010(cXML, nTypeTrans, cTypeMessage, cVersion)
   ...
   IIf(lLog, AdpLogEAI(1, "ATFI010", nTypeTrans, cTypeMessage, cXML), ConOutR(STR0010))
      ...
      If cTypeMessage == EAI_MESSAGE_BUSINESS
         ...
         If oXml != Nil .And. Empty(cError) .And. Empty(cWarning)
            ...
            If Type("oXml:_TOTVSMessage:_BusinessMessage:_BusinessContent:_Code:Text") == "U" .Or. Empty(oXml:_TOTVSMessage:_BusinessMessage:_BusinessContent:_Code:Text)
               ...
               IIf(lLog, AdpLogEAI(5, "ATFI010", cXMLRet, lRet), ConOutR(STR0010))
               Return {lRet,cXMLRet}
            Else
      ...
      ElseIf cTypeMessage == EAI_MESSAGE_RESPONSE
         ...
         If Upper(oXML:_TOTVSMessage:_ResponseMessage:_ProcessingInformation:_Status:Text) == "OK"
            ...
            Else
               ...
               IIf(lLog, AdpLogEAI(5, "ATFI010", nTypeTrans, cTypeMessage, cXML), ConOutR(STR0010))
               Return {lRet, cXmlRet}
            EndIf
...
Function IntAdvExt(cEmpresa, cFil, cCode, cNumero, cTipo, cBaixa, cSeq, cVersao)
   ...
   Else
      aAdd(aResult, .F.)
      aAdd(aResult, STR0007 + Chr(10) + STR0022) // "Versão não suportada." "As versões suportadas são: 1.000"
   EndIf
Return aResult
...
Function IntAdvInt(cInternalID, cRefer, cVersao)
   ...
   If Empty(cTemp)
      aAdd(aResult, .F.)
      aAdd(aResult, STR0008 + cInternalID + STR0009) // "Ativo " + cInternalID + " não encontrado no de/para!"
   Else
      If cVersao == '1.000' .Or. cVersao == '1.001'
         aAdd(aResult, .T.)
         aTemp := Separa(cTemp, '|')
...
Static Function GetItem(cFil, cBase)
   Local cResult := PADL("1", TAMSX3("N1_ITEM")[1], "0")
   Local cQuery  := ""
   Local aAreaAnt := SN1->(GetArea())
   Local cAlias  := "SN3"
