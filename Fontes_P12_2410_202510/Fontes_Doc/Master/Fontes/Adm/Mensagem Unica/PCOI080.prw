...
Static cMessage   := "BudgetOperation"

/*/{Protheus.doc} PCOI080
Função de integração com o adapter EAI para envio e recebimento do cadastro de
operações da planilha orçamentária (AKF) utilizando o conceito de mensagem única.
...
/*/
Function PCOI080(cXml, cTypeTrans, cTypeMsg, cVersion, cTransac)
...
ElseIf (cTypeTrans == TRANS_SEND)
	If (cTypeMsg == EAI_MESSAGE_BUSINESS)
		...
		cValInt := _NoTags(RTrim(oModel:GetValue('AKFMASTER', 'AKF_CODIGO')))
		...
		cXMLRet += ' <InternalId>' + cEmpAnt + '|' + xFilial("AKF") + '|' + cValInt + '</InternalId>'
		cXMLRet += ' <Description>' + _NoTags(RTrim(oModel:GetValue('AKFMASTER', 'AKF_DESCRI'))) + '</Description>'
		cXMLRet += '</BusinessContent>'
		oModel := nil
	Endif
...
ElseIf (cTypeTrans == TRANS_RECEIVE)
			...
			If upper(oXml:xPathGetNodeValue('/TOTVSMessage/ResponseMessage/ProcessingInformation/Status')) = "OK"
				...
				For nX := 1 to len(aIntID)
					...
					If cEvent = 'DELETE' .and. !empty(cValInt)
						CFGA070Mnt(cRefer, "AKF", "AKF_CODIGO", nil, cValInt, .T.)
					ElseIf !empty(cValInt) .and. !empty(cValExt)
						CFGA070Mnt(cRefer, "AKF", "AKF_CODIGO", cValExt, cValInt)
					Else
						lRet  := .F.
	...
	ElseIf (cTypeMsg == EAI_MESSAGE_BUSINESS)  // Chegada de mensagem de negócios.
		...
		If Empty(cErro := oXml:Error())
			...
			cValInt := RTrim(CFGA070Int(cRefer, "AKF", "AKF_CODIGO", cValExt))
			aValInt := StrToKarr2(cValInt, "|", .T.)

			// Verifica se encontrou uma chave no de/para.
			If len(aValInt) > 2
				AKF->(dbSetOrder(1))  // AKF_FILIAL, AKF_CODIGO, AKF_DESCRI.
				lFound := AKF->(dbSeek(xFilial(nil, aValInt[2]) + aValInt[3], .F.))
			Endif
			...
			If lRet
				...
				If oModel:Activate()
					If nMVCOper <> MODEL_OPERATION_DELETE
						If nMVCOper == MODEL_OPERATION_INSERT
							...
							cValInt := oModel:GetValue('AKFMASTER', 'AKF_CODIGO')
							If empty(cValInt)
								cValInt := GetSXENum('AKF', 'AKF_CODIGO')
								oModel:SetValue('AKFMASTER', 'AKF_CODIGO', cValInt)
							Endif
							cValInt := cEmpAnt + '|' + xFilial("AKF") + '|' + cValInt
						Endif
						cNodePath := '/TOTVSMessage/BusinessMessage/BusinessContent/'
						oModel:SetValue('AKFMASTER', 'AKF_DESCRI', oXml:xPathGetNodeValue(cNodePath + 'Description'))
					Endif
				...
				Else
					lRet  := .F.
					cErro := STR0006 //"Erro ao ativar modelo PCOA080."
				Endif
