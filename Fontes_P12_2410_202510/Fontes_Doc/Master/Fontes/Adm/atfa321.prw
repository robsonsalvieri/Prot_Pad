...
lAutomato			:= IIF(Len(xAutoCab)>0,.T.,.F.)
...
IF lAutomato
	...
	If  nOpcAuto == 3 .or. nOpcAuto == 4 .or. nOpcAuto == 5 .or. nOpcAuto == 6 .or. nOpcAuto == 7 .or.  nOpcAuto == 8
		ATF321Cad("SNP",,nOpcAuto,aAutoCab,aAutoItens,.F.,lAutomato)
	ElseIf nOpcAuto == 9
		ATF321Bxa("SNP", , 9)
	Else	
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
Function ATF321Cad(cAlias,nReg,nOpc,xAutoCab,xAutoItens,lDireto,lAuto)
...
If !lDireto .And. !lAuto  .And. SNP->(!EOF())
...
IF lDireto .Or. lAuto
    ...
    If lAuto
        nPosTipCes	:= aScan(aAutoCab,{|x| x[1] == "NP_TIPCES"})
        If nOpc == 4 .or. nOpc == 5 .or. nOpc == 7 .or. nOpc == 8
            nPosCod	:= aScan(aAutoCab,{|x| x[1] == "NP_CODIGO"})
        EndIf	
    EndIf
ENDIF
...
If lRet
	...
	ElseIf nOpc == 3 //Incluir
		...
		IF lDireto .or. lAuto
            ...
            If lAuto
                M->NP_FILIAL	:= aAutoCab[1][2]
                M->NP_TIPCES	:= aAutoCab[nPosTipCes][2]
            EndIf
		ENDIF
	ElseIf nOpc == 4 //Alterar
		IF lAuto
			alEdtEnch	:= {"NP_FORNEC","NP_LOJA","NP_TIPCES"}
			M->NP_FILIAL	:= aAutoCab[1][2]
			M->NP_CODIGO	:= aAutoCab[nPosCod][2]
			M->NP_CBASE 	:= aAutoCab[nPosBase][2]
			M->NP_ITEM		:= aAutoCab[nPosItem][2]
			M->NP_FORNEC	:= aAutoCab[nPosFornece][2]
			M->NP_LOJA		:= aAutoCab[nPosLojaFor][2]
	...
	ElseIf nOpc == 5  //Excluir
		IF lAuto
			...
			For nX := 1 to Len(aAutoItens)
				If SNP->(DbSeek(PadR(aAutoCab[1][2], TamSx3("NP_FILIAL")[1]) + PadR(aAutoCab[nPosBase][2], TamSx3("NP_CBASE")[1]) +  PadR(aAutoCab[nPosItem][2], TamSx3("NP_ITEM")[1]) + PadR(aAutoCab[nPosCod][2], TamSx3("NP_CODIGO")[1]) + aAutoItens[nX][1][2]           ))
					If !ATF321Del()
						Return()
					EndIf
				EndIf
	...
	If !lAuto
		...
		Else
			Activate MsDialog oDlg on Init EnchoiceBar(oDlg,{||Eval({||Iif(Obrigatorio(aGets,aTela) .and. ATFDtVig(nOpc),(llSave := ATF321Grv(nOpc,lAutomato,cCodOri,alEdtGD),oDlg:End()),Nil)})},{||oDlg:End()})
		Endif
		If !llSave
			RollBackSX8()
		Else
	...
	Else
            ...
            If ATFDtVig(nOpc)
                llSave := ATF321Grv(nOpc,lAuto,cCodOri,alEdtGD)
                If !llSave
                    RollBackSX8()
                Else
                    ConfirmSX8()
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
Static Function ATF321Grv(nOpc,lAutomato,cCodOri,alEdtGD)
...
Local clCodBase := PadR(M->NP_CBASE, TamSx3("NP_CBASE")[1])
Local clItem 	:= PadR(M->NP_ITEM, TamSx3("NP_ITEM")[1])
Local lRet		:= .T.
Local nPosArray := 0  
	...
	If lAutomato
		...
		For nlI := 1 to Len(aCols)
			...
			For nlJ := 1 to Len(alCpoGD)
				alValGD[nlPos][nlJ] := aCols[nlI][nlJ]
			Next nlJ
			nlQuant ++
		Next nlI
		DBSelectArea("SNP")
		...
		For nlI := 1 to nlQuant
			If nOpc == 3
				...
				Else
					llRecLock := .T.
				EndIf
			ElseIf SNP->(DBSeek(xFilial("SNP") + clCodBase + clItem + M->NP_CODIGO + aCols[nlI][1][2]))
				llRecLock := .F.
			Else
				llRecLock := .T.
			EndIf
			...
			Next nlJ  
			For nlJ := 1 to Len(alCpoGD)
				FieldPut( FieldPos( alCpoGD[nlJ] ), alValGD[nlI][nlJ][2] )
			Next nlJ 
			SNP->(MsUnlock())
		Next nlI
	Else
...
If nOpc != 7 .AND. nOpc != 5
	If lAutomato
		SNP->(MsSeek(xFilial("SNP")+M->(NP_CBASE+NP_ITEM+NP_CODIGO+StrZero(Len(aCols),nTamSeq))))
	Else
		SNP->(MsSeek(xFilial("SNP")+M->(NP_CBASE+NP_ITEM+NP_CODIGO+StrZero(Len(oGetSNP:aCols),nTamSeq))))
	EndIf	
Else
	If lAutomato
		SNP->(MsSeek(xFilial("SNP")+M->(NP_CBASE+NP_ITEM+NP_CODIGO+StrZero((Len(aCols))-1,nTamSeq))))
	Else
	...
	If lAutomato
		...
		If lRet .and. (nOpc == 7 .or. nOpc == 8)
			...
			If DbSeek(xFilial("SNP")+M->NP_CBASE+M->NP_ITEM)
				If aCols[nTamanho][nPosVigIni][2] <= dUltDepr //se data inicial digitada for menor que a data da ultima depreciacao
					lRet := .F.
					Help(" ",1,"ATFA321DINI3",,STR0008,1,0)
				ElseIf aCols[nTamanho][nPosVigFim][2] <= dUltDepr //se data final digitada for menor que a data da ultima depreciacao
					lRet := .F.
...
If lAutomato
	...
	cBase := PadR(aAutoCab[nPosBase][2], TamSx3("NP_CBASE")[1])
	cItem := PadR(aAutoCab[nPosItem][2], TamSx3("NP_ITEM")[1])

	
	lRet := AF321VLINI(nlOpc,aAutoCab[nPosCod][2],cBase,cItem)
...
If lRet
	If lAutomato
		...
		SNP->(dbSetOrder(2))//NP_FILIAL+NP_CBASE+NP_ITEM+NP_CODIGO+NP_SEQ
		While SNP->(!Eof()) 
			If SNP->(NP_FILIAL + NP_CBASE + NP_ITEM + NP_CODIGO) == xFilial("SNP") + cBase + cItem + aAutoCab[nPosCod][2]
				RecLock("SNP",.F.)
				SNP->NP_STATUS := '2'
				SNP->(MsUnLock())
			EndIf	
