#INCLUDE "PROTHEUS.CH"
#INCLUDE "PCPA120.CH"
#INCLUDE "FWMVCDEF.CH"

/*/{Protheus.doc} ModelDef
Definição do Modelo de Consulta de Listas
@type  Static Function
@author Lucas Fagundes
@since 07/02/2022
@version P12
@return oModel - Modelo de dados definido
/*/
Static Function ModelDef()
    
    Local oModel    := Nil
    Local oStruGrid := FWFormModelStruct():New()
    Local oStruSMW  := FWFormStruct(1,"SMW")

    MontaStru(@oStruGrid, .T.)

    oModel := MPFormModel():New("PCPA120Con")
    oModel:AddFields("SMWMASTER", /*cOwner*/, oStruSMW)
    oModel:AddGrid("DETAILGRID", "SMWMASTER", oStruGrid,,,,,{|| LoadConsul()})

    oModel:GetModel("DETAILGRID"):SetDescription(STR0060) // "Grid detalhes da consulta"
    oModel:GetModel("DETAILGRID"):SetOnlyQuery(.T.)

Return oModel

/*/{Protheus.doc} ViewDef
Definição da View da Consulta de Listas
@type  Static Function
@author Lucas Fagundes
@since 07/02/2022
@version P12
@return oView - View definida
/*/
Static Function ViewDef()
    
    Local oModel    := FWLoadModel("PCPA120Con")
    Local oStruGrid := FWFormViewStruct():New()
    Local oStruSMW  := FWFormStruct(2,"SMW")
    Local oView     := Nil

    MontaStru(@oStruGrid, .F.)

    oView := FWFormView():New()

    oView:SetModel(oModel)

    oView:AddField("VIEW_SMW", oStruSMW, "SMWMASTER")
    oView:AddGrid("VIEW_GRID", oStruGrid, "DETAILGRID")

    oView:CreateHorizontalBox("SUPERIOR", 70,,.T.)
    oView:CreateHorizontalBox("INFERIOR", 100)
    oView:SetOwnerView("VIEW_SMW", "SUPERIOR")
    oView:SetOwnerView("VIEW_GRID", "INFERIOR")

Return oView

/*/{Protheus.doc} MontaStru
Função para montar a estrutura da grid.
@type  Static Function
@author Lucas Fagundes
@since 07/02/2022
@version P12
@param 01 oStruGrid, object, Objeto que será montado a estrutura
@param 02 lModel, logic, Indica se a chamada é para model ou view
@return oStruGrid, object, Objeto com a estrutura
/*/
Static Function MontaStru(oStruGrid, lModel)
    
    If lModel        
        // Estrutura do grid para o model
        oStruGrid:AddField(STR0061                                 ,;    //    [01]  C   Titulo do campo  // "Local" 
						   STR0062                                 ,;    //    [02]  C   ToolTip do campo // "Local onde a lista é utilizada"
						   "LOCAL"                                 ,;    //    [03]  C   Id do Field
						   "C"                                     ,;    //    [04]  C   Tipo do campo
						   Len(STR0033)                            ,;    //    [05]  N   Tamanho do campo
						   0                                       ,;    //    [06]  N   Decimal do campo
						   NIL                                     ,;    //    [07]  B   Code-block de validação do campo
						   NIL                                     ,;    //    [08]  B   Code-block de validação When do campo
						   {STR0032, STR0033}                      ,;    //    [09]  A   Lista de valores permitido do campo // "Estrutura" "Pré-estrutura"
						   .F.                                     ,;    //    [10]  L   Indica se o campo tem preenchimento obrigatório
						   Nil                                     ,;    //    [11]  B   Code-block de inicializacao do campo
						   NIL                                     ,;    //    [12]  L   Indica se trata-se de um campo chave
						   NIL                                     ,;    //    [13]  L   Indica se o campo pode receber valor em uma operação de update.
						   .T.)                                          //    [14]  L   Indica se o campo é virtual
        oStruGrid:AddField(STR0041                                 ,;    //    [01]  C   Titulo do campo  // "Produto"
						   STR0041                                 ,;    //    [02]  C   ToolTip do campo // "Produto"
						   "PRODUTO"                               ,;    //    [03]  C   Id do Field
						   "C"                                     ,;    //    [04]  C   Tipo do campo
						   GetSx3Cache("G1_COD", "X3_TAMANHO")     ,;    //    [05]  N   Tamanho do campo
						   0                                       ,;    //    [06]  N   Decimal do campo
						   NIL                                     ,;    //    [07]  B   Code-block de validação do campo
						   NIL                                     ,;    //    [08]  B   Code-block de validação When do campo
						   {}                                      ,;    //    [09]  A   Lista de valores permitido do campo
						   .F.                                     ,;    //    [10]  L   Indica se o campo tem preenchimento obrigatório
						   Nil                                     ,;    //    [11]  B   Code-block de inicializacao do campo
						   NIL                                     ,;    //    [12]  L   Indica se trata-se de um campo chave
						   NIL                                     ,;    //    [13]  L   Indica se o campo pode receber valor em uma operação de update.
						   .T.)                                          //    [14]  L   Indica se o campo é virtual             
        oStruGrid:AddField(STR0034                                 ,;    //    [01]  C   Titulo do campo  // "Descrição"
						   STR0034                                 ,;    //    [02]  C   ToolTip do campo // "Descrição"
						   "DESCRICAO"                             ,;    //    [03]  C   Id do Field
						   "C"                                     ,;    //    [04]  C   Tipo do campo
						   GetSx3Cache("B1_DESC", "X3_TAMANHO")    ,;    //    [05]  N   Tamanho do campo
						   0                                       ,;    //    [06]  N   Decimal do campo
						   NIL                                     ,;    //    [07]  B   Code-block de validação do campo
						   NIL                                     ,;    //    [08]  B   Code-block de validação When do campo
						   {}                                      ,;    //    [09]  A   Lista de valores permitido do campo
						   .F.                                     ,;    //    [10]  L   Indica se o campo tem preenchimento obrigatório
						   Nil                                     ,;    //    [11]  B   Code-block de inicializacao do campo
						   NIL                                     ,;    //    [12]  L   Indica se trata-se de um campo chave
						   NIL                                     ,;    //    [13]  L   Indica se o campo pode receber valor em uma operação de update.
						   .T.)                                          //    [14]  L   Indica se o campo é virtual
    Else
        // Estrutura do grid para a view
        oStruGrid:AddField("LOCAL"             ,;    // [01]  C   Nome do Campo
						   "00"                ,;    // [02]  C   Ordem
                           STR0061             ,;    // [03]  C   Titulo do campo    // "Local"
                           STR0062             ,;    // [04]  C   Descricao do campo // "Local onde a lista é utilizada"
                           NIL                 ,;    // [05]  A   Array com Help
                           "C"                 ,;    // [06]  C   Tipo do campo
                           Nil                 ,;    // [07]  C   Picture
                           NIL                 ,;    // [08]  B   Bloco de PictTre Var
                           NIL                 ,;    // [09]  C   Consulta F3
                           .F.                 ,;    // [10]  L   Indica se o campo é alteravel
                           NIL                 ,;    // [11]  C   Pasta do campo
                           NIL                 ,;    // [12]  C   Agrupamento do campo
                           NIL                 ,;    // [13]  A   Lista de valores permitido do campo (Combo)
                           NIL                 ,;    // [14]  N   Tamanho maximo da maior opção do combo
                           NIL                 ,;    // [15]  C   Inicializador de Browse
                           .T.                 ,;    // [16]  L   Indica se o campo é virtual
                           NIL                 ,;    // [17]  C   Picture Variavel
                           NIL)                      // [18]  L   Indica pulo de linha após o campo
        oStruGrid:AddField("PRODUTO"           ,;    // [01]  C   Nome do Campo
						   "01"                ,;    // [02]  C   Ordem
                           STR0041             ,;    // [03]  C   Titulo do campo    // "Produto"
                           STR0041             ,;    // [04]  C   Descricao do campo // "Produto"
                           NIL                 ,;    // [05]  A   Array com Help
                           "C"                 ,;    // [06]  C   Tipo do campo
                           Nil                 ,;    // [07]  C   Picture
                           NIL                 ,;    // [08]  B   Bloco de PictTre Var
                           NIL                 ,;    // [09]  C   Consulta F3
                           .F.                 ,;    // [10]  L   Indica se o campo é alteravel
                           NIL                 ,;    // [11]  C   Pasta do campo
                           NIL                 ,;    // [12]  C   Agrupamento do campo
                           NIL                 ,;    // [13]  A   Lista de valores permitido do campo (Combo)
                           NIL                 ,;    // [14]  N   Tamanho maximo da maior opção do combo
                           NIL                 ,;    // [15]  C   Inicializador de Browse
                           .T.                 ,;    // [16]  L   Indica se o campo é virtual
                           NIL                 ,;    // [17]  C   Picture Variavel
                           NIL)                      // [18]  L   Indica pulo de linha após o campo
        oStruGrid:AddField("DESCRICAO"         ,;    // [01]  C   Nome do Campo
						   "02"                ,;    // [02]  C   Ordem
                           STR0034             ,;    // [03]  C   Titulo do campo    // "Descrição"
                           STR0034             ,;    // [04]  C   Descricao do campo // "Descrição"
                           NIL                 ,;    // [05]  A   Array com Help
                           "C"                 ,;    // [06]  C   Tipo do campo
                           Nil                 ,;    // [07]  C   Picture
                           NIL                 ,;    // [08]  B   Bloco de PictTre Var
                           NIL                 ,;    // [09]  C   Consulta F3
                           .F.                 ,;    // [10]  L   Indica se o campo é alteravel
                           NIL                 ,;    // [11]  C   Pasta do campo
                           NIL                 ,;    // [12]  C   Agrupamento do campo
                           NIL                 ,;    // [13]  A   Lista de valores permitido do campo (Combo)
                           NIL                 ,;    // [14]  N   Tamanho maximo da maior opção do combo
                           NIL                 ,;    // [15]  C   Inicializador de Browse
                           .T.                 ,;    // [16]  L   Indica se o campo é virtual
                           NIL                 ,;    // [17]  C   Picture Variavel
                           NIL)                      // [18]  L   Indica pulo de linha após o campo
    EndIf

Return oStruGrid

/*/{Protheus.doc} LoadConsul
Fução responsavel por carregar a grid com os dados da consulta
@type  Static Function
@author Lucas Fagundes
@since 08/02/2022
@version P12
@return aLoad, Array, Array com os dados da consulta, que serão inseridos no modelo
/*/
Static Function LoadConsul()

    Local aLoad   := {}
    Local cAlias  := GetNextAlias()
    Local cCod    := SMW->MW_CODIGO
    Local cEst    := STR0032 // "Estrutura"
    Local cPreEst := STR0033 // "Pré-estrutura"

    // Consulta as tabela SG1 e SGG em busca de cadastros que utilizem a lista.
	BeginSql alias cAlias
		SELECT
			DISTINCT SG1.G1_COD AS SGS_COD,
            SB1.B1_DESC AS SB1_DESC,
            %exp:cEst% AS SGS_LOCAL
		FROM
			%table:SG1% SG1
        INNER JOIN %table:SB1% SB1
            ON SG1.G1_COD = SB1.B1_COD AND
            SB1.B1_FILIAL = %xfilial:SB1% AND
            SB1.%notDel%
		WHERE
            SG1.G1_LISTA = %exp:cCod% AND
			SG1.G1_FILIAL = %xfilial:SG1% AND
            SG1.%notDel%
        UNION
        SELECT
			DISTINCT SGG.GG_COD AS SGS_COD,
            SB1.B1_DESC AS SB1_DESC,
            %exp:cPreEst% AS SGS_LOCAL
		FROM
			%table:SGG% SGG
        INNER JOIN %table:SB1% SB1
            ON SGG.GG_COD = SB1.B1_COD AND
            SB1.B1_FILIAL = %xfilial:SB1% AND
            SB1.%notDel%
		WHERE
            SGG.GG_LISTA = %exp:cCod% AND
			SGG.GG_FILIAL = %xfilial:SGG% AND
            SGG.%notDel%
	EndSql

    // Carrega o array com os dados que serão carregados no modelo
    While !(cAlias)->(EOF())
        aAdd(aLoad, {0, {(cAlias)->(SGS_LOCAL),; 
                         (cAlias)->(SGS_COD),;
                         (cAlias)->(SB1_DESC)}})
        (cAlias)->(DbSkip())
    End

    (cAlias)->(dbCloseArea())

Return aLoad
