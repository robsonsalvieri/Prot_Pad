...
/*/{Protheus.doc} ImprodutivaCapacidadeRecurso
Classe para prover os dados do Monitor de acompanhamento do percentual de inatividade do recurso
@type Class
...
@return Nil
/*/
Class ImprodutivaCapacidadeRecurso FROM LongNameClass
...
/*/
Method BuscaDados(oFiltros, cTipo, cSubTipo) Class ImprodutivaCapacidadeRecurso
    Local aNiveis    := {}
    Local cCodRecur  := ""
    ...
    nHrImprod    := PCPMonitorUtils():TransformaMinutosCentesimaisParaTempo(oJsonCalc[dToC(dDatabase)]["I"],2)
    nHrDispRec := Val(StrTran(A680ConvHora(PCPA152Disponibilidade():buscaHorasRecurso(cCodRecur,dDataIni,dDataFin)["totalHoras"],"N","C"),":","."))
    nGaugeVal  := Round((nHrImprod / nHrDispRec) * 100,2)
    ...
    oGauge:SetLabel(STR0212) //"% Inatividade"
    oGauge:SetAppend("")
    oGauge:SetThick(20)
    oGauge:SetMargin(15)
    ...
    oGauge:SetThreshold("0",COR_VERDE_FORTE,0.2)
    oGauge:SetThreshold(aNiveis[1],COR_AMARELO_QUEIMADO,0.2)
    oGauge:SetThreshold(aNiveis[2],COR_VERMELHO_FORTE,0.2)

    If Val(aNiveis[2]) > 0
        oGauge:SetMarker("0",COR_PRETO,6,"line")
    EndIf
    oGauge:SetMarker(aNiveis[2],COR_PRETO,6,"line")
    oGauge:SetMarker(aNiveis[1],COR_PRETO,6,"line")

    If Val(aNiveis[1]) < 100
        oGauge:SetMarker("100",COR_PRETO,6,"line")
    EndIf
    ...
    PCPMonitorUtils():AdicionaTagMonitor(oJsonRet["tags"],@nPosTag,"po-icon-star",STR0106+": "+cValToChar(nHrImprod)+" "+STR0054) //"Horas Improdutivas" //" horas"
...
@return Nil
/*/
Method BuscaDetalhes(oFiltros, nPagina) Class ImprodutivaCapacidadeRecurso
    Local aPeriodos  := {}
    ...
    cQuery := " SELECT "
    cQuery += "  SH6.H6_FILIAL,SH6.H6_PRODUTO,SB1.B1_DESC,SH6.H6_RECURSO,SH6.H6_DTAPONT, "
    cQuery += "  SH6.H6_DATAINI,SH6.H6_HORAINI,SH6.H6_DATAFIN,SH6.H6_HORAFIN,SH6.H6_TEMPO, "
    cQuery += "  SH6.H6_TIPO,SH6.H6_TIPOTEM,SH6.H6_MOTIVO "
    ...
    While (cAlias)->(!Eof())
        ...
        oDados["items"][nPosCon]["H6_MOTIVO"]  := (cAlias)->H6_MOTIVO
        oDados["items"][nPosCon]["CYN_DSSP"]   := AllTrim(Posicione("CYN",1,xFilial("CYN",oFiltros["01_H6_FILIAL"])+(cAlias)->H6_MOTIVO, "CYN_DSSP"))
        (cAlias)->(dbSkip())
...
/*/
Method CargaMonitor() Class ImprodutivaCapacidadeRecurso
    Local aCategr    := {}
    Local aDetalhes  := {}
    Local aTags      := {}
    Local lRet       := .T.
    ...
    If !PCPMonitorCarga():monitorAtualizado("ImprodutivaCapacidadeRecurso")
        ...
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nPosTag,"po-icon-star",STR0106+": 110 "+STR0054) //"Horas Improdutivas" //" horas"
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nPosTag,"po-icon-star-filled", STR0099+": 200 "+STR0054) //"Capacidade" //" horas"
        ...
        oGauge:setLabelStyle("font-weight","bold")
        oGauge:SetThreshold("0",COR_VERDE_FORTE, 0.2)
        oGauge:SetThreshold("40",COR_AMARELO_QUEIMADO, 0.2)
        oGauge:SetThreshold("75",COR_VERMELHO_FORTE, 0.2)
        oGauge:SetMarker("0",COR_PRETO,6,"line")
        oGauge:SetMarker("40",COR_PRETO,6,"line")
        oGauge:SetMarker("75",COR_PRETO,6,"line")
        oGauge:SetMarker("100",COR_PRETO,6,"line")

        oCarga:setaTitulo(STR0213) //"Hrs. Improdutivas X Capacidade"
        oCarga:setaObjetivo(STR0214) //"Acompanhar o percentual de inatividade do recurso com base na relação entre os apontamentos improdutivos e a capacidade calculada para o período."
        oCarga:setaAgrupador("PCP")
        oCarga:setaFuncaoNegocio("ImprodutivaCapacidadeRecurso")
        ...
        oCarga:setaPropriedadeRecurso("02_H6_RECURSO",.F.)
        oCarga:setaPropriedade("03_NIVEIS",STR0205,STR0211,1,30,0,"po-lg-6 po-xl-6 po-md-6 po-sm-12") //"Atenção;Urgente" //"Níveis (% Inatividade)"
        
        oPrmAdc["04_FILTRODATA"]                                := JsonObject():New()
        oPrmAdc["04_FILTRODATA"]["opcoes"]                      := STR0192+":H6_DATAINI;"+STR0154+":H6_DATAFIN;"+STR0193+":H6_DTAPONT" //"Data Início:H6_DATAINI;Data Final:H6_DATAFIN;Data Apontamento:H6_DTAPONT"
...
/*/
Method ValidaPropriedades(oFiltros) Class ImprodutivaCapacidadeRecurso
    Local aNiveis  := {}
    Local aRetorno := {.T.,""}
    ...
    If aRetorno[1] 
        If !oFiltros:HasProperty("03_NIVEIS") .Or. oFiltros["03_NIVEIS"] == Nil .Or. Empty(oFiltros["03_NIVEIS"])
            aRetorno[1] := .F.
            aRetorno[2] := STR0215 //"O campo 'Níveis' deve ser preenchido com 2 valores separados por ponto e vírgula (ex.: '50;75'), definindo os percentuais de inatividade do recurso que indicam Atenção e Urgência, respectivamente."
        Else
			aNiveis := STRTOKARR(Replace(oFiltros["03_NIVEIS"],",","."), ";" )
            If Len(aNiveis) <> 2
                aRetorno[1] := .F.
            ...
            Else
                ...
                Else
                    If Val(aNiveis[1]) >= Val(aNiveis[2])
                        aRetorno[1] := .F.
                        aRetorno[2] := STR0216 //"No campo 'Níveis', o primeiro valor referente ao status 'Atenção' deve ser menor que o segundo, que representa 'Urgência'."
                    EndIf
                EndIf
            EndIf
        EndIf
...
Static Function bscColunas(lExpResult,cDataRef)
    ...
    PCPMonitorUtils():AdicionaColunaTabela(aColunas,@nIndice,"H6_MOTIVO" ,STR0159,"string",lExpResult) //"Motivo"
