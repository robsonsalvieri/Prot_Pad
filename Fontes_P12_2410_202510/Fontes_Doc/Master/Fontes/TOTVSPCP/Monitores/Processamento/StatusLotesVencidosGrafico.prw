...
Method CargaMonitor() Class StatusLotesVencidosGrafico
    ...
    If !PCPMonitorCarga():monitorAtualizado("StatusLotesVencidosGrafico")
        ...
        oSeries["Lotes"] := {{10, 20, 5}, COR_VERMELHO_FORTE }

        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nPosTag,"po-icon-calendar","01/02/2023 - 28/03/2023")
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nPosTag,"po-icon-calculator",PCPMonitorUtils():RetornaDescricaoTipoPeriodo("D"))
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nPosTag,"po-icon-bar-code","TUBO_CANETA")
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nPosTag,"po-icon-star-filled","3 "+STR0178) //"Lotes"
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nPosTag,"po-icon-star-filled","150 UN")

        oCarga:setaTitulo(STR0217) //"Acomp. Lotes Vencidos"
        oCarga:setaObjetivo(STR0228) //"Apresentar um gráfico contendo o número de lotes ou a quantidade vencida de um determinado produto, dentro de um número de períodos específico."
        ...
        oCarga:setaPropriedadeProduto("02_B8_PRODUTO",.F.)
...
Method BuscaDados(oFiltros, cTipo, cSubTipo) Class StatusLotesVencidosGrafico
    ...
    cQuery += "   AND SB8.B8_DTVALID >= '"+dToS(dDataIni)+"' AND SB8.B8_DTVALID < '"+dToS(dDataFin)+"' "
    ...
    oJsonRet["series"][1]["label"]   := IIF(oFiltros["03_TIPOINFO"] == "L",STR0178,cUnMed) //"Lotes"
    ...
    PCPMonitorUtils():AdicionaTagMonitor(oJsonRet["tags"],@nPosTag,"po-icon-bar-code",oFiltros["02_B8_PRODUTO"])
    PCPMonitorUtils():AdicionaTagMonitor(oJsonRet["tags"],@nPosTag,"po-icon-star-filled",cValToChar(nLtsTotal) + IIF(nLtsTotal > 1, " "+STR0178, " "+STR0177))//"Lotes" //"Lote"
...
Method BuscaDetalhes(oFiltros,nPagina) Class StatusLotesVencidosGrafico
    ...
    cQuery += " SELECT "
    cQuery += "     SB8.B8_FILIAL, SB8.B8_LOTECTL, SB8.B8_NUMLOTE, SB8.B8_PRODUTO, SB1.B1_DESC, "
    cQuery += "     SB8.B8_LOCAL, SB8.B8_DTVALID, SB8.B8_QTDORI, SB8.B8_SALDO, SB1.B1_UM "
    cQuery += " FROM "+RetSqlName("SB8")+" SB8 "
    ...
    cQuery += " WHERE SB8.B8_FILIAL = '" + xFilial("SB8",oFiltros["01_B8_FILIAL"]) + "' "
    cQuery += "   AND SB8.B8_PRODUTO = '"+oFiltros["02_B8_PRODUTO"]+"'  "
    cQuery += "   AND SB8.B8_DTVALID >= '"+dToS(dDataIni)+"' AND SB8.B8_DTVALID <"+Iif(dDataIni == dDataFin, "=", "")+" '"+dToS(dDataFin)+"' "
    cQuery += "   AND SB8.B8_SALDO > 0 "
    cQuery += "   AND SB8.D_E_L_E_T_  = ' ' "
...
Static Function montaColun(lExpResult)
    ...
    PCPMonitorUtils():AdicionaColunaTabela(aColunas,@nIndice,"B8_DTVALID",STR0179,"string",.T.) //"Data Validade"
