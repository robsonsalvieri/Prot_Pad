...
/*/{Protheus.doc} OPsAtrasadasMultiProd
Classe para prover os dados do Monitor de Ordens de Produção atrasadas para um ou mais produtos, de acordo com sua previsão de entrega
@type Class
...
@return Nil
/*/
Class OPsAtrasadasMultiProd FROM LongNameClass
...
/*/
Method CargaMonitor() Class OPsAtrasadasMultiProd
    Local aDetalhes   := {}
    Local aTags       := {}
    Local lRet        := .T.
    Local nIndTag     := 0
    ...
    If !PCPMonitorCarga():monitorAtualizado("OPsAtrasadasMultiProd")

        oSeries[STR0308] := {{10, 20, 5}, COR_VERMELHO_FORTE } //"Quantidade"
        ...
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nIndTag,"po-icon-bar-code","PONTEIRA_CANETA")
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nIndTag,"po-icon-star-filled","3 "+STR0203) //"Ordens"
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nIndTag,"po-icon-star-filled",STR0308+": 35") //"Quantidade"
       
        oCarga:setaTitulo(STR0309) //"Acomp. OPs em Atraso Multi Produtos"
        oCarga:setaObjetivo(STR0307) //"Apresenta um gráfico contendo o saldo a produzir das ordens de produção em atraso para vários produtos, com base na data de entrega prevista, para um número de períodos configurado."
        oCarga:setaAgrupador("PCP")
        oCarga:setaFuncaoNegocio("OPsAtrasadasMultiProd")
...
/*/
Method BuscaDados(oFiltros, cTipo, cSubTipo) Class OPsAtrasadasMultiProd
    Local aSaldos    := {}
    ...
    cQuery +=  " GROUP BY SC2.C2_FILIAL, SC2.C2_DATPRF "
    ...
    oJsonRet["series"][1]["label"]   := STR0308//"Quantidade"
    ...
    PCPMonitorUtils():AdicionaTagMonitor(oJsonRet["tags"],@nPosTag,"po-icon-star-filled",STR0308 + ": " + cValToChar(nQtPTotal))
...
@return Nil
/*/
Method BuscaDetalhes(oFiltros,nPagina) Class OPsAtrasadasMultiProd
    Local aPeriodos  := {}
    Local cAliasQry  := GetNextAlias()
...
/*/
Method ValidaPropriedades(oFiltros) Class OPsAtrasadasMultiProd
   Local aRetorno  := {.T.,""}
