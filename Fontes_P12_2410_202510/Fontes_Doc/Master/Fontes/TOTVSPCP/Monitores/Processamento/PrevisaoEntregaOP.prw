...
Method CargaMonitor() Class PrevisaoEntregaOP
    ...
    If !PCPMonitorCarga():monitorAtualizado("PrevisaoEntregaOP")
        ...
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nIndTag,"po-icon-calculator",STR0221 ) //"Quantidade Original")

        oCarga:setaTitulo(STR0218) //"Acomp. Previsão Entrega OPs"
        oCarga:setaObjetivo(STR0222) //"Apresentar um gráfico contendo a quantidade original ou o saldo a produzir de um determinado produto, com base nas ordens de produção com data de entrega prevista para um número de períodos futuros configurado."
        ...
        oPrmAdc["03_TIPOQUANT"]           := JsonObject():New()
        oPrmAdc["03_TIPOQUANT"]["opcoes"] := STR0223+":O;"+STR0224+":S"    //"Original da Ordem de Produção:O;Saldo a produzir:S"
        oCarga:setaPropriedade("03_TIPOQUANT","S",STR0225,4,,,"po-lg-6 po-xl-6 po-md-6 po-sm-12",/*oEstilos*/,/*cIcone*/,oPrmAdc["03_TIPOQUANT"]) //"Tipo Quantidade"
...
Method BuscaDados(oFiltros, cTipo, cSubTipo) Class PrevisaoEntregaOP
    ...
    PCPMonitorUtils():AdicionaSerieGraficoMonitor(oJsonRet["series"],@nIndSerie,COR_AZUL,aSaldos,cUnMed)
    ...
    PCPMonitorUtils():AdicionaTagMonitor(oJsonRet["tags"],@nPosTag,"po-icon-calculator",IIF(oFiltros["03_TIPOQUANT"] == "O",STR0221,STR0224))//"Quantidade Original" //"Saldo a Produzir"
...
Method BuscaDetalhes(oFiltros,nPagina) Class PrevisaoEntregaOP
    ...
    PCPMonitorUtils():AdicionaTagMonitor(oDados["tags"],@nPosTag,"po-icon-calculator",IIF(oFiltros["03_TIPOQUANT"] == "O",STR0221,STR0224))//"Quantidade Original" //"Saldo a Produzir"
