...
Local nQtdEnd    := 0
	...
	Do While lRet .And. Self:nQuant > 0
		...
		Do While lRet .And. QtdComp(nQtdEnd) > QtdComp(0)
			...
			If Self:oOrdServ:ChkDistr() .And. !WmsSkipCQ()
				If !Self:MakeInput()
					lRet := .F.
				EndIf
			EndIf
			...
			If !Self:AssignD12()
				lRet := .F.
			EndIf
			If Self:oMovServic:ChkSepara() .And. Self:oOrdServ:GetOrigem() == "SC9" .And. (Self:oMovServic:ChkMntVol() .Or. Self:oMovServic:ChkConfExp() .Or. Self:oMovServic:ChkDisSep())
...
Local nQtdUniEnd := 0
	BeginSql Alias cAliasQry
		SELECT COUNT(DISTINCT D14.D14_IDUNIT) D14_QTDUNI
		FROM %Table:D14% D14
		WHERE D14.D14_FILIAL = %xFilial:D14%
		AND D14.D14_LOCAL = %Exp:Self:oMovEndDes:GetArmazem()%
		AND D14.D14_ENDER = %Exp:Self:oMovEndDes:GetEnder()%
...
Local cAliasQry := Nil
	...
	BeginSql Alias cAliasQry
		...
		AND D14.D14_LOCAL = %Exp:Self:oMovEndDes:GetArmazem()%
		AND D14.D14_ENDER = %Exp:Self:oMovEndDes:GetEnder()%
		AND D14.D14_IDUNIT <> %Exp:Self:cUniDes% // Não deve considerar o unitizador destino atual
		AND D14.%NotDel%
	EndSql
	...
	If lLoteCtl
		BeginSql Alias cAliasQry
			...
			AND NOT EXISTS (SELECT DISTINCT 1
							FROM %Exp:cTmpSldD14% D142
							WHERE D142.D14_FILIAL = %xFilial:D14%
							AND D142.D14_IDUNIT = %Exp:Self:cUniDes%
							AND D142.D14_PRODUT = D14.D14_PRODUT
							AND D142.D14_LOTECT = D14.D14_LOTECT
							AND D142.%NotDel% )
		EndSql
	Else
		BeginSql Alias cAliasQry
...
Local nQtdPrdUni := 0
	BeginSql Alias cAliasQry
		SELECT SUM(D14.D14_QTDEST + D14.D14_QTDEPR) PRD_QUANT
