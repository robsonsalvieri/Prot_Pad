...
Local cAliasQry  := ""
	...
	If Self:IsMovUnit()
		...
		If Self:oMovServic:ChkRecebi() .And. Self:oMovEndDes:GetTipoEst() == 2
			...
			If (cAliasQry)->(!Eof())
				Do While (cAliasQry)->(!Eof())
					...
					Self:oEstEnder:oProdLote:SetNumLote((cAliasQry)->D0S_NUMLOT) // Sub-Lote do produto principal que deverá ser o mesmo no componentes
		...
		ElseIf Self:oMovServic:ChkTransf() .And. Empty(Self:cUniDes) .And. Self:oMovEndDes:GetTipoEst() == 2

			// Caso seja uma transferênde de Unitizador para um endereço de Picking, devemos complementar a query de seleção do registro da D14
			// para que o calculo de Entrada e Saida Prevista seja feito corretamente.
			cAliasQry := GetNextAlias()
			BeginSql Alias cAliasQry
				SELECT D14.D14_CODUNI,
						D14.D14_PRDORI,
				...
				JOIN %Table:D13% D13
					...
					AND D13.D13_DOC = %Exp:Self:oOrdServ:GetDocto()%
					AND D13.D13_LOCAL = D14.D14_LOCAL
					AND D13.D13_PRODUT = D14_PRODUT
					AND D13.D13_LOTECT = D14_LOTECT
					AND D13.D13_NUMLOT = D14_NUMLOT
					AND D13.D13_NUMSEQ = %Exp:Self:oOrdServ:GetNumSeq()%
					AND D13.D13_IDDCF = %Exp:Self:oOrdServ:GetIdDCF()%		
					AND D13.%NotDel%
				WHERE D14.D14_FILIAL = %xFilial:D14%
			...
			If (cAliasQry)->(!Eof())
				Do While (cAliasQry)->(!Eof())
					...
					Self:oEstEnder:SetQuant((cAliasQry)->D13_QTDEST) //--Grava quantidade movimentada
					// Seta o bloco de código para informações do documento para o Kardex
					Self:oEstEnder:SetBlkDoc({|oMovEstEnd|;
						oMovEstEnd:SetOrigem(Self:oOrdServ:GetOrigem()),;
						oMovEstEnd:SetDocto(Self:oOrdServ:GetDocto()),;
				...
				If !(Self:oMovEndOri:GetArmazem() == Self:oMovEndDes:GetArmazem())
					...
					If lRet .And. Self:oMovPrdLot:HasRastro() .And. !Empty(Self:oMovPrdLot:GetLoteCtl()+Self:oMovPrdLot:GetNumLote())
						If QtdComp(Self:nQtdMovto) >= QtdComp(Self:nQtdOrig)
							//Se a quantidade movimentada for maior que a solicitada para a SD4, empenha apenas a quantidade original da requisição
	...
	Do While (cAliasDCF)->(!Eof())
		...
		If oMntVol:LoadData()
			If oMntVol:GetStatus() == "3"
				lLibSC9 := oMntVol:LiberSC9()
				If ValType(lLibSC9) == 'L' .And. lLibSC9  // Se teve liberacao, verifica se existem itens a serem liberados de outra DCF na mesma montagem de volumes gerada.
					Self:LibOutServ(oMntVol)
				EndIf
			EndIf
		EndIf
		(cAliasDCF)->(dbSkip())
	...
	If Self:oOrdServ:GetOrigem() == "DH1"
		...
		Do While (cAliasDH1)->(!Eof())
			...
			Else
				...
				nPotenc:= 0
				If !Empty(DH1->DH1_LOTECT)
					aLotes := SldPorLote(Self:oMovPrdLot:GetProduto(),cLocOrigem,DH1->DH1_QUANT,ConvUm(Self:oMovPrdLot:GetProduto()),DH1->DH1_LOTECT, DH1->DH1_NUMLOT,Self:oMovEndDes:GetEnder())
					If Len(aLotes) > 0
						nPotenc:= IIF( Len(aLotes[LOTE]) >= 12, aLotes[LOTE][POTENCIA], CriaVar('D3_POTENCI') )
					EndIf
				EndIf
				...
				aAdd(aRotAuto[02],{"D3_POTENCI", nPotenc              				, Nil}) // [15]
		...
		If lRet
			...
			EndSql
			If (cAliasSD3)->(Eof())
				Self:cErro := STR0012 //Registros de movimentos internos (SD3) não foram criados. Movimento será interrompido. Entre em contato com a TI.
				lRet := .F.
			EndIf
				...
				AND ((DCSA.DCS_MNTEXC = '1' 
					AND EXISTS( SELECT DCFB.DCF_CARGA
								...
								AND ((DCFB.DCF_CARGA = %Exp:cCargaVz%
									...
									OR DCFB.DCF_DOCTO <> DCFA.DCF_DOCTO) //Nao permite mais misturar de pedidos diferentes DLOGWMSMSP-17359
								AND DCRB.%NotDel% ))
...
/*----------------------------------------------------------------------
---ValTrocEnd
---Método utilizado ao trocar o endereço destino via coletor, para verificar se a estrutura do 
endereço informado faz parte da sequência de abastecimento do produto.
...
Local cAliasQry := GetNextAlias()
	...
	IF Self:IsMovUnit()
		...
		EndSql
		If (cAliasQry)->(!Eof())
			Self:cErro := STR0010 // "Estrutura fisica do endereço não existe para um ou mais produtos do unitizador."
			lRet := .F. 
		EndIf
		(cAliasQry)->(dbCloseArea())
	...
	Else 
		...
		EndSql
		If (cAliasQry)->(Eof())
		   Self:cErro := STR0011 //"Endereço com estrutura não cadastrada na sequência de abastecimento do produto."
			lRet:= .F.    
		End If 
		(cAliasQry)->(dbCloseArea())
	EndIf
...
/*/{Protheus.doc} LibOutServ
	Libera pedidos e volumes de outros serviços na mesma montagem de volume
	A quantidade em volumes deve ser a mesma necessária
	@author user
	@since 23/05/2024
	@param oMntVol, WMSDTCMontagemVolume, objeto para liberacao
	@see DLOGWMSMSP-16392
	/*/
Method LibOutServ(oMntVol) CLASS WMSBCCMovimentoServico
	Local aArea := GetArea()
	...
	BeginSql Alias cAliasDCF
		Select DCF_SERVIC FROM %Table:DCF% DCF WHERE DCF.%NotDel% AND DCF.DCF_FILIAL = %xFilial:DCF% AND DCF.DCF_ID = %Exp:oMntVol:GetIdDCF()%
	EndSql
	If (cAliasDCF)->(!Eof())

		//Query para buscar todos as DCF/movimentos/itens a liberar dessa montagem
		BeginSql Alias cAliasD0I
			Select D0I_IDDCF FROM %Table:D0I% D0I WHERE D0I.%NotDel% AND D0I.D0I_FILIAL = %xFilial:D0I% AND D0I.D0I_CODMNT = %Exp:oMntVol:GetCodMnt()%
		EndSql
		Do While (cAliasD0I)->(!Eof())
			cAliasVld := GetNextAlias()
			BeginSql Alias cAliasVld
				SELECT DISTINCT DCF.DCF_ID,DC5.DC5_MLTATI,DCF.DCF_CARGA,DCF.DCF_DOCTO,
								DC5.DC5_LIBPED,DCS.DCS_CODMNT,DCR.DCR_IDORI,DCR.DCR_IDDCF,DCF.DCF_SERVIC,DC5.DC5_MNTVOL
				FROM %Table:DCF% DCF
					INNER JOIN %Table:DCR% DCR ON DCR.%NotDel%
						AND DCR.DCR_FILIAL = %xFilial:DCR%
						AND DCR.DCR_IDDCF = DCF.DCF_ID
						AND DCR.DCR_SEQUEN = DCF.DCF_SEQUEN
					INNER JOIN %Table:SC9% SC9 ON SC9.%NotDel%
						AND SC9.C9_FILIAL = %xFilial:SC9%
						AND SC9.C9_CLIENTE = DCF.DCF_CLIFOR
						AND SC9.C9_LOJA = DCF.DCF_LOJA
						AND SC9.C9_PEDIDO = DCF.DCF_DOCTO
						AND SC9.C9_CARGA = DCF.DCF_CARGA
						AND SC9.C9_IDDCF = DCF.DCF_ID
					INNER JOIN %Table:D12% D12 ON D12.%NotDel%
						AND D12.D12_FILIAL = %xFilial:D12%
						AND D12.D12_IDDCF = DCR.DCR_IDORI 
						AND D12.D12_IDMOV = DCR.DCR_IDMOV 
						AND D12.D12_IDOPER = DCR.DCR_IDOPER
					INNER JOIN %Table:DCS% DCS ON DCS.%NotDel%
						AND DCS.DCS_FILIAL = %xFilial:DCS%
						AND DCS.DCS_CARGA = D12.D12_CARGA
						AND DCS.DCS_PEDIDO = DCF.DCF_DOCTO
					INNER JOIN %Table:DCT% DCT ON DCT.%NotDel%
						AND DCT.DCT_FILIAL = %xFilial:DCT%
						AND DCT.DCT_CODMNT = DCS.DCS_CODMNT 
						AND DCT.DCT_CARGA = DCS.DCS_CARGA 
						AND DCT.DCT_PEDIDO = DCS.DCS_PEDIDO 
					INNER JOIN %Table:D0I% D0I ON D0I.%NotDel%
						AND D0I.D0I_FILIAL = %xFilial:D0I%
						AND D0I.D0I_CODMNT = DCS.DCS_CODMNT
					INNER JOIN %Table:DC5% DC5 ON DC5.%NotDel%
						AND DC5.DC5_FILIAL = %xFilial:DC5%
						AND DC5.DC5_SERVIC = D12.D12_SERVIC
						AND DC5.DC5_TAREFA = D12.D12_TAREFA
						AND DC5.DC5_OPERAC IN ('3','4')
					INNER JOIN %Table:DCV% DCV ON DCV.%NotDel%
				...
				WHERE DCF.%NotDel%
					...
					AND DCF.DCF_ID = %Exp:(cAliasD0I)->D0I_IDDCF%
					AND DCT.DCT_CODMNT = %Exp:oMntVol:GetCodMnt()%
					AND SC9.C9_CARGA = %Exp:oMntVol:GetCarga()%
					AND SC9.C9_PEDIDO = %Exp:oMntVol:GetPedido()%
					AND DCS.DCS_STATUS = '3'//--Finalizado
					AND SC9.C9_BLWMS <> '05' //--Não liberado
					AND D12.D12_ATUEST = '1'//Atualiza estoque
					AND D12.D12_STATUS = '1'//Executado
			EndSql
			If (cAliasVld)->(!Eof())
				If (cAliasVld)->DC5_LIBPED = "6" .And. (cAliasVld)->DC5_MNTVOL = "1"
					If (cAliasDCF)->DCF_SERVIC = (cAliasVld)->DCF_SERVIC
						cAliasDCR := GetNextAlias()		//Se é o mesmo serviço, só libera se na ultima execução envolvendo esta montagem + serviço não conseguiu (DCS_STATUS = 2).				 
						BeginSql Alias cAliasDCR		//Pois se esta DCF está nessa execucao, deve deixá-la liberar no processo normal.
							Select SUM(DCR.DCR_QUANT) AS DCR_QUANT FROM %Table:DCR% DCR
								INNER JOIN %Table:D12% D12 ON (D12.%NotDel% 
									AND D12.D12_FILIAL = %xFilial:D12% 
									AND D12.D12_IDDCF = DCR.DCR_IDORI 
									AND D12.D12_IDMOV = DCR.DCR_IDMOV
									AND D12.D12_IDOPER = DCR.DCR_IDOPER
									AND D12.D12_SERVIC = %Exp:(cAliasDCF)->DCF_SERVIC%)
								WHERE DCR.%NotDel% 
									AND DCR.DCR_FILIAL = %xFilial:DCR% 
									AND DCR.DCR_IDORI = %Exp:(cAliasVld)->DCR_IDORI%
									AND D12.D12_ATUEST = '1'
									AND D12.D12_STATUS = '1' //GROUP BY DCR.DCR_IDORI,D12.D12_QTDLID
						EndSql
						If (cAliasDCR)->(!Eof()) 
							cAliasD12 := GetNextAlias()	
							BeginSql Alias cAliasD12		//Pois se esta DCF está nessa execucao, deve deixá-la liberar no processo normal.
								Select SUM(D12.D12_QTDLID) AS D12_QTDLID FROM %Table:D12% D12
									WHERE D12.%NotDel% 
										AND D12.D12_FILIAL = %xFilial:D12% 
										AND D12.D12_IDDCF = %Exp:(cAliasVld)->DCR_IDORI%
										AND D12.D12_ATUEST = '1'
										AND D12.D12_STATUS = '1' //GROUP BY DCR.DCR_IDORI,D12.D12_QTDLID
										AND D12.D12_SERVIC = %Exp:(cAliasDCF)->DCF_SERVIC%
							EndSql						
						 	If (cAliasD12)->(!Eof()) .And. (cAliasDCR)->DCR_QUANT = (cAliasD12)->D12_QTDLID
								oMntVol2 := WMSDTCMontagemVolume():New()
								oMntVol2:SetCarga(oMntVol:GetCarga())
								oMntVol2:SetPedido(oMntVol:GetPedido())
								oMntVol2:SetIdDCF((cAliasD0I)->D0I_IDDCF)
								oMntVol2:SetMultAtiv((cAliasVld)->DC5_MLTATI) 
								oMntVol2:SetCodMnt(oMntVol:GetCodMnt())
								If oMntVol2:LoadData()
									oMntVol2:LiberSC9()
								EndIf
								oMntVol2 := Nil
							EndIf
						...
						EndIf
						(cAliasDCR)->(dbCloseArea())
					ElseIf (cAliasDCF)->DCF_SERVIC != (cAliasVld)->DCF_SERVIC  //Se na query encontrou registro com serviço diferente, deve ser liberado.
						oMntVol2 := WMSDTCMontagemVolume():New()
						oMntVol2:SetCarga(oMntVol:GetCarga())
						oMntVol2:SetPedido(oMntVol:GetPedido())
						oMntVol2:SetIdDCF((cAliasVld)->DCF_ID)
						oMntVol2:SetMultAtiv((cAliasVld)->DC5_MLTATI) 
						oMntVol2:SetCodMnt(oMntVol:GetCodMnt())
						If oMntVol2:LoadData()
							oMntVol2:LiberSC9()
