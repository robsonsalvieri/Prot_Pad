...
Local lAbandona   := .F.
	...
	cCliFor    := oMovimento:oOrdServ:GetCliFor()
	cLjCliFor  := oMovimento:oOrdServ:GetLoja()
...
Local nQtdTot   := 0
	...
	Do While !lEncerra .And. !lAbandona
		...
		If !lEncerra .And. !lAbandona
			...
			If ((cProduto <> cPrdAnt) .Or. lWMSConf )
				...
				WmsValUM(Nil,;     // Quantidade movimento
						...
						lQtdBar)   // Indica que quantidade já preenchida
				
				DbSelectArea('DC5') //incluído para evitar erro na função Tabela por falta de alias posicionado
				// Monta tela produto
		...
		EndIf
	EndDo
	If ExistBlock("WV074TEF")//Para que seja possível remover a tecla de função criada no Ponto de Entrada WV074TEP.
...
Local nQtdLid    := 0
	...
	Begin Transaction
		...
		Do While lRet .And. (cAliasQry)->(!Eof()) .And. QtdComp(nQtConf) > 0
			...
			nQtConf -= nQtdLid
			// PE para permitir executar ações adicionais na gravação da conferência por operador
			If lWV074GCO
...
Local cWhere     := ""
	...
	Begin Transaction
		...
		BeginSql Alias cAliasQry
			...
			AND D12.D12_LOJA = %Exp:cLjCliFor%
			AND D12.D12_ORDTAR = %Exp:cOrdTar%
			AND D12.D12_STATUS IN ('2','3','4')
			AND (D12.D12_RECHUM = %Exp:__cUserID%
				OR D12.D12_RECHUM = %Exp:cRecHVazio% )
	...
	If lRet .And. !lDiverge
		...
		Else
			...
			If ExistBlock("WMS074CF") // Executado após a finalização da atividade de conferência
...
Valida se existe o documento e série informados
...
"Essa função foi criada para o uso "EXCLUSIVO" do ponto de entrada WV074TEP, 
para a inclusão de uma tecla de atalho que permita a alteração do endereço de conferência sem a
necessidade de encerrar e iniciar nova a conferência, abrir telas de endereço e de documento."
