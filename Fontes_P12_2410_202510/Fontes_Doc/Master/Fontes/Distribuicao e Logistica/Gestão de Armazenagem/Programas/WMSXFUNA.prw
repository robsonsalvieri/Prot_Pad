...
#DEFINE WMSXFUNA09 "WMSXFUNA09"
#DEFINE WMSXFUNA10 "WMSXFUNA10"
#DEFINE WMSXFUNA11 "WMSXFUNA11"
#DEFINE WMSXFUNA12 "WMSXFUNA12"
#DEFINE WMSXFUNA13 "WMSXFUNA13"
#DEFINE WMSXFUNA14 "WMSXFUNA14"
#DEFINE WMSXFUNA15 "WMSXFUNA15"
#DEFINE WMSXFUNA16 "WMSXFUNA16"
...
Local aTamSX3  := {TamSx3('BF_QUANT'), TamSx3('BF_EMPENHO'), TamSx3('BF_QEMPPRE'),;
                    TamSx3('BF_QTSEGUM'), TamSx3('BF_EMPEN2'), TamSx3('BF_QEPRE2')}
Local aAreaAnt := GetArea()
...
Default cLoja     := ''
	...
	If cAcao == '1' .Or. cAcao == '2'
		...
		cQuery := " BF_QUANT, BF_EMPENHO, BF_QEMPPRE, BF_QTSEGUM, BF_EMPEN2, BF_QEPRE2"
		...
		TcSetField(cAliasSBF,'BF_QUANT',  'N',aTamSX3[1][1],aTamSX3[1][2])
		TcSetField(cAliasSBF,'BF_EMPENHO','N',aTamSX3[2][1],aTamSX3[2][2])
		TcSetField(cAliasSBF,'BF_QEMPPRE','N',aTamSX3[3][1],aTamSX3[3][2])
		TcSetField(cAliasSBF,'BF_QTSEGUM','N',aTamSX3[4][1],aTamSX3[4][2])
		TcSetField(cAliasSBF,'BF_EMPEN2', 'N',aTamSX3[5][1],aTamSX3[5][2])
		TcSetField(cAliasSBF,'BF_QEPRE2', 'N',aTamSX3[6][1],aTamSX3[6][2])
...
//------------------------------------------------------------------------------
Function WmsExeServ(lIsJob, lEncExe, lExibe, lExecRegra)
Local lRet      := .T.
Local lWmsNew   := SuperGetMV("MV_WMSNEW",.F.,.F.)
...
Default lEncExe    := .F.
Default lExibe     := .T.
Default lExecRegra := .T.
	If !lWmsNew
		If !Empty(__aLibDCF)
			If lIsJob .And. !lEncExe
				StartJob( "WmsExeJob2", GetEnvServer(), .F., lIsJob, lEncExe, cEmpAnt, cFilAnt, __cUserID, __aLibDCF)
			Else
				lRet := WmsExeJob2(lIsJob, lEncExe, cEmpAnt, cFilAnt, __cUserID, __aLibDCF)
			EndIf
			__aLibDCF := {}
		EndIf
	Else
		If __oOrdServ != Nil .And. !Empty(__oOrdServ:aLibDCF)
			// Ponto de Entrada WMSXFBXS - Antes Execucao
			If ExistBlock( "WMSXFBXS" )
				lRetPE := ExecBlock( "WMSXFBXS", .F., .F., {__oOrdServ:GetRecno()} )
				If ValType(lRetPE) == 'L'
					LRet := lRetPE
				EndIf
			EndIf
			If lRet
				If lIsJob .And. !lEncExe
					StartJob( "WmsExeJob1", GetEnvServer(), .F., lIsJob, lEncExe, cEmpAnt, cFilAnt, __cUserID, __oOrdServ:aLibDCF, lExAuSpCp, .F., lExecRegra)
				Else
					lRet := WmsExeJob1(lIsJob, lEncExe, cEmpAnt, cFilAnt, __cUserID, __oOrdServ:aLibDCF, lExAuSpCp, lExibe, lExecRegra)
				EndIf
				// Ponto de Entrada WMSXFEXS - Apos Execucao automática do serviço WMS
				If ExistBlock("WMSXFEXS")
					ExecBlock("WMSXFEXS", .F., .F., {__oOrdServ:aLibDCF})
				EndIf
				__oOrdServ:aLibDCF := {}
			Endif
		EndIf
...
Function WmsExeJob1(lIsJob, lEncExe, cEmpAmb, cFilAmb, cUsuario, aLibDCF, lExAuSpCp, lExibe, lExecRegra )
Local lRet       := .T.
Local lContinua  := .T.
...
Local cListIdDcf := ""
Local nX         := 0
Default lExecRegra := .T. //Indica se deve processar regra de convocação
	If lIsJob
		// Seta job para nao consumir licensas
		RpcsetType(3)
		// Seta job para empresa filial desejadas
		RpcSetEnv(cEmpAmb,cFilAmb,cUsuario,/*cSenha*/,"WMS","WmsExeJob1")
	EndIf
	...
	If !Empty(oOrdSerExe:aWmsAviso) .And. lExAuSpCp .And. !Empty(cListIdDcf)
		...
		oOrdSerRev:RevPedAut("% " + FormatIn(cListIdDcf,",") + " %")
	EndIf
...
Function WmsExeJob2(lIsJob, lEncExe, cEmpAmb, cFilAmb, cUsuario, aLibDCF)
Local lRet       := .T.
Local nX         := 0
	If lIsJob
		// Seta job para nao consumir licensas
		RpcsetType(3)
		// Seta job para empresa filial desejadas
		RpcSetEnv(cEmpAmb,cFilAmb,cUsuario,/*cSenha*/,"WMS","WmsExeJob2")
	EndIf
...
Local cAliasQry := ""
	...
	Else
		...
		BeginSql Alias cAliasQry
			...
			AND D14.D14_ESTFIS = %Exp:cEstFis%
...
Local aIdDCF      := {}
	...
	cQuery := WmsQrQtOri(cCarga, cPedido, cProduto, cLote, cSubLote, cTarefa)
	cQuery := ChangeQuery(cQuery)
	cAliasQry := GetNextAlias()
	DbUseArea(.T.,'TOPCONN',TcGenQry(,,cQuery),cAliasQry,.F.,.T.)
	If !(cAliasQry)->(Eof())
...
Local cTimeUlt := ''
Local cTimeAux := ''
	//- caso o parametro seja o MV_DOCSEQ, o mesmo irá usar a função padrão 
	//- ProxNum, pois ela é a responsável por efetuar a busca correta e gerir 
	//- o controle de acesso a essa sequencia, evitando possiveis duplicidades.
	If AllTrim(cParametro) == 'MV_DOCSEQ'
		cCodAnt := ProxNum()
	Else 
		While !LockByName("WMSPROXSEQ", .T., .F.)
			...
			If nC == 60
				...
				/*Caso não consiga travar em 60 interações (3 segundos), efetuar o UnLockByName de maneira
				forçada se ultrapassar 30s do último Lock, pois pode indicar que um processo anterior
				fez LockByName mas caiu antes do UnLockByName*/
				cTimeUlt := GetGlbValue('WMSPROXSEQ')
				If !Empty(cTimeUlt)
					cTimeAux := Time()
					If Substr(cTimeUlt,1,2) == '23' .And.  Substr(cTimeAux,1,2) == '00'
						cTimeAux := '24' + SubStr(cTimeAux,3)
					EndIf
					cTimeAux := ElapTime(cTimeUlt, cTimeAux)
					If Val(SubStr(cTimeAux,1,2)) > 0 .Or. Val(SubStr(cTimeAux,4,2)) > 0 .Or. Val(Substr(cTimeAux,7,2)) >= 10
...
Local lRetPE    := .T.
	...
	//-- Valida Servico WMS de transferência quando informado
	If !Empty(cServico) .AND. !FWIsInCallStack("WMSA430") .AND. !FWIsInCallStack("WMSMOVEST") 
...
Private lMsErroAuto := .F.
	...
	If !lWmsNew
		// Ponto de entrada para manipulação da serie do produto
		If ExistBlock("WMSSERORI")
			cSerOrig := ExecBlock( "WMSSERORI", .F., .F., {cProduto, cLocOrig, cEndOrig, cLoteCtl, cNumLote})
			cSerOrig := IIF( ValType(cSerOrig) <> "C" .Or. Empty(cSerOrig), "", cSerOrig )
		EndIf
	EndIf
	...
	//Necessário validar se está na versão alterada do MATA261 caso contrário continua executando o mata260
	If GetApoInfo("MATA261.PRX")[4] < dDtUsa261
		...
		If lRet
			...
			Begin Transaction
				...
				If lRet .AND. aParam150[17] == "SD3" .And. !Empty(cNumSeq)
	...
	Else 
		...
		Begin Transaction
			...
			aAdd(aRotAuto[02],{"D3_DESCRI" , cProduto, Nil}) // [02] Descrição origem 
			aAdd(aRotAuto[02],{"D3_UM"     , cWmsUMI, Nil}) // [03] Unidade de medica origem
			aAdd(aRotAuto[02],{"D3_LOCAL"  , cLocOrig, Nil}) // [04] Armazém origem
			aAdd(aRotAuto[02],{"D3_LOCALIZ", cEndOrig, Nil}) // [05] Endereço origem
			...
			aAdd(aRotAuto[02],{"D3_LOCAL"  , cLocDest, Nil}) // [09] Armazém destino
			aAdd(aRotAuto[02],{"D3_LOCALIZ", cEndDest, Nil}) // [10] Endereço destino DH1->DH1_LOCALI
			aAdd(aRotAuto[02],{"D3_NUMSERI", cSerOrig, Nil}) // [11] Numero de série
			...
			aAdd(aRotAuto[02],{"D3_QUANT"  , nQtdMovto, Nil}) // [16] Quantidade
			aAdd(aRotAuto[02],{"D3_QTSEGUM", nQuant2UM, Nil}) // [17]
			aAdd(aRotAuto[02],{"D3_ESTORNO", lEstorna, Nil}) // [18]
			aAdd(aRotAuto[02],{"D3_NUMSEQ" , cNumSeq, Nil}) // [19] Numero sequencial
			...
			aAdd(aRotAuto[02],{"D3_SERVIC" , cServico, Nil}) // [22] Servico
			aAdd(aRotAuto[02],{"D3_ITEMGRD", CriaVar("D3_ITEMGRD"), Nil}) // [23]
			
			aAdd(aWmsCMov, {"CPROGRAMA", "DLGXFUN"})
			aAdd(aWmsCMov, {"CESTORIG", cEstOrig})     //-- Estrutura Fisica Padrao
			aAdd(aWmsCMov, {"CSERVICO", cServico})     //-- Servico
			aAdd(aWmsCMov, {"CTAREFA", cTarefa})       //-- Tarefa
			aAdd(aWmsCMov, {"CATIVIDADE", cAtividade}) //-- Atividade
			aAdd(aWmsCMov, {"CANOMALIA", ""})          //-- Anomalia
			aAdd(aWmsCMov, {"CESTDEST", cEstDest})     //-- Estrutura Fisica Destino
			aAdd(aWmsCMov, {"CENDDEST", cEndDest})     //-- Endereço Destino
			aAdd(aWmsCMov, {"CHRINICIO", cHrInicio})   //-- Hora Inicio
			aAdd(aWmsCMov, {"CATUESTOQUE", "S"})       //-- Atualiza Estoque
			aAdd(aWmsCMov, {"CCARGA", cCarga})         //-- Numero da Carga
			aAdd(aWmsCMov, {"CUNITIZA", cUnitiza})     //-- Numero do Unitizador
			aAdd(aWmsCMov, {"CORDTAR", cOrdTar})       //-- Ordem da Tarefa
			aAdd(aWmsCMov, {"CORDATIV", "ZZ"})         //-- Ordem da Atividade
			aAdd(aWmsCMov, {"CRHUMANO", cRHumano})     //-- Recurso Humano
			aAdd(aWmsCMov, {"CRFISICO", cRFisico})     //-- Recurso Fisico
			...
			MSExecAuto({|x,y,Z| MATA261(x,y,Z)}, aRotAuto, If(lEstorna,6,3),aWmsCMov) //Estorno ou Inclusão
			If lMsErroAuto
				If !IsTelNet()
					MostraErro()
				Else
...
Local cAliasSBE := Nil
	If lConfirm .And. !Empty(cArmazem) .And. !Empty(cEndereco)
		...
		If (cAliasSBE)->(Eof())
			...
			lRet := .F.
		Else
			If cTipServ != '3' .And. DLTipoEnd((cAliasSBE)->BE_ESTFIS) != 5
...
/*/
//----------------------------------------------------------
Static aCacheWMS := {} // Variável estática para controle de cache. {cFilial, cProduto, lIntWMS}
Function IntWMS(cProduto)
Local lIntWMS   := SuperGetMV('MV_INTWMS',.F.,.F.)
Local lWmsNew   := SuperGetMV("MV_WMSNEW",.F.,.F.)
Local cDadosProd :=SuperGetMV("MV_ARQPROD",.F.,"SB1") //Por questão de performance lê o parâmetro para somente  fazer o dbSeek quando cliente usar tabela SBZ
Local aAreaAnt  := {}
Local cFilialPrd:= ''
Local cAliasQry := Nil
Default cProduto := ""
	If lIntWMS .And. !Empty(cProduto)
		...
		//Só faz a busca se o cliente estiver configurado para usar SBZ (produto por filial)
		If cDadosProd == 'SBZ' .And. !RetArqProd(cProduto)
			cFilialPrd := xFilial("SBZ")
		Else
			cFilialPrd := xFilial("SB5")
		EndIf

		If (nPos := aScan(aCacheWMS,{|x| x[1] + x[2] == cFilialPrd + cProduto})) > 0
			lIntWMS := aCacheWMS[nPos][3]
		Else
			...
			If lIntWMS .And. lWmsNew
				...
				cAliasQry := GetNextAlias()
				BeginSql Alias cAliasQry
					SELECT SB5.R_E_C_N_O_ RECNOSB5
					FROM %Table:SB5% SB5
				...
				RestArea(aAreaAnt)
			EndIf
			aAdd(aCacheWMS,{cFilialPrd,cProduto,lIntWMS})
		EndIf
	EndIf
Return lIntWMS
//----------------------------------------------------------
...
Local cAliasDC8 := GetNextAlias()
	...
	BeginSql Alias cAliasDC8
		...
		AND DC8.%NotDel%
	EndSql
	Do While (cAliasDC8)->(!Eof())
		If Empty(cRet)
			cRet := (cAliasDC8)->DC8_CODEST
		Else
			cRet += "|"+ (cAliasDC8)->DC8_CODEST
		EndIf
		(cAliasDC8)->(dbSkip())
...
Local cAliasD11 := Nil
	...
	Do While (cAliasD11)->(!Eof())
		...
		If IntWMS(cPrdCmp) .And. (cAliasD11)->B1_MSBLQL == '1'
			If !Empty(cErro)
				cErro += CHR(13)+CHR(10)
			Else
...
Local nTamOrdem := TamSX3("DC5_ORDEM")[1]
	...
	BeginSql Alias cAliasDC5
		SELECT MIN(DC5.DC5_ORDEM) DC5_ORDEM
		FROM %Table:DC5% DC5
		WHERE DC5.DC5_FILIAL = %xFilial:DC5%
		AND DC5.DC5_SERVIC = %Exp:DC5->DC5_SERVIC%
		AND DC5.%NotDel%
	EndSql
	If (cAliasDC5)->(!Eof())
...
Function WmsConsDC6()
Local aAreaAnt  := GetArea()
Local cAliasDC6 := GetNextAlias()
Local cOrdem    := ""
	BeginSql Alias cAliasDC6
		SELECT MIN(DC6.DC6_ORDEM) DC6_ORDEM
		FROM %Table:DC6% DC6
		WHERE DC6.DC6_FILIAL = %xFilial:DC6%
		AND DC6.DC6_TAREFA = %Exp:DC6->DC6_TAREFA%
		AND DC6.%NotDel%
	EndSql
	If (cAliasDC6)->(!Eof())
...
Default cProduto := ""
	...
	If lWmsNew .And. IntWms(IIf(!Empty(cProduto),cProduto,""))
		...
		EndIf
		If !Empty(cProduto)
			cWhere += " AND D14.D14_PRDORI = '"+cProduto+"'"
		EndIf
		cWhere += "%"
		cWhereAux := "%"
		If !Empty(cArmazem)
			cWhereAux +=  " AND SBF.BF_LOCAL = '"+cArmazem+"'"
		EndIf
		If !Empty(cProduto)
			cWhereAux +=  " AND SBF.BF_PRODUTO = '"+cProduto+"'"
		EndIf
		cWhereAux += "%"
		cAliasQry := GetNextAlias()
		BeginSql Alias cAliasQry
			SELECT DISTINCT D14.D14_PRODUT PRODUTO
			FROM %Table:D14% D14"
			WHERE D14.D14_FILIAL = %xFilial:D14%
			AND D14.%NotDel%
			%Exp:cWhere%
			UNION ALL
			SELECT DISTINCT SBF.BF_PRODUTO PRODUTO
			FROM %Table:SBF% SBF
...
Local cServico  := ""
	If IntWMS(SD7->D7_PRODUTO)
		...
		BeginSql Alias cAliasSB5
			SELECT SB5.B5_SERVENT
			FROM %Table:SB5% SB5
			WHERE SB5.B5_FILIAL = %xFilial:SB5%
			AND SB5.B5_COD = %Exp:SD7->D7_PRODUTO%
			AND SB5.%NotDel%
		EndSql
		If (cAliasSB5)->(!Eof())
...
Local cRetPe := ""
	...
	//Ponto de entrada para alteração dos caracteres para validação.
	If	ExistBlock("WMSALTVC")
		cRetPE:=ExecBlock("WMSALTVC",.F.,.F.,{cCaracter})
		If Valtype(cRetPe) == "C"
			cCaracter := cRetPE
		EndIf
	EndIf
...
Local cMvCq	   := GetMvNNR('MV_CQ','98')
	...
	If __cLastArmz != cArmazem .Or. __lArmzUnit == Nil
		If WmsX312118("NNR","NNR_AMZUNI",.T.) // Verifica no SX3
			__lArmzUnit := (cMvCq != cArmazem) .And. (Posicione("NNR",1,xFilial("NNR")+cArmazem,"NNR_AMZUNI") == "1")
		Else
			__lArmzUnit := .F.
		EndIf
...
Local nX := 0
	...
	If nFilPed > 0 .and. nNumDoc > 0 .and. nItem > 0 //Verifica se campo esta marcado para aapresenta na tela
...
Local cDocInv	:= SB7->B7_DOC 
	...
	If !(cArmazOri+cEnderOri == cArmazem+cEndereco)
		...
		BeginSql Alias cAliasSB7
			SELECT B7_QUANT FROM(
			SELECT SUM(SB7.B7_QUANT) AS B7_QUANT
			FROM %Table:SB7% SB7
			WHERE SB7.B7_FILIAL = %xFilial:SB7%
			AND SB7.B7_DOC = %Exp:cDocInv%
			AND SB7.B7_IDUNIT = %Exp:cIdUnit%
			AND SB7.B7_LOCAL = %Exp:cArmazOri%
			AND SB7.B7_LOCALIZ = %Exp:cEnderOri%
			AND SB7.%NotDel% )TMP
			WHERE B7_QUANT IS NOT NULL
		EndSql
		
		If (cAliasSB7)->( !Eof() )
			If (cAliasSB7)->B7_QUANT == 0
				lRet := .T.
			EndIf
		EndIf	
...
//----------------------------------------------------------
/*/{Protheus.doc} GetWmsSX3
Função criada com o objetivo de otimizar a leitura de propriedades dos campos SX3.
Objetivo 1: evitar o uso de TamSX3, onde há um dbGoto que causa lentidão em grandes processos;
Objetivo 2: evitar o uso de GetSX3Cache, que armazena o conteúdo de SX3 em array. Como esse array
pode assumir um tamanho muito grande, já que armazena informações de todos os campos lidos, será
tratado em um array específico do WMS (aWmsSX3) de forma a conter apenas campos usados pelo WMS.

O retorno poderá ser variável, com o objetivo de otimizar a execução.
Se nIndice não for informado, retornará um array contendo tamanho, decimal e tipo;
Se nIndice for informado, retornará somente o conteúdo baseado no índice.

@param cCampo Nome do campo
@param nIndice: indica o índice da informação que será retornada.
	1-X3_TAMANHO
	2-X3_DECIMAL
	3-X3_TIPO
@return undefined
	Se nIndice não for informado, retorna array contendo tamanho, decimal e tipo do campo
	Se nIndice for informado, retornará o conteúdo especificado por nIndice
		1-Numérico (tamanho do campo);
		2-Numérico (número de decimais);
		3-Caracter (tipo do campo);
...
/*/
//----------------------------------------------------------
Static aWmsSX3 := {} // Variável estática para controle de cache. {cCampo, nTam, nDec, cTipo}
Function GetWmsSX3(cCampo, nIndice)
Local aAux := {}
Local uRet := nil
Local nPos := 0
Default nIndice := 0

	If (nPos := aScan(aWmsSX3, {|x|x[1] == cCampo})) > 0
		aAux := aWmsSX3[nPos]
	Else
		//Posicionando na SX3 para que a função GetSx3Cache funcioane na validação de tabela Sx3 aberta.
		SX3->(RecNo())
		aAux := Aadd(aWmsSX3, {cCampo, ;
...
//--------------------------------------------------------------------
/*/{Protheus.doc} WMSValTPOD
Verifica se produto origem e destino integram com WMS e exibe mensagem em caso negativo.

@author Wander Horongoso
@since  16/09/2020
@param cPrdOri, character, Código do produto origem
@param cPrdDes, character, Código do produto destino
@return: lógico, se verdadeiro os produtos integram ou não com WMS.
Se falso um dos produtos integra com WMS e o outro não.
/*/
//--------------------------------------------------------------------
Function WMSValTPOD(cPrdOri, cPrdDes)
Local lRet := .T.
Local lPrdOriWMS := .F.
Local lPrdDesWMS := .F.

	If SuperGetMV("MV_WMSTPOD",.F.,.F.) == .F.
		lPrdOriWms := IntWMS(cPrdOri)
		lPrdDesWMS := IntWMS(cPrdDes)
		If lPrdOriWms .And. !lPrdDesWms
			cMensagem := STR0114 //'Para transferência ambos os produtos devem ter o mesmo controle.'
			cMensagem += CRLF+WmsFmtMsg(STR0115, {{"[VAR01]", AllTrim(cPrdOri)}}) //'Produto origem ([VAR01]) controlado pelo WMS.'
			cMensagem += CRLF+WmsFmtMsg(STR0116, {{"[VAR01]", AllTrim(cPrdDes)}}) //'Produto destino ([VAR01]) não controlado pelo WMS.'
			WmsMessage(cMensagem,,1)
			lRet := .F.
		ElseIf !lPrdOriWms .And. lPrdDesWms
			cMensagem := STR0114 //'Para transferência ambos os produtos devem ter o mesmo controle.'
			cMensagem += CRLF+WmsFmtMsg(STR0117, {{"[VAR01]", AllTrim(cPrdOri)}})
			cMensagem += CRLF+WmsFmtMsg(STR0118, {{"[VAR01]", AllTrim(cPrdDes)}})
			WmsMessage(cMensagem,,1)
			lRet := .F.
		EndIf
	EndIf
...
//--------------------------------------------------------------------
/*/{Protheus.doc} XBSC7WMS
Cria consulta especifica

@author Carlos Augusto
@since  05/04/2021
@return: lógico, se verdadeiro, apresenta a tela.
/*/
//--------------------------------------------------------------------
Function XBSC7WMS()
	Local lRet      := .T.
	...
	AAdd( aTitle, RetTitle( 'C7_FILIAL' ) )
	AAdd( aTitle, RetTitle( 'C7_PRODUTO' ) )
	AAdd( aTitle, RetTitle( 'C7_DESCRI' ) )
	AAdd( aTitle, RetTitle( 'C7_NUM' ) )
	AAdd( aTitle, RetTitle( 'C7_ITEM' ) )
	AAdd( aTitle, RetTitle( 'C7_SEQUEN' ) )


	cQuery := "SELECT C7_FILIAL,C7_PRODUTO,C7_NUM,C7_ITEM,C7_SEQUEN,C7_DESCRI"
	cQuery += " FROM " + RetSQLTab('SC7')
	cQuery += " WHERE 1=1 "

	If Type("aCols") == "A"
		IF FWISINCALLSTACK("MATA410")  .And. C5_TIPO == 'D'
			cQuery += " AND C7_FORNECE = '"+ C5_CLIENTE + "'"
		EndIf
		cQuery += " AND C7_PRODUTO = '"+ aCols[n][2] + "'"
	Else
		oModel := FWModelActive()
		cQuery += " AND C7_PRODUTO = '" + oModel:GetValue("A324D0M","D0M_PRODUT") + "'"
	EndIf
	...
	If !(cAliasQry)->( Eof() )
		While !(cAliasQry)->( Eof() )
			AAdd( aItens, { (cAliasQry)->C7_FILIAL,;
							(cAliasQry)->C7_PRODUTO,;
							(cAliasQry)->C7_DESCRI,;
							(cAliasQry)->C7_NUM,;
							(cAliasQry)->C7_ITEM,;
							(cAliasQry)->C7_SEQUEN} )
	...
	If lRet
		nSelec := TmsF3Array( aTitle, aItens, STR0121, .F. ) //'Itens Ped. Compra'
		If nSelec <> 0
			SC7->(DbSetOrder(1))
			If !SC7->(DbSeek(FwXFilial("SC7") + aItens[nSelec,4] + aItens[nSelec,5] + aItens[nSelec,6]))
				lRet := .F.
			EndIf
		Else
			lRet := .F.
		EndIf
	Else
		Help( ,, STR0119,,STR0120, 1, 0 ) //"Atenção. Nenhum pedido de compra encontrado com os parâmetros de busca."
	EndIf
Return( lRet )

//--------------------------------------------------------------------
/*/{Protheus.doc} WMSVlSdInve
Valida se as quantidaes totais do produto estão batando com as tabelas de estoque.
Observação: Esta validação não contempla produto componente.
@author Roselaine Adriano
@since  15/07/2021
@param cLocal, character, Código do local de inventário
@param cEnd, character, Código do Endereco de inventário
@param cProd, character, Código do produto inventário
@return: lógico, se verdadeiro não existe diferença entre Sb8, D14 e SB2.
Se falso existe divergência de saldo.
/*/
//--------------------------------------------------------------------
Function WMSVldInve(cLocal, cEnd, cProduto,cTipInv,lExibMsg)
Local lRet := .T.
Local cAliasQry := Nil
Local cAliasWms := GetArea()
Local lSLdDif   := .F.
Local lMsgprlt  := ""

	If cTipInv == "2" .And. WmsArmUnit(cLocal) .And. WmsEndDoca(cLocal,cEnd)
		WmsMessage(STR0122,WMSXFUNA09,1,lExibMsg) //Não é permitido inventariar endereços do tipo 'DOCA' em armazéns unitizados controlados pelo WMS.
		lRet := .F.
	EndIf
	//fazer a busca pra ver se acha pelo menos um caso diferente para o produto. Se produto passado como parâmetro, ele será utilizado. Se informado endereço, lê todos os endereços da D14
	If lRet
		If cTipInv =="2" .AND. !Empty(cEnd)
			cAliasQry := GetNextAlias()
			BeginSql Alias cAliasQry
				SELECT DISTINCT D14.D14_PRODUT
				FROM %Table:D14% D14
				WHERE D14.D14_FILIAL = %xFilial:D14%
				AND D14.D14_Local = %Exp:cLocal%
				AND D14.D14_ENDER = %Exp:cEnd%
				AND D14.D14_PRDORI = D14.D14_PRODUT // Não considera na validação produto componente
				AND D14.%NotDel%
				AND NOT EXISTS (SELECT DISTINCT 1
	                			FROM %Table:CBA% CBA
	                			INNER JOIN %Table:D14% D14B
	                			ON D14B.D14_FILIAL = %xFilial:D14%
	                			AND D14B.D14_LOCAL  = CBA.CBA_LOCAL
	                			AND D14B.D14_ENDER = CBA.CBA_LOCALI
	                			AND D14B.D14_LOCAL  = D14.D14_LOCAL
	                			AND D14B.D14_PRODUT = D14.D14_produt
	                			AND D14B.%NotDel%
	                			WHERE CBA.CBA_FILIAL = %xFilial:CBA%
	                			AND CBA.CBA_STATUS = '0'
	                			AND CBA.%NotDel%)
			EndSql
			Do While (cAliasQry)->(!Eof())
				If !WMSVlDfSl(cLocal,(cAliasQry)->D14_PRODUT,lExibMsg,@lSLdDif,@lMsgprlt)
					If !lSLdDif
						lRet := .F.
						Exit
					EndIf
			...
			If lSLdDif
			 	lRet :=.F.
				WmsMessage(WmsFmtMsg(lMsgprlt,{}),WMSXFUNA12,1,lExibMsg) // "Existe divergência de saldo entre a D14 e SB8 para o produto : [VAR01]. Consulte documentação referente análise de estoque por endereço."
			EndIf
		Else
			If !Empty(cProduto)
				If IntWMS(cProduto)
					If !WMSVlDfSl(cLocal,cProduto,lExibMsg,@lSLdDif,@lMsgprlt)
						lRet := .F.
					EndIf
					If lSLdDif
						lRet := .F.
...
//--------------------------------------------------------------------
/*/{Protheus.doc} WMSVlDfSl
Valida se as quantidaes totais do produto na D14 estão batendo com a Sb8 e Sb3.
@param cLocal, character, Código do local de inventário
@param cProd, character, Código do produto inventário
@author Roselaine Adriano
@since  15/07/2021
/*/
//--------------------------------------------------------------------
Function WMSVlDfSl(cLocal, cProduto,lExibMsg,lSLdDif,lMsgprlt)
Local lRet := .T.
Local cAliasQry := Nil
Local lWMSVSIV  := SuperGetMV("MV_WMSVSIV",.F.,.F.)
Local oEstEnder := WMSDTCEstoqueEndereco():New()
Local nInt := TamSX3('D14_QTDEST')[1]+1
Local nDec := TamSX3('D14_QTDEST')[2]

	//As validações abaixo não contemplam produtos componente.
	cAliasQry := GetNextAlias()
	BeginSql Alias cAliasQry
		...
		AND D14.D14_LOCAL = %Exp:cLocal%
		AND D14.D14_PRDORI = D14.D14_PRODUT
		AND D14.%NotDel%
		GROUP BY D14.D14_PRODUT
   		 		,B2_QATU
		HAVING CAST(SUM(D14.D14_QTDEST) AS DECIMAL (%Exp:nInt%,%Exp:nDec%)) <> CAST(SB2.B2_QATU AS DECIMAL (%Exp:nInt%,%Exp:nDec%))
	EndSql
	If (cAliasQry)->(!Eof())
		WmsMessage(WmsFmtMsg(STR0123,{{"[VAR01]",cProduto}}),WMSXFUNA10,5,lExibMsg) // "Existe divergência de saldo entre a D14 e SB2 para o produto : [VAR01]. Consulte documentação referente análise de estoque por endereço."
		lRet := .F.
	EndIf
	(cAliasQry)->(DbCloseArea())
	...
	If lRet .And. Rastro(cProduto)
		...
		BeginSql Alias cAliasQry
			SELECT SUM(D14.D14_QTDEST) D14_QTDEST
    			...
    			,D14.D14_PRODUT
			FROM  %Table:D14% D14
			INNER JOIN (SELECT SUM(SB8.B8_SALDO) B8_SALDO
			       					,SB8.B8_PRODUTO
									,SB8.B8_LOCAL
   		 				FROM  %Table:SB8% SB8
    					...
    					GROUP BY SB8.B8_PRODUTO
								,SB8.B8_LOCAL) SB8
			ON SB8.B8_PRODUTO = D14.D14_PRODUT
			...
			GROUP BY D14.D14_PRODUT
   		 			,SB8.B8_SALDO
			HAVING CAST(SUM(D14.D14_QTDEST) AS DECIMAL (%Exp:nInt%,%Exp:nDec%)) <> CAST(SB8.B8_SALDO AS DECIMAL (%Exp:nInt%,%Exp:nDec%))
		EndSql
		If (cAliasQry)->(!Eof())
			WmsMessage(WmsFmtMsg(STR0124,{{"[VAR01]",cProduto}}),WMSXFUNA11,5,lExibMsg) // "Existe divergência de saldo entre a D14 e SB8 para o produto : [VAR01]. Consulte documentação referente análise de estoque por endereço."
			lRet := .F.
		EndIf
		(cAliasQry)->(DbCloseArea())

		//Inserir validação por lote mostrando os lotes com problema para o produto
		If lRet .And. lWMSVSIV
			cAliasQry := GetNextAlias()
			BeginSql Alias cAliasQry
				SELECT CASE WHEN SB8.B8_SALDO IS NULL THEN 0 ELSE SB8.B8_SALDO END AS B8_SALDO,
   		 			CASE WHEN SUM(D14_QTDEST) IS NULL THEN 0 ELSE SUM(D14_QTDEST) END AS D14_QTDEST,
   					D14.D14_PRODUT,
    				D14.D14_LOTECT,
					SB1.B1_DESC
				FROM  %Table:D14% D14
				...
				AND SB1.%NotDel%
				LEFT JOIN (SELECT SUM(SB8.B8_SALDO) B8_SALDO
			       					,B8_LOTECTL
   		 						FROM  %Table:SB8% SB8
   		 						WHERE SB8.B8_FILIAL =  %xFilial:SB8%
								AND SB8.B8_LOCAL = %Exp:cLocal%
								...
								AND SB8.%NotDel%
    							GROUP BY B8_LOTECTL) SB8
				ON SB8.B8_LOTECTL = D14.D14_LOTECT
				WHERE D14.D14_FILIAL = %xFilial:D14%
				AND D14.D14_PRODUT =  %Exp:cProduto%
				AND D14.D14_LOCAL = %Exp:cLocal%
				AND D14.D14_PRDORI = D14.D14_PRODUT
				...
				GROUP BY D14.D14_PRODUT
					...
					,D14.D14_LOTECT
					,SB1.B1_DESC
   		 		HAVING CASE WHEN SUM(D14.D14_QTDEST)  IS NULL THEN 0 ELSE CAST(SUM(D14.D14_QTDEST) AS DECIMAL (%Exp:nInt%,%Exp:nDec%)) END <> CASE WHEN SB8.B8_SALDO IS NULL THEN 0 ELSE CAST(SB8.B8_SALDO AS DECIMAL (%Exp:nInt%,%Exp:nDec%)) END
				UNION
		   		SELECT CASE WHEN SUM(SB8.B8_SALDO) IS NULL THEN 0 ELSE SUM(SB8.B8_SALDO) END AS B8_SALDO,
				   	Case WHEN D14.D14_QTDEST IS NULL THEN 0 ELSE D14.D14_QTDEST END AS D14_QTDEST,
					SB8.B8_PRODUTO,
					B8_LOTECTL,
					SB1.B1_DESC
				FROM %Table:SB8% SB8
				...
				AND SB1.%NotDel%
				LEFT JOIN (SELECT SUM(D14.D14_QTDEST)  AS D14_QTDEST
			       					,D14.D14_LOTECT
   		 					FROM   %Table:D14% D14
   		 					WHERE D14.D14_FILIAL = %xFilial:D14%
							AND D14.D14_LOCAL = %Exp:cLocal%
							AND D14.D14_PRODUT = %Exp:cProduto%
       		 				AND D14.%NotDel%
       		 				GROUP BY D14.D14_LOTECT) D14
				ON D14.D14_LOTECT = SB8.B8_LOTECTL
				WHERE SB8.B8_FILIAL =  %xFilial:SB8%
				AND SB8.B8_PRODUTO =  %Exp:cProduto%
				AND SB8.B8_LOCAL = %Exp:cLocal%
				AND SB8.%NotDel%
				GROUP BY SB8.B8_PRODUTO
   		 				...
   		 				,B8_LOTECTL
						,SB1.B1_DESC
   		 		HAVING CASE WHEN SUM(SB8.B8_SALDO)  IS NULL THEN 0 ELSE CAST(SUM(SB8.B8_SALDO) AS DECIMAL (%Exp:nInt%,%Exp:nDec%)) END <> CASE WHEN D14.D14_QTDEST IS NULL THEN 0 ELSE CAST(D14.D14_QTDEST AS DECIMAL (%Exp:nInt%,%Exp:nDec%)) END
			EndSql
			Do While (cAliasQry)->(!Eof())
				lSLdDif := .T.
				If Empty(lMsgprlt)
					lMsgprlt := STR0125 + Chr(13)+Chr(10) + cValToChar((cAliasQry)->D14_PRODUT) + ' - ' + (cAliasQry)->B1_DESC +  ' / ' + cValToChar((cAliasQry)->D14_LOTECT)  + Chr(13)+Chr(10) //"Produtos e lotes possuem divergência entre saldos por lote estoque (SB8) e WMS (D14). Serão listados os Produtos/Lote com divergência: "
				Else
					lMsgprlt +=  cValToChar((cAliasQry)->D14_PRODUT) + ' - ' + (cAliasQry)->B1_DESC +  ' / ' + cValToChar((cAliasQry)->D14_LOTECT) + Chr(13)+Chr(10)
				EndIf
				(cAliasQry)->(DbSkip())
			EndDo
			(cAliasQry)->(DbCloseArea())
		EndIf
	EndIf
	//Inserir validação da D13 com D14 e Saldo inicial
	If lRet .AND. !lSLdDif
		cAliasQry := oEstEnder:SelectKard(.F.,cLocal,cProduto)
		If (cAliasQry)->(!Eof()) .And. (cAliasQry)->NR_COUNT > 0
			WmsMessage(WmsFmtMsg(STR0126,{{"[VAR01]",cProduto}}),WMSXFUNA13,5,lExibMsg) //"Existe divergência Kardex(D13) endereço para o produto: [VAR01] .Consulte documentação referente análise de estoque por endereço. "
			lRet := .F.
		EndIf
		(cAliasQry)->(dbCloseArea())
	EndIf
...
//--------------------------------------------------------------------
/*/{Protheus.doc} WmsVD12Aglu
Efetua verificação de registro da D12 (movimentos) com campo D12_AGLUTI = '2' (não aglutina)
mas que deveria estar gravado com '1', pois possui mais de 1 movimento de aglutinação.
Função criada por alguma falha no sistema que não foi identificada.
Apesar de não haver ocorrências e isso possivelmente não ocasionar erros, foi prevista também
a situação contrária (D12_AGLUTI = '1' mas somente 1 registro na DCR).
@author Wander Horongoso
@since  11/08/2021
/*/
//--------------------------------------------------------------------
Function WmVD12Aglu(cIdDCF, cIdMov, cAgluti)
Local cAliasQry := GetNextAlias()
Local lAtu := .F.
Local cQry := ''
Local lRet := .T.
Local cNewAgluti := cAgluti

	BeginSQL Alias cAliasQry
		SELECT DCR.DCR_IDOPER, Count(1) TOTAL
		FROM %Table:DCR% DCR
		WHERE DCR.DCR_FILIAL = %xFilial:DCR%
		AND DCR.DCR_IDORI = %Exp:cIdDCF%
		AND DCR.DCR_IDMOV = %Exp:cIdMov%
		AND DCR.%NotDel%
		GROUP BY DCR.DCR_IDOPER
	EndSQL

	While (cAliasQry)->(!Eof())
		lAtu := .F.

		If (cAliasQry)->TOTAL > 1 .And. cAgluti == '2'
			cNewAgluti := '1'
			lAtu := .T.
		ElseIf (cAliasQry)->TOTAL = 1 .And. cAgluti == '1'
			cNewAgluti := '2'
			lAtu := .T.
		EndIf

		If lAtu
			cQry := "UPDATE " + RetSqlName('D12')
			cQry += " SET D12_AGLUTI = '" + cNewAgluti + "'"
			cQry += " WHERE D12_FILIAL = '" + xFilial('D12') + "'"
			cQry += " AND D12_IDDCF = '" + cIdDCF + "'"
			cQry += " AND D12_IDMOV = '" + cIdMov + "'"
			cQry += " AND D12_IDOPER = '" + (cAliasQry)->DCR_IDOPER + "'"
			cQry += " AND D_E_L_E_T_ = ' '"

			If TcSqlExec(cQry) < 0
				WmsMessage('Erro na atualização do indicador de movimento aglutinado: ' + TcSqlError() + '.',WMSXFUNA09)
				lRet := .F.
			EndIf
		EndIf
...
//--------------------------------------------------------------------
/*/{Protheus.doc} WmsConout
Grava conout das regras de aglutinação  para geração de OS (WMSA150) e alteração de movimento (WMSA332),
com base em um parâmetro oculto.
@author Wander Horongoso
@since  11/08/2021
/*/
//------------------	--------------------------------------------------
function WmsConout(cTexto)
	If SuperGetMv('MV_WMSVGOS',.F.,.F.)
		LogMsg("WMSXFUNA",0, 0, 1, cValToChar(TIME()), '', '__GRVOS__ ' + AllTrim(Str(ThreadId())) + ' ' + cTexto)
	EndIf
Return

//--------------------------------------------------------------------
/*/{Protheus.doc} WmsQrQtOri
Retorna a query com a quantidade total da última atividade da OS.
Função criada por haver 2 funções que montavam a mesma query.
@author Wander Horongoso
@since  07/02/2022.
/*/
//--------------------------------------------------------------------
Static Function WmsQrQtOri(cCarga, cPedido, cProduto, cLote, cSubLote, cTarefa)
Local cQuery := ""

	cQuery += " SELECT SUM(DCR_QUANT) QTDORI"
	cQuery += " FROM ("
	cQuery +=     " SELECT dcr_idmov, dcr_quant, MAX(db_ordativ) dbordativ"
	cQuery +=         " FROM " +  RetSqlName('DCF') + " DCF"
	...
	cQuery +=         " INNER JOIN " + RetSQLName("DCF") + " DCFORI"
	cQuery +=         "    ON dcfori.dcf_filial = '" + xFilial("DCF") + "'"
	cQuery +=         "   AND dcfori.dcf_id = dcr.dcr_idori"
	cQuery +=         "   AND dcfori.d_e_l_e_t_ = ' '"

	cQuery +=         " INNER JOIN " + RetSQLName("SDB") + " SDB"
	cQuery +=         "    ON sdb.db_filial = '" + xFilial("SDB") + "'"
	cQuery +=         "   AND sdb.db_doc = dcfori.dcf_docto"
	...
	cQuery +=         " GROUP BY dcr.dcr_idmov, dcr.dcr_quant"
	cQuery += " ) aux"
	cQuery += " WHERE 1=1"

Return cQuery

/*/{Protheus.doc} WmsDocSD3
	Função que implementa a regra da função A261RetINV, com a diferença de também tratar documentos
	com prefixo CEX e CFT, caso o parâmetro MV_WMSNEXFT esteja ativo.
	@type  Function
	@author wander.horongoso
	@since 15/02/2022
	@version 1.0
	@param cDoc, char, Número do documento pré-determinado pela função NextNumero
	@return cRet, char, Novo número do documento, após novas buscas.
	/*/
Function WmsDocSD3(cDoc)
Local aAreaAnt := GetArea()
Local aAreaSD3 := SD3->(GetArea())
Local cFilSD3  := xFilial('SD3')
Local lNCEXCFT := SuperGetMV('MV_WMSEXFT', .F., .T.)
Local aPre := {}
Local cPre := ''
Local nX := 0
Local nY := 0
...
Local lPre := .F.
	...
	//Indica que considera documentos com prefixo CEX e CFT para determinar o próximo número
	If lNCEXCFT
		aPre := {'INVEN', 'SK'}
	Else
		aPre := {'CEX', 'CFT', 'INVEN', 'SK'}
	EndIf

	dbSelectArea('SD3')
	dbSetOrder(2)

	For nX := 1 to Len(aPre)
		cPre := aPre[nX]

		If Upper(SubStr(cDoc, 1, Len(cPre))) == cPre
			/*colocado em comentário pois não foi encontrada forma de esse código ser executado.
			  Equipe de materiais também não conseguiu explicar como simular.
			dbSeek(cFilSD3+'z', .T.)
			If !Eof() .And. D3_FILIAL == cFilSD3 .And. !(Upper(SubStr(D3_DOC, 1, Len(cPre)))) == cPre
				cDoc 		:= D3_DOC
				cRet 		:= Soma1(cDoc)
				lContinua 	:= .F.
			EndIf*/
			...
			//If lContinua
				...
				while nY >= 1
					...
					While nY > 1
						nY--
						If (Upper(SubStr(D3_DOC, 1, Len(aPre[nY]))) == aPre[nY])
							cPre := aPre[nY]
							nY++
							Exit
						EndIf
		...
		//Se achou um valor para o documento, pode sair do laço
		If !lContinua
			Exit
		EndIf

		//Se achou docs com prefixo, não achou outros docs e é o último prefixo, assume 1
		If lContinua .And. lPre .And. nX == Len(aPre)
...
//--------------------------------------------------------------------
/*/{Protheus.doc} WmsVldAbas
Valida se a estrutura informada para o produto no inventario possui sequencia de abastecimento.
Valida se os saldos entre SB2/SB8/D14/cálculo kardex D13 estão corretos.
@param cLocal, character, Código do local do inventário
@param cProduto, character, Código do produto do inventário
@param cEnder, character, Código do endereço do inventário
@author Fagner Ferraz Barreto
@Since 27/10/2022
/*/
//--------------------------------------------------------------------
Function WmsVldAbas(cLocal ,cProduto ,cEnder)
	Local aAreaAnt 	:= GetArea()
	...
	//--Ativação de WorkArea
	SBE->( DbSetOrder(1) )
	DC3->( DbSetOrder(2) )

	If IntWms(cProduto) .And. lWmsNew  //--Controla WMS

		If Empty(cEnder)

			WmsMessage(WmsFmtMsg(STR0127,{{"[VAR01]",AllTrim(cProduto)}}),WMSXFUNA14,5) //--Produto [VAR01] possui controle de WMS, é obrigatório o preenchimento do campo endereço.
			lRet :=	.F.

		Else

			If SBE->( DbSeek( FWxFilial("SBE") + cLocal + cEnder ) )

				If !( DC3->( MsSeek( FWxFilial("DC3") + cProduto + cLocal + SBE->BE_ESTFIS ) ) )
					If IsTelNet()
						WMSVTAviso(WMSXFUNA15,WmsFmtMsg(STR0128,{{"[VAR01]",AllTrim(SBE->BE_ESTFIS)},{"[VAR02]",AllTrim(cLocal)},{"[VAR03]",AllTrim(cProduto)}})) //--Estrutura [VAR01] do local [VAR02] não localizada na seq de abastec do produto [VAR03].
						lRet := .F.
					Else
						WmsMessage(WmsFmtMsg(STR0128,{{"[VAR01]",AllTrim(SBE->BE_ESTFIS)},{"[VAR02]",AllTrim(cLocal)},{"[VAR03]",AllTrim(cProduto)}}),WMSXFUNA15,5) //--Estrutura [VAR01] do local [VAR02] não localizada na seq de abastec do produto [VAR03].
						lRet :=	.F.
					EndIf
				EndIf
		...
		If lRet
			//Valida se os saldos SB8/SB2/D14 e Kardex estão corretos.
			If !WMSVldInve(cLocal, cEnder, cProduto,'1',.T.)  //faz validação por produto se os saldos estão ok 
               lRet := .F. 
			EndIf
		EndIf 
	EndIf
...
/*/{Protheus.doc} WmsMat2611
Função usada no MATA261, criada para que a regra do WMS fique fora dos fontes de outros módulos.
O objetivo da função é verificar se a execução do MATA261 foi chamada pela função WmsMovEst(WMS Atual).

Devido à descontinuação do MATA260, a integração passou a ser feita com o MATA261.
Foi mantida a mesma regra já existente, para evitar problemas. Entretanto, seria possível
usar IsInCallStack ("WMSMovEst").

@type  Function
@author wander.horongoso
@since 19/07/2022
@version 1.0
@param aWmsCMov, array, Contém as informações específicas, carregadas somente via WmsMovEst.
@param cCampo, char, Campo a ser pesquisado no array
@return cRet, char, conteúdo com base no campo informado
/*/
Function WmsMat2611(aWmsCMov, cCampo)
Local cRet := ""
Local nPosCampo := 0
Default aWmsCMov := {}
	...
	If Empty(aWmsCMov) 
		Return ""
	EndIf 
	nPosCampo := aScan(aWmsCMov, {|x| AllTrim(x[1])==cCampo})
	If nPosCampo > 0 
		cRet := AllTrim(aWmsCMov[nPosCampo, 2])
	EndIf
...
/*/{Protheus.doc} 
	(Informa de armazem/endereco tem controle WMS)
	@author Equipe WMS
	@since 08/01/2024
	...
	/*/
Function WmsVlClEnd(cArmazem,cEndereco)
	Local aArea := GetArea()
	Local lRet  := .T.
