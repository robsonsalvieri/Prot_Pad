...
#include 'parmtype.ch'
Static _cCP	:=	""
user function MYENCODE(cString)
Local cRet	:=	""
Local nX
Local aCodes	
Local cChar	:=	""
paramtype 0 var cString as Character default ""	
...
For nX:=1 To Len(cString)
	...
	If cChar$"&#;<>/\[]"+Chr(13)+Chr(10)+Chr(9)  
		cRet	+=	"&#"+Alltrim(StrZero(Asc(cChar),4))+";"
	ElseIf Asc(cChar) > 127
...
User Function MyDecode(cString)
Local cRet	:=	""
Local nX
Local aCodes	:=	{}
Local cChar		:=	""
...
For nX:=1 To Len(cString)
	cChar	:=	Substr(cString,nX,7)
	If Substr(cChar,1,2)=="&#" .And.Substr(cChar,1,7)==";"
		nCharSeek	:=	Val(Substr(cChar,3,4))
		nPos	:=	Ascan(aCodes,{|x| x[1]==nCharSeek})
		cRet	+=	Chr(aCodes[nPos,2])
	Else
		cRet	+=	cChar
	Endif
Next
