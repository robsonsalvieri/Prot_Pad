...
/*/
//-----------------------------------------------------------------------
Function nfseXMLUni( cCodMun, cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela, aAIDF, cCodcanc )
...
//	DEFAULT aAIDF       := PARAMIXB[9]
	...
	ElseIf cTipo == "1" .And. !Empty(cMotCancela)
		If tssHasRdm("nfseXMLCan")
			//cString := u_nfseXMLCan(cNota,cMotCancela, cCodCanc) // essa chamada utiliza RdMake que utiliza User Funcion
			cString := tssExecRdm("nfseXMLCan",.F.,cNota,cMotCancela,cCodCanc) //essa chamada usa no NSFEXmlUni que chama pelo RPO
		EndIf	
	EndIf
...
Function nfseXMLCan( cNota, cMotCancela, cCodCanc )
