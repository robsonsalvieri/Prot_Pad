...
Function DAMDFE(cIdEnt,oDamdfe,oSetup,cFilePrint)
...
Local nLinQrChav	:= 0
	...
	nLinQrChav += 10
	oDamdfe:Say( nLinQrChav-nPosIniCtg, 0332, Transform(AllTrim(aCab[5]),"@R! NN.NNN.NNN/NNNN-99"), oFont11)
	nLinQrChav += 10
	...
	Do Case
		Case Type("oNfe:_MDFE:_INFMDFE:_EMIT:_CNPJ") == "O"
			cAux := TransForm(oNfe:_MDFE:_INFMDFE:_EMIT:_CNPJ:TEXT, "@R! NN.NNN.NNN/NNNN-99")
...
Static Function DamdfeVCP(oDamdfe, nLin )
	...
	Local nLinCond	:= 0
	local nQtdLoop	:= 0
	Local cMaskCnpj	:= "@R 999.999.999-99"
	...
	if type("oNfe:_MDFE:_INFMDFE:_INFMODAL:_RODO:_INFANTT:_VALEPED:_DISP") <> "U"
		...
		For nCount := 1 To nQtdLoop
			...
			endIf
			//Responsável CNPJ
			oDamdfe:Say(nLin-nPosIniCtg, 0005,  Padr(Transform(cCGC, cMaskCnpj ),18) , oFont10N)

			//Fornecedor CNPJ
			oDamdfe:Say(nLin-nPosIniCtg, 0105,  Padr(Transform(aValePed[nCount]:_CNPJForn:TEXT, "@R! NN.NNN.NNN/NNNN-99" ),18) , oFont10N)
