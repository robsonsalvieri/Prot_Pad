...
Private oFont08, oFont08n, oFont09, oFont10n, oFont28n 
...
oPrint:say ( 0145, 0930, STR0001, oFont28n )	//"FINIQUITO"           	
...
oPrint:Box ( 0456, 0062, 0880, nColMax-21 )    				//Box
	...
	oPrint:say ( 0623, 0064, STR0007+;
		(Substr(TrmDesc("SX5","33"+SRA->RA_ESTCIVI,"SX5->X5_DESCRI"),1,15)), oFont09 )    //"ESTADO CIVIL: " 
	...
	oPrint:say ( 0729, 1410, STR0012+DtoC(SRA->RA_DEMISSA), oFont09 )    //"FECHA DE RETIRO" 
	oPrint:line( 0774, 0062, 0774, nColMax-21 ) 		   		//Linha Horizontal
	oPrint:say ( 0782, 0064, STR0013+cCausa, oFont09 )    //"MOTIVO DEL RETIRO: "
		oPrint:line ( 0774, 1050, 0827, 1050) 
		oPrint:line ( 0774, 1200, 0827, 1200)
	oPrint:say ( 0782, 1210, STR0014+AllTrim(Str(SRG->RG_SALMES)), oFont09 )    //"REMUNERACION MENSUAL Bs: "
	...
	oPrint:say ( 0835, 0450, AllTrim(Str(aTempoServico[1])), oFont09 )        
	oPrint:say ( 0835, 0500, STR0016, oFont09 )    //"ANOS"
    oPrint:say ( 0835, 0650, AllTrim(Str(aTempoServico[2])), oFont09 )
	oPrint:say ( 0835, 0700, STR0017, oFont09 )    //"MESES"
	oPrint:say ( 0835, 0850, AllTrim(Str(aTempoServico[3]+1)), oFont09 )
...
oPrint:Box ( 1568, 0062, 2052, nColMax-21 )    			//Box     
	...
	oPrint:say ( 1576, 2000, Transform(nValor, "@E 99,999,999.99"), oFont08 ) 
	...
	oPrint:say ( 1636, 1000, Transform(Val(cHoras), "999.99"), oFont08)
	...
	oPrint:say ( 1795, 1000, Transform(Val(cHoras), "999.99"), oFont08)
	nBsAgNavidad+= nValor
	oPrint:say ( 1795, 1210, STR0032, oFont09 )    //"MESES Y"                     
	RetHoraVal( "0763", @cHoras, @nValor, @cDesc )
	oPrint:say ( 1795, 1500, Transform(Val(cHoras), "999.99"), oFont08)
	nBsAgNavidad+= nValor
	...
	oPrint:say ( 1795, 2000, Transform(nBsAgNavidad, "@E 99,999,999.99"), oFont08)
	...
	oPrint:say ( 1848, 1040, Transform(0, "999.99"), oFont08)
	...
	nHVacacion+= Val(cHoras)
	nBsVacacion+= nValor
	oPrint:say ( 1848, 1500, Transform(nHVacacion, "999.99"), oFont08)
	oPrint:say ( 1848, 1685, STR0018, oFont09 )    //"DIAS"

	oPrint:say ( 1848, 1860, STR0023, oFont09 )    //"Bs" 
	oPrint:say ( 1848, 2000, Transform(nBsVacacion, "@E 99,999,999.99"), oFont08) 
	...
	nBsPrima+= nValor
	oPrint:say ( 1901, 1040, Transform(Val(cHoras), "999.99"), oFont08)	 
	oPrint:say ( 1901, 1210, STR0032, oFont09 )    //"MESES Y"
	RetHoraVal( "0767", @cHoras, @nValor, @cDesc )	 
	nBsPrima+= nValor
	oPrint:say ( 1901, 1500, Transform(Val(cHoras), "999.99"), oFont08)
	oPrint:say ( 1901, 1685, STR0018, oFont09 )    //"DIAS"
	oPrint:say ( 1901, 1860, STR0023, oFont09 )    //"Bs"	
	oPrint:say ( 1901, 2000, Transform(nBsPrima, "@E 99,999,999.99"), oFont08)
	...
	oPrint:say ( 2007, 0200, Transform(Year(SRA->RA_DEMISSA),"9999"), oFont08)	
	oPrint:say ( 2007, 0500, STR0036, oFont09 )    //"GESTION"
		oPrint:line ( 1999, 0750, 2052, 0750)						//Linha Vertical
	oPrint:say (2007, 0800, MesExtenso(Month(SRA->RA_DEMISSA)), oFont08) 
	...
	oPrint:say ( 2007, 1450, Transform(Val(cHoras), "999.99"), oFont09)            
	...
	oPrint:say ( 2007, 2000, Transform(nValor, "@E 99,999,999.99"), ofont08)
	nTotBenSocial+= nValor
...
If !(len(aPdd) < 1)
	...
	For nCont:=1 to nAte
		oPrint:say ( nLinha, 0380, aPdd[nCont][2], oFont08)
		oPrint:say ( nLinha, 1300, Transform(aPdd[nCont][3],"@E 99,999,999.99"), oFont08)
		nLinha+=53   
...
oPrint:Box ( 0027, 0045, 2638, nColMax-7 )
	...
	oPrint:say ( 1500, 1000, STR0058, oFont09 )    //"INSTRUCCIONES"
	oPrint:say ( 1600, 0350, STR0059, oFont09 )    //"1. En todos los casos en los cuales proceda el pago de benefícios sociales y que no"
	oPrint:say ( 1650, 0350, STR0060, oFont09 )    //"estén comprendidos en el despido por las causales en el Art. 16 de la  Ley  General"
	oPrint:say ( 1700, 0350, STR0061, oFont09 )    //"del Trabajo y el Art. 9 de su Reglamento, el Finiquito de contrato se suscribirá en"
	oPrint:say ( 1750, 0350, STR0062, oFont09 )    //"el presente FORMULARIO."
	oPrint:say ( 1800, 0350, STR0063, oFont09 )    //"2. Los señores Directores, Jefes Departamentales e Inspectores  Regionales, son los"
	oPrint:say ( 1850, 0350, STR0064, oFont09 )    //"únicos funcionarios facultados para revisar y refrendar todo Finiquito de  contrato"
	oPrint:say ( 1900, 0350, STR0065, oFont09 )    //"de Trabajo, concuya intervención alcanzará la correspondiente eficacia jurídica, en"
	oPrint:say ( 1950, 0350, STR0066, oFont09 )    //"aplicación del Art. 22 de la Ley General del Trabajo."
	oPrint:say ( 2000, 0350, STR0067, oFont09 )    //"La  intervención  de  cualquer  otro   funcionario  del  Ministerio  de  Trabajo  y"
	oPrint:say ( 2050, 0350, STR0068, oFont09 )    //"Microempresa carecerá de toda validez legal."
	oPrint:say ( 2100, 0350, STR0069, oFont09 )    //"3. Las  partes  intervinientes  en  la  suscripción del presente FINIQUITO, deberán"
	oPrint:say ( 2150, 0350, STR0070, oFont09 )    //"acreditar suidentidad personal con los documentos señalados por ley."
	oPrint:say ( 2200, 0350, STR0071, oFont09 )    //"4. Este  Formulario  no  constituye  Ley entre partes por su carácter esencialmente"
	oPrint:say ( 2250, 0350, STR0072, oFont09 )    //"revisable, porlo  tanto las  cifras en el contenidas no causan estado ni revisen el" 
	oPrint:say ( 2300, 0350, STR0073, oFont09 )    //"sello de cosa juzgada"
