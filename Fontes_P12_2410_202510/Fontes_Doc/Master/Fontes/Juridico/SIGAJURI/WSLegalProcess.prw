...
//-------------------------------------------------------------------
/*/{Protheus.doc} JWhrFltAdd(aSQL, cNSZName, lTask)
...
Default cRotina     := ""
	...
	If cRotina == "1" // Se é pesquisa avançada (TJD)
		If cAssJurGrp <> "'00'"
			cAssPesqAv := WSJAPsqAva(cAssJurGrp)
			cWhere += " AND NSZ001.NSZ_TIPOAS IN (" + vldStrSql(cAssPesqAv) + ") "
		EndIf

		If !Empty(cTpAssunto)
			cTpAssunto := WSJAPsqAva(cTpAssunto)
	...
	cWhere += VerRestricao(,,cAssJurGrp) // Verifica restrição de escritório e área

	If Len(aSQL) > 0 //adiciona as condições da pesquisa avançada
		cWhere += JWhrFltAdd(aSQL, Alltrim(cNSZName), cTarget == 'tasks', cTabelas)
	EndIf

	If !Empty(aSQLRestri) //adiciona as restrições de usuário
		If cRotina == "1" // Se é pesquisa avançada (TJD)
			cWhere += " AND ("+Ja162SQLRt(aSQLRestri, , , , , , , ,WsJGetTpAss(cAssPesqAv , .T.), )+")"
		Else
			cWhere += " AND ("+Ja162SQLRt(aSQLRestri, , , , , , , , , )+")"
		EndIF
	EndIF

	If !Empty(cTpAsCfg) // Configurações do produto (TJD)
...
Default cListFil    := ''
	...
	cQryWhere :=   " WHERE " + JWhrWSNSZ(.T., cListFil)
	cQryWhere +=     " AND NSZ.NSZ_SITUAC = '1'"
...
@return cRetorno  - Where com as restrições de acesso
@since 06/01/2025
@version 1.0
/*/
//-------------------------------------------------------------------
Function JWhrWSNSZ(lDashboard, cListFil, cAliasNSZ, cAssJur, lValid)
...
Default lValid     := .T.
	...
	If lValid
		cAsj := JUsuCfgAss(lDashboard, cAssJur)
	Else
		cAsj := cAssJur
	EndIf
	...
	cWhere += " AND " + cAliasNSZ + ".NSZ_TIPOAS IN (" + cAsj + ")" 
