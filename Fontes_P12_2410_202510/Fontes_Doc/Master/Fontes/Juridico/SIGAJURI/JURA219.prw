...
Default cNumProc := " "
	...
	aAdd(aFiltro, xFilial("CTO") )
	cQuery := ChangeQuery(cQuery)

	dbUseArea( .T., "TOPCONN", TcGenQry2( ,, cQuery, aFiltro ), cAliasCTO, .T., .F. )	
	...
	aAdd(aFiltro, cNumProc)
	aAdd(aFiltro, cNumProc)
	aAdd(aFiltro, xFilial("NZZ") )

	cQuery := ChangeQuery(cQuery)

	dbUseArea( .T., "TOPCONN", TcGenQry2( ,, cQuery, aFiltro ), cAliasNZZ, .T., .F. )
