...
If lRet
	...
	Begin Transaction
		...
		If cTipoEmi == "1" //Fatura -> Gera o(s) titulo(s) financeiros
			...
			For nI := 1 To Len( aParcelas )
				...
				If nTotImpBas > 0
					...
					lRetISS       := IIF(JurGetDados("SA1", 1, xFilial("SA1") + NXA->NXA_CLIPG + NXA->NXA_LOJPG, "A1_RECISS") == "1" .And. GetNewPar("MV_DESCISS",.F.), nVlrIss, 0) > 0
				...
				lDistrImp := J203ImpSE1(Len(aParcelas) > 1, aParcelas[nI][1], aParcelas[nI][2], nTotImpBas, @nBaseCalc, nValIRF, nVlrInss, nVlrIss, cNatureza, @cMemoLog, aParcelas[nI][3], nI) // Distribui os impostos IRF/INSS/ISS para primeira parcela do título.
				...
				If lRet
					...
					// Só manipula a base de cálculo dos impostos se for legado (forma antiga de cálculo dos impostos)
					...
					If Len(aMotorImp) > 0 .And. Len(aMotorImp[1]) > 0
						...
						If aScan(aMotorImp, {|cCodImp| cCodImp[1] == PadR("ISS", _nCodImp)}) < 1
							If _AliqISS > 0.00 .And. lRetISS .And. nI == 1 .And. lDistrImp
		...
		ElseIf cTipoEmi $ "2|3|4" //Minuta de Fatura/Pre Fatura/Minuta de Sócio -> Gera Simulacao de impostos
			...
			lRetISS   := JurGetDados("SED", 1, xFilial("SED") + cNatureza, "ED_CALCISS") == "S" .And. JurGetDados("SA1", 1, xFilial("SA1") + NXA->NXA_CLIPG + NXA->NXA_LOJPG, "A1_RECISS") == "1" .And. GetNewPar("MV_DESCISS",.F.)
			cParcela  := GetMv("MV_1DUP")
			...
			For nI := 1 To Len(aParcelas)
				...
				aRet := J203VerImp(aSE1, cTipoEmi, @aImpostos, cFilAnt, NXA->NXA_CMOEDA, nTotImpBas, Len(aParcelas))
...
If cTipoEmi $ "1|2|3|4"  // Fatura/Minuta de Fatura/Pre Fatura/Minuta de Sócio
	...
	Else
		...
		If AliasInDic("FKK") .And. FindFunction("FINA024CLI") .And. IIF(FindFunction("FTemMotor"), FTemMotor(), .F.)
			If Len(aMotorImp) > 0 .And. FindFunction("JBusFK4Imp") .And. FK7->(ColumnPos("FK7_IDPAI")) > 0 // Proteção @12.1.2510
				...
				If Len(aImpRetidos) > 0
					For nLoop := 1 To Len(aMotorImp)
						...
						// aImpostos é para gravar os valores desses impostos nos campos da NXA (Exemplo: NXA_IRRF). E também para garantir o cenário híbridos dos impostos.
						If nPosImpPrd > 0
							If nPosImp > 0
								aImpostos[nPosImpPrd][1] += nValImp // Só incremento o valor do imposto caso tenha o registro gerado na FK4.
							Else
								aImpostos[nPosImpPrd][1] := nValImp // Atribuo o valor cheio do imposto para registro que foi simulado via FinCalImp (Imposto que estão em regime de caixa).
							EndIf
							aImpostos[nPosImpPrd][2] := .T.
							lMotor := .T.
						EndIf
						If lExistOIC .And. nValAlq > 0 .And. nValImp > 0 .And. cMoedNac == cMoeFat
			...
			Else
				...
				For nLoop := 1 To Len(aInfoImp)
					...
					If FindFunction("JBuscaAliq") // @12.1.2510
		...
		// Simulação do cálculo dos impostos pelo legado (quando não utiliza configurador de tributos / motor de impostos) apenas para Minutas
		// Quando é emissão de fatura pega direto da SE1
		If !lFatura .And. !lMotor
		...
		Else
			For nLoop := 1 To Len(aImpostos)
				...
				If nLoop == 1 .And. !aImpostos[1][2]
					...
					nValAlq    := _AliqIRRF
					nValImp    := IIf(lFatura, SE1->E1_IRRF, M->E1_IRRF)
					nVlBaseImp := IIf(lFatura, SE1->E1_BASEIRF, M->E1_BASEIRF)
					aImpostos[nLoop][1] += nValImp
				ElseIf nLoop == 2 .And. !aImpostos[2][2]
					cCodImp    := PadR("ISS", _nCodImp)
					nValAlq    := _AliqISS
					nValImp    := IIf(lFatura, SE1->E1_ISS, M->E1_ISS)
					nVlBaseImp := IIf(lFatura, SE1->E1_BASEISS, M->E1_BASEISS)
					aImpostos[nLoop][1] += nValImp
				ElseIf nLoop == 3 .And. !aImpostos[3][2]
					cCodImp    := PadR("INSS", _nCodImp)
					nValAlq    := _AliqINSS
					nValImp    := IIf(lFatura, SE1->E1_INSS, M->E1_INSS)
					nVlBaseImp := IIf(lFatura, SE1->E1_BASEINS, M->E1_BASEINS)
					aImpostos[nLoop][1] += nValImp
				ElseIf nLoop == 4 .And. !aImpostos[4][2]
					cCodImp    := PadR("PIS", _nCodImp)
					nValAlq    := _AliqPIS
					nValImp    := IIf(lFatura, SE1->E1_PIS, M->E1_PIS)
					nVlBaseImp := IIf(lFatura, SE1->E1_BASEPIS, M->E1_BASEPIS)
					aImpostos[nLoop][1] += nValImp
				ElseIf nLoop == 5 .And. !aImpostos[5][2]
					cCodImp    := PadR("COF", _nCodImp)
					nValAlq    := _AliqCOF
					nValImp    := IIf(lFatura, SE1->E1_COFINS, M->E1_COFINS)
					nVlBaseImp := IIf(lFatura, SE1->E1_BASECOF, M->E1_BASECOF)
					aImpostos[nLoop][1] += nValImp
				ElseIf nLoop == 6 .And. !aImpostos[6][2]
					cCodImp    := PadR("CSL", _nCodImp)
					nValAlq    := _AliqCSLL
					nValImp    := IIf(lFatura, SE1->E1_CSLL, M->E1_CSLL)
					nVlBaseImp := IIf(lFatura, SE1->E1_BASECSL, M->E1_BASECSL)
					aImpostos[nLoop][1] += nValImp
				EndIf
...
If lRet
	...
	If oFilaExe:OpenReport()
		...
		While !KillApp()
			...
			If(Len(aRetFila) > 1 .And. aRetFila[2] > 0) .And. aRetFila[1][26][2] == "2"
				nI   := 1
				aAux := {}
...
@Param  nTotImpBas, Valor total da base para calculo das alíquotas de imposto
@param aMotorImp  , Dados dos impostos que foram simulados pelo configurador de tributos (motor de impostos)
		aMotorImp[1][1] // Grava o código/ID dos impostos.
		aMotorImp[1][2] // Armazena a alíquota dos impostos.
		aMotorImp[1][3] // Armazena a base de cálculo dos impostos.
...
/*/
//-------------------------------------------------------------------
Function J203PerNat(cNatureza, cCliPag, cLojPag, nTotImpBas, aMotorImp)
...
Default aMotorImp   := {}
	...
	If Len(aMotorImp) > 0 .And. AliasInDic("FKK") .And. FindFunction("FINA024CLI") .And. IIF(FindFunction("FTemMotor"), FTemMotor(), .F.)
		aMotorImp := {}
		aInfoImp  := FINCalImp("2", cNatureza, cCliPag, cLojPag, cFilAnt, nTotImpBas, dDataBase, .F.,, AvKey("FT", "E1_TIPO"),,, {})
		...
		For nLoop := 1 To Len(aInfoImp)
			...
			If nPosImp > 0
				...
				EndIf
			EndIf
			aAdd(aMotorImp, {cCodImp, nValAlq, nVlBaseImp, nValImp}) // Armaneza dados dos impostos simulados pelo configurador de tributos.
		Next nLoop
	EndIf
	...
	If (cAlias)->(!Eof())
		aRet[1] := Iif(Empty(aRet[1]), IIF(Empty((cAlias)->ED_PERCIRF), 0, Val(AVKEY((cAlias)->ED_PERCIRF, "NXA_PIRRF"))) , aRet[1])
		aRet[2] := Iif(Empty(aRet[2]), IIF(Empty((cAlias)->ED_PERCPIS), 0, Val(AVKEY((cAlias)->ED_PERCPIS, "NXA_PPIS")))  , aRet[2])
		aRet[3] := Iif(Empty(aRet[3]), IIF(Empty((cAlias)->ED_PERCCOF), 0, Val(AVKEY((cAlias)->ED_PERCCOF, "NXA_PCOFIN"))), aRet[3])
		aRet[4] := Iif(Empty(aRet[4]), IIF(Empty((cAlias)->ED_PERCCSL), 0, Val(AVKEY((cAlias)->ED_PERCCSL, "NXA_PCSLL"))) , aRet[4])
		aRet[5] := Iif(Empty(aRet[5]), IIF(Empty((cAlias)->ED_PERCINS), 0, Val(AVKEY((cAlias)->ED_PERCINS, "NXA_PINSS"))) , aRet[5])
		
		If (!lIRMotor .Or. !lISSMotor) .And. Len(aSA1 := JurGetDados('SA1', 1, xFilial('SA1') + cCliPag + cLojPag, {'A1_NATUREZ', 'A1_ALIQIR', 'A1_RECIRRF','A1_RECISS'})) == 4
			cNatcli := aSA1[1]
			nCliIR  := aSA1[2]
			lRetIRF := aSA1[3] == '1'
			lRetISS := aSA1[4] == '1'
		...
		If lRetIRF .And. !lIRMotor .And. nCliIR > 0
			aRet[1] := nCliIR
		EndIF
		
		If lRetISS .And. !lISSMotor
...
Define o valor da base de cálculo do imposto de IRF (E1_BASEIRF) e se
deve ou não ser retido o imposto.
...
@param  nBaseCalc  - Base de imposto que será considerada para o cálculo
...
@return lRet Se .T. retém o imposto, .F. não deve ser retido.
...
Local aImpostos := {}
	...
	If lFatParcel .And. nTotImp > nValParc
		cMemoLog += I18N(STR0301, {AllTrim(NXA->NXA_COD), nTotImp, cImpostos, nValParc, nParcela, AllTrim(NXA->NXA_CCDPGT), AllTrim(cNatureza)}) + CRLF // "A somatória de #2, dos impostos #3 ultrapassa o valor #4 da parcela #5. Verificar a condição de pagamento #6 e natureza #7."
...
/*/
//-------------------------------------------------------------------
Static Function J203GrvOIC(cCodImp, nVlrAlq, nVlrImp, cTipoTit, nVlBaseImp, nVlBaseTot, lMotor, lParcelada)
Local cChaveOIC  := ""
Local aArea      := GetArea()
Local lInclui    := .T.
	...
	If !lInclui .And. lParcelada .And. (!lMotor  .And. cCodImp != PadR("IRF", _nCodImp) .Or. lMotor) .And. nVlBaseImp != nVlBaseTot  // Incrementa o valor de todos os impostos (menos IRRF) quando for legado e fatura parcelada (Cenario necessario quando tiver base diferenciada)
		RecLock("OIC", .F.)
		OIC->OIC_VLRIMP += Round(nVlrImp, _nDecVlrImp)
		If cCodImp != PadR("INSS", _nCodImp) .Or. lMotor
			OIC->OIC_BASIMP += nVlBaseImp
		EndIf
	EndIf
