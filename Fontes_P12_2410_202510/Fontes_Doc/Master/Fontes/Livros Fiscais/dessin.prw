...
±±∫Programa  ≥DESSIN    ∫Autor  ≥Sueli               ∫ Data ≥  06/06/2007 ∫±±
...
If lProcessa
	...
	aAdd (aStru, {"B2_DOC",	    	"C",	06,	0})
	aAdd (aStru, {"B2_SERIE", 	    "C",	TamSx3("F3_SERIE")[1],	0})
	...
	aAdd (aStru, {"B3_NUMINI", 	    "N",	06,	0})
	aAdd (aStru, {"B3_NUMFIM",		"N",	06,	0})
	...
	//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
	aStru	:=	{}                     
	aAdd (aStru, {"B4_EMISSAO",		"C",	08,	0})
	aAdd (aStru, {"B4_DOC",	    	"C",	06,	0})
	aAdd (aStru, {"B4_SERIE", 	    "C",	TamSx3("F3_SERIE")[1],	0})	
	aAdd (aStru, {"B4_ALIQ",	   	"N",	05,	0})
	aAdd (aStru, {"B4_VBASE", 	    "N",	13,	2})
	aAdd (aStru, {"B4_VIMP",		"N",	13,	2})
	aAdd (aStru, {"B4_SDOC", 	    "C",	02,0})
	...
	IndRegua ("TB4", cArq, "B4_EMISSAO+B4_DOC+B4_SERIE")
		...
		If Left (SF3->F3_CFO, 1)>="5"
			...
			If SA1->(DbSeek (xFilial ("SA1")+SF3->F3_CLIEFOR+SF3->F3_LOJA))
				If !TP1->(DbSeek (SA1->A1_CGC))
					...
					TP1->P1_CONT    := Iif(cCidade$UPPER(SA1->A1_MUN),"S","N")
				...
				If !TB1->(DbSeek (SF3->F3_NFISCAL+SF3->F3_SERIE+SA1->A1_CGC))
					...
					TB1->B1_CONT       	:= Iif(cCidade$SA1->A1_MUN,"S","N")
					TB1->B1_CODBARRA   	:= 0
				...
				If !TB2->(DbSeek (SF3->F3_NFISCAL+SF3->F3_SERIE+SA1->A1_CGC)) 
					...
					TB2->B2_SDOC    	:= SerieNfId("SF3",2,"F3_SERIE")
					MsUnLock()
	         End If
			EndIf
		...
		Else 
			...
			If SA2->(DbSeek (xFilial ("SA2")+SF3->F3_CLIEFOR+SF3->F3_LOJA))  
				If !TA1->(DbSeek (SA2->A2_CGC))
					...
					TA1->A1_CONT    := Iif(cCidade$UPPER(SA2->A2_MUN),"S","N")
			...
			If !TA2->(DbSeek (SA2->A2_CGC+DTOS(SF3->F3_EMISSAO)))
					...
					TA2->A2_CONT    := Iif(cCidade$UPPER(SA1->A1_MUN),"S","N")
					TA2->A2_DOC     :=	SF3->F3_NFISCAL
					TA2->A2_CODBARR := 0
...
While SFI->(!Eof())                                              
	If !TB3->(DbSeek (DTOS(SFI->FI_DTMOVTO)+SFI->FI_PDV)) 
		...
		TB3->B3_NUMINI  :=	Val(SFI->FI_NUMINI)
		TB3->B3_NUMFIM  :=	Val(SFI->FI_NUMFIM)
...
While SD2->(!Eof())                                              
	If !TB4->(DbSeek (DTOS(SD2->D2_EMISSAO)+SD2->D2_DOC+SD2->D2_SERIE)) 
		RecLock ("TB4", .T.)
		TB4->B4_EMISSAO :=	DTOS(SD2->D2_EMISSAO)
		TB4->B4_DOC     :=	SD2->D2_DOC
		TB4->B4_SERIE   :=	SD2->D2_SERIE
		TB4->B4_SDOC    := SerieNfId("SF3",2,"F3_SERIE") 
	Else
		RecLock ("TB4", .F.)              
	EndIf	
...
±±∫Programa  ≥GeraReg      ∫Autor  ≥Sueli               ∫ Data ≥  06/06/2007 ∫±±
    	    ...
    	    IF TA2->(dbseek(TA1->A1_CNPJ)) 
	           While TA2->(!eof()) .and.TA1->A1_CNPJ == TA2->A2_CNPJ
				     RecLocK("TRG",.T.)	
   	                 ...
   	                 TRG->CAMPO	:="A2"+TA2->A2_INSCM+TA2->A2_CONT+TA2->A2_EMISSAO+TA2->A2_DOC+StrZero(TA2->A2_CODBARRA,9)+TA2->A2_SERIE+NUM2CHR(TA2->A2_VALTOT,13,2)+NUM2CHR(TA2->A2_VALIMP,13,2)+TA2->A2_RET
