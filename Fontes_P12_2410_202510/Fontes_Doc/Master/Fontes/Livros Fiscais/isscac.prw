...
±±∫Programa  ≥ISSCAC    ∫Autor  ≥Microsiga           ∫ Data ≥  08/08/06   ∫±±
...
If lProcessa
	...
	aAdd (aStru, {"B2_DOC",	   	"C",	TamSX3("F2_DOC")[1],	0})
	...
	aAdd (aStru, {"B4_DOC",	    	"C",	TamSX3("F2_DOC")[1],	0})
	aAdd (aStru, {"B4_SERIE", 	  	"C",	TamSX3("D2_SERIE")[1],0})
		...
		If Left (SF3->F3_CFO, 1)>="5"
			...
			If SA1->(DbSeek (xFilial ("SA1")+SF3->F3_CLIEFOR+SF3->F3_LOJA))
				If !TP1->(DbSeek (SA1->A1_CGC))
					...
					TP1->P1_CONT    := Iif("CACERES"$UPPER(SA1->A1_MUN),"S","N")
				...
				If !TB1->(DbSeek (SF3->F3_NFISCAL+SF3->F3_SERIE+SA1->A1_CGC))
					...
					TB1->B1_CONT       	:= Iif("CACERES"$SA1->A1_MUN,"S","N")
					TB1->B1_CODBARRA   	:= 0
		...
		Else
			...
			If SA2->(DbSeek (xFilial ("SA2")+SF3->F3_CLIEFOR+SF3->F3_LOJA))
				If !TA1->(DbSeek (SA2->A2_CGC))
					...
					TA1->A1_CONT    := Iif("CACERES"$UPPER(SA2->A2_MUN),"S","N")
				...
				If !TA2->(DbSeek (SA2->A2_CGC+DTOS(SF3->F3_EMISSAO)))
					...
					TA2->A2_CONT    := Iif("CACERES"$UPPER(SA1->A1_MUN),"S","N")
					TA2->A2_DOC     :=	SF3->F3_NFISCAL
					TA2->A2_CODBARR := 0
					TA2->A2_SERIE   :=	SF3->F3_SERIE
...
While SD2->(!Eof())                                              
	If !TB4->(DbSeek (DTOS(SD2->D2_EMISSAO)+SD2->D2_DOC+SD2->D2_SERIE)) 
		...
		TB4->B4_SDOC    :=	SerieNfId("SD2",2,"D2_SERIE")
	Else
		RecLock ("TB4", .F.)              
	EndIf	
