...
static function XmlCTeIMP(cSerie,cNota,cCliente,cLoja,cEvento)
	...
	Local aBenef 	:= {}
	Local cEstICMSD := Alltrim(SuperGetMv("MV_GTPESDE",.F.,'RJ'))
	...
	ElseIf SubStr(SFT->FT_CLASFIS, 2, 2) $ '20,90' .And. AllTrim(cCFOP) $ cCteosCFOP
		...
		Else 
			...
			IF SA1->A1_EST $ cEstICMSD
				aBenef := tagcBenef(xFilial('SD2'), cNota, cSerie, cCliente, cLoja )
				If !Empty(aBenef[1])
					cString += '<vICMSDeson>'+aBenef[2]+'</vICMSDeson>'
					cString += '<cBenef>'+aBenef[1]+'</cBenef>'
				EndIf
			EndIF 
	...
	/* IBSCBS */
	cDocumentItemId := GetAdvFVal('SD2','D2_IDTRIB', SF2->(F2_FILIAL + F2_DOC + F2_SERIE + F2_CLIENTE + F2_LOJA), 3  ) //D2_FILIAL+D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_COD+D2_ITEM                                                                                                     

	if !Empty(cDocumentItemId) .And. findClass('totvs.protheus.backoffice.fiscal.integration.taxinformation')
		cString += GetXmlRefTri(cDocumentItemId)
	endif		
...
Static Function GerLeiTr(cNota,cSerie,cCliente,cloja) 
	...
	IF nRetencoes > 0 
		cRet += space(2) + "Impostos de Contribuição Social: " 

		IF nRetPIS > 0
			cRet += "PIS:" + space(1)
			cRet += Transform(nRetPIS,'@E 999,999,999.99')
			cRet += " " + Alltrim(SuperGetMv( "MV_CTMSPIS", .F., "") ) + " / "	
		ENDIF

		IF nRetCOFINS > 0			
			cRet += "COFINS:" + space(1) 
			cRet += Transform(nRetCOFINS,'@E 999,999,999.99')
			cRet += " " + Alltrim(SuperGetMv( "MV_CTMSCOF", .F., "" ) ) + " / "
		ENDIF

		IF nValCsll > 0
			cRet += "CSLL:" + space(1)
			cRet += Transform(nValCsll,'@E 999,999,999.99')
			cRet += " " + Alltrim(SuperGetMv( "MV_CTMSCSL", .F., "Retenção conforme IN/SRF Nº 459, DE 17 DE OUTUBRO DE 2004") ) + "  / "
		ENDIF

		If nValINS > 0			
			cRet += "INSS:" + space(1)
			cRet += Transform(nValINS,'@E 999,999,999.99')
			cRet += " " + Alltrim(SuperGetMv( "MV_CTMSINS", .F., "Retenção conforme IN RFB971/09 art 122") ) + "  / "
		EndIf

		If nValIRR > 0			
			cRet += "IR:" + space(1)
			cRet += Transform(nValIRR,'@E 999,999,999.99')
			cRet += " " + Alltrim(SuperGetMv( "MV_CTMSIR", .F., "Retenção conforme Decreto Estadual nº 129, de 10 de maio de 2023") ) + "  "
		EndIf
...
Static Function XmlCTeQrC(cModalidade,cDoc_Chv,cAmbiente)
	...
	ElseIf cEstado $ 'MG'
		If cAmbiente == '2' //HOMOLOGAÇÃO
			cURLQrc := 'https://hportalcte.fazenda.mg.gov.br/portalcte/sistema/qrcode.xhtml'
		Else			
			cURLQrc := 'https://portalcte.fazenda.mg.gov.br/portalcte/sistema/qrcode.xhtml'
		EndIf
	ElseIf cEstado $ 'MS'
		If cAmbiente == '2' //HOMOLOGAÇÃO
...
Default cLojDev   := ''
	...
	If (cAliasQry)->(!Eof())
		cCod := (cAliasQry)->CDV_CODAJU 
	EndIf
	(cAliasQry)->(DbCloseArea())

	cQuery := "SELECT SUM(CD2_DESONE) CD2_DESONE  FROM " + RetSqlName("CD2") + " "+;
			  " WHERE CD2_FILIAL = '"+ xFilial("CD2") +"' "+;
			  " AND CD2_DOC 	= '"+ cDoc +"' "+;
			  " AND CD2_SERIE 	= '"+ cSerie +"' "+;
			  " AND CD2_CODCLI 	= '" + cCliDev + "' "+;
			  " AND CD2_LOJCLI 	= '" + cLojDev + "' "+;
			  " AND CD2_DESONE 	> 0 "+;
			  " AND D_E_L_E_T_ = ' ' "
	...
	dbUseArea(.T., "TOPCONN", TCGENQRY(,,cQuery), cAliasQry, .F., .T.)
	If (cAliasQry)->(!Eof())
		cVal := TransForm((cAliasQry)->CD2_DESONE,"9999999.99")  
	EndIf
	(cAliasQry)->(DbCloseArea())

	AADD(aRet,ALLTRIM(cCod))
	AADD(aRet,ALLTRIM(cVal))

Return aRet

//-----------------------------------------------------------------------------------------------------
/*/{Protheus.doc} GetXmlRefTri(cDocumentItemId)

Monta Grupo CBS /IBS conforme a nova reforma tributária 
Dados estraidos do configurador de Tributos
@return cString   XML com a Identificaçao da CTeOS
...
/*/
//------------------------------------------------------------------------------------------------------
Static Function GetXmlRefTri(cDocumentItemId)
	Local cXML               := ''
	Local nBaseIbsCbs        := 0
	Local cCst               := ''
	Local cClassTrib         := ''
	Local nValTotDFe         := 0
	Local aId                := {cDocumentItemId}
	Local cDocumentItemTaxes := ''

    Local aIdsTrib           := {}
    Local cIdTotvsTrib       := ''
    Local i                  := 0

	Local oDocumentData      := nil
    Local oDocumentItemId 	 := JsonObject():New() as json
    Local oTaxByTotvsId      := JsonObject():New() as json
	Local oJsIbsEstadual  	 := Nil 
	Local oJsIbsMunicipal    := Nil 
	Local oJsCbs             := Nil

	Local nAliquotaTributo   := 0
	Local nValorTributo      := 0
	Local nTotalIbs          := 0
	Local lGeraXML           := .T.
	
	//Instacia classe e seta o id do item do documento
	oDocumentData := totvs.protheus.backoffice.fiscal.tciclass.TCIWritten():New()
	oDocumentData:setId(aId)
	
	//Recupera e formata os dados do id informado
	cDocumentItemTaxes := oDocumentData:getDataId()
	cDocumentItemId := aId[1]
	...
	for i := 1 to Len(aIdsTrib)
		...
		If !oTaxByTotvsId[cDocumentItemId]:HasProperty(cIdTotvsTrib)
			oTaxByTotvsId[cDocumentItemId][cIdTotvsTrib]:= JsonObject():New()
			oTaxByTotvsId[cDocumentItemId][cIdTotvsTrib]:= oDocumentItemId['dados_Id'][cDocumentItemId][aIdsTrib[i]]
		EndIf 
	next  		

	if lGeraXML .And. oTaxByTotvsId[cDocumentItemId]:HasProperty(IBS_ESTADUAL) .or. ;
		oTaxByTotvsId[cDocumentItemId]:HasProperty(IBS_MUNICIPAL)

		oJsIbsEstadual  := oTaxByTotvsId[cDocumentItemId][IBS_ESTADUAL]
		oJsIbsMunicipa	:= oTaxByTotvsId[cDocumentItemId][IBS_MUNICIPAL]
		...
		oJsCbs := oTaxByTotvsId[cDocumentItemId][CBS_FEDERAL]

		if Empty(cCst) .and. Empty(cClassTrib)
			cCst := Iif(!Empty(oJsCbs['dados_escriturados']['cst']),oJsCbs['dados_escriturados']['cst'],'000')

			//Proteção para o caso de não existir o campo CJ3_CCT
			if lExistCJ3_CCT
				cClassTrib := Iif(!Empty(oJsCbs['dados_escriturados']['cclasstrib']),oJsCbs['dados_escriturados']['cclasstrib'],'000001')
			endif
		endif		

		cXML += '<IBSCBS xmlns="http://www.portalfiscal.inf.br/cte">'
		cXML += '<CST>' + cCst + '</CST>'
		cXML += '<cClassTrib>' + cClassTrib + '</cClassTrib>'
		cXml += '<gIBSCBS>'
		cXml += '<vBC>' + ltrim(str(nBaseIbsCbs,16,2)) + '</vBC>'
		cXml += '<gIBSUF>'
		If oJsIbsEstadual <> Nil
			nAliquotaTributo := oJsIbsEstadual['aliquota_tributo']
			nValorTributo    := oJsIbsEstadual['valor_tributo']
		Else
			nAliquotaTributo := 0
			nValorTributo    := 0
		EndIF 
		...
		cXml += '</gIBSUF>'
		If oJsIbsMunicipa <> Nil
			nAliquotaTributo := oJsIbsMunicipa['aliquota_tributo']
			nValorTributo    := oJsIbsMunicipa['valor_tributo']
		Else
			nAliquotaTributo := 0
			nValorTributo    := 0
		EndIF 
		...
		cXml += '</gIBSMun>'
		cXml += '<vIBS>' + lTrim(str(nTotalIbs,16,2)) + '</vIBS>'
		if oJsCbs != nil
			nAliquotaTributo := oJsCbs['aliquota_tributo']
			nValorTributo    := oJsCbs['valor_tributo']
		Else
			nAliquotaTributo := 0
			nValorTributo    := 0
		endif
		...
		cXml += '</gIBSCBS>'
		cXml += '</IBSCBS>'
		nValTotDFe := SF2->F2_VALBRUT + nTotalIbs
		cXml += '<vTotDFe>' + ltrim(str(nValTotDFe,16,2)) +'</vTotDFe>'
	EndIf 

//Libera os recursos utilizados pela instância da classe
FreeObj(oDocumentData)
oDocumentData := Nil
FreeObj(oDocumentItemId)
oDocumentItemId := Nil
