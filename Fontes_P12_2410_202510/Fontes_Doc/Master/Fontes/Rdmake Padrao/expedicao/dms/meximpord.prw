...
User Function MEXIMPORD()
	Local oReport
	Private cNroOR := ParamIXB[1]
	Private cDesc := "Impresión de la orden de reparación"
	Private cPerg := ""
	Private cNombreInf := "MEXIMPORD"
	Private cTitulo := "MEXIMPORD"
...
Static Function RptDefIO()
	...
	TRCell():New(oSecEmpres, "M0_CGC", "SM0", "RFC", , 25)
	...
	TrCell():New(oSecClient, "A1_TIPO", "SA1", "Tipo",, 25)
	TrCell():New(oSecClient, "A1_CGC", "SA1", "RFC", , 25)
	...
	// Partes Pendientes
	oSecPendie := TRSection():New(oReport, "Partes Pendientes")
	TrCell():New(oSecPendie, "BM_GRUPO"  , "SB1", "Grupo",, 30)
	TrCell():New(oSecPendie, "B1_CODITE" , "SB1", "Cod Item",, 30)
	TrCell():New(oSecPendie, "B1_DESC"   , "SB1", "Descripcion",, 30)
	TrCell():New(oSecPendie, "VO3_QTDREQ", "VO3", "Cant Solicit",, 30)
	TrCell():New(oSecPendie, "VO3_VALPEC", "VO3", "Vl Pieza Ne", PesqPict("VO3", "VO3_VALLIQ"), 30)
	TrCell():New(oSecPendie, "VO3_VALLIQ", "VO3", "Vl Pieza", PesqPict("VO3", "VO3_VALLIQ"), 30,,, "RIGHT")
	oSecPendie:SetLinesBefore(1)

	// Valor total de las partes pendientes
	oSecTotPen := TRSection():New(oReport, "Valor total de las partes pendientes")
	TrCell():New(oSecTotPen, "VO3_VALPEC", "VO3", "Subtotal Piezas", PesqPict("VO3", "VO3_VALLIQ"), 25,,, "CENTER",,,,,,,, .T.)
	TrCell():New(oSecTotPen, "VZ1_VALDES", "VZ1", "Descuentos", PesqPict("VO3", "VO3_VALLIQ"), 25,,, "CENTER",,,,,,,, .T.)
	TrCell():New(oSecTotPen, "VO3_VALLIQ", "VO3", "Total Piezas", PesqPict("VO3", "VO3_VALLIQ"), 30,,, "CENTER",,,,,,,, .T.)
	oSecTotPen:SetLinesBefore(1)
	oSecTotPen:SetLineStyle()
...
Static Function ImpInforme(oReport)
	...
	Local oSecTotPen := oReport:Section("Valor total de las partes pendientes")
	...
	// impresión de piezas pendientes
	aOSTTPCP := piezasPend(cNroOR, cTipTem)
	...
	If Len(aOSTTPCP) > 0
		...
		oSecTitulo:Init()
		oSecTitulo:Cell("IZQUIERDA"):SetValue("")
		oSecTitulo:Cell("TITULO"):SetValue("****** P A R T E S  P E N D I E N T E ******")
		oSecTitulo:Cell("DERECHA"):SetValue("")
		oSecTitulo:PrintLine()
		oSecTitulo:Finish()
		...
		nLen := Len(aOSTTPCP)
		For nCntFor := 1 To nLen
			If aOSTTPCP[nCntFor, 5] > 0
				oSecProduc:Cell("BM_GRUPO"):SetValue(aOSTTPCP[nCntFor, 1])
				oSecProduc:Cell("B1_CODITE"):SetValue(aOSTTPCP[nCntFor, 2])
				oSecProduc:Cell("B1_DESC"):SetValue(aOSTTPCP[nCntFor, 3])
				oSecProduc:Cell("VO3_QTDREQ"):SetValue(aOSTTPCP[nCntFor, 4])
				oSecProduc:Cell("VO3_VALPEC"):SetValue(aOSTTPCP[nCntFor, 5])
				oSecProduc:Cell("VO3_VALLIQ"):SetValue(aOSTTPCP[nCntFor, 6])
				oSecProduc:PrintLine()
				nTotParPen += aOSTTPCP[nCntFor, 6]
			EndIf
		Next
		oSecProduc:Finish()
		
		oSecTotPen:Init()
		oSecTotPen:Cell("VO3_VALPEC"):SetValue(nTotParPen)		
		oSecTotPen:Cell("VO3_VALLIQ"):SetValue(nTotParPen)//nTotPartes
		oSecTotPen:PrintLine()
		oSecTotPen:Finish()
	EndIf
	...
	oSecTotGen:Cell("TOT3"):SetValue(nTotServic + nTotPartes + nTotParPen)
...
/*{Protheus.doc} piezasPend
	Esta función tiene como objetivo devolver piezas pendientes con saldo en la orden de servicio.
@author	MIL - Maza Informática Ltda.
@type	function
@since	18/09/2025
@param	cNroOR, Número de la orden de reparación.
@param	cTipTem, character, codigo tipo de tempo.
@return	Array, Datos de piezas pendientes en el sistema operativo
*/
Static Function piezasPend(cNroOR, cTipTem)
	Local aArea   := FWGetArea()
	Local aRet    := {}
	Local cFiltro := ""
	...
	cFiltro += "%"	
	If !Empty(cTipTem)
		cFiltro += " AND VSJ.VSJ_TIPTEM = '"+cTipTem+"' " 
	EndIf	
	cFiltro += "%"	

	BeginSQL Alias "PIEZAPEND"
		SELECT VSJ.VSJ_GRUITE,
			   VSJ.VSJ_CODITE,
			   SB1.B1_DESC,
			   VSJ.VSJ_QTDITE,
			   VSJ.VSJ_VALPEC,
			   (VSJ_QTDITE*VSJ_VALPEC) TOT_PIEZA
		FROM %table:VSJ% VSJ
		JOIN %table:SB1% SB1 ON SB1.B1_FILIAL = %xFilial:SB1%
					   AND SB1.B1_GRUPO = VSJ.VSJ_GRUITE
					   AND SB1.B1_CODITE = VSJ.VSJ_CODITE
					   AND SB1.%notDel%
		WHERE VSJ.VSJ_FILIAL = %xFilial:VSJ%
		AND VSJ.VSJ_NUMOSV = %exp:cNroOR%
		AND VSJ.VSJ_QTDITE > 0
		AND VSJ.%notDel%		
		%exp:cFiltro%
		ORDER BY VSJ.VSJ_NUMOSV,
				 VSJ.VSJ_TIPTEM,
				 VSJ.VSJ_GRUITE,
				 VSJ.VSJ_CODITE
	EndSQL
	...
	While !PIEZAPEND->(EOF())
		...
		AAdd(aRet, {PIEZAPEND->VSJ_GRUITE, PIEZAPEND->VSJ_CODITE, PIEZAPEND->B1_DESC, PIEZAPEND->VSJ_QTDITE, PIEZAPEND->VSJ_VALPEC, PIEZAPEND->TOT_PIEZA})

		PIEZAPEND->(DbSkip())
	EndDo

	PIEZAPEND->(DBCloseArea())
