...
If EasyEntryPoint("EICPO557")
   ExecBlock("EICPO557",.F.,.F.,lEntrou)
Else
...
IndRegua("Work",FileWork+TEOrdBagExt(),"WKFABR + WKCOD_I",;
         "AllwaysTrue()",;
         "AllwaysTrue()",;
...
WHILE .T.
   ...
   oDlg:= oSend( MSDialog(), "New", 6.5, 0, 33.5, 80,;
                 cCadastro,,,.F.,,,,,oMainWnd,.F.,,,.F.)

    oFntDlg := NIL
    DEFINE FONT oFntDlg  NAME "Ms Sans Serif" SIZE 0,-9

    oPanel:= TPanel():New(0, 0, "", oDlg,, .F., .F.,,, 90, 165) //MCF - 08/07/2015
    ...
    nLin:=1.4 ; nColS1:=1.0  ; nColG1:=7.0 ; nColS2:=17.0 ; nColG2:=21.0
      ...
      oSend(TSay(),"New",nLin         ,nColS1,{|| OemToAnsi(STR0011)},oPanel,,oFntdlg,,,,.F. ) //"Nro. do P.O.:"
      oSend(TGet(),"New",nLin         ,nColG1 ,bSetGet(TPO_Num)   ,oPanel,60,8,;
   ...
   V:=oSend(TGet(),"New",nLin         ,nColG2 ,bSetGet(TImport)   ,oPanel,10,8,;
      ...
      oSend(TGet(),"New",nLin         ,nColG1 ,bSetGet(TNr_Pro)   ,oPanel,60,8,;
                                       AVPict("W2_NR_PRO"),{||MFLAG:="Nr_Pro", PO557Val()    },,,oFntDlg,,,.F.,,,,,,,,,)


      oSend(TSay(),"New",nLin         ,nColS2,{|| STR0015                 },oPanel,,oFntdlg,,,,.F. ) //"Date "
      oSend(TGet(),"New",nLin         ,nColG2 ,bSetGet(TDt_Pro)   ,oPanel,42,8,;
   ...
   V:=oSend(TGet(),"New",nLin         ,nColG1 ,bSetGet(TPais)   ,oPanel,60,8,,,,,oFntDlg,,,.F.,,,,,,,,,)
      oSend(V,"DISABLE")
   ...
   V:=oSend(TGet(),"New",nLin         ,nColG2 ,bSetGet(TTipo_Emb)   ,oPanel,10,8,;
      ...
      If !lNewProforma
         ...
         oSend(TSay(),"New",5.5,nColS1,{|| STR0020 },oPanel,,oFntdlg,,,,.F. ) //"Observação.:"
         oSend(TGet(),"New",70 ,56 ,bSetGet(aMsg[1]) ,oPanel,190,10,,,,,oFntDlg,,,.T.,,,,,,,,,)
         oSend(TGet(),"New",85 ,56 ,bSetGet(aMsg[2]) ,oPanel,190,10,,,,,oFntDlg,,,.T.,,,,,,,,,)
         oSend(TGet(),"New",100,56 ,bSetGet(aMsg[3]) ,oPanel,190,10,,,,,oFntDlg,,,.T.,,,,,,,,,)
         oSend(TGet(),"New",115,56 ,bSetGet(aMsg[4]) ,oPanel,190,10,,,,,oFntDlg,,,.T.,,,,,,,,,)
         oSend(TGet(),"New",130,56 ,bSetGet(aMsg[5]) ,oPanel,190,10,,,,,oFntDlg,,,.T.,,,,,,,,,)
      Else
         oSend(TSay(),"New",5.5,nColS1,{|| STR0020 },oPanel,,oFntdlg,,,,.F. ) //"Observação.:"
         oSend(TGet(),"New",70 ,56 ,bSetGet(cMsg1) ,oPanel,190,10,,,,,oFntDlg,,,.T.,,,,,,,,,)
         oSend(TGet(),"New",85 ,56 ,bSetGet(cMsg2) ,oPanel,190,10,,,,,oFntDlg,,,.T.,,,,,,,,,)
         oSend(TGet(),"New",100,56 ,bSetGet(cMsg3) ,oPanel,190,10,,,,,oFntDlg,,,.T.,,,,,,,,,)
         oSend(TGet(),"New",115,56 ,bSetGet(cMsg4) ,oPanel,190,10,,,,,oFntDlg,,,.T.,,,,,,,,,)
         oSend(TGet(),"New",130,56 ,bSetGet(cMsg5) ,oPanel,190,10,,,,,oFntDlg,,,.T.,,,,,,,,,)
      Endif
...
WHILE !SW3->(EOF()) .AND. SW3->W3_PO_NUM==TPO_Num .AND. SW3->W3_FILIAL==xFilial("SW3")
   ...
   MSProcTXT(STR0041+Transform(SW3->W3_COD_I,_PictItem)) //"Processamento Item   " //MCF - 08/07/2015
...
*--------------------------*
LOCAL cMoeDolar:=EasyGParam("MV_SIMB2",,"US$"), W, I
LOCAL nProfTot:= 0  
LOCAL nPesoPO:= 0 
LOCAL nFreteProf:= 0 
LOCAL nInlandProf:= 0 
...
AVPRINT oPrn NAME STR0043 //"Proforma Invoice"
   oPrn:Rebuild()
   ProcRegua(5+WORK->(Easyreccount("Work")))
   ...
   AVPAGE
        ...
        WHILE !SW3->(EOF()) .AND. SW3->W3_FILIAL==xFilial("SW3") .AND. ;
           ...
           PO557VERFIM()
           SB1->(DBSEEK(xFilial("SB1")+SW3->W3_COD_I))
           lSeekSA5 := SA5->(DBSEEK(xFilial("SA5")+SW3->W3_COD_I+SW3->W3_FABR+SW3->W3_FORN))
           lSeekSYG := SYG->(DBSEEK(xFilial("SYG")+SW2->W2_IMPORT+SW3->W3_FABR+SW3->W3_COD_I))
           ...
           If SW3->(FieldPos("W3_PART_N")) # 0 .AND. !EMPTY(SW3->W3_PART_N) //ASK 08/10/2007 
              PARTE2:=2
              PO557VERFIM()
              oPrn:Say( nLinha:=nLinha+50,420 ,ALLTRIM(SW3->W3_PART_N),aFontes:TIMES_NEW_ROMAN_12,,,1)
           Else               
              IF lSeekSA5 .and. !EMPTY(ALLTRIM(SA5->A5_CODPRF))
                 PARTE2:=2
                 PO557VERFIM()
                 oPrn:Say( nLinha:=nLinha+50,420 ,ALLTRIM(SA5->A5_CODPRF),aFontes:TIMES_NEW_ROMAN_12,,,1)
              EndIf   
           ENDIF
           IF lSeekSA5 .and. ! EMPTY(ALLTRIM(SA5->A5_PARTOPC))
              ...
              NEXT
           ENDIF
           IF lSeekSYG .and. ! EMPTY(ALLTRIM(SYG->YG_REG_MIN))
              PARTE2:=2
              PO557VERFIM()
