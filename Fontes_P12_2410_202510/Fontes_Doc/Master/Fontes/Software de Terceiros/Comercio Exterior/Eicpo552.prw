...
If EasyEntryPoint("EICPO552")
   ExecBlock("EICPO552",.F.,.F.)
   Return
EndIf
   ...
   */
   ...
   bFlag := {||PO552Wk1Grv()}
   Processa({|lEnd|ProcRegua(SW5->(Easyreccount("SW5"))),;
   SW5->(DBEVAL(bFlag,,bWhile))},STR0017) //"Pesquisando Informações..."

   nOpca:=0

   aButtons := {}
   AAdd (aButtons, {"ALTDATAS", {|| PO552AterData(),oMark:oBrowse:Refresh(),oMark:oBrowse:Reset(),SysRefresh()} , STR0018 })
   AAdd (aButtons, {"MENSAGENS", {|| IF(PO552Verifica(),(nOpca:=2,oDlg:End()),)} , STR0019 })
   AAdd (aButtons, {"Salva Modelo", {|| PO552GravaMsg()} , STR0129})
   AAdd (aButtons, {"TODOS", {|| PO552AllMark()} , STR0020 })
   AAdd (aButtons, {"IMPRIMIR", {|| PO552ProcRel() } , STR0128 })
   ...
   IF Work1->(Easyreccount("Work1")) > 0
      DO WHILE .T.
         ...
         ACTIVATE MSDIALOG oDlg ON INIT ;
                  (EnchoiceBar (oDlg,;
                                 {||nOpca:=0,oDlg:End()},;//FSY - 10/08/2012
                                 {||nOpca:=0,oDlg:End()},;
                                 ,;
                                 aButtons)) //LRL 31/03/04 -Alinhamento MDI   //BCO 13/12/11 - Tratamento para acesso via ActiveX alterando o align para antes do INIT
            IF nOpca == 0
               DELMENSAGEM
               E_RESET_AREA
               Return .F.
...
DO WHILE .T.
   ...
   DEFINE MSDIALOG oDlg TITLE cCadastro From 9,0 To 24,50 OF oMainWnd //OF PIXEL
   ...
   nLin := nLin + 1
   @ nLin,nColS SAY STR0039 SIZE 128,8 OF oPanel PIXEL //"Data de Embarque"
   MFLAG := "DatEmb"
   @ nLin,nColG GET dDataEmb VALID PO552Val() SIZE 40,8 OF oPanel PIXEL// Substituido pelo assistente de conversao do AP5 IDE em 24/11/99 ==>    @ nLin,nColG GET dDataEmb VALID Execute(PO552Val) SIZE 32,8

   nLin := nLin + 15
   @ nLin,nColS SAY STR0040 SIZE 128,8 OF oPanel PIXEL //"Data de Entrega"
   MFLAG := "DatEnt"
   @ nLin,nColG GET dDataEnt VALID PO552Val() SIZE 40,8 OF oPanel PIXEL    //32
...
AVPRINT oPrn NAME STR0043 //"EmissÒo de InstruþÒo de Embarque / Shipping Instructions"
   ...
   AVPAGE
        ...
        DO WHILE ! Work1->(EOF())
           ...
           mDescri:= Alltrim(mDescri) + if( lSeekSA5, " " + Alltrim(SA5->A5_PARTOPC), "")
        ...
        //ASK 15/10/2007 - Tratamento para Embarque com mais de um P.O.
           ...
           oPrn:Say( nLinha    ,490  ,memoline(aItens[j][3],22,1),ofont1)//FDR - 27/09/12 - Alterado quantidade de caracter por linha //FSY - 06/09/2013 - Ajustada tamanho da descriçao para quebra de linha
          ...
          FOR W:=2 TO MLCOUNT(/*mDescri*/aItens[j][3],22)                  // GFP - 12/08/2013 FSY - 06/09/2013 - Ajustada tamanho da descriçao para quebra de linha
              IF ! EMPTY(ALLTRIM(memoline(/*mDescri*/aItens[j][3],22,W)))  // GFP - 12/08/2013 FSY - 06/09/2013 - Ajustada tamanho da descriçao para quebra de linha
                 Parte2 := 3
			     PO552VERFIM()
			     BateTraco()
                 oPrn:Say( nLinha,490  ,memoline(/*mDescri*/aItens[j][3],22,W), aFontes:TIMES_NEW_ROMAN_12)  // GFP - 12/08/2013//FSY - 06/09/2013 - Ajustada tamanho da descriçao para quebra de linha
                 nLinha := nLinha + 50
              ENDIF
           NEXT
           nLinha := nLinha + 10
...
//FSM - 15/06/2011
If Select("Work_Men") == 0
   MsgInfo(STR0130) //Sem modelo disponível para salvar
   Return Nil
EndIf

ProcRegua(Work_Men->(EasyRecCount("Work_Men")))

cCodigo := WORK_MEN->WKORDEM
...
If SY7->(DBSEEK(xFilial('SY7')+'8'+SW2->W2_PO_NUM))
   SY7->(Reclock("SY7",.F.))
Else
   SY7->(Reclock("SY7",.T.))
Endif
SY7->Y7_COD:= Right(cCodigo, AVSX3("Y7_COD" ,3))
SY7->Y7_FILIAL := xFilial('SY7')
SY7->Y7_POGI:= "8"
