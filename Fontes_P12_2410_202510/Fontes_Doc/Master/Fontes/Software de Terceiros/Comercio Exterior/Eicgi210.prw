...
Function Eicgi210()        // incluido pelo assistente de conversao do AP5 IDE em 16/02/00
...
If EasyEntryPoint("EICGI210")
   ExecBlock("EICGI210",.F.,.F.)
Else
...
IF nAnuente == 1
   IndRegua("Work1",cFile1+TEOrdBagExt(),"W5_PO_NUM+W5_CC+W5_SI_NUM+W5_COD_I+Str(W5_REG,"+Alltrim(Str(AVSX3("W5_REG",3)))+")")
ELSE
   IndRegua("Work1",cFile1+TEOrdBagExt(),"W7_PO_NUM+W7_CC+W7_SI_NUM+W7_COD_I+Str(W7_REG,"+Alltrim(Str(AVSX3("W7_REG",3)))+")")
ENDIF
Set Index To (cFile1+TEOrdBagExt())

IndRegua("Work2",cFile2+TEOrdBagExt(),"WV_LOTE")
Set Index To (cFile2+TEOrdBagExt())
...
DO While .T.
   ...
   IF nAnuente ==1 
      @ 40,75 GET cPGI_NUM F3 "SW4"  PICTURE "@!" SIZE 55,8   ; //LRS - 06/02/2015 - Correção da dimensão do MsDialog e do CPGI_NUM
               VALID GI210ValPGI() 
   ELSE
...
aAdd(aButtons,{"EDIT",{||(nOpcSW5:=1,oSend(oDlg,'End'))},STR0018})// LRL 13/02/04  //"Manutenção Lote"
...
Do While (.T.)
   ...
   /*If Empty (cProcesso)  // Nopado por GFP  25/06/2015 - Este trecho forçava o preenchimento do campo WV_HAWB na inclusão do lote, não sendo possivel associa-lo posteriormente.
...
If !IsMemVar("nSaldo")
   nSaldo := 0
EndIf
if EasyEntryPoint("EICGI21A")
   lRet := ExecBlock("EICGI21A",.F.,.F.,"WV_QTDE")
endif
