...
Function Eicpo558()
*---------------------------*
If EasyEntryPoint("EICPO558")
   ExecBlock("EICPO558",.F.,.F.)
Else

   nLinha := 0
   nSpread:=0
   If EasyEntryPoint("EICPO57S")
   ...
   DEFINE MSDIALOG oDlg TITLE "Emissão da Commercial Invoice" FROM 8, 0 TO  35, 80 //"Emissão da Comercial Invoice"
   ...
   DEFINE SBUTTON FROM 35,260 TYPE 6 ACTION (nOpcao:=1,oDlg:End()) ENABLE OF oDlg PIXEL
...
nInvoice:=EasyGParam("MV_INVOICE")
cTexto:=""
cEndeImport:=""
  ...
  IncProc( STR0010 ) //"Imprimindo..."
    ...
    Do while ! SW9->(EOF()) .AND. SW9->W9_FILIAL == xFilial("SW9") .AND. SW9->W9_HAWB == cHouse    
       ...
       Do while ! SW8->(EOF()) .AND. SW8->W8_FILIAL == xFilial("SW8") .AND. SW8->W8_INVOICE == SW9->W9_INVOICE .AND. SW8->W8_FORN == SW9->W9_FORN
          ...
          oPrn:Say( nLinha ,1710 ,trans(SW8->W8_PRECO/**SW2->W2_PARID_U*/,AVSX3("W8_PRECO",6)),aFontes:TIMES_NEW_ROMAN_12,,,,1) //MCF - 29/01/2016
