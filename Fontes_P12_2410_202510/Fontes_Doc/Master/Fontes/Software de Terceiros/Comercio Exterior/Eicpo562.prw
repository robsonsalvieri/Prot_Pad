...
If EasyEntryPoint("EICPO562")
   ExecBlock("EICPO562",.F.,.F.)
Else
   ...
   WHILE .T.
      ...
      nOpcao:=0
      oDlgTelaPrincipal:= oSend( TDialog(), "New", 120, 100, 290, 490,;
      ...
      DEFINE FONT oFntDlg  NAME "Ms Sans Serif" SIZE 0,-9
         ...
         oPanel:= TPanel():New(0, 0, "", oDlgTelaPrincipal,, .F., .F.,,, 90, 165) //MCF - 16/07/2015
         oPanel:Align:= CONTROL_ALIGN_ALLCLIENT
      
         oSend(TSay(),"New",25, 10 ,{|| STR0002},oPanel,,oFntDlg,,,,.T. ) //"Nº do P.O.:"
         @25,50 MSGET cPoNum Picture AVPict("W2_PO_NUM");
                              Valid {|| PO562Val()} Size 40,08 OF oPanel PIXEL F3 "SW2"
                              
   //        oSend(TGet(),"New",25, 50 ,bSetGet(cPoNum)   ,oDlgTelaPrincipal,,8,;
   //                      AVPict("W2_PO_NUM"),{|| PO562Val()},,,oFntDlg,,,.T.,,,,,,,,,"SW2")

         oSend( SButton(), "New", 25, 145 ,6 ,{||nOpcao:=1,oSend(oDlgTelaPrincipal,"End") }, oPanel, .T.,,)
...
AVPRINT oPrn NAME STR0007 //"Proposta de Importacao"
   ...
   AVPAGE
      ...
      ELSE
         ...
         oPrn:Say( Linha     , 725  , TRANS(SA2->A2_REPRCGC,TEgetCnpj("A2_REPRCGC")),aFontes:TIMES_NEW_ROMAN_12 )
      ...
      Linha := Linha+70
      oPrn:Say( Linha, 25, IF(EasyGParam("MV_ID_CLI")$cSim,SA1->A1_NOME,SY1->Y1_NOME), aFontes:TIMES_NEW_ROMAN_12 )
      IncProc(STR0008) //"Imprimindo..."
...
//-----------> Fornecedores.
SA2->( DBSETORDER( 1 ) )
lSeekSA2 := SA2->( DBSEEK( xFilial()+SW3->W3_FABR+EICRetLoja("SW3","W3_FABLOJ") ) )
...
oPrn:Say( Linha,1185, if( lSeekSA2, LEFT(SA2->A2_NREDUZ,10), "")     , aFontes:TIMES_NEW_ROMAN_08_NEGRITO )
...
FOR i:=1 TO n1 + 1   // Soma um para bater o ultimo.
    ...
    IF EMPTY( MEMOLINE( cDescrItem,30, i ) )
       IF nNumero == 1
           If SW3->(FieldPos("W3_PART_N")) # 0 .AND. !EMPTY(SW3->W3_PART_N) //ASK 08/10/2007 
              ...
              lPulaLinha := .T.     
           Else   
              If lSeekSA5 .and. !Empty( SA5->A5_CODPRF )
        ...
        ELSEIF nNumero == 2
           If lSeekSA5 .and. !Empty( MEMOLINE(SA5->A5_PARTOPC,24,1) )
        ...
        ELSEIF nNumero == 3
           If lSeekSA5 .and. !Empty( MEMOLINE(SA5->A5_PARTOPC,24,2) )
        ...
        ELSEIF nNumero == 4
           If lSeekSYG .and. !Empty( SYG->YG_REG_MIN )
    ...
    IF lSeekSA2 .and. !EMPTY( RIGHT(SA2->A2_NREDUZ, 10 ) )  .AND.  i == 2
