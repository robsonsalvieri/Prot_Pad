...
Function MDTA081()
	...
	Private cCadastro := OemtoAnsi(STR0001) //"Grupo de Doenças (CID)"
	Private aCHKDEL := {}
	Private aChkSql := {}
	Private bNGGRAVA
...
Function MDTGRUPCID ()
	...
	Local aGrupo :=  {  {'A00','Cólera'},;
						...
						{'U07','Diagnóstico de doença respiratória aguda pelo 2019-nCoV [COVID-19]'},;
...
Static Function MenuDef()
	...
	Local aRotina :=	{ 	{STR0002, "AxPesqui" , 0, 1},; //"Pesquisar"
							...
							//Removido, site datasus não disponibiliza mais a informação
...
Function MDT081IMP()
	...
	Local nPos
	Local lRetCopy := .F.
	Local cMascara := STR0008 //"CSV(delimitado por vírgula)     | *.CSV"
	Local cTela
	Local cArqLoc := ""
	Private cArquivo := ""
	...
	cTela := STR0009 + Chr(13); //"Esta rotina tem como objetivo importar as Categorias do C.I.D. "
			 ...
			 + STR0011 + Chr(13); //"Antes de confirmar a execução do processo, fazer uma cópia de segurança da tabela TLG."
			 + STR0012 + Chr(13) + Chr(13); //"Caso ocorra algum problema durante o processo as cópias de segurança deverão ser restauradas."
			 + STR0013 + Chr(13) + Chr(13); //"Este processo poderá levar algum tempo para ser executado."
			 + STR0014 //"Deseja efetuar o processamento?"

	If !MsgYesNo(cTELA, STR0015) //"Atenção"
		Return .T.
	EndIf
	...
	If !File(cPath + cArqLoc)
		MsgStop(STR0017 + cArquivo + STR0018 + Chr(13) + Chr(13) + STR0019) //"Arquivo "###" não encontrado."###"O processo sera cancelado."
		Return .F.
	EndIf
...
Static Function f81ImpCsv()
	...
	While (!FT_FEof())
		...
		If nRecno == 1 //Verifica Cabecalho
			...
			If nPosGrupo == 0 .or. nPosDescri == 0
				cTela := STR0022 + Chr(13); //"O arquivo informado para a importação é inválido."
						 + STR0023 + Chr(13); //"Para o processamento é necessário que o cabeçalho(Primeira Linha) "
						 + STR0024 + cGruField + STR0025 + Chr(13); //"do arquivo esteja definindo os campos: '"###"' para o código do Grupo. e "
						 + "'" + cDesField + STR0026 //"' para a descrição do Grupo."
				MsgStop(cTELA, STR0015) //"Atenção"
				Return .F.
			EndIf
			//Pula Linha
			FT_FSKIP()
	...
	MsgInfo(STR0027) //"Importação concluída com sucesso!"
