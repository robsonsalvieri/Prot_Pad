...
//################################################################
Private cCadastro  := STR0001 //"Laudos x Famílias de Equipamentos"
...
Else
	aRotina :=  { { STR0002, "AxPesqui"  , 0 , 1},;   //"Pesquisar"
                  { STR0003, "NGCAD01"   , 0 , 2},;   //"Visualizar"
                  { STR0005, "MDT219EQ"  , 0 , 4, 3} } //Famílias
Endif
...
Monta um browse das Famílias de Equipamentos.
...
DEFINE MSDIALOG oDlgFam TITLE STR0001 From aSize[7],0 To aSize[6],aSize[5] OF oMainWnd PIXEL //"Laudos x Famílias de Equipamentos"
	...
	aSort(oGetFam:aCOLS,,,{ |x, y| x[1] < y[1] }) //Ordena por Famílias
...
If nOpca == 1
	fGravaEQ(@oGetFam)//Grava Famílias
Endif
...
AADD(aTRBFA,{ "TRB_CODFAM",NIL,STR0008,})//"Cod. Família"
...
Processa({|lEnd| fBuscaFam( cAliasTRB , oGetFam )},STR0010,STR0011)//"Buscando Famílias..."#"Espere"
...
If (cAliasTRB)->(Reccount()) <= 0
	...
	RestArea(aArea)
	lRefresh := .t.
	Msgstop(STR0012,STR0013) //"Não existem Famílias cadastradas"#"ATENÇÃO" 
	Return .t.
Endif  

nOpcao := 0

DEFINE MSDIALOG oDlgEquip TITLE OemToAnsi(STR0005) From 64,160 To 580,730 OF oMainWnd Pixel  //"Famílias"
	...
	oPnlMSG := TPanel():New(0, 0, Nil, oDlgEquip, Nil, .T., .F., Nil, Nil, 0, 55, .T., .F. )
		...
		TSay():New(19,12,{|| OemtoAnsi(STR0014) },oPnlMSG,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,200,010)//"Estas são as famílias cadastrados no sistema."
...
Funcao para retornar todas as Famílias.
