...
If lRestRepom
	...
	lRet    := VldRepDTR(oModel,cCodOpe,@cGerCTC)
	nOpc	:= oModel:GetOperation()

	If lRet .And. nOpc == 4 .And. cGerCTC != "0"
		lRet := TMSC15ARep( 7 , FwFldGet("DTQ_FILORI"), FwFldGet("DTQ_VIAGEM"), .T. )
	EndIf
...
*/
//-------------------------------------------------------------------
Static Function VldRepDTR( oModel, cCodOpe, cGerCTC )
Local lRet      := .T. 
Local aArea     := GetArea()
...
If nOpc == 3 .Or. nOpc == 4
	...
	cGerCTC := Posicione("DEG",1,FWxFilial("DEG")+FwFldGet("DM5_CODOPE"),"DEG_GERCTC")
