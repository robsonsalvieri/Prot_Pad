		...
		If DUF->(Found())
			...
			While (cAliasNew)->( !Eof() )
				...
				If (Empty(cLinha)) .And. Len(aBusca) == 0
					aRetorno := {(cAliasNew)->DUG_TES,(cAliasNew)->DUG_CODMSG, IIf(lTMSCTRIB, "", M_Posicione('SF4',1,xFilial('SF4') + (cAliasNew)->DUG_TES,'F4_ISS'))}
				EndIf
...
If (cAliasNew)->(!Eof())
	...
	If SF4->(MsSeek(xFilial('SF4')+(cAliasNew)->DUG_TES))
		If	SF4->F4_MSBLQL <> '1' //Somente TES Desbloqueadas podem ser utilizadas.
			...
			aRetorno := { (cAliasNew)->DUG_TES,(cAliasNew)->DUG_CODMSG, IIf(lTMSCTRIB, "", M_Posicione('SF4',1,xFilial('SF4') + (cAliasNew)->DUG_TES,'F4_ISS')), (cAliasNew)->DUG_TES }
		EndIf
	EndIf
EndIf
...
Function TmsRegTPOer(cCliDev,cLojDev,cDocTms,cCodPro,cTipNFC,cTipoCli,cSeqIns)
...
Default cSeqIns  := ""
	...
	DV1->(dbSetOrder(1)) //DV1_FILIAL+DV1_CODCLI+DV1_LOJCLI+DV1_DOCTMS+DV1_CODPRO+DV1_TIPNFC+DV1_TIPCLI+DV1_SEQINS+DV1_REGTRI 
	If DV1->(MsSeek(xFilial("DV1")+cCliDev+cLojDev+cDocTms+cCodPro+cTipNFC+cTipoCli+cSeqIns))
		cRet := DV1->DV1_OPER
	Else
		If DV1->(MsSeek(xFilial("DV1")+cCliDev+cLojDev+cDocTms+cCodPro+cTipNFC+cVzTipCli+cSeqIns))
			cRet := DV1->DV1_OPER
		EndIf
	EndIf
