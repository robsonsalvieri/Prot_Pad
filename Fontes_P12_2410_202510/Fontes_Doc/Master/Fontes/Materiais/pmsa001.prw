...
Aviso(STR0012,STR0013,{STR0014},2) //"Importacao de projetos"###"Atencao! Certifique-se de que o arquivo foi gerado com este formato de data no Microsoft Project (Ferramentas - Opcoes): 31/12/00 12:33"###"Ok"
      
Processa({||A001Import()},STR0015) //"Importando CSV. Aguarde..."
...
Private nPosComp
Private nPosTpVRel
Private cCadastro	:= STR0001 //"Importar arquivo .CSV"
Private aTxt        := {}
Private aParam       := {}
...
If ParamBox({	{6,STR0008,SPACE(50),"","FILE(mv_par01)","", 55 ,.T.,STR0009},;  //"Arquivo"###"Arquivo .CSV |*.CSV"
			    {3,STR0016,1,{STR0017,STR0018,STR0046},70,,.F.},; //"Versao do Project"###"Portugues"###"Ingles" //"Espanhol"
			    {1,STR0011,SPACE(TamSX3("AF8_PROJET")[1]),"","dbSeek(xFilial('AF8')+AllTrim(mv_par03))","AF8","", 85 ,.F.},;
			    {3,STR0069,3,{STR0070,STR0071,STR0072},70,,.F.},;  //"Separador Milhar"##"Ponto"##"Virgula"##"Nao Tem"
			    {3,STR0073,1,{STR0071,STR0070},70,,.F.} ;  //"Separador Decimal"##"Virgula"##"Ponto"
			    },STR0004,@aParam,,{{5,{|| A001CfgCol(aCamposExc)}}}) //"Importar .CSV" //"Atualizar Projeto"
	...
	nPosRecurs := aScan(aCpoImp,"$A001RECURS")
 	...
 	Do Case
	 	Case (nPosTarefa == 0) 
			Aviso(STR0039,STR0052,{STR0041},2) //"Estrutura .CSV Invalida"### "O campo 'código da Tarefa/EDT' é obrigatório existir na estrutura para a importação CSV possa funcionar corretamente."###"Fechar"	 	
			Return
	 	Case (nPosNivel == 0)
			Aviso(STR0039,STR0053,{STR0041},2) //"Estrutura .CSV Invalida"### "O campo 'Nível de estrutura' é obrigatório existir na estrutura para a importação CSV possa funcionar corretamente."###"Fechar"		
			Return
		Case (nPosID == 0)
			Aviso(STR0039,STR0054,{STR0041},2) //"Estrutura .CSV Invalida"### "O campo 'ID' é obrigatório existir na estrutura para a importação CSV possa funcionar corretamente."###"Fechar"		
			Return
		Case (nPosDescri == 0)
			Aviso(STR0039,STR0055,{STR0041},2) //"Estrutura .CSV Invalida"### "O campo 'descrição' é obrigatório existir na estrutura para a importação CSV possa funcionar corretamente."###"Fechar"		
			Return
		Case (nPosDataI == 0)
			Aviso(STR0039,STR0056,{STR0041},2) //"Estrutura .CSV Invalida"### "O campo 'Data Início' é obrigatório existir na estrutura para a importação CSV possa funcionar corretamente."###"Fechar"
			Return
		Case (nPosDataf == 0)
			Aviso(STR0039,STR0057,{STR0041},2) //"Estrutura .CSV Invalida"### "O campo 'Data Final' é obrigatório existir na estrutura para a importação CSV possa funcionar corretamente."###"Fechar"
 			Return
	EndCase
	...
	For nX := 1 To Len(aTxt)
		...
		IncProc(STR0080) //"Analisando os dados..."
		If ValType(aTxt) !='A' .OR. Len(aTxt[nx]) != Len(aCpoImp)
			Aviso(STR0039,STR0043,{STR0041},2) //"Estrutura .CSV Invalida"### "A quantidade de campos do arquivo .CSV difere do configurado. Utilize a Sincronizacao com o MS-Project para importar este projeto."###"Fechar"
			Return
		EndIf
		
       If Len(aTxt[nx][nPosTarefa]) > LEN(AF9->AF9_TAREFA)
			Aviso(STR0039+STR0047+Str(nx),STR0040,{STR0041},2) //"Estrutura .CSV Invalida"###"O sistema nao suporta o tamanho do codigo da estrutura do .CSV . Utilize a Sincronizacao com o MS-Project para importar este projeto."###"Fechar" //"Linha "
			Return
		EndIf
		...
		If (!aTxt[nX][nPosNivel] $ "000")
			...
			If !( (nX == 1 .OR. (nX==2 .AND. (Val(aTxt[nX,nPosNivel]) == 1))) .and. aTxt[nX][nPosNivel] $ "1/001" .and. !Empty(aParam[03]) .and. AllTrim(aTxt[nX][nPosTarefa]) == AllTrim(aParam[03]) ) .AND. Val(aTxt[nX,nPosNivel])<>0
	...
	(AllTrim(aTxt[1][nPosTarefa]) == AllTrim(aParam[03]) .OR. AllTrim(aTxt[2][nPosTarefa]) == AllTrim(aParam[03])) 
		...
		For nX := 1 To Len(aCopyTxt)
			IncProc(STR0081)//"Reorganizando os niveis..."
			//+---------------------------------------------------------+
	...
	If nConfirma == 1
		...
		aEdt	:=	{}		
		For nx := 1 to Len(aTxt)
			IncProc(STR0059+Alltrim(Str(nX))+STR0060+Alltrim(Str(Len(aTxt)) )) //importando linha # de
			cTrfAtu		:= ""
		...
		If nPosPredec > 0
			For nx := 1 to Len(aTxt)
				IncProc(STR0061+Alltrim(Str(nX))+STR0060+Alltrim(Str(Len(aTxt)))) //Analisando predecessoras - Tarefa # de 
				If !Empty(aTxt[nx][nPosTarefa])
					dbSelectArea("AF9")
					dbSetOrder(1)
		...
		IncProc(STR0066) //"Carregando EDTs para atualizar acumulados..."
		...
		If !Empty(aSelectEDT)
			...
			IncProc(STR0067) //"Atualizando acumulados previstos..."
			PmsEdtPrv(AF8->AF8_PROJET, AF8->AF8_REVISA, Padr(AF8->AF8_PROJET,Len(AFC->AFC_EDT)), aSelectEDT)
			IncProc(STR0068) //"Atualizando acumulados realizados..."
...
Function A001CfgCol(aCamposExc)
...
Local lCampos1   := .T.
Local lCampos2   := .F.
Local aFunc		 := { 	{STR0032,"$A001CODIGO"},; //"*Codigo da EDT/Tarefa"
						{STR0033,"$A001NIVEL"},; //"*Nivel da Estrutura de Topicos"
						{STR0034,"$A001DESCRI"},; //"*Descricao da EDT/Tarefa"
						{STR0035,"$A001DATAI"},; //"*Data e Hora Inicial da EDT/Tarefa"
						{STR0036,"$A001DATAF"},; //"*Data e Hora Final da EDT/Tarefa"
						{STR0037,"$A001PREDEC"},; //"*Predecessoras"
						{STR0038,"$A001ID"} ; //"*ID"
					}
Local aFuncNO    := {	{STR0042,"$A001CONF"} ,; //"%Concluido"
                        {STR0044,"$A001TPREST" },; // "Tipo Restricao"
						{STR0045,"$A001RECURS" } ; //"Nome dos Recursos"
                    } 
...
DEFINE MSDIALOG oDlg1 FROM 00,00 TO 300,520 TITLE STR0019 PIXEL  //"Selecione os campos"

@08,05  SAY STR0020 PIXEL OF oDlg1  //"Campos Disponiveis"
@08,143 SAY STR0021 PIXEL OF oDlg1 //"Campos Selecionados"
@45,240 SAY STR0022 PIXEL OF oDlg1 //"Mover"
@50,237 SAY STR0023 PIXEL OF oDlg1 //"Campos"
...
@16,98  BUTTON aBtn[1] PROMPT STR0024 SIZE 42,11 PIXEL; //" Add.Todos >>"
ACTION AddAllFld(@aCampos1,oCampos1,@aCampos2,oCampos2,@aCamposA,@aCamposB) WHEN IIf(lA001UsrBlk,lRetUsrBlk,.T.)

@28,98  BUTTON aBtn[2] PROMPT STR0025 SIZE 42,11 PIXEL; //"&Adicionar >>"
...
@40,98  BUTTON aBtn[3] PROMPT STR0026 SIZE 42,11 PIXEL; //"<< &Remover "
...
@52,98  BUTTON aBtn[4] PROMPT STR0027 SIZE 42,11 PIXEL; //"<< Rem.Todos"
ACTION DelAllFld(@aCampos1,oCampos1,@aCampos2,oCampos2,@aCamposA,@aCamposB,aFunc) WHEN IIf(lA001UsrBlk,lRetUsrBlk,.T.)

@115,98 BUTTON aBtn[5] PROMPT STR0028 SIZE 42,11 PIXEL; //"  Restaurar "
...
If nPos2 <> 0 .And. Len(aCampos2) <> 0
	If (nPosCampo := aScan(aFunc,{|x| AllTrim(x[2]) == aCampos2[nPos2][2]})) == 0
		...
		Next
	Else
		MsgAlert(STR0031) //"Os campos fixos nao podem ser retirados da lista de selecionados"
	Endif
	oCampos1:SetArray(aCamposA)
	oCampos1:Refresh()
	oCampos2:SetArray(aCamposB)
...
Else
	...
	While Len(cLineRec) > 0
		...
		Else
			...
			Else
				If ParamBox({	{1,STR0048,cCodRec,"@",'.F.',,'.F.',40,.F.},; //"Recurso invalido"
								{1,STR0049,CriaVar('AE8_RECURS'),"@!",'ExistCpo("AE8",mv_par02)','AE8','.T.',40,.T.},;  //"Selecione "
								{5,STR0050,.F.,100,,.F.};  //"Associar sempre a este recurso."
								},STR0051,aRet)  //"Recurso invalido. Selecione o recurso correto."
					AE8->(dbSetOrder(1))
...
Function PmsOrdCsv(aCposCSV) // Ordena as colunas inclusas pelo usuario
Local nPosTarefa := 0
Local nPosNivel  := 0
Local nPosID     := 0
Local nPosCalend := 0
