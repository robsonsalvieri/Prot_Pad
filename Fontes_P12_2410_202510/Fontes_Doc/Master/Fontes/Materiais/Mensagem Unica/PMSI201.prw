...
/*/
//-------------------------------------------------------------------
Function PMSI201(cXML, nTypeTrans, cTypeMessage)
   Local lRet             := .T.                       //
   Local cXMLRet          := ""                        //
   Local cError           := ""                        //
   Local cWarning         := ""                        //
   ...
   If nTypeTrans == TRANS_RECEIVE
      ...
      If cTypeMessage == EAI_MESSAGE_BUSINESS
         ...
         If oXml != Nil .And. Empty(cError) .And. Empty(cWarning)
            ...
            Else
               lRet    := .F.
               cXmlRet := STR0006 //O código da EDT (Obra) é obrigatório!
               aAdd(aMessages, {cXMLRet , 1, Nil})
            EndIf

            //Pesquisa o IntenalId da Obra
            ...
            If Upper(oXml:_TOTVSMessage:_BusinessMessage:_BusinessEvent:_Event:Text) == "UPSERT"
               ...
               // Nível da tarefa
               aAdd(aEDT, {"AFC_NIVEL", "002", Nil})

               // Nível Pai
               aAdd(aEDT, {"AFC_EDTPAI", cProjeto, Nil})
               ...
               If Type("oXml:_TOTVSMessage:_BusinessMessage:_BusinessContent:_Amount:Text") != "U" .And. !Empty(oXml:_TOTVSMessage:_BusinessMessage:_BusinessContent:_Amount:Text)
                  aAdd(aEDT, {"AFC_QUANT", Val(oXml:_TOTVSMessage:_BusinessMessage:_BusinessContent:_Amount:Text), Nil})
               EndIf
            ...
            If lret
               ...
               If nOpcx == 4
                  // Grava na tabela de EDT (Obra)
                  dbSelectArea("AFC")
                  AFC->(dbSetOrder(1)) //AFC_FILIAL+AFC_PROJET+AFC_REVISA+AFC_EDT+AFC_ORDEM

                  If AFC->(MsSeek(PadR(Separa(cValInt, "|")[2], TamSX3("AFC_FILIAL")[1]) + PadR(Separa(cValInt, "|")[3], TamSX3("AFC_PROJET")[1]) + PadR(Separa(cValInt, "|")[4], TamSX3("AFC_REVISA")[1]) + PadR(Separa(cValInt, "|")[5], TamSX3("AFC_EDT")[1])))
...
//-------------------------------------------------------------------
/*/{Protheus.doc} IntEDTExt
Monta o InternalID do EDT de acordo com o código passado
no parâmetro.
...
@sample  IntEDTExt(, , '0000000001', '0001', '001') irá retornar
{.T., '01|01|0000000001|0001|001'}
/*/
//-------------------------------------------------------------------
Function IntEDTExt(cEmpresa, cFil, cProjeto, cRevisao, cEDT, cVersao)
   Local   aResult  := {}
   Default cEmpresa := cEmpAnt
   Default cFil     := xFilial('AFC')
   Default cRevisao := '0001'
   Default cVersao  := '2.000'

   If cVersao == '2.000'
      aAdd(aResult, .T.)
      aAdd(aResult, cEmpresa + '|' + RTrim(cFil) + '|' + RTrim(cProjeto) + '|' + RTrim(cRevisao) + '|' + RTrim(cEDT))
   Else
      aAdd(aResult, .F.)
      aAdd(aResult, STR0020 + CRLF + STR0023) //"Versão da EDT não suportada." + CRLF + "As versões suportadas são: 2.000"
   EndIf
Return aResult

//-------------------------------------------------------------------
/*/{Protheus.doc} IntEDTInt
Recebe um InternalID e retorna o código do EDT.
...
@return  aResult Array contendo no primeiro parâmetro uma variável
         ...
         a filial, o código do projeto, a revisão do projeto e o
         código do EDT.

@sample  IntEDTInt('01|01|0000000001|0001|001') irá retornar
{.T., {'01', '01', '0000000001', '0001', '001'}}
/*/
//-------------------------------------------------------------------
Function IntEDTInt(cInternalID, cRefer, cVersao)
   Local   aResult  := {}
   Local   cAlias   := "AFC"
   Local   cField   := "AFC_PROJET"
   Local   cTemp    := ""
   ...
   If Empty(cTemp)
      cTemp := CFGA070Int(cRefer, cAlias, "AFC_EDT", cInternalID)
   EndIf

   If Empty(cTemp)
      aAdd(aResult, .F.)
      aAdd(aResult, STR0024 + AllTrim(cInternalID) + STR0025) //"EDT " " não encontrado no de/para!"
   Else
      If cVersao == '2.000'
         aAdd(aResult, .T.)
         aTemp := Separa(cTemp, '|')
