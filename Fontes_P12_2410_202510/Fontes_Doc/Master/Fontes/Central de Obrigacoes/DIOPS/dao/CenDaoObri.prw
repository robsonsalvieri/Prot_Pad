...
Method New(aFields) Class CenDaoObri
	_Super:New(aFields)
    self:cAlias := "B3A"
    self:cfieldOrder := "B3A_CODIGO,B3A_CODOPE"
Return self
...
Method commit(lInclui) Class CenDaoObri
	...
	If B3A->(RecLock("B3A",lInclui))
        ...
        If lInclui
            ...
            B3A->B3A_CODIGO := _Super:normalizeType(B3A->B3A_CODIGO,self:getValue("requirementCode")) /* Column B3A_CODIGO */
            B3A->B3A_CODOPE := _Super:normalizeType(B3A->B3A_CODOPE,self:getValue("operatorRecord")) /* Column B3A_CODOPE */
