#include "TOTVS.CH"

#DEFINE N_TO_N   1 // Relação N para N
#DEFINE ONE_TO_N 2 // Relação 1 para N
#DEFINE N_TO_ONE 3 // Relação N para 1
#DEFINE ZERO_TO  4 // Relação 0 para 1 ou N
#DEFINE TO_ZERO  5 // Relação N ou 1 para 0

#DEFINE NO_BEHAVIOR 0 // Sem comportamento
#DEFINE CASCADE 1     // Relacionamento em cascata

Class CenCltBra from CenCollection

    Method New() Constructor
    Method initEntity()
    Method setProcessing()
    Method getMessage()
    Method setEndProc(nRecno)
    Method setExpired()
    Method GetProcess(cIdProc)
    Method initRelation()

EndClass

Method New() Class CenCltBra
    _Super:new()
    self:oMapper := CenMprBra():New()
    self:oDao := CenDaoBra():New(self:oMapper:getFields())
return self

Method initEntity() Class CenCltBra
return CenBra():New()

Method setProcessing() Class CenCltBra
Return self:getDao():setProcessing()

Method getMessage() Class CenCltBra
    Local lFound := self:getDao():getMessage()
    self:goTop()
Return lFound

Method setEndProc(nRecno) Class CenCltBra
    Local lFound := self:getDao():setEndProc(nRecno)
    self:goTop()
Return lFound

Method setExpired() Class CenCltBra
    Local lFound := self:getDao():setExpired()
Return lFound

Method GetProcess(cIdProc) Class CenCltBra
    Local lFound := self:getDao():GetProcess(cIdProc)
    self:goTop()
Return lFound

Method initRelation() Class CenCltBra

    Local oRelation := CenRelation():New()

    oRelation:setCollection(CenCltBrb():New())
    oRelation:setName("monitFormEvents")
    oRelation:setRelationType(ONE_TO_N)
    oRelation:setBehavior(CASCADE)
    oRelation:setKey({;
        {"formSequential","formSequential"},;
        {"operatorRecord","operatorRecord"};
        })
    self:setRelation(oRelation)

    oRelation := CenRelation():New()

    oRelation:setCollection(CenCltBnw():New())
    oRelation:setName("monitFormCertificates")
    oRelation:setRelationType(ONE_TO_N)
    oRelation:setBehavior(CASCADE)
    oRelation:setKey({;
        {"formSequential","formSequential"},;
        {"operatorRecord","operatorRecord"};
        })
    self:setRelation(oRelation)

return self:listRelations()