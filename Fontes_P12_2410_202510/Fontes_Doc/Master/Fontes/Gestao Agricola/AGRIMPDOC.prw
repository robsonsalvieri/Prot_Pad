...
############################################################################
# Função   : AGRIMPDOC                                                     #
# Descrição: Impressão dos documentos do produto da ordem do carregamento  #
# Autor    : Inácio Luiz Kolling                                           #
# Data     : 04/08/2014                                                    #  
############################################################################
# Parâmetro: cSafra - Safra chave para localização                         #
# Parâmetro: clote  - Lote  chave para localização                         #
############################################################################
*/       
Function AGRIMPDOC(cSafra,cLote)
	Local aArea 		:= GetArea()
	Local nx
	...
	If File(cPathDot)
		...
		For nx := 1 To Len(aMatMac)
			...
			Count To nQtdNP9
			(cAlias)->(dbGotop())
			//	Localiza numero de termo de conformidade
			cTermo := ALLTRIM((cAlias)->NP9_NUMTC)
			(cAlias)->(dbCloseArea())
			...
			If !Empty(nQtdNP9)
				...
				If !Empty(aMatMac[nx,1,nCodA])
					...
					if ValType(cConte) = "D"
						...
						Dtoc(cConte)
					EndIf
					// resolve Mes/Ano para campo de Validade
					if aMatMac[nx,1,nSequ] ="VALIDADE"
						cConte := (aMES[MONTH(dData)] + " / " + AllTrim(Str(Year(dData))))
					Endif
					(cAlias)->(dbCloseArea())
				Else
					nPosOn := At("_",aMatMac[nx,1,nMac])
					cTabel := SubStr(aMatMac[nx,1,nMac],1,nPosOn-1)
					cTabel := If(Len(cTabel) = 2,"S"+cTabel,cTabel)
//TODO Procurar maneira de retornar valores de campos virtuais 
					If 		ALLTRIM(aMatMac[nx,1,nMac]) == "NP9_CTRDES"
						cConte := Posicione("NP3",1,xFilial("NP3")+NP9->NP9_CULTRA,"NP3_DESCRI")
					ElseIf	ALLTRIM(aMatMac[nx,1,nMac]) == "NP9_CTVDES"
						cConte := Posicione("NP4",1,xFilial("NP4")+NP9->NP9_CTVAR,"NP4_DESCRI")
					ElseIf ALLTRIM(aMatMac[nx,1,nMac]) == "NJU_DESCRI"
						cConte := Posicione("NJU",1,xFilial("NJU")+NP9->NP9_CODSAF,"NJU_DESCRI")
					Else
						cConte := &(cTabel+"->"+aMatMac[nx,1,nMac])
					EndIf

				EndIf
//TODO Procurar maneira de tornar essa VALIDADE dinamica				
				if aMatMac[nx,1,nSequ] ="VALIDADE"
					cConte := (aMES[MONTH(cConte)] + " / " + AllTrim(Str(Year(cConte))))
				Endif				

				cMacro := Alltrim(aMatMac[nx,1,nSequ])
				If	(ValType(cConte)="N" .and. cConte = 0) .or. (ValType(cConte)="C" .and. cConte="   ") .or. ValType(cConte)="U"
//TODO procurar fundamento legal(Edital) para tratamento de campos com valor zerado
		...
		OLE_CLOSEFILE(hWord)
		//tempo para processamento no Word
		Sleep(1000)
