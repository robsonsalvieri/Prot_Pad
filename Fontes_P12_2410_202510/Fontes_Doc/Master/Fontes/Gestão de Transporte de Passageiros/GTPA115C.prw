...
If ValType(oStruGIC) == "O"
    ...
    oStruGIC:SetProperty('GIC_STATUS'   ,MODEL_FIELD_INIT,{||"C"})
...
If GIC->GIC_STATUS <> 'C' .and. GIC->GIC_STATUS <> 'D' // Bilhetes diferente de cancelado e devolvido  
	...
	If GIC->GIC_ORIGEM == '2' .and. !(GIC->GIC_ORIGEM == '2' .and. GIC->GIC_TIPO == 'E' ) 
        lRet        := .F.
        cMsgErro    := STR0004//"Não é possivel cancelar ou devolver uma passagem de origem eletrônica."	
	Endif
	
	If lRet .and. GIC->GIC_ORIGEM == '2' .and. GIC->GIC_TIPO == 'E' 
        lRet        := .F.
        cMsgErro    := STR0007 //"Não é possivel cancelar uma passagem de origem eletrônica e tipo embarcado."
        cMsgSol     := STR0008 //"Para esse tipo de bilhete, selecione a opção de Devolução"
	Endif

	If lRet .and. !VldBilRef(GIC->GIC_CODIGO)
        lRet        := .F.
        cMsgErro    := STR0005//"Não é possivel cancelar ou devolver esse bilhete pois o mesmo ja se encontra cancelado/devolvido"
	Endif
Else
    lRet        := .F.
    cMsgErro    := STR0006//"Não é possivel cancelar ou devolver esse bilhete pois o mesmo ja é um bilhete cancelado/devolvido"
Endif

If lRet .and. GIC->GIC_STATUS == 'I' 
	cMsgErro    := STR0009 //"Não é possivel cancelar bilhetes inutilizados"
    lRet := .F.
EndIf
...
oMdlGIC:LoadValue("GIC_STATUS","C")
...
Local oModel    := FwLoadModel('GTPA115C')
...
oStruGIC:AddGroup('GRP000', STR0001+"Cancelamento",'', 2)//'Dados de '
...
oStruGIC:SetProperty( 'GIC_DTVEND', MVC_VIEW_TITULO, STR0002+Capital(" Cancelamento"))//'Dt.'
oStruGIC:SetProperty( 'GIC_MOTCAN', MVC_VIEW_TITULO, STR0003+Capital(" Cancelamento"))//'Mot.'
