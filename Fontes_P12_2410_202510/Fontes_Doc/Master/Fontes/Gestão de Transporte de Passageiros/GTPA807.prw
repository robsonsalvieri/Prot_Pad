...
If G99->G99_STATRA == 'A' .AND. G99->G99_TIPCTE $ '2'
	G807Comple()	
Else
    FwAlertHelp(STR0001, STR0002) //"Apenas CTE autorizado e de anulação poderá ser feito a substituição.", "STATUS" 
Endif
...
@see (links_or_references)
/*/
Static Function G807Comple()
Local aArea        := GetArea()
Local cTitulo      := STR0003 //"CTE de substituição"
Local cAliaVisu    := "VIEWDEF.GTPA807"
...
If nRet == 0
    MsgInfo(STR0004, STR0005) //"Atenção", "Substituição gerada!"
EndIf
...
aTabFrt :=  G801TabFrt(,G99->G99_TABFRE,,'2')
...
While (cAliasQry)->(!Eof())	
    ...
    AADD(aCampos,{{"G9R_ITEM"  ,G9R->G9R_ITEM},{"G9R_DESCRI",G9R->G9R_DESCRI},{"G9R_VALOR",G9R->G9R_VALOR}})
    (cAliasQry)->(DbSkip())    
End
...
oModel := MPFormModel():New("GTPA807",/*PREVALID*/, bPosValid/*POSVALID*/, /*COMMIT*/)
oModel:SetDescription(STR0006)  //"Entrada de Encomendas"

//Cabeçalho G99 -- Entrada de Encomendas
oModel:AddFields("MASTERG99",/*oOwner*/, oStrG99,,,)
oModel:GetModel("MASTERG99"):SetDescription(STR0006)  //"Entrada de Encomendas"
...
oModel:GetModel("DETAILG9R"):SetDescription(STR0007)  //"Declaração de Encomendas"
...
oModel:GetModel("DETAILG9Q"):SetDescription(STR0008)  //"Serviços de transporte"
...
oModel:GetModel("DETAILG9P"):SetDescription(STR0009)  //"Lista de Estados"
...
oStG99E:SetProperty("G99_VALOR", MVC_VIEW_CANCHANGE, .T.)
...
Local cChvFis       := Alltrim(oMdlG99:GetValue('G99_CHVFIS'))
...
If !Empty(cNumero)
    ...
    AtualiSD2(cChvFis, cNumero, cSerie)
...
/*/
//-------------------------------------------------------------------
Static Function AtualiSD2(cChave, cNfOri, cSerOri)
...
//            Else
//                lRet        := .F.
//                cMsgErro    := STR0038  //"Não foi possivel excluir a nota"
//                cMsgSol     := ""
//            Endif
//                
//        EndIf
