...
//---------------------------------------------------------------------
/*/{Protheus.doc} TAFDAPI27

Rotina de geração do Detalhamento Tipo 27 da DAPI-MG
...
/*/
//---------------------------------------------------------------------
Function TAFDAPI27(aWizard, nCont, aFil)
...
Begin Sequence
	...
	/***** DETALHAMENTO TIPO 27 - Ressarcimento Substituição Tributária  ********/
	aDetalhe := DapiAprSQL(cFilDapi, cUFID, DToS(aWizard[1][1]), DTOS(aWizard[1][2]), "00070","0","ICMSST")
	nPos := 0
	...
	While nPos < Len(aDetalhe)
		...
		cStrTxt := Left(cStrTxt,27) + PADL(Alltrim(aDetalhe[nPos,1]),9)					//Nota Fiscal
		cStrTxt := Left(cStrTxt,36) + PADL(Alltrim(aDetalhe[nPos,2]),3)					//Serie
		cStrTxt := Left(cStrTxt,39) + FormatData(STOD(aDetalhe[nPos,3]),.F.,5)			//Data Documento
		cStrTxt := Left(cStrTxt,47) + FormatData(STOD(aDetalhe[nPos,4]),.F.,5)			//Data do Visto
		cStrTxt := Left(cStrTxt,55) + StrTran(StrZero(aDetalhe[nPos,5], 16, 2),".","")	//Valor do Ajuste
		cStrTxt += CRLF
