...
Function NF030Html(aCabCpo,aItCpo,aRodCpo,cColor, cPicture, cMessage, lShowHelp)
	...
	if ( aRetFun[1] == .F. )
		...
		if lRetFun
			...
			cString += CRLF +'	<input type=hidden name="CVERSAOHTMLNFC001" value="1">
			...
			cString += CRLF +'					</div>'
					...
					for nRod := 1 to len(aRodCpo)
						...
						elseif cCpoType == "L"
							If aRodCpo[nRod][2] == "VISUAL"
								cString += CRLF +'<input disabled=true name="'+aRodCpo[nRod][1]+'" id="'+aRodCpo[nRod][1]+'" type="checkbox" value="%'+aRodCpo[nRod][1]+'%" class="' + aRodCpo[nRod][1] + '"style="width: 15px;border-radius: 5px;height: 15px;border: solid 1px; margin-right: 10px;" />'
							Else
			...
			cString += CRLF +'				(document.getElementById(cCampo).checked) ? document.getElementById(cCampo).value = true : document.getElementById(cCampo).value = false;'
			cString += CRLF +'			}'
...
Function NF030CRHTE(cMessage, lShowHelp)
	...
	If FwAliasInDic('DKK') .And. FwAliasInDic('DKL')
		...
		DKL->(DbSetOrder(2)) //DKL_FILIAL+DKL_CODDKK
