...
If IsCtbJob() // se tem o schedule da CTBA193 criado
	...
	ProcLogAtu("ERRO","NOEXEC",STR0071)
	
	//If lAuto
	Help(" ",1,"NOEXEC",,STR0071,3,0)
	//Endif
...
//		Aadd(aDados2,{aListaJob[nX,1],If(Empty(xFilial("CT2",cFilAtu)),"  ", IIf( lFWCodFil, FWGETCODFILIAL, SM0->M0_CODFIL )),'','',''})
	...
	oTimer:= TTimer():New(nTimeOut,{|| If(lEnded192,oBtn3:LVISIBLE := .T.,NIL), If(!lEnded192,CheckProc(@cArquivo, oBrowse1,oBrowse2,aDados1,aDados2,aFils,cContaDe,cContaAte,cFilAte,@cArqLog,dDataIni,dDataFim,cTpSald,lMoedaEsp,cMoeda,nMoedaIni,nMoedaFim,oMeter1,oMeter2),Nil)  },oDlg)	
	oTimer:Activate()
...
nPend     := CheckPend( IIF( cFil <> Nil,cFil, Nil ) , cArquivo )
lEnded192 := (nPend == 0)
...
//Verifica se tabela temporaria para controle jแ existe, se sim, apaga para criar novamente
lApaga := ApTabLog(cArquivo)

If !MSFile(cArquivo, ,__CRDD ) .And. lApaga
	...
	FwdbCreate(cArquivo,aEstrut,"TOPCONN", .T.)
EndIf	

If Select(cArquivo) == 0
	dbUseArea(.T.,__CRDD,cArquivo,cArquivo, .T., .F. )
EndIf
...
/*  Novo Processo Procedure - Processo 07 - Reprocessamento por conta contabil
/{Protheus.doc} EngPos07Compile
...
/*/
// Processo 07 - REPROCESSAMENTO Cpor contas
Function EngPos07Compile(cProcesso as character, cEmpresa as character, cProcName as character, cLocalDB as character, cBuffer as character, cError as character)
...
ฑฑณFuno    ณApTabLogณ Autor ณ Douglas Rodrigues da Silvaณ Data ณ 28/06/23 ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณDescrio ณApaga tabela temporaria                                       ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณSintaxe   |ApTabLog(cTblCtrl)                                            ณฑฑ
ฑฑภฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤูฑฑ
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
Static Function ApTabLog(cArquivo)

	Local lRet   := .F.
	
	Default cArquivo := "TMPCTA"

	// tabela existe? se sim deleta
	If TCCanOpen(cArquivo)

		// area esta aberta? se sim fecha
		IIf(Select(cArquivo) > 0, (cArquivo)->(DbCloseArea()),.F.)
				
		lRet := TCDelFile(cArquivo)
	Else
		lRet := .T.	
	EndIf
