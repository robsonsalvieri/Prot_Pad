...
begin
   ...
   While (@@Fetch_status = 0 ) begin         
      ...
      If @iRecno = 0 begin
         ...
         If @cTIPO = '1' begin
            Update CQ1###
                set CQ1_DEBITO = CQ1_DEBITO + @nCTX_DEBITO
              Where R_E_C_N_O_ = @iRecno
         End
         If @cTIPO = '2' begin
            Update CQ1###
              set CQ1_CREDIT  = CQ1_CREDIT + @nCTX_CREDIT
              Where R_E_C_N_O_ = @iRecno
         End
         ##CHECK_TRANSACTION_COMMIT
      /* ---------------------------------------------------------------
      ...
      if @cCT2_DTLP != ' ' begin
         ...
         select @cTabela = 'CQ0'
         select @cIdent  = ' '
         Exec CTB025_##  @cFilial_CQ0, @cTabela, @cIdent, @cCONTA, @cCUSTO, @cITEM, @cCLVL, @cCT2_DTLP, @cMOEDA, @IN_TPSALDO, @IN_EMPANT, @IN_FILIAL, @IN_TRANSACTION, @OUT_RESULTADO OutPut
      end
      /*---------------------------------------------------------------
      ...
      /*-----------------------------------------------------------------
        ...
        ----------------------------------------------------------------- */
      If @cFilAnt = @cFILCT2 and @cContaAnt = @cCONTA and @cMoedaAnt = @cMOEDA and @cAnoMes = SUBSTRING( @cDATA, 1, 6 ) and @cDtLpAnt = @cCT2_DTLP and @cTipoAnt = @cTIPO begin
         If @cTIPO = '1' begin
            select @nDebMes = @nDebMes + @nVALOR
         end
         If @cTIPO = '2' begin
         ...
         If @iRecno = 0 begin
            ...
            Insert into CQ0### ( CQ0_FILIAL,   CQ0_CONTA,  CQ0_MOEDA,  CQ0_DATA, CQ0_TPSALD,  CQ0_SLBASE,   CQ0_DTLP,  CQ0_LP, CQ0_STATUS,   CQ0_DEBITO, CQ0_CREDIT, R_E_C_N_O_ )
							    values( @cFilAnt,    @cContaAnt,  @cMoedaAnt, @cDataF,  @IN_TPSALDO, @cCTX_SLBASE, @cCTX_DTLP, @cCTX_LP, @cCTX_STATUS, 0,   0,   @iRecno )
            ##CHECK_TRANSACTION_COMMIT
            ##FIMTRATARECNO
		    end
            /*---------------------------------------------------------------
         ...
         if @cCTX_DTLP != ' ' begin
            ...
            select @cTabela = 'CQ1'
            select @cIdent  = ' '
            Exec CTB025_##  @cFilial_CQ1, @cTabela, @cIdent, @cContaAnt, @cCUSTO, @cITEM, @cCLVL,  @cCTX_DTLP, @cMoedaAnt, @IN_TPSALDO, @IN_EMPANT, @IN_FILIAL, @IN_TRANSACTION, @OUT_RESULTADO OutPut
         end
      end
   end
   close CUR_CTB190
   ...
   If @IN_LCUSTO = '1' begin
      ...
      While (@@Fetch_status = 0 ) begin 
         ...
         If @iRecno = 0 begin
            ...
            If @cTIPO = '1' begin
               Update CQ3###
                   set CQ3_DEBITO = CQ3_DEBITO + @nCTX_DEBITO
                 Where R_E_C_N_O_ = @iRecno
            End
            If @cTIPO = '2' begin
               Update CQ3###
                   set CQ3_CREDIT  = CQ3_CREDIT + @nCTX_CREDIT
                 Where R_E_C_N_O_ = @iRecno
            End
            ##CHECK_TRANSACTION_COMMIT
         /* ---------------------------------------------------------------
         ...
         if @cCT2_DTLP != ' ' begin
            ...
            select @cIdent  = ' '
            select @cTabela = 'CQ3'
            Exec CTB025_##  @cFilial_CQ3, @cTabela, @cIdent, @cCONTA, @cCUSTO, @cITEM, @cCLVL, @cCT2_DTLP, @cMOEDA, 
                           @IN_TPSALDO, @IN_EMPANT, @IN_FILIAL, @IN_TRANSACTION, @OUT_RESULTADO OutPut
         ...
         if @@fetch_status = -1 select @cCONTA = ' '
            ...
            ATUALIZA SALDOS MES qdo necessario
           ----------------------------------------------------------------- */
         If @cFilAnt = @cFILCT2 and @cContaAnt = @cCONTA and @cCustoAnt = @cCUSTO and @cMoedaAnt = @cMOEDA and @cAnoMes = SUBSTRING( @cDATA, 1, 6 )  and @cDtLpAnt = @cCT2_DTLP  and @cTipoAnt = @cTIPO begin
            If @cTIPO = '1' begin
               select @nDebMes = @nDebMes + @nVALOR
            end
            If @cTIPO = '2' begin
            ...
            If @iRecno = 0 begin
               ...
               Insert into CQ2### ( CQ2_FILIAL,   CQ2_CONTA,  CQ2_CCUSTO, CQ2_MOEDA,  CQ2_DATA, CQ2_TPSALD,  CQ2_SLBASE,   CQ2_DTLP,   CQ2_LP,CQ2_STATUS,   CQ2_DEBITO, CQ2_CREDIT, R_E_C_N_O_ )
			                    values( @cFilial_CQ2, @cContaAnt, @cCustoAnt, @cMoedaAnt, @cDataF,  @IN_TPSALDO, @cCTX_SLBASE, @cCTX_DTLP, @cCTX_LP, @cCTX_STATUS, 0,   0,   @iRecno )
               ##CHECK_TRANSACTION_COMMIT
               ##FIMTRATARECNO
                end
               /*---------------------------------------------------------------
            ...
            if @cCTX_DTLP != ' ' begin
               ...
               select @cIdent  = ' '
               select @cTabela = 'CQ2'
               Exec CTB025_##  @cFilial_CQ2, @cTabela, @cIdent, @cContaAnt, @cCustoAnt, @cITEM, @cCLVL, @cCTX_DTLP, @cMoedaAnt, 
                              @IN_TPSALDO, @IN_EMPANT, @IN_FILIAL, @IN_TRANSACTION, @OUT_RESULTADO OutPut
            end
         end
      end
   ...
   If @IN_LITEM = '1' OR @IN_LCLVL = '1' begin
      EXEC CTB231_## @IN_FILIAL,  @IN_LITEM, @IN_LCLVL, @IN_DATADE, @IN_DATAATE,  @IN_LMOEDAESP, @IN_MOEDA, @IN_TPSALDO, @IN_CONTA, 
                     @IN_INTEGRIDADE, @IN_MVCTB190D, @IN_EMPANT, @IN_TRANSACTION, @OUT_RESULTADO Output
   End
   /*---------------------------------------------------------------
