...
/*---------------------------------------------------------------------------------------------------------------------
    ...
    Descricao   -  <d> Integraùùo entre ERP Livros Fiscais X TAF (SPED)-SYD(NCM-Protheus)x C0A(NCM- TAF) </d>
...
DECLARE @ID_C0A         CHAR('C0A_ID')
DECLARE @CODIGO_C0A     CHAR('C0A_CODIGO')
...
DECLARE @EXNCM          VARCHAR('C0A_PERNCM')
        ...
        SELECT 
            ...
            INNER JOIN C0A### C0A
                ON C0A.D_E_L_E_T_ = ' ' 
                    ...
                    AND C0A.C0A_CODIGO = LTRIM(RTRIM(SYD.YD_TEC))
            LEFT JOIN V80### V80
                ON V80.D_E_L_E_T_ = ' '
