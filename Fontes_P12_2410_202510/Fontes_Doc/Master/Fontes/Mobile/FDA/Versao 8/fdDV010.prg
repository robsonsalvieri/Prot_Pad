...
Function InitPDEsp(aCabPed,aItePed,aColIte)
Local oDlg,oCab,oFldProd,oObj,oBrwProd,oBrwItePed
Local aObj := { {},{},{},{},{} }
Local cCliente:=""
...
Local cCond := "", cTes := ""
Local nIVA := 0, cEst := space(40), nPrc:=0.00  , nicm:=0, nipi:=0, ndescMax:=0
...
SetParam(aCabPed[3,1],aCabPed[4,1], @cCliente, @cCond,@cTes,@nIVA,cProDupl,aPrdPrefix,.F.)
...
@ 30,03 BROWSE oBrwProd SIZE 137,60 NO SCROLL ACTION ;
PVSeleciona(oBrwProd,aColIte,aItePed,@nItePed,aCabPed,aObj,cManPrc,cManTes,@nOpIte,"D") of oFldProd
...
@ 20,143 BUTTON oUp    CAPTION UP_ARROW SYMBOL SIZE 12,10 ACTION PVSobe(aGrupo,nGrupo,oBrwProd,@nTop,aItePed,lCodigo) OF oFldProd
@ 39,143 BUTTON oLeft  CAPTION LEFT_ARROW SYMBOL SIZE 12,10 ACTION GridLeft(oBrwProd) OF oFldProd
@ 58,143 BUTTON oRight CAPTION RIGHT_ARROW SYMBOL SIZE 12,10 ACTION GridRight(oBrwProd) OF oFldProd
@ 77,143 BUTTON oDown  CAPTION DOWN_ARROW SYMBOL  SIZE 12,10 ACTION PVDesce(aGrupo,nGrupo,oBrwProd,@nTop,aItePed,lCodigo) OF oFldProd

@ 94,03 BUTTON oObj CAPTION STR0022 ACTION PVQTde(aObj[3,3]) SIZE 30,11 of oFldProd //"Qtde."
AADD(aObj[3],oObj) // 2 - Botao Quantidade
@ 94,40 GET oObj VAR aColIte[4,1] SIZE 35,15 of oFldProd
AADD(aObj[3],oObj) // 3 - Get Quantidade

@ 94,080 BUTTON oObj CAPTION STR0023 SIZE 33,11 of oFldProd //"Preço"
AADD(aObj[3],oObj) // 4 - Botao Preco
@ 94,115 GET oObj VAR aColIte[6,1] PICTURE "@E 9,999.99" READONLY SIZE 40,15 of oFldProd
...
@ 111,085 BUTTON oBtnOK CAPTION "+" SIZE 22,10 ACTION PVGrvIte(aColIte,aItePed, nItePed, aCabPed,aObj,cManTes,cProDupl,nOpIte,2,"D") of oFldProd
@ 111,130 BUTTON oBtnExcluir CAPTION "-" SIZE 22,10 ACTION PVExcIte(aItePed,@nItePed,aCabPed,aObj,.F.,2) of oFldProd	
...
ADD FOLDER oDet CAPTION STR0026 ON ACTIVATE PVSetDetalhes(oBrwProd,aControls,@cCod,cDescD,cUN,nQTD,nEnt,nICM,nIPI,cEst,nDescMax) Of oDlg //"Detalhe"
...
@ 125,71 BUTTON oObj CAPTION STR0004  ACTION PDGravarPed(aCabPed,aItePed,aColIte,cSfaInd) SIZE 40,12 OF oCab //"Gravar"
...
ADD COLUMN oCol TO oBrwItePed ARRAY ELEMENT 15 HEADER STR0002 WIDTH 45 //"IVA" //"IVA"
ADD COLUMN oCol TO oBrwItePed ARRAY ELEMENT 16 HEADER STR0003 WIDTH 45 //"Total do Item" //"Total"
