...
If dbSeek(RetFilial("HC6")+cNumPed)
	...
	While !Eof() .And. HC6->HHC6_FILIAL == RetFilial("HC6") .And. HC6->HC6_NUM == cNumPed
		dbSelectArea("HB1")
		dbSetOrder(1)
		...
		Else  
			...
			aCabPed[11,1] := aCabPed[11,1] - (HC6->HC6_QTDVEN * Round((HC6->HC6_PRCVEN - (HC6->HC6_PRCVEN * (HC6->HC6_DESC / 100))),2))
...
if Val(HA3->HA3_PROXPED) > Val(HA3->HA3_PEDFIM)
...
cNumPed	:=	AllTrim(HA3->HA3_PROXPED)
// -------------------------------------------------------------------------
