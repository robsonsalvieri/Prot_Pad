...
While !eof() .And. HX5->HX5_FILIAL == RetFilial("HX5") .and. HX5->X5_TABELA = "OC"
...
If !Empty(cOcoCad)
	If Empty(cOco)
		...
		If MsgYesOrNo(STR0005,STR0006) //"Você deseja Excluir a Ocorrência?"###"Cancelar"
			...
			dbSelectArea("HD5")
			dbSetOrder(1)
			If dbSeek(RetFilial("HD5")+HA1->A1_COD + HA1->A1_LOJA + DtoS(dData))
				dbDelete()
			EndIf
		Else
			Return Nil
...
If Empty(cCodRot)
	dbSelectArea("HD7")
	dbSetOrder(3)
	If !dbSeek(RetFilial("HD7")+DtoS(dData)+HA1->A1_COD + HA1->A1_LOJA )
		lUpdRot := .F.
	EndIf
Else
	dbSelectArea("HD7")
