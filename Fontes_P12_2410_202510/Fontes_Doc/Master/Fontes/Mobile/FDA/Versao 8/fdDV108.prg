...
dbSelectArea("HD1")     
dbSetOrder(1)
If dbSeek(RetFilial("HD1")+cNumPed)
	aSize(aItePed,0)
	...
	While !Eof() .And. HD1->HD1_FILIAL == RetFilial("HD1") .And. HD1->HD1_DOC == cNumPed
		dbSelectArea("HB1")
		dbSetOrder(1)
		if dbSeek(RetFilial("HB1")+HD1->HD1_COD)
			...
			If HB1->(FieldPos("B1_PBRUTO")) > 0
				...
				If 	cUmPeso = "1"
					aCabPed[17,1] := aCabPed[17,1] + (HD1->HD1_QUANT * nPrdPeso)
				Else
					If HB1->HB1_TIPCONV = "M"
		...
		Else  
			...
			//aCabPed[11,1] := aCabPed[11,1] - (HD1->HD1_QUANT * HD1->HD1_VUNIT )
		...
		If HB1->(FieldPos("B1_PBRUTO")) > 0
			...
			Else
				...
				Else
					nPrdPeso := Round(nPrdPeso / HB1->HB1_CONV,2)
				EndIf
				aCabPed[17,1] := aCabPed[17,1] + (HD1->HD1_QTDVEN * nPrdPeso)
			EndIf
		EndIf
    ...
    For nI:=1 to Len(aColIte)
    	If aColIte[nI,2] <> 0  // Se o FIELDPOS for Diferente de Zero
     		If nI == 6 //Atualizar preco com o preco atual de tabela
               	HPR->( dbSetOrder(1) )
               	HPR->( dbSeek(RetFilial("HPR")+HD1->HD1_COD+aCabPed[8,1]) )
               	If HPR->(Found())
