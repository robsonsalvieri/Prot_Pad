...
If aCabPed[2,1] == 1 .Or. aCabPed[2,1] == 4
	...
	HF1->HF1_QTDITE	:= Len(aItePed)
//	HC5->HC5_VEND1 	:= HA3->HA3_COD
	HF1->HF1_EMISSAO	:= dDataBase
	HF1->HF1_STATUS 	:= "N"
	HF1->HF1_VALOR 	:= aCabPed[12,1]	//Total
	dbCommit()
Else
	dbSeek(RetFilial("HF1")+aCabPed[1,1])
	if HF1->(Found())
		For nI:=1 to Len(aCabPed)
	 		If aCabPed[nI,2] <> 0  // Se o FIELDPOS for Diferente de Zero
           		HF1->(FieldPut(HF1->(aCabPed[nI,2]), aCabPed[nI,1]))
	...
	dbSelectArea("HD1")
	dbSetOrder(1)
	dbSeek(RetFilial("HD1")+aCabPed[1,1]) 
	While !Eof() .And. HD1->HD1_FILIAL == RetFilial("HD1") .And. HD1->HD1_DOC = aCabPed[1,1]
	    dbDelete() 
	    dbSkip()
	EndDo 
Endif
...
For nCont:=1 to len(aItePed)
	dbAppend() 
	HD1->HD1_FILIAL := RetFilial("HD1")
	HD1->HD1_DOC	   := aCabPed[1,1]
	HD1->HD1_ITEM   := StrZero(nCont,3)
...
GrvAtend(1, aCabPed[1,1], , HF1->HF1_FORNECE, HF1->HF1_LOJA,)
...
While !Eof() .and. HF1->HF1_FORNECE == cCodCli .and. HF1->HF1_LOJA == cLojaCli
	cStPedido:= ""
	cNumDev:=""
	cNumDev:=HF1->HF1_DOC
	if HF1->HF1_STATUS = "N"
		If HF1->(IsDirty())		//Nao transmitido
			AADD(aDev,{cNumDev,HF1->HF1_EMISSAO,HF1->HF1_VALOR}) 
		Endif
	Endif
