...
If Len(aItePed) > 0 .And. cTabAnt <> aCabPed[8,1]
	...
	Else
      ...
      For ni := 1 to Len(aItePed) 
          ...
          dbSelectArea("HPR")
       	  dbSetOrder(1)
		  dbSeek(RetFilial("HPR")+aItePed[ni,1]+aCabPed[8,1])	//Procura preço de venda usando a nova tabela
		  ...
		  Else           
			 dbSelectArea("HB1")
			 dbSetOrder(1)
			 dbSeek(RetFilial("HB1")+aItePed[ni,1])     
			 aItePed[ni,6] := HB1->HB1_PRV1
		  Endif                                 
...
If !Empty(cTab) .And. !Empty(cCond)
	...
	If !Empty(HTC->HTC_COND) .And. (HTC->HTC_COND <> cCond)
		MsgStop(STR0008 + HTC->HTC_COND,STR0009) //"Condição de Pagto. inválida para esta tabela de preços. A condição válida é: "###"Aviso"
	 	//aCabPed[7,1] := HTC->HTC_COND	//atualiza condicao pagto.
		lRet := .f.
	Endif
Endif
Return lRet