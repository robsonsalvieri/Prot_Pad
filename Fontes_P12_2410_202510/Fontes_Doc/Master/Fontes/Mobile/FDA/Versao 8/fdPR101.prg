...
if Empty(HA3->HA3_PEDFIM)
	...
	lAtu:= .T.
Endif
if Empty(HA3->HA3_PROXPED)
	HA3->HA3_PROXPED		:= "000001"
	lAtu:= .T.
Endif
...
if Empty(HA3->HA3_CLIFIM)
	...
	lAtu:= .T.
Endif
if Empty(HA3->HA3_PROXCLI)
	HA3->HA3_PROXCLI		:= "000001"
	lAtu:= .T.
Endif 
if Val(HA3->HA3_PROXCLI) < Val(HA3->HA3_CLIINI)
	HA3->HA3_PROXCLI		:= HA3->HA3_CLIINI
	lAtu:= .T.
Endif
if Val(HA3->HA3_PROXPED) < Val(HA3->HA3_PEDINI)
	HA3->HA3_PROXPED		:= HA3->HA3_PEDINI
	lAtu:= .T.
Endif
...
If dbSeek(RetFilial("HCF")+"MV_DTSYNC")
	...
	If dbSeek(RetFilial("HCF")+"MV_LASTACC")
		dDtFirst   := StoD(HCF->HCF_VALOR)
	Else
		dbAppend()
		HCF->HCF_FILIAL := RetFilial("HCF")
		HCF->HCF_PARAM := "MV_LASTACC"
		HCF->HCF_VALOR := DtoS(Date())
		dbCommit()
		dDtFirst   := Date()
	EndIf
...
Else	
	...
	cEmpNome := EMP->EMP_NOME 
    cFilial  := EMP->EMP_FILIAL
EndIf
...
"HCQ","HCR","HCS","HCT","HMT","HIN","HF2","HD2","HF1","HD1","HEL","HA6","HB6" }
...
ADD COLUMN oCol TO oBrwRecs ARRAY ELEMENT 1 HEADER "" WIDTH 10 MARK
...
If aTables[nTable, 2] = "HC5"
	...
	While !HC5->(Eof())
		If AllTrim(HC5->HC5_STATUS) = "N"
			aAdd(aRecs, {.F., HC5->HC5_NUM, "", Recno()})
		EndIf
	...
	For ni := 1 To Len(aRecs)
		If aRecs[ni, 1]
			...
			If dbSeek(RetFilial("HC6")+HC5->HC5_NUM)
				While !HC6->(Eof()) .And. HC6->HC6_FILIAL == RetFilial("HC6") .And. HC5->HC5_NUM = HC6->HC6_NUM
