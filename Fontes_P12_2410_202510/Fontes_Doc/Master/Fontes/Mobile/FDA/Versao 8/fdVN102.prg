...
If dbSeek(RetFilial("HC5")+cNumPed)
	...
	While !Eof() .And. HC6->HC6_FILIAL == RetFilial("HC6") .And. HC6->C6_NUM = cNumPed
		dbDelete()	
		dbSkip()			
	EndDo
...
ElseIf nOperacao = 5  // Inclusao da Nota (Pronta Entrega)
	dbSelectArea("HF2")
	dbSetOrder(1)
	If !dbSeek(RetFilial("HF2")+cNumPed) // Uso a mesma variavel do pedido.
		Return Nil
	EndIf
EndIf
...
If nOperacao = 1
	dbSelectArea("HAT")
	dbSetOrder(1)
	If !dbSeek(RetFilial("HAT")+DtoS(HC5->C5_EMISS) + "1" + HC5->C5_NUM)
		dbAppend()
		lGravaAtend := .T.
		lInclui := .T. 
	Else
...
ElseIf nOperacao = 5 // inclusao da nota Pronta entrega 
	dbSelectArea("HAT")
	dbSetOrder(1)
	If !dbSeek(RetFilial("HAT")+DtoS(HF2->F2_EMISSAO) + "4" + HF2->F2_DOC)
		dbAppend()
		lGravaAtend := .T.
		lInclui := .T. 
	Else
...
ElseIf nOperacao = 6  // Exclusao da Nota (Pronta Entrega) 
	dbSelectArea("HAT")
	dbSetOrder(1)
	If dbSeek(RetFilial("HAT")+cData + "4" + cNumPed)
		dbDelete()
	EndIf		
EndIf
