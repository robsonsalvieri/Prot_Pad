...
Local cResp	:= ""
cResp:= if(MsgYesOrNo(STR0017,STR0018),"Sim","Não") //"Você deseja Excluir o Cliente Selecionado?"###"Cancelar"
If cResp="Sim"
	dbSelectArea("HC5")
	dbSetOrder(2)
	...
	While !Eof() .and. HC5->HC5_FILIAL == RetFilial("HC5") .and. HC5->HC5_CLI == cCodCli .and. HC5->HC5_LOJA == cLojaCli
		If HC5->HC5_STATUS = "N"
			MsgAlert(STR0019,STR0020) //"Não será possível excluir, existem pedidos para este Cliente!"###"Aviso"
			return nil
		Endif
		dbSkip()
	Enddo   
	...
	While !Eof() .And. HU5->HU5_FILIAL == RetFilial("HU5") .And. HU5->HU5_CLIENTE == cCodCli .And. HU5->HU5_LOJA == cLojaCli
		If HU5->HU5_STATUS = "N"
			MsgAlert(STR0021,STR0020) //"Não será possível excluir, existem novos contatos para este Cliente!"###"Aviso"
			return nil
		Endif
		dbSkip()
	Enddo   
	...
	If HA1->(Found()) 
		dbDelete()
		dbSkip()
		MsgAlert(STR0022,STR0020) //"Cliente Excluído com Sucesso!"###"Aviso"
	Endif
	CloseDialog()	
