...
dbSelectArea("HCF")
dbSetOrder(1)
If dbSeek(RetFilial("HCF")+"MV_SFABLOQ")
...
If HC5->(Found())
	While !Eof() .And. HC5->HC5_FILIAL == RetFilial("HC5") .And. HC5->C5_CLI == cCodCli .And. HC5->C5_LOJA
		If HC5->C5_STATUS == "N" .And. HC5->C5_NUM <> cCodPedAlt
			dbSelectArea("HC6")
			dbSetOrder(1)
			dbSeek(RetFilial("HC6")+HC5->C5_NUM)
			If HC6->(Found())
				While !Eof() .And. HC6->HC6_FILIAL == RetFilial("HC6") .And. HC6->C6_NUM == HC5->C5_NUM
            		If HC6->C6_DESC > 0 
