...
Else
	...
	If !Empty(aCabPed[1,1])
		...
		If aCabPed[2,1] == 4  // Gera Novo Pedido a Partir da Consulta de Ultimos Pedidos
			dbSeek(RetFilial("HF1")+aCabPed[3,1] + aCabPed[4,1] + cNumPedSrc)
		Else
			dbSeek(RetFilial("HF1")+aCabPed[3,1] + aCabPed[4,1] + aCabPed[1,1])
		EndIf
        If Found()
...
If aCabPed[2,1] == 1 .Or. aCabPed[2,1] == 4
	dbSelectArea("HF1")
	dbSetOrder(1)
	if dbSeek(RetFilial("HF1")+aCabPed[1,1]) 
		AADD(aDev,{aCabPed[1,1],HF1->HF1_EMISSAO,HF1->HF1_VALOR}) 
...
If dbSeek(RetFilial("HF1")+cNumDev)
	// Guarda a data para excluir o Atendimento
	dData := HF1->HF1_EMISSAO
	...
	dbSetOrder(1)
	dbGoTop()
	dbSeek(RetFilial("HD1")+cNumDev)

	While !Eof() .And. HD1->HD1_FILIAL = RetFilial("HD1") .And. HD1->HD1_DOC = cNumDev
		dbDelete()	
		dbSkip()			
	EndDo
