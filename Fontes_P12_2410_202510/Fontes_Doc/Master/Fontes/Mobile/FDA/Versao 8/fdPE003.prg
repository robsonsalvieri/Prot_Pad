...
If Len(aIteNot) > 0 .And. cTabAnt <> aCabNot[7,1]
	...
	Else
      ...
      For ni := 1 to Len(aIteNot) 
          ...
          dbSelectArea("HPR")
       	  dbSetOrder(1)
		  dbSeek(RetFilial("HPR")+aIteNot[ni,3]+aCabNot[7,1])	//Procura preço de venda usando a nova tabela
			
		  If !Eof()                        
		     aIteNot[ni,23] := HPR->HPR_UNI
		  Else           
			 dbSelectArea("HB1")
			 dbSetOrder(1)
			 dbSeek(RetFilial("HB1")+aIteNot[ni,3])     
			 aIteNot[ni,23] := HB1->HB1_PRV1
		  Endif                                 
...
If !Empty(cTab) .And. !Empty(cCond)
	dbSelectArea("HTC")
	dbSetOrder(1)
	dbSeek(RetFilial("HTC")+cTab) 
	//Alert("Tab. " + cTab + " Cond. " + HTC->HTC_COND)
	If !Empty(HTC->HTC_COND) .And. (HTC->HTC_COND <> cCond)
		MsgStop(STR0010 + HTC->HTC_COND,STR0011) //"Condição de Pagto. inválida para esta tabela de preços. A condição válida é: "###"Aviso"
	 	//aCabNot[7,1] := HTC->HTC_COND	//atualiza condicao pagto.
		lRet := .f.
	Endif
Endif
Return lRet
