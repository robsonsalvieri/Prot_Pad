...
If HAT->(dbSeek(cData))
	While !HAT->(Eof()) .And. HAT->AT_DATA == cData
		...
		If HAT->AT_FLGVIS == "1"
			...
			AADD(aResumo1,{ HA1->A1_NOME, HAT->AT_LOJA, HAT->AT_NUMPED, HAT->AT_VALPED  })
			nOcor3++
			nPedidos3++
			nVisitas3++
			nItems3   += HAT->AT_QTDIT
			nTotVend3 += HAT->AT_VALPED
		...
		If HAT->AT_FLGVIS == "2"
			...
			AADD(aResumo2,{ HA1->A1_NOME, HAT->AT_LOJA ,HX5->X5_DESCRI })
			nOcor2++
		EndIf
		// Terceiro - Monta o Array com Resumo das notas
		If HAT->AT_FLGVIS == "4"
			nNotas4++
			nItems4   += HAT->AT_QTDIT
			nTotNot4  += HAT->AT_VALPED
			nOcor4++
			nVisitas4++
		endif
