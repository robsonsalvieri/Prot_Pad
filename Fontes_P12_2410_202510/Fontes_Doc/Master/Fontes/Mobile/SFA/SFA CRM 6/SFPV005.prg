...
Local nSIte	       := Len(aItePed)//Nr. de itens do pedido
Local nSTPed	   := Round(nTotPed,2)//Total do Pedido 
Local nDPed	       := Round(nDescPedRG,2)//Descto. da regra
...
For ni := 1 To Len(aItePed)
	...
	If aItePed[ni,11] <> 1
		If cCalcProtheus == "T"
			nDescPed := nDescPed + (aItePed[ni,4] * Round((aItePed[ni,6] * (aItePed[ni,7] / 100)),nQtdDec))
		Else
...
// Desconta a Indenizacao (quando houver) do Total de Pedido
If cSfaInd = "T"
	nTPed := nTPed - nInd
EndIf
nTPed := Round(nTPed,2)
