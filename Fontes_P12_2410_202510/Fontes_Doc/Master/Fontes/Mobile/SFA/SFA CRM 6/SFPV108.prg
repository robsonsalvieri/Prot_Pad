...
If dbSeek(cNumPed)
	...
	If aCabPed[2,1] == 4
		...
		Return nil
	Else      // Se for Alteracao do Pedido ou Consulta dos itens
	...
	While !Eof() .And. HC6->C6_NUM == cNumPed
		...
		if dbSeek(HC6->C6_PROD)
			...
			If HB1->(FieldPos("B1_PBRUTO")) > 0
				...
				If 	cUmPeso = "1"
					aCabPed[17,1] := aCabPed[17,1] + (HC6->C6_QTDVEN * nPrdPeso)
				Else
					If HB1->B1_TIPCONV = "M"
     		...
     		If aColIte[nI,2] <> 0  // Se o FIELDPOS for Diferente de Zero
     			...
     			If nI != 14
  				...
  				Else
					...
					ElseIf HC6->C6_STATUS = "N"
						cStPedido := STR0012 //"Novo"
					Else
						cStPedido := STR0009    //"Indefinido"
					EndIf
					aColIte[nI,1] := HC6->C6_STATUS
  				EndIf
			Endif
		Next 
...
If cCalcProtheus == "T"	//Desconto Protheus
	If lAdcIte
		...
		nTotItem := aColIte[4,1] * Round((aColIte[6,1] - (aColIte[6,1] * (aColIte[7,1] / 100))),nQtdDec)
	Else
		nTotItem := aItePed[nItePed,4] * Round((aItePed[nItePed,6] - (aItePed[nItePed,6] * (aItePed[nItePed,7] / 100))),nQtdDec)
	Endif
Else	//Desconto Padrao
	If lAdcIte 
		// Total item = total item - (total item * (desct / 100))
		nTotItem := aColIte[9,1] - Round((aColIte[9,1] * (aColIte[7,1] / 100)),nQtdDec)
	Else
		nTotItem := aItePed[nItePed,9] - Round((aItePed[nItePed,9] * (aItePed[nItePed,7] / 100)),nQtdDec)
	Endif
Endif
