...
While !Eof() .And. AllTrim(HUW->UW_CODCAMP) == AllTrim(cCodCamp)
    HUZ->(dbSetOrder(1))
    if HUZ->(dbSeek(RetFilial("HUZ") + HUW->UW_CODSCRI))
...
dbSelectArea("HUE")
dbSetOrder(2)
dbSeek(RetFilial("HUE") + cCodScr+cSep)
...
For nI:=1 to Len(aPergC)
	...
	if dbSeek(RetFilial("HRE") + aPergc[nI,1])
		HUC->(dbAppend())
		HUC->UC_FILIAL := RetFilial("HUC")
		HUC->UC_CODIGO		:=cCodigo
		HUC->UC_CODCAMP		:=HUW->UW_CODCAMP
		HUC->UC_CLIENTE 	:=HA1->HA1_COD
		HUC->UC_LOJA		:=HA1->HA1_LOJA
		HUC->UC_CODCONT		:=HU5->HU5_CODCON
		HUC->UC_DATA		:=Date()
		HUC->UC_INICIO      :=tIni
		HUC->UC_FIM			:=Time()
		HUC->(dbCommit())
