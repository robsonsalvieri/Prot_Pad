...
dbSelectArea("HX5")
dbSetOrder(1)
dbSeek(RetFilial("HX5") + "OC")            

While !eof() .and. HX5->HX5_TABELA = "OC"
	AADD(aOco,Alltrim(HX5->HX5_CHAVE) + "-" + AllTrim(HX5->HX5_DESCRI))
	dbSkip()
Enddo
AADD(aOco,"")
...
dbSelectArea(cAlias)
dbSetOrder(1)
If !dbSeek(RetFilial(cAlias) + cSeek)
...
If !Empty(cOcoCad)
	If Empty(cOco)
		...
		If MsgYesOrNo(STR0005,STR0006) //"Você deseja Excluir a Ocorrência?"###"Cancelar"
			...
			GrvAtend(4, , cOco, HA1->HA1_COD, HA1->HA1_LOJA,)
	...
	Else
		...
		GrvAtend(3,, cOco, HA1->HA1_COD, HA1->HA1_LOJA,)
...
//Marca para nao exportar a alteracao no HA1
...
If Empty(cCodRot)
	dbSelectArea("HD7")
	dbSetOrder(3)
	If !dbSeek(RetFilial("HD7") + DtoS(dData)+HA1->HA1_COD + HA1->HA1_LOJA )
		lUpdRot := .F.
	EndIf
Else
	dbSelectArea("HD7")
...
If lUpdRot
	HD7->HD7_FLGVIS	:=cNpos
	HD7->HD7_OCO    :=cOco
	dbCommit()
EndIf

// Atualiza Ocorrencia no Cliente
HA1->HA1_FLGVIS	:=cNpos
HA1->HA1_OCO	:=cOco
dbCommit()
...
dbSetOrder(1)
If dbSeek(RetFilial("HD5") + HA1->HA1_COD + HA1->HA1_LOJA + DtoS(dData))
	If !MsgYesOrNo("Ja existe ocorrencia para esse cliente nesta Data!"+Chr(10)+"Deseja Gravar uma nova Ocorrencia?","Atenção")
		Return Nil
	EndIf
EndIf
dbAppend()
HD5->HD5_FILIAL := RetFilial("HD5")
HD5->HD5_ROTER := cCodRot
HD5->HD5_CODNPO := cOco
HD5->HD5_DTHR := DtoS(dData)
HD5->HD5_CODCLI := HA1->HA1_COD
HD5->HD5_LOJA :=  HA1->HA1_LOJA
dbCommit()
