...
function LoginGed(pServerName: PAnsiChar;pTrustedLogin:Boolean=False; vServerName:string=''; lCloud: Boolean=False): WordBool;
var
...
begin
  ...
  Begin
    ...
    Begin
      ...
      vIManSessions.Add(pServerName,vIManSession);
    End;
  ...
  Begin
    ...
    Begin
      CoInitialize(nil);
      vIManDMS := CoManDMS.Create;
      cxt := CoContextItems.Create;
      cmd := CoLoginCmd.Create;

      Try
        cxt.Add('NRTDMS', vIManDMS);
        cxt.Add('SelectedNRTSessionNames', vServerName);  // server is your server name
        ...
        If pTrustedLogin Then
        Begin
          cxt.Add('IManExt.LoginDlg.TrustedLogin', true); // This is only required if you are not using REST login
          cxt.Add('IManExt.SilentLogin', true);  // Uncomment to prevent login form displaying
        End;
        ...
        //    MessageDlg('Status..' +  inttostr(cmd.Status) , mtWarning,[mbok],0);
        // Now we are connected and have a session object
        //vIManDMS.Get_Sessions( vIManSessions );
        IManDMS(vIManDMS).Get_Sessions(vIManSessions);
        vIManSessions.ItemByIndex(1,vIManSession);
      Except
        On e:exception Do
        Begin
          ShowMessage('Erro - ' + e.message);
          fnCreateFile('SIGAGEDW.LOG', 'Error on Login:' + e.Message);
        End;
      End ;

      If Assigned(vIManSession) Then
      Begin
        vIManSession.Get_Connected(Result);
      End ;
    End;
...
function ImportDoc(plcParams: String): String;
var
    ...
    Try
      Try
        ...
        Begin
          ...
          fnCreateFile('SIGAGEDW.LOG', 'Retorno: ' + vRetorno );
          Result := vRetorno;
        End;
      Except
        On e: Exception Do
...
function AttachDoc(pcParametros: String): String;
var
  vConnected: WordBool;
...
begin
    ...
    cUsuario := Base64Decode(pUserID);   //Descriptografa da Base64.
    cSenha   := Base64Decode(pPassword);
...
begin
  Result := -1;
  fnCreateFile('SIGAGEDW.LOG', 'aParams ' + aParams + ' | aFuncId ' + IntToStr(aFuncID));
  vParams := vParams + aParams;

  try
    if aFuncID = 1 then  //Login no GED WorkSite (Interwoven) via TrustedLogin
    begin
      if LoginGed(aParams, True, vParams) then
        Result := 1
      else
        Result := -1;
    end
    ...
    begin
      if LoginGed(aParams, False, vParams) then
        Result := 1
      else
        Result := -1;
    end
    ...
    begin
      vDoc := ImportDoc(aParams);
      if trim(vDoc) <> '' then
      begin
        vChar := PAnsiChar(AnsiString(vDoc));
        StrECopy(aBuff, vChar);
        Result := 1
      end else
        Result := -1;
    end
    ...
    begin
      ...
      begin
        ...
        Result := -1;
    End 
    Else If aFuncID = 7 Then // Login Cloud Sem TrustedLogin
    Begin
      If LoginGed(aParams, True, vParams, True) then
        Result := 1
      Else
        Result := -1;
    End
    Else If aFuncID = 8 Then // Login Cloud Com TrustedLogin
    Begin
      If LoginGed(aParams, False, vParams, True) then
        Result := 1
      Else
        Result := -1;
    End;    
