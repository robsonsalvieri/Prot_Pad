function createDataset(fields, constraints, sortFields) {
	...
	var nFormId = parseInt(dsParamsSIGAJURI.getValue(0, "nFormIdContrato"));
	...
	var campos = new Array('cdAssJur', 'sAssJur', 'cdTipoCon', 'sTipoCon', 'cdGrupo', 'sGrupo', 'cdUser', 'sUser', 'sPrioridade', 'sPrazo');
	...
	try{
		...
		cardDto.setAdditionalComments("Ficha criada pelo Widget SIGAJURI_Contratos");
...
function getValores(constraints){
	...
	for (var i = 0; i < constraints.length; i++){
		...
		} else if (constraints[i].fieldName == "sTipoCon"){
...
function isUpdate(valores){
	...
	var cdTipoCon = valores[2];
	var cdGrupo = valores[4];
	var cdUser = valores[6];
	...
	var dsCards = DatasetFactory.getDataset("wcmSIGAJURI_Contratos", fields, constraints, null);
