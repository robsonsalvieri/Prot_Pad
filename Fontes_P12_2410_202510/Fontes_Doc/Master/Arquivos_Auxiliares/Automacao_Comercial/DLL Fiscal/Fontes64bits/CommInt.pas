...
uses
  Windows, Messages, SysUtils, System.Classes, CommObjs;
...
type
  ...
  TCommEvent = procedure(Sender: TObject; Status: UINT) of object;
  ...
  protected
    ...
    procedure HandleCommEvent(Sender: TObject; Status: UINT);
    procedure UpdateDataControlBlock;
    property  DeviceName: AnsiString read FDeviceName write SetDeviceName;
...
const
  ...
  CommEventList: array[TCommEventType] of UINT =
    ( EV_BREAK,
      EV_CTS,
      EV_DSR,
      EV_ERR,
...
{ RaiseCommError }
procedure RaiseCommError(Msg: AnsiString; ErrCode: Integer);
var
  E: ECommError;
begin
...
var
   cNewFDeviceName : array [0..200] of Char;
begin
  StrPCopy(cNewFDeviceName,FDeviceName);
  FHandle := CreateFile( cNewFDeviceName,
...
begin
  ...
  try
    FillChar(Overlapped, Sizeof(Overlapped), 0);
    Overlapped.hEvent := FEvent.Handle;
    if (not WriteFile(FHandle, Buf, Count, DResult,
    ...
    if FEvent.WaitFor(FWriteTimeout) <> wrSignaled then
     ...
     begin
       GetOverlappedResult(Handle, Overlapped, DResult, False);
       FEvent.ResetEvent;
     end;
     Result := DResult;
  finally
...
begin
  ...
  try
    FillChar(Overlapped, Sizeof(Overlapped), 0);
    Overlapped.hEvent := FEvent.Handle;
    if (not ReadFile(FHandle, Buf, Count, DResult,
          @Overlapped)) and (GetLastError <> ERROR_IO_PENDING) then
    begin
      ErrorCode := GetLastError;
...
procedure TCustomComm.SetDeviceName(const Value: AnsiString);
begin
