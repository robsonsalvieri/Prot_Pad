...
(function () {
  ...
  contextPrototype.text = function(aX, aY, aText, aColor) {
    this.currentPath_.push({type: "text", x: aX, y: aY, text: aText, color: aColor});
  };
  ...
  contextPrototype.stroke = function(aFill) {
    ...
    lineStr.push(' ">');
	  ...
	  if (this.shadowColor)
  	{
    	var a = processStyle(this.shadowColor);
	    var shadowColor = a[0];
  	  lineStr.push('<g_vml_:shadow on="true"',
//  	             ' type="', 'emboss', '"', 
//	  	             ' obscured=', 'true', 
//  	             ' opacity=', 0.5,
    	             ' offset="', this.shadowOffsetX,'pt,', this.shadowOffsetX3,'pt"',
      	           ' color="', shadowColor, '" />');
		}  
    ...
    for (var i = 0; i < this.currentPath_.length; i++) {
      ...
      if (p.type == "text") {
        var c = this.getCoords_(p.x, p.y);
    		lineStr.push('<g_vml_:rect style="position:absolute; left:', c.x, ';top:', c.y,';xwidth:300;xheight:240;">')
    		lineStr.push('<g_vml_:textbox>');
    		if (p.color)
		    	lineStr.push('<span style="color:', p.color, '">');
	    	lineStr.push( p.text);
    		if (p.color)
		    	lineStr.push('</span>');
    		lineStr.push('</g_vml_:textbox>');
		    lineStr.push('</g_vml_:rect>');
			}
		}
  ...
  contextPrototype.createPattern = function() {
	...
	contextPrototype.drawText = function(aX, aY, aText)
  {  
    var a = processStyle(this.color);
    var color = a[0];       
    
    this.beginPath();
    this.text(aX, aY, aText, color);
    this.stroke();
