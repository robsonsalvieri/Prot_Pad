...
function verificaCPF(xcpf){
	...
	var dac = "", inicio = 2, fim = 10, soma, digito, i, j, x, auxcpf = ""
	
	
	for (x=0;x<=cpf.length-1;x++) {  
		auxcpf += cpf.substring(0,1)
	}
	if (auxcpf == cpf) {
		return( false )  
	}
...
function formatahora(hora) {  
	...
	if( hora.indexOf(":")!= -1 ) 
	{
		...
		if(parseInt(strmin) > 59){strmin="59"}
		retorno = hora.substr(0,hora.indexOf(":")) + ':' + strmin
		if(strmin.length == 1){retorno = retorno + '0'}
		return retorno;
	}
	if(hora.length == 0){
		hora = '0:00';}
...
function numero( e )
{
	...
	if( x > 47 && x < 58 || x == 8 || ( x == 46 && valor.indexOf( "." ) == -1 ) )
	{
		if(x.length==4){
		obj.value = obj.value + ':';}
		return true;
