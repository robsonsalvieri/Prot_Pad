var wcmContrato = SuperWidget.extend({
	loading: null, // tela de Loading
	configContrato: null, // Datatable
    ...
    init: function(){
    	...
    	if(!this.isEditMode){
	    	...
	    	this.loading = FLUIGC.loading('#wcmContrato_'+this.instanceId);
	    	...
	    	// Carrega os dados das configuracoes de Contratos existentes na Datatable.
	    	this.updateDatatableCon();
	    	
	    	// Carrega os Dropdowns de Assunto Juridico e Tipos de Follow-Up
	    	this.loadDatasetValues('dsAssJur', ['id', 'AssJur'], new Array(), null);
	    	this.loadDatasetValues('dsTipoContrato', ['id', 'TipoCon'], new Array(), null);
    	}
    },
    ...
    bindings: {
        local: {
            ...
            'save-button': ['click_saveContrato'],
            'clear-button': ['click_clearContrato'],
            'delete-button': ['click_deleteContrato'],
    ...
    loadDatasetValues : function(name, fields, constraints, order){
    	...
    	case "dsAssJur":
    		...
    		break;
    	case "dsTipoContrato":
    		msg = "${i18n.getTranslation('application.loading.dsTipoCon')}";
    		break;
    	case "wcmSIGAJURI_Contratos":
    		msg = "${i18n.getTranslation('application.loading.wcmSIGAJURI_Contratos')}";
    		break;
    	case "dsSaveContrato":
    		msg = "${i18n.getTranslation('application.loading.dsSaveContrato')}";
    		break;
    	case "dsDeleteContrato":
    		msg = "${i18n.getTranslation('application.loading.dsDeleteContrato')}";
    		break;
    	default:
    		...
    		success: function(data){ // Funcao callback de sucesso.
    			...
    			if(data != null && data.values != null && data.values.length > 0){ // Caso o resultado não seja vazio.
    				...
    				case "dsTipoContrato":
    					...
    					_this.loadSelect(data.values, 'TipoCon');
    					break;
    		    	case "wcmSIGAJURI_Contratos":
    		    		// Carregar o Datatable de configurações de Contrato
    				...
    				case "dsSaveContrato":
    					...
    					if (data.values[0]['Status']=='ok'){
	    					FLUIGC.toast({
	        			        title: "${i18n.getTranslation('application.success.Title.dsSaveContrato')}",
	        			        message: "${i18n.getTranslation('application.success.dsSaveContrato')}",
	        			        type: 'success'
	        			    });
	    		    		_this.clearContrato();
    					} else {
    				...
    				case "dsDeleteContrato":
    					...
    					if (data.values[0]['Status']=='ok'){
	    					FLUIGC.toast({
	        			        title: "${i18n.getTranslation('application.success.Title.dsDeleteContrato')}",
	        			        message: "${i18n.getTranslation('application.success.dsDeleteContrato')}",
	        			        type: 'success'
	        			    });
	    		    		_this.clearContrato();
    					} else {
    		    	...
    		    	case "dsAssJur":
    		    		...
    		    		break;
    		    	case "dsTipoContrato":
    		    		msgVazio = "${i18n.getTranslation('application.warning.dsTipoCon')}";
    		    		titleVazio = "${i18n.getTranslation('application.warning.Title.dsTipoCon')}";
    		    		typeVazio = 'error';
    		    		break;
    		    	case "wcmSIGAJURI_Contratos":
	...
	loadSelect: function(data, tipoSelect){
		...
		case "AssJur":
    		...
    		break;
    	case "TipoCon":
    		select = $('#sTipoContrato_'+this.instanceId);
    		options = '<option value="">' + "${i18n.getTranslation('application.placeholder.TiposContrato.StandardValue')}" + '</option>';
    		break;
    	default: return;
		}
	...
	clearZoom: function(el,ev){
		...
		var val  = $('#cdGrupoResponsavel_'+this.instanceId).val();
    ...
    saveContrato: function(){
    	...
    	if (this.validateFields()){
	    	...
	    	constraints.push(DatasetFactory.createConstraint('cdTipoCon', $('#sTipoContrato_'+this.instanceId).val(), $('#sTipoContrato_'+this.instanceId).val(), ConstraintType.MUST));
			constraints.push(DatasetFactory.createConstraint('sTipoCon', $('#sTipoContrato_'+this.instanceId+' :selected').text(), $('#sTipoContrato_'+this.instanceId+' :selected').text(), ConstraintType.MUST));
	    	...
	    	this.loadDatasetValues('dsSaveContrato', fields, constraints, null);
    	}
    },
    ...
    deleteContrato: function(){
    	...
    	var selectedRow = this.configContrato.getRow(this.configContrato.selectedRows()[0]);
    	...
    	if (selectedRow != null){
        	...
        	this.loadDatasetValues('dsDeleteContrato', fields, constraints, null);    		
    	}
    },
    
    deleteContratoById: function(id){
		var fields = new Array();
    	var constraints = new Array();
     ...
     * Para a parte inicial, ver updateDatatableCon.
    ...
    loadDatatableCon: function(data){
    	...
    	this.configContrato = FLUIGC.datatable('#dtContratos_'+this.instanceId, {
    		dataRequest: data,
    		renderContent: '.template_datatableCon',
    	    //renderContent: ['metadata#id', 'cdAssJur', 'sAssJur', 'cdTipoCon', 'sTipoCon', 'cdGrupo', 'sGrupo', 'cdUser', 'sUser', 'sPrioridade'],
    	    header: [
    	        {
    	        ...
    	        {
    	        	'title': "${i18n.getTranslation('application.datatable.column.cdTipoCon')}",
    	        	'display': false
				},
				{ 'title': "${i18n.getTranslation('application.datatable.column.sTipoCon')}"},
				{
    ...
    datatableBinding: function(){
    	...
    	$('#dtContratos_'+this.instanceId).on('fluig.datatable.onselectrow', function(){
    	    var selectedRow = _this.configContrato.getRow(_this.configContrato.selectedRows()[0]);
    	    $('#sAssuntoJuridico_'+_this.instanceId).val(selectedRow.cdAssJur);
			$('#sTipoContrato_'+_this.instanceId).val(selectedRow.cdTipoCon);
    	...
    	$('#dtContratos_'+this.instanceId).on('click', '[data-delete-row]', function(ev){
     ...
     */
    updateDatatableCon: function(){
    	// Recupera os valores dos selects para que eles atuem como filtros.
    	var selectAssJur = $('#sAssuntoJuridico_'+this.instanceId).val();
    	var selectTipoCon = $('#sTipoContrato_'+this.instanceId).val();
    	
    	// Prepara variaveis de Campos, Filtros e Ordenação para chamar o Dataset.
    	var fields = new Array('metadata#id', 'cdAssJur', 'sAssJur', 'cdTipoCon', 'sTipoCon', 'cdGrupo', 'sGrupo', 'cdUser', 'sUser', 'sPrioridade', 'sPrazo');
    	var constraints = new Array();
    	var sort = new Array();
    	...
    	if (selectTipoCon != ''){
    		constraints.push(DatasetFactory.createConstraint('cdTipoCon', selectTipoCon, selectTipoCon, ConstraintType.MUST));
    	}
    	...
    	this.loadDatasetValues('wcmSIGAJURI_Contratos', fields , constraints, sort);
    },
    ...
    validateFields: function(){
		...
		var cdTipoCon = $('#sTipoContrato_'+this.instanceId).val();
	    ...
	    if (cdTipoCon == ''){ // Valida se o campo Tipo de Follow-Up foi preenchido.
	    	msg += "<li>${i18n.getTranslation('application.label.TipoCon')}</li>";
	    	success = false;
		}
		if (cdGrupo == '' && cdUser == ''){ // Valida se o campo Grupo foi preenchido.
	    	msg += "<li>${i18n.getTranslation('application.error.Validation.User.Group')}</li>";
	    	success = false;
	    }
