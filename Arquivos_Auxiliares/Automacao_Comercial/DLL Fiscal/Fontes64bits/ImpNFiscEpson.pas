...
type
  ...
  TImpNfEpsonT88V = class(TImpNFiscal)
  private
  public
  ...
  Function OpenEpsonNF( sPorta:AnsiString; iVelocidade : Integer ):AnsiString;
  Function CloseEpsonNF : AnsiString;
...
var
        ...
        fFuncImprimeTexto		: function(Texto : AnsiString) :Integer; StdCall;
        fFuncImprimeTextoTag		: function(Texto : AnsiString) : Integer; StdCall;
        fFuncFormataTX		        : function(Texto : AnsiString ; TipoLetra : Integer; Italico : Integer; Sublinhado : Integer; Expandido : Integer; Enfatizado : Integer) : Integer; StdCall;
        fFuncAcionaGuilhotina	        : function(Aciona : Integer) : Integer; StdCall;
        fFuncComandoTX		        : function(Comando : AnsiString; Tamanho : Integer) : Integer; StdCall;
        ...
        fFuncImprimeCodigoBarrasCODABAR : function(Texto : AnsiString) : Integer; StdCall;
        fFuncImprimeCodigoBarrasCODE128 : function(Texto : AnsiString) : Integer; StdCall;
        fFuncImprimeCodigoBarrasCODE39  : function(Texto : AnsiString) : Integer; StdCall;
        fFuncImprimeCodigoBarrasCODE93  : function(Texto : AnsiString) : Integer; StdCall;
        fFuncImprimeCodigoBarrasEAN13   : function(Texto : AnsiString) : Integer; StdCall;
        fFuncImprimeCodigoBarrasEAN8	: function(Texto : AnsiString) : Integer; StdCall;
        fFuncImprimeCodigoBarrasITF	: function(Texto : AnsiString) : Integer; StdCall;
        fFuncImprimeCodigoBarrasUPCA	: function(Texto : AnsiString) : Integer; StdCall;
        fFuncImprimeCodigoBarrasUPCE	: function(Texto : AnsiString) : Integer; StdCall;
        fFuncImprimeCodigoBarrasPDF417  : function(Correcao : Integer; Altura : Integer; Largura : Integer; Colunas : Integer; Codigo : AnsiString) : Integer; StdCall;
        fFuncImprimeCodigoQRCODE	: function(Restauracao : Integer; Modulo : Integer; Tipo : Integer; Versao : Integer; Modo : Integer; Codigo : AnsiString) : Integer; StdCall;
        fFuncGerarQRCodeArquivo	        : function(FileName : AnsiString; Dados : AnsiString) : Integer; StdCall;
        fFuncImprimeBmpEspecial	        : function(FileName : AnsiString; nX : Integer; nY : Integer; Angulo : Integer) : Integer; StdCall;
        fFuncHabilita_Log		: function(Estado : Integer; Caminho : AnsiString) : Integer; StdCall;
...
//----------------------------------------------------------------------------
Function OpenEpsonNF( sPorta:AnsiString; iVelocidade : Integer) : AnsiString;
  function ValidPointer( aPointer: Pointer; sMSg:AnsiString; sArqDll:AnsiString = 'INTERFACEEPSONNF.DLL' ) : Boolean;
  begin
...
var
  ...
  bRet : Boolean;
  sPathLog,sVelIni,sIni : AnsiString;
  fArq : TIniFile;
...
begin
  ...
  Begin
    ...
    begin
      LjMsgDlg('A dll InterfaceEpsonNF não foi encontrado.');
      bRet := False;
    end;
    ...
    begin
      ...
      begin
        GravaLog('EPSON Nao Fiscal -> ConfiguraSerial ');
        ...
        begin
          ShowMessage('A velocidade da porta está diferente da velocidade cadastrada no arquivo SIGALOJA.INI.' + CHR(10)+CHR(13)+
        ...
        GravaLog('EPSON Nao Fiscal <- ConfiguraSerial :' + IntToStr(iRet));
      end;

      GravaLog('EPSON Nao Fiscal -> IniciaPorta :' + sPorta);
      iRet := fFuncIniciaPorta(Trim(sPorta));
      GravaLog('EPSON Nao Fiscal <- IniciaPorta : ' + IntToStr(iRet));

      If iRet <> 1 then
      begin
        LjMsgDlg('EPSON Nao Fiscal : Erro na abertura da porta' + CHR(10)+CHR(13)+
        ...
        GravaLog('EPSON Nao Fiscal : Erro na abertura da porta' + CHR(10)+CHR(13)+
                 'Verifique: ' + CHR(10)+CHR(13)+
                 '- a configuração do arquivo SIGALOJA.INI' + CHR(10)+CHR(13)+
                 '- chave [LogDLL], seção VelPorta : configurando a velocidade da porta para conexão com a impressora' + CHR(10)+CHR(13)+
                 '- se o Windows está se comunicando com a impressora: teste por meio do programa do fabricante a impressão');
      ...
      begin
       ...
       GravaLog('EPSON Nao Fiscal -> Habilita_Log ');
       iRet := fFuncHabilita_Log(1,sPathLog);
       GravaLog('EPSON Nao Fiscal <- Habilita_Log : ' + IntToStr(iRet));

       If iRet = 1
       then GravaLog('EPSON Nao Fiscal -> Arquivo de Log de Impressora Habilitado em : "' + sPathLog + '"')
       else GravaLog('EPSON Nao Fiscal -> Arquivo de Log não Habilitado ');
...
begin
  ...
  Begin
    ...
    begin
      GravaLog('EPSON Nao Fiscal -> FechaPorta ');
      iRet := fFuncFechaPorta();
      GravaLog('EPSON Nao Fiscal <- FechaPorta :' + IntToStr(iRet));
...
//----------------------------------------------------------------------------
function TImpNfEpsonT88V.Abrir(sPorta : AnsiString; iVelocidade : Integer ; iHdlMain:Integer) : AnsiString;
Var
  sRet : AnsiString;
begin

If Not bOpened
Then sRet := OpenEpsonNF(sPorta, iVelocidade )
Else sRet := '0|';

If Copy(sRet,1,1) = '0'
then GravaLog('EPSON Nao Fiscal : Sucesso ao abrir porta');
...
//----------------------------------------------------------------------------
function TImpNfEpsonT88V.Fechar( sPorta:AnsiString ):AnsiString;
begin
Result := CloseEpsonNF;
end;

//----------------------------------------------------------------------------
function TImpNfEpsonT88V.ImpTexto( Texto : AnsiString):AnsiString;
var
  iRet,nPos  : Integer;
  oTexto : TStringList;
  sAux,sTextoImp, sTextoEnv: AnsiString;
  bCorte : Boolean;
begin
bCorte    := False;
sTextoImp := Texto;
...
begin
  ...
  If iRet = 1 then
  begin
    GravaLog('EPSON Nao Fiscal -> AcionaGuilhotina ');
    iRet := fFuncAcionaGuilhotina(1); //Manda corte inteiro
    GravaLog('EPSON Nao Fiscal <- AcionaGuilhotina :' + IntToStr(iRet));
  end;
...
begin
  ...
  begin
    ...
    begin
      GravaLog('EPSON Nao Fiscal -> ImprimeTextoTag : ' + Texto);
      ...
      Begin
         ...
         Begin
           GravaLog('EPSON Nao Fiscal -> ConfiguraCodigoBarras');
           iRet := fFuncConfiguraCodigoBarras(50, 1, 0, 0, 30);
           GravaLog('EPSON Nao Fiscal <- ConfiguraCodigoBarras :' + IntToStr(iRet));
         End;
         ...
         End;
      End;
      GravaLog('EPSON Nao Fiscal <- ImprimeTextoTag :' + IntToStr(iRet));
    end
    else
    begin
      GravaLog('EPSON Nao Fiscal -> ImprimeTexto : ' + Texto);
      For nPos := 0 to Pred(oTexto.Count) do
          iRet := fFuncImprimeTexto(oTexto.Strings[nPos]);
      GravaLog('EPSON Nao Fiscal <- ImprimeTexto :' + IntToStr(iRet));
    end;
  end;  
...
function TImpNfEpsonT88V.StatusImp(Tipo: Integer): AnsiString;
begin
...
//----------------------------------------------------------------------------
function TImpNfEpsonT88V.ImpCodeBar( Tipo,Texto:AnsiString  ):AnsiString;
var
  iRet : Integer;
  sAux : AnsiString;
begin
...
begin
    ...
    If iRet = 1 then
    begin
      GravaLog('EPSON Nao Fiscal -> ImpCodeBar ( Tipo :' + Tipo + '; Texto: '+ Texto + ')');
      ...
      If Tipo = '<upc-a>'
         ...
         GravaLog('EPSON Nao Fiscal - Tipo de Código de Barras :' + Tipo + ' não encontrado ');

      GravaLog('EPSON Nao Fiscal <- ImpCodeBar :' + IntToStr(iRet));
...
//----------------------------------------------------------------------------
function TImpNfEpsonT88V.ImpBitMap( Arquivo:AnsiString ):AnsiString;
var
  iRet : Integer;
begin
...
If iRet = 1 then
begin
  GravaLog('EPSON Nao Fiscal -> ImprimeBmpEspecial');
  iRet := fFuncImprimeBmpEspecial(Arquivo,0,0,0);
  GravaLog('EPSON Nao Fiscal <- ImprimeBmpEspecial : ' + IntToStr(iRet));
end;
...
//----------------------------------------------------------------------------
function TImpNfEpsonT88V.VerStatus(): Integer;
var
   iRet : Integer;
   sMsg : AnsiString;
begin
GravaLog('EPSON Nao Fiscal -> VerStatus');
iRet := fFuncLe_Status();
GravaLog('EPSON Nao Fiscal <- VerStatus : ' + IntToStr(iRet));
...
//----------------------------------------------------------------------------
function TImpNfEpsonT88V.TrataTags( var Texto : AnsiString ): AnsiString;
var
   sRet,sAux,sCorrigeQrCode : AnsiString;
   nX : Integer;
begin
sRet := '';
...
begin
  ...
  GravaLog(' EPSON Não Fiscal - Inserida a tag <lmodulo>5</lmodulo> para impressão do QRCode ');
end;
...
//----------------------------------------------------------------------------
function TImpNfEpsonT88V.AbreGaveta: AnsiString;
var
  iRet : Integer;
begin

GravaLog('EPSON Nao Fiscal -> AcionarGaveta');
iRet := fFuncAcionaGaveta();
GravaLog('EPSON Nao Fiscal <- AcionarGaveta : ' + IntToStr(iRet));
...
//=============================================================================
initialization
  RegistraImpressora('EPSON TM-T88V'  , TImpNfEpsonT88V  , 'BRA' ,'      ');
  RegistraImpressora('EPSON TM-T20'  , TImpNfEpsonT88V  , 'BRA' ,'      ');
