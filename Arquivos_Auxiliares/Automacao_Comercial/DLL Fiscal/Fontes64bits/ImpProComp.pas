...
Type
  ...
  public
    ...
    function LeStatus( var S:AnsiString ): Integer;
...
//---------------------------------------------------------------------------
function TImpFiscalProComp.Abrir(sPorta : AnsiString; iHdlMain:Integer) : AnsiString;
...
//---------------------------------------------------------------------------
function TImpFiscalProComp.Fechar( sPorta:AnsiString ) : AnsiString;
begin
  result := '0';
  FreeLibrary(fHandle);
  fHandle := 0;
...
//---------------------------------------------------------------------------
function TImpFiscalProComp.ImpostosCupom(Texto: AnsiString): AnsiString;
begin
  Result := '0';
end;

//---------------------------------------------------------------------------
function TImpFiscalProComp.LeituraX : AnsiString;
var
  iRet : Integer;
  sPar : AnsiString;
begin
...
//---------------------------------------------------------------------------
function TImpFiscalProComp.LeAliquotas:AnsiString;
var
  iRet : Integer;
  sPar : AnsiString;
  sRet : AnsiString;
...
function TImpFiscalProComp.LeAliquotasISS:AnsiString;
var
  iRet : Integer;
  sPar : AnsiString;
  sRet : AnsiString;
...
//---------------------------------------------------------------------------
function TImpFiscalProComp.LeCondPag:AnsiString;
Begin
   result := '0|' + FormasPgto;
end;
//----------------------------------------------------------------------------
function TImpFiscalProComp.PegaPDV:AnsiString;
var
  iRet : Integer;
  sRet    : AnsiString;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.PegaCupom(Cancelamento:AnsiString):AnsiString;
var
  sRet  : AnsiString;
  iRet  : Integer;
begin
...
//---------------------------------------------------------------------------
function TImpFiscalProComp.ReducaoZ( MapaRes:AnsiString ) : AnsiString;
var
  iRet : Integer;
  sRet    : AnsiString;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.GravaCondPag( condicao:AnsiString ):AnsiString;
var
  iRet : Integer;
  sPagto : AnsiString;
  iPos : Integer;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.AdicionaAliquota( Aliquota:AnsiString; Tipo:Integer ): AnsiString;
var
  iRet : Integer;
  sPar : AnsiString;
  sRet : AnsiString;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.AbreCupom(Cliente:AnsiString; MensagemRodape:AnsiString):AnsiString;
var
  iRet : Integer;
begin
  iRet := fFuncAbreCupom;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.CancelaCupom( Supervisor:AnsiString ):AnsiString;
var
  iRet : Integer;
begin
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.RegistraItem( codigo,descricao,qtde,vlrUnit,vlrdesconto,aliquota,vlTotIt,UnidMed:AnsiString; nTipoImp:Integer ): AnsiString;

Var sFormato,
    sTipoDesc,
    sUnidade,
    sTamDescr,
    sLegendaOp : AnsiString;
    iRet,i,iPos : Integer;
    sPar : AnsiString;
    sRet : AnsiString;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.CancelaItem( numitem,codigo,descricao,qtde,vlrunit,vlrdesconto,aliquota:AnsiString ):AnsiString;
var
  iRet : Integer;
begin
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.DescontoTotal( vlrDesconto:AnsiString;nTipoImp:Integer ): AnsiString;
var
  iRet        : Integer;
  sOper       : AnsiString;
  sTipo       : AnsiString;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.AcrescimoTotal( vlrAcrescimo:AnsiString ): AnsiString;
var
  iRet        : Integer;
  sOper       : AnsiString;
  sTipo       : AnsiString;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.Pagamento( Pagamento,Vinculado,Percepcion:AnsiString ): AnsiString;

Var
   sReg,  sSubtr : AnsiString;
   iRet,i,iPos: Integer;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.FechaCupom( Mensagem:AnsiString ):AnsiString;
Var
   sTamMsg,sMsg:AnsiString;
   iRet: Integer;
begin
   sMsg := Mensagem;
   sMsg := TrataTags( sMsg );
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.AbreCupomNaoFiscal( Condicao,Valor,Totalizador,Texto:AnsiString ): AnsiString;
var
  iRet        : Integer;
  i           : Integer;
  sPagto      : AnsiString;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.TextoNaoFiscal( Texto:AnsiString;Vias:Integer ):AnsiString;
var
  iRet   : Integer;
  sLinha : AnsiString;
  i      : Integer;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.FechaCupomNaoFiscal: AnsiString;
var
  iRet : Integer;
begin
...
//---------------------------------------------------------------------------
function TImpFiscalProComp.TotalizadorNaoFiscal( Numero,Descricao:AnsiString ):AnsiString;
var
  iRet : Integer;
begin
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.MemoriaFiscal( DataInicio,DataFim:TDateTime ;ReducInicio,ReducFim,Tipo:AnsiString ): AnsiString;
var
  iRet : Integer;
  sTipo: AnsiString;
begin
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.Autenticacao( Vezes:Integer; Valor,Texto:AnsiString ): AnsiString;
var
  iRet : Integer;
  i : Integer;
  //sTipo,sLocal,sPos:AnsiString;
begin
  Texto:=Trim(Texto);
  if Texto<>'' then
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.AbreEcf:AnsiString;
begin
  result := '0|';
End;

//----------------------------------------------------------------------------
function TImpFiscalProComp.StatusImp( Tipo:Integer ):AnsiString;
var
  iRet     : Integer;
  sRet     : AnsiString;
begin
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.Status( Tipo:Integer; Texto:AnsiString ):AnsiString;
  // Parametros
    ...
    1 : if Texto = '0' then
        Begin
          ...
          begin
            ...
            if (iRetorno = 0) then
            Begin
              if Pos('COMANDO OK',AnsiString(ptRetorno))<> 0 Then
                bErro := False
              else
                sRet:=Copy(AnsiString(ptRetorno),10,length(AnsiString(ptRetorno)));
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.Gaveta:AnsiString;
var
  iRet : Integer;
begin
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.Retorno:AnsiString;
var
  ptRetorno          : Pointer;
  lRetornou          : Boolean;
  dtHoraInicio       : TDateTime;
...
begin
   ...
   if iRetorno = -1 then
      ...
      sMensagem := 'A AnsiString TOTAL não é aceita.'
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.PegaSerie : AnsiString;
begin
    result := '1|Funcao nao disponivel';
end;

//----------------------------------------------------------------------------
function TImpFiscalProComp.LeStatus( var S:AnsiString ): Integer;
var
  ptBuffer  : Pointer;
  iBitTeste : Byte;
  iRet      : Integer;
...
//-----------------------------------------------------------
function TImpFiscalProComp.Pedido( Totalizador, Tef, Texto, Valor, CondPagTef:AnsiString ): AnsiString;
begin
  Result:='0';
end;

//----------------------------------------------------------------------------
function TImpFiscalProComp.RelGerInd( cIndTotalizador , cTextoImp : AnsiString ; nVias : Integer ; ImgQrCode: AnsiString) : AnsiString;
begin
  Result := RelatorioGerencial(cTextoImp , nVias , ImgQrCode);
end;

//----------------------------------------------------------------------------
function TImpFiscalProComp.RelatorioGerencial( Texto:AnsiString ;Vias:Integer; ImgQrCode: AnsiString):AnsiString;
var iRet : Integer;
    sRet : AnsiString;
begin
  iRet := fFuncEncerraNaoFiscal;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.ImprimeCodBarrasITF( Cabecalho , Codigo, Rodape : AnsiString ; Vias : Integer):AnsiString;
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.RecebNFis( Totalizador, Valor, Forma:AnsiString ): AnsiString;
begin
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.FechaEcf:AnsiString;
begin
  result := ReducaoZ('N');
end;

//------------------------------------------------------------------------------
function TImpFiscalProComp.DownloadMFD( sTipo, sInicio, sFinal : AnsiString ):AnsiString;
Begin
...
//------------------------------------------------------------------------------
function TImpFiscalProComp.GeraRegTipoE( sTipo, sInicio, sFinal, sRazao, sEnd, sBinario : AnsiString ):AnsiString;
Begin
  MessageDlg( MsgIndsMFD, mtError,[mbOK],0);
  Result := '1';
End;
...
//------------------------------------------------------------------------------
function TImpFiscalProComp.LeTotNFisc:AnsiString;
Begin
  Result := '0|-99' ;
End;

//------------------------------------------------------------------------------
function TImpFiscalProComp.DownMF(sTipo, sInicio, sFinal : AnsiString):AnsiString;
Begin
  MessageDlg( MsgIndsMFD, mtError,[mbOK],0);
  Result := '1';
End;
...
//------------------------------------------------------------------------------
function TImpFiscalProComp.RedZDado( MapaRes : AnsiString): AnsiString ;
Begin
  Result := '1';
End;

//----------------------------------------------------------------------------
function TImpFiscalProComp.IdCliente( cCPFCNPJ , cCliente , cEndereco : AnsiString ): AnsiString;
begin
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.EstornNFiscVinc( CPFCNPJ , Cliente , Endereco , Mensagem , COOCDC : AnsiString ) : AnsiString;
begin
...
//------------------------------------------------------------------------------
function TImpFiscalProComp.ImpTxtFis(Texto : AnsiString) : AnsiString;
begin
...
//----------------------------------------------------------------------------
function TImpFiscalProComp.GrvQrCode(SavePath, QrCode: AnsiString): AnsiString;
begin
