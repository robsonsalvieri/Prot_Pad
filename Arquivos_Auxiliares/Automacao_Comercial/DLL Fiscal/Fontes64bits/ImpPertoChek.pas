...
///  CMC7 PertoChek
///
Function TCMC7_Perto.Abrir( sPorta, sMensagem: AnsiString ) : AnsiString;
Begin
  Result := OpenPerto(sPorta);
End;

//----------------------------------------------------------------------------
Function TCMC7_Perto.Fechar : AnsiString;
Begin
  Result := ClosePerto;
End;

//---------------------------------------------------------------------------
function TCMC7_Perto.LeDocumento : AnsiString;
var
  sRet : AnsiString;
...
//---------------------------------------------------------------------------
function TCMC7_Perto.LeDocCompleto : AnsiString;
var
  sRet : AnsiString;
...
///  Impressora de Cheque PertoChek
///
Function TImpChequePerto.Abrir( aPorta: AnsiString ) : Boolean;
Begin
  Result := (OpenPerto(aPorta) = '0');
End;

//----------------------------------------------------------------------------
Function TImpChequePerto.Fechar( aPorta:AnsiString ) : Boolean;
Begin
  Result := (ClosePerto = '0');
End;

//----------------------------------------------------------------------------
Function TImpChequePerto.Imprimir( Banco,Valor,Favorec,Cidade,Data,
                                Mensagem,Verso,Extenso,Chancela,Pais:PChar ): Boolean;
var
  sValor  : AnsiString;
  sData   : AnsiString;
  sMsg    : AnsiString;
  ...
  fArquivo    : TIniFile;
Begin
  GravaLog(' Inicio da Função - Imprimir ');

  GravaLog(' *** Variaveis *** ');
  GravaLog(' Banco -> ' + Banco);
  GravaLog(' Valor -> ' + Valor);
  GravaLog(' Favorec -> ' + Favorec);
  GravaLog(' Cidade -> ' + Cidade);
  GravaLog(' Data -> ' + Data);
  GravaLog(' Mensagem -> ' + Mensagem);
  GravaLog(' Verso -> ' + Verso);
  GravaLog(' Extenso -> ' + Extenso);
  GravaLog(' Chancela -> ' + Chancela);
  GravaLog(' Pais -> ' + Pais);

  GravaLog(' *** Fim das Variaveis *** ');

  if Length(Data) = 6
  then sData  := Copy(Data,5,2)+Copy(Data,3,2)+Copy(Data,1,2)
  else sData  := Copy(Data,7,2)+Copy(Data,5,2)+Copy(Data,3,2);
  GravaLog(' Data [tratado] -> ' + Data);

  sValor := FormataTexto(Valor, 12, 2, 2);
  GravaLog(' Valor [tratado] -> ' + Valor);

  sMsg := StrPas(Mensagem)+space(20);
  GravaLog(' sMsg [tratado] -> ' + sMsg);
...
// tem 128k de memória.
   sVer := 'V';
   GravaLog(' Antes de EnviaComando -> sVer [' + PChar(sVer) + ']/ sRet [' + sRet + ']');
   EnviaComando(PChar(sVer),sRet);
   GravaLog(' Depois de EnviaComando <- sVer [' + PChar(sVer) + ']/ sRet [' + sRet + ']');
   sVer := Copy(sRet,1,Pos(' ',sRet));
   sRet := '';
   If Pos('08',sVer) > 0 then
   begin
     sValor := '$0'+sValor+Banco;
     GravaLog(' sValor -> ' + sValor);
   end
   Else
   Begin
     ...
     Begin
       ...
       If length(Data)=6
       then sValor := '$0'+sValor+Banco
       Else sValor := '$4'+sValor+Banco;

       GravaLog(' sValor [INI] -> ' + sValor);
     End
     ...
     Begin
       ...
       //Se recebe com 4 dígitos, imprime com 4.
       If length(Data)=6
       then sValor := '$1'+sValor+Banco
       Else sValor := '$5'+sValor+Banco;
  ...
  begin
    ...
    sVerso  := Copy(Verso,1,74) + Space (74 - Length (Verso)) + #255
               ...
               + #255 + #255 + #255 + #255 + #255;
    GravaLog(' sVerso [outros 13 fecha] -> ' + sVerso);
  end;

  sVerso := 'X'+sVerso;
  GravaLog(' sVerso [X] -> ' + sVerso);

  Result:= false;

  // Altera Data
  GravaLog(' Antes de EnviaComando -> sVer [' + PChar(sData) + ']/ sRet [' + sRet + ']');
  EnviaComando(PChar(sData),sRet);
  GravaLog(' Depois de EnviaComando -> sVer [' + PChar(sData) + ']/ sRet [' + sRet + ']');

  // Altera a Cidade
  GravaLog(' Antes de EnviaComando -> sVer [' + PChar(sCidade) + ']/ sRet [' + sRet + ']');
  EnviaComando(PChar(sCidade),sRet);
  GravaLog(' Depois de EnviaComando -> sVer [' + PChar(sCidade) + ']/ sRet [' + sRet + ']');

  // Altera Favorecido
  GravaLog(' Antes de EnviaComando -> sVer [' + PChar(sFav) + ']/ sRet [' + sRet + ']');
  EnviaComando(PChar(sFav),sRet);
  GravaLog(' Depois de EnviaComando -> sVer [' + PChar(sFav) + ']/ sRet [' + sRet + ']');

  // Altera Campo Extra
  GravaLog(' Antes de EnviaComando -> sVer [' + PChar(sMsg) + ']/ sRet [' + sRet + ']');
  EnviaComando(PChar(sMsg),sRet);
  GravaLog(' Depois de EnviaComando -> sVer [' + PChar(sMsg) + ']/ sRet [' + sRet + ']');

  // Imprime o Cheque
  GravaLog(' Antes de EnviaComando -> sVer [' + PChar(sValor) + ']/ sRet [' + sRet + ']');
  if EnviaComando(PChar(sValor),sRet) then
  Begin
     GravaLog(' Depois de EnviaComando -> sVer [' + PChar(sValor) + ']/ sRet [' + sRet + ']');
     Result:= True;
     If Length(Verso) > 0 then
     Begin
        ShowMessage('Retire o cheque da impressora e tecle <ENTER>');
        sRet := 'Insira o verso do cheque ';
        GravaLog(' Antes de EnviaComando -> sVer [' + PChar(sVerso) + ']/ sRet [' + sRet + ']');

        if EnviaComando(PChar(sVerso), sRet) then
        begin
          Result:= True;
          GravaLog(' Depois de EnviaComando -> TRUE');
        end
        else
         GravaLog(' Depois de EnviaComando -> FALSE');
     End;
  End;

  GravaLog(' Fim da Função - Imprimir ');
End;
...
//----------------------------------------------------------------------------
function TImpChequePerto.StatusCh( Tipo:Integer ):AnsiString;
begin
...
begin
  ...
  Begin
     ...
     // Altera Data
     GravaLog(' Antes de EnvComm -> CMD - [' + sCmd + ']');
     iRet:=EnvComm(sCmd);
     GravaLog(' Depois de EnvComm <- iRet - [' + IntToStr(iRet) + ']');
     ...
     Begin
        ...
        begin
          ShowMessage(sMsg);
          GravaLog(' Mensagem - [' + sMsg + ']');
        end;

        GravaLog(' Antes de RecComm -> ');
        iRet := RecComm(30, Resp);
        GravaLog(' Depois de RecComm <- iRet - [' + IntToStr(iRet) + '] / Resp - [' + Resp + ']');
        ...
        Begin
           ...
           if ( sCmd='V' ) and (Copy(StrPas(Resp),1,2)='VF')
           then Retorno:='000';
           ...
           Begin
              For i := 1 To 37 Do
                 ...
                 Begin
                    ...
                    Begin
                       GravaLog(' Antes de EnvComm -> ');
                       EnvComm('>');
                       GravaLog(' Depois de EnvComm <- ');

                       ShowMessage('Retire o documento');
                       GravaLog(' Solicitado - "Retire o documento" ');
  ...
  If Result
  then GravaLog('Retorno da Função EnviaComando - Result [True]')
  else GravaLog('Retorno da Função EnviaComando - Result [False]');
End;
