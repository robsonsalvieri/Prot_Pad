...
type
  ...
  public
    ...
    function ImpTxtFmt( cTexto: String ): String;
    function AltCodeBar(cMsg : String; var bTemTag : Boolean; cTipo: String = 'B'):String;
    function ImpTxtComum( Texto : String ): String;
    function CortaPapel : String ;
    function AbreGaveta(): String; override;
    function SetaEscBema : Integer;
  end;

  Function OpenBemaNF( sPorta:String; iVelocidade : Integer ):String;
  Function CloseBemaNF : String;

implementation

var
        fHandle  : THandle; //'MP2032.DLL'
        fHandle2 : THandle; //'SIUSBXP.DLL'
        aArTags : array [1..27] of String;

        //Funções da DLL
        fFuncIniciaPorta 		: function(Porta : PChar): Integer; StdCall;
        fFuncBematechTX                 : function(Texto : String) :Integer; StdCall;
        fFuncComandoTX		        : function(Texto : String; Flag : Integer) : Integer; StdCall;
        fFuncCaracterGraf               : function(Texto : String; Tamanho: Integer) : Integer; StdCall;
        fFuncLe_Status		        : function() : Integer; StdCall;
        fFuncAutenticaDoc               : function( Texto : String;  Tempo : Integer) : Integer; StdCall;
        ...
        fFuncFormataTX                  : function ( BufTrans : PCHAR;  TpoLtra : Integer;  Italic : Integer;  Sublin : Integer;  Expand : Integer;  Enfat : Integer) : Integer ; StdCall;
        ...
        fFuncImprimeCodigoBarrasUPCA    : function ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasUPCE    : function  ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasEAN13   : function  ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasEAN8    : function  ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasCODE39  : function  ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasCODE93  : function ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasCODE128 : function ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasITF     : function ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasCODABAR : function ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasISBN    : function  ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasMSI     : function  ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasPLESSEY : function ( Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoBarrasPDF417  : function ( NivelCorrecaoErros ,  Altura ,  Largura ,  Colunas : Integer ;  Codigo : String) : Integer ; StdCall;
        fFuncImprimeCodigoQRCODE        : function (errorCorrectionLevel , moduleSize , codeType , QRCodeVersion , encodingModes : Integer; codeQr : String) : Integer ; StdCall;

        // Funções para impressão de BitMap
        fFuncImprimeBitmap              : function  ( Name : String;  mode : Integer) : Integer ; StdCall;
        fFuncImprimeBmpEspecial         : function  ( Name : String;  xScale ,  yScale ,  angle : Integer)  : Integer ; StdCall;
...
cCMDESCBEMA     : String =  Chr(29)+Chr(249)+Chr(32);
cBNegIni	: String =  Chr(27)+Chr(69);
cBNegFim	: String =  Chr(27)+Chr(70);
cBItaIni	: String =  Chr(27)+Chr(52);
cBItaFim	: String =  Chr(27)+Chr(53);
cBCondenIni	: String =  Chr(27)+Chr(15);
cBCondenFim	: String =  Chr(27)+Chr(72);
cBExpanIni	: String =  Chr(27)+Chr(87)+Chr(1);
cBExpanFim	: String =  Chr(27)+Chr(87)+Chr(0);
cBDuplaAIni	: String =  Chr(27)+Chr(100)+Chr(1);
cBDuplaAFim	: String =  Chr(27)+Chr(100)+Chr(0);
cBAlinRigth	: String =  Chr(27)+Chr(97)+Chr(2);
cBSubliIni	: String =  Chr(27)+Chr(45)+Chr(1);
cBSubliFim	: String =  Chr(27)+Chr(45)+Chr(0);
cBEnfatiIni	: String =  Chr(27)+Chr(69);
cBEnfatiFim	: String =  Chr(27)+Chr(70);
cBSubEscIni	: String =  Chr(27)+Chr(83)+Chr(1);
cBSobEscIni	: String =  Chr(27)+Chr(83)+Chr(0);
cBSubSobFim	: String =  Chr(27)+Chr(84);
cBNormal	: String =  CHR(27)+CHR(64);
cBCentralizado  : String =  CHR(27) + CHR(97) + CHR(1) ;
cBCorteTotal    : String =  CHR(27) + CHR(119);
cBCorteParcial  : String =  CHR(27) + CHR(109);

cSetaEscBema    : String =  Chr(29) + Chr(249) + Chr(53) + Chr(48);

{ ***********************   Tags do AUTODEF.CH ********************************}
TAG_NEGRITO_INI : String = '<b>';	//Inicia Texto em Negrito
TAG_ITALICO_INI	: String = '<i>';	//itálico
TAG_CENTER_INI	: String = '<ce>';	//centralizado
TAG_SUBLI_INI	: String = '<s>';	//sublinhado
TAG_EXPAN_INI 	: String = '<e>';	//expandido
TAG_CONDEN_INI	: String = '<c>';	//condensado
TAG_NORMAL_INI	: String = '<n>';	//normal
TAG_PULALI_INI	: String = '<l>';	//pula 1 linha
TAG_PULANL_INI	: String = '<sl>';	//pula NN linhas
TAG_RISCALN_INI	: String = '<tc>';	//risca a linha caracter especifico
TAG_TABS_INI	: String = '<tb>';	//tabulação
TAG_DIREITA_INI	: String = '<ad>'; //alinhado a direita
TAG_ELITE_INI	: String = '<fe>';	//habilita fonte elite
TAG_TXTEXGG_INI	: String = '<xl>';	//habilita texto extra grande
TAG_GUIL_INI	: String = '<gui>';//ativa guilhotina
TAG_EAN13_INI 	: String = '<ean13>';	//codigo de barra ean13
TAG_EAN8_INI	: String = '<ean8>';	//codigo de barra ean8
TAG_UPCA_INI	: String = '<upc-a>'; //codigo de barras upc-a
TAG_CODE39_INI	: String = '<code39>';//codigo de barras CODE39
TAG_CODE93_INI	: String = '<code93>'; //codigo de barras CODE93
TAG_CODABAR_INI	: String = '<codabar>';//codigo de barras CODABAR
TAG_MSI_INI	: String = '<msi>'; //codigo de barras MSI
TAG_CODE11_INI	: String = '<code11>';//codigo de barras CODE11
TAG_PDF_INI	: String = '<pdf>'; //codigo de barras PDF
TAG_COD128_INI	: String = '<code128>'; //codigo de barras CODE128
TAG_I2OF5_INI	: String = '<i2of5>'; //codigo I2OF5
TAG_S2OF5_INI 	: String = '<s2of5>'; //codigo S2OF5
TAG_QRCODE_INI	: String = '<qrcode>';	//codigo do tipo QRCODE
TAG_BMP_INI	: String = '<bmp>'; //imprimi logotipo carregado

//Tags disponibilizadas apenas para a bematech
TAG_ITF	   : String = '<itf>';
TAG_ISBN   : String = '<isbn>';
TAG_PLESSEY: String = '<plessey>';

//----------------------------------------------------------------------------
Function OpenBemaNF( sPorta:String; iVelocidade : Integer) : String;
  function ValidPointer( aPointer: Pointer; sMSg:String; sArqDll:String = 'MP2032.DLL' ) : Boolean;
  begin
...
begin
  ...
  If Not bOpened Then
  Begin
    GravaLog(' Bematech Não Fiscal -> Carrega MP2032.DLL ');
    fHandle  := LoadLibrary( 'MP2032.DLL' );
    GravaLog(' Bematech Não Fiscal <- Carrega MP2032.DLL :' + IntToStr(fHandle));
    ...
    begin
      LjMsgDlg('A DLL: MP2032.DLL e/ou SIUSBXP.DLL não foi encontrado.');
      bRet := False;
    end;
    ...
    begin
      ...
      try
        ...
        //Log do Arquivo de Configuração
        sIni := ExtractFilePath(Application.ExeName) + 'MP2032.INI';
        If FileExists(sIni) then
        begin
          ListaArq.Clear;
          ListaArq.LoadFromFile(sIni);

          GravaLog(' ******** Arquivo MP2032.INI *******');
          GravaLog( ListaArq.Text );
          GravaLog(' ******** Final da Leitura do Arquivo MP2032.INI *******');
        End;
      Except
...
//----------------------------------------------------------------------------
function TImpNfBema4200.Abrir(sPorta : String; iVelocidade : Integer ; iHdlMain:Integer) : String;
Var
  sRet : String;
begin
...
//----------------------------------------------------------------------------
function TImpNfBema4200.Fechar( sPorta:String ):String;
begin
Result := CloseBemaNF();
end;

//----------------------------------------------------------------------------
function TImpNfBema4200.ImpTexto( Texto : String):String;
var
  iRet  : Integer;
  sAux,sRet: String;
  bCorte : Boolean;
...
//------------------------------------------------------------------------------
function TImpNfBema4200.AltCodeBar(cMsg : String; var bTemTag : Boolean; cTipo: String = 'B'):String;
var
   nI : Integer;
   cTexto,cIndic,cTag,cTroca : String;
   aTags : array [1..18] of String;
begin
...
//-----------------------------------------------------------------------------
Function  TImpNfBema4200.ImpTxtFmt( cTexto: String ): String;
var
  cI,cAux,cLinha, cTag, cRet: String;
  aTags	 : array [1..18] of String;
...
//------------------------------------------------------------------------------
function TImpNfBema4200.ImpTxtComum( Texto : String ) : String;
var
  iRet, nPos, nPos2 : Integer;
  sTextoImp,sAux : String;
  oTexto : TStringList;
begin
...
//----------------------------------------------------------------------------
function TImpNfBema4200.ImpCodeBar( Tipo,Texto:String  ):String;
var
  iRet,iPasso,iInicio : Integer;
  sAux , sCmd, sTamQrCd: String;
begin
...
//----------------------------------------------------------------------------
function TImpNfBema4200.ImpBitMap( Arquivo:String ):String;
var
  iRet : Integer;
begin
...
//----------------------------------------------------------------------------
function TImpNfBema4200.TrataTags( var Texto : String ): String;
var
   sRet,sAux, cMsg, cTag : String;
   nX : Integer;
   bTemTag : Boolean;
begin
sRet := '';
...
//------------------------------------------------------------------------------
function TImpNfBema4200.CortaPapel : String;
var
   iRet : Integer;
begin
...
//----------------------------------------------------------------------------
function TImpNfBema4200.AbreGaveta : String;
var
  iRet : Integer;
  sCmd : String;
begin
