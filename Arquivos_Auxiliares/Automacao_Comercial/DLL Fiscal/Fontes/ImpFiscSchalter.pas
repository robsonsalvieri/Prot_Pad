...
///
  ...
  public
    ...
    function ImpostosCupom(Texto: String): String; override;  //só compatibilização
...
begin
    ...
    // Tabela com os códigos do status da impressora. Estes correspondem a posição 3 da string de retorno
    //--------------------------------------------------------------------------
...
var
  ...
  sRet        : String;             // Utilizada para armazenar dados do retorno das funções
  sGrandTot   : String;             // Grande Total
  sNroECF     : String;             // Numero do ECF
  sSeriePDV   : String;             // Armazena a série do PDV
  sNumCupom   : String;             // Pega o próximo nro de cupom
  sCupomIni   : String;             // Pega o número do primeiro cupom impresso
  sCupomFin   : String;             // Pega o número do último cupom impresso
  sNroReduz   : String;             // Numero de Reduções Z realizadas pelo ECF
  sTotCanc    : String;             // Total de Cancelamentos do dia
  sTotDesc    : String;             // Total de descontos do dia
  sTotSubst   : String;             // Total de Substituição Tributária
  sTotIsento  : String;             // Total de isentos
  sTotNaoTrib : String;             // Total de Nao tributado
  sDataReduz  : String;             // Data da Redução Z;
  sTotISS     : String;             //Total de ISS
  sAliq       : String;             // Aliquota cadastrada na impressora
  sNroInterv  : String;             // Número de intervenções técnicas
...
begin
  ...
  begin
    ...
    if pAux = nil then
    begin
        //Sexta posição da string: 0) Normal; 1) Pouco Papel
        sRet := copy(vret,6,1);
        If sRet = '1' then
  ...
  begin
    if StrToInt( AchaPagto( aAuxiliar[i],aFormas ) ) >= 0 then
      Begin
        ...
        // 3) Valor do pagamento: String de 10 Caracteres (não deve-se usar vírgula ou ponto)
...
var
  ...
  sRet      : String;
  sNumAnt   : String;           // Último cupom impresso;
  i         : Integer;
  aFormas   : array of String;
  sFormas   : String;
  sPos      : String;           //Posição do totalizador não fiscal cadastrado na impressora;
  sPosTot   : String;
  iPos      : Integer;
  iTamanho  : Integer;
begin
