...
#define E000_SUCESS              "E000_SUCESS"
#define E000_SUCESS_TEXT         "Sucesso."
...
// ##################################################################################
#command birtDataset <identifier> ;
=> static _DATASET := nil;;
	function r_<identifier>();;
		if (_DATASET == nil);;
			_DATASET := _r<identifier>():_r<identifier>(<"identifier">);;
		endif;;
	return _DATASET;;
...
// ##################################################################################
#command define report <var> name <identifier> ;
...
=> 	[<var>:format(<"format">)];;
	<var>:show();<var>:_BIRTReport();freeObj(<var>);_DATASET := nil
			
#command save report <var> ;
	[format <format: HTML, PDF>];
	output <output>;
=> 	[<var>:format(<"format">)];;
	<var>:save(<"output">);<var>:_BIRTReport();freeObj(<var>);_DATASET := nil

#command print report <var> ;
	[format <format: HTML, PDF>];
=> 	[<var>:format(<"format">)];;
	<var>:print();<var>:_BIRTReport();freeObj(<var>);_DATASET := nil
