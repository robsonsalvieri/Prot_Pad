...
Static oStAuxPosic := tHashMap():New()
Static oStCAuxRet  := tHashMap():New()
...
Function LjAuxPosic(cTipoCad, cCampo, xValor, cCmpRet)
    ...
    cChave := cTipoCad + cCampo + cValTochar(xValor) + cCmpRet
    if oStAuxPosic:get(cChave, @cRet)
        Return cRet
    endIf
...
Function LjCAuxRet(cComponent)
    ...
    Local nPos      := 0
    Local xConteudo := ""
    Local cChave    := MIH->MIH_ID + cComponent
    
    if oStCAuxRet:get(cChave, @xConteudo)
        Return xConteudo
    endIf
    ...
    oStCAuxRet:Set(cChave, xConteudo)
...
Function LjCadxF3Mu()
    ...
    if subStr(cCmpChave, 1, 1) == "X"
        ...
        if at("+", cCmpChave) > 0
            ...
            aAux    := strTokArr( alltrim(cCmpChave), "+")
            cTabela := GetSx3Cache(aAux[1], "X3_ARQUIVO")

            for nCont := 1 to len(aAux)
                nTamChave += TamSx3(aAux[nCont])[1]
            next nCont
        else
        ...
        if at("+", cCmpChave) == 0
            cSql += " AND " + PrefixoCPO(cTabela) + "_FILIAL = '" + xFilial(cTabela) + "'"
        endIf
    endIf
