...
/*/
//--------------------------------------------------------
Function RetailSales(aAutoCab, aAutoItens, aAutoPagtos, nOpcAuto, aAutoTrib)
    ...
    If lRotAuto
        ...
        aAdd(aStruct, {"SL1MASTER", aAutoCab})
        aAutoItens  := FwVetByDic(aAutoItens , "SL2", .T., 1)
        aadd(aStruct, {"SL2DETAIL", aAutoItens})
        aAutoPagtos := FwVetByDic(aAutoPagtos, "SL4", .T., 1)
        aadd(aStruct, {"SL4DETAIL", aAutoPagtos})
        
        If lStDKM
            aAutoTrib   := FwVetByDic(aAutoTrib  , "DKM", .T., 1)        
            aadd(aStruct, {"DKMDETAIL", aAutoTrib})
        EndIf
        If nOpcAuto == 4
        ...
        lRet := FWMVCRotAuto(ModelDef(), "SL1", nOpcAuto, aStruct )
...
Static Function ModelDef()
    ...
    If oModelDef == Nil
        ...
        oModelDef:SetRelation( 'SL2DETAIL', { { 'L2_FILIAL', 'xFilial( "SL2" )' }, { 'L2_NUM', 'L1_NUM' }, { 'L2_CLIENT', 'L1_CLIENTE' }, { 'L2_CLILOJA', 'L1_LOJA' } }, SL2->( IndexKey( 1 ) ) )

        if lStDKM
            oModelDef:AddGrid("DKMDETAIL", "SL2DETAIL", oMStruDKM, /*bLinePre*/, /*bLinePost*/, /*bPre*/, /*bPost*/, /*bLoad*/)
            oModelDef:SetRelation('DKMDETAIL', { {'DKM_FILIAL', 'xFilial("DKM")'},{'DKM_DOC', 'L2_DOC'}, {'DKM_SERIE', 'L2_SERIE'}}, DKM->( IndexKey(1) ) )     //DKM_FILIAL+DKM_DOC+DKM_SERIE+DKM_FORNEC+DKM_LOJA+DKM_ESPECI+DKM_ITEM+DKM_COD+DKM_TRIB
            oModelDef:GetModel('DKMDETAIL'):SetDescription("Tributos do item da venda")
            oModelDef:GetModel('DKMDETAIL'):SetOptional(.T.)
        endIf
...
Static Function ViewDef()
    ...
    Local oView     := Nil
    Local nBoxSL2   := iif(lStDKM, 20, 40)
    ...
    oView:CreateHorizontalBox( 'BOX_SL1', 40)
    oView:CreateHorizontalBox( 'BOX_SL2', nBoxSL2)
    if lStDKM
        oView:AddGrid('VIEW_DKM', oVStruDKM, 'DKMDETAIL')
        oView:CreateHorizontalBox('BOX_DKM', 20)
        oView:SetOwnerView('VIEW_DKM', 'BOX_DKM')
    endIf
...
Static Function LoadStrModel()
    ...
    if lStDKM
        oMStruDKM 	:= FWFormStruct(1, 'DKM')
        LjxAddFil('DKM', oMStruDKM, 1)

        oMStruDKM:SetProperty("DKM_TIPO"  , MODEL_FIELD_INIT, FWBuildFeature(STRUCT_FEATURE_INIPAD, '"N"'      ) )    //N=Normal
        oMStruDKM:SetProperty("DKM_ORIGEM", MODEL_FIELD_INIT, FWBuildFeature(STRUCT_FEATURE_INIPAD, '"LOJA701"') )
    endIf
...
Static Function LoadStrView()
    ...
    if lStDKM
        oVStruDKM := FWFormStruct(2, 'DKM')
        LjxAddFil('DKM', oVStruDKM, 2)
    endIf
...
/*/
//--------------------------------------------------------
Function RsGrvVenda(aCab, aItens, aPagtos, nOpc, aTributos)
    ...
    Else
        ...
        EndIf
        lRet := MsExecAuto( {|a,b,c,d,e| RetailSales(a,b,c,d,e)}, aCab, aItens, aPagtos, nOpc, aTributos)
    EndIf
