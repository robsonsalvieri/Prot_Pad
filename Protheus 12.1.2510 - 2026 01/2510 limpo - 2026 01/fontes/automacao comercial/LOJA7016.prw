...
Local aSD3     := {} 								//Itens - Array que contem os campos da tabela SD3 que serao usados para na executar a MSEXECAUTO do MATA240.
...
Local nOpe     := 0                                 //Tipo de operacao 3= inclusao e 5=estorno no caso de Alteracao 
...
Local dDataDig := oModelMFJ:ADATAMODEL[1][Ascan( oModelMFJ:ADATAMODEL[1],{|x| x[1] == 'MFJ_DATA'})][2]
Local cNumSer  := oModelMFJ:ADATAMODEL[1][Ascan( oModelMFJ:ADATAMODEL[1],{|x| x[1] == 'MFJ_NUMSER'})][2] 
Local nPosLTCTL:= Ascan( oModelMFJ:ADATAMODEL[1],{|x| x[1] == 'MFJ_LOTCTL'})	// Posi็ใo do campo MFJ_LOTCTL
Local nPosLOTE := Ascan( oModelMFJ:ADATAMODEL[1],{|x| x[1] == 'MFJ_NUMLOT'})	// Posi็ใo do campo MFJ_NUMLOT
Local nPosNSER := Ascan( oModelMFJ:ADATAMODEL[1],{|x| x[1] == 'MFJ_NUMSER'})	// Posi็ใo do campo MFJ_NUMSER
Local lLocaliza := Localiza(PadR(cProduto,Len(SD3->D3_COD))) 					// Se o Produto possuir localizacao
Local cLocaEstq := ""															// Local do Produto Saldo por Localiza็ใo
Local aCabSD3 	:= {}	//Cabe็alho - Array que contem os campos da tabela SD3 que serao usados para na executar a MSEXECAUTO do MATA240.
Local aAreaSBF	:= {}	// Salva แrea tabela SBF
...
ElseIf nQuant == 0
	// Para inclusใo do Movimento Interno(SD3) via ExecAuto necessแrio passar Quantidade maior que Zero
	nQuant := 1
EndIf
...
If !Empty(cTPMov) .And. lRet 
	If Val(cTPMov) >= 500 //Requisicao
		...
		// Array Quebra Operacional
		AAdd(aCabSD3,{'D3_FILIAL'  	,xFilial("SD3"), Nil})
		AAdd(aCabSD3,{'D3_COD'		,cProduto, Nil})		
        AAdd(aCabSD3,{'D3_LOCAL'   	,cLocal, Nil})
		AAdd(aCabSD3,{'D3_TM'      	,Alltrim(cTPMov), Nil})
		AAdd(aCabSD3,{'D3_EMISSAO'	,iif(!Empty(dDataDig),dDataDig,dDataBase), Nil})
		If nOpe ==  6			
			AAdd(aCabSD3,{'D3_NUMSEQ'	,cSD3NUM, Nil})		
		Endif	
		If lLocaliza // Se o Produto possuir localizacao
			AAdd(aCabSD3,{'D3_LOTECTL'	,PadR(M->MFJ_LOTCTL,Len(SD3->D3_LOTECTL))	,Nil})
			AAdd(aCabSD3,{'D3_NUMLOTE'	,PadR(M->MFJ_NUMLOT,Len(SD3->D3_NUMLOTE))	,Nil})
			AAdd(aCabSD3,{'D3_NUMSERI'	,PadR(M->MFJ_NUMSER,Len(SD3->D3_NUMSERI))	,Nil})

			// Busca pela Localiza็ใo do Produto na tabela Saldos por Localiza็ใo
			aAreaSBF := SBF->(GetArea())
			DbSelectArea("SBF")
			DbSetOrder(4)  //BF_FILIAL+BF_PRODUTO+BF_NUMSERI
			If DbSeek(xFilial("SBF")+cProduto+cNumSer)
				cLocaEstq := SBF->BF_LOCALIZ
			Endif	
			RestArea(aAreaSBF)
		Else
			// Se o Produto nใo tem Controle de Lote/Endere็o limpa os objetos
			oModelMFJ:ADATAMODEL[1][nPosLTCTL][2] := ""
			oModelMFJ:ADATAMODEL[1][nPosLOTE][2] := ""
			oModelMFJ:ADATAMODEL[1][nPosNSER][2] := ""
		EndIf

		// Array Itens de Movimenta็ใo Interna
		aadd(aSD3,{	{'D3_FILIAL'  	,xFilial("SD3")	,Nil},;		
					{'D3_COD'		,cProduto		,Nil},;
					{'D3_LOCAL'   	,cLocal			,Nil},;
					{'D3_TM'      	,Alltrim(cTPMov),Nil},;
					{'D3_QUANT'		,nQuant			,Nil},;
					{'D3_EMISSAO'	,dDataBase	   	,Nil},;
					{'D3_NUMSEQ'	,cSD3NUM		,Nil},;					
					{'D3_LOTECTL'	,PadR(oModelMFJ:ADATAMODEL[1][nPosLTCTL][2],Len(SD3->D3_LOTECTL)), Nil} ,;
					{'D3_NUMLOTE'	,PadR(oModelMFJ:ADATAMODEL[1][nPosLOTE][2],Len(SD3->D3_NUMLOTE)), Nil},;
					{'D3_NUMSERI'	,PadR(oModelMFJ:ADATAMODEL[1][nPosNSER][2],Len(SD3->D3_NUMSERI)), Nil},;
					{'D3_LOCALIZ'	,cLocaEstq, Nil},;										
					{"INDEX"		,3				,Nil}})						


		lMSHelpAuto := .T.

		MsgRun(STR0004,STR0005,{||MSExecAuto({|x,y,z|MATA241(x,y,z)},aCABSD3, aSD3, nOpe)})//"Registrando movimenta็ใo internas."##"Aguarde..." 
