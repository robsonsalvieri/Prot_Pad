...
#DEFINE PICTURE_CNPJ "@R! NN.NNN.NNN/NNNN-99"
...
If cProg $ "A440/A920"
	...
	WHERE
		...
		SF2.%NotDel% AND
		SF2.F2_TIPO NOT IN('D','B','P','I','5','6')
	ORDER BY
...
If ("_NFORI"$cReadVar) .Or. ( lUsaNewKey .And. ("_SERIORI"$cReadVar .Or. "_ITEMORI"$cReadVar )  )
	Do Case
		Case cPrograma $ "A440|A466|LOJA920|ARUS466"
			...
			While !Eof() .And. (cAliasSF1)->F1_FILIAL = xFilial("SF1") .And.;
				...
				While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
					...
					If (cAliasSF4)->F4_PODER3 == "N" .And. !Empty((cAliasSD1)->D1_TES) .And. (cAliasSD1)->D1_ORIGLAN<>"LF" .And. !((cAliasSD1)->D1_TIPO $ "D|5|6")
		...
		OtherWise
			...
			aPesq     := {{Space(Len(SD2->D2_DOC+SD2->D2_SERIE)),"@!"},{Ctod(""),"@D"}}
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	...
	If (cAliasTRB)->(LastRec())<>0
		...
		@ aPosObj[3,1]+03,aPosObj[3,2]+00 SAY OemToAnsi(STR0027) PIXEL //Pesquisar por:
		@ aPosObj[3,1]+00,aPosObj[3,2]+40 MSCOMBOBOX oCombo VAR cCombo ITEMS aOrdem SIZE 100,044 OF oDlg PIXEL ;
		VALID ((cAliasTRB)->(dbSetOrder(oCombo:nAt)),(cAliasTRB)->(dbGotop()),xPesq := aPesq[(cAliasTRB)->(IndexOrd())][1],;
				oGet:Picture := aPesq[(cAliasTRB)->(IndexOrd())][2], oGet:Refresh(),.T.)  

		@ aPosObj[3,1]+03,aPosObj[3,2]+160 SAY OemToAnsi(STR0026) PIXEL //Localizar
	  	@ aPosObj[3,1]+00,aPosObj[3,2]+200 MSGET oGet VAR xPesq Of oDlg PICTURE aPesq[(cAliasTRB)->(IndexOrd())][2] SIZE 150,012 PIXEL ;
			...
			Do Case
		 		Case cPrograma $ "A440|A466|LOJA920|ARUS466"
					...
					If cPaisLoc <> "BRA"
						...
						EndIf
						If nPNIT <> 0 .And. cPaisLoc $ "COL|PER"
...
If lMostraTela
	...
	Else
		...
		If lConsolida
			...
			bLine := {|| {If(aFilsCalc[oQual:nAt,1],oOk,oNo),aFilsCalc[oQual:nAt,2],aFilsCalc[oQual:nAt,3],Transform(aFilsCalc[oQual:nAt,4], PICTURE_CNPJ),aFilsCalc[oQual:nAt,5],aFilsCalc[oQual:nAt,6]}}
		Else
			...
			bLine := {|| {If(aFilsCalc[oQual:nAt,1],oOk,oNo),aFilsCalc[oQual:nAt,2],aFilsCalc[oQual:nAt,3],Transform(aFilsCalc[oQual:nAt,4], PICTURE_CNPJ)}}
		EndIf
 	    	...
 	    	If ( nValida==1 .and. aArray[nIt,4]==cSelCNPJIE ) .or. ( nValida==2 .and. aArray[nIt,4]+aArray[nIt,5]==cSelCNPJIE ) .or.;
	 	  	...
	 	  	Else
	 	  		If nValida == 1
		 	  		...
		 	  		Help(nil,1,STR0060,nil,STR0061+Transform(cSelCNPJIE, PICTURE_CNPJ)+STR0062,3,0)
		 	  	ElseIf nValida == 2
		 	  	   //'SIGACUSCNPJIE' ; 'A Consolidação por CNPJ+IE está habilitado. Selecione apenas Filiais com o mesmo CNPJ+IE [' ; '] já marcado, ou refaça sua seleção!'
		 	  		Help(nil,1,STR0063,nil,STR0064+Transform(Substr(cSelCNPJIE,1,14), PICTURE_CNPJ)+" - "+Substr(cSelCNPJIE,15)+STR0062,3,0)
		 	  	ElseIf nValida == 3
		 	  	...
		 	  	Else
					...
					Help(nil,1,STR0068,nil,STR0069+Transform(Substr(cSelCNPJIE,1,14), PICTURE_CNPJ)+" - "+Substr(cSelCNPJIE,15)+STR0062,3,0)
		 	  	Endif
	 	   Endif
		Else
			nSelCNPJIE++
...
While SD4->(!EOF()) .And. SD4->(D4_FILIAL+D4_OP) ==  xFilial("SD4")+cNumOp
	...
	//(cOp, cProduto , cLocal , nRecSD4, cTRT, cLote, cNumlot, cOrdem, cOpOrig, cSeq)
	If lCpoSGO
		If !SD4->(A103ChkBN(cNumOp,D4_COD,D4_LOCAL,SD4->(Recno()),D4_TRT,D4_LOTECTL,D4_NUMLOTE,D4_ORDEM,D4_OPORIG,D4_SEQ))
...
//³ MV_PERCPRM - Parametro utilizado para verificar o percentual de "Produção a  maior"|
//|              permitido no sistema											       |
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Local nProdMaior := SuperGetMV("MV_PERCPRM",.F., 0)

SC2->(dbSetOrder(1))
SC2->(dbSeek(xFilial("SC2")+cNumOp))

If !lGanhoProd .And. nQtdProd > SC2->(C2_QUANT - C2_QUJE)
	Aviso(STR0034,STR0053,{"OK"}) //Atenção##Quantidade superior ao saldo da ordem de produÇÃo.
	lRet := .F.
Else
	If lGanhoProd .And. (nQtdProd + SC2->C2_QUJE) > (SC2->C2_QUANT + (SC2->C2_QUANT * nProdMaior / 100))   
		Aviso(STR0034,STR0091,{"OK"}) //Atenção##'Quantidade superior ao percentual de "produção a maior"'
		lRet := .F.
	Else
...
/*/{Protheus.doc} A410aChSGO
Valida a existencia de campos na SGO
@return lRet
/*/

Static Function ACusChSGO ()
