...
/*/
Function MATA116(xAutoCab,xAutoItens,lAutoGFE,lPreNota,aRatcc, xNatRend,xjTribAuto)
...
If lRet
 	While lInclui 
		lRet := Mata116A(xAutoCab,xAutoItens,@lInclui,lAutoGFE,lPreNota,aRatcc, xNatRend,xjTribAuto)
	EndDo
Endif
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function Mata116A(xAutoCab,xAutoItens,lInclui,lAutoGFE,lPreNota,aRatcc, xNatRend,xjTribAuto)
...
Private nInfAdic	:= 0
Private lFormulS	:= .F.
Private jTribAuto   := JsonObject():New()
Default xjTribAuto  := JsonObject():New()

Default lAutoGFE := .F.
Default lPreNota := .F.

aRotina := MenuDef() 
jTribAuto := xjTribAuto
		...
		Else
			...
			If Len(aCols) > 0
				...
				If nOpc == 0 .Or. !A116TudOk(If(lPreNota,'P','N'),l116Exclui,lColab)
					...
					lContinua := .F.
				Else
					NfeFldFin(,l103Visual,aRecSE2,0,,@aHeadSE2,@aColsSE2,@aHeadSEV,@aColsSEV,@aFldCbAtu[6],,,,,,,@aColTrbGen,@nColsSE2,@aParcTrGen,@cIdsTrGen)
					...
					Else
						...
						If !lPrenota
							If l116Auto 
								...
								EndIf
							EndIf
						If lTabDKM .and. lTrbGen .and. MaFisFound("NF") .and. valtype(jTribAuto) =='J'.AND. FindClass('totvs.protheus.backoffice.fiscal.TaxOperandIntegrator.TaxOperandIntegrator')
							oIntegrador := totvs.protheus.backoffice.fiscal.TaxOperandIntegrator.TaxOperandIntegrator():New()
							// Processa os tributos
							oIntegrador:ProcessItemTaxJson(jTribAuto)
						Endif
        ...
        While ( !Eof() .And. SD1->D1_FILIAL == xFilial("SD1") .And.;
			...
			If cPaisLoc == "BRA" .AND. cMvEstado=="SC" 
				...
				if lCFG //Tributos Genericos
					aRetCrePre := oTribGen:getPresumedCredit(nItemNF)
