...
*/
Function MATA140(xAutoCab,xAutoItens,nOpcAuto,lSimulaca,nTelaAuto,xCompDKD,xJAutoVinc)
...
Local aCores    := {	{ 'Empty(F1_STATUS)','ENABLE' 		},;// NF Nao Classificada
						...
						{ 'F1_TIPO=="5"'	,'br_azul.bmp'	},;// NF de Credito
					    { 'F1_TIPO=="6"'	,'br_amarelo.bmp'}}// NF de Debito
...
Private nQtdAnt    	:= 0
Private aAutoDKD	:= {}
Private oJAutoVinc 	:= nil // Memória dos documentos vinculados, quando NF de crédito/débito ou Compra Governamental, em execauto.

DEFAULT nOpcAuto	:= 3
DEFAULT lSimulaca	:= .F.
DEFAULT nTelaAuto   := 0
Default xJAutoVinc	:= nil
...
If (SuperGetMV("MV_CONFFIS",.F.,"N") == "S")
	aCores    := {	{ '((F1_STATCON $ "1|4") .OR. EMPTY(F1_STATCON)) .AND. Empty(F1_STATUS)'	, 'ENABLE' 		},;	// NF Nao Classificada
					...
					{ '((F1_STATCON $ "1|4") .OR. EMPTY(F1_STATCON)) .AND. F1_TIPO=="D"'    	, 'BR_AMARELO'	},;	// NF de Devolucao
					{ '((F1_STATCON $ "1|4") .OR. EMPTY(F1_STATCON)) .AND. F1_TIPO=="5"'    	, 'br_azul.bmp'	},;	// NF de Credito
					{ '((F1_STATCON $ "1|4") .OR. EMPTY(F1_STATCON)) .AND. F1_TIPO=="6"'    	, 'br_amarelo.bmp'	},;	// NF de Debito
...
Private aAltDKD		:= {}
Private oGetDKD		:= Nil
Private oJDocOrig 	:= nil // Memória dos documentos vinculados, quando NF de crédito/débito ou Compra Governamental, em execauto.
...
//Verifica se tem os documentos referenciados da Compra Governamental
if l140Auto .and. oJAutoVinc != nil 
	oJDocOrig := oJAutoVinc
endif
...
if FWSX1Util():ExistItem("MTA140","Habilita Complem. de Itens ?")
	lShowDKD    := if(mv_par04 == 1,.T.,.F.)
endif
...
If lContinua
	...
	If lContinua
		...
		If lDKD //Tem DKD, verifica se tem campos adicionais para serem apresentados
			lTabAuxD1 := A103DKD(,,l140Altera,l140Visual,lShowDKD) //MATA103COM
		Endif
		...
		If l140Auto
			...
			If !l140Exclui
				...
				If l140Inclui
					...
					ElseIf cPaisLoc == "BRA" .And. cTipo $ "5|6" .And. (nX := _oAutoCab["F1_TPCOMPL"]) > 0;
						 	.And. ( (cTipo == "5" .and. aAutoCab[nX,2] $ "12345") .or. (cTipo == "6" .and. aAutoCab[nX,2] $ "12345678") ) 
						cTpCompl := aAutoCab[nX,2]
					EndIf
...
Local lWMSSaas      := FindFunction("WMSSaasHas") .And. WMSSaasHas()
Local lIntWMS		:= IntWMS()
Local lDocRef 		:= iif(FindFunction("A103ChkDKNAmb"),A103ChkDKNAmb(),.F.) // -- Tabela e interface de documentos referenciados
Local cTpMovNF		:= "1"
Local lIncItNFRF	:= .F.
Local aDadTemp		:= {}
Local aItVincTmp	:= {}
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If cTipo$"DB" .Or. (cTipo == "5" .And. cTpCompl == "3")
	dbSelectArea("SA1")
	dbSetOrder(1)
...
For nX := 1 To nMaxFor
	...
	Begin Transaction
		...
		If nX == 1 .And. lGrava
			...
			SF1->F1_EST    := IIF(!Empty(cUfOrigP),cUfOrigP,IIf(cTipo$"DB" .Or. (cTipo == "5" .And. cTpCompl == "3"),SA1->A1_EST,SA2->A2_EST))
			SF1->F1_TIPO   := cTipo
			...
			If ( (lcTpCompl .And. cTipo == "C") .Or. ( (cTipo == "5" .And. cTpCompl $ "12345") .Or. (cTipo == "6" .And. cTpCompl $ "12345678") ) )
		...
		If lTravou
			...
			If aCols[nX][nUsado+1]
				...
				If lDKD .And. lTabAuxD1 .And. laColsDKD .And. Len(aColsDKD) > 0 .And. (nJ	:= aScan(aColsDKD,{|x| x[1] == SD1->D1_ITEM})) > 0;
					.or. type("lDKD") == "L" .and. lDKD .and. (type("lTabAuxD1") == "L" .and. !lTabAuxD1) .and. type("lShowDKD") == "L" .and. !lShowDKD
					A103DKDGRV(aHeadDKD,aColsDKD,nJ,"D",lTabAuxD1)
				Endif
			...
			Else
				...
				If lDKD .And. lTabAuxD1 .And. laColsDKD .And. Len(aColsDKD) > 0 .And. (nJ	:= aScan(aColsDKD,{|x| x[1] == SD1->D1_ITEM})) > 0;
					.or. type("lDKD") == "L" .and. lDKD .and. (type("lTabAuxD1") == "L" .and. !lTabAuxD1) .and. type("lShowDKD") == "L" .and. !lShowDKD
					A103DKDGRV(aHeadDKD,aColsDKD,nJ,"",lTabAuxD1)
				Endif 
...
//Grava DKN, caso tenha documentos referenciados, devido a Compra Governamental, oriundos do Monitor XML
if ( lDocRef .AND. FwIsInCallStack("MATA140") )

	lIncItNFRF := ( (cTipo == "5" .and. cTpCompl == '3') .Or. (cTipo == "6" .and. cTpCompl == '4') )
	if ( cTipo == "5" .and. cTpCompl == "3")
		cTpMovNF := "2" //Saída, pois quando é crédito e devolução, é na SF2 os dados
	endif

	if ( lInclui .AND. oJDocOrig != nil )//-- Grava documentos referenciados
		if !lIncItNFRF
			A140GRDKNDC(oJDocOrig["documentos"], cNFiscal, cSerie, cA100For, cLoja, "MATA103", cTpMovNF, "")
		else
			For nX := 1 to nMaxFor
				if oJDocOrig:hasproperty('itens') .and. len(oJDocOrig['itens']) > 0
					aDadTemp := aClone(oJDocOrig["itens"])
					nPos := aScan(aDadTemp, {|x| x['item'] + alltrim(x['produto'])  == aCols[nX][nPosItNF] + Alltrim(aCols[nX][nPosCod]) })
					if nPos > 0
						aItVincTmp := aClone(oJDocOrig["itens"][nPos]['documentos'])
						A140GRDKNDC(aItVincTmp, cNFiscal, cSerie, cA100For, cLoja, 'MATA103', cTpMovNF, aCols[nX][nPosItNF])
					endif
				endif
			next nX++
			FwFreeArray(aItVincTmp)
			FwFreeArray(aDadTemp)
		endif
	elseif ( lExclui )
		A140DELREF(cNFiscal, cSerie, cA100For, cLoja, cTpMovNF, "MATA103")
	endif
endif
...
/*/{Protheus.doc} A140GRDKNDC
	Função geral que grava os campos na tabela DKN.
	@type  Function
	@author renan.martins
	@since 10/2025
	@param aDados, array, array com o json que contêm os dados da NF de referencia, para gravar na DKN
	@param cNumDoc, caracter, número da NF que está sendo inserida
	@param cSerieDoc, caracter, número da série da NF que está sendo inserida
	@param cFornecDoc, caracter, código do fornecedor
	@param cLojaDoc, caracter, loja do fornecedor
	@param cOrigem, caracter, nome do fonte/função que originou a chamada
	@param cTpMov, caracter, tipo de movimento 1 - Entrada / 2 - Saída
	@param cNumItem, se gravação por item (Débito e Crédito), informar o número do item da NF atual.
	@return nil
/*/
function A140GRDKNDC(aDados, cNumDoc, cSerieDoc, cFornecDoc, cLojaDoc, cOrigem, cTpMov, cNumItem)
	Local nFor			:= {}
	Local nSize			:= 0
	Local cFilAtu		:= fwxFilial("DKN")
	Local lExistDKN 	:= .F.
	Local cFornCli 		:= ""
	Local cLjForCli 	:= ""
	...
	nSize := Len(aDados)
	if ( nSize > 0)
		DKN->(DbSetOrder(1))//DKN_FILIAL, DKN_DOC, DKN_SERIE, DKN_CLIFOR, DKN_LOJA, DKN_DOCREF, DKN_SERREF, DKN_PARREF, DKN_LOJREF, DKN_ITNFRE, DKN_TPMOV, DKN_ORIGEM
		for nFor := 1 to nSize

			cFornCli  := if(aDados[nFor]:hasProperty("forclirf"),aDados[nFor]["forclirf"],cFornecDoc)
			cLjForCli := if(aDados[nFor]:hasProperty("lojarf"),aDados[nFor]["lojarf"],cLojaDoc)

			lExistDKN := .F.
			if aDados[nFor]:hasProperty('item')
				lExistDKN := DKN->(DbSeek(cFilAtu + cNumDoc + cSerieDoc + cFornecDoc + cLojaDoc + aDados[nFor]['documento'] + aDados[nFor]['serie'] + cFornCli + cLjForCli + aDados[nFor]['item'] + cTpMov + cOrigem))
			else
				lExistDKN := ExistRegDKN(cOrigem,cTpMov,cNumDoc,cSerieDoc,cFornecDoc,cLojaDoc,aDados[nFor]['documento'],aDados[nFor]['serie'],cFornCli,cLjForCli)
			endif
			
			if !lExistDKN
				DKN->( RecLock("DKN",.T.) )
					// -- Dados do documento de crédito/débito
					DKN->DKN_FILIAL	:= cFilAtu
					DKN->DKN_TPMOV	:= cTpMov //-- Entrada
					DKN->DKN_DOC	:= cNumDoc
					DKN->DKN_SERIE	:= cSerieDoc
					DKN->DKN_CLIFOR	:= cFornecDoc
					DKN->DKN_LOJA	:= cLojaDoc
					if ( !empty(cNumItem) )
						DKN->DKN_ITEMNF := cNumItem
					endif

					// -- Dados do documento referênciado
					DKN->DKN_DOCREF := aDados[nFor]['documento']
					DKN->DKN_SERREF := aDados[nFor]['serie']
					
					DKN->DKN_PARREF := cFornCli
					DKN->DKN_LOJREF := cLjForCli

					DKN->DKN_CHVNFE := aDados[nFor]['chave']
					if ( aDados[nFor]:hasProperty('item') )
						DKN->DKN_ITNFRE := aDados[nFor]['item']
						DKN->DKN_ITXML := aDados[nFor]['itemxml']
					endif
					DKN->DKN_ORIGEM := cOrigem
...
/*/{Protheus.doc} A140DELREF
	Função geral que exclui os registros na tabela DKN.
	@type  Function
	@author renan.martins
	@since 10/2025
	@param cNumDoc, caracter, número da NF que vai ser excluída
	@param cSerieDoc, caracter, número da série da NF que vai ser excluída
	@param cFornecDoc, caracter, código do fornecedor
	@param cLojaDoc, caracter, loja do fornecedor
	@param cTpMov, caracter, tipo de movimento 1 - Entrada / 2 - Saída
	@param cOrigem, caracter, nome do fonte/função que originou a chamada
	@return nil
/*/
Function A140DELREF(cNumDoc, cSerieDoc, cFornecDoc, cLojaDoc, cTpMov, cOrigem)
	Local aAreaDKN		:= DKN->(GetArea())
	Local cAliasTmp 	:= ""
	Local cQuery		:= ""
	Local oQry			:= Nil
	...
	cQuery += " WHERE DKN.DKN_FILIAL  	= ? "
	cQuery += "    AND DKN.DKN_DOC    	= ? "
	cQuery += "    AND DKN.DKN_SERIE  	= ? "
	cQuery += "    AND DKN.DKN_CLIFOR 	= ? "
	cQuery += "    AND DKN.DKN_LOJA 	= ? "
	cQuery += "    AND DKN.DKN_TPMOV	= ? "
	cQuery += "    AND DKN.DKN_ORIGEM	= ? "
	cQuery += "    AND DKN.D_E_L_E_T_	= ' ' "

	oQry := FwExecStatement():New(cQuery)

	oQry:SetString(1, FWxFilial("DKN"))
	oQry:SetString(2, cNumDoc)
	oQry:SetString(3, cSerieDoc)
	oQry:SetString(4, cFornecDoc)
	oQry:SetString(5, cLojaDoc)
	oQry:SetString(6, cTpMov)
	oQry:SetString(7, cOrigem)

	cAliasTmp := oQry:OpenAlias()
	DKN->(DbSelectArea("DKN"))
	
	while ( !(cAliasTmp)->(Eof()) )
		DKN->( DbGoTo( (cAliasTmp)->REC) )
...
/*/{Protheus.doc} ExistRegDKN
	Verifica se existe DKN (por NF) com os parâmetros passados
	...
	@version 1.0
/*/
Function ExistRegDKN(cOrigem,cTpMov,cDoc,cSerieNF,cFornece,cLojaFor,cDocRef,cSerieRef,cForCliRef,cLojaRef) 
	...
	cQry += "    AND DKN.DKN_DOCREF  = ? "
	cQry += "    AND DKN.DKN_SERREF  = ? "
	cQry += "    AND DKN.DKN_PARREF  = ? "
	cQry += "    AND DKN.DKN_LOJREF  = ? "
	cQry += "    AND DKN.DKN_TPMOV   = ? "
	...
	oObjQry:SetString(6,cDocRef) 
	oObjQry:SetString(7,cSerieRef) 
	oObjQry:SetString(8,cForCliRef) 
	oObjQry:SetString(9,cLojaRef) 
	oObjQry:SetString(10,cTpMov) 
	oObjQry:SetString(11,cOrigem) 
	oObjQry:SetString(12,Space(1)) 
