...
Local cC7JustFi		:= iif( SC7->(fieldPos("C7_JUSTIFI")) > 0, "C7_JUSTIFI|", "" )
Local cDescCli 		:= ""
Local cTamSx3		:= ""
...
If SCR->CR_TIPO $ 'PC|AE'
	bLoad := {|oModel, lCopy | loadGridIt(oModel, lCopy, cMT094CPC)} //-- Bloco de carga de dados para PC e AE
EndIf

If SCR->CR_TIPO $ 'PV|DV' .And. FindFunction('FatItGrid') //-- Bloco de carga de dados para PV e DV
	bLoad := {|oModel | FatItGrid(oModel)}
EndIf
...
If SCR->CR_TIPO $ 'IP|PC|AE'
	oStr4:= FWFormStruct(1,'SC7',{|cCampo| AllTrim(cCampo) $ "C7_ITEM|C7_PRODUTO|C7_DESCRI|C7_UM|C7_SEGUM|C7_QUANT|C7_PRECO|C7_TOTAL|C7_QTSEGUM|C7_VALFRE|C7_DESPESA|C7_SEGURO|C7_VALEMB|C7_VALIPI|C7_ICMSRET|C7_CODITE|C7_CODGRP|" + cC7JustFi + cFieldSC7})
...
Elseif SCR->CR_TIPO $ 'PV|DV'
	oStr4:= FWFormStruct(1,'SC6',{|cCampo| AllTrim(cCampo) $ "C6_ITEM|C6_PRODUTO|C6_DESCRI|C6_UM|C6_SEGUM|C6_QTDVEN|C6_UNSVEN|C6_PRCVEN|C6_VALOR|C6_DESCONT|C6_VALDESC"})
EndIf



If !(SCR->CR_TIPO $ 'SC|SA|CT|RV|MD|A1|A2|PV|DV')
...
If SCR->CR_TIPO $ 'PV|DV' //-- Cliente para Pedido de Vendas - Faturamento
	
	SC5->(dbSetOrder(1))
	If SC5->(MsSeek(FwxFilial("SC5")+AllTrim(SCR->CR_NUM)))				
		If !(SC5->C5_TIPO $ "D")
			cDescCli := STR0066 //Cliente
			cTamSx3  := FWSX3Util():GetFieldStruct( "A1_NOME" )[3]
		Else
			cDescCli := STR0016 //Fornecedor
			cTamSx3  := FWSX3Util():GetFieldStruct( "A2_NOME" )[3]
		EndIf
	EndIf

	oStr1:AddField(cDescCli																,;	// 	[01]  C   Titulo do campo  
					 cDescCli															,;	// 	[02]  C   ToolTip do campo
					 "CR_FORNECE"														,;	// 	[03]  C   Id do Field
					 "C"																,;	// 	[04]  C   Tipo do campo
					 cTamSx3															,;	// 	[05]  N   Tamanho do campo
					 0																	,;	// 	[06]  N   Decimal do campo
					 NIL																,;	// 	[07]  B   Code-block de validação do campo
					 NIL																,;	// 	[08]  B   Code-block de validação When do campo
					 NIL																,;	//	[09]  A   Lista de valores permitido do campo
...
cMoedaDoc 	:= SuperGetMV("MV_MOEDA"+AllTrim(Str(GetAdvFval("SCR", "CR_MOEDA", xFilial("SCR")+SCR->CR_TIPO+SCR->CR_NUM, 1),2)),.F.,"2")
...
oStr1:AddField( 	 STR0055															,;	// 	[01]  C   Titulo do campo
					 ...
					 NIL																,;	//	[09]  A   Lista de valores permitido do campo
					 .F.																,;	//	[10]  L   Indica se o campo tem preenchimento obrigatório
					 {|| cMoedaDoc}													,;	//	[11]  B   Code-block de inicializacao do campo
					 NIL																,;	//	[12]  L   Indica se trata-se de um campo chave
					 NIL																,;	//	[13]  L   Indica se o campo pode receber valor em uma operação de update.
...
ElseIf SCR->CR_TIPO $ 'PC|AE'
	oModel:addGrid('GridDoc','FieldSCR',oStr4,/*bLinePre*/,,,,bLoad)
	oModel:getModel('GridDoc'):SetOnlyQuery(.T.)
	oModel:getModel('GridDoc'):SetDescription(STR0020)
	// Aumenta numero maximo de linha das GRID's
	oModel:GetModel( "GridDoc" ):SetMaxLine( 9999 )
ElseIf SCR->CR_TIPO $ 'NF|CP'
	oModel:addGrid('GridDoc','FieldSCR',oStr4)
	oModel:getModel('GridDoc'):SetOnlyQuery(.T.)
	oModel:getModel('GridDoc'):SetDescription(STR0020)
	// Aumenta numero maximo de linha das GRID's
...
If SCR->CR_TIPO $ 'IP|PC|AE'
	oStr4:= FWFormStruct(2,'SC7',{|cCampo| AllTrim(cCampo) $ "C7_ITEM|C7_PRODUTO|C7_DESCRI|C7_UM|C7_SEGUM|C7_QUANT|C7_PRECO|C7_TOTAL|C7_QTSEGUM|C7_VALFRE|C7_DESPESA|C7_SEGURO|C7_VALEMB|C7_VALIPI|C7_ICMSRET|C7_CODITE|C7_CODGRP" + cC7JustFi})
	oStr4:RemoveField( 'C7_DESC' )
ElseIf SCR->CR_TIPO == 'SA'
...
Elseif SCR->CR_TIPO $ 'PV|DV'
	oStr4:= FWFormStruct(2,'SC6',{|cCampo| AllTrim(cCampo) $ "C6_ITEM|C6_PRODUTO|C6_DESCRI|C6_UM|C6_SEGUM|C6_QTDVEN|C6_UNSVEN|C6_PRCVEN|C6_VALOR|C6_DESCONT|C6_VALDESC"})
EndIf

If !(SCR->CR_TIPO $ 'SC|SA|CT|RV|MD|IM|IR|IC|A1|A2|PV|DV')
	//-- Campo fornecedor para tipos diferentes de SA e SC
...
If SCR->CR_TIPO $ 'PV|DV' //-- Cliente para Pedido de Vendas - Faturamento
	...
	oStr1:AddField("CR_FORNECE"															,;	// [01]  C   Nome do Campo
					"10"																,;	// [02]  C   Ordem
					cDescCli															,;	// [03]  C   Titulo do campo//"Descrição"
					cDescCli															,;	// [04]  C   Descricao do campo//"Descrição"
					NIL																	,;	// [05]  A   Array com Help
					"C"																	,;	// [06]  C   Tipo do campo
					""																	,;	// [07]  C   Picture
					NIL																	,;	// [08]  B   Bloco de Picture Var
	...
	For nI := 1 To Len(aMT094CPC)
		If !Empty(aMT094CPC[nI]) .And. Upper(SubStr(aMT094CPC[nI], 1, 2)) == "C7"
			oStr4:AddField(aMT094CPC[nI]												,;	// [01]  C   Nome do Campo
						...
						AllTrim(GetSx3Cache(aMT094CPC[nI],"X3_PICTURE"))					,;	// [07]  C   Picture
						NIL																	,;	// [08]  B   Bloco de Picture Var
						NIL																	,;	// [09]  C   Consulta F3
						.F.																	,;	// [10]  L   Indica se o campo é alteravel
						NIL																	,;	// [11]  C   Pasta do campo
...
If lLGPD .And. !(SCR->CR_TIPO $ 'SC|SA|CT|RV|MD|IM|IR|IC|A1|A2|PV|DV')
	oStr1:SetProperty( "CR_FORNECE" , MVC_VIEW_OBFUSCATED , OfuscaLGPD(,"A2_NOME") )
Endif

If lLGPD .And. (SCR->CR_TIPO $ 'PV|DV')
	If cDescCli == "Cliente"
		oStr1:SetProperty( "CR_FORNECE" , MVC_VIEW_OBFUSCATED , OfuscaLGPD(,"A1_NOME") )
	Else
		oStr1:SetProperty( "CR_FORNECE" , MVC_VIEW_OBFUSCATED , OfuscaLGPD(,"A2_NOME") )
	EndIf
Endif
...
If SCR->CR_TIPO $ 'IP|PC|IC|CT'
	oView:AddUserButton(STR0069 ,'',{|| AViewQuote(SCR->CR_TIPO) } ) 
EndIf
...
ElseIf SCR->CR_TIPO $ 'PV|DV'
	...
	oView:EnableTitleView('GridDoc' , STR0020 ) //"Itens" 
	oView:SetViewProperty('GridDoc'  , 'ONLYVIEW' )
	oView:SetNoInsertLine('GridDoc')
	oView:AddUserButton(STR0068 ,'',{|| A097Visual(,,'2') },,,,.T.) 
EndIf
...
//--------------------------------------------------------------------
/*/{Protheus.doc} AViewQuote()
Realiza a abertura da tela de Visualizar Cotação
@author Ali Ahmad
@since 30/09/2025
@version 1.0
@return Nil
/*/
//--------------------------------------------------------------------
Static Function AViewQuote(cTipo)
	...
	If !Empty(cNumCot)
		DHU->(DbSetOrder(1))
		If DHU->(MsSeek(fwxFilial("DHU", cFil) + cNumCot))
			FWExecView(STR0069, 'NFCCOTA', MODEL_OPERATION_VIEW) //-- Visualizar Cotação
		Else
			Help("",1,"VIEWQUOTE1",,STR0070, 1,0) //-- Somente cotações originadas no NFC poderão ser visualizadas.
		EndIf
	Else 
		Help("",1,"VIEWQUOTE2",,STR0071, 1,0) //-- Liberação não é proveniente de cotação.
	EndIf
...
Local SD1TMP  		As Character 
Local cQuery 		As Character
Local cBusca		As Character  
Local nOrder  		As Numeric
Local lCm020Blq 	As Logical

lCm020Blq := GetAPOInfo("COMA020.prw")[4] >= Ctod("12/08/2025")
cBusca	  := ""
...
If cIdOption <> OP_VIW .Or. SCR->CR_STATUS == '02'
	// Se a moeda do documento for igual a moeda do aprovador, não faz conversão
	If SCR->CR_MOEDA == aSaldo[2]

    	nMoedaCalc := SCR->CR_TOTAL

	Else //Se for moedas diferentes, faz a conversão

			nMoedaCalc := xMoeda(SCR->CR_TOTAL,; // Valor total do documento
						Iif(SCR->CR_TIPO != "NF",SCR->CR_MOEDA,1),; // Moeda do documento (se for NF, usa a moeda base)
						aSaldo[2],; 			// Moeda do aprovador
						SCR->CR_EMISSAO,; 		// Data de emissão do documento
						TamSX3('CR_TOTAL')[2],; // Tamanho decimal do CT_TOTAL
						SCR->CR_TXMOEDA) 		// Taxa da moeda do documento
	EndIf
	If SCR->CR_TIPO != "NF"	
	...
	Else 
		oFieldSAK:LoadValue( "AK_SLDCALC", oFieldSAK:GetValue("AK_SLDDATE")) //Mantem o saldo atual, pois alçada do tipo NF não deve ser consumido o saldo do aprovador
	EndIf 
Else
	nMoedaCalc := SCR->CR_TOTAL
...
DBselectArea("DHL")
DHL->(DbSetOrder(1))
DHL->(MsSeek(xFilial("DHL") + SAL->AL_PERFIL))
...
If SCR->CR_TIPO $ 'IP|SA|IC|IR|IM' .Or. (SCR->CR_TIPO == 'SC' .And. lAlcSolCtb)
	...
	//--------------------------------------
	A094GetDBM(SCR->CR_NUM, SCR->CR_ITGRP, SCR->CR_TIPCOM, SCR->CR_GRUPO, SCR->CR_USER, SCR->CR_APROV)
EndIf
...
If SCR->CR_TIPO $ 'PC|AE'
	...
	cFornName := Alltrim(GetAdvFVal("SA2","A2_NOME",fwxFilial("SA2")+SC7->C7_FORNECE+SC7->C7_LOJA,1))
	oFieldSCR:LoadValue("CR_FORNECE"  	,cFornName)
	oFieldSCR:LoadValue("CR_DTOTAL"		,nMoedaCalc)
...
ElseIf SCR->CR_TIPO $ "NF"
	...
	cQuery := " SELECT SD1.D1_ITEM, SD1.D1_COD, SD1.D1_QUANT, SD1.D1_VUNIT, SD1.D1_PEDIDO, SD1.D1_ITEMPC, SD1.D1_FORNECE, SD1.D1_LOJA, SD1.D1_EMISSAO,SD1.D1_DTDIGIT"
	cQuery += " FROM ? SD1 "  
	cQuery += "	WHERE SD1.D_E_L_E_T_= ? " 
	cQuery += " AND SD1.D1_FILIAL 	= ? "
	cQuery += "	AND SD1.D1_DOC = ? AND SD1.D1_SERIE = ? "
	cQuery += "	AND SD1.D1_FORNECE = ? AND SD1.D1_LOJA= ? "
	...
	oQry:SetString(nOrder++,SubsTr(SCR->CR_NUM,1,nTamF1Doc))
	oQry:SetString(nOrder++,SubsTr(SCR->CR_NUM,(nTamF1Doc+1),nTamF1Ser))
	oQry:SetString(nOrder++,SubsTr(SCR->CR_NUM,(nTamF1Doc+nTamF1Ser+1),nTamF1For))
	oQry:SetString(nOrder++,SubsTr(SCR->CR_NUM,(nTamF1Doc+nTamF1Ser+nTamF1For+1),nTamF1Loj))

	cQuery    := oQry:GetFixQuery()
	SD1TMP := MpSysOpenQuery(cQuery,"SD1TMP")
		...
		While !SD1TMP->(EOF())
			...
			If SC7->(dbSeek(xFilEnt(xFilial("SC7"),"SC7")+Padr(SD1TMP->D1_PEDIDO,TamSX3("C7_NUM")[1])+PadR(SD1TMP->D1_ITEMPC,TamSX3("C7_ITEM")[1])))
				...
				if lNfMotBloq .and. Len(aNfMotBloq) > 0
					cPed 	:= SD1TMP->D1_PEDIDO
					cItemNF := SD1TMP->D1_ITEMPC
					cMotblq := arrtokstr(aNfMotBloq)
					cBusca  := AllTrim(cPed) + AllTrim(cItemNF)
					
					if !Empty(cMotblq) 
						cDiverg := ""
						
						//Tratamento para manter o comportamento legado, pois anteriormente era gravado apenas o item no motivo de bloqueio. 
						If !lCM020Blq .Or. (lCM020Blq .and. SD1TMP->D1_DTDIGIT <= "20250812")
							cBusca := AllTrim(cItemNF)
						EndIf 
						...
						If cBusca+"P" $ cMotblq
							cDiverg += If(!Empty(cDiverg),"/"+STR0040,STR0040) //"Preço"
						Endif
						
						If cBusca+"E" $ cMotblq
							cDiverg += If(!Empty(cDiverg),"/"+ STR0067,STR0067) //Prz. Entrega
						Endif
...
If SCR->CR_TIPO == 'IP'
	...
	While !DBMTMP->(EOF())
		...
		If SC7->(dbSeek(xFilial("SC7")+ cNum + cItem ) ) .AND. IIf(cIdOption == OP_VIW, .T., DBMTMP->DBM_APROV == "2")
			If cDocBkp <> cNum + cItem	
				...
				if !empty(cC7JustFi)
					oModelGrid:LoadValue("C7_JUSTIFI"		, SC7->C7_JUSTIFI  )	
				endif
...
ElseIf SCR->CR_TIPO == 'SC' .And. lAlcSolCtb
	...
	While !DBMTMP->(EOF())
		...
		If SC1->(dbSeek(xFilial("SC1")+ cNum + cItem ) ) .AND. IIf(cIdOption == OP_VIW, .T., DBMTMP->DBM_APROV == "2")
			If cDocBkp <> cNum + cItem
				If nLinha # 1
					oModelGrid:AddLine()
				EndIf
	...
	While !DBMTMP->(EOF())
		...
		If lSeek
			If lComp
				...
				If CNZ->(dbSeek(xFilial("CNZ")+CNE->(CNE_CONTRA+CNE_REVISA+CNE_NUMMED+CNE_NUMERO)+cItem+cItemRa))
...
ElseIf SCR->CR_TIPO $ 'PV|DV'
	If SC5->(MsSeek(FwxFilial("SC5")+AllTrim(SCR->CR_NUM)))
		...
		If !(SC5->C5_TIPO $ "D")
			oFieldSCR:LoadValue("CR_FORNECE"  ,POSICIONE("SA1",1,FWxFilial("SA1")+SC5->(C5_CLIENTE+C5_LOJACLI),"A1_NOME"))
		Else
			oFieldSCR:LoadValue("CR_FORNECE"  ,POSICIONE("SA2",1,FWxFilial("SA2")+SC5->(C5_CLIENTE+C5_LOJACLI),"A2_NOME"))
		EndIf
	 ...
	 If FWFormCommit(oModel)
		Do Case
			...
			Case SCR->CR_TIPO $ "PV|DV"
				cChave := FwxFilial("SC5")+Substr(SCR->CR_NUM,1,len(SC5->C5_NUM))
				dbSelectArea("SC5")
				dbSetOrder(1)
				MsSeek(cChave)
...
If SCR->CR_TIPO $ "PC|SC|IP|AE|A1|A2|MD|IM|CT|IC|RV|IR|CP|SA|NF|PV|DV"
	SAK->(dbSetOrder(1))
...
If lRet .And. SCR->CR_TIPO $ "PC|IP|CP|AE|NF|SC" .And. cOperID == OP_LIB
	If !(lRet := A094PcoLan())
...
//--------------------------------------------------------------------
Function A094Bloqu()
Local lMtalCper As Logical
Local aTpCper 	As Array 
Local cTpAlcPer As Character

lMtalCper 	:= ExistBlock("MTALCPER")
aTpCper 	:= {}
cTpAlcPer 	:= ""
...
if lMtalCper
	aTpCper := Execblock("MTALCPER",.F.,.F.)
	
	If ValType(aTpCper) == "A" .And. Len(aTpCper) > 0 
		cTpAlcPer := aTpCper[1][1]  
	endif 
endif 
...
ElseIf !(SCR->CR_TIPO $ 'SA|ST|SC|PC|IP|CT|IC|RV|IR|MD|IM|AE|CP|PV|DV') .And. SCR->CR_TIPO != cTpAlcPer
...
If lRet .And. SCR->CR_TIPO >= "A1" .AND. SCR->CR_TIPO <= "A9" // Documentos do modulo Agro
	If FindFunction("OGXUtlOrig") //Identifica que esta utilizando o sigaagr				
		...
		EndIf
	EndIf
ElseIf lRet .And. !(SCR->CR_TIPO $ 'SA|ST|SC|PC|IP|CT|IC|RV|IR|MD|IM|AE|CP|PV|DV') .And. SCR->CR_TIPO != cTpAlcPer
	lRet := .F.
...
IF SCR->CR_TIPO == "PC" .Or. SCR->CR_TIPO == "AE" .Or. SCR->CR_TIPO == "IP"
	lVldPCO := !A094VrfLan(SC7->C7_FILIAL,SC7->C7_NUM,SCR->CR_TIPO,SCR->CR_GRUPO,SCR->CR_ITGRP,If(lPosCrTipo,SCR->CR_TIPCOM,""))
ElseIf	SCR->CR_TIPO == "SC"
	dbSelectArea("SC1")
	DbSetOrder(1)
	If DbSeek(xFilial("SC1")+Substr(SCR->CR_NUM,1,len(SC1->C1_NUM))) 
		lVldPCO := !A094VrfLan(SC1->C1_FILIAL,SC1->C1_NUM,SCR->CR_TIPO,SCR->CR_GRUPO,SCR->CR_ITGRP,If(lPosCrTipo,SCR->CR_TIPCOM,""))
	EndIf
ENDIF
...
If lLanPCO .and. lVldPCO
	...
	dbSelectArea("DBM")
	DbSetOrder(1)
	IF DbSeek(xFilial("DBM")+SCR->CR_TIPO + SCR->CR_NUM+ SCR->CR_GRUPO+ SCR->CR_ITGRP+ SCR->CR_USER+ SCR->CR_USERORI)
	  	dbSelectArea("SC7")
		DbSetOrder(1)
		IF DbSeek(xFilial("SC7")+Substr(SCR->CR_NUM,1,len(SC7->C7_NUM)))
			dbSelectArea("SCH")
			DbSetOrder(1)
			IF DbSeek(xFilial("SCH")+padr(SCR->CR_NUM,TAMSX3("CH_PEDIDO")[1])+SC7->C7_FORNECE+SC7->C7_LOJA+SC7->C7_ITEM+DBM->DBM_ITEMRA)
				lRet := PcoVldLan('000055','03','MATA097')
			ENDIF
		Endif	
	ENDIF	
	If !lRet
...
/*/
//--------------------------------------------------------------------
Static Function A094VrfLan(cFilSCR,cNum,cTipo,cGrupo,cItemGrp,cTipCom )
Local lRet       := .F. 
...
SCR->(DbSetOrder(1)) //CR_FILIAL+CR_TIPO+CR_NUM+CR_NIVEL  
  lRatCC := cTipo == 'IP' .AND. EMPTY(SC7->C7_CC) .AND. !EMPTY(cGrupo )                                                                                                                   
If SCR->(DbSeek(cFilSCR+cTipo+cNum))
	While  !lRet .and. !Eof() .and. cFilSCR == SCR->CR_FILIAL .and. cTipo == SCR->CR_TIPO .and. cNum == AllTrim(SCR->CR_NUM ) 
		If !lRatCC .OR. (lRatCC .AND. cGrupo  == AllTrim(SCR->CR_GRUPO ))

			If SCR->CR_TIPO == "IP" .And.; 
				(AllTrim(SCR->CR_GRUPO) <> AllTrim(cGrupo) .Or.;
				AllTrim(SCR->CR_ITGRP) <> AllTrim(cItemGrp) .Or.;
				If(lPosCrTipo,AllTrim(SCR->CR_TIPCOM),"") <> AllTrim(cTipCom))
...
/*/{Protheus.doc} loadGridIt
	Realiza a carga do grid de itens da SC7 - Somente PC e AE
@author Leandro Fini
@since 07/2024
@return aLoad --> Dados da carga
/*/
Static Function loadGridIt(oModelGrid, lCopy, cMT094CPC)

Local aLoad 		:= {}
Local aFields		:= oModelGrid:GetStruct():aFields
Local aItens 		:= {}
Local nX			:= 1
Local nZ 			:= 0
Local cEntCtb 		:= ""
Local aStrField 	:= {}
Local lHasIpi		:= oModelGrid:HasField("C7_VALIPI")
Local lHasICMS		:= oModelGrid:HasField("C7_ICMSRET")
Local lHasCodGrp	:= oModelGrid:HasField("C7_CODGRP")
Local lHasCodite	:= oModelGrid:HasField("C7_CODITE")
Local aMT094CPC		:= {}
Local cAliasSC7		:= ""
Local cQuery		:= ""
...
Default cMT094CPC := ""
	...
	cQuery := " SELECT SC7.C7_NUM,SC7.C7_ITEM, SC7.C7_PRODUTO,SC7.C7_DESCRI,SC7.C7_UM,SC7.C7_SEGUM,SC7.C7_PRECO,SC7.C7_QUANT,SC7.C7_TOTAL,SC7.C7_MOEDA,SC7.C7_QTSEGUM,SC7.C7_VALFRE,SC7.C7_RESIDUO,SC7.C7_DESPESA,SC7.C7_SEGURO,SC7.C7_VALEMB,SC7.C7_ITEMGRD, SC7.R_E_C_N_O_" + cEntCtb
	...
	MpSysOpenQuery(cQuery,cAliasSC7)
	...
	While !(cAliasSC7)->(EOF())
		...
		for nX := 1 to len(aStrField)
			if (cAliasSC7)->(FieldPos(aStrField[nX])) > 0
				aAdd(aItens,(cAliasSC7)->&(aStrField[nX]))					
			elseif aStrField[nX] == 'X_DMOEDA'
				aAdd(aItens,(cAliasSC7)->C7_MOEDA)
			elseif aStrField[nX] == 'C7_CODITE' .and. lHasCodite
				aAdd(aItens,IniAuxCod((cAliasSC7)->C7_PRODUTO,"C7_CODITE"))
			elseif aStrField[nX] == 'C7_CODGRP' .and. lHasCodGrp
				aAdd(aItens,IniAuxCod((cAliasSC7)->C7_PRODUTO,"C7_CODGRP"))
			elseif GetSx3Cache(aStrField[nX], "X3_CONTEXT") == "V" //caso seja virtual, carrega inicializador padrão
				aAdd(aItens,( & ( GetSx3Cache(aStrField[nX],"X3_RELACAO") ) ) )
			else
				aAdd(aItens,SC7->&(aStrField[nX]))
			endif
		next nX
...
/*/{Protheus.doc} A094GetDBM
	Obtém dados da DBM e retorna um alias com os dados.
@author juan.felipe
@since 23/04/2025
@param cNum, character, número do documento.
@param cItGrp, character, item do grupo de aprovação.
@param cTipCom, character, tipo de compras.
@param cGrupo, character, grupo de aprovação.
@param cUser, character, usuário do aprovador.
@param cAprov, character, código do aprovador.
@param lNextAlias, logical, indica se deve gerar o alias com base na função GetNextAlias ou o alias fixo DBMTMP.
@return cAlias, character, Alias da tabela com os dados.
/*/
Function A094GetDBM(cNum, cItGrp, cTipCom, cGrupo, cUser, cAprov, lNextAlias)
    Local cQuery As Character
    Local oQuery As Object
    Local lPosCrTipo As Logical
    Local cAlias As Character
    Local nParamIndex As Numeric
    ...
    Default cUser := ''
    Default cAprov := ''
    Default lNextAlias := .F.

	cAlias := Iif(lNextAlias, GetNextAlias(), "DBMTMP")
	lPosCrTipo  := SCR->(FieldPos("CR_TIPCOM")) > 0
	nParamIndex := 1

	cQuery := " SELECT DBM.DBM_NUM, DBM.DBM_ITEM, DBM.DBM_ITEMRA, DBM.DBM_APROV, DBM.R_E_C_N_O_ "
	cQuery += " FROM " + RetSQLName("DBM") + " DBM "
	cQuery += " WHERE "

	If FWModeAccess("DBM", 3) == "E"
		cQuery += " DBM.DBM_FILIAL = ? AND "
	EndIf

	cQuery += " DBM.DBM_NUM = ? AND "
	cQuery += " DBM.DBM_ITGRP = ? AND "

	If lPosCrTipo .AND. !Empty(cTipCom)
		cQuery += "DBM.DBM_TIPCOM = ? AND "
	EndIf

	cQuery += " DBM.DBM_GRUPO = ? AND "
	cQuery += " DBM.DBM_USER = ? AND "
	cQuery += " DBM.DBM_USAPRO = ? AND "
    ...
    If FWModeAccess("DBM", 3) == "E"
        oQuery:SetString(nParamIndex, xFilial("SC7"))
		nParamIndex++
    EndIf

    oQuery:SetString(nParamIndex, cNum)
	nParamIndex++
    oQuery:SetString(nParamIndex, cItGrp)
	nParamIndex++

    If lPosCrTipo .AND. !Empty(cTipCom)
        oQuery:SetString(nParamIndex, cTipCom)
		nParamIndex++
    EndIf

    oQuery:SetString(nParamIndex, cGrupo)
	nParamIndex++
    oQuery:SetString(nParamIndex, cUser)
	nParamIndex++
    oQuery:SetString(nParamIndex, cAprov)

    // Abre a área com a query montada
