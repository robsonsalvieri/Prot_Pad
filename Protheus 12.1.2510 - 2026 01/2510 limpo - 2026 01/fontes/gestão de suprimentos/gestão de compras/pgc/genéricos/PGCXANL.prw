...
Static _lMVPcoInte := SuperGetMV('MV_PCOINTE',.F.,'2') == '1' //-- Indica se o PCO está integrado
Static _oJsonResp := JsonObject():New() //-- Define Json para resposta da análise de cotação
Static _aContract := {}
Static _aHdSCE := {}
...
Static Function GerContra(aWinners, aHdSCE)
    ...
    If FwIsInCallStack("PGCA010")
        If !FwIsInCallStack('NFCCtrGRV') .And. !ExecModCtr(aData, aHdSCE, .T.)
            lRet := .F.					
        EndIf
    Else 
        Begin Transaction
...
/*{Protheus.doc} NFCGetCost
    Função que retorna o custo do produto conforme os impostos informados.
@param aTax, array, array com os impostos a serem considerados no cálculo do custo.
@param cSupplier, character, código do fornecedor.
@param cStore, character, código da loja do fornecedor.
@param lTotalize, logical, indica se deve totalizar o custo da cotação completo do fornecedor.
@author juan.felipe
@since 15/12/2025
@version 1.0
@return nCost, numeric, custo do produto conforme os impostos informados.
*/
Function NFCGetCost(aTax As Array, cQuote As Character, cSupplier As Character, cStore As Character, cCorpName As Character, cProp As Character, lTotalize As Logical) As Numeric
    Local nCost   As Numeric
    Local nY      As Numeric
    Local lTaxGen As Logical
    ...
    nCost   := 0
    nY      := 0
    lTaxGen := IIf(FindFunction("ChkTrbGen"), ChkTrbGen("SC8", "C8_IDTRIB"), .F.)
    cBranch := ""
    cAlias  := ""
    cQuery  := ""
    oQuery  := Nil

    If Empty(cSupplier+cStore) .Or. (!lTotalize .And. (Empty(SC8->C8_TES) .And. (lTaxGen .And. Empty(SC8->C8_IDTRIB)))) //-- Não processa fornecedor participante
        Return nCost
    Endif

    If lTotalize //-- Indica se totaliza a cotação completa do fornecedor
        ...
        oQuery:SetString(5, cCorpName)
        oQuery:SetString(6, cProp)
        ...
        While (cAlias)->(!Eof())
            ...
            nCost += NFCGetCost(aTax,, SC8->C8_FORNECE, SC8->C8_LOJA,,, .F.)
            (cAlias)->(DbSkip())
        EndDo
        ...
        oQuery:Destroy()
        FreeObj(oQuery)
    Else //-- Calcula o custo conforme os impostos informados
        MaFisIni(cSupplier, cStore, "F", "N", "R",,,,,,,,,,,,,,,,,,,,,,,,,,,,, lTaxGen)

        MaFisIniLoad(1)

        For nY := 1 To Len(aTax)
            MaFisLoad(aTax[nY,3], SC8->(FieldGet(FieldPos(aTax[nY,2]))), 1)
        Next nY

        MaFisEndLoad(1)
        nCost := Ma160Custo("SC8", 1)
        MaFisEnd()
    EndIf
Return nCost

/*{Protheus.doc} NFCCtrGRV
    Função que realiza a gravação da análise de cotação quando for geração de contrato e integração com PCO.
@param oModel, object, modelo de dados do CNTA300.
@author juan.felipe
@since 12/01/2026
@version 1.0
@return lRet, logical, indica se a gravação foi realizada com sucesso.
*/
Function NFCCtrGRV(oModel As Object) As Logical
    Local lRet As Logical
    Local oAnlRepo As Object
    Default oModel := Nil

    lRet := .T.

    If oModel <> Nil .And. oModel:GetOperation() == MODEL_OPERATION_INSERT .And. _lMVPcoInte
        If Type("__aGenContra") == "A" .And. aScan(__aGenContra, CN9->CN9_NUMERO) == 0
            aAdd(__aGenContra, CN9->CN9_NUMERO)
        EndIf

        oAnlRepo := pgc.analyzeQuotationRepository.pgcAnalyzeQuotationRepository():New()

        If FwIsInCallStack('analyzeQuoteByProposal') //-- Análise por proposta
            _oJsonResp := oAnlRepo:analyzeQuoteByProposal(_oJsonCtr['params'], _oJsonCtr, _oJsonCtr['justification'], _oJsonCtr['observations']) //-- Gera atualizações da SC8/SCE
        Else //-- Análise por item
            _oJsonResp := oAnlRepo:analyzeQuoteByItem(_oJsonCtr) //-- Gera atualizações da SC8/SCE
        EndIf

        lRet := oAnlRepo:lOk

        If lRet
            UpdateSC8(_aContract, CN9->CN9_NUMERO, _aHdSCE)
        Else
            __aGenContra := {}
            oModel:SetErrorMessage(,,,, 'NFCANLERROR', EncodeUTF8(_oJsonResp['message']))
        EndIf
    EndIf
Return lRet

/*{Protheus.doc} NFCAnlResp
    Função que retorna o resultado da análise de cotação.
@author juan.felipe
@since 12/01/2026
@version 1.0
@return _oJsonResp, object, objeto Json com o resultado da análise de cotação.
*/
Function NFCAnlResp()
Return _oJsonResp
