...
If Inclui .Or. Altera
	If SD1->(dbSeek(SF1->F1_FILIAL + SF1->F1_DOC + SF1->F1_SERIE + SF1->F1_FORNECE + SF1->F1_LOJA))
		While SF1->F1_FILIAL + SF1->F1_DOC + SF1->F1_SERIE + SF1->F1_FORNECE + SF1->F1_LOJA == SD1->D1_FILIAL + SD1->D1_DOC + SD1->D1_SERIE + SD1->D1_FORNECE + SD1->D1_LOJA .And. !SD1->(Eof())
			...
			cXMLRet +=  GetTaxes(cVersion,Val(SD1->D1_ITEM))
...
/*/
//-------------------------------------------------------------------
Static Function GetTaxes(cVersion,nCountIt)
...
If cPaisLoc == "BRA"
	// {Imposto, Valor, Aliquota, Base de Cálculo, Retenção, Valor CFGTRIB, Aliquota CFGTRIB, Base CFGTRIB}
	...
	If oTribGen == Nil .And. FindClass('backoffice.com.generictaxes.generictaxes')
		...
		cJsonRet   := oTribGen:getallRules( nCountIt , {"dados_itens"})
		oJson1 := JsonObject():New()
		oJson1:FromJson(cJsonRet)
		aTribGen := MaFisRet(,"NF_TRIBGEN")	
		For nX := 1 to len(aTribGen)							
			cIdTotvs  := aTribGen[nX][11]
			Do Case
				Case cIdTotvs ==  '000060'  // IBSUF
					aAdd(aImpostos, {"IBSEST",; //Imposto
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["val_trib"])),; //Valor
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["aliq_trib"])),; //Aliquota
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["base_trib"])),; //Aliquota
									.T.}) //Retenção
				Case cIdTotvs == '000061'// IBSMUN
					aAdd(aImpostos, {"IBSMUN",; //Imposto
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["val_trib"])),; //Valor
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["aliq_trib"])) ,; //Aliquota
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["base_trib"])) ,; //Aliquota
									.T.}) //Retenção
				Case cIdTotvs == '000062'// CBS
					aAdd(aImpostos, {"CBS",; //Imposto
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["val_trib"])),; //Valor
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["aliq_trib"])) ,; //Aliquota
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["base_trib"] )),; //Aliquota
									.T.}) //Retenção
				Case cIdTotvs == '000063'// IS
					aAdd(aImpostos, {"IS",; //Imposto
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["val_trib"])),; //Valor
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["aliq_trib"])) ,; //Aliquota
									AllTrim(Str(oJson1["dados_itens"][alltrim(str(nCountIt))][alltrim(aTribGen[nX][1])]["base_trib"])) ,; //Aliquota
									.T.}) //Retenção
...
For nI := 1 To Len(aImpostos)
	If &(aImpostos[nI][2]) > 0 .Or. ;
		((&(aImpostos[nI][2]) == 0 .And. aImpostos[nI][1] == "CBS") .Or. ;
		(&(aImpostos[nI][2]) == 0 .And. aImpostos[nI][1] == "IBSMUN") .Or. ;
		(&(aImpostos[nI][2]) == 0 .And. aImpostos[nI][1] == "IBSEST") .Or. ;
		(&(aImpostos[nI][2]) == 0 .And. aImpostos[nI][1] == "IS"))
