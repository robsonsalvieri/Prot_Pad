...
Default lAgrup    := .F.
	...
	ElseIf (cTabela == 'NSZ' .And. (aCamposSel[12] .OR. aCamposSel[4] == 'NSZ' )) // Verificação se é filho da NSZ
		...
		If (Len(aRelac) > 1)
			aRelac := JMainSX9(aRelac, cTabela, aCamposSel[5])
		EndIf
...
//-------------------------------------------------------------------
/*/{Protheus.doc} JMainSX9
Função que verifica o relacionamento principal através da validação da chave da tabela "pai"

@param aRelac    - Relacionamento entre tabelas
@param cTabPai   - Tabela pai
@param cTabFilha - Tabela filha

@return aMainRel - Retorna o relacionamento principal

@since 23/10/2025
/*/
//-------------------------------------------------------------------
Static Function JMainSX9(aRelac, cTabPai, cTabFilha)
Local aMainRel := {}
Local aMainChv := StrTokArr(AllTrim(InfoSx2(cTabPai, "X2_UNICO")),"+")
Local nI       := 0
Local nIndFind := 0

	For nI := 2 to Len(aMainChv)
		nIndFind := aScan(aRelac, {|x| AllTrim(x[1]) == AllTrim(aMainChv[nI])})

		If (nIndFind > 0)
			aAdd(aMainRel, aClone(aRelac[nIndFind]))
		EndIf

	Next nI

	If Len(aMainRel) == 0
		aMainRel := aClone(aRelac)
	EndIf
