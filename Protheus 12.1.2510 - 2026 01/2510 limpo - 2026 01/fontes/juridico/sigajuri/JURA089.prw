...
//-------------------------------------------------------------------
/*/{Protheus.doc} JA089NSW
Filtro para consulta padrão de Subclasse - Protheus / TJD

@param oJson - Objeto JSON com os valores dos campos para filtro - TJD

@Return cQuery - String com a cláusula WHERE para filtro da consulta

@since 09/12/25
@version 1.0
/*/
//-------------------------------------------------------------------
Function JA089NSW(oJson)
Local cQuery    := ""
Local cClassNSW := ""
Local nCt       := 0
Local oCmpPesq  := Nil
	...
	If IsPesquisa()
		...
		For nCt := 1 To Len(oCmpPesq)
			If oCmpPesq[nCt]:CNomeCampo == 'NSZ_CCLASS'
				cClassNSW := AllTrim(oCmpPesq[nCt]:Valor)
				If Len(cClassNSW) > 4
					cClassNSW := StrTran(cClassNSW, ";", "','")
				Endif
				Exit
			Endif
		Next
		If !Empty(cClassNSW)
			cQuery := "@#NSW->NSW_CCLASS IN ('" + cClassNSW + "')"
		EndIf
	Else
		If oJson == Nil .And. !Empty(FwFldGet('NSZ_CCLASS')) 
			cQuery := "@#NSW->NSW_CCLASS == '" + FwFldGet('NSZ_CCLASS') + "'@#" 
		ElseIf !Empty(oJson["NSZ_CCLASS"])
			cQuery := " NSW_CCLASS = '" + oJson["NSZ_CCLASS"] + "'"
		Endif
	Endif
