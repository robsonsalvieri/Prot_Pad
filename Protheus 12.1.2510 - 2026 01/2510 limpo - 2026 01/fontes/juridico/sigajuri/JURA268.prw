...
#DEFINE CURLINICIAISDTA "http://juridico.totvsbpo.com.br:8082"
...
@param  cEncode  - Codificação do arquivo TXT que será gerado (UTF-8, ISO-8859-1, etc)
@param  lBrkLine - Indica se deve adicionar uma quebra de linha após cada linha do TXT
...
/*/
//-------------------------------------------------------------------
Function J268PdfTxt(cDirPdf, cEncode, lBrkLine)
Local cMarca     := GetMark()
Local cParams    := "-raw -nopgbrk -enc UTF-8" //Parâmetros para o pdftotext
Local cNomeTxt   := "inicial_" + Lower(cMarca) + JurTimeStamp(1) + ".txt"
Local cSpool     := JRepDirSO("\spool\")
Local cFile      := ""
Local aLinhas    := {}
Local nI         := 0
...
Local oTxt       := Nil
Local cArquivo   := ""
Local cPdfTxtNam := "pdftotext"

Default cEncode  := ""
Default lBrkLine := .F.

	If (!Empty(cEncode)) // UTF-8 é o padrão do pdftotext
		cParams += "-raw -enc " + cEncode //Adiciona o parâmetro de codificação se for informado
	EndIf
	...
	If "Windows" $ GetSrvInfo()[2]
		cPdfTxtNam := "pdftotext.exe"
	EndIf

	cDirPdf := cRootPath + cSpool + cArquivo
	cCmd    := cPdfTxtNam + ' ' + cParams + ' "'+cDirPdf+'" ' + cNomeTxt
			...
			If (oTxt:Open())
				If oTxt:hasLine()
					...
					For nI := 1 To Len(aLinhas)
						...
						If lBrkLine
							cInicTxt += CRLF //Adiciona uma quebra de linha se lBrkLine for verdadeiro
						EndIf
					Next
					oTxt:Close()
				Else
					...
					JurConout(STR0002 + STR0009 + cFile) //"O arquivo foi gerado, porém seu conteúdo é inadequado para leitura" //"Caminho do arquivo:"
				Endif
			Endif
		Endif
	Else
		...
		JurConout(STR0005 + STR0010 + cCmd) //"Erro ao executar a função WaitRunSrv" //"Comando executado:"
	Endif
...
Default lTeste  := .F.
	...
	Else
		JurConout(STR0008 + oRest:GetLastError()) //"Retorno da requisição"
		lRet := .F.
	Endif
...
Default cNZ2Cod := ""
	...
	If Len(cDOC) == 11
		...
		If !lRet
			...
			If NZ2->(dbSeek(xFilial("NZ2") + cDOC)) //Se encontrar, usa o registro para adicionar o envolvido na NT9
	...
	ElseIf Len(cDOC) == 14 //Quando for CNPJ
		...
		If SA1->(dbSeek(xFilial("SA1") + cDOC)) //Se encontrar, usa o registro para adicionar o envolvido na NT9
...
@Return cMsg    - Mensagem de aviso caso não encontre natureza judicial que valida CNJ
...
Default cNumPro := ""
	...
	If NQ1->(dbSeek(xFilial("NQ1") + "Judicial")) .and. NQ1->NQ1_TIPO == '1' .and. NQ1->NQ1_VALCNJ == '1' .and. JGetParTpa(cTipoAss,"MV_JNUMCNJ","2") == "1"
		cNatur := NQ1->NQ1_COD
...
@param	aRetAPI = Informações que foram retornadas pela API Python
		...
		aRetAPI[4] Lista de CNPJs polo passivo
		aRetAPI[5] Número do processo
@param	lAltera = Indica se é para alterar um registro existente
...
Default lAltera  := .F.
	...
	If !lRet
		If lAltera .AND. !Empty(aRetAPI[1])
			...
			If NZ2->(DbSeek(xFilial("NZ2") + aRetAPI[1]))
				nOperation := 4
			EndIf
		EndIf
		...
		if !oModel184:SetValue("NZ2MASTER","NZ2_CGC"   , aRetAPI[1])
			oModel184:SetValue("NZ2MASTER","NZ2_CGC"   , "")
		Endif
...
@return oResponse - Objeto JSON com as informações retornadas pela API Python
...
Default lTeste   := .F.
	...
	If !Empty(cDirPdf)
		aVldIniDTA:= J268VldLDt()
		cBody := J268PdfTxt(cDirPdf) //Faz a conversão do arquivo PDF para txt e codifica em Base64

		//Faz a requisição com o TXT convertido no body
		If !Empty(cBody)
		
			If (aVldIniDTA[1]) // Se o cliente tiver acesso as iniciais via DTA
				aRetAPI := J268IniDTA(aVldIniDTA[2], cDirPdf, cBody)
			Else //Faz a requisição via API Python
...
//-------------------------------------------------------------------
/*/{Protheus.doc} J268IniDTA
Função responsável por realizar uma requisição para a API de iniciais DTA.

@param cToken    - Token de autorização para acessar a API.
@param cFileName - Nome do arquivo PDF que será enviado.
@param cFileStr  - Conteúdo do arquivo PDF convertido para texto.

@returns aRetAPI - Array com as informações retornadas pela API:
        aRetAPI[1] - CPF ou CNPJ do autor.
        aRetAPI[2] - Nome do autor.
        aRetAPI[3] - E-mail do autor.
        aRetAPI[4] - Lista de CNPJs do polo passivo.
        aRetAPI[5] - Número do processo.
        aRetAPI[6] - Valor da causa.
        aRetAPI[7] - Detalhes do processo.
        aRetAPI[8] - Dados customizados retornados pela API.
        aRetAPI[9] - Rito do processo.

@since 16/10/2025
@version 1.0
/*/
//-------------------------------------------------------------------
Function J268IniDTA(cToken, cFileName, cFileStr)
Local oBody       := JsonObject():New()
Local oRest       := Nil
Local oJsAdvProp  := Nil
Local oJson       := Nil
Local aRetAPI     := {}
Local aHeader     := {}
Local aCNPJ       := {}
...
Local lRet        := .T.
	...
	oJsAdvProp := JReadFile()

	oRest := FWRest():New('https://juridicoiniciais.azurewebsites.net')
	oRest:SetPath("/api/parseTXT")

	aHeader := {"Authorization:" + cToken, "Content-Type: application/json; charset=utf-8", "Accept-Encoding: gzip, deflate, br"}
	
	oBody['inicial'] := cFileStr
	oBody['filename'] := cFileName
	
	oBody['advProp'] := oJsAdvProp['advProp'] // Adiciona as propriedades do advogado no JSON

	If (Len(oBody['advProp']))
		For nI := 1 to Len(oBody['advProp'])
			oBody['advProp'][nI]['item'][oBody['advProp'][nI]['nome']]['description'] := JurEncUTF8(oBody['advProp'][nI]['item'][oBody['advProp'][nI]['nome']]['description'])
		Next nI
	EndIf

	//Adiciona o TXT convertido no body da requisição
	oRest:SetPostParams(oBody:toJson())

	// Exibe o JSON enviado no console
	JurConout(STR0016 + oBody:toJSON()) //"JSON enviado: "

	If oRest:Post(aHeader) //Faz a requisição para o endpoint
	
		// Exibe o JSON recebido no console
		cResult := DecodeUTF8(oRest:GetResult())
		JurConout(STR0017 + cResult) //"JSON recebido: "

		oJson:= JsonObject():new()
		oJson:fromJson(cResult)

		// Verifica se tem a propriedade "custom" no JSON retornado
		If (oJson:hasProperty("custom"))
			For nI := 1 to Len(oJson['custom'])
				oJson['custom'][nI]['valor'] := J268F3Get(oJson['custom'][nI]['campo'], oJson['custom'][nI]['valor'])
			Next nI
		Else
			oJson['custom'] := {}
		EndIf
		...
		// Area Juridica
		If (oJson:hasProperty("area_juridica"))
			aAdd(oJSon['custom'], JsonObject():New())
			aTail(oJson['custom'])['nome'] := "area_juridica"
			aTail(oJson['custom'])['campo'] := "NSZ_CAREAJ"
			
			If (Empty(oJson['area_juridica']))
				aTail(oJson['custom'])['valor'] := ""
			Else
				aTail(oJson['custom'])['valor'] := J268F3Get("NSZ_CAREAJ", DecodeUTF8(oJson['area_juridica']))
			EndIf
		EndIf

		// Tipo de Ação
		If (oJson:hasProperty("tipo_acao"))
			aAdd(oJSon['custom'], JsonObject():New())
			aTail(oJson['custom'])['nome'] := "tipo_acao"
			aTail(oJson['custom'])['campo'] := "NUQ_CTIPAC"

			If (Empty(oJson['tipo_acao']))
				aTail(oJson['custom'])['valor'] := ""
			Else
				aTail(oJson['custom'])['valor'] := J268F3Get("NUQ_CTIPAC", DecodeUTF8(oJson['tipo_acao']))
			EndIf
		EndIf		

		// Rito do Processo
		If (oJson:hasProperty("rito_processo"))
			aAdd(oJSon['custom'], JsonObject():New())
			aTail(oJson['custom'])['nome'] := "rito_processo"
			aTail(oJson['custom'])['campo'] := "NSZ_CRITO"
			aTail(oJson['custom'])['valor'] := J268F3Get("NSZ_CRITO", DecodeUTF8(SubStr(oJson['rito_processo'], At(';', oJson['rito_processo'])+1 )))
		EndIf

		// Réu do Processo
		If (oJson:hasProperty("reu_processo"))
			For nI := 1 to Len(oJson['reu_processo'])
				oJson['reu_processo'][nI]['nome'] := DecodeUTF8(oJson['reu_processo'][nI]['nome'])
				oJson['reu_processo'][nI]['CNPJ'] := oJson['reu_processo'][nI]['cpf_cnpj']
			Next nI
		EndIf

		aCustom := oJson['custom']

		// Exibe o JSON custom no console
		VarInfo(STR0019, oJson) //"oJson Final: "
	Else
		//Retorno de Erro da requisição
		JurConout(STR0018 + oRest:GetLastError()) //"POST - https://juridicoiniciais.azurewebsites.net - Retorno de Erro da Request : "
		VarInfo(STR0020, oRest:GetResult()) //"oRestError: "
		lRet := .F.
	Endif

	If lRet 
		cAutor   := DecodeUTF8(oJson['autor_processo']['nome'])
		cCpfCnpj := oJson['autor_processo']['cpf_cnpj']
		cCpfCnpj := StrTran(JurLmpCpo(cCpfCnpj,.F.),'#','')
		cEmail   := ''
		aCNPJ    := oJson['reu_processo']
		cNumPro  := oJson['numero_do_processo']
		cValor   := StrTran(cValToChar(oJson['valor_da_causa']),".",",") // Converte o valor da causa para string e troca o ponto por vírgula
		cDetalhe := oJSon['resumo_fatos'] + CRLF +;
					"-------------------" + CRLF +;
					oJSon['argumentos_autor']
		...
		aAdd(aRetAPI, cNumPro)
		aAdd(aRetAPI, cValor)
		aAdd(aRetAPI, cDetalhe)
		aAdd(aRetAPI, aCustom) // Adiciona o array custom ao retorno
	Endif
Return aRetAPI

//-------------------------------------------------------------------
/*/{Protheus.doc} J268F3Get(cFieldName, cValueDTA)
Busca o valor através da descrição que o DTA envia.

@param cFieldName = Nome do campo que será buscado
@param cValueDTA = Valor que será buscado na tabela X3_F3
@return cValueDTA = Retorna o valor encontrado na tabela X3_F3 ou vazio se não encontrar

@since 16/06/2025
@version 1.0
/*/
//-------------------------------------------------------------------
Static Function J268F3Get(cFieldName, cValueDTA)
Local cXBAlias   := getSx3Cache(cFieldName,"X3_F3")
Local cMsgFltErr := ""
Local cQuery     := ""
Local aRet       := {}

	If (!Empty(cXBAlias) .AND. !Empty(cValueDTA))
		aRet := JWsConF3Ls( cXBAlias, cValueDTA, "", "", "", @cMsgFltErr, @cQuery )
		...
		If (!Empty(cMsgFltErr))
			JurConout("[J268F3Get]" + cFieldName + ": " + cMsgFltErr)
		EndIf
	EndIf
Return cValueDTA
...
Local nI         := 0
	...
	If !Empty(aRet[1])
		...
		If SRA->(dbSeek(xFilial("SRA") + aRet[1])) //Se encontrar, usa o registro para adicionar o envolvido no JSON
			cRAMat := SRA->RA_MAT
			oNT9['NT9_ENTIDA'] := "SRA"
		...
		If !lRet
			...
			If SA1->(DbSeek(xFilial("SA1") + aRet[1])) // Se encontrar, usa o registro para adicionar o envolvido no JSON
				cA1Cod := SA1->A1_COD
				cA1Loj := SA1->A1_LOJA
				oNT9['NT9_ENTIDA'] := "SA1"
		...
		If !lRet
			...
			If NZ2->(dbSeek(xFilial("NZ2") + aRet[5])) //Se encontrar, usa o registro para adicionar o envolvido no JSON
				cNZ2Cod := NZ2->NZ2_COD
			Else
				NZ2->(DbSetOrder(2)) //NZ2_FILIAL+NZ2_NOME
				If !NZ2->(dbSeek(xFilial("NZ2") + aRet[2]))
					cNZ2Cod := J268GrvNZ2(lRet, aRet) //Caso contrário, cadastra como parte contrária
					NZ2->(DbSetOrder(1)) //NZ2_FILIAL+NZ2_COD
					NZ2->(dbSeek(xFilial("NZ2") + cNZ2Cod))
				Else
					cNZ2Cod := NZ2->NZ2_COD
				EndIf
			EndIf
	...
	If Len(aRet[4]) > 0  //Se houver dados no array de CNPJ(aRet[4])
		For nI := 1 To Len(aRet[4])
			...
			If !Empty(cCNPJ)
				...
				If SA1->(dbSeek(xFilial("SA1") + cCNPJ)) //Se encontrar, usa o registro para adicionar o envolvido no JSON
					cA1Cod := SA1->A1_COD
					cA1Loj := SA1->A1_LOJA
					oNT9['NT9_ENTIDA'] := "SA1"
					oNT9['NT9_CODENT'] := cA1Cod + cA1Loj
				...
				Else
					If VALTYPE(aRet[4][nI]) == "J" .AND. VALTYPE(aRet[4][nI]['CNPJ']) == "C" .AND. !Empty(aRet[4][nI]['CNPJ'])
						...
						If !Empty(cNZ2Cod)
							...
							If NZ2->(DbSeek(xFilial("NZ2") + cNZ2Cod))
								oNT9['NT9_ENTIDA'] := "NZ2"
								oNT9['NT9_CODENT'] := cNZ2Cod
								oNT9['NT9_NOMEEN'] := JurEncUTF8(AllTrim(NZ2->NZ2_NOME))
								oNT9['NT9_DOCENV'] := AllTrim(NZ2->NZ2_CGC)
	...
	If (Len(aRet) > 6) .AND. !Empty(aRet[7])
		oResponse['data'][1]['detalhe'] := aRet[7] // Adiciona o detalhe do processo
		oResponse['data'][1]['custom'] := aRet[8] // Adiciona o array custom ao JSON
	EndIf
...
//-------------------------------------------------------------------
/*/{Protheus.doc} J268VldLDt
Valida se o cliente pode utilizar a API de iniciais com DTA.

@return   lRet = Retorna se o cliente pode utilizar a API de iniciais com DTA
		 .lRet = .T. - Cliente pode utilizar a API de iniciais com DTA
		 .lRet = .F. - Cliente utiliza a leitura de iniciais padrão.
		cToken = Token de acesso para a API de iniciais com DTA
...
Local oWsdl   := Nil
Local cBody   := ""
Local cIndUsr := SuperGetMV("MV_JINDUSR",, '')
Local cIndPsw := SuperGetMV("MV_JINDPSW",, '')
Local cToken  := ""
Local nCount  := 0
Local nIndex  := 0
Local lRet    := .F.
	...
	xRet := oWsdl:ParseURL(CURLINICIAISDTA + "/WSLEGALDATAIA.apw?wsdl")
	xRet := oWsdl:SetOperation("MTRECURSOIA")
	cBody := '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:jur="' + CURLINICIAISDTA + '/">'
	cBody += '<soapenv:Header/>'
	cBody += 	'<soapenv:Body>'
	cBody += 		'<jur:MTRECURSOIA>'
	cBody += 			'<jur:USUARIO>' + cIndUsr + '</jur:USUARIO>'
	cBody += 			'<jur:SENHA>' + cIndPsw + '</jur:SENHA>'
	cBody += 			'<jur:RECURSO>iniciais</jur:RECURSO>'
	cBody += 		'</jur:MTRECURSOIA>'
	cBody += 	'</soapenv:Body>'
	cBody += '</soapenv:Envelope>'
	
	xRet := oWsdl:SendSoapMsg( cBody )
	...
	If (oXML:Parse( xRet ))
		oXML:DOMChildNode()
		While(oXml:cName != "MTRECURSOIARESULT" .And. nCount < 10)
			oXML:DOMChildNode()
			nCount++ // Usado para evitar loop infinito
		EndDo
		
		nIndex := aScan(oXML:DOMGetChildArray(), {|x| x[1] == "HASFEATURE" })
		If (nIndex > 0)
			lRet := oXML:DOMGetChildArray()[nIndex][2] == "true"
			If (lRet)
				nIndex := aScan(oXML:DOMGetChildArray(), {|x| x[1] == "CTOKEN" })
				cToken := oXML:DOMGetChildArray()[nIndex][2]
			EndIf
		EndIf
	EndIF
Return {lRet, cToken}
...
//-------------------------------------------------------------------
/*/{Protheus.doc} JReadFile
Função responsável por realizar a leitura de um arquivo JSON.

@return oJson - Objeto JSON contendo as propriedades lidas do arquivo.
				Caso o arquivo não seja encontrado ou esteja vazio, retorna
				um JSON com a propriedade `advProp` como um array vazio.

@since 16/10/2025
@version 1.0
/*/
//-------------------------------------------------------------------
Static Function JReadFile()
Local nHandle      := 0
Local cBuffer      := ""
Local nBytes       := 0
Local nBloco       := 1000
...
Local cFilecontent := ""
Local cFileName    := "advProp.txt"

	JurConout(STR0021) //"Iniciando a Leitura do Arquivo"
	VarInfo("JReadFile: ",File(cFileName)) //Verifica se o arquivo existe
	If File(cFileName)
		nHandle := fopen(cFileName , FO_READWRITE + FO_SHARED )
		While (nBytes := FREAD(nHandle, @cBuffer, nBloco)) > 0 // Lê os bytes
			cBuffer := cBuffer
			cFilecontent += cBuffer 
			cBuffer := ''
		End

		If Len(cFilecontent) == 0
			JurConout(STR0022 + cFileName) //"Arquivo vazio: "
			fclose(nHandle) //Fecha o arquivo
			oJson['advProp'] := {} //Converte o objeto JSON em array
		Else
			//Codifica o conteúdo do arquivo em UTF-8
			cFilecontent := DecodeUTF8(cFilecontent)

			//Exibe o conteúdo do arquivo no console
			JurConout(STR0023 + cFilecontent) //"Conteudo do arquivo: "

			oJson:fromJson(cFilecontent) //Transforma o conteúdo do arquivo em objeto JSON
			fclose(nHandle) //Fecha o arquivo
		EndIf
		JurConout(STR0024) //"Arquivo lido com sucesso"
	Else
		JurConout(STR0025 + cFileName) //"Arquivo não encontrado: "
		oJson['advProp'] := {} //Converte o objeto JSON em array
	EndIf
Return oJson
