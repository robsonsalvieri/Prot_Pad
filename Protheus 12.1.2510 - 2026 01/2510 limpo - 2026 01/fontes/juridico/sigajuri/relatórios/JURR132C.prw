...
//-------------------------------------------------------------------
/*/{Protheus.doc} JURR132C(cUser, cThread, lAndam)
...
JRelatorio(aRelat,aCabec,aSessao,J132CQrPrin(cNome),cNome) //Chamada da função de impressão do relatório em TMSPrinter

Return Nil

//-------------------------------------------------------------------
/*/{Protheus.doc} J132CQrPrin(cUser, cThread)
...
cQuery +=  " LEFT OUTER JOIN " + RetSqlName("NQB") + " NQB " 
cQuery +=   " ON ( NQB.NQB_FILIAL = '" + xFilial("NQB") + "'  "
cQuery +=   " AND  NWA.D_E_L_E_T_ = NQB.D_E_L_E_T_ "
cQuery +=   " AND  NWA.NWA_CESPEC = NQB.NQB_COD ) "
cQuery +=  " WHERE SU5.U5_FILIAL = '" + xFilial("SU5") + "' AND SA2.A2_COD || SA2.A2_LOJA = AC8.AC8_CODENT AND SU5.D_E_L_E_T_ = ' ' AND AC8.AC8_ENTIDA ='SA2'"
cQuery +=     " AND (SU5.U5_CODCONT = '" + cNome + "' OR UPPER(SU5.U5_CONTAT) LIKE UPPER('%" + Alltrim(cNome) + "%')) "
...
/*/
//-------------------------------------------------------------------
Static Function JRelatorio(aRelat,aCabec,aSessao,cQuery,cNome)
...
Local oPrint    := Nil
Local aDados    := {}
Local TMPC      := GetNextAlias()


oPrint := FWMsPrinter():New( cNomeRel, IMP_PDF,,,.T. ) // Inicia o relatório

DbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),TMPC,.T.,.T.)

If (TMPC)->(!EOF())

	ImpCabec(@oPrint, @nLin, aRelat, aCabec,cNome) // Imprime cabeçalho
	...
	While (TMPC)->(!EOF())
		...
		If nLin >= nFimL // Verifica se a linha corrente é maior que linha final permitida por página
			...
			ImpCabec(@oPrint, @nLin, aRelat, aCabec,cNome) // Cria um novo cabeçalho
			nLinCalc := nLin // Inicia o controle das linhas impressas
		...
		For nI := 1 To Len(aSessao) // Inicia a impressão de cada sessão do relatório
			...
			For nJ := 6 to Len(aSessao[nI]) // Lê as informações de cada campo a ser impresso. O contador começa em 6 pois é a partir dessa posição que estão as informações sobre o campo
				...
				cValor := JTrataVal(cTabela,cCpoTab,cCpoQry,cTipo,TMPC,,.F.) // Retorna o conteúdo/valor a ser impresso. Chama essa função para tratar o valor caso seja um memo ou data
				
				If cCpoTab == "U5_CONTAT"
					If cNomeAnt == cValor
						cValor := ""
					Else
						cNomeAnt := cValor // Grava o valor anterior para tratamento para não repetir o contato várias vezes
					EndIf
				ElseIf cCpoTab == "U5_CODCONT"
					If cCodAnt == cValor
						cValor := ""
					Else
						cCodAnt := cValor // Grava o valor anterior para tratamento para não repetir o contato várias vezes
					EndIf
				EndIf
			...
			JImpRel(aDados,@nLin,@nLinCalc,@oPrint, @nLinFinal,lHori, @lTitulo, @lLinTit, aRelat,aCabec,,cNome, lImpTit)
			If lImpTit
				lImpTit := .F.
			EndIf
...
/*/
//-------------------------------------------------------------------
Static Function ImpCabec(oPrint, nLin, aRelat, aCabec, cNome)
...
oPrint:Say( nLin, 65 , STR0009 + ":", oFontTitFil) //"Advogado Correspondente"
oPrint:Say( nLin, 65 + nTamCarac * 24, cNome, oFontDescFil)
...
//-------------------------------------------------------------------
/*/{Protheus.doc} JTrataVal(cTabela,cCpoTab,cCpoQry,cTipo,TMPC,SUB,lSub)
...
@param TMPC     Alias aberto da query principal
...
/*/
//-------------------------------------------------------------------
Static Function JTrataVal(cTabela,cCpoTab,cCpoQry,cTipo,TMPC,SUB,lSub)
Local cValor := ""
...
Else 
	If cTipo == "D" // Tipo do campo
		TCSetField(TMPC, cCpoQry 	, "D") //Muda o tipo do campo para data.
		cValor   := AllTrim(AllToChar((TMPC)->&(cCpoQry))) //Conteúdo a ser gravado
	ElseIf cTipo == "M"
		DbSelectArea(cTabela)
		(cTabela)->(dbGoTo((TMPC)->&(cCpoQry))) // Esse seek é para retornar o valor de um campo MEMO
		cValor := AllTrim(AllToChar((cTabela)->&(cCpoTab) )) //Retorna o valor do campo
	Else
		cValor := AllTrim(AllToChar((TMPC)->&(cCpoQry)))
	EndIf
EndIf
...
//-------------------------------------------------------------------
/*/{Protheus.doc} JImpRel(aDados, nLin, nLinCalc, oPrint, nLinFinal, lHori, lTitulo, lLinTit, aRelat, aCabec, lSalta, cNome, lImpTit
...
/*/
//-------------------------------------------------------------------
Static Function JImpRel(aDados,nLin,nLinCalc,oPrint,nLinFinal,lHori, lTitulo, lLinTit, aRelat,aCabec, lSalta, cNome, lImpTit)
Local nJ
Local lQuebLin  := .F.
//Local lImpTit   := .T.
Local cTitulo   := ""
...
For nJ := 1 to Len(aDados)
	...
	If lSalta .And. lQuebLin // Se precisa continuar a impressão do conteúdo atual na próxima página 
		...
		JImpRel(aSobra,@nLin,@nLinCalc,@oPrint, @nLinFinal,lHori, @lTitulo, @lLinTit, aRelat,aCabec, @lSalta, cNome, .T.)
		aEval(aSobra,{|x| x[4] := ""})
	EndIf
