...
Default cLstCajuri := ""
	...
	cDesc := Substring(Replace(cDesc,' ',''),0,2000)
	...
	cQuery += " AND SUBSTRING(REPLACE(NT4_DESC,' ',''),0,2000) LIKE '%" + cDesc + "%'"
...
@param  cNumPro  - Número do processo
@return aCajuVld - Lista de processos
...
Local nCont    := 0
	...
	// Não incluir cajuris repetidos por conta de instâncias com mesmo número de processo
	For nCont := 1 To Len(aRetorno)
		If aScan(aCajuVld, {|x| x[1] == aRetorno[nCont][1]}) == 0
			Aadd(aCajuVld, aRetorno[nCont])
		EndIf
	Next nCont
...
//-------------------------------------------------------------------
/*/{Protheus.doc} J233AndLogs(aAndamentos)
Registra logs sobre o processamento dos andamentos automáticos, caso a tag TRACE esteja ativada.

@param aAndamentos - Array com dados dos andamentos por processo
		aAndamentos[1][1]       - Array de cajuris encontrados
		aAndamentos[1][1][n][1] - Cajuri do processo
		aAndamentos[1][1][n][2] - Tipo de assunto jurídico do processo
		aAndamentos[1][1][n][3] - Recno da Instância (NUQ) do processo
		aAndamentos[1][1][n][4] - Cód. Natureza da Instância (NUQ)
		aAndamentos[1][2]       - Array com andamentos do processo
		aAndamentos[1][2][n][1] - Data do andamento
		aAndamentos[1][2][n][2] - Descrição do andamento
		aAndamentos[1][3]       - Número do processo

@since  15/12/2025
/*/
//-------------------------------------------------------------------
Static Function J233AndLogs(aAndamentos)
Local lTraceLog   := GetPvProfString(GetEnvServer(), "Trace", "0", GetADV97()) == "1"
Local nLog        := 0
Local nLogCajuri  := 0
Local cCajuris    := ""
...
Default aAndamentos := {}
	...
	If lTraceLog
		For nLog := 1 To Len(aAndamentos)
			cCajuris := ""
			For nLogCajuri := 1 To Len(aAndamentos[nLog][1])
				cCajuris += aAndamentos[nLog][1][nLogCajuri][1] + ", "
			Next nLogCajuri
			cCajuris := Left(cCajuris, Len(cCajuris)-2)
			JurConOut(STR0073 + I18n(JConvUTF8(STR0074), {cCajuris}))                                 // STR0073 - "Aviso: " STR0074 - "Processos considerados: #1 "
			JurConOut(STR0073 + I18n(JConvUTF8(STR0075), {AllTrim(Str(Len(aAndamentos[nLog][2])))}))  // STR0073 - "Aviso: " STR0075 - "Andamentos a serem processados: #1 "
			JurConOut(STR0073 + I18n(JConvUTF8(STR0076), {aAndamentos[nLog][3]}))                     // STR0073 - "Aviso: " STR0076 - "Número do processo: #1 "
		Next nLog
	EndIf
...
//-------------------------------------------------------------------
/*/{Protheus.doc} J223LogO1H(cCajuri, cNumProc, cCodInst)
Consulta os logs da O1H sobre os processamentos da fila do LegalData.

@param cCajuri  - Código do Assunto Jurídico
@param cNumProc - Número do Processo
@param cCodInst - Código da Instância

@return oResponse - Objeto JSON com os dados da consulta

@since  16/12/2025
/*/
//-------------------------------------------------------------------
Function J223LogO1H(cCajuri, cNumProc, cCodInst)
...
Default cCodInst := ""
	...
	If Empty(cCajuri) .And. Empty(cNumProc) .And. Empty(cCodInst)
		oResponse["filaLegalData"] := JConvUTF8(STR0078) // "Por favor, insira ao menos um filtro para realizar a pesquisa."
	EndIf

	If Empty(oResponse["filaLegalData"])
		cQuery += " SELECT O1H.O1H_COD,"
		cQuery +=        " O1H.O1H_CAJURI,"
		cQuery +=        " O1H.O1H_CINSTA,"
		cQuery +=        " O1H.O1H_STATUS,"
		cQuery +=        " O1H.O1H_OPERAC,"
		cQuery +=        " O1H.O1H_RETORN,"
		cQuery +=        " O1H.O1H_DTINCL,"
		cQuery +=        " O1H.O1H_HRINCL,"
		cQuery +=        " O1H.O1H_DTPROC,"
		cQuery +=        " O1H.O1H_HRPROC,"
		cQuery +=        " NUQ.NUQ_NUMPRO"
		cQuery +=   " FROM " + RetSqlName("O1H") + " O1H"
		cQuery +=   " LEFT JOIN " + RetSqlName("NUQ") + " NUQ"
		cQuery +=     " ON NUQ.NUQ_CAJURI = O1H.O1H_CAJURI"
		cQuery +=    " AND NUQ.NUQ_COD = O1H.O1H_CINSTA"
		cQuery +=    " AND NUQ.NUQ_FILIAL = '?'"
		aAdd(aParams, {"U", xFilial("NUQ")})
		cQuery +=    " AND NUQ.D_E_L_E_T_ = ' '"
		cQuery +=  " WHERE O1H.D_E_L_E_T_ = ' '"

		If !Empty(cCajuri)
			cQuery += " AND O1H.O1H_CAJURI = ?"
			aAdd(aParams, {"C", cCajuri})
		EndIf

		If !Empty(cCodInst)
			cQuery += " AND O1H.O1H_CINSTA = ?"
			aAdd(aParams, {"C", cCodInst})
		EndIf

		If !Empty(cNumProc)
			cQuery += " AND NUQ.NUQ_NUMPRO = ?"
			aAdd(aParams, {"C", cNumProc})
		EndIf

		cQuery += " ORDER BY O1H.O1H_HRINCL, O1H.O1H_DTINCL"
		...
		oResponse["filaLegalData"] := {}

		While (cAlias)->(!Eof())
			aAdd(oResponse['filaLegalData'], JsonObject():New())
			aTail(oResponse['filaLegalData'])['codigoFila']            := (cAlias)->O1H_COD
			aTail(oResponse['filaLegalData'])['codigoAssuntoJuridico'] := (cAlias)->O1H_CAJURI
			aTail(oResponse['filaLegalData'])['codigoInstancia']       := (cAlias)->O1H_CINSTA
			aTail(oResponse['filaLegalData'])['numeroProcesso']        := AllTrim((cAlias)->NUQ_NUMPRO)
			aTail(oResponse['filaLegalData'])['dataInclusao']          := (cAlias)->O1H_DTINCL
			aTail(oResponse['filaLegalData'])['horaInclusao']          := (cAlias)->O1H_HRINCL
			aTail(oResponse['filaLegalData'])['retornoServico']        := AllTrim((cAlias)->O1H_RETORN)
			aTail(oResponse['filaLegalData'])['dataProcessamento']     := (cAlias)->O1H_DTPROC
			aTail(oResponse['filaLegalData'])['horaProcessamento']     := (cAlias)->O1H_HRPROC
			aTail(oResponse['filaLegalData'])['operacaoPublicacao']    := JConvUTF8(JTrataCbox("O1H_OPERAC", (cAlias)->O1H_OPERAC))
			aTail(oResponse['filaLegalData'])['statusProcessamento']   := JConvUTF8(JTrataCbox("O1H_STATUS", (cAlias)->O1H_STATUS))

			(cAlias)->(dbSkip())
		EndDo

		If Empty(oResponse['filaLegalData'])
			oResponse['filaLegalData'] := JConvUTF8(STR0077) // "Não foram encontrados registros para os filtros informados."
		EndIf
