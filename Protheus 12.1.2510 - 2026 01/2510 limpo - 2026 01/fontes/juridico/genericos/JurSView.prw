...
//-------------------------------------------------------------------
/*/{Protheus.doc} JurSVCall
Função para a chamada do SV utilizando parâmetros

@param cLayout , Nome do Design ou da Visão de Dados que será executada
@param cTipo   , Tipo do layout (report, pivot-table ou data-grid)
@param aParams , Array com o parâmetro e conteúdo
				 Nome parametro, Conteúdo
				{{"MV_PAR01"   , "1"     },
				 {"MV_PAR02"   , 2       }}
@param cResult , 1 - Impressora / 2 - Tela / 3 - Salva em disco
@param cNomeArq, Nome do arquivo
@param cExten  , Tipo de extensão para gerar o arquivo 
@param cPasta  , Destino para salvar o relatório

@Return lSuccess, Sucesso ao executar o layout do SV

@author  Jacques Alves Xavier
@since   28/10/2025
/*/
//-------------------------------------------------------------------
Function JurSVCall(cLayout, cTipo, aParams, cResult, cNomeArq, cExten, cPasta)
Local lSuccess   As logical
Local oSmartView As object
Local nI         As Number
Local jPrint     As Json
 
	oSmartView := totvs.framework.smartview.CallSmartView():new(cLayout, cTipo)

	For nI := 1 To Len(aParams)
		oSmartView:SetParam(aParams[nI][1], aParams[nI][2])
	Next nI
	...
	oSmartView:setShowParams(.F.)

	If cTipo == "report"
		oSmartView:setPrintType(1)

		jPrint := jsonObject():new()
		jPrint["extension"] := cExten
		jPrint["name"] := cNomeArq
