		...
		If lRet
			...
			For nX := 1 to oGrid:GetQtdLine()//Validação geral da grid
				If lRet  .And. !oGrid:IsDeleted(nX) .And. !oGrid:IsEmpty(nX)
					...
					// Verifica se existe ao menos uma linha com ano-mês inicial "1900-01" para a entidade, NUU, histórico do Caso
					If lCondAdic .And. !Empty(cVlAMIni) .And. !lDtini .And. (cAlias == "NUU" .And. cVlAMIni == cAMIniPad)
						lDtini := .T.
					EndIf
			...
			// Caso não exista pelo menos uma linha com ano-mês inicial "1900-01" retorna .F.
			If lRet .And. !lIsRest .And. (!lDtini .And. cAlias == "NUU")
				lRet 	 := .F.
				cMsg 	 := STR0344 //"É obrigatório incluir o registro inicial com ano-mês '1900-01' antes de salvar!"
				cSolucao := STR0345 //"o histórico precisa de um registro com o ano-mês inicial '1900-01'"
			EndIf
...
Function JCasoAtual(cCaso)
	...
	ElseIf Len(aCasos) > 1
		...
		If Empty(aRet)
			cQuery1 := "SELECT A.NY1_CCLIEN, A.NY1_CLOJA, A.NY1_SEQ"
			cQuery1 +=  " FROM ("
			...
			cQuery1 +=       " ) A"
			cQuery1 += " ORDER BY A.NY1_SEQ DESC"
...
If !Empty(cFatura) ;
	...
	nGrossUp := nVlDpTrib / (1 - nGrossUp)

	aVlTaxas[1] := nGrossUp - nVlDpTrib
...
Local oQuery  := Nil
    ...
    cQuery +=     "ON FKK.FKK_FILIAL = FKN.FKN_FILIAL "
    cQuery +=    "AND FKK.FKK_CODFKN = FKN.FKN_CODIGO "
    cQuery +=    "AND FKK.D_E_L_E_T_ = ? " // #1
    cQuery +=  "WHERE FKK.FKK_IDRET = ? "  // #2
    cQuery +=    "AND FKN.FKN_FILIAL = ? " // #3
    cQuery +=    "AND FKN.D_E_L_E_T_ = ? " // #4

 	aAdd(aParams, {"C", " "})
    aAdd(aParams, {"C", cCodFKN})
    aAdd(aParams, {"C", xFilial("FKN")})
 	aAdd(aParams, {"C", " "})
