...
@param  nQtdParc , numerico, Quantidade de parcelas dos títulos
@param  nTitGer  , numerico, Nº da parcela posicionada
...
/*/
//-------------------------------------------------------------------
Function JIncTitCR( nSE1Recno, dEmissao, nQtdParc, nTitGer)
Local aArea       := GetArea()
Local lRet        := .T.
Local lExistOHT   := AliasInDic("OHT")
Local lIncTitLiq  := .F.
...
Default nQtdParc  := 1 // Este parâmetro só é utilizado quando é feito a liquidação/reliquidação e para esse cenário já está sendo passado o valor correto.
Default nTitGer   := IIf(FindFunction("JParcSE1"), JParcSE1(nSE1Recno), 1) // Este parâmetro é utilizado na geração dos registros da OIB.
...
Else
	...
	If !lExistOHT .Or. !lIncTitLiq
		If FindFunction("J255APosHis")
			J255APosHis(nSE1Recno, dEmissao, /*lExecLote*/, /*aOHIBxAnt*/, /*lGrParcAnt*/, /*nSaldoRet*/, /*lSincLG*/, /*dDataBx*/, nQtdParc, nTitGer) // Inclui posição histórica do contas a receber.
		EndIf
...
If Len(aFaturas) > 0
	...
	For nTitGer := 1 To Len(aTitGer) // Inclusão da OHH para o(s) título(s) novo(s)
		JIncTitCR(aTitGer[nTitGer][9], dDatabase, Len(aTitGer), nTitGer)
	Next
EndIf
...
//-------------------------------------------------------------------
/*/{Protheus.doc} JurVldSimb
Validação do simbolo da moeda, caso exista uma moeda ja cadastrada com o mesmo simbolo.

@return lRet

@author Guilherme Augusto Vieira
@since  21/10/2025
@obs    Função chamada nos fontes CTBa140 (VldSimbCto/Ctb140Vld())
/*/
//-------------------------------------------------------------------
Function JurVldSimb()
Local lRet    := .T.
...
Local lFSinc  := SuperGetMV("MV_JFSINC",.F.,'2') == "1"
	...
	If !Empty(M->CTO_SIMB) .And. lIntPFS .And. lFSinc //Valida parâmetros MV_JURXFIN e MV_JFSINC

		cSQL := "SELECT CTO_MOEDA"
		cSQL +=  " FROM " + RetSQLName("CTO")
		cSQL += " WHERE CTO_FILIAL = '" + xFilial("CTO") + "'"
		cSQL +=   " AND CTO_SIMB = '" + M->CTO_SIMB + "'"
		cSQL +=   " AND D_E_L_E_T_ = ' '"

		dbUseArea(.T., "TopConn", TCGenQry(, , cSQL), cAlias, .F., .F.)

		If !(cAlias)->(Eof())
			JurMsgErro(STR0183, , STR0184) //STR0183 - "Já existe uma moeda cadastrada com este símbolo!" / STR0184 - "Utilize um símbolo de moeda diferente dos já cadastrados."
			lRet := .F.
		EndIf
...
//-------------------------------------------------------------------
/*/{Protheus.doc} JBasImpLiq
Função chamada no momento da geração do título de liquidação
para informar a base de cálculo dos impostos quando utilizado
impostos pelo configurador de tributos.

@param  aTitLiq , Array com os dados do título que será gerado na liquidação
                  (Utilizado na chamada da ExecAuto FINA040)
@param  aBaseImp, Base de cálculo dos impostos dos títulos liquidados
@param  nBasePis, Base de cálculo do PIS calculado com base nos títulos liquidados
@param  nBaseCof, Base de cálculo do COFINS calculado com base nos títulos liquidados
@param  nBaseCsl, Base de cálculo do CSLL calculado com base nos títulos liquidados
@param  nParcela, Número da parcela que está sendo processada.
@param  nTotLiq , Valor total da liquidação (somatória das parcelas)
@param  oMdlFO0 , Submodelo do cabeçalho da liquidação
@param  oMdlFO2 , Submodelo dos títulos gerados pela liquidação

@author Abner Fogaça
@since  15/10/25
@Obs    Chamada no fonte FINA460a na função F460ACommit
/*/
//-------------------------------------------------------------------
Function JBasImpLiq(aTitLiq, aBaseImp, nBasePis, nBaseCof, nBaseCsl, nParcela, nTotLiq, oMdlFO0, oMdlFO2)
Local aBaseImpCFG := {}
Local aImpCFG     := {}
Local nVlrBaseImp := 0
Local aImpPrd     := {"IRF", "PIS", "COF", "CSL", "INSS", "ISS"} // Impostos padrões
Local nLoop       := 0
Local nPosImp     := 0
Local nPropImp    := oMdlFO2:GetValue("FO2_TOTAL") / nTotLiq
Local lImpParc1	  := SuperGetMv("MV_RTIPFIN",.F.,"F")
	
	For nLoop := 1 To Len(aBaseImp)
		If aBaseImp[nLoop] > 0
			nVlrBaseImp := aBaseImp[nLoop]
			Exit
		EndIf
	Next nLoop
	
	aImpCFG := FINCalImp("2",  oMdlFO0:GetValue("FO0_NATURE"), SE1->E1_CLIENTE, SE1->E1_LOJA, SE1->E1_FILIAL, nVlrBaseImp, dDataBase, .F., {}, AvKey("FT", "E1_TIPO"),,, {})
	
	If Len(aImpCFG) > 0
		For nLoop := 1 To Len(aImpCFG)
			cCodImp  := Alltrim(aImpCFG[nLoop][8])
			nPosImp  := AScan(aImpPrd, cCodImp)
			
			If nPosImp > 0
				If lImpParc1
					If nParcela == 1
						If cCodImp == "IRF"
							aAdd(aBaseImpCFG, {"IRF" , aBaseImp[4]})
						ElseIf cCodImp == "INSS"
							aAdd(aBaseImpCFG, {"INSS", aBaseImp[5]})
						ElseIf cCodImp == "ISS"
							aAdd(aBaseImpCFG, {"ISS" , aBaseImp[6]})
						EndIf
					Else
						If cCodImp == "IRF"
							aAdd(aBaseImpCFG, {"IRF" , 0})
						ElseIf cCodImp == "INSS"
							aAdd(aBaseImpCFG, {"INSS", 0})
						ElseIf cCodImp == "ISS"
							aAdd(aBaseImpCFG, {"ISS" , 0})
						EndIf
					EndIf
				Else
					If cCodImp == "IRF"
						aAdd(aBaseImpCFG, {"IRF" , aBaseImp[4] * nPropImp})
					ElseIf cCodImp == "INSS"
						aAdd(aBaseImpCFG, {"INSS", aBaseImp[5] * nPropImp})
					ElseIf cCodImp == "ISS"
						aAdd(aBaseImpCFG, {"ISS" , aBaseImp[6] * nPropImp})
					EndIf
				EndIf

			
				If cCodImp == "PIS"
					aAdd(aBaseImpCFG, {"PIS" , nBasePis})
				ElseIf cCodImp == "COF"
					aAdd(aBaseImpCFG, {"COF" , nBaseCof})
				ElseIf cCodImp == "CSL"
					aAdd(aBaseImpCFG, {"CSL" , nBaseCsl})
				EndIf
			EndIf
		Next nLoop
		If Len(aBaseImpCFG) > 0
			aAdd(aTitLiq, {'AUTBASEIMP', aBaseImpCFG, Nil})
		EndIf
	EndIf
...
//-------------------------------------------------------------------
/*/{Protheus.doc} JParcSE1
Retorna qual é a parcela do título que está sendo gerado.

@param nRecnoSE1  Recno do título que está sendo gerado.

@obs: Essa função está sendo chamado nas funções JIncTitCR e
      J255APosHis
...
Local oQuery    := Nil
	...
	While !(cAliasQry)->(EOF())
		nQtdParc ++
		If SE1->E1_PARCELA == (cAliasQry)->E1_PARCELA .And. nParcela == 0
			nParcela := nQtdParc
		EndIf
		(cAliasQry)->(DbSkip())
	EndDo
...
//-------------------------------------------------------------------
/*/{Protheus.doc} JurTitPFS
Retorna Título, conforme Documento Fiscal vinculado à Fatura.

@param cDoc   Documento Fiscal vinculado à Fatura.
@param cSerie Série do Documento Fiscal vinculado à Fatura.

@return cTitulo Título da Fatura vinculada ao Documento Fiscal.

@obs: Função chamada pela filtrarnf() do programa nfseXmlNac

@author Reginaldo.Borges
@since  22/12/2025
/*/
//-------------------------------------------------------------------
Function JurTitPFS(cDoc, cSerie, cNatPFS)
...
Local oQuery    := Nil
	...
	cQuery := " SELECT E1_NUM, E1_NATUREZ "
	cQuery +=   " FROM " + RetSqlName("NXA") + " NXA"
	cQuery +=  " INNER JOIN " + RetSqlName("OHT") + " OHT "
	cQuery +=     " ON OHT.OHT_FILIAL = ? "
	aAdd(aParams, {"C", xFilial("OHT")})
	cQuery +=    " AND OHT.OHT_CFATUR = NXA.NXA_COD "
	cQuery +=    " AND OHT.OHT_FTESCR = NXA.NXA_CESCR "
	cQuery +=    " AND OHT.D_E_L_E_T_ = ? "
	aAdd(aParams, {"C", " "})
	cQuery +=  " INNER JOIN " + RetSqlName("SE1") + " SE1 "
	cQuery +=     " ON SE1.E1_FILIAL = OHT.OHT_FILTIT "
