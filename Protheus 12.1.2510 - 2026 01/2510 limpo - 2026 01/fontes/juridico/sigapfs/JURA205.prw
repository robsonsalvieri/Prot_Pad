...
If lFirst
	If (!Empty(cSerie) .And. Len(FWGetSX5("01", cSerie)) > 0) .Or. (!FWIsInCallStack("JA203HFatu") .And. Sx5NumNota(@cSerie, cTpNrFfs,,,,,,, @cTypeDoc)) //Valida a serie do parâmetro ou solicita apresenta tela para seleção
		cSerieNF := cSerie
	Else
		...
		For nI := 1 To Len( aStruSD2 )
			...
			If cPaisLoc == "BRA"
				...
				Case cCampo == 'D2_ALIQISS'
					...
					If nAliqImp == 0
						nAliqImp := IIf(SB1->B1_ALIQISS <= 0, nAliqISS, SB1->B1_ALIQISS)
					EndIf
					aAdd(aSD2, nAliqImp)
