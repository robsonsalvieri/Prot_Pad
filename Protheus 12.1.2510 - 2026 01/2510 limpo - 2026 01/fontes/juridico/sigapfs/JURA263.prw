...
Local cDestPath  := ""
Local cMsgRet    := ""
Local cPathSpool := JRepDirSO(JurFixPath(GetSrvProfString("RootPath", "\undefined"), 0, 2) + "\spool\")
...
Local lExisFunc  := FindFunction("JurSVCall")
Local aParams    := {}
...
Default cNomeArq := ""
	...
	If lOk
		...
		For nI := 1 To nTotalMark
			...
			ElseIf lConfigSV .And. lExisFunc
				Aadd(aParams, {"MV_PAR01", cNameTbTmp, "Hidden"})
				Aadd(aParams, {"MV_PAR02", nRecTmp   , "Hidden"})
				Aadd(aParams, {"MV_PAR03", cCodSocio , "Hidden"})

				If !lAutomato
					cNomeArq += STR0029 + AllTrim((cAlsTmp)->E1_CLIENTE) + "_" + ;
										  AllTrim((cAlsTmp)->E1_LOJA)    + "_" + ;
										  AllTrim((cAlsTmp)->U5_CODCONT) + "_" + ;
										  AllTrim((cAlsTmp)->CTO_MOEDA)  + "_" + FwTimeStamp(1)
				EndIf

				JurSVCall("JURIDICO.SV.SIGAPFS.JURAPAD036_AVISO_COBRANCA.DEFAULT.REP", "report", aParams, cTpSaida, cNomeArq, "pdf", cPasta)

				If cTpSaida <> "3" .And. !lWebApp
					JurOpenFile(cNomeArq + ".pdf", cPathSpool, cTpSaida, .F., @cMsgRet)
				EndIf
			Else
