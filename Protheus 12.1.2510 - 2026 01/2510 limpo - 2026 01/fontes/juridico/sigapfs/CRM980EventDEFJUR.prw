...
Local lJescJur   := SuperGetMV("MV_JESCJUR",, .F.) //Se escritório de advocacia
	...
	If nOperation == MODEL_OPERATION_UPDATE .And. lJescJur
		//--------------------------------------------------
		// Verifica se será possível bloquear ou inativar o cliente
		//--------------------------------------------------
		If oModel:IsFieldUpdated("SA1MASTER", "A1_MSBLQL") .AND. oModel:GetValue("SA1MASTER", "A1_MSBLQL") == '1' .And. FindFunction("J148VLDDEL")
			lValid := J148VLDDEL(oModel:GetValue("SA1MASTER", "A1_COD"), oModel:GetValue("SA1MASTER", "A1_LOJA"))
		EndIf
	EndIf
