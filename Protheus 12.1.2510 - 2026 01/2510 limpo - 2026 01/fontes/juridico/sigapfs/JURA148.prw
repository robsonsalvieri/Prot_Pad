...
If ( nOperation == 3 .Or. nOperation == 4 )
	...
	If (oModel:IsFieldUpdated("NUHMASTER", "NUH_ATIVO") .AND. oModel:GetValue("NUHMASTER", "NUH_ATIVO") == '2') .OR.;
	   (oModel:IsFieldUpdated("SA1MASTER", "A1_MSBLQL") .AND. oModel:GetValue("SA1MASTER", "A1_MSBLQL") == '1')
		...
		lRet := J148VLDDEL(oModel:GetValue("SA1MASTER", "A1_COD"),oModel:GetValue("SA1MASTER", "A1_LOJA"))
...
Return
//---------------------------------------------------------
Method ModelPosVld(oModel, cModelID) Class JA148COMMIT
Local nOper    := oModel:GetOperation()
Local cCliente := oModel:GetValue("SA1MASTER", "A1_COD")
Local cLoja    := oModel:GetValue("SA1MASTER", "A1_LOJA")
Local lRet     := .T.

	If nOper == OP_EXCLUIR .And. oModel:lValid 
		If FindFunction("JurIntLD") .And. !Empty(NUH->NUH_CODLD)
			lRet := JurIntLD("DELETE", "JURA148", NUH->NUH_CODLD, NUH->(Recno())) // Realiza a exclusão online do cliente na Legal Desk
		Else
			J170GRAVA(oModel, xFilial('SA1') + cCliente + cLoja) // Grava na fila de sincronização a exclusão do cliente
		EndIf

		If lRet .And. FindFunction("JExcAnxSinc")
...
/*/
//-------------------------------------------------------------------
Static Function J148VLDCAS(cCliente, cLoja)
Local cQuery     := ""
...
//-------------------------------------------------------------------
/*/{Protheus.doc} J148VLDDEL
Função chamada para retornar caso exista casos ou contratos relacionados ao cliente.
...
/*/
//-------------------------------------------------------------------
Function J148VLDDEL(cCliente, cLoja)
Local nCaso      := 0
Local nContrat   := 0
Local lRet := .T.

Default cIdSA1 := "SA1MASTER"

	nCaso:= J148VLDCAS(cCliente, cLoja)

	If nCaso > 0
		lRet := JurMsgErro(STR0109 ,, STR0110) // Não é possivel inativar o cliente pois há casos vinculados em andamento # Encerre os casos vinculados ao cliente.
	EndIf

	If lRet
		nContrat := J148VLDCTR(cCliente, cLoja)

		If nContrat > 0
			lRet := JurMsgErro(STR0129 ,, STR0130) //Não é possivel inativar o cliente pois há contratos ativos vinculados # Encerre os contratos vinculados ao cliente.
		EndIf
	EndIf
Return lRet

//-------------------------------------------------------------------
/*/{Protheus.doc} J148VLDCTR
Verifica se existem contratos em aberto.
...
/*/
//-------------------------------------------------------------------
Static Function J148VLDCTR(cCliente, cLoja)
Local cQuery   := ""
...
Local ocQuery  := Nil
	...
	ocQuery:SetString(++nParam, Space(1)) // D_E_L_E_T = ' '
	...
	ocQuery:SetString(++nParam, cCliente) // Código do Cliente
	ocQuery:SetString(++nParam, cLoja) // Loja do Cliente
