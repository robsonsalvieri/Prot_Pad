...
Local lPresCaso  := SuperGetMV( 'MV_JCASO3',, .F. ) // Preserva o número do Caso de origem no remanejamento do Caso? .T. True/Verdadeiro - Preserva; .F. False/Falso - Não Preserva.
Local lRemanPres := IsInCallStack("J063NovCas") .And. cNumClien == "2" .And. lPresCaso // Vem de remanajamento, numeração independente do cliente e preserva a numeração do caso
	...
	If nOperation ==  3 .Or. nOperation ==  4 // se for Inclusão ou Alteração
		...
		If lOk .And. nOperation ==  3 // se for Inclusão
			...
			If lOk .And. !lRemanPres // Não é um remanejamento de caso, com numeração independente e não reaproveita a numeração
...
Local aArea      := GetArea()
Local cNumClien  := SuperGetMV( 'MV_JCASO1',, '1' ) // Seqüência da numeração do caso (1 - Por cliente / 2 - Independente)
Local lUsaLacuna := SuperGetMV( 'MV_JCASO2',, .F. ) // Reaproveita lacunas existentes na numeração? .T. True/Verdadeiro - Reaproveita; .F. False/Falso - Não reaproveita.
Local lUsado     := .F.
Local nTamCaso   := 0
