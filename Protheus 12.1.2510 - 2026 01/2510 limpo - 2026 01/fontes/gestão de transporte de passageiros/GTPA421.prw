...
Local nTamDescr := 60 
Local oCommit	:= GTP421COMMIT():New()
	...
	oModel:AddGrid( 'GICDETAIL', 'G6XMASTER', oStruGIC, bPreLin ,, , ,  {|oModel| LoadGIC(oModel)})
	...
	oModel:AddGrid(  'GZFDETAIL', 'G6XMASTER', oStruGZF, /*bLinePre*/, /*bLinePost*/, /*bPre*/, /*bLinePost*/, {|oModel| LoadGZF(oModel)})
	...
	oModel:AddGrid(  'TOTDETAIL', 'G6XMASTER', oStruTotG, /*bLinePre*/, /*bLinePost*/, /*bPre*/, /*bLinePost*/ , {|oModel| LoadTOTGIC(oModel)} )
	...
	oModel:InstallEvent("GTP421COMMIT", /*cOwner*/, oCommit)
...
If (cOperation == "DELETE")
	If (Valtype(oGridGIC:GetValue('GIC_CARGA')) == 'L' .And. oGridGIC:GetValue('GIC_CARGA')) .Or. (Valtype(oGridGIC:GetValue('GIC_CARGA')) == 'C' .And. oGridGIC:GetValue('GIC_CARGA')=='T')
		If !lJob
	...
	ElseIf oGridGZF:SeekLine({{'GZF_TPPASS', cTipo},{'GZF_LOCORI', oGridGIC:GetValue("GIC_LOCORI")}}) // Se a quantidade de bilhetes da linha zerar a linha dos totais pode ser deletada
		...
		Else
			...
			oGridGZF:LoadValue('GZF_QUANT ', oGridGZF:GetValue('GZF_QUANT')  - nQuant)
			oGridGZF:LoadValue('GZF_TARIFA', oGridGZF:GetValue('GZF_TARIFA') - oGridGIC:GetValue('GIC_TAR'))
			oGridGZF:LoadValue('GZF_TXEMB ', oGridGZF:GetValue('GZF_TXEMB')  - oGridGIC:GetValue('GIC_TAX'))
			oGridGZF:LoadValue('GZF_PEDAGI', oGridGZF:GetValue('GZF_PEDAGI') - oGridGIC:GetValue('GIC_PED'))
			oGridGZF:LoadValue('GZF_SEGURO', oGridGZF:GetValue('GZF_SEGURO') - oGridGIC:GetValue('GIC_SGFACU'))
			oGridGZF:LoadValue('GZF_OUTROS', oGridGZF:GetValue('GZF_OUTROS') - oGridGIC:GetValue('GIC_OUTTOT'))
			oGridGZF:LoadValue('GZF_TOTAL ', oGridGZF:GetValue('GZF_TOTAL')  - oGridGIC:GetValue('GIC_VALTOT'))
		EndIf
...
ElseIf (cOperation == "UNDELETE")
	...
	Else
		...
		oGridGZF:LoadValue('GZF_FILIAL', xFilial('GZF'))
		oGridGZF:LoadValue('GZF_AGENCI', oModel:GetValue("G6XMASTER", "G6X_AGENCI"))
		oGridGZF:LoadValue('GZF_TPPASS', cTipo  )
		oGridGZF:LoadValue('GZF_QUANT ', nQuant )
		oGridGZF:LoadValue('GZF_TARIFA', oGridGIC:GetValue('GIC_TAR'))
		oGridGZF:LoadValue('GZF_TXEMB ', oGridGIC:GetValue('GIC_TAX'))
		oGridGZF:LoadValue('GZF_PEDAGI', oGridGIC:GetValue('GIC_PED'))
		oGridGZF:LoadValue('GZF_SEGURO', oGridGIC:GetValue('GIC_SGFACU'))
		oGridGZF:LoadValue('GZF_OUTROS', oGridGIC:GetValue('GIC_OUTTOT'))
		oGridGZF:LoadValue('GZF_TOTAL ', oGridGIC:GetValue('GIC_VALTOT'))
		oGridGZF:LoadValue('GZF_LOCORI', oGridGIC:GetValue("GIC_LOCORI"))
...
Local lPerIncMan	:= .F. 
 	...
 	If GZC->(DbSeek(xFilial("GZC")+cCod)) .AND. GZC->GZC_TIPO $ cTipoGZC
		...
		If !FwIsInCallStack('bLoad421Bil')  .And. !FwIsInCallStack('LoadGrid')
...
Static Function SetF3Struct(cTipo, oStruGIC, oModel)
	...
	If cTipo = "MODEL"
		...
		oStruGIC:SetProperty("GIC_DSCG9B", MODEL_FIELD_INIT , FWBuildFeature( STRUCT_FEATURE_INIPAD, "If(!INCLUI, Posicione('G9B',1,xFilial('G9B')+GIC->GIC_CODG9B,'G9B_DESCRI'),'')"))
...
/*/
//--------------------------------------------------------------------------------------------------------------------
Class GTP421COMMIT FROM FWModelEvent
	Method New()
	Method AfterTTS() //Método que é chamado pelo MVC quando ocorrer as ações do  após a transação.
End Class

//---------------------------------------------------------
Method New() Class GTP421COMMIT
Return

//---------------------------------------------------------
Method AfterTTS(oModel, cModelId) Class GTP421COMMIT

Local oStruGZE   := oModel:GetModel('GZEDETAIL') //Tabela de Depósitos

Local aNewGZE	:= {}
Local aDelGZE	:= {}

	If FwIsInCallStack('GTPA421B') .And. oModel:GetOperation() == MODEL_OPERATION_UPDATE
		//DSERGTP-8038: Ajuste no commit para acrescentar a
		//anexação de documentos GTV para estes tipos de depósitos		 
...
local nVlrTot	  := 0 
Local nTamDescr	  := 60 
Local nTaCodGZG   := TamSx3('GZG_COD')[1]
Local nTaCodGZC   := TamSx3('GZC_CODIGO')[1]
Local nTaCodSeq   := TamSx3('GZG_SEQ')[1]
Local lIntReq     := GTPGetRules("HABINTREQ",,,.F.)
	...
	If nOperation == MODEL_OPERATION_INSERT 
		...
		While (cAliasRD)->(!Eof())	
			If (cAliasRD)->CODIGO <> 0 .And. (cAliasRD)->CODIGO <> 11
				...
				If !lIncFHC .OR. ( lIncFHC .AND. IncluiFich((cAliasRD)->CODIGO) )
					...
					oModel:GetModel(cMdlGZG):SetValue("GZG_SEQ"   , StrZero(oModel:GetModel(cMdlGZG):Length(),nTaCodSeq))
					oModel:GetModel(cMdlGZG):SetValue("GZG_COD"   , StrZero((cAliasRD)->CODIGO,nTaCodGZG))
					oModel:GetModel(cMdlGZG):SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero((cAliasRD)->CODIGO,nTaCodGZG) , 'GZC_DESCRI'),nTamDescr))
		...
		BeginSQL alias cAliasReq    
			...
			// 1 - Vendas por tipo (GQL/GQM)
			SELECT 
				CASE 
					WHEN GQL.GQL_TPVEND = '1' THEN '13' 
					ELSE '12' 
				END AS TIPO,
				SUM(GQM.GQM_VALOR) AS TOTAL
			FROM %TABLE:GQM% GQM
			...
			// 3 - Pagamentos GZP vinculados ao GZT (novidade)
			SELECT 
				GZP.GZP_TPAGTO AS TIPO,
		...
		While (cAliasReq)->(!Eof() .AND. (cAliasReq)->(TOTAL) > 0)
			
			If !Empty(oGridGZGD:GetValue('GZG_COD')) .Or. oGridGZGD:IsDeleted()
				oGridGZGD:AddLine()
			EndIf

			cTPAgto := (cAliasReq)->(TIPO)
			...
			If nTpagto > 0
				...
				oGridGZGD:SetValue("GZG_SEQ"   , StrZero(oGridGZGD:Length(),nTaCodSeq))
				oGridGZGD:SetValue("GZG_COD"   , StrZero(nTpagto,nTaCodGZG))		
				oGridGZGD:SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(nTpagto,nTaCodGZG) , 'GZC_DESCRI'),nTamDescr))
		...
		// Pagamentos GZP por tipo (referência GIC)
		...
		While (cAliasTef)->(!Eof())
			...
			IF lIntReq .AND. (cAliasTef)->TIPO == 'OS' 
				(cAliasTef)->(dbSkip())
				Loop
			ENDIF

			cTPAgto := (cAliasTef)->TIPO
			nTpagto := Gtp421Pgto("2",cTPAgto)			

			If (cAliasTef)->TOTAL > 0 .And. nTpagto > 0
				If !Empty(oGridGZGD:GetValue('GZG_COD')) .Or. oGridGZGD:IsDeleted()
					oGridGZGD:AddLine()
				EndIf
				...
				oGridGZGD:SetValue("GZG_TIPO"  , "2")
				oGridGZGD:SetValue("GZG_CARGA" , .T.)
				oGridGZGD:SetValue("GZG_VALOR" , (cAliasTef)->TOTAL )
			Endif		
		...
		While (cAliasTax)->(!Eof() .AND. (cAliasTax)->(TOTALG57) > 0)
			...
			If !Empty(cCodTax)
				...
				oGridGZGR:SetValue("GZG_SEQ"   , StrZero(oGridGZGR:Length(),nTaCodSeq))
		...
		While (cAliasGZT)->(!Eof())	
			...
			If ( !Empty((cAliasGZT)->GZC_CODIGO) )
				...
				oModel:GetModel(cMdlGZG):SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + PadR((cAliasGZT)->GZC_CODIGO,nTaCodGZC) , 'GZC_DESCRI'),nTamDescr))
				...
				If !EmpTy(cCodTXConv) .AND. (cAliasGZT)->GZC_CODIGO $ cCodTXConv				
					...
					oModel:GetModel(cMdlGZGInv):SetValue("GZG_SEQ"   , StrZero(oModel:GetModel(cMdlGZGInv):Length(),nTaCodSeq))
					oModel:GetModel(cMdlGZGInv):SetValue("GZG_COD"   , (cAliasGZT)->GZC_CODIGO)
					oModel:GetModel(cMdlGZGInv):SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + PadR((cAliasGZT)->GZC_CODIGO,nTaCodGZC) , 'GZC_DESCRI'),nTamDescr))
		...
		While (cAliasReC)->(!Eof())	
			If (cAliasReC)->(TOTAL) > 0
				...
				If (nTpagto == 20 .Or. nTpagto == 34 .Or. nTpagto == 60 )
					...
					oGridGZGR:SetValue("GZG_NUMFCH", cNumFch)
					oGridGZGR:SetValue("GZG_SEQ"   , StrZero(oGridGZGR:Length(),nTaCodSeq))
					oGridGZGR:SetValue("GZG_COD"   , StrZero(nTpagto,nTaCodGZG))
					oGridGZGR:SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(nTpagto,nTaCodGZC) , 'GZC_DESCRI'),nTamDescr))
					oGridGZGR:SetValue("GZG_TIPO"  , "1")
					oGridGZGR:SetValue("GZG_CARGA" , .T.)
		...
		While (cAliasReC)->(!Eof())	
			...
			If nVlrTot > 0 .And. (nTpagto > 0 .OR. cTPAgto $ "FA") 
				...
				oGridGZGR:SetValue("GZG_SEQ"   , StrZero(oGridGZGR:Length(),nTaCodSeq))
				If cTPAgto == "FA"
					oGridGZGR:SetValue("GZG_COD"   , Padl("6A",nTaCodGZC, "0"))
					oGridGZGR:SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + Padl("6A",nTaCodGZC, "0") , 'GZC_DESCRI'),nTamDescr))
				Else
					oGridGZGR:SetValue("GZG_COD"   , StrZero(nTpagto,nTaCodGZG))				
					oGridGZGR:SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(nTpagto,nTaCodGZC) , 'GZC_DESCRI'),nTamDescr))
				Endif
		...
		If nVlrCom > 0
			...
			oGridGZGD:SetValue("GZG_NUMFCH", cNumFch)
			oGridGZGD:SetValue("GZG_SEQ"   , StrZero(oGridGZGD:Length(),nTaCodSeq))
			oGridGZGD:SetValue("GZG_COD"   , StrZero(28, nTaCodGZG))
			oGridGZGD:SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(28, nTaCodGZC) , 'GZC_DESCRI'),nTamDescr))
			oGridGZGD:SetValue("GZG_TIPO"  , "2")
			oGridGZGD:SetValue("GZG_CARGA" , .T.)
		...
		If lStrGYV
			...
			If (cAliasDep)->VLRTOTAL > 0
				...
				oGridGZGD:SetValue("GZG_NUMFCH", cNumFch)
				oGridGZGD:SetValue("GZG_SEQ"   , StrZero(oGridGZGD:Length(),nTaCodSeq))
				oGridGZGD:SetValue("GZG_COD"   , StrZero(29, nTaCodGZG))
				oGridGZGD:SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(29, nTaCodGZC) , 'GZC_DESCRI'),nTamDescr))
				oGridGZGD:SetValue("GZG_TIPO"  , "2")
				oGridGZGD:SetValue("GZG_CARGA" , .T.)
		...
		If GIC->(FIELDPOS("GIC_EMPRJI")) > 0 .AND. GIC->(FIELDPOS("GIC_INTEGR")) > 0 .AND.;
			...
			If (cAliasOut)->VLRTOTAL > 0
				...
				oGridGZGR:SetValue("GZG_NUMFCH", cNumFch)
				oGridGZGR:SetValue("GZG_SEQ"   , StrZero(oGridGZGR:Length(),nTaCodSeq))
				oGridGZGR:SetValue("GZG_COD"   , StrZero(30, nTaCodGZG))
				oGridGZGR:SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(30, nTaCodGZC) , 'GZC_DESCRI'),nTamDescr))
				oGridGZGR:SetValue("GZG_TIPO"  , "1")
				oGridGZGR:SetValue("GZG_CARGA" , .T.)
				...
				oGridGZGD:SetValue("GZG_NUMFCH", cNumFch)
				oGridGZGD:SetValue("GZG_SEQ"   , StrZero(oGridGZGD:Length(),nTaCodSeq))
				oGridGZGD:SetValue("GZG_COD"   , StrZero(31, nTaCodGZG))
				oGridGZGD:SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(31, nTaCodGZC) , 'GZC_DESCRI'),nTamDescr))
				oGridGZGD:SetValue("GZG_TIPO"  , "2")
				oGridGZGD:SetValue("GZG_CARGA" , .T.)
		...
		If AliasInDic('H6M') .And. AliasInDic('H6N') .And.;
			GZC->(dbSeek(xFilial('GZC')+StrZero(32,nTaCodGZC)))
			...
			If (cAliasH6N)->H6N_VLPEND > 0
					...
					oGridGZGD:SetValue("GZG_NUMFCH", cNumFch)
					oGridGZGD:SetValue("GZG_SEQ"   , StrZero(oGridGZGD:Length(),nTaCodSeq))
					oGridGZGD:SetValue("GZG_COD"   , StrZero(32, nTaCodGZG))
					oGridGZGD:SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(32, nTaCodGZC) , 'GZC_DESCRI'),nTamDescr))
					oGridGZGD:SetValue("GZG_TIPO"  , "2")
					oGridGZGD:SetValue("GZG_CARGA" , .T.)
...
ElseIf (cOperation == "CANSETVALUE") .And. !IsInCallStack("bLoad421Bil")  .And. !FwIsInCallStack('LoadGrid')
...
If (cOperation == "DELETE") 
	...
	ElseIf !Empty(oGridGZGD:GetValue('GZG_CQVINC'))
	 	Return .T.
	EndIf
ElseIf (cOperation == "CANSETVALUE") .And. !IsInCallStack("bLoad421Bil")  .And. !FwIsInCallStack('LoadGrid')// Permite alterar o registro se não for originado do Load bLoad421Bil
...
cNumFch    := oModelG6X:GetValue('G6X_NUMFCH')
	...
	If oModelGZGR:SeekLine( { { "GZG_COD", StrZero(1,nTaCodGZG) } } )
		oModelGZGR:SetNoDeleteLine(.F.)
		oModelGZGR:DeleteLine()
		oModelGZGR:SetNoDeleteLine(.T.)
	EndIf
	If oModelGZGD:SeekLine( { { "GZG_COD", StrZero(2,nTaCodGZG) } } )
	...
	For nY := 1 to Len(aCheque)	// Caso seja encontrado um cheque com perfil de receita e que precise ser vinculado
		If aCheque[nY][2] == 'R' .And. aCheque[nY][4]
			If !oModelGZGR:SeekLine( { { "GZG_COD", StrZero(1,nTaCodGZG) } } )
				...
				oModelGZGR:SetValue("GZG_SEQ"   , StrZero(oModelGZGR:Length(),nTaCodSeq))
				oModelGZGR:SetValue("GZG_COD"   , StrZero(1,nTaCodGZG))
				oModelGZGR:SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(1,nTaCodGZC) , 'GZC_DESCRI'),nTamDescr))
			...
			ElseIf oModelGZGR:SeekLine( { { "GZG_COD", StrZero(1,nTaCodGZG) } } ) .And. aCheque[nY][4]
				nValorR += aCheque[nY][3]
				oModelGZGR:SetValue("GZG_VALOR"  , nValorR)
			EndIf
		...
		ElseIf aCheque[nY][2] == 'D' .And. aCheque[nY][4]			
			If !oModelGZGD:SeekLine( { { "GZG_COD", StrZero(2,nTaCodGZG) } } )
				...
				oModelGZGD:SetValue("GZG_SEQ"   , StrZero(oModelGZGD:Length(),nTaCodSeq))
				oModelGZGD:SetValue("GZG_COD"   , StrZero(2,nTaCodGZG))
				oModelGZGD:SetValue("GZG_DESCRI", Padl(POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(2,nTaCodGZC) , 'GZC_DESCRI'),nTamDescr))
			...
			ElseIf oModelGZGD:SeekLine( { { "GZG_COD", StrZero(2,nTaCodGZG) } } ) .And. aCheque[nY][4] 
				nValDesp += aCheque[nY][3]
				oModelGZGD:SetValue("GZG_VALOR"  , nValDesp)
			EndIf
...
//-------------------------------------------------------------------
/*/{Protheus.doc} LoadGIC()
Função de carregamento da GIC - Bilhetes
...
Local oGridG6X    := oModel:GetModel():GetModel('G6XMASTER')
Local oGridGIC    := oModel:GetModel():GetModel('GICDETAIL')
Local cAliasGIC   := GetNextAlias()
Local cAgencia    := ""
Local cNumFch     := ""
Local cWhere	  := ""
...
Local aRet		  := {}
	...
	BeginSQL alias cAliasGIC    
		...
		WHERE GIC_FILIAL  = %xFilial:GIC%
			...
			AND GIC.%NotDel%	
			ORDER BY  GIC.GIC_STATUS 	

	EndSQL

	aRet := FwLoadByAlias(oGridGIC, cAliasGIC)
...
//-------------------------------------------------------------------
/*/{Protheus.doc} LoadGZF()
Função de carregamento da GZF - Localidade
@author  Karyna Martins
@since   22/10/2025
@version P12
/*/ 
//-------------------------------------------------------------------
Static Function LoadGZF( oModel )
Local oGridG6X    := oModel:GetModel():GetModel('G6XMASTER')
Local oMdlGZF     := oModel:GetModel():GetModel('GZFDETAIL')
Local cAliasGZF   := GetNextAlias()
Local cAgencia    := ""
Local cNumFch     := ""
Local cWhere	  := ""
Local dDataIni    := Stod("  /  /  ")
...
Local aRet		  := {}
	...
	//Totaliza os bilhetes por localidade
	BeginSQL alias cAliasGZF    

		SELECT GIC_TIPO GZF_TPPASS, 
			GIC_ORIGEM GZF_ORIGEM, 
			GIC_LOCORI GZF_LOCORI,
			COUNT(*) GZF_QUANT,
			SUM(GIC_TAR) GZF_TARIFA,
			SUM(GIC_PED) GZF_PEDAGI,
			SUM(GIC_TAX) GZF_TXEMB,
			SUM(GIC_SGFACU) GZF_SEGURO,
			SUM(GIC_OUTTOT) GZF_OUTROS,
			SUM(GIC_VALTOT) GZF_TOTAL					
		FROM %Table:GIC% GIC	
		WHERE GIC_FILIAL  = %xFilial:GIC%
			AND GIC_AGENCI = %Exp:cAgencia%
			AND (
	...
	aRet := FwLoadByAlias(oMdlGZF, cAliasGZF)

	(cAliasGZF)->(DbCloseArea())
...
@see (links_or_references)
/*/
Static Function LoadTOTGIC(oModel)
Local oMdlTot	:= oModel:GetModel():GetModel("TOTDETAIL")
Local cTmpAlias	:= QryTotGeral(oModel)
Local aRet		:= {}

aRet	:= FWLoadByAlias(oMdlTot, cTmpAlias) 	
...
If oGrid:GetOperation() == MODEL_OPERATION_INSERT .Or. FwIsInCallStack('GTPA421B')
