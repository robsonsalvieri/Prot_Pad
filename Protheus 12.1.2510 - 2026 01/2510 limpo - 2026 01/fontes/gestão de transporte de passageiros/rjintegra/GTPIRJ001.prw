					...
					If lContinua
						...
						For nY := 1 To Len(aFldDePara)
							...
							If !Empty(cTagName) .And. !Empty((xValor := oRJIntegra:GetJsonValue(nX, cTagName, cTipoCpo)))
								...
								If !lContinua
									cErro := I18N(STR0004, {cCampo, GTPXErro(oModel)}) +CRLF+' campo/conteúdo/estado/idCidade: ['+cCampo+']['+xValor+']['+oMdlGI1:GetValue("GI1_UF")+']['+oRJIntegra:GetJsonValue(nX, "idCidade", "C")+'] Acao: '+Iif(nOpc==3,'Inclusão','Alteração')	// "Falha ao gravar o valor do campo #1 (#2)."
									If aScan(aError, {|x| AllTrim(x) == cErro}) == 0
										GtpGrvLgRj(		STR0009,; //"Localidade"
														oRJIntegra:cUrl,;
														oRJIntegra:cPath,;
