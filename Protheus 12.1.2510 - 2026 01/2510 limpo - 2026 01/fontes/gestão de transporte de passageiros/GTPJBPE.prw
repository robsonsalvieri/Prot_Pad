...
BeginSql Alias cAliasJob
	...
	SELECT GIC.GIC_CODIGO,
	       ...
	       CASE WHEN ((GIC.GIC_STATUS IN ('V','I')
	           ...
	           ELSE 5
	       END ORDEMREG,GIC.GIC_DTVEND,GIC.GIC_HRVEND
	FROM %Table:GIC% GIC
	...
	WHERE GIC_FILIAL = %xFilial:GIC%
	  ...
	  AND GIC.%NotDel%
	ORDER BY GI6.GI6_FILRES,GIC.GIC_DTVEND,GIC.GIC_HRVEND, GIC.GIC_CODIGO
...
iF lH87
	H87->(DbSetOrder(2)) //H87_FILIAL+H87_LOCORI+H87_LOCDES
EndIf                                                                                                 
...
While (cAliasJob)->(!Eof())
	...
	If 	G9O->(DbSeek(xFilial("G9O") + GIC->GIC_ORIGEM + cTpBil + GIC->GIC_STATUS + cTpLinha)) .Or. ;				//Procura primeiramente a chave exata (com o tipo de linha)
		G9O->(DbSeek(xFilial("G9O") + GIC->GIC_ORIGEM + cTpBil + GIC->GIC_STATUS + Space(TamSx3('G9O_GQCCOD')[1]) ))//Caso não encontrar, procura a chave sem o tipo de linha	
		...
		If lHasH60 .And. ( GIC->GIC_VLRPGT > 0 .And. ;
			...
			Endif
		Else 
			If lH87 .And. H87->(DbSeek(xFilial('H87')+GIC->(GIC_LOCORI+GIC_LOCDES)))
				cProdTar	:= H87->H87_PRDTAR
				cProdTax	:= H87->H87_PRDTAX
				cProdPED	:= H87->H87_PRDPED
				cProSGFACU	:= H87->H87_PRDSEG
				cProUtTot	:= H87->H87_PRDOUT
			Else 
		...
		Endif
	Else
		cMsgErrLog := STR0029 // "Não existe regra de produto x tipo bilhete cadastrada"
		lSkip := .T.
	Endif
				...
				If (VldBilhete())
					...
					If !Empty(aDadosDev[NOTA]) 
						...
						If SF2->(DbSeek(xFilial("SF2")+ cChvNF))
							...
							If GIC->GIC_VENDRJ $ 'CAN|VBR' .OR. (Empty(GIC->GIC_VENDRJ) .AND. GIC->GIC_TIPO = 'W') //Quando for cancelamento sem substituição
...
ManutVIP(cAgencia, aAgenGNF, cDtIni, cDtFim, cDtIniGNF, cDtFinGNF)
...
Default aItensNF:= {}
	...
	If lRet .And. Len(aItensNF) > 0
		...
		For nX := 1 To Len(aItensNF)
			...
			If !Empty(aItensNF[nX][1])
				...
				If Empty(SB1->B1_TS) .Or. !(SF4->( dbSeek( xFilial("SF4") + SB1->B1_TS)))	
					...
					cMsg += CRLF + 'Cód.Produto pesquisado: [' + aItensNF[nX][1] + ']'
					lRet := .F.
					Exit
				Else 
					lFindProd := .T.									
		...
		If lRet .And. !lFindProd
			cMsg := 'Não parametrizado produto para geração da nota fiscal.'
			lRet := .F.
		EndIf 
	...
	If lRet .And. !(cStatus $ 'C|D')
		...
		If SF2->(DbSeek(xFilial("SF2")+ PadR(cNumBil,TamSx3('F2_DOC')[1])+PadR(cSerie,TamSx3('F2_Serie')[1])+SA1->A1_COD+SA1->A1_LOJA))
			If !(SF2->F2_ESPECIE == 'CTE')
				GrvNfGIC("", GIC->GIC_CODIGO, GIC->GIC_TIPO, "V", "", "1")
				lRet := .F.
			EndIf
		EndIf
...
@see (links_or_references)
/*/
Static Function ManutVIP(cAgencia, aAgenGNF, cDataIni, cDataFim, cDtIniGNF, cDtFinGNF)
	...
	While (cAliasQry)->(!Eof())
		...
		If GIC->GIC_STATUS == 'T' .AND. !Empty(GIC->GIC_BILREF)
			cChaveNf := GetBilVenda(GIC->GIC_BILREF)
			...
			If !Empty(cChaveNf) 
				...
				If SF2->(DBSeek(xFilial('SF2')+cChaveNf))
					...
					GetRecnoSD2(cChaveNf,@aRegSD2)
					SF2->(MaDelNFS(aRegSD2,aRegSE1,aRegSE2,.F.,.F.,.T.,.F.))

					GrvNfGIC(cChaveNf, (cAliasQry)->GIC_CODIGO, Nil, 'C', Nil, '1',,Nil,cChvSF3,cChvSFT)
		...
		If GIC->GIC_STATUS <> 'T' .Or. lCanBil
			Reclock("GIC", .F.)
				GIC->GIC_STAPRO := '5'
			GIC->(MsUnlock())
		Else
			GrvErro(GIC->GIC_CODIGO, "2", STR0024 + GIC->GIC_BILREF + STR0025) //"Bpe de origem "##" não encontrado ou não faturado"
		Endif
...
/*/{Protheus.doc} GetBilVenda(cCodBil)
Localiza o bilhete de venda original
@type function
@author João Pires
@since 31/10/2025
@version 1.0
/*/
Static Function GetBilVenda(cCodBil)
	Local aAreaGIC  := GIC->(GetArea())
	Local cRet		:= ''
	Local cChave    := ''
	...
	If GIC->(DBSeek(xFilial("GIC") + cCodBil)) .AND. !Empty(GIC->GIC_CHVBPE)	
		cChave := GIC->GIC_CHVBPE	

		GIC->(DbSetOrder(12)) //GIC_FILIAL+GIC_CHVBPE
		GIC->(DBSeek(xFilial('GIC') + cChave))
		
		While GIC->(!Eof()) .AND. GIC->GIC_CHVBPE == cChave
			If !Empty(GIC->GIC_NOTA) .And. !Empty(GIC->GIC_SERINF) .And. !Empty(GIC->GIC_CLIENT)
				cRet := GIC->(GIC_NOTA+GIC_SERINF+GIC_CLIENT+GIC_LOJA)
