...
Static Function TP502Grv(oModel)
    ...
    Local oGridGQW  := oModel:GetModel("GQWDETAIL")
    Local oGridGQM  := oModel:GetModel("GQMDETAIL")
    Local oGridH7D  := oModel:GetModel( 'H7DDETAIL' )
    Local lRet      := .T.
    ...
    If oModel:VldData()        
        ...
        If nOp == MODEL_OPERATION_DELETE
            GetAttached(oGridGIC,oGridG57,oGridG99,oGridGQL,oGridDGZG,oGridEGZG,oGridGQW,oGridGQM,oGridH7D)
        EndIf             
...
@version 12.1.2310
/*/
Static Function GetAttached(oGridGIC,oGridG57,oGridG99,oGridGQL,oGridDGZG,oGridEGZG,oGridGQW,oGridGQM,oGridH7D)    
    Local nX        := 0
    Local nY        := 0
    Local nH        := 0
    Local aAttached := {}
    ...
    For nY:= 1 To oGridH7D:Length()
        ...
        For nX := 1 to oGridGIC:Length()
            oGridGIC:GoLine(nX)  
            cSeek := oGridGIC:GetValue("GIC_CODIGO")    //GIC_FILIAL+GIC_CODIGO
            If !Empty(cSeek)
                cSeek := xFilial('GIC') + cSeek 
                Aadd(aAttached,xFilial('AC9')+'GIC'+xFilial('GIC')+cSeek)  ////AC9_FILIAL, AC9_ENTIDA, AC9_FILENT, AC9_CODENT, AC9_CODOBJ, R_E_C_N_O_, D_E_L_E_T_
            EndIf             
        Next nX

        For nX := 1 to oGridG57:Length()
            oGridG57:GoLine(nX)  
            cNumMov := oGridG57:GetValue('G57_NUMMOV')
            cSerie  := oGridG57:GetValue('G57_SERIE')
            cSubSer := oGridG57:GetValue('G57_SUBSER')
            cNumCom := oGridG57:GetValue('G57_NUMCOM')
            cCodTab := oGridG57:GetValue('G57_CODIGO')
            cTipo   := oGridG57:GetValue('G57_TIPO')
            cSeek   := cNumMov + cSerie + cSubSer + cNumCom + cCodTab + cTipo      
            If !Empty(cSeek)        
                cSeek := xFilial('G57') + cSeek //G57_FILIAL+G57_NUMMOV+G57_SERIE+G57_SUBSER+G57_NUMCOM+G57_CODIGO+G57_TIPO
                Aadd(aAttached,xFilial('AC9')+'G57'+xFilial('G57')+cSeek)
            EndIf             
        Next nX
        For nX := 1 to oGridG99:Length()
            oGridG99:GoLine(nX)  
            cCodTab := oGridG99:GetValue('G99_CODIGO')
            cSeek   := cCodTab        
            If !Empty(cSeek)
                cSeek := xFilial('G99') + cSeek     //G99_FILIAL+G99_CODIGO
                Aadd(aAttached,xFilial('AC9')+'G99'+xFilial('G99')+cSeek)
            EndIf             
        Next nX
        For nX := 1 to oGridGQL:Length()
            oGridGQL:GoLine(nX)  
            cCodTab := oGridGQL:GetValue('GQL_CODIGO')
            cSeek   := cCodTab        
            If !Empty(cSeek)
                cSeek := xFilial('GQL') + cSeek    //GQL_FILIAL+GQL_CODIGO
                Aadd(aAttached,xFilial('AC9')+'GQL'+xFilial('GQL')+cSeek)
            EndIf            
            For nH := 1 to oGridGQM:Length()
                oGridGQM:GoLine(nH)  
                cCodTab := oGridGQM:GetValue('GQM_CODGQL')
                cCodNsu := oGridGQM:GetValue('GQM_CODNSU')
                cCodAut := oGridGQM:GetValue('GQM_CODAUT')
                cSeek   := cCodTab + cCodNsu + cCodAut
                If !Empty(cSeek)        
                    cSeek := xFilial('GQM') + cSeek //GQM_FILIAL+GQM_CODGQL+GQM_CODNSU+GQM_CODAUT
                    Aadd(aAttached,xFilial('AC9')+'GQM'+xFilial('GQM')+cSeek)
                EndIf            
            Next nX
        ...
        For nX := 1 to oGridDGZG:Length()
            oGridDGZG:GoLine(nX)  
            cAgenc  := oGridDGZG:GetValue('GZG_AGENCI')
            NroFich := oGridDGZG:GetValue('GZG_NUMFCH')
            cSeq    := oGridDGZG:GetValue('GZG_SEQ')
            cTipo   := oGridDGZG:GetValue('GZG_TIPO')
            cSeek   := cAgenc + NroFich + cSeq + cTipo        
            If !Empty(cSeek)        
                cSeek := xFilial('GZG') + cSeek //GZG_FILIAL+GZG_AGENCI+GZG_NUMFCH+GZG_SEQ+GZG_TIPO
                Aadd(aAttached,xFilial('AC9')+'GZG'+xFilial('GZG')+cSeek)
            EndIf             
        Next nX
        For nX := 1 to oGridEGZG:Length()
            oGridEGZG:GoLine(nX) 
            cAgenc  := oGridEGZG:GetValue('GZG_AGENCI')
            NroFich := oGridEGZG:GetValue('GZG_NUMFCH')
            cSeq    := oGridEGZG:GetValue('GZG_SEQ')
            cTipo   := oGridEGZG:GetValue('GZG_TIPO')
            cSeek   := cAgenc + NroFich + cSeq + cTipo        
            If !Empty(cSeek)
                cSeek := xFilial('GZG') + cSeek     //GZG_FILIAL+GZG_AGENCI+GZG_NUMFCH+GZG_SEQ+GZG_TIPO
                Aadd(aAttached,xFilial('AC9')+'GZG'+xFilial('GZG')+cSeek)
        ...
        For nX := 1 to oGridGQW:Length()
            oGridGQW:GoLine(nX)  
            cCodTab := oGridGQW:GetValue('GQW_CODIGO')
            cSeek   := cCodTab 
            If !Empty(cSeek)    //GQW_FILIAL+GQW_CODIGO      
                cSeek := xFilial('GQW') + cSeek          
                Aadd(aAttached,xFilial('AC9')+'GQW'+xFilial('GQW')+cSeek)          
            EndIf             
        Next nX
    Next nY        
    If Len(aAttached) > 0
...
//------------------------------------------------------------------------------------------
/*/{Protheus.doc} Dettach(aAttached)

Função para exclusão da base de conhecimento

@Params:
	aAttached*:	array, possui as chaves de AC9 dos documentos que deverão ser excluídos
...
/*/
//------------------------------------------------------------------------------------------
Static Function Dettach(aAttached)
	...
	For nI := 1 to Len(aAttached)
		
		If ( AC9->(DbSeek(aAttached[nI]))	)
