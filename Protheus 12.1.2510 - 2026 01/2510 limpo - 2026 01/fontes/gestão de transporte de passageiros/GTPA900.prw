...
Local cNumero	:= oModel:GetValue('GY0MASTER', 'GY0_NUMERO')
CN9->(dbSetOrder(1))
If CN9->(dbSeek(xFilial("CN9")+GY0->GY0_CODCN9+cRevVig))
	...
	Else
		...
		If lRet
			...
			lRet := lRet .And. InativGY0() .And. InativGID(cNumero,cRevAtu)
...
/*/{Protheus.doc} InativGID(cNumero,cRevisa)
//Inativa os horários revisados dos contratos anteriores
@author kaique.olivero	
@since 08/10/2025
@version 1.0
@return ${return}, ${return_description}
@type function
/*/
Static Function InativGID(cNumero,cRevisa)
Local lRet 		:= .T.
Local cAliasGID	:= GetNextAlias()

BeginSql Alias cAliasGID 
	SELECT GID.R_E_C_N_O_ RECNO
	FROM %Table:GID% GID
	INNER JOIN %Table:GYD% GYD ON GYD.%NotDel%
		AND GYD.GYD_FILIAL = %xFilial:GYD%
		AND GYD.GYD_NUMERO = %Exp:cNumero%
		AND GYD.GYD_REVISA < %Exp:cRevisa%
		AND GYD.GYD_CODGI2 = GID.GID_LINHA
	WHERE GID.GID_FILIAL = %xFilial:GID%
		AND GID.%NotDel%
		AND GID_REVCON < %Exp:cRevisa% 
		AND GID_STATUS = '1'
	GROUP BY GID.R_E_C_N_O_
EndSql

If (cAliasGID)->(!EoF())
	While (cAliasGID)->(!EoF())
		GID->(dbGoto((cAliasGID)->RECNO))
			RecLock('GID', .F.)
				GID->GID_STATUS  := '2'
			GID->(MsUnLock())
		(cAliasGID)->(DbSkip())
