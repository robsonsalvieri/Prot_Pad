...
Static __nThread   := 6
Static nMinThreads := 1
Static nMaxThreads := 20
Static cAgenPerg   := ''
Static cGridFocus  := ''
Static lNewTot     := .F.
Static lHabFilBrw  := .F.
...
EndIf
oStruGZGD:AddTrigger('GZG_STATIT','LEGENDA',{||.T.}, bTrig)
oStruGZGE:AddTrigger('GZG_STATIT','LEGENDA',{||.T.}, bTrig)
...
If !(oMdl:GetOperation() == MODEL_OPERATION_INSERT)
    oStruGZGD:AddField(	;
        ...
        {|oModel|SetIniFld(oModel)},;			//	[11]  B   Code-block de inicializacao do campo
...
If cField == 'GQM_CONFER'
    ...
    Endif
ElseIF cField == 'GZG_STATIT'
   cRet:= SetIniFld(nIL,uVal)
Endif
...
If oModel:GetOperation() == MODEL_OPERATION_VIEW 
    ...
    oView:AddOtherObject('PANEL_LEGENDA1', {|oView| ViewLegenda(oView)} ) // botoes com as acoes
    oView:AddOtherObject('PANEL_LEGENDA2', {|oView| ViewLegenda(oView)} ) // botoes com as acoes
    oView:SetOwnerView("PANEL_LEGENDA1",'BOX2DIR')
    oView:SetOwnerView("PANEL_LEGENDA2",'BOX2ESQ')
    ...
    If GZG->(FIELDPOS( "GZG_CONFER" )) > 0 .AND. GZG->(FIELDPOS( "GZG_DTCONF" )) > 0 .AND.;
      GZG->(FIELDPOS( "GZG_VLACER" )) > 0
        ...
        While ((cAliasGZGD)->(!EOF()))
            ...
            oModelDGZG:SetValue( "GZG_STATIT", (cAliasGZGD)->GZG_STATIT)
        ...
        While ((cAliasGZGE)->(!EOF()))
            ...
            oModelEGZG:SetValue( "GZG_STATIT", (cAliasGZGE)->GZG_STATIT)
...
Function GA500Abre()
    ...
    If G6X->(DbSeek(xFilial("G6X")+G59->G59_AGENCI+G59->G59_NUMFCH))
        If G6X->G6X_FECHCX
            FWAlertWarning(I18n(STR0186, { G6X->G6X_CODCX } ), STR0018) //"A arrecadação não poderá ser aberta, pois o caixa Nr.: #1 está fechado." //"Atenção"
            RestArea(aAreaAtu)
            Return .T. 
        EndIf 
    EndIf   
    RestArea(aAreaAtu)      
    FWMsgRun(,{|| ProNewAbr()}, STR0023, STR0099)  //"Aguarde enquanto os documentos estão sendo conferidos..." //"Processando Fechamento"
Return .T.
...
Function GA500Fech()
    ...
    FWMsgRun(,{|| ProNewFech()}, STR0023, STR0099)  //"Aguarde enquanto os documentos estão sendo conferidos..." //"Processando Fechamento"
...
Static Function GA500ConfFich(cOperation)
    ...
    If DbSeek(xFilial("G6X")+G59->G59_AGENCI+G59->G59_NUMFCH) .And. G6X->G6X_STATUS <> '4'

        If cOperation == "1"
            RecLock("G6X",.F.)
                G6X->G6X_STATUS := "3"  // Ficha Conferida
            G6X->(MsUnLock())                
        ElseIf cOperation == "2" 
            RecLock("G6X",.F.)
                G6X->G6X_STATUS := "2"  // Ficha Entregue
            G6X->(MsUnLock())        
        EndIf
...
If !IsBlind() .And. aPeriod[2] $ '1|5' 
    FWAlertHelp(STR0184, STR0185 + Alltrim(aPeriod[5]) )     //'Ficha de remessa não permitido.' //'Efetuar a entrega da ficha de remessa: '
    Return .F.
EndIf 
...
Function G500Reproc()
    ...
    If FWAlertYesNo(STR0179, STR0180)    //"Processa registro posicionado?"   "Reprocessamento"
        If G59->G59_STATUS
            FWMsgRun(,{|| ProNewFech()}, STR0023, STR0099)  //"Aguarde enquanto os documentos estão sendo conferidos..." //"Processando Fechamento"
        Else 
            FWAlertInfo(STR0181+CRLF+STR0182, STR0183)     //'Ficha em aberto.' 'Reprocessamento permitido apenas para ficha Fechado.' 'Status da Ficha'
        EndIf 
    Else 
...
/*
{Protheus.doc} ProNewFech()
    Função de processamento dos registros
    Montagem inicial por threads definidas por parâmetro do módulo.
    type    Function
    author  José Carlos
    since   22/10/2025
    param   Nil
return
*/
Static Function ProNewFech()
    Local cAliasQry   := NIL 
    Local cTipoP      := Padr( 'TF' ,TamSx3( 'E2_TIPO' )[1])
    Local cParcelaP   := StrZero(1, TamSx3( 'E2_PARCELA' )[1])
    Local cNatTitP    := ''
    Local cPrefixoP   := ''
    Local cTipoR      := Padr( 'TF' , TamSx3( 'E1_TIPO' )[1] )
    Local cParcelaR   := StrZero(1,TamSx3( 'E1_PARCELA' )[1])
    Local cNatTitR    := ''
    Local cPrefixoR   := ""
    Local nE1_PREFIXO := TamSx3( 'E1_PREFIXO' )[1]
    Local nE2_PREFIXO := TamSx3( 'E2_PREFIXO' )[1]
    Local oGrid
    Local cError      := ""
    Local cNum        := ""
    Local cFilTit     := ""
    Local nCnt        := 0
    Local cNatRet     := GPA281PAR("NATUREZA")
    Local aAreaAtu    := GetArea() 
    If ! FwIsInCallStack("G500Reproc") 

        If G59->G59_STATUS
            FWAlertInfo(STR0027, STR0018) //"Atenção" //"Este registro já foi fechado!"
            Return
        EndIf
    ...
    BeginSql Alias cAliasQry
        ...
        WHERE GZG.GZG_FILIAL = %xFilial:GZG%
            AND GZG_AGENCI = %Exp:G59->G59_AGENCI%
            AND GZG_NUMFCH = %Exp:G59->G59_NUMFCH%
            AND GZG_STATIT IN (' ','0','2')
            AND GZG.%NotDel%
    EndSql

    If (cAliasQry)->(!Eof()) 

        // Quantidade de threads para processamento via parâmetro do módulo
        __nThread := GetThreads()

        // Neste ponto a função RecCount() sempre traz Zero.
        nCnt := Contar(cAliasQry,".T.")

        // Posiciona no Topo
        (cAliasQry)->(DbGoTop())

        // Avalia a quantidade de abertura de Threads
        If nCnt < __nThread
            __nThread := nMinThreads
        EndIf 

        //Objeto do Controlador de Threads (Instancia para Execução das Threads)
        oGrid := FWIPCWait():New("GTPA421D"+cEmpAnt,2000)
        //Inicia as Threads
        ...
        oGrid:Start("GTPA421D")    // Gera titulos a receber ou a pagar          
        //Se der erro em alguma Thread sai imediatamente.	
        oGrid:SetNoErrorStop(.T.)

        While (cAliasQry)->(!Eof()) 

            cFilTit := (cAliasQry)->GI6_FILRES

            If (cAliasQry)->GZG_TIPO == '1' //RECEITA
                cPrefixoR := PadR(Iif(!Empty((cAliasQry)->GZC_PREFIX),(cAliasQry)->GZC_PREFIX,"REC"), nE1_PREFIXO)
                cNum      := GetNextNum( 'SE1' , cPrefixoR, cParcelaR, cTipoR )
                cNatTitR  := Iif(!Empty((cAliasQry)->GZC_NATUR),(cAliasQry)->GZC_NATUR,cNatRet)
                oGrid:Go('INCLUI',(cAliasQry)->RECNO, cPrefixoR, cNum, cParcelaR, cTipoR,(cAliasQry)->GI6_CLIENT, (cAliasQry)->GI6_LJCLI,(cAliasQry)->GZC_GERTIT,cNatTitR,cFilTit) 
            Else //DESPESA
                cPrefixoP := PadR(Iif(!Empty((cAliasQry)->GZC_PREFID),(cAliasQry)->GZC_PREFID,"DSP"), nE2_PREFIXO) 
                cNum      := GetNextNum( 'SE2' , cPrefixoP, cParcelaP, cTipoP)
                cNatTitP  := Iif(!Empty((cAliasQry)->GZC_NATURD),(cAliasQry)->GZC_NATURD,cNatRet) 
                oGrid:Go('INCLUI',(cAliasQry)->RECNO, cPrefixoP, cNum, cParcelaP, cTipoP,(cAliasQry)->GI6_FORNEC, (cAliasQry)->GI6_LOJA,(cAliasQry)->GZC_GERTID,cNatTitP,cFilTit)                
            EndIf 
    ...
    Else 
        GZG->(DbSetOrder(1))
        If ! (GZG->(DbSeek(xFilial("GZG")+G59->G59_AGENCI+G59->G59_NUMFCH)))
            AnalisaProcs(.T.,.F.)
        EndIf 
    EndIf 
...
/*
{Protheus.doc} AnalisaProcs(lFechamento)
    Atualiza o registro posicionado conforme a opção do fechamento.
    type    Function
    author  José Carlos
    since   22/10/2025
    param   Nil
return
*/
Static Function AnalisaProcs(lFechamento,lRecDesp)
    Local aAreaAtu    := GetArea()
    Local cAliasQry   := Nil
    Local cOcorrencia := '0'
    Local lStatus     := G59->G59_STATUS

    Default lFechamento := .T.
    Default lRecDesp    := .T.
    ...
    If cAliasQry <> Nil .And. (cAliasQry)->(!Eof()) 
        cOcorrencia := '2'  //Inconsistência Financeira
    Else 
        cOcorrencia := Iif(lFechamento,'1',cOcorrencia)
    EndIf 
    ...
    If lFechamento .And. cOcorrencia == '1'
        GA500ConfFich("1")
    ElseIf !lFechamento .And. cOcorrencia == '0'
        GA500ConfFich("2")
    EndIf                         
    ...
    */
    If G59->(FieldPos('G59_STFINA')) > 0
        ...
        EndIf 
        RecLock('G59',.F.)
            G59->G59_STFINA := cOcorrencia
            G59->G59_STATUS := lStatus
        G59->(MsUnLock())
    EndIf 

    If !lFechamento // Abertura do fechamento
        RecLock('G59',.F.)
...
/*
{Protheus.doc} GetNextNum(cTabela, cPrefixo, cParcela, cTipo)
    Funcção de retorno de próximo número do título a receber ou a a pagar
    type    Function
    author  José Carlos
    since   22/10/2025
    param   cTabela SE1 ou SE2, 
            cPrefixo Prefixo do título, 
            cParcela Parcela , 
            cTipo Tipo do título
return  Caracter 
*/
Static Function GetNextNum(cTabela, cPrefixo, cParcela, cTipo)
    Local cNum      := ''
    Local cTitChave := ''
...
/*
{Protheus.doc} ProNewAbr()
    Função de processamento da abertura
    type    Function
    author  José Carlos
    since   22/10/2025
    param   Nil
...
Static Function ProNewAbr()
    ...
    If !(G59->G59_STATUS) 
        FWAlertInfo(STR0025, STR0018) //"Atenção" //"Este registro já está aberto!"
        Return .T. 
    EndIf  
    ...
    If lRet .AND. (IsBlind() .or. FWAlertYesNo(STR0024, STR0018)) //"Atenção" //"Toda a apuração desse período será zerada e deverá ser processada novamente, deseja prosseguir?"
        ...
        BeginSql Alias cAliasQry
            SELECT 
                GZG.R_E_C_N_O_ RECNO
                , CASE WHEN GZG.GZG_TIPO = '1' THEN GI6.GI6_CLIENT ELSE GI6.GI6_FORNEC END AS CODCLIFOR   
                , CASE WHEN GZG.GZG_TIPO = '1' THEN GI6.GI6_LJCLI  ELSE GI6.GI6_LOJA END AS LJCLIFOR                 
            FROM %Table:GZG% GZG 
            INNER JOIN %Table:GI6% GI6 
                ON GI6.GI6_FILIAL = %xFilial:GI6%
                AND GI6.GI6_CODIGO = GZG.GZG_AGENCI
            ...
            WHERE GZG.GZG_FILIAL = %xFilial:GZG%
                ...
                AND GZG.GZG_AGENCI = %Exp:G59->G59_AGENCI%
                AND GZG.GZG_NUMFCH = %Exp:G59->G59_NUMFCH%
                AND GZG.GZG_STATIT <> '0'
                AND GZG.%NotDel%
        EndSql
        ...
        If (cAliasQry)->(!Eof()) 
            ...
            oGrid:SetEnvironment( cEmpAnt , cFilAnt )    
            //Função para ser executada na Thread
            oGrid:Start("GTPA421D")    // Estorna titulos a receber ou a pagar          
            //Se der erro em alguma Thread sai imediatamente.	
            oGrid:SetNoErrorStop(.T.)

            While (cAliasQry)->(!Eof()) 
                oGrid:Go('ESTORNO',(cAliasQry)->RECNO,Nil,Nil,Nil,Nil,(cAliasQry)->CODCLIFOR,(cAliasQry)->LJCLIFOR,Nil,Nil,Nil)
                (cAliasQry)->(dbSkip())
            EndDo
        Else 
            GZG->(DbSetOrder(1))
...
/*
{Protheus.doc} ViewLegenda()
    Função apresentação da legenda 
    type    Function
    author  José Carlos
    since   22/10/2025
    param   Nil
return
*/
Static Function ViewLegenda(oPanel)
    Local oFontB := TFont():New('Consolas',, 16,, .T.,,,,, .F., .F.)
    Local oBPS1  := NIL
    Local oBPS2  := NIL
    oBPS1  := THButton():New(001, 080, STR0170 , oPanel, {|| LegTitulos()  }, 50, 10, oFontB, STR0171)   //"[Legenda]" "Consulta Legenda"
    oBPS2  := THButton():New(001, 160, STR0172 , oPanel, {|| GA500View(FWViewActive())  }, 80, 10, oFontB, STR0173) //"[Consulta Título]" "Informações do título"
Return Nil 

/*
{Protheus.doc} LegTitulos()
    Função de montagem de legenda 
    type    Function
    author  José Carlos
    since   22/10/2025
    param   Nil
return
*/
Static Function LegTitulos()
    Local aCores := {}
    aAdd(aCores, {"BR_BRANCO"   , STR0174})         //'Não processado'
    aAdd(aCores, {"BR_VERDE"	, STR0175})         //'Título gerado'
    aAdd(aCores, {"BR_AMARELO"	, STR0176})         //'Erro na geração'
    aAdd(aCores, {"BR_AZUL"		, STR0177})         //'Sem Vinc. Financ.'
    BrwLegenda('Receita/Despesa', STR0178, aCores)  //'Legenda'
Return 

/*
{Protheus.doc} GetThreads()
    Função de número de threads por parâmetro do módulo
    type    Function
    author  José Carlos
    since   22/10/2025
    param   Nil
...
Static Function GetThreads()
    Local nRetorno := GTPGetRules("NRTHIA421D",,,nMaxThreads) 
    If nRetorno > nMaxThreads
        nRetorno := nMaxThreads  
    ElseIf nRetorno == 0
        nRetorno := nMinThreads
    EndIf 
Return nRetorno 

Static Function SetIniFld(oModel,cConteudo)
    Local cRet     := 'BR_BRANCO'
    Local aLegenda :=  {{' ','BR_BRANCO'},{'0','BR_BRANCO'},{'1','BR_VERDE'},{'2','BR_AMARELO'},{'3','BR_AZUL'}}
    
    Default cConteudo := ''

    If !Empty(cConteudo)
        cRet := aLegenda[Ascan( aLegenda,{|e| e[1] == cConteudo})][2]
    Else 
        cRet := aLegenda[Ascan( aLegenda,{|e| e[1] == GZG->GZG_STATIT})][2]
    EndIf 
