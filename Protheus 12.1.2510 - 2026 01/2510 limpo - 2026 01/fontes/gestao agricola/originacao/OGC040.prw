	...
	BeginSql Alias cAliasDXD
		...
		SELECT DXD.DXD_FILIAL, DXD.DXD_CODIGO, DXD.DXD_CLACOM, DXD.DXD_SAFRA, DXP_ITECAD, DXP_FILIAL, DXP_CODIGO, DXP_STATUS, NJR_FILIAL, NJR_CODCTR, NJR_UM1PRO, NJR_UMPRC, NJR_VLRBAS, NJR_CODPRO, DXP.R_E_C_N_O_  DXPRECNO
		  ...
		  INNER JOIN %Table:NJR% NJR ON NJR.NJR_CODCTR = DXP.DXP_CODCTP
		  							 AND NJR.%notDel%
	    				             AND NJR.NJR_FILIAL =  %Exp:xFilial('NJR')%
	    WHERE DXD.%notDel%
		      %exp:cFiltro%
...
static function OGC040DB(oBrwObj, cBrwName)
	...
	case cBrwName == "B"	//blocos
		...
		DbSelectArea((__cTabFar))
		DbGoTop()
		If DbSeek((__cTabBlc)->DXD_FILIAL+(__cTabBlc)->DXD_SAFRA+(__cTabBlc)->DXD_CODIGO+(__cTabBlc)->DXP_CODIGO)
		...
		While !(__cTabFar)->(Eof())
			if (__cTabFar)->MARK == "1" //selecionado
			 	...
			 	If "2" $ (__cTabFar)->DXP_STATUS // Se forem fardos com take-up efetuado
			 		aAdd(aFardEmbar, {(__cTabFar)->DXI_FILIAL, (__cTabFar)->DXI_SAFRA, (__cTabFar)->DXI_ETIQ, "04", (__cTabFar)->DXI_CODIGO, _cFilCtr, (__cTabFar)->DXI_BLOCO} ) // Fardos para validação de instrução de embarque
			 		aAdd(aFardTkp, {(__cTabFar)->DXI_FILIAL, (__cTabFar)->DXI_SAFRA, (__cTabFar)->DXI_ETIQ, "02", _cCodCtr, _cFilCtr,(__cTabFar)->DXP_CODIGO,(__cTabFar)->DXI_BLOCO} )
			 	EndIf
