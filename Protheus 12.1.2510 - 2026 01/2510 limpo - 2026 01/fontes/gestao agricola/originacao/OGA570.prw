	...
	if _lAlgodao
		...
		for nA := 1 to oModelNN8:Length()
			 ...
			 For nB := 1 to oModelN8D:Length()
			     oModelN8D:GoLine( nB )
			     aAdd(aBlocos, {oModelN8D:GetValue("N8D_SEQVNC"), oModelN8D:GetValue("N8D_BLOCO") , fGetVinc(oModelNJR:GetValue("NJR_CODCTR"), oModelNN8:GetValue("NN8_ITEMFX"), oModelN8D:GetValue("N8D_SEQVNC") ),oModelN8D:GetValue("N8D_FILORG")})
			 next nB
...
static function fGetVinc(cCodCtr, cItemFx, cSeqVnc)
	...
	While (cAliasDXI)->(!Eof())
		...
		aAdd(aRecnos, {(cAliasDXI)->DXI_RECNO, (cAliasDXI)->DXI_FATURA, (cAliasDXI)->DXI_TIPPRE,(cAliasDXI)->DXI_FILIAL   })
