...
/*/{Protheus.doc} AGRO01MAPA
Gatilho chamado na alteração do Código Mapa
...
Function AGRO01MAPA()
    ...
    Local oNCR   := oModel:GetModel("FORMNCR")
    Local cNewMp := oNCR:GetValue("NCR_MAPA")
    Local nOperation := oModel:GetOperation()
    Local cPrincAtiv := AllTrim(oNCR:GetValue("NCR_PRATIV"))

    // Só age se houve alteração real e existir princípio ativo
    If nOperation == MODEL_OPERATION_UPDATE .AND. AllTrim(NCR->NCR_MAPA) != AllTrim(cNewMp) .AND. !Empty(NCR->NCR_PRATIV)

        MsgInfo(STR0009, STR0010) //"O campo Princípio Ativo será limpo pois o Código Mapa foi alterado."#"Aviso"
        // Limpa campo Principio Ativo
        cPrincAtiv := ""
    EndIf
