...
Static cMessage   := "AgriculturalInputFormulation"
Static cModelId   := "AGRPA020"
Static nTamCod    := TamSX3("NCI_CODIGO")[1]

/*/{Protheus.doc} AGRPI020
Função de integração com o adapter EAI para envio e recebimento do cadastro de
formulação (NCI) utilizando o conceito de mensagem única.
...
/*/
Function AGRPI020(cXml, cTypeTrans, cTypeMsg, cVersion, cTransac)
...
If (cTypeTrans == TRANS_SEND)
	If (cTypeMsg == EAI_MESSAGE_BUSINESS)
		...
		cValInt := oModel:GetValue('NCIMASTER', 'NCI_CODIGO')
		...
		cXMLRet += ' <InternalId>' + AP020IntId(nil, cValInt) + '</InternalId>'
		cXMLRet += ' <Code>' + _NoTags(RTrim(cValInt)) + '</Code>'
		If (oModel:GetOperation() <> MODEL_OPERATION_DELETE)
			cXMLRet += ' <Description>' + StrTran(_NoTags(RTrim(oModel:GetValue('NCIMASTER', 'NCI_OBS'))), CRLF, "&#x0d;&#x0a;") + '</Description>'
			cXMLRet += ' <ShortDescription>' + _NoTags(RTrim(oModel:GetValue('NCIMASTER', 'NCI_DESCRI'))) + '</ShortDescription>'
		Endif
		cXMLRet += '</BusinessContent>'
		oModel := nil
	Endif
...
ElseIf (cTypeTrans == TRANS_RECEIVE)
			...
			If upper(oXml:xPathGetNodeValue('/TOTVSMessage/ResponseMessage/ProcessingInformation/Status')) = "OK"
				...
				For nX := 1 to len(aIntID)
					...
					If cEvent = 'DELETE' .and. !empty(cValInt)
						CFGA070Mnt(cRefer, "NCI", "NCI_CODIGO", nil, cValInt, .T.)
					ElseIf !empty(cValInt) .and. !empty(cValExt)
						CFGA070Mnt(cRefer, "NCI", "NCI_CODIGO", cValExt, cValInt)
					Else
						lRet  := .F.
	...
	ElseIf (cTypeMsg == EAI_MESSAGE_BUSINESS)  // Chegada de mensagem de negócios.
		...
		If Empty(cErro := oXml:Error())
			...
			cValInt := RTrim(CFGA070Int(cRefer, "NCI", "NCI_CODIGO", cValExt))
			aValInt := StrToKarr2(cValInt, "|", .T.)

			// Verifica se encontrou uma chave no de/para.
			If len(aValInt) > 2
				NCI->(dbSetOrder(1))  // NCI_FILIAL, NCI_CODIGO.
				lFound := NCI->(dbSeek(xFilial(nil, aValInt[2]) + aValInt[3], .F.))
			Endif
					...
					If nMVCOper <> MODEL_OPERATION_DELETE
						...
						If nMVCOper == MODEL_OPERATION_INSERT
							...
							If !empty(cValInt)
								...
								NCI->(dbSetOrder(1))  // NCI_FILIAL, NCI_CODIGO.
								If NCI->(dbSeek(xFilial() + cValInt, .F.))
									cValInt := ""
								Endif
							Endif
							...
							If empty(cValInt)
								cValInt := GetSXENum('NCI', 'NCI_CODIGO')
							Endif

							// Atualiza o código no modelo.
							If oModel:GetValue('NCIMASTER', 'NCI_CODIGO') <> cValInt
								oModel:SetValue('NCIMASTER', 'NCI_CODIGO', cValInt)
							Endif

							cValInt := AP020IntId(nil, cValInt)
						Endif

						If oXml:XPathHasNode(cNodePath + 'Description')
							xValue := oXml:xPathGetNodeValue(cNodePath + 'Description')
							oModel:SetValue('NCIMASTER', 'NCI_OBS',    xValue)
						Endif

						If oXml:XPathHasNode(cNodePath + 'ShortDescription')
							xValue := oXml:xPathGetNodeValue(cNodePath + 'ShortDescription')
							oModel:SetValue('NCIMASTER', 'NCI_DESCRI', xValue)
						Endif
					Endif
...
@since   03/09/2081
/*/
Function AP020IntId(cFilOrig, cCodOrig)
Default cFilOrig := cFilAnt
Default cCodOrig := NCI->NCI_CODIGO
Return _NoTags(cEmpAnt + '|' + xFilial("NCI", cFilOrig) + '|' + cCodOrig)


/*/{Protheus.doc} AP020Cod
...
@since   21/09/2018
/*/
Function AP020Cod(cRefer, cValExt)
Local cValInt := RTrim(CFGA070Int(cRefer, "NCI", "NCI_CODIGO", cValExt))
