...
Static cMessage   := "AgriculturalInputMaintainer"
Static cModelId   := "AGRPA010"
Static nTamCod    := TamSX3("NC7_CODIGO")[1]

/*/{Protheus.doc} AGRPI010
Função de integração com o adapter EAI para envio e recebimento do cadastro de
mantenedores (NC7) utilizando o conceito de mensagem única.
...
/*/
Function AGRPI010(cXml, cTypeTrans, cTypeMsg, cVersion, cTransac)
...
If (cTypeTrans == TRANS_SEND)
	If (cTypeMsg == EAI_MESSAGE_BUSINESS)
		...
		cValInt := oModel:GetValue('NC7MASTER', 'NC7_CODIGO')
		...
		cXMLRet += ' <InternalId>' + AP010IntId(nil, cValInt) + '</InternalId>'
		cXMLRet += ' <Code>' + _NoTags(RTrim(cValInt)) + '</Code>'
		If (oModel:GetOperation() <> MODEL_OPERATION_DELETE)
			cXMLRet += ' <Description>' + StrTran(_NoTags(RTrim(oModel:GetValue('NC7MASTER', 'NC7_OBS'))), CRLF, "&#x0d;&#x0a;") + '</Description>'
			cXMLRet += ' <ShortDescription>' + _NoTags(RTrim(oModel:GetValue('NC7MASTER', 'NC7_DESCRI'))) + '</ShortDescription>'
		Endif
		cXMLRet += '</BusinessContent>'
		oModel := nil
	Endif
...
ElseIf (cTypeTrans == TRANS_RECEIVE)
			...
			If upper(oXml:xPathGetNodeValue('/TOTVSMessage/ResponseMessage/ProcessingInformation/Status')) = "OK"
				...
				For nX := 1 to len(aIntID)
					...
					If cEvent = 'DELETE' .and. !empty(cValInt)
						CFGA070Mnt(cRefer, "NC7", "NC7_CODIGO", nil, cValInt, .T.)
					ElseIf !empty(cValInt) .and. !empty(cValExt)
						CFGA070Mnt(cRefer, "NC7", "NC7_CODIGO", cValExt, cValInt)
					Else
						lRet  := .F.
	...
	ElseIf (cTypeMsg == EAI_MESSAGE_BUSINESS)  // Chegada de mensagem de negócios.
		...
		If Empty(cErro := oXml:Error())
			...
			cValInt := RTrim(CFGA070Int(cRefer, "NC7", "NC7_CODIGO", cValExt))
			aValInt := StrToKarr2(cValInt, "|", .T.)

			// Verifica se encontrou uma chave no de/para.
			If len(aValInt) > 2
				NC7->(dbSetOrder(1))  // NC7_FILIAL, NC7_CODIGO.
				lFound := NC7->(dbSeek(xFilial(nil, aValInt[2]) + aValInt[3], .F.))
			Endif
					...
					If nMVCOper <> MODEL_OPERATION_DELETE
						...
						If nMVCOper == MODEL_OPERATION_INSERT
							...
							If !empty(cValInt)
								...
								NC7->(dbSetOrder(1))  // NC7_FILIAL, NC7_CODIGO.
								If NC7->(dbSeek(xFilial() + cValInt, .F.))
									cValInt := ""
								Endif
							Endif
							...
							If empty(cValInt)
								cValInt := GetSXENum('NC7', 'NC7_CODIGO')
							Endif

							// Atualiza o código no modelo.
							If oModel:GetValue('NC7MASTER', 'NC7_CODIGO') <> cValInt
								oModel:SetValue('NC7MASTER', 'NC7_CODIGO', cValInt)
							Endif

							cValInt := AP010IntId(nil, cValInt)
						Endif

						If oXml:XPathHasNode(cNodePath + 'Description')
							xValue := oXml:xPathGetNodeValue(cNodePath + 'Description')
							oModel:SetValue('NC7MASTER', 'NC7_OBS',    xValue)
						Endif

						If oXml:XPathHasNode(cNodePath + 'ShortDescription')
							xValue := oXml:xPathGetNodeValue(cNodePath + 'ShortDescription')
							oModel:SetValue('NC7MASTER', 'NC7_DESCRI', xValue)
						Endif
					Endif
...
@since   28/08/2018
/*/
Function AP010IntId(cFilOrig, cCodOrig)
Default cFilOrig := cFilAnt
Default cCodOrig := NC7->NC7_CODIGO
Return _NoTags(cEmpAnt + '|' + xFilial("NC7", cFilOrig) + '|' + cCodOrig)


/*/{Protheus.doc} AP010Cod
...
@since   21/09/2018
/*/
Function AP010Cod(cRefer, cValExt)
Local cValInt := RTrim(CFGA070Int(cRefer, "NC7", "NC7_CODIGO", cValExt))
