...
Method FormatData( aListContent ) Class MovreadXGspMessageReader
	...
	For	nZ := 1 To nLenList
		...
		aDataJson := FWVetByDic(aDataJson, 'SE5')
...
Static Function GetChavMov(aCab as Array, nOpc as Numeric)
	...
	If nOpc == 7 //Transferência de Contas
		...
		aAdd(aRetrn1,{xFilial("FK5",cFilMov),cBncMov,cAgcMov,cCntMov,cDatMov,cNtzMov,cNroMov})
		aAdd(aRetrn1,{xFilial("FK5",cFilMov),cBncDst,cAgcDst,cCntDst,cDatMov,cNtzDst,cNroMov})	
    ...
    If !Empty(aChvMov)
		...
		If !(cTmp)->(Eof())
			If nOpc == 7  //Transferencia de Contas
				cRetFk5ID += STR0012 + (cTmp)->FK5_IDMOV//"Origem: "
				cRetFk5ID += " | " + STR0013 + BscTrsfP2(aChvMov[2]) //"Destino: "
			ElseIf nOpc == 8 //Estorno Transferência
