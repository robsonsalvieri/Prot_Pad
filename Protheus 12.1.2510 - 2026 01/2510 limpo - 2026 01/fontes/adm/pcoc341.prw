...
Local lProc 	  := (GetNewPar("MV_PCOCPRC","0")=="1") .And. FwBulk():CanBulk() //Procedure foi substituida por FWBulk
...
For nX := 1 to Len(aProcCube)
	...
	If lProc
		...
		For nZ := 1 TO Len(aQuery)
			...
			dbUseArea(.T.,"TOPCONN",TcGenQry(,,aQuery[nZ]),cTrb)
			(cTrb)->(dbGoTop())

			While (cTrb)->(!EOF())
				
				If Len(aFields) < 1
					aFields		:= (cTrb)->(DBStruct()) // Pega a estrutura do alias
					oBulkPCO:SetFields(aFields) // Define os campos que serใo atualizados
				EndIf

				If Len(aFields) > 0 
					cWhen := "" // Resetar a condi็ใo do cWhen
					For nI := 1 to nQtdVal
						// Montar a condi็ใo do cWhen de acordo com o nQtdVal 
						If nI > 1 
							cWhen += " .OR. "
						EndIf
						cWhen += "AKT_SLD" + StrZero(nI, 3) + " <> 0 "
					Next nI
					
					//Se algum valor estiver diferente de zero, adicionar na temporแria via FWBulk
					If (&((cTrb)->(cWhen)))
						aBkValues := {}
						// Monta o array com os valores
						For nI := 1 to Len(aFields)
							If (SubStr(aFields[nI][1], 1, 7) == "AKT_SLD" .And. aFields[nI][2] == "N") // Caso seja um campo de saldo (AKT_SLD001, AKT_SLD002 ...)
								// Passar por um Round() para evitar o estouro de tamanho de decimais na temporแria
								// Round com nPoint =6, pois os campos de saldo sao criados com tamanho de decimais fixo em 6 na PcoCriaTemp()
								aAdd(aBkValues, Round(&("(cTrb)->"+aFields[nI][1]), 6)) 
							Else	
								aAdd(aBkValues, &("(cTrb)->"+aFields[nI][1]))
							EndIf
						Next nI
						If Len(aBkValues) > 0
							If !oBulkPCO:AddData(aBkValues)
								//- quebra a execu็ใo com erro, pois nใo atualizou o registro
								UserException(oBulkPCO:GetError())
							EndIf
						EndIf 
					EndIf
				EndIf
		...
		// O oBulkPCO ้ aberto dentro do for, deve ser fechado e liberado dentro do for
		oBulkPCO:Destroy()
		FreeObj(oBulkPCO)
...
DBSelectArea(cAliasSld)
PcoLimpTemp(cAliasSld) // Na procedure dinโmica era realizado um DELETE FROM

oBulkFin := FWBulk():New(cAliasSld) // Inicializar o FWBulk
...
While (cTrb)->(!EOF()) 
	If Len(aFields) < 1
		aFields	:= (cTrb)->(DBStruct())
		oBulkFin:SetFields(aFields)
	EndIf
	If Len(aFields) > 0
		aBkValues := {} // Resetar o array de inser็ใo do Bulk
		...
		Next nI 
		If Len(aBkValues) > 0
			If !oBulkFin:AddData(aBkValues)
				//- quebra a execu็ใo com erro, pois nใo atualizou o registro
				UserException(oBulkFin:GetError())
			EndIf
		EndIf
	EndIf
	(cTrb)->(dbSkip())
