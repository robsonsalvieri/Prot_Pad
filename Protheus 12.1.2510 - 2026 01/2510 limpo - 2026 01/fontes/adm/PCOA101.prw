...
#Define LIMITE_CONTAS  15000 //Limite de contas para não apresentar a pré visualização da árvore na importação em lote
...
If Pco101X1( "PCO101", @cCtOrcDe, @cCtOrcAte, @nCsdBloq )  //Pergunte( "PCO101", .T. ) 
	...
	FwMSgRun(,{||lRet:= Pco101Tree(cAliasTrb, aStructCta) },STR0024,STR0025) //caso confirma a tela retorna .T. senao retorna .F. #"Carregando árvore de contas."
	If lRet	
		Processa({||Pco101Proc(cAliasTrb, aStructCta)},STR0026) //#"Gravando Planilha"     
	Else
		lRet := .F.
	EndIf
...
If nQuant > LIMITE_CONTAS
	MsgAlert(STR0027+cValtoChar(LIMITE_CONTAS)+ STR0028,STR0029)// #"A importação contém mais de " #" contas. Para otimizar o desempenho, a pré-visualização da árvore de contas não será exibida."  #"Atenção"  
	oTree:Reset()
	TSay():New(007,007,{|| STR0027+cValtoChar(LIMITE_CONTAS)+ STR0028} ,oPanel,,oFont,,,,.T.,,,,)
EndIf
...
Local lRet	:= .T.
	...
	If (lRet := Aviso(STR0022, STR0023,{STR0013, STR0014},2) == 1) //"Confirma inclusão ?"###"Confirma a inclusao das contas selecionadas na planilha orçamentaria ?"###"Sim"###"Nao"
		...
		dbSelectArea("AK3")
		dbSetOrder(1)
		ProcRegua(Len(aStructCta))
		For nX := 2 TO Len(aStructCta)
			IncProc(STR0030+aStructCta[nX][1]) //"Gravando: "          
