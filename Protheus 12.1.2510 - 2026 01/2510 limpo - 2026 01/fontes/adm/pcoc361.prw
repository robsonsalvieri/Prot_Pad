...
Local aCposQry := {}
Local cCampos := ""
	Local lProc :=  lMovimento .And. ( SuperGetMV("MV_PCOCPRC",.T.,"0") == "1" ) 
	Local cTrb		:= ""
	Local aFields 	  := {}
	Local bExc
	Local nFor		:= 0
	Private oBkPc361		:= FWBulk():CanBulk()
...
aQuery := PcoCriaQry( cCodCube, nNivel, nMoeda, cArqAS400, If(nSerie==1, nQtdVal/nQtdSerie, nQtdVal), aDataSld, aQryDim, ""/*cWhere*/, cWhereTpSld, oStructCube:nNivTpSld, lMovimento, aDataIni, .T./*lAllNiveis*/, /*aCposNiv*/, /*lDebito*/, /*lCredito*/, lProc, aCposQry )
	...
	If lProc
		For nZ := 1 TO Len(aQuery)
			...
			if (cTrb)->(!EOF()) .and. len(aFields)<=0
				cCampos := ""
				aFields	:= (cTrb)->(DBStruct())
				oBkPc361:setFields(aFields)
				For nFor := 1 to len(aFields)
					If (SubStr(aFields[nFor][1], 1, 7) == "AKT_SLD" .And. aFields[nFor][2] == "N")
				 		cCampos += "(Round("+alltrim(aFields[nFor][1])+",6)), "
					else
						cCampos += alltrim(aFields[nFor][1])+", "
					endif
				NExt nFor
				cCampos := substr(cCampos,1,len(cCampos)-2)
			endif
			if len(aFields)>0
				bExc	:= &("{|| iif((cTrb)->(!oBkPc361:AddData({"+cCampos+"})),UserException(oBkPc361:GetError()),nil) }")
				(cTrb)->(DbEval(bExc))
			endif
			(cTrb)->(dbclosearea())
		Next nZ
		...
		If !oBkPc361:Close()
			...
			UserException(oBkPc361:GetError())
		EndIF
		oBkPc361:Destroy()
		FreeObj(oBkPc361)
	Else	
