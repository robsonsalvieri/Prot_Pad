	...
	While (cAliasFJZ)->(!Eof())
		...
		If lConstituicao .Or. lReversao
			...
			If !Empty((cAliasFJZ)->(E1_NUMLIQ)) .And. lConstituicao
				...
				//Ponto de entrada para que traga ou não títulos em tela.
				//Verifica ponto de entrada que, caso o retorno seja falso, não trará o título em tela.
				If __lF645LIQ
					lRetLiq := ExecBlock("F645LIQ", .F., .F., {cFilAux, cPrefixo, cNumero, cParcela, cTipo, cCliente, cLoja})
					//Valida se o retorno é lógico e se é falso
					If ValType(lRetLiq) == "L" .AND. !lRetLiq 
						(cAliasFJZ)->(dbSkip())
						Loop
					EndIf
				EndIf
		...
		If (cAliasFJZ)->E1_TIPO $ MVABATIM
			...
			If __lFK7IDPAI
				lTituloPai := cTitPai ==  AllTrim((cAliasFJZ)->FK7_IDPAI)
			Else
				lTituloPai := cTitPai == AllTrim((cAliasFJZ)->E1_TITPAI)
			Endif

			If lReversao .and. lTituloPai
				(cAliasFJZ)->(dbSkip())
				Loop
			EndIF
...
Static Function FA645TiRev(cFilCli, cCliente, cLoja, lVerBas) As Character
	...
	cQuery += " E1_SDACRES,E1_SDDECRE,E1_MOEDA,'1' TIPO  "
	If __lFK7IDPAI
		cQuery += " ,FK7_IDPAI "
	Else
		cQuery += " ,E1_TITPAI "
	...
	If __lFK7IDPAI
		...
		cQuery += " AND FK7.FK7_FILTIT = E1_FILIAL "
		cQuery += " AND FK7.FK7_PREFIX = E1_PREFIXO "
		cQuery += " AND FK7.FK7_NUM = E1_NUM "
		cQuery += " AND FK7.FK7_PARCEL = E1_PARCELA "
		cQuery += " AND FK7.FK7_TIPO = E1_TIPO "
		cQuery += " AND FK7.FK7_CLIFOR = E1_CLIENTE "
		cQuery += " AND FK7.FK7_LOJA = E1_LOJA "
	...
	cQuery += "   E1_TIPO, E1_SITUACA ,FJZ_PROC, FJZ_QTDATR,FJZ_SITUAC,FJZ_SLDBRT,FJZ_SITPAI, "
	cQuery += "   FJZ_SITPDD , FJZ_SALDO , FJZ_VENCTO ,FJZ_VENCRE,FJZ_VALOR,FJZ_QTDARE, "
	cQuery += "   E1_SDACRES,E1_SDDECRE,E1_MOEDA,'2' TIPO "
	If __lFK7IDPAI
		cQuery += " ,FK7_IDPAI "
	Else
		cQuery += " ,E1_TITPAI "
	...
	If __lFK7IDPAI
		cQuery += "   AND FK7.FK7_IDPAI IN "
		cQuery += "   ( "
		cQuery += " SELECT FK7F.FK7_IDDOC "
	Else
	...
	If __lFK7IDPAI
		cQuery += " INNER JOIN " + RetSqlName("FK7") + " FK7F "
		cQuery += " ON FK7F.FK7_ALIAS =  'SE1' "
		cQuery += " AND FK7F.FK7_FILTIT = E1_FILIAL "
		cQuery += " AND FK7F.FK7_PREFIX = E1_PREFIXO "
		cQuery += " AND FK7F.FK7_NUM = E1_NUM "
		cQuery += " AND FK7F.FK7_PARCEL = E1_PARCELA "
		cQuery += " AND FK7F.FK7_TIPO = E1_TIPO "
		cQuery += " AND FK7F.FK7_CLIFOR = E1_CLIENTE "
		cQuery += " AND FK7F.FK7_LOJA = E1_LOJA "
		cQuery += " AND FK7F.D_E_L_E_T_ = ' ' "
	EndIf 
	...
	cQuery += "   ) "
	If __lFK7IDPAI
		cQuery += " ORDER BY E1_FILORIG,E1_PREFIXO, E1_NUM, E1_PARCELA,FK7_IDPAI, E1_TIPO "
	Else
...
Default lVerBas		:= .F.
	...
	If MV_PAR13 == 2 // Regra padrão desconsiderando renegociados
		...
		cQuery += " '1' TIPO, E1_VENCORI " + CRLF
		If __lFK7IDPAI
			cQuery += ", FK7.FK7_IDPAI " + CRLF
		Else
			cQuery += ", E1_TITPAI " + CRLF
		EndIf
		...
		If __lFK7IDPAI
			...
			cQuery += " ON FK7.FK7_ALIAS =  'SE1' " + CRLF
			cQuery += " AND FK7.FK7_FILTIT = SE1PAI.E1_FILIAL " + CRLF
			cQuery += " AND FK7.FK7_PREFIX = SE1PAI.E1_PREFIXO " + CRLF
			cQuery += " AND FK7.FK7_NUM = SE1PAI.E1_NUM " + CRLF
			cQuery += " AND FK7.FK7_PARCEL = SE1PAI.E1_PARCELA " + CRLF
			cQuery += " AND FK7.FK7_TIPO = SE1PAI.E1_TIPO " + CRLF
			cQuery += " AND FK7.FK7_CLIFOR = SE1PAI.E1_CLIENTE " + CRLF
			cQuery += " AND FK7.FK7_LOJA = SE1PAI.E1_LOJA " + CRLF
		...
		If __lFK7IDPAI
			cQuery += " INNER JOIN " + RetSqlName("FK7") + " FK7 " + CRLF
			cQuery += " ON FK7.FK7_ALIAS =  'SE1' " + CRLF
			cQuery += " AND FK7.FK7_FILTIT = SE1ABT.E1_FILIAL " + CRLF
			cQuery += " AND FK7.FK7_PREFIX = SE1ABT.E1_PREFIXO " + CRLF
			cQuery += " AND FK7.FK7_NUM = SE1ABT.E1_NUM " + CRLF
			cQuery += " AND FK7.FK7_PARCEL = SE1ABT.E1_PARCELA " + CRLF
			cQuery += " AND FK7.FK7_TIPO = SE1ABT.E1_TIPO " + CRLF
			cQuery += " AND FK7.FK7_CLIFOR = SE1ABT.E1_CLIENTE " + CRLF
			cQuery += " AND FK7.FK7_LOJA = SE1ABT.E1_LOJA " + CRLF
			cQuery += " AND FK7.D_E_L_E_T_ = ' ' " + CRLF
		EndIf 
		cQuery += " WHERE " + CRLF
		...
		If __lFK7IDPAI
			cQuery += "   AND FK7.FK7_IDPAI IN " + CRLF
			cQuery += "   ( " + CRLF
			cQuery += " SELECT FK7F.FK7_IDDOC " + CRLF
		Else
		...
		cQuery += " FRV.D_E_L_E_T_ = ' ' " + CRLF
		If __lFK7IDPAI
			cQuery += " INNER JOIN " + RetSqlName("FK7") + " FK7F " + CRLF
			cQuery += " ON FK7F.FK7_ALIAS =  'SE1' " + CRLF
			cQuery += " AND FK7F.FK7_FILTIT = SE1SUB.E1_FILIAL " + CRLF
			cQuery += " AND FK7F.FK7_PREFIX = SE1SUB.E1_PREFIXO " + CRLF
			cQuery += " AND FK7F.FK7_NUM = SE1SUB.E1_NUM " + CRLF
			cQuery += " AND FK7F.FK7_PARCEL = SE1SUB.E1_PARCELA " + CRLF
			cQuery += " AND FK7F.FK7_TIPO = SE1SUB.E1_TIPO " + CRLF
			cQuery += " AND FK7F.FK7_CLIFOR = SE1SUB.E1_CLIENTE " + CRLF
			cQuery += " AND FK7F.FK7_LOJA = SE1SUB.E1_LOJA " + CRLF
			cQuery += " AND FK7F.D_E_L_E_T_ = ' ' " + CRLF
		EndIf 
		cQuery += " WHERE " + CRLF
		...
		If __lFK7IDPAI
			cQuery += " ORDER BY E1_FILORIG,E1_PREFIXO, E1_NUM, E1_PARCELA,FK7_IDPAI, E1_TIPO " + CRLF
		Else
	...
	Else //Regra ANS
		...
		If __lFK7IDPAI
			cQuery += " INNER JOIN " + RetSqlName("FK7") + " FK7F " + CRLF
			cQuery += " ON FK7F.FK7_ALIAS =  'SE1' " + CRLF
			cQuery += " AND FK7F.FK7_FILTIT = E1_FILIAL " + CRLF
			cQuery += " AND FK7F.FK7_PREFIX = E1_PREFIXO " + CRLF
			cQuery += " AND FK7F.FK7_NUM = E1_NUM " + CRLF
			cQuery += " AND FK7F.FK7_PARCEL = E1_PARCELA " + CRLF
			cQuery += " AND FK7F.FK7_TIPO = E1_TIPO " + CRLF
			cQuery += " AND FK7F.FK7_CLIFOR = E1_CLIENTE " + CRLF
			cQuery += " AND FK7F.FK7_LOJA = E1_LOJA " + CRLF
			cQuery += " AND FK7F.D_E_L_E_T_ = ' ' " + CRLF
		EndIf 
		cQuery += " WHERE " + CRLF
...
While (cAliasFJZ)->(!Eof())
	...
	If lConstituicao 
		If !Empty((cAliasFJZ)->(E1_NUMLIQ))
			...
			If __lF645LIQ
				lRetLiq := ExecBlock("F645LIQ", .F., .F., {cFilAnt, (cAliasFJZ)->E1_PREFIXO, (cAliasFJZ)->E1_NUM, (cAliasFJZ)->E1_PARCELA, (cAliasFJZ)->E1_TIPO, (cAliasFJZ)->E1_CLIENTE, (cAliasFJZ)->E1_LOJA})
				//Valida se o retorno é lógico e se é falso
				If ValType(lRetLiq) == "L" .AND. !lRetLiq 
					(cAliasFJZ)->(dbSkip())
					Loop
...
//-------------------------------------------------------------------
/*/{Protheus.doc} VldUsoBillRel
  Define se pode ser utilizado o relacionamento da FK7_IDPAI através do campo E1_TITPAI
  @return lRet as Logical
  @author Wesley França
  @since 23/12/2025
/*/
//-------------------------------------------------------------------
Static Function VldUsoBillRel()
	local lRet as Logical 
