...
Method CalcRatIr(nBaseTit) Class FinBCRateioIR
	...
	// Rateio p/ CPF
		For nX := 1 To Len(::aRatIRF)
			...
			If ::aRatIRF[nX][6] > 0 .And. FindFunction("FIrfRedIse")
				nBasRedIse := IIf(::aRatIRF[nX][10] > 0, ::aRatIRF[nX][10], ::aRatIRF[nX][nPosBase])
				::aRatIRF[nX][6]  := FIrfRedIse(nBasRedIse, ::aRatIRF[nX][6])
			EndIf
