...
If lDocEntr .And. FWSX3Util():GetFieldType("N1_IDTRIB" ) == 'C' .AND. !oSN1:HasField("N1_IDTRIB") //Adiciona o campo N1_IDTRIB, caso nao esteja na estrutura
	oSN1:AddField(FWX3Titulo("N1_IDTRIB"),FWX3Titulo("N1_IDTRIB"),"N1_IDTRIB","C",TamSX3("N1_IDTRIB")[1],TamSX3("N1_IDTRIB")[2],,,,.F.,,.F.,.F.,.F.,,)
EndIf
...
If cPaisLoc <> "BRA" .And. ExistBlock("AF012TOK")
	oSN1 := oModel:GetModel("SN1MASTER")
	lDocEntr := ExecBlock("AF012TOK",.F.,.F., {oSN1, oAux, lDocEntr})
EndIf
...
Local nQuantas	  := AtfMoedas()
Local lDocEntr    As Logical
Local lVlAquis    As Logical

lDocEntr	      := FwIsInCallStack("MATA103") .Or. FWisincallstack('ATFJob103') .Or. FWIsInCallStack("MATA101") .Or.;
					 FWIsInCallStack("MATA116")  .Or. FWIsInCallStack("MATA119") .Or. CallsOfEst() .or.  (FindFunction("CallsMI").and. CallsMI() );
					.or. (FWIsInCallStack("A143GERANF") .and. cPaisLoc=="ARG"  )

lVlAquis := (lMile .OR. __lAtfAuto) .AND. oModel:GetValue("SN1MASTER","N1_VLAQUIS") != 0 //Variável para não sobrepor o N1_VLAQUIS se informado no execauto/mile
...
Else
	...
	If lRet
		...
		If !lVlAquis .AND. (oAux:GetValue('N3_TIPO') == "01" .And. nOper == MODEL_OPERATION_INSERT .And. !__lClassifica);
...
If lRet
	...
	If !lDetail // Temos uma limitação técnica que só podemos carregar até 999 componentes visuais em tela, sendo assim, caso o F12 Exibe painel de detalhes esteja com SIM, ocultamos a opção de Salvar e Incluir Novo
	...
	Else
		nRet := FWExecView( cTitulo , cPrograma, nOperation, /*oDlg*/, {|| .T. } ,{ |oModel|AF012ATDOK(oModel)}/*bOk*/ , /*nPercReducao*/, /*aEnableButtons*/, /*bCancel*/ , /*cOperatId*/, /*cToolBar*/)
	EndIf
	__lIncluMan  	:= .F.
