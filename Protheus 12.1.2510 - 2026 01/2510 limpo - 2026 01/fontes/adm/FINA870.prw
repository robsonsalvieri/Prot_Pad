...
Function FIN870INC(cAlias,nReg,nOpc,aFil870)
	...
	cQryFilSA2 := " ON ISNULL(SUBSTRING(SE2.E2_FILORIG, 1, " + cValToChar(nMenorFil) + "),' ') = ISNULL(SUBSTRING(SA2.A2_FILIAL, 1, " + cValToChar(nMenorFil) + "),' ') "		  	    
	...
	cQuery := "SELECT SE2.E2_FILIAL, SE2.E2_EMISSAO, SE2.E2_PREFIXO, SE2.E2_NUM, SE2.E2_PARCELA, SE2.E2_TIPO, SE2.E2_SALDO, SE2.E2_EMISSAO, "
	If __nTpAglut == 1		//TIPO TITULO+TIPO FORNECEDOR+COD.RET+CNPJ
		cQuery += "  SE2.E2_RETINS, SE2.E2_CNPJRET, SA2.A2_TIPO TPFORN"
	ElseIf __nTpAglut == 2	//TIPO TITULO+TIPO FORNECEDOR+COD.RET
		cQuery += "  SE2.E2_RETINS, '' E2_CNPJRET , SA2.A2_TIPO TPFORN"
	...
	ElseIf __nTpAglut == 4	//TIPO TITULO+COD.RET
		cQuery += "  SE2.E2_RETINS, '' E2_CNPJRET, '' TPFORN"
	ElseIf __nTpAglut == 5	//TIPO TITULO
		cQuery += " '' E2_RETINS, '' E2_CNPJRET, '' TPFORN"
	EndIf
	cQuery += ", SE2.E2_VALOR, SE2.E2_FORNECE, SE2.E2_LOJA, SE2.E2_FILORIG "	
	cQuery += "FROM " + RetSqlName('SE2') +" SE2 "
	cQuery += "INNER JOIN " + RetSqlName('SA2') +" SA2 "
	cQuery +=  cQryFilSA2	

	If !__lUseClas
		cQuery += " AND SA2.A2_COD = SUBSTRING( SE2.E2_TITPAI ," + Str(nTamTit) + ","+ Str(nTamFor)+ ")"
		cQuery += " AND SA2.A2_LOJA = SUBSTRING( SE2.E2_TITPAI ," + Str(nTamTit+nTamFor)+","+ Str(nTamLj)+ ")"	
	Else
		cQuery += " INNER JOIN " + RetSqlName("FK7") + " FK7F ON"
		cQuery += "     SE2.D_E_L_E_T_ = FK7F.D_E_L_E_T_"
		cQuery += "	AND FK7F.FK7_ALIAS = 'SE2'"
		cQuery += " AND SE2.E2_FILIAL =  FK7F.FK7_FILTIT"
		cQuery += " AND SE2.E2_PREFIXO = FK7F.FK7_PREFIX"
		cQuery += " AND SE2.E2_NUM     = FK7F.FK7_NUM"
		cQuery += " AND SE2.E2_PARCELA = FK7F.FK7_PARCEL"
		cQuery += " AND SE2.E2_TIPO    = FK7F.FK7_TIPO"
		cQuery += " AND SE2.E2_FORNECE = FK7F.FK7_CLIFOR"
		cQuery += " AND SE2.E2_LOJA    = FK7F.FK7_LOJA"

		cQuery += " INNER JOIN " + RetSqlName("FK7") + " FK7P ON"
		cQuery += "     FK7F.D_E_L_E_T_ = FK7P.D_E_L_E_T_"
		cQuery += " AND FK7F.FK7_FILIAL = FK7P.FK7_FILIAL"
		cQuery += " AND FK7F.FK7_IDPAI  = FK7P.FK7_IDDOC"
		cQuery += " AND SA2.A2_COD = FK7P.FK7_CLIFOR"
		cQuery += " AND SA2.A2_LOJA = FK7P.FK7_LOJA"
	...
	If MV_PAR04 == 1 //Seleciona Filial.				
		cQuery += "SE2.E2_FILIAL " + GetRngFil(aSelFil,"SE2")+" AND "		
	Else
		cQuery += "SE2.E2_FILIAL = '" + XFilial("SE2") + "' AND "
	Endif
	// Trata liberacao para baixa
	If lVerLibBx
		cQuery += "(SE2.E2_DATALIB <> ' ' "
		cQuery += "OR (SE2.E2_SALDO+SE2.E2_SDACRES-SE2.E2_SDDECRE<="+ALLTRIM(STR(nVlMinLib,17,2))+")) AND "
	Endif
	cQuery += "SE2.E2_TIPO IN ('INS','INA', 'INP') AND "
	cQuery += "SE2.E2_SALDO > 0 AND "
	cQuery += "SE2.E2_ORIGEM <> 'FINA870 ' AND "
	cQuery += "SE2.E2_RETINS <> ' ' AND "
	cQuery += "SE2.E2_EMISSAO BETWEEN '" + DTOS(MV_PAR01) + "' AND '" + DTOS(MV_PAR02) + "' AND "
	cQuery += "SE2.D_E_L_E_T_ = ' ' "	
	If __nTpAglut == 1 
		cQuery += "ORDER BY SE2.E2_FILIAL, SE2.E2_TIPO, SE2.E2_CNPJRET,  SE2.E2_RETINS"
	Else		
		cQuery += "ORDER BY SE2.E2_TIPO, TPFORN, SE2.E2_CNPJRET, SE2.E2_RETINS,  SE2.E2_FILORIG"	
	EndIf	 	
...
Function FIN870CAN(cAlias as Char, nRec as Numeric, nOpc as Numeric, lAutomato as Logical)
	...
	If lRet .AND. aRet[1] = 2
		...
		If FWM->(DbSeek(cFilSE2+cChaveSE2))
			...
			cQry += "FK7.FK7_IDDOC 	= FWM.FWM_FK7DES "
			cQry += "AND FK7.D_E_L_E_T_ = ? "
			cQry += "INNER JOIN " + RetSqlName('SE2') +" SE2 ON "
			cQry += "SE2.E2_FILIAL = FK7.FK7_FILTIT	"
			cQry += "AND SE2.E2_PREFIXO = FK7.FK7_PREFIX	"
			cQry += "AND SE2.E2_NUM = FK7.FK7_NUM	"
			cQry += "AND SE2.E2_PARCELA = FK7.FK7_PARCEL	"
			cQry += "AND SE2.E2_TIPO = FK7.FK7_TIPO	"
			cQry += "AND SE2.E2_FORNECE = FK7.FK7_CLIFOR	"
			cQry += "AND SE2.E2_LOJA = FK7.FK7_LOJA	"	
			...
			oStateQry := FwExecStatement():New(cQry)

			oStateQry:SetString(nSeq++, ' ')	// FK7.D_E_L_E_T_ 
			oStateQry:SetString(nSeq++, ' ')	// SE2.D_E_L_E_T_
			oStateQry:SetString(nSeq++, FWM->FWM_PROCES) // FWM_PROCES
			oStateQry:SetString(nSeq++, '1')	// FWM_STATUS
			oStateQry:SetString(nSeq++, ' ')	// FWM.D_E_L_E_T_
			
			cAlsQry := oStateQry:OpenAlias(GetNextAlias())
	...
	If lRet
		...
		If FWM->(DbSeek(cFilSE2+cChaveSE2))
			...
			cQry2 += "FK7.FK7_IDDOC  = FWM.FWM_FK7ORI "
			cQry2 += "AND FK7.D_E_L_E_T_  = ? "
			cQry2 += "INNER JOIN " + RetSqlName('SE2') +" SE2 ON "
			cQry2 += "SE2.E2_FILIAL = FK7.FK7_FILTIT	"
			cQry2 += "AND SE2.E2_PREFIXO = FK7.FK7_PREFIX	"
			cQry2 += "AND SE2.E2_NUM = FK7.FK7_NUM	"
			cQry2 += "AND SE2.E2_PARCELA = FK7.FK7_PARCEL	"
			cQry2 += "AND SE2.E2_TIPO = FK7.FK7_TIPO	"
			cQry2 += "AND SE2.E2_FORNECE = FK7.FK7_CLIFOR	"
			cQry2 += "AND SE2.E2_LOJA = FK7.FK7_LOJA	"
			cQry2 += "AND SE2.D_E_L_E_T_ = ? "
			cQry2 += "WHERE "
			cQry2 += "FWM_PROCES = ? "
			If aRet[1] = 1
				cQry2 += "AND FWM_SUBPRO =  ? "
			Endif
			cQry2 += "AND FWM_STATUS = ? "
			cQry2 += "AND FWM.D_E_L_E_T_  = ? "
			cQry2 := ChangeQuery(cQry2)
			
			// Criação do ExecStatement
			oStatQry2 := FwExecStatement():New(cQry2)

			oStatQry2:SetString(nParam++, ' ')	// FK7.D_E_L_E_T_ 
			oStatQry2:SetString(nParam++, ' ')	// SE2.D_E_L_E_T_
			oStatQry2:SetString(nParam++, FWM->FWM_PROCES) // FWM_PROCES
			If aRet[1] = 1
				oStatQry2:SetString(nParam++, FWM->FWM_SUBPRO) //FWM_SUBPRO
			EndIf
			oStatQry2:SetString(nParam++, '1')	// FWM_STATUS
			oStatQry2:SetString(nParam++, ' ')	// FWM.D_E_L_E_T_

			cAlsQry2 := oStatQry2:OpenAlias(GetNextAlias())
		Endif
	Endif
...
Static Function LoadFWM(oGridModel As Object, lCopia As Logical) As Array 
	...
	cQry += " WHERE "
	cQry += " FWM.FWM_FILIAL = ? "
	cQry += " AND FK7DES.FK7_FILTIT = ? "
	cQry += " AND FK7DES.FK7_PREFIX = ? "
	cQry += " AND FK7DES.FK7_NUM =  ? "
	cQry += " AND FK7DES.FK7_PARCEL = ? "
	cQry += " AND FK7DES.FK7_TIPO = ? "
	cQry += " AND FK7DES.FK7_CLIFOR = ? "
	cQry += " AND FK7DES.FK7_LOJA = ? "
	cQry += " AND FWM.D_E_L_E_T_ = ? "
	cQry := ChangeQuery(cQry)
	...
	oStateQry:SetString(nSeq++, ' ')	// FK7DES.D_E_L_E_T_
	oStateQry:SetString(nSeq++, ' ')	// FK7ORI.D_E_L_E_T_
	oStateQry:SetString(nSeq++, ' ')	// SE2.D_E_L_E_T_
	oStateQry:SetString(nSeq++, xFilial("FWM")) // FWM_FILIAL
	oStateQry:SetString(nSeq++, xFilial("SE2",SE2->E2_FILORIG)) // FK7DES.FK7_FILTIT
	oStateQry:SetString(nSeq++, SE2->E2_PREFIXO) // FK7DES.FK7_PREFIX 
	oStateQry:SetString(nSeq++, SE2->E2_NUM) 	 // FK7DES.FK7_NUM
	oStateQry:SetString(nSeq++, SE2->E2_PARCELA) // FK7DES.FK7_PARCEL
	oStateQry:SetString(nSeq++, SE2->E2_TIPO) 	 // FK7DES.FK7_TIPO
	oStateQry:SetString(nSeq++, SE2->E2_FORNECE) // FK7DES.FK7_CLIFOR 
	oStateQry:SetString(nSeq++, SE2->E2_LOJA) 	 // FK7DES.FK7_LOJA  
	oStateQry:SetString(nSeq++, ' ')	// FWM.D_E_L_E_T_

	cAlias := oStateQry:OpenAlias(GetNextAlias())
