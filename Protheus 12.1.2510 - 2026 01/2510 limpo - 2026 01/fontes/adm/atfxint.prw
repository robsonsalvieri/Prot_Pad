	...
	If lContinua
		...
		For nx := 1 to len(arrayCTE[3])  //ITERANDO BENS COM CTE PASSADOS PELO TIME DE SUPRIMENTOS
			...
			if cStart <= cItemCTE
				...
				If SN3->( DbSeek(xFilial("SN3") + cBaseCTE + cStart + "01" + "0" + "001" ) ) .OR. ;
				   SN3->( DbSeek(xFilial("SN3") + cBaseCTE + cStart + "03" + "0" + "001" ) )  //Posicionando na SN3 para busca e incorporacao do CTE
				...
				If lGrvSN3 .AND. SN1->( DbSeek(xFilial("SN1") + cBaseCTE + cStart ) )
...
lAFCfgTrib	:= (SD1->(FieldPos("D1_IDTRIB")) > 0 .AND. !Empty(SD1->D1_IDTRIB)) .AND. !Empty(SuperGetMv("MV_VLATFCT",.F.,"")) .AND. FindFunction('backoffice.stock.calculationcost.CalculationNFETrbGen',.T.) .And. FindFunction("ATFVlCGTrb") .AND. ;
					( FindFunction("backoffice.stock.calculationcost.HasRulesCust",.T.) .AND. backoffice.stock.calculationcost.HasRulesCust() )
...
If SD1->(Dbseek(cBuscaSD1))
	...
	If cOper == "1" // Inclusão
		...
		If lAFCfgTrib .AND. FN0->( FieldPos("FN0_IDTRIB") ) > 0 
			FN0->FN0_IDTRIB := SD1->D1_IDTRIB
		EndIf 
