...
Static _lALAANALIT  := NIL
Static _lALANIVEL   := NIL
Static _lALAATUSIN  := NIL
Static lPcoa050		:= .F.
Static lSint2Plan	:= Nil
...
*/
Function PcoAtuSld(cTipoMov,cAliasAKD,aValor,dData,cConfigDe,cConfigAte,lReproc,lForcaAtu,lAtuAfter)
...
lPcoa050	:= FwIsInCallStack("A050OK")
...
Static Function Pco_PutQueue(aJobs, cTipoMov, dData, cReproc, aValor, cFilAnt, nRecAKD,cNivel,lAnalit,cAtuSin)
...
If _lALAANALIT == Nil .or. _lALANIVEL == Nil .or. _lALAANALIT == Nil
	_lALAANALIT  := ALA->( FieldPos("ALA_ANALIT") ) > 0
	_lALANIVEL   := ALA->( FieldPos("ALA_NIVEL")  ) > 0
	_lALAATUSIN  := ALA->( FieldPos("ALA_ATUSIN") ) > 0
Endif
...
If lPcoa050 
	nPos := AScan(aCubosTask, aJobs[1])
	If nPos <= 0
		AAdd(aCubosTask,aJobs[1])
	EndIf
Else
	oTask := totvs.protheus.backoffice.control.general.Schedule():New()
...
EndIf
If lSint2Plan ==Nil 
	lSint2Plan := SuperGetMV("MV_PCOSINF",.F.,"0") == "1"
EndIf
			...
			While !Eof() .And. AKW->AKW_FILIAL+AKW->AKW_COD==xFilial()+cConfig .And. (Empty(cNivSint) .Or.cNivSint == AKW->AKW_NIVEL)
				...
				If !Empty(AKW->AKW_ATUSIN)
					...
					If !Empty(cContaSint)
						...
						If lPcoa050 .and. lSint2Plan
							Pco_PutQueue( aClone({cConfig, cChavSin }), cTipoMov, dData, IIf(lReproc,'1','2')/*cReproc*/, aValor, cFilAnt, (cAliasAKD)->(Recno())/*nRecAKD*/,AKW->AKW_NIVEL,.F.,cContaSint)
						Else
	...
	While !KillApp()                             
		...
		While !Eof()
			...
			If	MsRLock()  
				...
				//Se foi incluido Sintéticas na fila
				If _lALAANALIT == Nil .or. _lALANIVEL == Nil .or. _lALAATUSIN
					_lALAANALIT  := ALA->( FieldPos("ALA_ANALIT") ) > 0
					_lALANIVEL   := ALA->( FieldPos("ALA_NIVEL") ) > 0
					_lALAATUSIN   := ALA->( FieldPos("ALA_ATUSIN") ) > 0
				Endif
				If _lALAANALIT .and. _lALANIVEL .and. _lALAATUSIN
					lAnalit:= ALATRB->ALA_ANALIT == "1"
					cNivSin:= IIf(!Empty(AllTrim(ALATRB->ALA_NIVEL)), ALATRB->ALA_NIVEL, "")
					cAtuSin:= AllTrim(ALATRB->ALA_ATUSIN)
				EndIf

				PcoWriteSld('AKD',AL1->AL1_CONFIG,{ALATRB->ALA_VALOR1,ALATRB->ALA_VALOR2,ALATRB->ALA_VALOR3,ALATRB->ALA_VALOR4,ALATRB->ALA_VALOR5},ALATRB->ALA_TIPOMV,STOD(ALATRB->ALA_DATAMV),cChave,cAtuSin,ALATRB->ALA_REPROC=='1', cNivSin, lAnalit  )
