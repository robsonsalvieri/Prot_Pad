...
While !CV5->(Eof()) .and. CV5->CV5_FILIAL == cFilCV5 .and. CV5->CV5_COD <= cCodFim	
	...
	If CV5->CV5_EMPDES <> cEmpDSTAtu .or. CV5->CV5_FILDES <> cFilDSTAtu		/// SE A EMPRESA/FILIAL DESTINO FOREM DIFERENTES DA ATUAL ABERTA
		...
		If !ca295Abre(aFiles,CV5->CV5_EMPDES,CV5->CV5_FILDES,.T.,.T.,@aXFilial,,,.T.)		/// SE NÃO CONSEGUIU ABRIR OS ARQUIVOS
			CV5->(dbSkip())													/// TENTA O PROXIMO REGISTRO DE INTERCOMPANY
			Loop
		Endif                  			
...
While CV5->(!Eof()) .and. CV5->CV5_FILIAL == cFilCV5 .and. CV5->CV5_COD <= cCodFim
	If cEmpFilAtu <> CV5->(CV5_EMPDES+CV5_FILDES)
		If aScan(aEmp2Use,{|x| x[1]+x[2] == CV5->(CV5_EMPDES+CV5_FILDES) } ) <= 0
			__nTamFil := 0
			__nTamFil += Len(FWSM0Layout(CV5->CV5_EMPDES, 01))
			__nTamFil += Len(FWSM0Layout(CV5->CV5_EMPDES, 02))
			__nTamFil += Len(FWSM0Layout(CV5->CV5_EMPDES, 03))

			aAdd(aEmp2Use,{CV5->CV5_EMPDES, PadR(AllTrim(CV5->CV5_FILDES), __nTamFil) } )
		EndIf
...
While !SM0->(Eof())											//// RODA O ARQUIVO DE EMPRESAS
	...
	If Eval(bVerEmp)
		...
		Else
			aVldSX3 := StartJob("C295VldSX3",cEnvServer,.T.,cEmpOld,cFilOld)

			If ValType(aVldSX3) <> "U" .and. len(aVldSX3) > 0
				If !aVldSX3[1]
					If !lWf
						MsgAlert(aVldSX3[2])
					Endif
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Function ca295Abre(aFiles,cEmp,cFil,lDst,lSX2,aXFilial,lFechar,lOpenSX2,lxFilJob)
...
For nFiles := 1 to Len(aFiles)
	...
	If lDST
		if cEmpAnt <> cEmp .and. lxFilJob
			cFilAlias := StartJob("C295xFil",cEnvServer,.T.,cEmp,cFil,cAlias)
		else
...
*/
//------------------------------------------------------------------------------------
Function C295VldSX3(cEmpVld as character, cFilVld as character)
...
//-----------------------------------------------------------------------------------
/*{Protheus.doc} C295xFil
Função para pegar o XFilial da tabela em empresa de destino

Essa solução foi necessária devido o compartilhamento de tabelas estarem
diferentes por empresa.

@param cEmp		Indica a empresa que será aberta
@param cFil		Indica a filial que será aberta
@param cAlias	Indica a tabela que será verificada no FWxFilial

@author  Renan Gremes
@version P12
@since   05/12/2025
@return  cRet -> Retorna o XFilial da tabela/empresa de destino
*/
//------------------------------------------------------------------------------------
Function C295xFil(cEmp as character, cFil as character, cAlias as character) as logical
