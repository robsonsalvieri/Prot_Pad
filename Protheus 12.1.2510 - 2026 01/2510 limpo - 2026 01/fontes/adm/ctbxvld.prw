...
If !Empty(cEntidade)
	...
	If lRet
		...
		If cAlias == "CV0"
			cQuery += " AND  "+cAlias+"_PLANO = ? " 
			cQuery += " AND ( "+cAlias+"_BLOQUE = ? " 
			cQuery += " OR (" +cAlias+"_DTIBLQ <> ? AND "+cAlias+"_DTFBLQ <> ? AND ? BETWEEN "+cAlias+"_DTIBLQ AND "+cAlias+"_DTFBLQ ) " 
			cQuery += " OR ( (" +cAlias+"_DTIEXI <> ? AND ? < "+cAlias+"_DTIEXI )" 
			If lCtbExDtFim
				cQuery += " OR (" +cAlias+"_DTFEXI <> ? AND ?  > "+cAlias+"_DTFEXI ) ) " 
			Else
				cQuery += " ) "
			EndIf
		Else
			cQuery += " AND ( "+cAlias+"_BLOQ = ? " 
			cQuery += " OR (" +cAlias+"_DTBLIN <> ? AND "+cAlias+"_DTBLFI <> ? AND ? BETWEEN "+cAlias+"_DTBLIN AND "+cAlias+"_DTBLFI ) " 
			cQuery += " OR ( (" +cAlias+"_DTEXIS <> ? AND ? < "+cAlias+"_DTEXIS )" 
			If lCtbExDtFim
				cQuery += " OR (" +cAlias+"_DTEXSF <> ? AND ?  > "+cAlias+"_DTEXSF ) ) " 
		...
		_oQry := FwExecStatement():New(cQuery)

		nQry := 1 
		_oQry:SetString(nQry++, xFilial(cAlias))
		_oQry:SetString(nQry++, cEntidade)
		If cAlias == "CV0"
			_oQry:SetString(nQry++, cPlano)
		EndIf
		_oQry:SetString(nQry++, '1')
		_oQry:SetString(nQry++, Space(1))
		_oQry:SetString(nQry++, Space(1))
		_oQry:SetDate(nQry++, dData)
		_oQry:SetString(nQry++, Space(1))
		_oQry:SetDate(nQry++, dData)
		If lCtbExDtFim
			_oQry:SetString(nQry++, Space(1))
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Function CTBMInUse(cMoeda,lValidCTO,cFilAtu)
...
If lValidCTO
	...
	Else
		dbSelectArea("CTO")
		dbSetOrder(1)
		cFilAtu := iif(FwIsInCallStack("Cta295Vld"), cFilAtu, xFilial())//Intercompany precisa mandar o cFilAtu obrigatoriamente, para respeitar o compartilhamento da empresa destino
		lRet := MsSeek(cFilAtu+cMoeda)
	...
	If lValidCTO
		...
		Else
			For nCont := 1 to __nQuantas	/// Roda todas as moedas
				...
				If !CTBMInUse(cCoin4, lValidCTO, cFilAtu) /// Verifica Moeda no CTO ausencia ou bloqueio.
					If lShowMsg
...
dbSelectArea("CT0")
cFilCT0Atu  := xFilial("CT0")

Default cPlano  := "01"
Default lHelp	:= .T.
Default lAtuDescEnt := .T.

If len(aCtbEntInd) == 0 .or. cFilCT0Ant <> cFilCT0Atu
	aCtbEntInd := CTBEntGtIn() //Ganho de performance
	cFilCT0Ant :=cFilCT0Atu
EndIf

aIndexes := aClone(aCtbEntInd)

If !Empty(cIdEntid)
	dbSelectArea("CT0")
	dbSetOrder(1)
	If dbSeek(cFilCT0Atu+cIdEntid)
