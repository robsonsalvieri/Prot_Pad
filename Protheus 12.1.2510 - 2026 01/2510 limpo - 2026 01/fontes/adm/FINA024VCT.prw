...
Static Function ModelDef() As Object
	...
	If oStruFKP:HasField("FKP_DIAQUI")
		oStruFKP:SetProperty('FKP_DIAVCT' , MODEL_FIELD_WHEN , {|| F024FKPWHE(oModel,'FKP_DIAVCT') } )
		oStruFKP:SetProperty('FKP_DIAQUI' , MODEL_FIELD_WHEN , {|| F024FKPWHE(oModel,'FKP_DIAQUI') } )
		oStruFKP:SetProperty('FKP_DTVLVC' , MODEL_FIELD_WHEN , {|| F024FKPWHE(oModel,'FKP_DTVLVC') } )
	EndIf
...
Function F024FKPWhe(oModel As Object, cCampo As Character)
	...
	If cCampo $ 'FKP_DATVCT'
		lRet := oModel:GetValue("FKPMASTER","FKP_TIPVCT") == '2' .And. oModel:GetValue("FKPMASTER","FKP_PRDVCT") <> '3' //Tipo de Vencto = Periodo
	EndIF	

	If cCampo $ 'FKP_PRDVCT|FKP_QTPERI'
		lRet := oModel:GetValue("FKPMASTER","FKP_TIPVCT") == '2'	//Tipo de Vencto = Periodo
	Endif	

	If cCampo $ 'FKP_DIAQUI'
		lRet := oModel:GetValue("FKPMASTER","FKP_PRDVCT") == '3' .And. oModel:GetValue("FKPMASTER","FKP_TIPVCT") == '2'	//Tipo de Vencto = Periodo
	Endif	

	If cCampo $ 'FKP_DIAVCT|FKP_DTVLVC'
		lRet := oModel:GetValue("FKPMASTER","FKP_PRDVCT") <> '3' .Or. oModel:GetValue("FKPMASTER","FKP_TIPVCT") == '1'	//Tipo de Vencto = Periodo
	Endif	

	IF cCampo $ 'FKP_PRDVCT|FKP_QTPERI|FKP_DIAVCT|FKP_DATVCT|FKP_DIAQUI|FKP_DTVLVC'
