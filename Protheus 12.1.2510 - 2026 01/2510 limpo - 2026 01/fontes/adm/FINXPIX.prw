...
Function AbatimentR(nRecTitPai As Numeric, nTxMoeda As Numeric, nMoedaDest As Numeric, cListaAb As Char, oObjAb As Object, cSeparador As Char) As Array
    ...
    If nRecTitPai > 0
        ...
        __nTituPai := IIf(__nTituPai == Nil, TamSx3("E1_PREFIXO")[1]+TamSx3("E1_NUM")[1]+TamSx3("E1_PARCELA")[1]+;
                                             TamSx3("E1_TIPO")[1]+TamSx3("E1_CLIENTE")[1]+TamSx3("E1_LOJA")[1], __nTituPai)
        ...
        If (FindFunction("UsaBillRel") .AND. UsaBillRel() <> Nil)
            If oObjAb == Nil 
                cQry := " SELECT SE1F.E1_TIPO, SE1F.E1_VALOR, ISNULL(SE1F.E1_TXMOEDA, 0) E1_TXMOEDA "
                cQry += " FROM " + RetSqlName("SE1") + " SE1P"

                cQry += " INNER JOIN " + RetSQLName("FK7") + " FK7P ON"
                cQry += "     FK7P.D_E_L_E_T_ = SE1P.D_E_L_E_T_ "
                cQry += " AND SE1P.E1_FILIAL  = FK7P.FK7_FILTIT"
                cQry += " AND FK7P.FK7_ALIAS  = ?"
                cQry += " AND FK7P.FK7_PREFIX = SE1P.E1_PREFIXO"
                cQry += " AND FK7P.FK7_NUM    = SE1P.E1_NUM"
                cQry += " AND FK7P.FK7_PARCEL = SE1P.E1_PARCELA"
                cQry += " AND FK7P.FK7_TIPO   = SE1P.E1_TIPO"
                cQry += " AND FK7P.FK7_CLIFOR = SE1P.E1_CLIENTE"
                cQry += " AND FK7P.FK7_LOJA   = SE1P.E1_LOJA"

                cQry += " INNER JOIN " + RetSqlName("FK7") + " FK7F ON"
                cQry += "     FK7F.D_E_L_E_T_ = FK7P.D_E_L_E_T_"
                cQry += " AND FK7F.FK7_IDPAI  = FK7P.FK7_IDDOC"
                        
                cQry += " INNER JOIN " + RetSqlName("SE1") + " SE1F ON"
                cQry += "     SE1F.D_E_L_E_T_ = FK7F.D_E_L_E_T_"
                cQry += " AND FK7F.FK7_FILTIT = SE1F.E1_FILIAL"
                cQry += " AND FK7F.FK7_PREFIX = SE1F.E1_PREFIXO"
                cQry += " AND FK7F.FK7_NUM    = SE1F.E1_NUM"
                cQry += " AND FK7F.FK7_PARCEL = SE1F.E1_PARCELA"
                cQry += " AND FK7F.FK7_TIPO   = SE1F.E1_TIPO"
                cQry += " AND FK7F.FK7_CLIFOR = SE1F.E1_CLIENTE"
                cQry += " AND FK7F.FK7_LOJA   = SE1F.E1_LOJA"

                cQry += " WHERE " 
                cQry += "     SE1F.E1_FILORIG = ?"
                cQry += " AND SE1F.E1_PREFIXO = ?"
                cQry += " AND SE1F.E1_NUM = ?"
                cQry += " AND SE1F.E1_PARCELA = ?"
                cQry += " AND SE1F.E1_TIPO IN (?)"
                cQry += " AND SE1F.E1_CLIENTE = ?"
                cQry += " AND SE1F.E1_LOJA = ?"
                cQry += " AND SE1F.D_E_L_E_T_ = ? "
             
                cQry := ChangeQuery(cQry)
                oObjAb := FwExecStatement():New(cQry)
            EndIf
            oObjAb:SetString(nParam++, "SE1")
            oObjAb:SetString(nParam++, SE1->E1_FILORIG)
            oObjAb:SetString(nParam++, SE1->E1_PREFIXO)
            oObjAb:SetString(nParam++, SE1->E1_NUM)
            oObjAb:SetString(nParam++, SE1->E1_PARCELA)
            oObjAb:SetIn(nParam++, FinSetIn(cListaAb, cSeparador))
            oObjAb:SetString(nParam++, SE1->E1_CLIENTE)
            oObjAb:SetString(nParam++, SE1->E1_LOJA)
            oObjAb:SetString(nParam++, ' ')
        Else
            If oObjAb == Nil 
            ...
            oObjAb:SetUnsafe(1, RetSqlName("SE1"))
...
Static Function ImposEmBx(cChaTitPai As Char, cFilOrig As Char, oImpEmBx As Object) As Char
    ...
    If !Empty(cChaTitPai)
        ...
        If (FindFunction("UsaBillRel") .AND. UsaBillRel() <> Nil)
            If oImpEmBx == Nil 
                cQuery := " SELECT SE1F.E1_TIPO, SE1F.E1_VALOR, SE1F.E1_SEQBX, SE1F.R_E_C_N_O_ REGIMPSE1, SE1F.E1_EMISSAO"
                cQuery += " FROM " + RetSqlName("SE1") + " SE1P"
                
                cQuery += " INNER JOIN " + RetSQLName("FK7") + " FK7P ON"
                cQuery += "     FK7P.D_E_L_E_T_ = SE1P.D_E_L_E_T_"
                cQuery += " AND SE1P.E1_FILIAL  = FK7P.FK7_FILTIT"
                cQuery += " AND FK7P.FK7_ALIAS  = ?"
                cQuery += " AND FK7P.FK7_PREFIX = SE1P.E1_PREFIXO"
                cQuery += " AND FK7P.FK7_NUM    = SE1P.E1_NUM"
                cQuery += " AND FK7P.FK7_PARCEL = SE1P.E1_PARCELA"
                cQuery += " AND FK7P.FK7_TIPO   = SE1P.E1_TIPO"
                cQuery += " AND FK7P.FK7_CLIFOR = SE1P.E1_CLIENTE"
                cQuery += " AND FK7P.FK7_LOJA   = SE1P.E1_LOJA"

                cQuery += " INNER JOIN " + RetSqlName("FK7") + " FK7F ON"
                cQuery += "     FK7F.D_E_L_E_T_ = FK7P.D_E_L_E_T_"
                cQuery += " AND FK7F.FK7_IDPAI  = FK7P.FK7_IDDOC"

                cQuery += " INNER JOIN " + RetSqlName("SE1") + " SE1F ON"
                cQuery += "     SE1F.D_E_L_E_T_ = FK7F.D_E_L_E_T_"
                cQuery += " AND FK7F.FK7_FILTIT = SE1F.E1_FILIAL"
                cQuery += " AND FK7F.FK7_PREFIX = SE1F.E1_PREFIXO"
                cQuery += " AND FK7F.FK7_NUM    = SE1F.E1_NUM"
                cQuery += " AND FK7F.FK7_PARCEL = SE1F.E1_PARCELA"
                cQuery += " AND FK7F.FK7_TIPO   = SE1F.E1_TIPO"
                cQuery += " AND FK7F.FK7_CLIFOR = SE1F.E1_CLIENTE"
                cQuery += " AND FK7F.FK7_LOJA   = SE1F.E1_LOJA"

                cQuery += " WHERE " 
                cQuery += "     SE1P.E1_FILORIG = ?"
                cQuery += " AND SE1P.E1_PREFIXO = ?"
                cQuery += " AND SE1P.E1_NUM = ?"
                cQuery += " AND SE1P.E1_PARCELA = ?"
                cQuery += " AND SE1P.E1_TIPO = ?"
                cQuery += " AND SE1P.E1_CLIENTE = ?"
                cQuery += " AND SE1P.E1_LOJA = ?"
                cQuery += " AND SE1P.D_E_L_E_T_ = ? "
                cQuery += " ORDER BY SE1F.R_E_C_N_O_ "
             
                cQuery := ChangeQuery(cQuery)
                oImpEmBx := FwExecStatement():New(cQuery)
            EndIf
            oImpEmBx:SetString(nParam++, "SE1")
            oImpEmBx:SetString(nParam++, cFilOrig)
            oImpEmBx:SetString(nParam++, SE1->E1_PREFIXO)
            oImpEmBx:SetString(nParam++, SE1->E1_NUM)
            oImpEmBx:SetString(nParam++, SE1->E1_PARCELA)
            oImpEmBx:SetString(nParam++, SE1->E1_TIPO)
            oImpEmBx:SetString(nParam++, SE1->E1_CLIENTE)
            oImpEmBx:SetString(nParam++, SE1->E1_LOJA)
            oImpEmBx:SetString(nParam++, ' ')            
        Else            
            If oImpEmBx == Nil
                ...
                oImpEmBx := FWExecStatement():New(cQuery)
            EndIf
            oImpEmBx:SetUnsafe(nParam++, RetSqlName("SE1"))
            oImpEmBx:SetString(nParam++, cChaTitPai)
            oImpEmBx:SetString(nParam++, cFilOrig)

        EndIf

        cTempSE1 := oImpEmBx:OpenAlias()
