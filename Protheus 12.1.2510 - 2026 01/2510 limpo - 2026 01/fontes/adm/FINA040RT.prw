...
Local nIndEZ4	:= Iif(FWSIXUtil():ExistIndex('SEZ' , '8'), 8,1)
	...
	aAdd( aAuxFK7, {"FK7_FILTIT", "SE1->E1_FILIAL"})
	aAdd( aAuxFK7, {"FK7_PREFIX", "SE1->E1_PREFIXO"})
	aAdd( aAuxFK7, {"FK7_NUM"   , "SE1->E1_NUM"})
	aAdd( aAuxFK7, {"FK7_PARCEL", "SE1->E1_PARCELA"})
	aAdd( aAuxFK7, {"FK7_TIPO"  , "SE1->E1_TIPO"})
	aAdd( aAuxFK7, {"FK7_CLIFOR", "SE1->E1_CLIENTE"})
	aAdd( aAuxFK7, {"FK7_LOJA"  , "SE1->E1_LOJA"})
...
Static Function LoadFK1(oGridModel AS Object, lCopia AS Logical) AS Array
	...
	cQry += " SELECT ? FK1.R_E_C_N_O_ RECNO FROM  " + RetSqlName("FK1") + " FK1"
	...
	cQry += " AND FK7.FK7_FILTIT = ? "
	cQry +=	" AND FK7.FK7_PREFIX = ? "
	cQry +=	" AND FK7.FK7_NUM = ? "	
	cQry +=	" AND FK7.FK7_PARCEL = ? "
	cQry +=	" AND FK7.FK7_TIPO = ? "	
	cQry += " AND FK7.FK7_CLIFOR = ? "
	cQry += " AND FK7.FK7_LOJA = ? "
	cQry += " AND FK7.D_E_L_E_T_ = ? "
	cQry := ChangeQuery(cQry)

	oQryExec := FWExecStatement():New(cQry)
	oQryExec:SetUnsafe(nParam++, cSelect)
	oQryExec:SetString(nParam++, FwxFilial("FK1"))
	oQryExec:SetString(nParam++, "ES")
	oQryExec:SetString(nParam++, Space(1))
	oQryExec:SetString(nParam++, Space(1))
	oQryExec:SetString(nParam++, "SE1")
	oQryExec:SetString(nParam++, FwxFilial("SE1", SE1->E1_FILORIG))
	oQryExec:SetString(nParam++, SE1->E1_PREFIXO)
	oQryExec:SetString(nParam++, SE1->E1_NUM)
	oQryExec:SetString(nParam++, SE1->E1_PARCELA)
	oQryExec:SetString(nParam++, SE1->E1_TIPO)
	oQryExec:SetString(nParam++, SE1->E1_CLIENTE)
	oQryExec:SetString(nParam++, SE1->E1_LOJA)
	oQryExec:SetString(nParam++, Space(1))

	oQryExec:SetFields(aCampos)

	cAliasBxs := oQryExec:OpenAlias()
