...
Local nIndEZ4	:= Iif(FWSIXUtil():ExistIndex('SEZ' , '8'), 8,1)
	...
	aAdd( aAuxFK7, {"FK7_FILTIT", "SE2->E2_FILIAL"})
	aAdd( aAuxFK7, {"FK7_PREFIX", "SE2->E2_PREFIXO"})
	aAdd( aAuxFK7, {"FK7_NUM"   , "SE2->E2_NUM"})
	aAdd( aAuxFK7, {"FK7_PARCEL", "SE2->E2_PARCELA"})
	aAdd( aAuxFK7, {"FK7_TIPO"  , "SE2->E2_TIPO"})
	aAdd( aAuxFK7, {"FK7_CLIFOR", "SE2->E2_FORNECE"})
	aAdd( aAuxFK7, {"FK7_LOJA"  , "SE2->E2_LOJA"})
...
Static Function LoadFK2(oGridModel AS Object, lCopia AS Logical) AS Array
	...
	// Filtra baixas
	cQry += " SELECT ? FK2.R_E_C_N_O_ RECNO FROM " + RetSqlName("FK2") + " FK2"
	...
	oQryExec:SetString(nParam++, FwxFilial("FK2"))
	oQryExec:SetString(nParam++, "ES")
	oQryExec:SetString(nParam++, Space(1))
	oQryExec:SetString(nParam++, Space(1))
	oQryExec:SetString(nParam++, "SE2")
	oQryExec:SetString(nParam++, FwxFilial("SE2", SE2->E2_FILORIG))
	oQryExec:SetString(nParam++, SE2->E2_PREFIXO)
	oQryExec:SetString(nParam++, SE2->E2_NUM)
	oQryExec:SetString(nParam++, SE2->E2_PARCELA)
	oQryExec:SetString(nParam++, SE2->E2_TIPO)
	oQryExec:SetString(nParam++, SE2->E2_FORNECE)
	oQryExec:SetString(nParam++, SE2->E2_LOJA)
	oQryExec:SetString(nParam++, Space(1))
