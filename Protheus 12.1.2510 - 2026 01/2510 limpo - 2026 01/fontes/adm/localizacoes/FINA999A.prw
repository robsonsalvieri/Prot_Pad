...
/*/{Protheus.doc} GetHdrCols
	...
	@return 
/*/
Static Function GetHdrCols(lF850, aSE2, aPagos, lPA)
Local nX		 := 0
Local nI 		 := 0
Local nY 		 := 0
...
Default lPA  	 := .F.
	...
	If lF850
		...
		aEOP := IIF(!lPA .And. !IsInCallStack("F850LNOK"), {{nNumOrdens, nValOrdens, cMoeda, cPgtoElt, nMoedaCor, lPA, 0, lShowPOrd}},;
						{{nNumOrdens, 0, "", cPgtoElt, nMoedaCor, lPA, 0, lShowPOrd}})
		...
		For nX := 1 To Len(aPagos)
			...
			aDOP[nI, 11] := IIF(!Empty(aPagos[nX, 21]), aPagos[nX, 21], cNatureza) //H_NATUREZA
			...
			aDOP[nI, 15] := IIF(Len(aPagos[nX]) > 23, aPagos[nX, 24],"") //H_TERC
	...
	Else
		...
		For nX := 1 To Len(aPagos)
			...
			aDOP[nI, 10] := aPagos[nX, 15] //H_TOTALVL
			aDOP[nI, 11] := cNatureza //Modalidad
			...
			If cPaisLoc == "MEX"
				aDOP[nI, 15] := cDesc //Descripción 
			EndIf

			If cPaisLoc == "PAR"
				aDOP[nI, 15] := aPagos[nX, 9]  //H_RETIVA
				aDOP[nI, 16] := aPagos[nX, 11] //H_RETIR
			EndIf

		Next nX

		If lPa085a .And. Len(aDOP) == 0
			aAdd(aDOP, Array(16))
			nI := Len(aDOP)
			aDOP[nI,  2] := cFornece  //H_FORNECE
			aDOP[nI,  3] := cLoja 	  //H_LOJA
			aDOP[nI,  7] := nValor    //H_TOTAL
			aDOP[nI, 11] := cNatureza //Modalidad
		EndIf
