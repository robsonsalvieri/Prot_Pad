...
If lLancPad70
	...
	//+--------------------------------------------------------------+
	aGetSx5 := FwGetSX5("09", "FIN")
	If Len(aGetSx5) > 0
		cLoteCom := AllTrim(aGetSx5[1][4])
	Else 
		cLoteCom := "FIN"
	EndIf
...
If cPaisLoc == "ARG" .And.  Len(aSuss) > 0
	...
	IF Len(aLimSus) > 0 .and. Len(aSe2) > 0
		For nI := 1 to Len(aLimSus) //Caso o array aLimSus esteja maior que zero
			For nY := 1 to Len(aSe2) // apago os valores de reten็ใo para os fonecedores que nใo superam o minimo.
				For nZ := 1 to Len(aSe2[nY][1])
					IF aLimSus[nI][1] == aSe2[nY][1][nZ][1] .and.  aLimSus[nI][2] == aSe2[nY][1][nZ][2] .And. (Len(aSe2[nY][1][nZ][_RETSUSS]) > 0 .And. !(Alltrim(aSe2[nY][1][nZ][_RETSUSS][1][8]) $ "4|1"))
						aSe2[nY][1][nZ][_RETSUSS] := {}
					EndIF
				Next nZ
			Next nY			
		Next nI
...
/*--- Parte superior da OP com os dados das ordens de pago ------------------------------------------------------------------------------------------------*/
	...
	@ nLinIni + 000,006 SAY OemToAnsi(STR0064)  Size 70,08 			PIXEL OF 	oPnlOrd //"ำrdenes de Pago: "
	@ nLinIni + 000,070 SAY oNumOrdens Var nNumOrdens 				PIXEL OF 	oPnlOrd	//<cant_๓rdenes_de_pago>

	@ nLinIni + 000,160 SAY OemToAnsi(STR0065)  Size 70,08 			PIXEL OF 	oPnlOrd //"Total por pagar: "
	@ nLinIni + 000,250 SAY oValOrdens Var nValOrdens  Size 80,08 	PIXEL OF	oPnlOrd	PICTURE PesqPict("SE2","E2_VALOR") //<total_por_pagar>
...
/* browser com as ops a serem geradas*/
	...
	If cPaisLoc == "ARG"
		...
		aPagos[oLbx:nAT][3],;
		aPagos[oLbx:nAT][4],;
		aPagos[oLbx:nAt][H_TERC],;
		PADL(lTrim(aPagos[oLbx:nAT][5]),16,CHR(127)),;
		PADL(lTrim(aPagos[oLbx:nAT][6]),16,CHR(127)),;
		PADL(lTrim(aPagos[oLbx:nAT][7]),16,CHR(127)),;
		PADL(lTrim(aPagos[oLbx:nAT][8]),16,CHR(127)),;
		PADL(lTrim(aPagos[oLbx:nAT][9]),16,CHR(127)),;
		PADL(lTrim(aPagos[oLbx:nAT][10]),16,CHR(127)),;
		PADL(lTrim(aPagos[oLbx:nAT][11]),16,CHR(127)),;
		PADL(lTrim(aPagos[oLbx:nAT][12]),16,CHR(127)),;
		PADL(lTrim(aPagos[oLbx:nAT][13]),16,CHR(127)),;
		Iif(aPagos[oLbx:nAT][14],STR0047,STR0048)}}
	ElseIf cPaisLoc == "ANG"
...
If lFINA999 .And. !lAutomato
	lRet := VlLinF999(.T., lPa)
EndIf

If lRet .And. cPaisLoc == "ARG" .and. !lAutomato
...
/* painel para o resumo das formas de pagamento */
	...
	oPnlSaldPg := TPanel():New(0,0,"",oPnlResPg,,.F.,.F.,,,50,50,,)
	...
	oPnlDocTerc := TPanel():New(0,0,"",oPnlResPg,,.F.,.F.,,,50,50,,)
	...
	oPnlSep12:nWidth := 1
	/* campo para total em documentos de terceiros */
	oPnlDocProp := TPanel():New(0,0,"",oPnlResPg,,.F.,.F.,,,50,50,,)
...
EndIf
@ 75,004 To 100,250 Pixel Of oDlg LABEL OemToAnsi(STR0098) // "Ganancias"
@ 85,006 SAY OemToAnsi("Ganancias : ")  		Size 40,08 Pixel Of  oDlg
@ 85,050 SAY oRet VAR nRet Size 60,08 PIXEL OF oDlg Picture Tm(nRet,19,nDecs)

@ 85,110 SAY OemToAnsi("Ingresos Brutos : ") Size 40,08 Pixel Of  oDlg // "Ingresos Brutos"
@ 85,154 SAY oRetIB Var nRetIB  Size 60,08 Pixel Of  oDlg Picture Tm(nRetIB,19,nDecs)
...
Else
	...
	If nLenDoc >0
		...
		For nCpo := 1 To nLen3os
			If nCpo == nPosSaldo
				oBrw3os:AddColumn(TCColumn():New(&("RetTitle('" + oGetDad2:aHeader[nCpo,2] + "')"),&("{|| aDoc3os[oBrw3os:nAt," + AllTrim(Str(nCpo+1)) + "]}"),PesqPict("SE1",oGetDad2:aHeader[nCpo,2]),,,"RIGHT",020,.F.,.F.,,,,,)) 
			ElseIf nCpo == nPosMArg
				...
				oBrw3os:AddColumn(TCColumn():New(AllTrim(STR0174),&("{|| FA850CNVAL(aDoc3os[oBrw3os:nAt," + AllTrim(Str(nPosSaldo+1)) +"],aDoc3os[oBrw3os:nAt," + AllTrim(Str(nPosMArg+1)) +"]," + AllTrim(Str(nMoedaSele)) +")}"),PesqPict("SE1",oGetDad2:aHeader[nPosSaldo,2]),,,"RIGHT",020,.F.,.F.,,,,,))
			Else 
				oBrw3os:AddColumn(TCColumn():New(&("RetTitle('" + oGetDad2:aHeader[nCpo,2] + "')"),&("{|| aDoc3os[oBrw3os:nAt," + AllTrim(Str(nCpo+1)) + "]}"),PesqPict("SE1",oGetDad2:aHeader[nCpo,2]),,,,020,.F.,.F.,,,,,))
			EndIf
		Next
...
Else
	...
	If cPaisLoc == "ARG" .And. Len(aPagConCBU) > 0 .And. !(FJS->FJS_TIPO $"TF |TB ")
			Help(" ", 1, "A850OPECBU", , STR0466, 2, 0,,,,,, {STR0467}) //"Seleccion๓ pago en CBU informada", "ฺnicamente se permiten pagos mediante dep๓sito o transferencia bancaria."
			lRet := .F.
			lPagoCBU := .T.
	ElseIf  cPaisLoc == "ARG" .And. Len(aPagConCBU) > 0 .And. (FJS->FJS_TIPO == "TB ")
			lTipoPag := .F. 
	EndIf

	//Valida o tipo informado
	If cPaisLoc $ "ARG|RUS|" .And. Type("cOpcElt") <> "U" .And. lTipoPag
...
Return aClone(aTasaCamb)
  ...
  //บDescricao ณ Validacion de fechas operaci๓n sujeta a Retenci๓n           บ
Static Function FecSujRet()                
Local cFchIRet := ctod("//")      
Local cFchFRet := ctod("//")
 
		cFchIRet := SA2->A2_DTINISM      
		cFchFRet := SA2->A2_DTFIMSM	

	If !Empty(cFchIRet) .And. !Empty(cFchFRet)      
		If  (cAliasTmp)->E2_EMISSAO >= cFchIRet .And. (cAliasTmp)->E2_EMISSAO <= cFchFRet 
			MsgAlert( STR0465 + ".",STR0223) //"OPERACIำN SUJETA A RETENCIำN" 
		EndIf
	EndIf     
