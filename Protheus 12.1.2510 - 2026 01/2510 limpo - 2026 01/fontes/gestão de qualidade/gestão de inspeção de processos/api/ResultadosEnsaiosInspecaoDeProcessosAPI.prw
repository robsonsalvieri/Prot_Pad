...
WSRESTFUL processinspectiontestresults DESCRIPTION STR0001 FORMAT APPLICATION_JSON //"Resultados Ensaios Inspeção de Processos"
	...
	WSSYNTAX "api/qip/v1/vldsingleresult/{Login}/{RecnoQPK}/{RecnoQPR}/{OperationID}" ;
	PATH "/api/qip/v1/vldsingleresult" ;
	TTALK "v1"

	WSMETHOD GET vldmultipleresult;
	DESCRIPTION STR0084; //"Indica se permite a inclusao do resultado multiplo de formulario atual"
	WSSYNTAX "api/qip/v1/vldmultipleresult/{Login}/{RecnoQPK}/{OperationID}" ;
	PATH "/api/qip/v1/vldmultipleresult" ;
	TTALK "v1"

	WSMETHOD GET dateeditpermission;
	DESCRIPTION STR0064; //"Indica a permissão para edição do campo data da amostra"
	WSSYNTAX "api/qip/v1/dateeditpermission/{Login}/{RecnoQPK}/{RecnoQPR}/{OperationID}" ;
	...
	WSSYNTAX "api/qip/v1/timeeditpermission/{Login}/{RecnoQPK}/{RecnoQPR}/{OperationID}" ;
...
WSMETHOD GET vldsingleresult PATHPARAM Login, RecnoQPK, RecnoQPR, OperationID QUERYPARAM Form WSSERVICE processinspectiontestresults
	...
	If oAPIManager:existePEIntegracaoAppMinhaProducaoCompilado("QIP")
		...
		oReturnPE := Execblock('QIPINTAPI',.F.,.F.,{oObj, "processinspectiontestresults/api/qip/v1/vldsingleresult", "ResultadosEnsaiosInspecaoDeProcessosAPI", "validaResultadoUnico"})
...
WSMETHOD GET vldmultipleresult PATHPARAM Login, RecnoQPK, OperationID QUERYPARAM Form WSSERVICE processinspectiontestresults
	...
	If oAPIManager:existePEIntegracaoAppMinhaProducaoCompilado("QIP")
		...
		oReturnPE := Execblock('QIPINTAPI',.F.,.F.,{oObj, "processinspectiontestresults/api/qip/v1/vldmultipleresult", "ResultadosEnsaiosInspecaoDeProcessosAPI", "validaResultadosMultiplos"})
		If Valtype(oReturnPE) == 'J' .And. ValType(oReturnPE['permite']) == "L" .And. ValType(oReturnPE['mensagem']) == "C"
			oResponse := oReturnPE
		EndIf
	EndIf
...
WSMETHOD GET dateeditpermission PATHPARAM Login, RecnoQPK, RecnoQPR, OperationID QUERYPARAM Form WSSERVICE processinspectiontestresults
