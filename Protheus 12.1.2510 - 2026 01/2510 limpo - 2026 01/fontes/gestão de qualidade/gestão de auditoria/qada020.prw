...
IF nOpcA == 1
	...
	If !Empty(xFilial("QUJ"))
		...
		Next
	Else	
		If (QUJ->(dbSeek(xFilial("QUJ")+QU2->QU2_CHKLST+QU2->QU2_REVIS)))
			lDelChkLst := .F.
		EndIf
	EndIf	  
...
cUltRevCkl := oQADA100Aux:retornaProximaRevisaoEfetivadaDoCheckList(QU2->QU2_CHKLST)

If QU2->QU2_EFETIV # "1" .And. lContinua .And. !Empty(cUltRevCkl)
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³Verifica se o Check List esta efetivado           		     ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ	
   	Help("",1,"020CHKNEFE")                                            
