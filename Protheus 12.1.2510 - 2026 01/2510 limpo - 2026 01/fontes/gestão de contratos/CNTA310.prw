...
static _oModel300 as object
static _oModel301 as object
...
_oModel300 := FWLoadModel("CNTA300") 
_oModel301 := FWLoadModel("CNTA301")
...
FwFreeObj(_oModel300)
FwFreeObj(_oModel301)
	...
	If lRet .And. lCNRJREV
		...
		If CN9->( DbSeek(xFilial('CN9')+cContra+cNovaRev) )
			If Alltrim(CN9->CN9_SITUAC) == DEF_SREVS .And. Empty(Alltrim(CN9->CN9_APROV)) //Aprovação automativa para contratos "Em Revisão" e "Sem alçada de aprovação"
				...
				If CN9->CN9_ESPCTR == "1"
					oModAprov := FWLoadModel("CNTA300")
				Else
					oModAprov := FWLoadModel("CNTA301")
				EndIf
				oModAprov:SetOperation(MODEL_OPERATION_UPDATE)
				If lRet := oModAprov:Activate()
					If ( lRet := oModAprov:VldData() )
						oModAprov:CommitData()
					EndIf
				EndIf
				
				If !lRet
					SetAutoLog(cContra, oModAprov)
				EndIf				
				oModAprov:Deactivate()
				Cn300Clear()
				FwFreeObj(oModAprov)
			Endif
		EndIf
		...
		If lRet
			oModel300 := GetM300()
			oModel300:SetOperation(MODEL_OPERATION_VIEW)
...
//------------------------------------------------------------------
/*/{Protheus.doc} A310TpRevRj()
@description Inclui mensagens do modelo no AutoGrLog p/ exibicao atraves da funcao MostraErro
@author TOTVS
@since 04/09/2015
/*/
//-------------------------------------------------------------------
Static Function SetAutoLog(cContra as character, oModel as object)
...
//------------------------------------------------------------------
/*/{Protheus.doc} GetM300()
@description Abre o modelo conforme parametro "MV_CNRJREV"
@author TOTVS
@since 04/09/2015
/*/
//-------------------------------------------------------------------
Static Function GetM300(cEspCtr as character)
	Local oModel as object
	Default cEspCtr := CN9->CN9_ESPCTR
