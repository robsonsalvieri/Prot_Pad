...
Function AGDR010(lShowDlg)
	...
	Private cFileSystem := gStartPath()
...
Function AGDR010FWD(lDir)
	...
	if lDir
		cNewPathArq	:= tFileDialog("", STR0009,, tmp, .F., GETF_RETDIRECTORY)
		...
		cNewPathArq	:= tFileDialog(cTipo, STR0013,,, .T.)
		IF !Empty( cNewPathArq )
...
Static Function fCpos_Word()
	...
	aAdd(aExp, {"AGD_INDICE_MOEDA", 	NEA->NEA_IDXMOE,		GetSx3Cache("NEA_IDXMOE", "X3_PICTURE"),    GetSx3Cache("NEA_IDXMOE", "X3_TITULO") })
	...
	if lLinux
		...
		if lSucess
			...
			If nResult == 0
				...
				if nResult == 0
					cArqWord := cFileSystem + _cFolder + gBarraApSrv() + "content.xml"
		...
		If File(cArqWord,,lChangeCase)
			...
			FErase(cArqWord)
			oFile := FWFileWriter():New(cArqWord, .T.)
			oFile:Create()
			oFile:Write(cConteudo)
			oFile:Close()
...
/*/{Protheus.doc} gStartPath
Retorna o startpath do appserver.ini sempre com a barra no final
@type function
@version 12
@author jc.maldonado
@since 10/12/2025
@return character, cStartPath
/*/
Static Function gStartPath()
	Local cStartPath := allTrim(GetSrvProfString("StartPath", ""))
	Local cBarra     := gBarraApSrv()

	If Right(cStartPath, 1) != cBarra
		cStartPath += cBarra
	EndIf
return cStartPath

/*/{Protheus.doc} gFileName
Retorna o nome do arquivo a partir do caminho completo do mesmo.
@type function
@version 12
@author jc.maldonado
@since 10/12/2025
@param cPath, character, caminho completo para o arquivo
@return character, nome do arquivo
/*/
Static Function gFileName(cPath)
	Local cFileName := ""
	Local cBarra    := "\"
	Local aPath     := {}
	...
	aPath := StrTokArr2(cPath, cBarra)
	if ! Empty(aPath)
		cFileName := AllTrim(aPath[Len(aPath)])
	endIf
Return cFileName

/*/{Protheus.doc} gBarraApSrv
Retorna a barra de acordo com o tipo do sistema operacional
@type function
@version 12
@author jc.maldonado
@since 11/12/2025
@return character, barra
/*/
Static Function gBarraApSrv()
	Local cBarra := "\"
