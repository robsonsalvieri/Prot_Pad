...
Function AGDA010T(nOperation, aTaxes, cCodNeg, cMoedRT, nTxMoeda, cCodPro, nVlrRT, cTes, cNaturez)
	...
	if nOperation == MODEL_OPERATION_VIEW
		...
		if Len(aTaxes) > 1 .and. !empty(aTaxes[2]) 
			cFornec := aTaxes[2, 1, aScan(aTaxes[1], {|xIt| xIt[2] == "NEN_CODFOR"})] 
			If !empty(cFornec) //imposto ja foi gravado
				// Carrega os valores do imposto
				oMdlTaxCab:LoadValue("T_CODFOR", cFornec)
				oMdlTaxCab:LoadValue("T_LOJFOR", aTaxes[2, 1, aScan(aTaxes[1], {|xIt| xIt[2] == "NEN_LOJFOR"})])
				oMdlTaxCab:LoadValue("T_TES",    aTaxes[2, 1, aScan(aTaxes[1], {|xIt| xIt[2] == "NEN_TES"})])
				oMdlTaxCab:LoadValue("T_NATURE", aTaxes[2, 1, aScan(aTaxes[1], {|xIt| xIt[2] == "NEN_NATURE"})])
...
Static Function loadAtax(aTaxes, oGrid)
	...
	For nX := 1 to Len(aTaxes[2/*aCols*/])
		//desconsidera os deletados e linhas vazias
		if !aTaxes[2, nx, len(aTaxes[2, nx])] .and. !empty(aTaxes[2, nX, aScan(aTaxes[1], {|xIt| xIt[2] == "NEN_IDTRIB"})]) 
			If oGrid:length() = 0 .OR. nX > 1
				oGrid:AddLine()
			Endif
			oGrid:LoadValue("NEN_FILIAL", xFilial('NEN'))
			oGrid:LoadValue("NEN_CODNEG", __cCodNeg)
...
Function AGDA10TIMP(oGrid, lLoadGrid, lService, cCodNegoc)
	...
	elseIf (cAlias)->(!Eof())
		while (cAlias)->(!Eof())
			nX++
			//para manter padrão de nomenclatura seguir definição d metodo getFields() da classe agdImpostosRetidosNegocioRepository
			...
			aData[nX]['aliquotaSistema']       	:= (cAlias)->NEN_ALIQS
			aData[nX]['valorSistema']          	:= (cAlias)->NEN_VALSIS
			aData[nX]['aliquotaFinal']  		:= (cAlias)->NEN_ALIQF
			aData[nX]['valorFinal']     		:= (cAlias)->NEN_VALFIN
			(cAlias)->(dbSkip())
		endDo
	endif
