...
If Type("aPanAgeTMS") == "U"
	aPanAgeTMS := Array(6)
EndIf
aPanAgeBak := Aclone(aPanAgeTMS)

If IsInCallStac("TMSAF76") .And. Type("aPanAgeTMS") == "A" .And. Len(aPanAgeTMS) == 7 .And. aPanAgeTMS[1] != "DT6"
	aAreaDT6   := DUD->(GetArea())

	DT6->(DbSetOrder(1))
	DT6->(Dbseek(xFilial("DT6") + DUD->(DUD_FILDOC + DUD_DOC + DUD_SERIE)))
	
	aPanAgeTMS := Array(8)
	aPanAgeTMS[1] := "DT6"
	aPanAgeTMS[2] := DT6->(Recno())
	aPanAgeTMS[3] := 2
	aPanAgeTMS[6] := "TMSA500MNT"
	aPanAgeTMS[7] := Array(3)
	aPanAgeTMS[7,1] := {}
	aPanAgeTMS[7,2] := {}
	aPanAgeTMS[7,3] := {}
	RestArea(aAreaDT6)
