...
Função Dummy. Utilizada na verificação da existencia de fonte em findfunction
Retorna a Versão da Classe
...
METHOD New( cAliasConf, cCodFon ) CLASS TMSBCACOLENT
    ...
    If Empty(cCodFon)
        Do Case
        Case cAliasConf == "DN1"
            cFunName := AllTrim(Upper(Funname()))
            If "TMSAF92" == cFunName .Or. "TMSAF93" == cFunName
                cCodFon := "05" //FAT com COLENT
            ElseIf "OMS" $ cFunName
                cCodFon := "03" //OMS com COLENT
            Else
                cCodFon := "01" //TMS com COLENT
            EndIf
        Case cAliasConf == "DND"
            cCodFon := "04" //Portal logistico
        Case cAliasConf == "DNS"
            cCodFon := "10" //YMS
        Case cAliasConf == "DNT"
            cCodFon := "11" //Agendamento
        Case cAliasConf == "DNM"
            cCodFon := "HE" //Here
        EndCase
    EndIf
...
METHOD DbGetToken() CLASS TMSBCACOLENT
    ...
    Local cJson := "" As Caracter
    Local lRet := .F. As Logical
    Local aAreas As Array
    Local oQry As Object

    If !Empty(::Alias_Config)
        aAreas := { GetArea("DN6"), GetArea(::Alias_Config), GetArea() }
        QueryCfg( @::Alias_Config, ::codfon, @cAliasQry, @oQry, @cJson )

        If (cAliasQry)->(Eof())
            ::result_ok := ""
            ::all_error += ( ::last_error := STR0001 + ::Alias_Config + STR0002 ) + CRLF	//-- "Configuração " # " não encontrada."

        Else
            DbSelectArea(::Alias_Config)
            DbGoTo((cAliasQry)->RECNO)
            If Empty(::codfon)
                ::codfon := (::Alias_Config)->(FieldGet(FieldPos(::Alias_Config+"_CODFON")))
            EndIf
            ::access_token  := (::Alias_Config)->(FieldGet(FieldPos(::Alias_Config+"_TOKEN")))
            ...
            Endif
            If AliasInDic("DNM") .And. ::Alias_Config == "DNM"
            ...
            If FindFunction("TMSAI80Cfg")
                TMSAI80Cfg( ::codfon, , @cJson, .F. )
    	        ::CFGParans:FromJson(cJson)
            EndIf
            lRet := .T.
...
METHOD Post(cApiRun,cBody,cChangeURL,cProcesso) CLASS TMSBCACOLENT
    ...
    If ::GetActiveToken()[1]
        ...
        ElseIf ::codFon == "12" .And. HTTPGetStatus(@cHttpCode) == 201
...
DEFAULT cSubPrc     := ""
    ...
    If ::GetActiveToken()[1]
        If lTMC30Pst
            xTmpBody := ExecBlock("TMC30PST",.F.,.F.,{cApiRun,cQueryParam,cChangeURL,"GET"})
            If ValType(xTmpBody) == "A" .And. Len(xTmpBody) == 3
                cApiRun     := xTmpBody[1]
                cQueryParam := xTmpBody[2]
                cChangeURL  := xTmpBody[3]
            EndIf
        EndIf
...
Function TM30AltStt( aDUDStatus, nEndPoint, cText, cProces )
    ...
    If aToken[1]
        ...
        If !Empty(cProces)
            ...
            MntQryDn5( "04", nEndPoint, cProcesDTC, cText )
        Else
            For nX := 1 To Len(aDUDStatus)
                
                cProcesDTC := PadR( aDUDStatus[nX][1], Len(DN5->DN5_PROCES) )

                MntQryDn5( "04", nEndPoint, cProcesDTC, cText, aDUDStatus[nX][2] )
...
/*{Protheus.doc} QueryCfg()
Executa query de busca de dados de conexão e integração
@author     Carlos A. Gomes Jr.
@since      05/11/2025
*/
Static Function QueryCfg( cPrefix, cCodFon, cAliasQry, oQry, cJson )
    ...
    If lDefEsp
        cQuery := "SELECT "
        cQuery += "DN1.DN1_DTTOKE DTTOKE, "
        cQuery += "DN1.DN1_HRTOKE HRTOKE, "
        cQuery += "DN1.DN1_EXPIRE EXPIRE, "
        cQuery += "DN1.DN1_URLTOK URLTOK, "
        cQuery += "DN1.DN1_ID ID, "
        cQuery += "DN1.DN1_SECRET SECRET, "
        cQuery += "DN1.DN1_TENANT TENANT, "
        cQuery += "DN1.DN1_USER USUAR, "
        cQuery += "DN1.DN1_PASSW PASSW, "
        cQuery += "DN1.DN1_URLAPP URLAPP, "
        cQuery += "DN1.R_E_C_N_O_ RECNO "
        cQuery += "FROM " + RetSQLName("DN1") + " DN1 "
        cQuery += "INNER JOIN " + RetSQLName("DN0") + " DN0 ON "
        cQuery +=   "DN0.DN0_FILIAL = ? AND "
        cQuery +=   "DN0.DN0_CODIGO = DN1.DN1_CODCON AND "
        cQuery +=   "DN0.DN0_ATIVO  = '1' AND "
        cQuery +=   "DN0.DN0_CODMOD = ? AND "
        cQuery +=   "DN0.D_E_L_E_T_ = ' ' " //adicionado espaço. comentario para poder subir.
        cQuery += "WHERE " 
        cQuery +=   "DN1.DN1_FILIAL = ? AND "
        cQuery +=   "DN1.DN1_MSBLQL = '2' AND "
        If !Empty(cCodFon)
            cQuery +=   "DN1.DN1_CODFON = ? AND "
        EndIf
        cQuery +=   "DN1.D_E_L_E_T_ = ' '"
        oQry := FwExecStatement():New(cQuery)
        oQry:SetString(1,FWxFilial("DN0"))
        oQry:SetNumeric(2,nModulo)
        oQry:SetString(3,FWxFilial("DN1"))
        If !Empty(cCodFon)
            oQry:SetString(4,cCodFon)
        EndIf
        oQry:SetFields({{"DTTOKE", "D", 8, 0}})
        ...
        If (cAliasQry)->(Eof())
			...
			oQry := Nil
        Else
            DN1->(DbGoTo((cAliasQry)->RECNO))
            cJson := DN1->DN1_DEFESP
            cPrefix := "DN1"
            lAntigo := .F.
        EndIf
    EndIf
    ...
    If lAntigo
        ...
        cQuery += "INNER JOIN " + RetSQLName("DN0") + " DN0 ON "
        cQuery +=   "DN0.DN0_FILIAL = ? AND "
        cQuery +=   "DN0.DN0_CODIGO = " + cPrefix + "." + cPrefix + "_CODCON AND "
        cQuery +=   "DN0.DN0_ATIVO  = '1' AND "
        cQuery +=   "DN0.DN0_CODMOD = ? AND "
        cQuery +=   "DN0.D_E_L_E_T_ = ' ' " //adicionado espaço. comentario para poder subir.
        cQuery += "WHERE "
        cQuery +=   cPrefix + "." + cPrefix + "_FILIAL = ? AND "
        cQuery +=   cPrefix + "." + cPrefix + "_MSBLQL = '2' AND "
        If !Empty(cCodFon)
            cQuery +=   cPrefix + "." + cPrefix + "_CODFON = ? AND "
        EndIf
        cQuery +=   cPrefix + "." + "D_E_L_E_T_ = ' '"
        oQry := FwExecStatement():New(cQuery)
        oQry:SetString(1,FWxFilial("DN0"))
        oQry:SetNumeric(2,nModulo)
        oQry:SetString(3,FWxFilial(cPrefix))
        If !Empty(cCodFon)
            oQry:SetString(4,cCodFon)
        EndIf
        oQry:SetFields({{"DTTOKE", "D", 8, 0}})
        cAliasQry := oQry:OpenAlias()
        If !(cAliasQry)->(Eof())
            (cPrefix)->(DbGoTo((cAliasQry)->RECNO))
        EndIf
    EndIf
