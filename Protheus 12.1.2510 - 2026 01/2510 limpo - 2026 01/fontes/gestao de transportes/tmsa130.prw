...
Local lContHis  := SuperGetMv("MV_CONTHIS",.F.,.T.) //-- Controla Historico da Tabela de Frete
Local lAux,bWheAux 
Local lNoUpd := .F.

Local bCommit := {|oModel| TMSA130Grv(oModel)}
...
oModel:= MpFormMOdel():New("TMSA130",  /*bPreValid*/ , bPosValid, bCommit,/*bCancel*/ )
...
/*/-----------------------------------------------------------
{Protheus.doc} TMSA130Grv
Gravação
Uso: TMSA130
@sample
TMSA130Grv(oModelGrid)
@author Valdemar Roberto Mognon
@since 17/11/2025
@version 1.0
-----------------------------------------------------------/*/
Function TMSA130Grv(oModel)
Local oMdlFldDTL := oModel:GetModel("MdFieldCDTL")
Local oMdlGrdDVE := oModel:GetModel("MdGridIDVE")
Local oQueryDNJ  := Nil
Local oQueryDT1  := Nil
Local nOperation := oMdlFldDTL:GetOperation()
Local nCntFor1   := 0
...
Begin Transaction
	...
	For nCntFor1 := 1 To oMdlGrdDVE:Length()
		If oMdlGrdDVE:IsDeleted(nCntFor1)
			Aadd(aComponent, oMdlGrdDVE:GetValue("DVE_CODPAS", nCntFor1))
		EndIf
	Next nCntFor1
	
	lRet:= FwFormCommit(oModel)

	If lRet .And. !Empty(aComponent) .And. nOperation == MODEL_OPERATION_UPDATE
		cQuery += "SELECT DNJ.R_E_C_N_O_ REGISTRO "
		cQuery +=   "FROM " + RetSQLName("DNJ") + " DNJ "
		cQuery +=  "WHERE DNJ_FILIAL = ? "
		cQuery +=    "AND DNJ_CODPAS IN (?) "
		cQuery += 	 "AND D_E_L_E_T_ = ? "

		cQuery    := ChangeQuery(cQuery)
		oQueryDNJ := FwExecStatement():New(cQuery)
		oQueryDNJ:SetString(1,FWxFilial("DNJ"))
		oQueryDNJ:SetIn(2,aComponent)
		oQueryDNJ:SetString(3,' ')
		cAliasDNJ := oQueryDNJ:OpenAlias()

		While (cAliasDNJ)->(!Eof())
			DNJ->(DbGoTo((cAliasDNJ)->REGISTRO))
			RecLock("DNJ",.F.)
			DNJ->(DbDelete())
			DNJ->(MsUnlock())
			(cAliasDNJ)->(DbSkip())
		...
		(cAliasDNJ)->(DbCloseArea())
		oQueryDNJ:Destroy()
		oQueryDNJ := Nil

		//Excluir registros da DT1
		DT0->(dbSetOrder(1))              
		If DT0->( MsSeek( xFilial( "DT0" ) + oMdlFldDTL:GetValue("DTL_TABFRE") + oMdlFldDTL:GetValue("DTL_TIPTAB") ) )  
			cQuery := "SELECT DT1.R_E_C_N_O_ REGISTRO "
			...
			cQuery +=    	"AND DT1_CODPAS IN (?) "
			cQuery += 	 	"AND D_E_L_E_T_ = ' ' "

			cQuery    := ChangeQuery(cQuery)
			oQueryDT1 := FwExecStatement():New(cQuery)
			oQueryDT1:SetString(1, FWxFilial("DT1")) 
			oQueryDT1:SetString(2, DT0->DT0_TABFRE)
			oQueryDT1:SetString(3, DT0->DT0_TIPTAB)
			oQueryDT1:SetString(4, DT0->DT0_CDRORI)
			oQueryDT1:SetString(5, DT0->DT0_CDRDES)
			oQueryDT1:SetString(6, DT0->DT0_CODPRO)
			oQueryDT1:SetIn(7, aComponent)
			
			cAliasDT1 := oQueryDT1:OpenAlias()

			While (cAliasDT1)->(!Eof())
				DT1->(DbGoTo((cAliasDT1)->REGISTRO))
				RecLock("DT1",.F.)
				DT1->(DbDelete())
				DT1->(MsUnlock())
				(cAliasDT1)->(DbSkip())
