...
If !Empty(cXMLCTe) .Or. nTypeTrans == TRANS_RECEIVE
	...
	ElseIf nTypeTrans == TRANS_SEND
		...
		If SFT->(DbSeek(SF3->F3_FILIAL + cEntSai + SF3->F3_SERIE + SF3->F3_NFISCAL + SF3->F3_CLIEFOR + SF3->F3_LOJA))
			...
			//-- Busca CBS / IBS
			oTMSConfiguradorTributosWritten:getIdTrib(SF3->F3_FILIAL, SF3->F3_NFISCAL, SF3->F3_SERIE, SF3->F3_CLIEFOR, SF3->F3_LOJA)
...
//			cXMLRet +=		'<OperationCode>'    + Iif(cEspecie == "RPS",cOpeCodNF,Iif(DT6->DT6_DOCTMS == "A",cOpeCodC,cOpeCodN)) + '</OperationCode>'
			...
			Else
				...
				aCBSIBS := oTMSConfiguradorTributosWritten:processResponseSearchListDataSul('000062')

				If !Empty(aCBSIBS)
				//-- Lista CBS
					For nCntFor1 := 1 To Len(aCBSIBS)
						If aCBSIBS[nCntFor1][2] == '000062' // Tabela F2C |  CBS_FEDERAL         '000062'
							cXMLRet +=		'<Tax>'
							cXMLRet +=			'<Taxe>CBSFED</Taxe>
							cXMLRet +=			'<CalculationBasis>' + AllTrim(Str(aCBSIBS[nCntFor1,3])) + '</CalculationBasis>'
							cXMLRet +=			'<Percentage>' + AllTrim(Str(aCBSIBS[nCntFor1,4])) + '</Percentage>'
							cXMLRet +=			'<ReductionBasedPercent>' + AllTrim(Str(aCBSIBS[nCntFor1,5])) + '</ReductionBasedPercent>'
							cXMLRet +=			'<Value>' + AllTrim(Str(aCBSIBS[nCntFor1,6])) + '</Value>'
							cXMLRet +=			'<CodeTaxSituation>' + aCBSIBS[nCntFor1,7] + '</CodeTaxSituation>'
							cXMLRet +=		'</Tax>'
						EndIf
					Next nCntFor1
				...
				aCBSIBS := oTMSConfiguradorTributosWritten:processResponseSearchListDataSul('000060')

				If !Empty(aCBSIBS)
				//-- Lista IBSEST
					For nCntFor1 := 1 To Len(aCBSIBS)
						If aCBSIBS[nCntFor1][2] == '000060' // Tabela F2C |  IBS_ESTADUAL        '000060'
							cXMLRet +=		'<Tax>'
							cXMLRet +=			'<Taxe>IBSEST</Taxe>
							cXMLRet +=			'<CalculationBasis>' + AllTrim(Str(aCBSIBS[nCntFor1,3])) + '</CalculationBasis>'
							cXMLRet +=			'<Percentage>' + AllTrim(Str(aCBSIBS[nCntFor1,4])) + '</Percentage>'
							cXMLRet +=			'<ReductionBasedPercent>' + AllTrim(Str(aCBSIBS[nCntFor1,5])) + '</ReductionBasedPercent>'
							cXMLRet +=			'<Value>' + AllTrim(Str(aCBSIBS[nCntFor1,6])) + '</Value>'
				...
				aCBSIBS := oTMSConfiguradorTributosWritten:processResponseSearchListDataSul('000061')

				If !Empty(aCBSIBS)
				//-- Lista IBSMUN
					For nCntFor1 := 1 To Len(aCBSIBS)
						If aCBSIBS[nCntFor1][2] == '000061' // Tabela F2C |  IBS_MUNICIPAL       '000061'
							cXMLRet +=		'<Tax>'
							cXMLRet +=			'<Taxe>IBSMUN</Taxe>
							cXMLRet +=			'<CalculationBasis>' + AllTrim(Str(aCBSIBS[nCntFor1,3])) + '</CalculationBasis>'
							cXMLRet +=			'<Percentage>' + AllTrim(Str(aCBSIBS[nCntFor1,4])) + '</Percentage>'
							cXMLRet +=			'<ReductionBasedPercent>' + AllTrim(Str(aCBSIBS[nCntFor1,5])) + '</ReductionBasedPercent>'
							cXMLRet +=			'<Value>' + AllTrim(Str(aCBSIBS[nCntFor1,6])) + '</Value>'
