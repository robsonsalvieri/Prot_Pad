...
Local cVlrISS        := IIF(JurGetDados("SA1", 1, xFilial("SA1") + NXA->NXA_CLIPG + NXA->NXA_LOJPG, "A1_RECISS") == "1" .And. GetNewPar("MV_DESCISS",.F.), "NXA.NXA_ISS", "0")
Local nValor         := 0
Local aValor         := {}
...
Local aFatPag        := JurGetDados('NXA', 1, xFilial('NXA') + cCodEsc + cFatura, {'NXA_CMOEDA','NXA_CLIPG','NXA_LOJPG','NXA_CCLIEN','NXA_CLOJA' })
	...
	cQryPrin := " SELECT ( NXA.NXA_VLFATH + NXA.NXA_VLACRE - NXA.NXA_VLDESC - NXA.NXA_IRRF - NXA.NXA_PIS - NXA.NXA_COFINS - NXA.NXA_CSLL - NXA.NXA_INSS - " + cVlrISS + ") + NXA.NXA_VLFATD " + cCpoGrossH + " AS INV_TOTAL_NET_DUE,"
	cQryPrin +=          " NXA.*, "
	cQryPrin +=          " NS7.*, "
	cQryPrin +=          " NUH.NUH_CLIEBI, "
	cQryPrin +=          " SU5.U5_CONTAT, "
	cQryPrin +=          " SU5.U5_EMAIL, "
	cQryPrin +=          " SU5.U5_DDD, "
	cQryPrin +=          " SU5.U5_FONE, "
	cQryPrin +=          " SU5.U5_FAX, "
	cQryPrin +=          " CTO.CTO_DESC, "
	cQryPrin +=          " CTO.CTO_CODISO, "
	cQryPrin +=          " RD0.RD0_NOME, "
	cQryPrin +=          " SYA_ESC.YA_SIGLA PAIS_ESC, "
	cQryPrin +=          " SYA_CLI.YA_SIGLA PAIS_CLI, "
	...
	If TRBPRIN->(!Eof())
		...
		Aadd(aArqXml,'<ledesxmlebilling2.0.4 xmlns="http://www.ledes.org/ledes204.xsd">')
		...
		Do While TRBPRIN->(!Eof()) .And. aRet[1]
			...
			Do While TRBPRIN->(!Eof()) .And. cCodChv == TRBPRIN->NS7_COD .And. aRet[1]
				...
				Aadd(aArqXml,Space(9)+"<contact_lname>" + RetFLName(1, TRBPRIN->NS7_CTNAC) + "</contact_lname>")
				Aadd(aArqXml,Space(9)+"<contact_fname>" + RetFLName(2, TRBPRIN->NS7_CTNAC) + "</contact_fname>")
				If !Empty(TRBPRIN->NXA_CPART)
					Aadd(aArqXml,Space(9)+"<contact_id>"+Alltrim(TRBPRIN->NXA_CPART)+"</contact_id>")
				EndIf
				If !Empty(TRBPRIN->NS7_TEL)
					Aadd(aArqXml,Space(9)+"<contact_phone>"+Alltrim(TRBPRIN->NS7_TEL)+"</contact_phone>")
					Aadd(aArqXml,Space(9)+"<contact_fax>"+Alltrim(TRBPRIN->NS7_TEL)+"</contact_fax>")
				EndIf
				Aadd(aArqXml,Space(9)+"<contact_email>"+Alltrim(TRBPRIN->NS7_EMAIL)+"</contact_email>")
				Aadd(aArqXml,Space(9)+"<file_item_nbr>" + Alltrim(Str(nItemNbr++)) + "</file_item_nbr>")
				Aadd(aArqXml,Space(6)+"</lf_billing_contact>")
				...
				//=========================================
					...
					Aadd(aArqXml,Space(9)+"<cl_id>"+Alltrim(Iif(Empty(TRBPRIN->NUH_CLIEBI), TRBPRIN->NXA_CCLIEN+TRBPRIN->NXA_CLOJA, TRBPRIN->NUH_CLIEBI))+"</cl_id>") 
					Aadd(aArqXml,Space(9)+"<cl_lf_id>"+Alltrim(Iif(Empty(TRBPRIN->NUH_CLIEBI), TRBPRIN->NXA_CCLIEN+TRBPRIN->NXA_CLOJA, TRBPRIN->NUH_CLIEBI))+"</cl_lf_id>")
					Aadd(aArqXml,Space(9)+"<cl_name>"+Alltrim(TRBPRIN->NXA_RAZSOC)+"</cl_name>")               
					Aadd(aArqXml,Space(9)+"<cl_address>")
					Aadd(aArqXml,Space(12)+"<address_1>"+Alltrim(TRBPRIN->NXA_ENDENT)+"</address_1>")
					Aadd(aArqXml,Space(12)+"<city>"+Alltrim(TRBPRIN->CC2_MUN)+"</city>")
					...
					Aadd(aArqXml,Space(12)+"<contact_lname>"+RetFLName(1, TRBPRIN->U5_CONTAT)+"</contact_lname>")
					Aadd(aArqXml,Space(12)+"<contact_fname>"+RetFLName(2, TRBPRIN->U5_CONTAT)+"</contact_fname>")
					If !Empty(TRBPRIN->U5_CONTAT)
						Aadd(aArqXml,Space(12)+"<contact_id>"+RetFLName(2, TRBPRIN->U5_CONTAT)+"</contact_id>")
					EndIf
					If !Empty(TRBPRIN->U5_DDD) .And. !Empty(TRBPRIN->U5_FONE)
						Aadd(aArqXml,Space(12)+"<contact_phone>"+Alltrim(TRBPRIN->U5_DDD + " " + TRBPRIN->U5_FONE)+"</contact_phone>")
					EndIf
					If !Empty(TRBPRIN->U5_FAX)
						Aadd(aArqXml,Space(12)+"<contact_fax>"+Alltrim(TRBPRIN->U5_FAX)+"</contact_fax>")
					EndIf

					Aadd(aArqXml,Space(12)+"<contact_email>"+Alltrim(TRBPRIN->U5_EMAIL)+"</contact_email>")
					Aadd(aArqXml,Space(12)+"<file_item_nbr>" + Alltrim(Str(nItemNbr++)) + "</file_item_nbr>")
					Aadd(aArqXml,Space(9)+"</client_contact>")
					Aadd(aArqXml,Space(9)+"<file_item_nbr>" + Alltrim(Str(nItemNbr++)) + "</file_item_nbr>")
					//=========================================
					...
					Aadd(aArqXml,Space(12)+"<inv_due_date>"+JSToFormat(Alltrim(TRBPRIN->NXA_DTVENC), "YYYY-MM-DD")+"</inv_due_date>")
					...
					Aadd(aArqXml,Space(12)+"<rejected_inv_replacement>N</rejected_inv_replacement>")
					...
					Aadd(aArqXml,Space(12)+"<inv_total_net_due>"+Alltrim(Str(nMInvTotDue, , 2))+"</inv_total_net_due>")
					Aadd(aArqXml,Space(12)+"<file_item_nbr>" + Alltrim(Str(nItemNbr++)) + "</file_item_nbr>")
					
					cQryMatter := " SELECT NXC.*, NVE.NVE_TITULO, NVE.NVE_CPGEBI, NVE.NVE_CCLIEN || NVE.NVE_LCLIEN CLI_LOJ, NVE.NVE_MATTER, NVE.NVE_NUMCAS,"
					cQryMatter +=        " NVE.NVE_TITEBI, NT7.NT7_TITULO "
					...
					If TRBMATT->(!Eof())
						...
						Do While TRBMATT->(!Eof()) .And. aRet[1]
							...
							Else
								Aadd(aArqXml,Space(15)+"<cl_matter_id>"+Alltrim(TRBMATT->CLI_LOJ)+"</cl_matter_id>")
							EndIf
							...
							Else
								...
								Aadd(aArqXml,Space(15)+"<lf_matter_id>"+Alltrim(TRBMATT->NVE_NUMCAS)+"</lf_matter_id>")
							EndIf
							...
							Else
								...
								If (nMDesconto > 0 .Or. nMAcresc > 0) .And. TRBMATT->NXC_VLTS <= 0 .And. TRBMATT->NXC_VFIXO <= 0
									cMAdjEXP  := Alltrim(Str(nMAcresc - nMDesconto))
								Else
									cMAdjFees := Alltrim(Str(nMAcresc - nMDesconto))
								EndIf
								
								cMAdj := Alltrim(Str(nMAcresc - nMDesconto))
							...
							lFee := nMNetFees > 0 // Tem honorários
							lExp := nMNetExp  > 0 // Tem despesas
							
							Aadd(aArqXml,Space(15)+"<matter_total_detail_fees>" + IIf(lFee, cMTotDetFees, "0") + "</matter_total_detail_fees>")
							Aadd(aArqXml,Space(15)+"<matter_total_detail_exp>" + IIf(lExp, cMTotDetExp, "0") + "</matter_total_detail_exp>")
							Aadd(aArqXml,Space(15)+"<matter_disc_credit_fees>" + IIf(lFee, cMAdjFees, "0") + "</matter_disc_credit_fees>")
							Aadd(aArqXml,Space(15)+"<matter_disc_credit_exp>" + IIf(lExp, cMAdjEXP, "0") + "</matter_disc_credit_exp>")
							Aadd(aArqXml,Space(15)+"<matter_disc_cred_total>" + cMAdj + "</matter_disc_cred_total>")
							Aadd(aArqXml,Space(15)+"<matter_perc_shar_fees>" + IIf(lFee, AllTrim(Str(nPercFat, , 2)), "0") + "</matter_perc_shar_fees>")
							Aadd(aArqXml,Space(15)+"<matter_perc_shar_exp>" + IIf(lExp, AllTrim(Str(nPercFat, , 2)), "0") + "</matter_perc_shar_exp>")
							Aadd(aArqXml,Space(15)+"<matter_disc_bill_pct_fees>" + IIf(lFee, cPerDesFee, "0") + "</matter_disc_bill_pct_fees>")
							Aadd(aArqXml,Space(15)+"<matter_tax_on_fees>0</matter_tax_on_fees>")
							Aadd(aArqXml,Space(15)+"<matter_tax_on_exp>0</matter_tax_on_exp>")
							Aadd(aArqXml,Space(15)+"<matter_net_fees>" + IIf(lFee, cMNetFees, "0") + "</matter_net_fees>")
							Aadd(aArqXml,Space(15)+"<matter_net_exp>" + IIf(lExp, cMNetExp, "0") + "</matter_net_exp>")
							Aadd(aArqXml,Space(15)+"<matter_funds_applied>0</matter_funds_applied>")
							Aadd(aArqXml,Space(15)+"<matter_total_due>" + cMTotalDue + "</matter_total_due>")
							Aadd(aArqXml,Space(15)+"<associated_line_items>N</associated_line_items>")
							nPosAsLnIt := Len(aArqXml) // Guarda posição do associated_line_items
							...
							If 	nMAcresc > 0
								...
								Aadd(aArqXml,Space(18)+"<disc_cred_type>Flat</disc_cred_type>")
								...
								Aadd(aArqXml,Space(18)+"<pre_post_split>NA</pre_post_split>")
							...
							If 	nMDesconto > 0
								...
								Aadd(aArqXml,Space(18)+"<disc_cred>" + cDisCred + "</disc_cred>")
								Aadd(aArqXml,Space(18)+"<disc_cred_type>Percent</disc_cred_type>")
							...
							If TRBTK->(!Eof())
								Do While TRBTK->(!Eof()) .And. aRet[1]
									...
									EndIf
									If !Empty(TRBTK->NRV_DESC)
										Aadd(aArqXml,Space(18)+"<tk_level>" + AjustClass(Alltrim(TRBTK->NRV_DESC)) + "</tk_level>")
									EndIf
									...
									Aadd(aArqXml,Space(18)+"<tk_rate_other_iso>"+Alltrim(Str(nValor))+"</tk_rate_other_iso>")
									Aadd(aArqXml,Space(18)+"<tk_other_iso></tk_other_iso>")
									Aadd(aArqXml,Space(18)+"<file_item_nbr>" + Alltrim(Str(nItemNbr++)) + "</file_item_nbr>")
									Aadd(aArqXml,Space(15)+"</tksum>")
									TRBTK->(DbSkip())
								EndDo
							...
							cQryFee +=  " INNER JOIN " + RetSqlname("NUS") + " NUS "
							cQryFee +=     " ON (NUS.NUS_FILIAL = '" + xFilial("NUS") + "'"
							cQryFee +=    " AND NUS.NUS_CPART  = NUE.NUE_CPART2"
							cQryFee +=    " AND NUE.NUE_ANOMES BETWEEN NUS_AMINI AND CASE WHEN NUS_AMFIM = '" + Space(TamSx3('NUS_AMFIM')[1]) + "' THEN '999999' ELSE NUS_AMFIM END"
							cQryFee +=    " AND NUS.D_E_L_E_T_ = ' ')"

							cQryFee +=  " INNER JOIN " + RetSqlname("NR2") + " NR2"
							cQryFee +=     " ON (NR2.NR2_FILIAL = '" + xFilial("NR2") + "'"
							cQryFee +=    " AND NR2.NR2_CATPAR = NUS.NUS_CCAT"
							...
							cQryFee +=    " AND RD0.RD0_CODIGO = NUE.NUE_CPART2"
							...
							cQryFee +=    " AND NS2.NS2_CCATEJ = NUS.NUS_CCAT"
							...
							TRBFEE->(dbGoTop())
							If TRBFEE->(!Eof())
								// Ajusta o associated_line_items do matter indicando que existe honorários ou despesas
								aArqXml[nPosAsLnIt] := Space(15)+"<associated_line_items>Y</associated_line_items>"
								Do While TRBFEE->(!Eof()) .And. aRet[1]
									...
									Aadd(aArqXml,Space(18)+"<tk_level>" + AjustClass(Alltrim(TRBFEE->NR2_DESC)) + "</tk_level>")
									...
									aValor := JA201FConv( cMoeEbi, TRBFEE->NUE_CMOEDA, nValorH, '8', dEmiFat, , , , cCodEsc, cFatura )
									If !Empty(aValor[4])
										IIF(lAutomato, aRet := {.F., aValor[4]}, Alert(aValor[4]))
										aRet[1] := .F.
										Exit
									...
									nFeeUnit       := Round(TRBFEE->NUE_TEMPOR, 2)
									nFeeRate       := Round(nValorH, 4)
									nFeeBaseAmount := Round(nValor1, 2)
									...
									aLog := LD98VlLanc(TRBFEE->NUE_COD, TRBFEE->NUE_CCLIEN, TRBFEE->NUE_CLOJA, TRBFEE->NUE_CCASO, TRBFEE->NUH_CEMP,;
										TRBFEE->NRV_CCATE , TRBFEE->NUS_CCAT, TRBFEE->RD0_SIGLA, TRBFEE->NRY_CFASE, TRBFEE->NRZ_CTAREF, TRBFEE->NS0_CATIV, "TS", aLog, "cEscEbi", "cEscrit" )
							...
							cQryExpens +=         " AND NXA.D_E_L_E_T_ = ' '"
							cQryExpens += " )EXPENSES"
							cQryExpens += " ORDER BY EXPENSES.IDENT ,EXPENSES.CHARGE_DATE, EXPENSES.EXP_CODIGO"
		...
		If aRet[1]
			Aadd(aArqXml,Space(3)+"</firm>")
			Aadd(aArqXml,"</ledesxmlebilling2.0.4>")
...
//-------------------------------------------------------------------
/*/{Protheus.doc} AjustClass
Retorna a classificação/categoria do participante no formato do arquivo LEDES

@param cClass     - Classificação/Categoria

@Return cClassFrm - Classificação/Categoria tratado no formato exigido pelo XSD LEDES

@author SISJURI
@since  16/12/2024
/*/
//-------------------------------------------------------------------
Static Function AjustClass(cClass)
Local cClassFrm := "ASSOC"

	cClass := Upper(NoAcento(cClass)) // Tira os acentos e deixa tudo em maiusculo

	If "SOCIO" $ cClass .Or. "PARTNER" $ cClass // Sócio
		cClassFrm := "PARTNR"
	
	ElseIf "PARALEGAL" $ cClass // Paralegal
		cClassFrm := "PARALG"

	ElseIf "TRAINEE" $ cClass // Trainee
		cClassFrm := "TRANEE"

	ElseIf "ANALIST" $ cClass .Or. "ANALYST" $ cClass // Analista
		cClassFrm := "ANALST"

	ElseIf "AUDITOR" $ cClass // Auditor
		cClassFrm := "AUDITR"

	ElseIf "ESCRITURARIO" $ cClass .Or. "SECRETARI" $ cClass .Or. "CLERK" $ cClass .Or. "SECRETARY" $ cClass // Escriturário ou secretário
		cClassFrm := "CLKSEC"

	ElseIf "CONSULTOR" $ cClass .Or. "CONSULTANT" $ cClass // Consultor
		cClassFrm := "CONSLT"

	ElseIf ("ADVOGAD" $ cClass .And. ("TEMPORARIO" $ cClass .Or. "TERCEIRIZADO" $ cClass)) .Or. ("CONTRACT" $ cClass .And. "ATTORNEY" $ cClass) // Advogados temporários/terceirizados
		cClassFrm := "TEMPAT"

	ElseIf (!("ADVOGAD" $ cClass) .And. ("TEMPORARIO" $ cClass .Or. "TERCEIRIZADO" $ cClass)) .Or. ("CONTRACT" $ cClass .And. !("ATTORNEY" $ cClass)) // Não advogados temporários/terceirizados
		cClassFrm := "TEMPOS"

	ElseIf "ADVOGAD" $ cClass .Or. "ATTORNEY" $ cClass // Advogado do escritório
		cClassFrm := "STFATT"

	ElseIf ("GERENTE" $ cClass .And. "PROJETO" $ cClass) .Or. "LEGAL PROJECT MANAGER" $ cClass // Gerente de projeto jurídico
		cClassFrm := "LGPRMG"

	ElseIf "ESTAGIARIO" $ cClass .Or. "LEGAL INTERN" $ cClass // Estagiário
		cClassFrm := "LGLINT"

	ElseIf "ASSISTENTE" $ cClass .Or. "AUXILIAR" $ cClass .Or. "LEGAL ASSISTANT" $ cClass // Assistente/Auxiliar jurídico
		cClassFrm := "LGLAST"

	ElseIf "ESPECIALISTA" $ cClass .Or. "EXPERT" $ cClass // Especialista
		cClassFrm := "EXPERT"
