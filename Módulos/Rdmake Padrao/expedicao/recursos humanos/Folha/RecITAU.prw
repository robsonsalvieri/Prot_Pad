...
User Function RecItau()
	...
	Private lPETpMovi 	:= ExistBlock( "GPM933TPMOV" )
	Private lPEDisponi 	:= ExistBlock( "GPM933DISPO" )
...
Static Function GERRImp()
	...
	aAdd( aTipo2, { 'TIPO DE MOVIMENTO            		 ',	 015, 017, 	 "N",	3,	0,	.T., "If(lPETpMovi,ExecBlock('GPM933TPMOV',.F.,.F.),'000')"									 	} ) // TIPO DE MOVIMENTO
	...
	aAdd( aTipoHE, { 'DISPONIBILIZAÇÃO 					 ',	 226, 227, 	 "C",   2,	0,	.T., "If(lPEDisponi,ExecBlock('GPM933DISPO',.F.,.F.),'01')"								 	 } ) //PRAZO PARA DISPONIBILIZAÇÃO DO HOLERITE
