...
/*/{Protheus.doc} nomeStaticFunction
	...
	/*/
WSRESTFUL tmschart007 DESCRIPTION STR0007   //-- "Documentos X Prazos de Entrega"
...
/*/{Protheus.doc} nomeStaticFunction
	...
	@see (links_or_references)
	/*/
WSMETHOD GET form WSSERVICE tmschart007
...
/*/{Protheus.doc} nomeStaticFunction
	...
	@see (links_or_references)
	/*/
WSMETHOD POST itemsDetails WSRECEIVE JsonFilter, drillDownFilter WSRESTFUL tmschart007
	...
	If oJsonFilter:GetJsonText("level") == "null" .Or. Len(oJsonFilter["level"]) == 0
		If Len(oJsonDD) == 0
			...
			aHeader := {;
				...
				{"nomrem"       ,  FWX3Titulo('DT6_NOMREM') ,           ,,.T.,.T.     },;    // #"Nome Remetente"
...
/*/{Protheus.doc} nomeStaticFunction
	...
	@see (links_or_references)
	/*/
WSMETHOD POST retdados WSRECEIVE JsonFilter WSSERVICE tmschart007
...
Static Function retDados(oResponse, oCoreDash, oJson)
	...
	If oJson:GetJsonText("level") == "null" .Or. Len(oJson["level"]) == 0
		...
		aAdd(aCab, STR0081 + " 2 " + STR0077  ) //-- Vencidos X Dias
		aAdd(aCab, STR0081 + " 5 " + STR0077  ) //-- Vencidos X Dias
		aAdd(aCab, STR0081 + " 7 " + STR0077  ) //-- Vencidos X Dias
		aAdd(aCab, STR0081 + " 7 " + STR0077 + " " +STR0101 ) //-- Venidos X Dias  "ou mais"


		//-- Vencidos
		aAdd(aData, TmsCd07Ven( dDataBase - 2 , dDataBase - 1 ) )
		aAdd(aData, TmsCd07Ven( dDataBase - 5 , dDataBase - 3 ) )
		aAdd(aData, TmsCd07Ven( dDataBase - 7 , dDataBase - 6 ) )
		aAdd(aData, TmsCd07Ven( dDataBase - 365 , dDataBase - 8 ) )
		...
		aDataFim := {}
		aAdd(aDataFim, oCoreDash:SetChart({STR0007},,/*lCurrency*/.F.,, STR0007  )) //-- "Documentos X Prazos de Entrega"
	EndIf
...
Static Function QueryDTQ(cSelect as Char, cFilter as Char)
    ...
    cQuery  += " AND DT6_PRZENT     >= '" + DToS(dDataBase - 365 ) + "' "
	cQuery  += " AND DT6_PRZENT     <= '" + DToS(dDataBase - 1 ) + "' "
