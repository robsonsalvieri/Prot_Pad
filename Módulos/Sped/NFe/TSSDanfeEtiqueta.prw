...
/*/
//-------------------------------------------------------------------
function ImpDfEtq(cUrl, cIdEnt, lUsaColab)
    local lOk        := .F.
    local aArea      := {}
    local aAreaCB5   := {}
    local aAreaSF3   := {}
...
static function DanfeSimp(oPrinter, nPosY, nPosX, oFontTit, oFontInf, aEmit, aNfe, aDest)
    ...
    if len(aEmit[2]) == 11
        ...
        oPrinter:Say( 161 + nPosY, 200 + nPosX, Transform(aEmit[2],"@R! NN.NNN.NNN/NNNN-99"), oFontInf)
    endif
    ...
    if len(aDest[2]) == 11
        ...
        oPrinter:Say( 244 + nPosY, 200 + nPosX, Transform(aDest[2], "@R! NN.NNN.NNN/NNNN-99"), oFontInf)
    endif
...
static function impZebra(aNFe, aEmit, aDest)
    ...
    if lEmitJurid
        MSCBSay(15, 66.25, transform( aEmit[nCNPJ], "@R! NN.NNN.NNN/NNNN-99" ), "N", "A", cFontMenor) //Emitente pessoa jurídica
    else
    ...
    if lDestJurid
        MSCBSay(15, 120, transform( aDest[nCNPJ], "@R! NN.NNN.NNN/NNNN-99" ), "N", "A", cFontMenor) //Destinatário pessoa jurídica
    else
