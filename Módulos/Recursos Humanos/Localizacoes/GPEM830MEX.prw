...
±±³gsantacruz  ³28/07/11³ Corr ³Se coloco formato a los salarios.         ³±±
...
±±³R.Berti     ³13/09/12³TFTLRJ³Creacion del fuente GPEM830MEX;           ³±±
±±³            ³        ³      ³Se corrije funciones para estaticas.      ³±±
±±³Laura Medina³23/10/14³TQRHAK³Se modifico para generar el archivo por   ³±±
±±³            ³        ³      ³Registro Patronal (y no Suc+Reg. Patronal)³±±
...
cQuery += " ORDER BY RCP_CODRPA,RCP_FILIAL,RCP_MAT"
...
While !(cAliasTmp)->(EOF())           
	...
	nTamLin := 168
	Do while !(cAliasTmp)->(EOF()) .and. cRegPat==(cAliasTmp)->RCP_CODRPA //cFilTmp+cRegPat==(cAliasTmp)->RCP_FILIAL+(cAliasTmp)->RCP_CODRPA
		nMax++;nNum++                                                                               
	    cLin    := Space(nTamLin)
