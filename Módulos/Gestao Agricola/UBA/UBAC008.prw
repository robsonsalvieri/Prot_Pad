...
Function UBAC008()
	...
	Else
		...
		ADD COLUMN oColumn DATA { || &(aArqTemp[9,1]) } TITLE STR0009 SIZE  aArqTemp[9,3]  ;
...
/*
UBAC008TRB  Cria arquivo de trabalho com dados para exibição da consulta
@param: Nil
@author: Inácio Luiz Kolling
@since: 10/06/2014
@Uso: UBA
...
Static Function UBAC008P()
	...
	While (cAlias)->(!Eof())
		...
		While (cAlias)->(!Eof()) .AND. (cAlias)->DXD_CODIGO = cBloco
			...
			nPsSai += If((!Empty((cAlias)->DXI_CODRES) .And. !Empty((cAlias)->DXI_ROMSAI)),(cAlias)->DXI_PSLIQU,0)
			nPSRes += If((!Empty((cAlias)->DXI_CODRES) .And.  Empty((cAlias)->DXI_ROMSAI)),(cAlias)->DXI_PSLIQU,0)			
		...
		Reclock(cAliasTRB,.T.)
		(cAliasTRB)->BLOCO	 := cBloco
		(cAliasTRB)->SAFRA 	 := cSafra
		(cAliasTRB)->DATAEMB := If(ValType(dData) = "C",StoD(dData),dData)
		(cAliasTRB)->TIPO	 := cTipo
		(cAliasTRB)->FARDOS	 := nQTDFr
		(cAliasTRB)->SAIDA1	 := nQTDSa
		(cAliasTRB)->RESERVA := nQTDRe
		(cAliasTRB)->PESOT	 := nPesoL
		(cAliasTRB)->SAIDA2	 := nPsSai
		(cAliasTRB)->RESERVA2:= nPSRes
		(cAliasTRB)->SALDOF  := nPesoL - (nPsSai + nPSRes)	
		(cAliasTRB)->SALDO1  := (nQTDFr - (nQTDSa + nQTDRe))
		(cAliasTRB)->ENDEREC := cEnder // Não mexer essa linha
		Msunlock()
		DbselectArea(cAlias)
	Enddo
Return
...
Static Function ReportDef()
	...
	oReport:SetTotalInLine(.F.)//Define que o acumulador será impresso em linhas.
//---------
// Seção 1
//---------
	...
	oSec1:SetReadOnly(.F.) 		// Define que o usuário não poderá alterar informações da seção, ou seja, não poderá remover as células pré-definidas.
	oSec1:ShowHeader()			// Define se apresenta titulo da seção
	...
	oSec1:SetTotalText(STR0046) // Texto da seção totalizadora //"Total Geral"
