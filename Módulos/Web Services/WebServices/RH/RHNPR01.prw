...
/*/{Protheus.doc} ImpNotVacation
Geração do Aviso de Férias pelo Meu RH
/*/

Function ImpNotVacation( aDados )
...
Local cFileName   := "" //nome do arquivo pdf esperado na saida
Local cLocal      := "" //local para a gravação do arquivo
...
// aDados[22] = local de impressão      ( local de impressão)
// aDados[23] = nome do arquivo         ( nome a ser impresso)
...
/*/{Protheus.doc} ImpRecVacation
Geracao do arquivo PDF no App MeuRH do recibo de Ferias e/ou Abono e 1a. Parcela do 13o. 
/*/
Function ImpRecVacation( cBranchVld, cMatSRA, cEmpSRA, cRecno, cLocal, cFileName )
	...
	If !Empty(aCabec) .And. !Empty(aVerbas) .And. !Empty(aInfo) 
		...
		cNameFun	:= If(lNomeSoc .And. !Empty(SRA->RA_NSOCIAL), SRA->RA_NSOCIAL, SRA->RA_NOMECMP)
		cNameFun    := Alltrim(EncodeUTF8(If(Empty(cNameFun), SRA->RA_NOME, cNameFun)))
