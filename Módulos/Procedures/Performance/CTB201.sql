...
/* ------------------------------------------------------------------------------------
    ...
    Descricao       - <d>  Atualizar Cubo 02 - CCUSTO </d>
    ...
    Entrada         - <ri> @IN_LATUAL   - Se '1' atualiza CVX, se '2' atualiza CVY e se '3', Atualiza Ambas
                           ...
                           @IN_VALOR    - Valor a adicionar no Debito ou Credito
...
begin
   ...
   If @IN_LATUAL in ('1', '3') begin
      ...
      If @iRecnoCVX is null or @iRecnoCVX = 0 begin
         ...
         insert into CVX### ( CVX_FILIAL,   CVX_CONFIG, CVX_MOEDA, CVX_DATA, CVX_TPSALD, CVX_SLDCRD, CVX_SLDDEB, CVX_NIV01, CVX_NIV02, R_E_C_N_O_  )
                     values ( @cFilial_CVX, @cConfig,   @IN_MOEDA, @IN_DATA, @IN_TPSALD, @nValCrd,   @nValDeb,   @IN_CONTA, @IN_CUSTO, @iRecnoCVX )
   ...
   If @IN_LATUAL in ('2', '3') begin
      ...
      If @iRecnoCVY is null or @iRecnoCVY = 0 begin
         ...
         ##CHECK_TRANSACTION_BEGIN @IN_TRANSACTION\
         insert into CVY### ( CVY_FILIAL,   CVY_CONFIG, CVY_MOEDA, CVY_DATA, CVY_TPSALD, CVY_SLDCRD, CVY_SLDDEB, CVY_NIV01, CVY_NIV02, R_E_C_N_O_ )
                     values ( @cFilial_CVY, @cConfig,   @IN_MOEDA, @cDataF,  @IN_TPSALD, @nValCrd,   @nValDeb,   @IN_CONTA, @IN_CUSTO, @iRecnoCVY )
         ##CHECK_TRANSACTION_COMMIT
         ##FIMTRATARECNO
      end else begin
         ##CHECK_TRANSACTION_BEGIN @IN_TRANSACTION\
