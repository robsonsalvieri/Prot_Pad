...
##IF_002({|| lColPer05 := (cPaisLoc $ 'COL|PER' .And. CtbMovSaldo("CT0",,"05") .And. FWAliasInDic('QL6') .And. CT2->(FieldPos('CT2_EC05DB'))>0)})
   declare @cEC05DB_NEW Char( 'CT2_EC05DB' )
   declare @cEC05CR_NEW Char( 'CT2_EC05DB' )
   declare @cEC05DB_TRW Char( 'CT2_EC05DB' )
   declare @cEC05CR_TRW Char( 'CT2_EC05DB' )      
##ELSE_002
   declare @cEC05DB_NEW Char( 01 )
   declare @cEC05CR_NEW Char( 01 )
...
begin
   ...
   select @cEC05DB_NEW = ' ', @cEC05CR_NEW = ' ', @cEC05DB_TRW = ' ', @cEC05CR_TRW = ' '
   ...
   While ( @@Fetch_status = 0 ) begin
      ...
      select @cTpSaldo = CT2_TPSALD, @cDc = CT2_DC,        @nValor = CT2_VALOR,  @cDebito = CT2_DEBITO,  
            ...
            ##ENDIF_004
            ##IF_005({|| lColPer05})
               , @cEC05DB_TRW = CT2_EC05DB
               , @cEC05CR_TRW = CT2_EC05CR
            ##ENDIF_005
        From TRW###
      ...
      If @iRecnoTRW is not null begin
         ...
         If @cDcN      = @cDc     and @cDebitoN = @cDebito and @cCreditN = @cCredit and @cCustoDN  = @cCustoD  and
            ...
            ##ENDIF_005
            ##IF_006({|| lColPer05})
               and @cEC05DB_NEW = @cEC05DB_TRW
               and @cEC05CR_NEW = @cEC05CR_TRW
            ##ENDIF_006
            begin
            select @lExec = '0'
         end else begin
         ...
         If @lExec = '1' begin
            ...
            exec CTB189_## @IN_FILIAL, @cOper,   @cDc,    @cDebito, @cCredit, @cCustoD,     @cCustoC, @cItemD, @cItemC, @cClvlD, @cClvlC, @cEC05DB_TRW, @cEC05CR_TRW,
                           @cTpSaldo,  @cMoedaN, @cDataN, @cDtlpN,  @nValor,   @IN_INTEGRID,@IN_TRANSACTION, @cResult OutPut
            /*---------------------------------------------------------------
              Exclusão/subtração de Totais por lote
              --------------------------------------------------------------- */
            exec CTB233_##  @cFilial_CT2, @cDataN,  @IN_LOTE,  @IN_SBLOTE, @IN_DOC, @cMoedaN, @cTpSaldo,  @cDc, @cOper, @IN_MVSOMA, @nValor,@IN_TRANSACTION, @cResult OutPut
            /*-------------------------------------------------------------------
            ...
            /*---------------------------------------------------------------
              ...
              --------------------------------------------------------------- */
            exec CTB189_## @IN_FILIAL, @cOper,   @cDcN,   @cDebitoN, @cCreditN, @cCustoDN,    @cCustoCN, @cItemDN, @cItemCN, @cClvlDN, @cClvlCN, @cEC05DB_NEW, @cEC05CR_NEW,
                           @cTpSaldoN, @cMoedaN, @cDataN, @cDtlpN,   @nValorN,  @IN_INTEGRID,  @IN_TRANSACTION, @cResult OutPut
            /*---------------------------------------------------------------
              Soma de Totais por lote
              --------------------------------------------------------------- */
            exec CTB233_## @cFilial_CT2, @cDataN,  @IN_LOTE,  @IN_SBLOTE, @IN_DOC, @cMoedaN, @cTpSaldoN,  @cDcN, @cOper, @IN_MVSOMA, @nValorN,  @IN_TRANSACTION, @cResult OutPut
            /*-------------------------------------------------------------------
   ...
   While ( @@Fetch_status = 0 ) begin
      select @cOper = '-'
      exec CTB189_## @IN_FILIAL, @cOper,   @cDcN,   @cDebitoN, @cCreditN,    @cCustoDN, @cCustoCN, @cItemDN, @cItemCN, @cClvlDN, @cClvlCN, @cEC05DB_TRW, @cEC05CR_TRW,
                     @cTpSaldoN, @cMoedaN, @cDataN, @cDtlpN, @nValorN,  @IN_INTEGRID,@IN_TRANSACTION, @cResult OutPut
      /*---------------------------------------------------------------
        Soma de Totais por lote
        --------------------------------------------------------------- */
