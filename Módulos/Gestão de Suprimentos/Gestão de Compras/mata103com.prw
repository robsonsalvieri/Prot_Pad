...
lRet := (Len(aAltDKD) > 0) .Or. lDKDValBru  //Possui campo customizado
...
lSeekDkd := DKD->(MsSeek(xFilial("DKD") + cNFiscal + cSerie + cA100For + cLoja + aColsDKD[nPosIt][nItDKD] + DtoS(dDEmissaoA) + cEspecieA))
If !lSeekDkd
	lGrv := .T. 
Endif

If lDKDValBru .And. nItD1 > 0 .And. nItDKD > 0
