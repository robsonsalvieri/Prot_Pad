...
QSSTRUCT MATQ12101 DESCRIPTION STR0001 MODULE 2	//-- Pedidos em aberto por fornecedores

QSMETHOD INIT QSSTRUCT MATQ12101
...
QSPARENTFIELD "C7_FORNECE" INDEX ORDER 3 LABEL STR0002	//-- Código do Fornecedor
...
//-- Opcoes de filtro
QSFILTER STR0005 WHERE "C7_TIPO = 1 AND C7_QUJE < C7_QUANT AND C7_RESIDUO <> 'S' AND C7_EMISSAO >= '" +DToS(Date() - 30) +"'"	//-- Últimos 30 dias
QSFILTER STR0006 WHERE "C7_TIPO = 1 AND C7_QUJE < C7_QUANT AND C7_RESIDUO <> 'S' AND C7_EMISSAO >= '" +DToS(Date() - 60) +"'" //-- Últimos 60 dias
QSFILTER STR0007 WHERE "C7_TIPO = 1 AND C7_QUJE < C7_QUANT AND C7_RESIDUO <> 'S' AND C7_EMISSAO >= '" +DToS(Date() - 90) +"'"	//-- Últimos 90 dias
QSFILTER STR0008 WHERE "C7_TIPO = 1 AND C7_QUJE < C7_QUANT AND C7_RESIDUO <> 'S' AND C7_EMISSAO >= '" +DToS(Date() - 120) +"'"	//-- Últimos 120 dias
QSFILTER STR0009 WHERE "C7_TIPO = 1 AND C7_QUJE < C7_QUANT AND C7_RESIDUO <> 'S' AND C7_EMISSAO >= '" +DToS(Date() - 360) +"'"	//-- Últimos 360 dias
QSFILTER STR0010 WHERE "C7_TIPO = 1 AND C7_QUJE < C7_QUANT AND C7_RESIDUO <> 'S'"	//-- Todos

//-- Campos da consulta rapida
QSFIELD "C7_FILIAL" LABEL STR0011	//-- Empresa
QSFIELD "C7_NUM" LABEL STR0012		//-- Número do Pedido
QSFIELD "C7_FORNECE" LABEL STR0002	//-- Código do Fornecedor
QSFIELD "C7_LOJA" LABEL STR0013		//-- Loja
QSFIELD "A2_NOME" LABEL STR0003		//-- Nome do Fornecedor
QSFIELD "A2_CGC" LABEL STR0004		//-- CNPJ/CPF
QSFIELD "MOEDA" BLOCK {|| SuperGetMV("MV_MOEDA"+AllTrim(Str(C7_MOEDA)),.F.,"")} FIELDS "C7_MOEDA" LABEL STR0014 TYPE "C" SIZE 15	//-- Moeda
...
//-- Acoes relacionadas
QSACTION MENUDEF "MATA121" OPERATION 2 LABEL STR0018	//-- Detalhes do Pedido
QSACTION MENUDEF "MATA020" OPERATION 2 LABEL STR0019	//-- Detalhes do Fornecedor

Return

QSSTRUCT MATQ12102 DESCRIPTION STR0020 MODULE 2	//-- Pedidos em aberto por produtos

QSMETHOD INIT QSSTRUCT MATQ12102

//-- Tabelas envolvidas na consulta e seus relacionamentos
QSTABLE "SC7" JOIN "SA2" ON "A2_COD = C7_FORNECE AND A2_LOJA = C7_LOJA"
QSTABLE "SC7" JOIN "SB1" ON "B1_COD = C7_PRODUTO"
QSTABLE "SC7" LEFT JOIN "SB5" ON "B5_COD = C7_PRODUTO"
QSTABLE "SC7" LEFT JOIN "SC1" ON "C1_PEDIDO = C7_NUM AND C1_ITEMPED = C7_ITEM"

//-- Campos/índices utilizados para a pesquisa
QSPARENTFIELD "C7_PRODUTO" INDEX ORDER 2 LABEL STR0021	//-- Código do Produto
QSPARENTFIELD "C7_NUM" INDEX ORDER 1 LABEL STR0012	//-- Número do Pedido
QSPARENTFIELD "B1_DESC" INDEX ORDER 3 SET RELATION TO "C7_PRODUTO" WITH "B1_COD" LABEL STR0022	//-- Descrição do Produto
QSPARENTFIELD "C1_NUM" INDEX ORDER 1 SET RELATION TO "C7_NUM","C7_ITEM" WITH "C1_PEDIDO","C1_ITEMPED" LABEL STR0023	//-- Número da SC
