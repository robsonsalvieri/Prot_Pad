...
oStrSC8:AddField(	STR0024,;									// 	[01]  C   Titulo do campo  - "Descrição"
					STR0024,;									// 	[02]  C   ToolTip do campo - "Descrição"
					 ...
					 NIL,;										//	[09]  A   Lista de valores permitido do campo
					 .F.,;										//	[10]  L   Indica se o campo tem preenchimento obrigatório
					 {|| NFCProdDesc(if(empty(SC8->C8_PRODUTO),fwfldget("C8_PRODUTO"),SC8->C8_PRODUTO), if(empty(SC8->C8_NUMSC),fwfldget("C8_NUMSC"),SC8->C8_NUMSC), if(empty(SC8->C8_ITEMSC),fwfldget("C8_ITEMSC"),SC8->C8_ITEMSC)) },; //	[11]  B   Code-block de inicializacao do campo
...
If (lBrazil)
	...
	oStrDHU:AddField(	STR0045,;							// 	[01]  C   Titulo do campo -- 'Total DIFAL'
						STR0045,;							// 	[02]  C   ToolTip do campo
						"DHU_TOTDIF",;								// 	[03]  C   Id do Field
						"N",;										// 	[04]  C   Tipo do campo
						TamSX3("C8_DIFAL")[1],;						// 	[05]  N   Tamanho do campo
						TamSX3("C8_DIFAL")[2],;						// 	[06]  N   Decimal do campo
						NIL,;										// 	[07]  B   Code-block de validação do campo
						NIL,;										// 	[08]  B   Code-block de validação When do campo
						NIL,;										//	[09]  A   Lista de valores permitido do campo
						.F.,;										//	[10]  L   Indica se o campo tem preenchimento obrigatório
...
If (lBrazil)
	...
	oStrSC8:SetProperty("C8_DIFAL"	, MODEL_FIELD_VALID, FWBuildFeature(STRUCT_FEATURE_VALID, "NF020ItTax('C8_DIFAL',   'DHU_TOTDIF')") )
...
cCpoExcl += "C8_TAXAFOR|C8_TOTFRE|C8_TOTPCO|C8_ITEMPCO|C8_TPDOC|C8_TPFRETE|C8_TXMOEDA|C8_VALEMB|C8_VALFRE|C8_VALIDA|C8_WF|C8_XGCT|C8_XREVISA"
...
If (lBrazil)
	...
	oStrDHU:AddField(	"DHU_TOTDIF",;							// [01]  C   Nome do Campo
							"28",;								// [02]  C   Ordem
							STR0045,;					// [03]  C   Titulo do campo	//'Valor DIFAL'
							STR0045,;						// [04]  C   Descricao do campo	//'Valor DIFAL'
							NIL,;								// [05]  A   Array com Help
							"N",;								// [06]  C   Tipo do campo
							PesqPict("SC8","C8_DIFAL"),;		// [07]  C   Picture
							NIL,;								// [08]  B   Bloco de Picture Var
							NIL,;								// [09]  C   Consulta F3
							.T.,;								// [10]  L   Indica se o campo é alteravel
							NIL,;								// [11]  C   Pasta do campo
...
If (lBrazil)
	...
	oStrDHU:SetProperty( 'DHU_TOTDIF' , MVC_VIEW_GROUP_NUMBER, 'GRP_NFCA020_004' )
...
If (lBrazil)
	...
	oStrSC8:SetProperty('C8_DIFAL'	, MVC_VIEW_ORDEM,'24')
...
local aFieldVld	:= {'C8_VLDESC', 'C8_VALIPI', 'C8_VALICM', 'C8_VALSOL', 'C8_ICMSCOM', 'C8_VALISS', 'C8_DIFAL'}
local nVlrTmp	:= 0
local cField    := ''
...
Static Function iniFldsTax(oModel)
	...
	If _oJsonSup['newParticipant'] .Or. _oJsonSup['cleanProposal']
		...
		oModelDHU:LoadValue('DHU_TOTDIF'  ,0)
	...
	Else
		...
		cQuery += " SUM(C8_VALICM) TOTICMS, SUM(C8_DIFAL) TOTDIFAL, SUM(C8_ICMSCOM) TOTICMSCOM, SUM(C8_VALSOL) TOTICMSSOL "
		cQuery += " FROM " + RetSQLName("SC8") + " SC8"
		cQuery += "   WHERE "
		cQuery += "     SC8.C8_FILIAL = ? "
		cQuery += "     AND SC8.C8_NUM = ?"
		...
		If !(cAliasTmp)->(eof())
			...
			oModelDHU:LoadValue('DHU_TOTDIF' , (cAliasTmp)->TOTDIFAL)
...
Function recalTot(oModel)
	...
	if oModel != nil .and. oModel:isActive()
		...
		If cPaisLoc == "BRA"
			nTotal += oModelDHU:GetValue("DHU_TOTIPI") + oModelDHU:GetValue("DHU_TOTISO") + oModelDHU:GetValue("DHU_TOTICM") + ;
...
If !(fwfldget(cTaxField) < 0)

    nPosic := aScan(aRetImp, {|X| X[1] == cTaxField})
    if nPosic > 0
        lNoMax := iif(fwfldget(cTaxField) > 100, .f., .t.)
    endif

	if ( lNoMax .or. VldValues(fwfldget("C8_TOTAL"), fwfldget(cTaxField), .t.) )
...
if nOpc == 1 // -- Digitando o campo C8_CODTAB
	if NF020VldTab(1,cForn,cLoja,cCodTab)
		nVlrTab := GetAdvFval("AIB","AIB_PRCCOM",fwxFilial("AIB") + cForn + cLoja + cCodTab + Alltrim(oModelSC8:GetValue('C8_PRODUTO')),2)
		if nVlrTab > 0
...
elseif nOpc == 2 // Alteração em massa via outras ações
	...
	for nX := 1 to oModelSC8:Length()
		oModelSC8:GoLine(nX)
		cCodTab := GetAdvFval("SA5","A5_CODTAB",fwxFilial("SA5") + cForn + cLoja + Alltrim(oModelSC8:GetValue("C8_PRODUTO")),1)
...
Function NF020Calc(lSaveF2D)
	...
	If !Empty(oModelDHU:GetValue('DHU_COND'))
		...
		DbSelectArea("SC8")
		SC8->(DbSetOrder(1))
		SC8->(MsSeek(FWxFilial('SC8')+PADR(oModelSC8:GetValue("C8_NUM"), TAMSX3("C8_NUM")[1])))

		aadd(aCabec ,{"C8_FORNECE"   ,PADR(_oJsonSup['supplierCode']        , TAMSX3("C8_FORNECE")[1])})
		aadd(aCabec ,{"C8_LOJA"      ,PADR(_oJsonSup['supplierStore']       , TAMSX3("C8_LOJA"   )[1])})
		aadd(aCabec ,{"C8_FORNOME"   ,PADR(_oJsonSup['supplierName' ]   	, TAMSX3("C8_FORNOME")[1])})
		aadd(aCabec ,{"C8_COND"      ,PADR(oModelDHU:GetValue("DHU_COND")	, TAMSX3("C8_COND"   )[1])})
		aadd(aCabec ,{"C8_FILENT"    ,PADR(oModelSC8:GetValue("C8_FILENT") 	, TAMSX3("C8_FILENT" )[1])})
		...
		aadd(aCabec ,{"C8_TPFRETE"   ,oModelSC8:GetValue("C8_TPFRETE")          })
		aadd(aCabec ,{"C8_VLDESC"    ,oModelSC8:GetValue("C8_VLDESC")           })
		aadd(aCabec ,{"C8_DESPESA"   ,oModelSC8:GetValue("C8_DESPESA")          })
		aadd(aCabec ,{"C8_SEGURO"    ,oModelSC8:GetValue("C8_SEGURO")           })
		...
		for nX := 1 to nTamGrid
			oModelSC8:GoLine(nX)
			aadd(aItens,{{"C8_NUMPRO"   ,PADR(oModelSC8:GetValue("C8_NUMPRO")     , TAMSX3("C8_NUMPRO" )[1]),Nil},;
						{"C8_PRODUTO"   ,PADR(oModelSC8:GetValue("C8_PRODUTO")	  , TAMSX3("C8_PRODUTO")[1]),Nil},;
						{"C8_ITEM"      ,PADR(oModelSC8:GetValue("C8_ITEM")       , TAMSX3("C8_ITEM"   )[1]),Nil},;
						{"C8_TES"       ,PADR(oModelSC8:GetValue("C8_TES")        , TAMSX3("C8_TES"    )[1]),Nil},;
						{"C8_UM"        ,PADR(oModelSC8:GetValue("C8_UM")     	  , TAMSX3("C8_UM"     )[1]),Nil},;
			...
			if (lExsC8CF)
				aadd(aItens[nX], {"C8_CF", PADR(oModelSC8:GetValue("C8_CF"), TAMSX3("C8_CF")[1]), Nil})
			endif

			if (_lTrbGen)
				aadd(aItens[nX], {"C8_IDTRIB", PADR(oModelSC8:GetValue("C8_IDTRIB"), TAMSX3("C8_IDTRIB")[1]), Nil})
			endif
		next nX
			...
			for nX := 1 to len(aImpVal)
				...
				if oModelSC8:GetValue('C8_TOTAL') > 0 //-- Deve definir impostos apenas quando o total do item for maior que zero
					for nZ := 1 to len(aImpVal[nX][2])
						...
						if ( (oTypView == 'O' .And. oView:HasField("VIEW_SC8", cField)) .Or. (oTypView <> 'O' .And. oModelSC8:HasField(cField)) .or. (lExsC8CF .and. cField == "C8_IDTRIB") .or.;
							( cField == "C8_BASEICM" .and. oModelSC8:HasField(cField)) ) //-- Procura se o campo de imposto está na view
...
local aCamposTx	:= {"C8_VALICM", "C8_VALIPI", "C8_VALSOL", "C8_ICMSCOM", "C8_VALISS", "C8_DIFAL", "C8_ALIQCMP", "C8_ALIQISS", "C8_PICM"}
local nFor		:= 0
default oView	:= FwViewActive()
...
local aCampos 	:= {"C8_VALICM", "C8_VALIPI", "C8_VALSOL", "C8_ICMSCOM", "C8_VALISS", "C8_DIFAL", "C8_ALIIPI", "C8_BASEIPI", "C8_BASESOL"}
local nFor		:= 0
...
Static Function NF020TOTTAX()
	...
	For nX := 1 to nSizeGrid
		...
		nValDIFAL	+= oModelSC8:GetValue('C8_DIFAL')
	...
	oModelDHU:LoadValue('DHU_TOTDIF', nValDIFAL		)
