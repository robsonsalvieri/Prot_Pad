...
For nX := 1 to Len(oGetCheck:aCols)
   	...
   	If !oGetCheck:aCols[nX,nUsado+1] .And. !oGetArea:Acols[Len(oGetArea:Acols),Len(oGetArea:aHeader)+1]
		...
		Else     
			...
			//Verifica se o check-list esta obsoleto
			cUltRevCkl := oQADA100Aux:retornaProximaRevisaoEfetivadaDoCheckList(oGetCheck:aCols[nX][nPosChkAud])

			If oGetCheck:aCols[nX][nPosChkRev] <> cUltRevCkl				
				//STR0076 - "O CheckList:" <checklist> <descri> 
				//STR0077 - "está obsoleto!" 
				//STR0078 - "Gostaria de substituir automaticamente pela revisão mais recente?"
				If MSGYESNO(STR0076 + oGetCheck:aCols[nX][nPosChkAud] + STR0077 + STR0078,"020CHKOBS")
					oGetCheck:aCols[nX][nPosChkRev] := cUltRevCkl
