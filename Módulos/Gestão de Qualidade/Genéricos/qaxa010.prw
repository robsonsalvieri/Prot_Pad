		...
		WHILE QDZTRB->(!EOF())
			If (nPos:= aScan(aTpPen,{|x| x[POS_aTpPen_TPPEND] == "I"})) > 0
				...
				If (nPos := aScan(aPenDoc,{|x| x[POS_PendDoc_TPPEND] == "I  "                   .And.;
					...
					aAdd(aPenDoc,{"I  ",QDZTRB->QDZ_DOCTO,QDZTRB->QDZ_RV,.T.,"X",INI_CAMPO_ADOCTOS_FIL_MAT,QDZTRB->R_E_C_N_O_,QDZTRB->QDZ_FILIAL,1,"P", "QDZ", cFilBsc, cMatBsc, cDepBsc})
...
Method montaTelaTransferenciaResponsabilidade() CLASS QAXA010AuxClass
	...
	ACTIVATE MSDIALOG oDlgMain ;
			ON INIT EnchoiceBar( oDlgMain, { || aAreaQAAOk := QAA->(GetArea()),;
			                                    nOpcao := QAX010FIM(nItem2,nItem3,nItem4,nItem5,aPenDoc,aTpPen,cCcFilial,cCcPara,cNDepto,cCcMatr,cFilAtu,cMatAtu,cDepAtu,oDlgMain,Nil,aAvisos),;
												IF(nOpcao == 1, oDlgMain:End(), ""), RestArea(aAreaQAAOk)},;
										   { || nOpcao := 2, oDlgMain:End() } )
...
Method montaPainelOpcoes(oPanOpcao, lEdicao) CLASS QAXA010AuxClass
	...
	@ 013,005 RADIO oItem4 VAR nItem4 3D SIZE 145,007 OF oPanOpcao PIXEL WHEN lEdicao;
	...
	@ 013,218 RADIO oItem5 VAR nItem5 3D SIZE 300,007 OF oPanOpcao PIXEL WHEN lEdicao;
...
Method montaTelaTransferenciaDepartamento() CLASS QAXA010AuxClass
	...
	// STR0020 - "Transferencia de Usuarios"
	DEFINE MSDIALOG oDlgMain TITLE OemToAnsi(STR0020) FROM aSize[7],000 TO aSize[6],aSize[5] OF oMainWnd PIXEL
	...
	@000,000 MSPANEL oPanelMain PROMPT " "+OemToAnsi(STR0107) SIZE 080,060 OF oDlgMain
	oPanelMain:Align := CONTROL_ALIGN_ALLCLIENT


	//STR0107 - "Transferencia de departamento"
	@000,000 MSPANEL oPanTras PROMPT " "+OemToAnsi(STR0107) SIZE 080,060 OF oPanelMain
	oPanTras:Align := CONTROL_ALIGN_TOP


	//STR0026 - "Filial"
	@ 015,006 SAY OemToAnsi(STR0026) SIZE 025,007 OF oPanTras PIXEL
	...
	//STR0021 - "Usuario"
	@ 028,006 SAY OemToAnsi(STR0021) SIZE 025,007 OF oPanTras PIXEL
	...
	//STR0059 - "Depto"
	@ 040,006 SAY OemToAnsi(STR0059) SIZE 035,007 OF oPanTras PIXEL
	...
	@ 000,010 Say oSayHtr VAR ""  SIZE 010,010 OF oPanelMain PIXEL 
	...
	@ 075,006 LISTBOX oHistCC VAR cUsr;
				...
				ALIAS "QAB" ;
				SIZE 300,085 OF oPanelMain PIXEL
	...
	ACTIVATE MSDIALOG oDlgMain CENTERED ;
			ON INIT EnchoiceBar( oDlgMain, { || nOpcao := QAX010FIM(nItem2,nItem3,nItem4,nItem5,aPenDoc,aTpPen,cCcFilial,cCcPara,cNDepto,cCcMatr,cFilAtu,cMatAtu,cDepAtu,oDlgMain,Nil,aAvisos),;
												IF(nOpcao == 1, oDlgMain:End(), "")},;
										   { || nOpcao := 2, oDlgMain:End() } )
