...
While .t.
	...
	ElseIf nTipo == 3 // producao
			If lVT100B
				...
				VTRead
			Else
				@ 1,00 VTSAY STR0008 VTGet cDoc      pict '@!'          when empty(cDoc) .or. VtLastkey()==5  Valid VldDoc(cDoc) // //'Documento '
...
For nP:= 1 to len(aItensPallet)
   ...
   If Empty(cNota+cSerie+cFornec+cLoja+cDoc) .and. ! UsaCB0("01") .And. CB0->CB0_ORIGEM <>"SD3" 
	...
	ElseIf !Empty(cNota+cSerie+cFornec+cLoja) .And.If(!UsaCB0("01"),.T.,CB0->CB0_ORIGEM <>"SD3")
		...
		aChavPesq :=RetNumSeq(cChave+aEtiqueta[1],aEtiqueta[2]*nQtdePro,cTipDis,cLote,cSLote,CB0->CB0_ITNFE)
		If Empty(aChavPesq)
			aChavPesq := RetNSeqSDA(aEtiqueta[1],aEtiqueta[2]*nQtdePro,cLote,cSLote)
			If !Empty(aChavPesq) 
				cTipDis   := "SDA"
...
elseIf lForcaQtd
	...
	nQtdePro := Iif( lChkBalance, 1, nAuxQtde )
...
Local nPos    	 := 0
Local nX
Local lTpConfFis := SuperGetMV("MV_TPCONFF",.F.,"1") == "2"
...
For nX:=1 to Len(aNF)
	...
	If lForConf
		If !Empty(aNumSeq) .AND. !Empty(aNF) .AND. lConfFis .AND. lTpConfFis
			SF1->(DbSetOrder(1))			
...
//---------------------------------------------------------------------------------------------------------
/*/{Protheus.doc} RetNumSeq
Retorna array com  o número sequencial do produto
...
@param cItemNfe, caracter, Informa o item da nota referente ao campo CB0_ITNFE
@return aNumSeq, Array, Contém o resultado da execução
			aNumSeq[1] Caracter, contém o número sequencial do produto 
			aNumSeq[2] numeric , contém o saldo do produto a endereçar
			aNumSeq[1] Caracter, contém o tipo de distribuição do produto por nota SD1 ou movimentação interna SD3
/*/
//---------------------------------------------------------------------------------------------------------
Static Function RetNumSeq(cChave,nQtde,cTipDis,cLote,cSLote,cItemNfe)
Local aNumSeq :={}
Local aArea   := SDA->(GetArea())
...
If cTipDis == "SD1"
	...
	Else
		While SD1->(!Eof() .and. xFilial('SD1')+cChave == ;
			...
			// Verifica se o item da nota fiscal a endereçar é diferente do item da etiqueta lida, somente quando utilizar CB0
			If UsaCB0( "01" ) .And.( !Empty( cItemNfe ) .And. SD1->D1_ITEM <> cItemNfe )
				SD1->(DbSkip())
				Loop 
			EndIf
...
Local nPos,cKey,nQtd,cProd,nPosID,cChavPesq
Local nX,nP
Local aEtiqueta,nSaldo,nQtdeBx
Local cLote     := Space(TamSX3("B8_LOTECTL")[1])
Local cSLote    := Space(TamSX3("B8_NUMLOTE")[1])
Local cNumSeri  := Space(TamSX3("BF_NUMSERI")[1])
Local cLocali	:= Space(TamSX3("CB0_LOCALI")[1])
Local aDistBKP  := aClone(aDist)
Local aHisEtiBKP:= aClone(aHisEti)
Local aGrava    :={}
Local aArea   	:={}
...
For nP:= 1 to len(aItensPallet)
	...
	If UsaCB0("01")
		...
		//Estorno do aDist
		If Empty(aDist)
			aadd(aDist,{aEtiqueta[12],aEtiqueta[1],aEtiqueta[2],CB0->CB0_FILIAL,{{cEtiqueta,aEtiqueta[2],CB0->CB0_CODETI}},cLote,cSLote,cNumseri})
		EndIf
		nPos := aScan(aDist,{|x| AllTrim(x[1]) == Alltrim(cKey) .and. x[2] == cProd})
		aadd(aGrava,{aDist[nPos,4],aDist[nPos,1],nQtd*-1})
...
For nX:= 1 to len(aGrava)
	...
	//Atualiza SBF/CB0/SDA
	If UsaCB0("01")
		aArea := GetArea()

		SBF->(dbSetOrder(1)) //BF_FILIAL+BF_LOCAL+BF_LOCALIZ+BF_PRODUTO+BF_NUMSERI+BF_LOTECTL+BF_NUMLOTE
		If SBF->(DbSeek(xFilial("SBF")+aEtiqueta[7]+aEtiqueta[9]+cProd+cNumseri+cLote+cSLote))
			RecLock("SBF",.F.)
			SBF->(DBDelete())
			SBF->(MsUnLock())
		EndIf
		...
		If SDA->(DbSeek(xFilial("SDA")+aEtiqueta[1]+aEtiqueta[7]+aEtiqueta[12]))                                                                                                                                      
			RecLock("SDA",.F.)
			SDA->DA_SALDO := nQtd
			SDA->(MsUnLock())
		EndIf
