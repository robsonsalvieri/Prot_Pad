...
±±≥FunáÖo	 ≥ Ctbr295	≥ Autor ≥ Marcos S. Lobo    	≥ Data ≥ 31/01/05   ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Comparativo de Saldo Mes a Mes Cta+C.Custo+Item+Cl.Valor	  ≥±±
...
/// DETERMINA O CABE«ALHO DE ACORDO COM OS PARAMETROS
/// Se usar a Conta
If mv_par28 == 1
	aAdd(aEnts,STR0011)
	cString := "CT1"
...
//≥Monta ParamBox para indicar como ser· a ordem de impressao≥
//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
If Len(aEnts) > 1	// APENAS SE USAR + DE 1 ENTIDADE
	/// GUARDA CONTEUDO DOS MV_PAR J¡ ABERTOS (SE HOUVER) - Chamada de ParamBox na funÁ„o (muda os mv_pars)
	For nPar := 1 To 40
	...
	While !lSair
		If !ParamBox(aPars,STR0032,aRet,,,,,,,,.F.,)
			aRet := aClone(aEnts)
			lSair := .T.
			Exit
		Else	// SE CONFIRMOU
...
For nQt := 1 to Len(aRet)
	If nQt > 1			/// A PARTIR DA 2™ COLUNA COLOCA VIRGULA
		cDesc2 += "/"
	EndIf
	If AllTrim(aRet[nQt]) == AllTrim(STR0011)	// CONTA
		AADD(aOrdem,"CONTA")   		
...
If lRet
	If Empty(mv_par12) .And. Empty(mv_par13)
		MsgAlert(STR0031)	//"As perguntas 'Tipo de Saldo' e 'Saldo a Comparar' n„o podem ficar em branco"
		lRet := .F.
	EndIf
EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥Program   ≥CTR295IMP ≥ Autor ≥ Marcos S. Lobo        ≥ Data ≥ 31/01/05 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Imprime -> Comparativo de Saldo Mes a Mes - Cta+Custo+Item ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
...
While !Eof()
	...
	************************* ROTINA DE IMPRESSAO *************************
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	// Se imprime a variaÁ„o percentual entre as linhas A e B
	///////////////////////////////////////////////////////////////////////////////////////////////
	If mv_par25 <> 2
		If (mv_par25 == 1 .or. mv_par25 == 3) .and. (lTemSldA .and. lTemSldB)	/// SE INDICOU O 1∫ e 2∫ TIPOS DE SALDO - (AxB)
			// Caso linha A ou B estejam zerados a variaÁ„o devem ser 100% (pelo c·lculo daria zero)
			IIF (lIsRedStor,nVar1	:= ROUND(( (COLB1*nFatRedSto) / (COLA1*nFatRedSto) )*100,2)		, nVar1	:= ROUND((COLB1/COLA1)*100,2)		)
			IIF (lIsRedStor,nVar2	:= ROUND(( (COLB2*nFatRedSto) / (COLA2*nFatRedSto) )*100,2)		, nVar2	:= ROUND((COLB2/COLA2)*100,2)       )
			IIF (lIsRedStor,nVar3	:= ROUND(( (COLB3*nFatRedSto) / (COLA3*nFatRedSto) )*100,2)		, nVar3	:= ROUND((COLB3/COLA3)*100,2)       )
			IIF (lIsRedStor,nVar4	:= ROUND(( (COLB4*nFatRedSto) / (COLA4*nFatRedSto) )*100,2)     , nVar4	:= ROUND((COLB4/COLA4)*100,2)       )
			IIF (lIsRedStor,nVar5	:= ROUND(( (COLB5*nFatRedSto) / (COLA5*nFatRedSto) )*100,2)     , nVar5	:= ROUND((COLB5/COLA5)*100,2)       )
			IIF (lIsRedStor,nVar6	:= ROUND(( (COLB6*nFatRedSto) / (COLA6*nFatRedSto) )*100,2)     , nVar6	:= ROUND((COLB6/COLA6)*100,2)       )
			IIF (lIsRedStor,nVar7	:= ROUND(( (COLB7*nFatRedSto) / (COLA7*nFatRedSto) )*100,2)     , nVar7	:= ROUND((COLB7/COLA7)*100,2)       )
			IIF (lIsRedStor,nVar8	:= ROUND(( (COLB8*nFatRedSto) / (COLA8*nFatRedSto) )*100,2)     , nVar8	:= ROUND((COLB8/COLA8)*100,2)       )
			IIF (lIsRedStor,nVar9	:= ROUND(( (COLB9*nFatRedSto) / (COLA9*nFatRedSto) )*100,2)     , nVar9	:= ROUND((COLB9/COLA9)*100,2)       )
			IIF (lIsRedStor,nVar10	:= ROUND(( (COLB10*nFatRedSto) / (COLA10*nFatRedSto) )*100,2)   , nVar10	:= ROUND((COLB10/COLA10)*100,2) )
			IIF (lIsRedStor,nVar11	:= ROUND(( (COLB11*nFatRedSto) / (COLA11*nFatRedSto) )*100,2)   , nVar11	:= ROUND((COLB11/COLA11)*100,2) )
			IIF (lIsRedStor,nVar12	:= ROUND(( (COLB12*nFatRedSto) / (COLA12*nFatRedSto) )*100,2)   , nVar12	:= ROUND((COLB12/COLA12)*100,2) )
			//Imprime as Varia??es em Percentual
			li++
			@ li,aColunas[COL_SEPARA1]  PSAY "|"
			@ li,aColunas[COL_CONTA]    PSAY STR0029+" AxB (%)" ///"VariaÁ„o"
	 		@ li,aColunas[COL_SEPARA3]  PSAY "|"
		...
		If (mv_par25 == 1 .or. mv_par25 == 4) .and. (lTemSldA .and. lTemSldC)	/// SE INDICOU O 1∫ e 2∫ TIPOS DE SALDO - (AxB)
			// Caso linha A ou B estejam zerados a variaÁ„o devem ser 100% (pelo c·lculo daria zero)
			IIF (lIsRedStor, nVar1	:= ROUND(( (COLC1*nFatRedSto) / (COLA1*nFatRedSto) )*100,2)		, nVar1	:= ROUND((COLC1/COLA1)*100,2)      )
			IIF (lIsRedStor, nVar2	:= ROUND(( (COLC2*nFatRedSto) / (COLA2*nFatRedSto) )*100,2)     , nVar2	:= ROUND((COLC2/COLA2)*100,2)      )
			IIF (lIsRedStor, nVar3	:= ROUND(( (COLC3*nFatRedSto) / (COLA3*nFatRedSto) )*100,2)     , nVar3	:= ROUND((COLC3/COLA3)*100,2)      )
			IIF (lIsRedStor, nVar4	:= ROUND(( (COLC4*nFatRedSto) / (COLA4*nFatRedSto) )*100,2)     , nVar4	:= ROUND((COLC4/COLA4)*100,2)      )
			IIF (lIsRedStor, nVar5	:= ROUND(( (COLC5*nFatRedSto) / (COLA5*nFatRedSto) )*100,2)     , nVar5	:= ROUND((COLC5/COLA5)*100,2)      )
			IIF (lIsRedStor, nVar6	:= ROUND(( (COLC6*nFatRedSto) / (COLA6*nFatRedSto) )*100,2)     , nVar6	:= ROUND((COLC6/COLA6)*100,2)      )
			IIF (lIsRedStor, nVar7	:= ROUND(( (COLC7*nFatRedSto) / (COLA7*nFatRedSto) )*100,2)     , nVar7	:= ROUND((COLC7/COLA7)*100,2)      )
			IIF (lIsRedStor, nVar8	:= ROUND(( (COLC8*nFatRedSto) / (COLA8*nFatRedSto) )*100,2)     , nVar8	:= ROUND((COLC8/COLA8)*100,2)      )
			IIF (lIsRedStor, nVar9	:= ROUND(( (COLC9*nFatRedSto) / (COLA9*nFatRedSto) )*100,2)     , nVar9	:= ROUND((COLC9/COLA9)*100,2)      )
			IIF (lIsRedStor, nVar10	:= ROUND(( (COLC10*nFatRedSto) / (COLA10*nFatRedSto) )*100,2)   , nVar10	:= ROUND((COLC10/COLA10)*100,2))
			IIF (lIsRedStor, nVar11	:= ROUND(( (COLC11*nFatRedSto) / (COLA11*nFatRedSto) )*100,2)   , nVar11	:= ROUND((COLC11/COLA11)*100,2))
			IIF (lIsRedStor, nVar12	:= ROUND(( (COLC12*nFatRedSto) / (COLA12*nFatRedSto) )*100,2)   , nVar12	:= ROUND((COLC12/COLA12)*100,2))
			//Imprime as Varia??es em Percentual
			li++	
			@ li,aColunas[COL_SEPARA1]  PSAY "|"
			@ li,aColunas[COL_CONTA]    PSAY STR0029+" AxC (%)" //"VariaÁ„o"
	 		@ li,aColunas[COL_SEPARA3]  PSAY "|"
			@ li,aColunas[COL_COLUNA1]  PSAY Transform(nVar1,"9,999,999.99 %")
			@ li,aColunas[COL_SEPARA4]  PSAY "|"
			@ li,aColunas[COL_COLUNA2]  PSAY Transform(nVar2,"9,999,999.99 %")
		...
		If (mv_par25 == 1 .or. mv_par25 == 5) .and. (lTemSldB .and. lTemSldC)	/// SE INDICOU O 1∫ e 2∫ TIPOS DE SALDO - (AxB)
			// Caso linha A ou B estejam zerados a variaÁ„o devem ser 100% (pelo c·lculo daria zero)
			IIf (lIsRedStor , nVar1		:= ROUND(( (COLC1*nFatRedSto) / (COLB1*nFatRedSto) )*100,2)		, nVar1		:= ROUND((COLC1/COLB1)*100,2)		)
			IIF (lIsRedStor , nVar2		:= ROUND(( (COLC2*nFatRedSto) / (COLB2*nFatRedSto) )*100,2)     , nVar2		:= ROUND((COLC2/COLB2)*100,2)       )
			IIF (lIsRedStor , nVar3		:= ROUND(( (COLC3*nFatRedSto) / (COLB3*nFatRedSto) )*100,2)     , nVar3		:= ROUND((COLC3/COLB3)*100,2)       )
			IIF (lIsRedStor , nVar4		:= ROUND(( (COLC4*nFatRedSto) / (COLB4*nFatRedSto) )*100,2)     , nVar4		:= ROUND((COLC4/COLB4)*100,2)       )
			IIF (lIsRedStor , nVar5		:= ROUND(( (COLC5*nFatRedSto) / (COLB5*nFatRedSto) )*100,2)     , nVar5		:= ROUND((COLC5/COLB5)*100,2)       )
			IIF (lIsRedStor , nVar6		:= ROUND(( (COLC6*nFatRedSto) / (COLB6*nFatRedSto) )*100,2)     , nVar6		:= ROUND((COLC6/COLB6)*100,2)       )
			IIF (lIsRedStor , nVar7		:= ROUND(( (COLC7*nFatRedSto) / (COLB7*nFatRedSto) )*100,2)     , nVar7		:= ROUND((COLC7/COLB7)*100,2)       )
			IIF (lIsRedStor , nVar8		:= ROUND(( (COLC8*nFatRedSto) / (COLB8*nFatRedSto) )*100,2)     , nVar8		:= ROUND((COLC8/COLB8)*100,2)       )
			IIF (lIsRedStor , nVar9		:= ROUND(( (COLC9*nFatRedSto) / (COLB9*nFatRedSto) )*100,2)     , nVar9		:= ROUND((COLC9/COLB9)*100,2)       )
			IIF (lIsRedStor , nVar10	:= ROUND(( (COLC10*nFatRedSto) / (COLB10*nFatRedSto) )*100,2)   , nVar10	:= ROUND((COLC10/COLB10)*100,2) )
			IIF (lIsRedStor , nVar11	:= ROUND(( (COLC11*nFatRedSto) / (COLB11*nFatRedSto) )*100,2)   , nVar11	:= ROUND((COLC11/COLB11)*100,2) )
			IIF (lIsRedStor , nVar12	:= ROUND(( (COLC12*nFatRedSto) / (COLB12*nFatRedSto) )*100,2)   , nVar12	:= ROUND((COLC12/COLB12)*100,2) )
			//Imprime as Variacoes em Percentual
			li++	
			@ li,aColunas[COL_SEPARA1]  PSAY "|"
			@ li,aColunas[COL_CONTA]    PSAY STR0029+" BxC (%)" // "VariaÁ„o"
	 		@ li,aColunas[COL_SEPARA3]  PSAY "|"
			@ li,aColunas[COL_COLUNA1]  PSAY Transform(nVar1,"9,999,999.99 %")
			@ li,aColunas[COL_SEPARA4]  PSAY "|"
			@ li,aColunas[COL_COLUNA2]  PSAY Transform(nVar2,"9,999,999.99 %")
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////
	// Se imprime a variaÁ„o em VALOR ($) entre as linhas A e B
	///////////////////////////////////////////////////////////////////////////////////////////////////
	If mv_par26 <> 2 
		...
		If (mv_par26 == 1 .or. mv_par26 == 4) .and. (lTemSldA .and. lTemSldC)	/// SE INDICOU O 1∫ e 3∫ TIPOS DE SALDO - (AxC)
			li++
			@ li,aColunas[COL_SEPARA1]  PSAY "|"
		...
		If (mv_par26 == 1 .or. mv_par26 == 5) .and. (lTemSldB .and. lTemSldC)	/// SE INDICOU O 2∫ e 3∫ TIPOS DE SALDO - (BxC)
			li++
			@ li,aColunas[COL_SEPARA1]  PSAY "|"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥Program   ≥CtCmpSldM ≥ Autor ≥ Marcos S. Lobo        ≥ Data ≥ 14.03.02 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥Gerar Arquivo TMP para Comparativos Saldo Mes a Mes (MAX 12)|±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥Retorno   ≥ .T. / .F.                                                  ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
...
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
/*/
Function CtCmpSldM( oMeter,oText,oDlg,lEnd,cArqtmp,;
...
If !Empty(cPlanoRef) .And. !Empty(cVersao)
	//Se o relatÛrio n„o possuir conta, o plano referencial e a vers„o ser„o desconsiderados.
	//Ser· considerado cÛd. config. livros em branco.
	Help("  ",1,"CTBNOPLREF",,STR0039,1,0) //"Plano referencial n„o disponÌvel nesse relatÛrio. O relatÛrio ser· processado desconsiderando a configuraÁ„o de livros."	 
	cPlanoRef		:= ""
	cVersao			:= ""	
	aSetOfBook[1]	:= ""
Endif
...
Case cAlias == "CT7"
	...
	If cFilUSU <> ".T." .and. !Empty(cString)		//// SE O FILTRO DE USU¡RIO NAO ESTIVER VAZIO
...
Case cAlias == "CTI"
	...
	If lUsaCusto ; AADD(aChave,"CUSTO") ; EndIf
		...
		Case cAlias == "CTU"
			...
			If lSldA
				For nMes := 1 to nLenMeses
					...
					cQuery += "   			WHERE CQ9_FILIAL = '"+xFilial("CQ9")+"' "
					cQuery += "   			AND CQ9_MOEDA = '"+cMoeda+"' "
					cQuery += "   			AND CQ9_TPSALD = '"+cSaldoA+"' "
					cQuery += "   			AND CQ9_IDENT = '"+cEntid+"' "
					cQuery += " 			AND CQ9_CODIGO	= ARQ."+cOrdQry+" "
					If lAcum
						cQuery += "    			AND CQ9_DATA <= '"+DTOS(aMeses[nMes,3])+"' "
					Else
						cQuery += "    			AND CQ9_DATA BETWEEN '"+DTOS(aMeses[nMes,2])+"' AND '"+DTOS(aMeses[nMes,3])+"' "
					EndIf
		...
		If ! lGravouTmp
			...
			If  Empty(aSetOfBook[5])				/// S” H¡ QUERY SEM O PLANO GERENCIAL
				//// SE FOR DEFINI«√O TOP
...
‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø±±
±±≥Program   ≥CtQryComp ≥ Autor ≥ Eduardo Nunes                ≥ Data ≥ 14/01/06 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Monta um trecho da query referente a tabela cAlias                |±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒ¥±±
±±≥Retorno   ≥ Caracter                                                          ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒ¥±±
±±≥Parametros≥                                                                   ≥±±
...
±±≥          ≥ lAcum      = Se È movimento acumulado(.T.) ou por                 ≥±±
±±≥          ≥              periodo(.F.) (mv_par16)                              ≥±±
...
±±≥FunáÖo	 ≥ GravaTMP	≥ Autor ≥ Eduardo Nunes Cirqueira ≥ Data ≥ 13/03/06 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Grava arquivo temporario com dados da query. 					 ≥±±
...
±±           ≥ cAlias : Alias da tabela que est· sendo apurada
...
±±∫Programa  ≥CTBR295   ∫Autor  ≥Microsiga           ∫ Data ≥  03/31/06   ∫±±
