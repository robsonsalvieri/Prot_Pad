...
@version P12
@Since 21/02/14
@obs Importação dos saldos iniciais por arquivo '.txt' 
/*/
//------------------------------------
Function WMSA490()
...
Local cFilAtual   := cFilAnt
	...
	If lRet
		...
		Do While (!FT_FEOF())
			...
			If lRet .And. !Empty(cLinha)
				...
				If Empty(cEmpresa)
					aAdd(aDiver,{nRecno,STR0049,cEmpresa}) // Empresa não informada.
					lRet := .F.
				EndIf
				
				//validação para logar no sistema com empresa e filial para atualizar o xFilial.
				If lRet .AND. (cEmpresa != cEmpAnt)
					aAdd(aDiver,{nRecno,STR0051,cEmpresa}) // Faça o Login na Empresa de destino dos dados.
					lRet := .F.
				EndIf
				...
				If lRet .OR. (cFilArq == " ") .OR. (cFilantc <> cFilarq)
					cFilAnt := cFilArq
				EndIf
	...
	If (cFilArq == " ") .OR. (cFilAtual <> cFilarq)
		cFilAnt := cFilAtual
	Endif
	FT_FUse()
Return lRet
