...
Local cLocalCQ    := SuperGetMV("MV_CQ",.F.,"")
	...
	If Self:GetStServ() == "3"
		...
		If Self:oServico:HasOperac({'8'})
			...
			EndIf
		EndIf
		//Verifica saldo do produto quando devolução crossdocking com reserva de pedido, desconsidera origem SC2, pois o B2_QATU ja foi atualizado pelo MATA250
		If lRet .And. Self:oServico:HasOperac({'2'}) .And. self:cOrigem != "SC2"
			lRet := Self:ChkQtdRes()
		EndIf
			...
			If Self:oServico:GetTipo() == "2"
				If Self:cOrigem == "SC9" .And. !Self:lCarteira
					...
					BeginSql Alias cAliasSC9
						...
						AND (SC9.C9_NFISCAL <> ' ' OR SC9.C9_REMITO <> ' ')
						AND SC9.%NotDel%
					EndSql
					If (cAliasSC9)->(!Eof())
	...
	If Self:LockDCF()
		If Self:GetStServ() == '3'
			...
			If lRet .And. WmsX212118("D0V")
				...
				If lRet .AND. ExistBlock('WMSOSR01')
					lRet := ExecBlock('WMSOSR01',.F.,.F.,{ Self:cIdDCF, Self:cDocumento, ::Self })
				EndIf
				...
				Do While lRet .And. (cAliasD12)->(!Eof())
					If Self:oMovimento:GoToD12((cAliasD12)->RECNOD12)
						If Self:oMovimento:GetStatus() == "1"
							//validação para que não entre na rotina para gerar transferencia e deixe o saldo do produto estornado para documento de transferencia MI na DOCA
							If cPaisLoc <> "BRA" .AND. FWIsIncallstack("MATA462TN")
		...
		EndIf
		If lRet
			//--Se a separação foi a maior, atualiza a DCF de separação com a quantidade de origem
			If QtdComp(Self:GetQuant()) > QtdComp(Self:GetQtdOri()) .And. Self:cOrigem = "SD4"
				Self:SetQuant(QtdComp(Self:GetQtdOri()))
			EndIf
...
Local nQtdSldEnd := 0
	...
	If lRet .And. Self:oMovimento:IsUpdEst()
	   	If Self:IsMovUnit() 
			cSerTran := Self:GetServdUn()
			If Empty(cSerTran) 
			   cSerTran := Self:oMovimento:oMovServic:FindTransf()
			Endif 
 		Else
		
			/*(GetSerTrDv) - Busca por B5_SERVTDV Servico de Tranf. Devoluc. 
			Se não encontrar, (GetSerTran) - busca por B5_SERVINT - Servico WMS padrao para Movimentacoes Internas. 
			Se não encontrar, (FindTransf) - busca o código de serviço na tabela DC5 filtrando por operação 8-Transferencia*/

			cSerTran := IIf(!Empty(Self:oMovimento:oMovPrdLot:GetSerTrDv()),Self:oMovimento:oMovPrdLot:GetSerTrDv(),;
						IIF(!Empty(Self:oMovimento:oMovPrdLot:GetSerTran()),Self:oMovimento:oMovPrdLot:GetSerTran(),Self:oMovimento:oMovServic:FindTransf()))		
		EndIf 
		If Empty(cSerTran)
			lRet := .F.
	...
	If lRet .And. QtdComp(Self:GetQuant()) <> QtdComp(nQtdReq)
		...
		If (cAliasQry)->(!Eof())
			If Self:oProdLote:HasRastro() .And. Empty(Self:oProdLote:GetLoteCtl())
				...
				(cAliasQry)->(dbCloseArea())
			EndIf
		ElseIf QtdComp(Self:GetQuant()) == QtdComp(Self:GetQtdOri())
			// Ajusta primeira SD4
...
Private lExecWMS    := .T.
	...
	cAliasSD3 := GetNextAlias()
	BeginSql Alias cAliasSD3
		SELECT SD3.R_E_C_N_O_
		FROM %Table:SD3% SD3
		WHERE SD3.D3_FILIAL = %xFilial:SD3%
		AND SD3.D3_DOC = %Exp:cDocumento%
		AND SD3.D3_NUMSEQ = %Exp:cNumSeq%
		AND SD3.D3_ESTORNO = ' '
		AND SD3.%NotDel%
	EndSql
	...
	AAdd(aRotAuto,{"WMS_R_E_C_N_O_", (cAliasSD3)->R_E_C_N_O_, Nil})
	...
	EndIf
	If lRet .And. Self:GetOrigem() == "DH1"
		SD3->(DbGoTo((cAliasSD3)->R_E_C_N_O_)) 
		If !Empty(SD3->D3_OP)
			SC2->(DbSetOrder(1))
			If SC2->(DbSeek(FWXFilial("SC2")+SD3->D3_OP))
				cTpOp := SC2->C2_TPOP
				lReserva := .F.
				GravaB2Emp("+", Self:GetQuant(),cTpOp,lReserva)
			EndIf
		EndIf
	EndIf
	(cAliasSD3)->(dbCloseArea())
...
//----------------------------------------------------------
/*/{Protheus.doc} RevPedAut
Realiza o estorno da ordem de serviço com base nos pedidos incompletos
@param cListIdDcf Lista de DCF_ID. Enviar como "% ('000001','000002') %"
/*/
...
METHOD GetServdUn() CLASS WMSDTCOrdemServicoReverse
Local cServtdv := ""
Local cAliasQry := ""
Local nCount := 0
Local lRet := .T. 
	...
	cAliasQry := GetNextAlias()
	BeginSql Alias cAliasQry
		SELECT DISTINCT SB5.B5_SERVTDV 
  		FROM %Table:D0S% D0S
 		INNER JOIN %Table:SB5% SB5
 		ON SB5.B5_FILIAL = %xFilial:SB5%
 		AND SB5.B5_COD = D0S.D0S_CODPRO
 		AND SB5.%NotDel% 
 		WHERE D0S.D0S_FILIAL =  %xFilial:D0S%
 		AND D0S.D0S_IDUNIT = %Exp:Self:cIdUnitiz%
		AND D0S.%NotDel% 
