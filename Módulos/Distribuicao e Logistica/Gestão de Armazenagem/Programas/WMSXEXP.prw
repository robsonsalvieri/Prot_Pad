...
Default dDtValid   := nil 
	...
	Else
		...
		EndSql
	EndIf
	WmsConout('WmsDivSC9 - ' + GetLastQuery()[2])
	Do While lRet .And. (cAliasSC9)->(!Eof()) .And. QtdComp(nQuant) > 0

		SC9->(DbGoTo((cAliasSC9)->RECNOSC9))
		WmsConout('WmsDivSC9 RecnoSC9 ' + cValToChar((cAliasSC9)->RECNOSC9))
		...
		If QtdComp(nQtdLib) <= QtdComp(nQuant)
			...
			If SC9->C9_LOTECTL != cLoteCtl
				WmsConout('WmsDivSC9 - Alterando lote de / para ' + SC9->C9_LOTECTL + ' / ' + cLoteCtl)
				SC9->C9_LOTECTL := cLoteCtl
				SC9->C9_NUMLOTE := cNumLote
				If !Empty(dDtValid)
					SC9->C9_DTVALID := dDtValid
		...
		Else
			...
			aDadosAlt := {}
			aAdd(aDadosAlt,{"C9_SEQUEN", cSeqSC9})
			aAdd(aDadosAlt,{"C9_QTDLIB", nQtdLib})
			aAdd(aDadosAlt,{"C9_QTDLIB2", nQtdLib2UM})
			aAdd(aDadosAlt,{"C9_IDDCF", cIdDCF})
			aAdd(aDadosAlt,{"C9_SERVIC", cServico})
			If SC9->C9_LOTECTL != cLoteCtl
				aAdd(aDadosAlt,{"C9_LOTECTL", cLoteCtl})
				aAdd(aDadosAlt,{"C9_NUMLOTE", cNumLote})
				If !Empty(dDtValid)
					aAdd(aDadosAlt,{"C9_DTVALID", dDtValid})
				EndIf
			EndIf				
			If lIntWms
				aAdd(aDadosAlt,{"C9_BLWMS", Iif(lWmsLibSC9,"05","01")})
			Else
				aAdd(aDadosAlt,{"C9_BLWMS", ""})
			EndIf
			aAdd(aDadosAlt,{"C9_ROMEMB", cRomaneio})
			nNewRecno := WmsCopy("SC9", aDadosAlt)
			WmsConout('WmsDivSC9 - Novo item do pedido Seq / QtdLib / Lote / DCF / Recno ' + cSeqSC9 + ' / ' + cValToChar(nQtdLib) + ' / ' + cLoteCtl + ' / ' + cIdDCF + ' / ' + cValToChar(nNewRecno))

			RecLock("SC9", .F.)
			// Atualiza informações
			WmsConout('WmsDivSC9 - RecnoSC9 ' + cValToChar(SC9->(Recno())) + ' Substraindo ' + cValToChar(nQtdLib) + ' de ' + cValToChar(SC9->C9_QTDLIB))
			...
			EndIf
			If lRet
				//Posiciona no novo recno gerado
		...
		Do While lRet .And. (cAliasSC9)->(!Eof()) .And. QtdComp(nQuant) > 0
			...
			If lRet
				...
				Else
					...
					If QtdComp(nDifLib) > 0
						cSeqSC9 := MaxSeqSC9(cPedido,cItem,cProduto)
						aAdd(aDadosAlt,{"C9_SEQUEN", Soma1(cSeqSC9,Len(SC9->C9_SEQUEN))})
						aAdd(aDadosAlt,{"C9_QTDLIB", nDifLib})
						aAdd(aDadosAlt,{"C9_QTDLIB2", nDifLib2UM})
						nNovoRecno := WmsCopy("SC9",aDadosAlt)
					EndIf
...
Default lDesSeqSC9 := .F.  //Indica se deve desconsiderar da query a sequência de SC9 passada por parâmetro
	...
	BeginSql Alias cAliasSC9
		...
		ORDER BY C9_SEQUEN
	EndSql
	WmsConout('WmsAgluSC9 - ' + GetLastQuery()[2])
	If (cAliasSC9)->(!Eof())
		...
		WmsConout('WmsAgluSC9 - RecnoSC9 / Seq / QtdLib / nQuant ' + cValToChar((cAliasSC9)->RECNOSC9) + ' / ' + cSeqSC9 + ' / ' + cValToChar(SC9->C9_QTDLIB) + ' / ' + cValToChar(nQuant))
...
-----------------------------------------------------------------------------*/
Function WmsAtuSDC(cOrigem,cOp,cTrt,cPedido,cItem,cSeqSC9,cProduto,cLoteCtl,cNumLote,cNumSerie,nQuant,nQuant2UM,cLocal,cEndereco,cIdDCF,nIndex,lEstorno,lOpEncer,lQuebraD4)
Local lRet       := .T.
Local lCriaSDC   := .T.
Local lWmsNew    := SuperGetMV("MV_WMSNEW",.F.,.F.)
Local aAreaAnt   := GetArea()
...
Default lQuebraD4 := .F.
	...
	cNumSerie := If( Empty(cNumSerie), Space(TamSx3("DC_NUMSERI")[1]), cNumSerie )
	cEndereco := If( Empty(cEndereco), Space(TamSx3("DC_LOCALIZ")[1]), cEndereco )
	...
	Else //-- Deve criar um novo registro de SDC
		If !lEstorno
			...
			ElseIf !lOpEncer .OR. !Empty(cEndereco) 
...
-----------------------------------------------------------------------------*/
Function WmsAtuSD4(cLocal,cProduto,cLoteCtl,cNumLote,cNumSerie,cEndereco,nQuant,cIdDCF,lEstorno,lMantLote,nRecSD4)
Local lRet        := .T.
Local lWmsNew     := SuperGetMv("MV_WMSNEW",.F.,.F.)
Local lHasSDC     := .F.
Local lOk         := .F.
...
Default cNumSerie := Space(Len(SDC->DC_NUMSERI))
Default lMantLote := .F.
Default nRecSD4 := 0
	...
	cQuery := "  SD4.R_E_C_N_O_ RECNOSD4 "
	cQuery += " FROM "+RetSqlName("SD4")+" SD4"
	cQuery += " WHERE SD4.D4_FILIAL= '"+xFilial("SD4")+"'"
	cQuery += " AND SD4.D4_LOCAL = '"+cLocal+"'"
	cQuery += " AND SD4.D4_COD = '"+cProduto+"'"
	cQuery += " AND (SD4.D4_LOTECTL = '  ' 	OR SD4.D4_LOTECTL = '"+cLoteCtl+"' )"	
	cQuery += " AND (SD4.D4_NUMLOTE = '  ' 	OR SD4.D4_NUMLOTE = '"+cNumLote+"' )"	
	cQuery += " AND SD4.D4_IDDCF  = '"+cIdDCF+"'"	
	cQuery += " AND SD4.D4_QTDEORI > 0 "
	
	If nRecSd4 > 0 
		cQuery += "	AND SD4.R_E_C_N_O_ = " + cValtoChar(nRecSd4)
	EndIf  
		...
		If lOk
			...
			If Rastro(cProduto)
				If !lEstorno
					...
					nDifSD4 := 0
					//--Pega diferença quando SD4 é maior que SDC, indicando quebra do empenho
					If SD4->D4_QUANT > nQtdSDC
						nDifSD4 := SD4->D4_QUANT - nQtdSDC
					EndIf

					//--Atualiza SD4 com a quantidade empenhada na SDC
					RecLock("SD4", .F.)
					...
					If nDifSD4 > 0 .Or. QtdComp(nQtdEmp+nQuant) < QtdComp(nQtdReq)
						dbSelectArea("SD4")
						...
						cLoteCtlAnt := CriaVar("D4_LOTECTL")
						cNumLoteAnt := CriaVar("D4_NUMLOTE")
						cAliasQry := GetNextAlias()
						BeginSql Alias cAliasQry
						...
						SD4->D4_DTVALID := dDtValid
						SD4->D4_DATA    := dDataOp
						SD4->D4_QUANT   := IIF( nDifSD4 > 0, nDifSD4, QtdComp( nQtdReq - (nQtdEmp+nQuant) ) )
						SD4->D4_QTSEGUM := IIF( nDifSD4 > 0, ConvUM(cProduto, nDifSD4, 0, 2), ConvUM(cProduto, SD4->D4_QUANT, 0, 2) )
						SD4->D4_QTDEORI := IIF( nDifSD4 > 0, nDifSD4, SD4->D4_QUANT )
				...
				Else
					...
					cAliasQry := GetNextAlias()
					BeginSql Alias cAliasQry
						SELECT COUNT(SD4.D4_QUANT) D4_QTD
						FROM %Table:SD4% SD4
						WHERE SD4.D4_FILIAL  = %xFilial:SD4%
						AND SD4.D4_COD = %Exp:cProduto%
						AND SD4.D4_OP = %Exp:cOp%
					...
					lQuebra	:= .F.
					lFirst	:= .F.
					//--Quando há mais de um registro SD4, indica que houve quebra do empenho 
					If !lWmsNew .And. nCountSD4 > 1
						lQuebra := .T.
						cLoteCtlAnt := CriaVar("D4_LOTECTL")
						cNumLoteAnt := CriaVar("D4_NUMLOTE")
						SD4->(dbSeek(xFilial('SD4')+cProduto+cOp+cTrt+cLoteCtl+cNumLote))
						While SD4->(!Eof()) .And.;
							SD4->( D4_COD+D4_OP+D4_TRT+D4_LOTECTL+D4_NUMLOTE ) == ( cProduto+cOp+cTrt+cLoteCtl+cNumLote )
							
							If lFirst
								cOpOrig 	:= SD4->D4_OPORIG
								nNewQuant	:= SD4->D4_QUANT
								nNewQtSeg	:= ConvUM(cProduto, SD4->D4_QUANT, 0, 2)
								nNewQtdOri	:= SD4->D4_QTDEORI

								RecLock("SD4", .F.)
								SD4->( dbDelete())
								SD4->(MsUnlock())

								nRecSD4 := SD4->( GetArea() ) //--Guarda posição

								//--Verifica se existe registro não empenhado para alteração, se não houver, cria um novo registro
								cAliasQry := GetNextAlias()
								BeginSql Alias cAliasQry
									SELECT SD4.R_E_C_N_O_ RECNOSD4
									FROM %Table:SD4% SD4
								...
								SD4->D4_DTVALID := dDtValid
								SD4->D4_DATA    := dDataOp
								SD4->D4_QUANT   += nNewQuant
								SD4->D4_QTSEGUM += ConvUM(cProduto, nNewQtSeg, 0, 2)
								SD4->D4_QTDEORI += nNewQtdOri
								SD4->D4_PRODUTO := cPrdPai
								SD4->D4_ROTEIRO := cRoteiro
								SD4->D4_IDDCF   := cIdDCF
								...
								RestArea( nRecSD4 ) //--Restaura posição getarea
					...
					//Cria novo registro enquanto houver saldo
					If !lQuebra .And. QtdComp(nQtdEmp-nQuant) < QtdComp(nQtdReq)
						dbSelectArea("SD4")
						cAliasQry := GetNextAlias()
						BeginSql Alias cAliasQry
							SELECT SD4.R_E_C_N_O_ RECNOSD4
...
Function WmsUndoPed(nRecnoSC9,lEstPed,cNFiscal,nQuant,nRecnoSd2,cEspecD2)
Local lRet      := .T.
Local lWmsNew   := SuperGetMV("MV_WMSNEW",.F.,.F.)
...
Default cNFiscal  := ""
	...
	If !lWmsNew
		//Quando gerou bloqueio de estoque e não gerou DCF , e , está sendo efetuado o estorno da liberação na sc9. 
		If Empty(SC9->C9_IDDCF) .AND. !Empty(SC9->C9_BLEST) 
            Return .T. //Não existe ordem de serviço
		ENDIF
		lRet := WmsDelDCF("1","SC9")
	Else
		//Quando o pais nao e BRa e esta emitindo um rmeito de transferencia de saida
		If cPaisLoc <> "BRA" .AND. Empty(cNFiscal) .And. AllTrim(cEspecD2) $ "RTS|RCD|NCP" .And. !Empty(nRecnoSD2) .AND. (FWIsInCallStack("MATA462TN") .OR. FWIsInCallStack("MATA102DN").OR. FWIsInCallStack("MATA466N"))
			//Validar quando e nota de credito se for vincuada a remito de devolução nao vai baixar estoque 
			If ((AllTrim(cEspecie) == "NCP") .And. FwIsInCallStack('MATA466N')) .AND. FindFunction("WMSVRemDMI") .AND. WMSVRemDMI(SD2->D2_CLIENTE,SD2->D2_LOJA,SD2->D2_SERIREM,SD2->D2_REMITO,SD2->D2_ITEMREM,SD2->D2_COD)
				lRet:=.T.
			ELse 
				lRet := oEstEnder:UndoFtMi(nRecnoSd2,!lEstPed, IIF(AllTrim(cEspecD2) $ "RCD|NCP",.F.,.T.))
			EndIf
		Else
			If !Empty(cNFiscal) 
				lRet := oEstEnder:UndoFatur(nRecnoSC9,!lEstPed,cNFiscal, SD2->D2_NUMSEQ, SD2->D2_SERIE)
				If lEstPed .And. lRet
...
Default nQuant  := 0
	//Atualiza o campo D12_AGLUTI caso movimento seja aglutinado mas o indicador foi gravado incorretamente (com valor 2)
	WmVD12Aglu(oMovimento:oOrdServ:GetIdDCF(), oMovimento:GetIdMovto(), oMovimento:GetAgluti())
		...
		If lRet
			...
			If lRet 
    			...
    			BeginSql Alias cAliasQry
        			...
        			AND SB2.B2_COD = %Exp:oMovimento:oMovPrdLot:GetProduto()%
        			AND SB2.B2_LOCAL = %Exp:oMovimento:oMovPrdLot:GetArmazem()%
        			AND SB2.%NotDel%
    			EndSql
    			If (cAliasQry)->(!Eof())
        			...
        			nSaldoSB2:=SaldoSB2()
        			IF nQuant > nSaldoSB2
						oMovimento:SetErro(WmsFmtMsg(STR0020,{{"[VAR01]",Transf(nSaldoSB2,PesqPictQt('B2_QATU',14))}})) //"Produto não possui saldo de estoque suficiente. Saldo Disponível:  [VAR01]"   
						lRet := .F.
					ENDIF
				ENDIF
				(cAliasQry)->(dbCloseArea())
			EndIf
			If lRet .AND. Rastro(oMovimento:oMovPrdLot:GetProduto())
				//SB8 precisa estar posicionada para calcular o saldo
				cAliasQry := GetNextAlias()
    			BeginSql Alias cAliasQry
        			SELECT SB8.R_E_C_N_O_ RECNOSB8
        			FROM %Table:SB8% SB8
        			WHERE SB8.B8_FILIAL = %xFilial:SB8%
        			AND SB8.B8_PRODUTO = %Exp:oMovimento:oMovPrdLot:GetProduto()%
        			AND SB8.B8_LOCAL   = %Exp:oMovimento:oMovPrdLot:GetArmazem()%
					AND SB8.B8_LOTECTL = %Exp:oMovimento:oMovPrdLot:GetLoteCtl()%
					AND SB8.B8_NUMLOTE = %Exp:oMovimento:oMovPrdLot:GetNumLote()%
        			AND SB8.%NotDel%
    			EndSql
				If (cAliasQry)->(!Eof())
        			SB8->(dbGoTo((cAliasQry)->RECNOSB8))
					nSaldoSb8 := SB8SALDO()
					IF nQuant > nSaldoSb8
						oMovimento:SetErro(WmsFmtMsg(STR0021,{{"[VAR01]",Transf(nSaldoSB8,PesqPictQt('B8_SALDO',14))}})) //"Produto não possui saldo de estoque suficiente para o lote. Saldo Disponível: [VAR01]"       
						lRet := .F.
					ENDIF
				EndIf 
				(cAliasQry)->(dbCloseArea())
...
Local lQtdMaior   := QtdComp(nQtdOrig) < QtdComp(nQtdMvto)
	...
	If oMntVolItem:LoadData()
		...
		If lRet .And. oMntVolItem:oMntVol:GetLibPed() == "6" .And. oMntVolItem:oMntVol:GetStatus() == "3" .AND. !oMovimento:ChkMntVol("1")
...
Return lRet
//---------------------------------------------------------
/*/{Protheus.doc} WmsCopy
Função para copiar o registro posicionado do alias informado.
@author amanda.vieira
@since 29/04/2020
@version 1.0
@param cAlias, Caracter, alias da tabela posicionada
@param aDadosAlt, array, array de campos que devem ser alterados no momento da cópia para não ocorrer chave duplicada
Preenchimento esperado do aDadosAlt {{"Campo","Valor"}}. Exemplo: {{"C9_SEQUEN","02"},{"C9_QTDLIB",20.50},{"C9_QTDLIB2",40}}
Importante: essa função não posiciona o registro no novo recno criado, 
apenas retorna o recno novo para caso deseje-se posicionar
/*/
//---------------------------------------------------------
Function WmsCopy(cAlias, aDadosAlt)
Local aCopy      := {}
Local aAreaAnt   := (cAlias)->(GetArea())
Local nNovoRecno := 0
Local oStruct    := FWFormStruct(1,cAlias,,.F.,.F.)
Local aCampos    := oStruct:GetFields()
	//Cópia dos dados do registro atual
	AEval( aCampos, { | aCampo |  aAdd(aCopy, {aCampo[3], (cAlias)->&(aCampo[3])}) } )

	RecLock(cAlias, .T.)
	//Repassa os dados para o novo registro que será incluído
	AEval( aCopy, { | aCopy | (cAlias)->&(aCopy[1]) := aCopy[2]})
	//Dados que serão alterados no momento da cópia
	AEval( aDadosAlt, { | aDadosAlt | (cAlias)->&(aDadosAlt[1]) := aDadosAlt[2]})
	(cAlias)->(MsUnlock())

	nNovoRecno := (cAlias)->(Recno())

	RestArea(aAreaAnt)
Return nNovoRecno

//---------------------------------------------------------
/*/{Protheus.doc} WmsBlqPd
Função para validar se o pedido encontra-se com bloqueio WMS (C9_BLWMS = '01').
Retorna .T. se o pedido tem bloqueio, senão .F.
...
Local lRet       := .F.
    ...
    If !Empty(SC9->C9_BLWMS) .And. SC9->C9_BLWMS == '01'  //01 - Bloqueado WMS   
        lRet := .T.     
    EndIf
...
//---------------------------------------------------------
/*/{Protheus.doc} WmsEnvMetr
Função para envio da metrica WMS
@author roselaine.adriano
@since 29/03/2023
@version 1.0
/*/
//---------------------------------------------------------
Function WmsEnvMetr(nMovWMS,cRotina,nOpcao)
Local lTMSMetri	:= FindFunction("TMSAddMet")
Local lWmsNew	:= SuperGetMv("MV_WMSNEW",.F.,.F.)

	If cRotina == "MATA461_Fat_Pedido" //Inserido esta validação pois no momento será enviado as metricas somente no momento da finalização dos movimentos de servico.
	   Return 
	EndIf 

	If lTMSMetri .AND. lWmsNew
		If nMovWMS > 0 .AND. nOpcao == 1
			TMSAddMet( cRotina /*cRotina*/, "wms-protheus_produtos-recebidos_total" /*IdMetrica*/, nMovWMS /*nValueMet*/, /*nTimeMet*/, /*cTypepMet*/, dDataBase + 1 /*dDtSend*/, 1/*nTipo*/ )
		ElseIf nMovWMS > 0 .AND. nOpcao == 2
			TMSAddMet( cRotina /*cRotina*/, "wms-protheus_produtos-expedidos_total" /*IdMetrica*/, nMovWMS /*nValueMet*/, /*nTimeMet*/, /*cTypepMet*/, dDataBase + 1 /*dDtSend*/, 1/*nTipo*/ )
		EndIf 
	EndIf
Return 


/*/{Protheus.doc} WMSLibVols
	(Procura outra DCF da mesma montagem de volumes.
	Serão liberados os pedidos se o serviço é montagem de volume durante a separação
	e as quantidades necessárias baterem com as separadas e embaladas.
	Chamada por UpdateDCS/RevMntVol durante o estorno de um produto na montagem de volumes)
	@type  Function
	@since 21/06/2024
	@param oMontVol, objeto, objeto da montagem sendo finalizado
	@see (DLOGWMSMSP-16392)
	/*/
Function WMSLibVols(oMontVol)
	Local aArea       := GetArea()
	Local cServicoAux := ""
	Local cAliasD0I   := GetNextAlias()
	Local oServico    := WMSDTCServicoTarefa():New()
	Local oOrdSerAux  := WMSDTCOrdemServico():New()
	Local oMovServ    := WMSBCCMovimentoServico():New()
	Local lRet 		  := MethisMemberOf(oMovServ,"LibOutServ",.T.)

	If lRet
		BeginSql Alias cAliasD0I
			SELECT D0I.D0I_IDDCF
				FROM %Table:D0I% D0I
				...
				WHERE D0I.D0I_FILIAL = %xFilial:D0I%
					AND D0I.D0I_CODMNT = %Exp:oMontVol:GetCodMnt()% 
					AND DCF.DCF_CARGA = %Exp:oMontVol:GetCarga()% 
					AND DCF.DCF_DOCTO = %Exp:oMontVol:GetPedido()% 
					AND D0I.%NotDel%
					AND EXISTS (SELECT DCT_CODMNT FROM %Table:DCT% DCT
									WHERE DCT.%NotDel%
										AND DCT_FILIAL = %xFilial:DCT%
										AND DCT_CODMNT = D0I.D0I_CODMNT
										AND DCT_CARGA = DCF.DCF_CARGA
										AND DCT_PEDIDO = DCF.DCF_DOCTO
										AND DCT_CODPRO = DCF.DCF_CODPRO)
		EndSql
		If (cAliasD0I)->(!Eof())
			oOrdSerAux:SetIdDCF((cAliasD0I)->D0I_IDDCF)
			oOrdSerAux:LoadData(9)
			cServicoAux := oOrdSerAux:GetServico()
			oServico:SetServico(cServicoAux)
			If oServico:LoadData()
				If oServico:GetTipo() = "2" .And. oServico:GetLibPed() = "6" .And. oServico:GetMntVol() = "1"
					oMontVol:SetIdDCF((cAliasD0I)->D0I_IDDCF)
					oMovServ:LibOutServ(oMontVol)
				EndIf
			EndIf
		EndIf
