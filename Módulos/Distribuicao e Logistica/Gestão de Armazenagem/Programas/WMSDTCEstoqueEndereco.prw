...
Static _lWMSATSBE := ExistBlock("WMSATSBE")
Static _lWMSSLPRO := ExistBlock("WMSSLPRO")
...
CLASS WMSDTCEstoqueEndereco FROM LongNameClass
	...
	METHOD FindLote()
	METHOD GetErro()
	METHOD UpdSaldo(cTipo,lEstoque,lEntPrev,lSaiPrev,lEmpenho,lBloqueio,lEmpPrev,lMovEstEnd,dDataMov)
	...
	METHOD UndoFatur(nRecno, lEmpenho, cNFiscal, cNumSeq, cSerieNf)
	METHOD MakeFatur(nRecnoSC9)
	METHOD MakePerda(nRecnoSBC,cProg)
	...
	METHOD SldPrdData(cProduto,cArmazem,dDataRef,cLoteCtl,cNumLote,cEndereco,cNumSerie,cIdUnitiz)
	...
	METHOD ClearData()
	METHOD Destroy()
	METHOD UpdEnder(lConsBlq,lLoadEnder)
	...
	METHOD GeraEmpReq(cOrigem,cOp,cTrt,cIdDCF,lEstorno,lCriaSDC,lEmpD14,lOpEncer)
	...
	METHOD WmsEmpPrde()
	METHOD MakeFtMI(nRecnoSD2, cIdDCF,cEnder,lAtuEmp)
	METHOD UndoFtMi(nRecno, lEmpenho, lAtuDCF)
...
METHOD New() CLASS WMSDTCEstoqueEndereco
	...
	Self:cCodNorma   := PadR("" , GetWmsSX3("DC3_CODNOR", 1))
	Self:cCodVolume  := PadR("" , GetWmsSX3("D14_CODVOL", 1))
	Self:cIdVolume   := PadR("" , GetWmsSX3("D14_IDVOLU", 1))
	Self:cIdUnitiz   := PadR("" , GetWmsSX3("D14_IDUNIT", 1))
	Self:cTipUni     := PadR("" , Iif(Self:lHasCodUni,GetWmsSX3("D14_CODUNI",1),6))
	Self:aD14_QTDEST := GetWmsSX3("D14_QTDEST")
	Self:aB2_RESERVA := GetWmsSX3("B2_RESERVA")
	Self:aB8_EMPENHO := GetWmsSX3("B8_EMPENHO")
	Self:aD11_RATEIO := GetWmsSX3("D11_RATEIO")
	Self:aDCF_ID     := GetWmsSX3("DCF_ID")
	Self:ClearData()
Return
...
METHOD GetQtdFat() CLASS WMSDTCEstoqueEndereco
Return Self:nQtdfat  //Retorno a quantidade de produtos expedidos para metricas WMS. 
...
/*/
//----------------------------------------
METHOD UpdSaldo(cTipo,lEstoque,lEntPrev,lSaiPrev,lEmpenho,lBloqueio,lEmpPrev,lMovEstEnd,dDataMov) CLASS WMSDTCEstoqueEndereco
Local lRet		 := .T.
Local lAchou	 := .F.
Local lAnalisEnd := .F.
Local aAreaD14   := D14->(GetArea())
...
Default dDataMov  := StoD("")
	...
	If lRet
		...
		Else
			...
			If _lWMSATD14
				ExecBlock("WMSATD14",.F.,.F.,{cTipo,lEstoque,lEntPrev,lSaiPrev,lEmpenho,lBloqueio,lEmpPrev,Self:nQuant,nQtd2UM,Self:oEndereco,Self:oProdLote})
			EndIf
			If lAnalisEnd
				Self:UpdEnder(,.F.)
			EndIf
			If lMovEstEnd
				...
				Self:GeraMovEst(cTipo,dDataMov)
			EndIf
		EndIf
	EndIf
	RestArea(aAreaD14)
...
Default lBloqueado   := .T.
	...
	//Esta função será valida para quandofor acionada a consulta de endereço de produção sem o lote , pois quando foi informando o lote na OP ja e empenhado na SB8.
	If nSaldo > 0 .And. Self:lProducao .AND. !Empty(Self:oProdLote:GetProduto()) .AND. Rastro(Self:oProdLote:GetProduto()) .AND. Empty(Self:oProdLote:GetLoteCtl()) .AND. Empty(Self:oProdLote:GetNumLote())
		nSaldoPR := Self:WmsEmpPrde()
		If nSaldoPR < nSaldo
			nSaldo := nSaldoPR 
		EndIf 
	EndIf
	...
	If _lWMSSLPRO
		nSaldoPE := ExecBlock("WMSSLPRO",.F.,.F.,{nSaldo,Self:oEndereco:GetArmazem(), Self:oEndereco:GetEnder(), Self:oProdLote:GetProduto(), Self:oProdLote:GetPrdOri(), Self:oProdLote:GetLoteCtl(), Self:oProdLote:GetNumLote()})
...
@param lD14SemSB8 .T.-> Indica que o produto possui registros na D14 mas não possui SB8 (ocorre ao alterar o rastro de N para L com B1_CTRMWS = 'S')
                  .F.-> Indica que a produto possui SB8 e o controle será feito por lá 
@version 4.0
/*/
//----------------------------------------
METHOD GetSldOrig(lMontado,lConsReser,lPai,lD14SemSB8) CLASS WMSDTCEstoqueEndereco
Local lRastro    := .F.
Local lSubLot    := .F.
Local aAreaAnt := GetArea()
Local cAliasQry  := Nil
...
Default lD14SemSB8 := .F.
	...
	If !lRastro .Or. (lRastro .And. lD14SemSB8)
	...
	Else
		If lMontado
			If lConsReser
				...
				EndSql
				TCSetField(cAliasQry,'B2_RESERVA','N',Self:aB2_RESERVA[1],Self:aB2_RESERVA[2])
				TCSetField(cAliasQry,'B8_SALDO'  ,'N',GetWmsSX3("B8_SALDO", 1), GetWmsSX3("B8_SALDO", 2))
				TCSetField(cAliasQry,'B8_EMPENHO','N',Self:aB8_EMPENHO[1],Self:aB8_EMPENHO[2])
				If (cAliasQry)->(!Eof())
			...
			EndSql
			TCSetField(cAliasQry,'B8_SALDO'  ,'N',GetWmsSX3("B8_SALDO", 1), GetWmsSX3("B8_SALDO", 2))
			TCSetField(cAliasQry,'B8_EMPENHO','N',GetWmsSX3("B8_EMPENHO", 1), GetWmsSX3("B8_EMPENHO", 2))
			TCSetField(cAliasQry,'B8_DTVALID','D',GetWmsSX3("B8_DTVALID", 1), GetWmsSX3("B8_DTVALID", 2))
					...
					If lConsReser
						BeginSql Alias cAliasQry2
							SELECT D14.D14_PRODUT,
									...
									+ ( %Exp:nLotReserv% - SUM(D14.D14_QTDPEM + D14.D14_QTDEMP + D14.D14_QTDBLQ)))  // Reserva estoque SB2/SB8 - reserva WMS D14
									) SALDO
							FROM %Table:D14% D14
							WHERE D14.D14_FILIAL = %xFilial:D14%
							AND D14.D14_LOCAL = %Exp:Self:oProdLote:GetArmazem()%
...
/*/
//----------------------------------------
METHOD UndoFatur(nRecno, lEmpenho, cNFiscal, cNumSeq, cSerieNf) CLASS WMSDTCEstoqueEndereco
Local lRet       := .T.
Local aProduto   := {}
	...
	If lRet
		...
		If Len(aProduto) > 0
			For nProduto := 1 To Len(aProduto)
				...
				IF Empty(SC9->C9_NFISCAL) .And. !(cPaisLoc=='BRA') //ROse	
					Self:SetBlkDoc({|oMovEstEnd|;
						oMovEstEnd:SetOrigem("SD2"),;
						oMovEstEnd:SetDocto(cNFiscal),;
						oMovEstEnd:SetNumSeq(cNumSeq),;
						oMovEstEnd:SetSerie(cSerieNf),;
						oMovEstEnd:SetClifor(SC9->C9_CLIENTE),;
						oMovEstEnd:SetLoja(SC9->C9_LOJA),;
	...
	If lRet
		...
		If Len(aProduto) > 0
			For nProduto := 1 To Len(aProduto)
				...
				Self:SetQuant(QtdComp(SC9->C9_QTDLIB * aProduto[nProduto][2]) )
				// Seta o bloco de código para informações do documento para o Kardex
				IF Empty(SC9->C9_NFISCAL) .And. !(cPaisLoc=='BRA')
					//Quando exitir remito associado pode ainda não haver o documento fiscal por isso é associado o documento da Sd2	
					...
					BeginSql Alias cAliasSD2
						...
						WHERE SD2.D2_FILIAL = %xFilial:SD2%
 						AND SD2.D2_CLIENTE =  %Exp:SC9->C9_CLIENTE%
						AND SD2.D2_LOJA =  %Exp:SC9->C9_LOJA%
						AND SD2.D2_PEDIDO =   %Exp:SC9->C9_PEDIDO%
						AND SD2.D2_ITEMPV  =   %Exp:SC9->C9_ITEM%
						AND SD2.D2_SEQUEN =   %Exp:SC9->C9_SEQUEN%
						AND SD2.%NotDel%
					EndSql
					If (cAliasSD2)->(!Eof())
						cDocSD2 :=(cAliasSD2)->D2_DOC
						cSerieSd2 := (cAliasSD2)->D2_SERIE	
						cClifor := (cAliasSD2)->D2_CLIENTE
						cLoja   := 	(cAliasSD2)->D2_LOJA
               			Self:SetBlkDoc({|oMovEstEnd|;
							oMovEstEnd:SetOrigem("SD2"),;
							oMovEstEnd:SetDocto(cDocSD2),; //ROSE ajuste numero documento remito  
							oMovEstEnd:SetSerie(cSerieSd2),;
							oMovEstEnd:SetClifor(cClifor),;
							oMovEstEnd:SetLoja(cLoja),;
				...
				If lRet //alteracao para envios metrica de produtos Expedidos.
					If SC9->C9_QTDLIB2 > 0 
						Self:nQtdfat += (QtdComp(SC9->C9_QTDLIB2 * aProduto[nProduto][2]))
					Else
						Self:nQtdfat += (QtdComp(SC9->C9_QTDLIB * aProduto[nProduto][2]))
					EndIf 
				EndIf
			Next
		Else
...
Default nQtdSpr    := 0
	...
	If Self:lProducao == Nil
		Self:lProducao := (FwIsInCallStack("MATA650") .And. FwIsInCallStack("F4Localiz")) .OR. FwIsInCallStack("MATA685")
...
Default cTipUni   := ""
	...
	//--Para os casos em que o lote não exista preenche com database semelhante a regra de estoque
	//--Se o lote existir, o database sera sobrescrito sempre que o load do oProdLote for realizado
	Self:oProdLote:SetDtValid(dDataBase)
	Self:oProdLote:SetDtFabr(dDataBase)
	...
	ElseIf QtdComp(nSaldoAtu) < QtdComp(nQuant)
		If QtdComp(nSaldoAtu) == 0 .AND. MethisMemberOf(Self:oProdLote,"SetVldArm",.T.)
			Self:oProdLote:SetVldArm(.F.)
			Self:oProdLote:SetArmazem("")
		EndIF
		...
		If QtdComp(nSaldoAtu) == 0 
			If !Empty(Self:oProdLote:nRecno)
				SB8->(dbGoTo(Self:oProdLote:nRecno))
			ElseIf lPartes
				//Quando é produto componente e esta processando o invetário de uma parte somente 
				//cria saldo como produto, porém se possuir saldo para o produto pai da estrutura em outro armazém deve pegar 
				//a mesma data de validade e fabricação qundo ja existe o lote
				//Pra isto abaixo busca o saldo na Sb8 para o produto PAI sem filtrar o local de estoque
				...
				BeginSql Alias cAliasSB8
					...
					AND SB8.B8_LOTECTL = %Exp:Self:oProdLote:GetLoteCtl()%
					AND SB8.B8_NUMLOTE = %Exp:Self:oProdLote:GetNumLote()%
					AND SB8.%NotDel%
				EndSql
				If (cAliasSB8)->(!Eof())
					SB8->(dbGoTo((cAliasSB8)->RECNOSB8))
					//Caso encontre saldo para o produto PAI da parte na SB8 verificar se a D14 esta com data de validade em branco 
				   //se estiver atualiza a data para o componente na D14
				   ...
				   BeginSql Alias cAliasComp
					...
					EndSql
					If (cAliasComp)->(!Eof())
						D14->(dbGoTo((cAliasComp)->RECNOD14))
						RecLock("D14",.F.)
							D14->D14_DTVALD := STOD((cAliasSb8)->B8_DTVALID)
							D14->D14_DTFABR := STOD((cAliasSb8)->B8_DFABRIC)
						D14->(MsUnlock())
					ENDIF
...
/*/
//-----------------------------------
METHOD UpdEnder(lConsBlq,lLoadEnder) CLASS WMSDTCEstoqueEndereco
Local lRet       := .T.
Local aAreaAnt   := SBE->(GetArea())
Local cAliasQry  := Nil
Local cStatus    := ""
Local cChkStatus := ""

Default lConsBlq := .F.
Default lLoadEnder := .T.

	cChkStatus := IIf(lConsBlq,"1|2|3|4|5|6","1|2|6")

	If !lLoadEnder .Or. Self:oEndereco:LoadData()
		cAliasQry := GetNextAlias()
		BeginSql Alias cAliasQry
		...
		If (cAliasQry)->(!Eof())
			...
			Else
				...
				EndIf
			EndIf
			If !Empty(cStatus) .And. cStatus <> (cAliasQry)-> BE_STATUS
			...
			EndIf
		EndIf
		// Permite o preenchimento de campos customizados na atualização do status do endereço
		If _lWMSATSBE
			ExecBlock("WMSATSBE",.F.,.F.,{ (cAliasQry)->RECNOSBE })
		EndIf
		(cAliasQry)->(dbCloseArea())
	EndIf
	RestArea(aAreaAnt)
...
Default cPrdOri   := cProduto
	...
	dUltFech := WMSC015Dat(cArmazem,cProduto,cPrdOri,cEndereco,cLoteCtl,cNumLote,cNumserie,cIdUnitiz)
...
Local oEstEnder := Nil 
Local lWMSVDSI := SuperGetMv("MV_WMSVDSI",.F.,.F.) //Parametro para validar divergencia de saldo estoque no processamento do inventário.
//"Indica deve validar saldos entre Estoque(SB8/SB2) e WMS(D14) no processamento do ajuste de inventário?"
	...
	If nSldB2 <> nSldD14
		/////////////////////////////
		/////LOG INVENTARIO//////////
		MA340GrvLg("214N -> ","06","PRODUT: "+cCod)
	...
	If lRastro
		...
		If nSldB8 <> nSldD14
			/////////////////////////////
			/////LOG INVENTARIO//////////
			MA340GrvLg("814N -> ","06","PRODUT: "+cCod)
			MA340GrvLg("         ","06","PRDORI: "+cCod)
			MA340GrvLg("         ","06","LOCAL: "+cLocal)
		...
		If D11->(dbSeek(xFilial("D11")+cCod))
			If SB2->(dbSeek(xFilial("SB2")+D11->D11_PRDORI+cLocal))
				...
				If nSldB2 <> nSldD14
					/////////////////////////////
					/////LOG INVENTARIO//////////
					MA340GrvLg("214P -> ","06","PRODUT: "+cCod)
				...
				If lRastro
					...
					If nSldB8 <> nSldD14
						/////////////////////////////
						/////LOG INVENTARIO//////////
						MA340GrvLg("814P -> ","06","PRODUT: "+cCod)
						MA340GrvLg("         ","06","PRDORI: "+D11->D11_PRDORI)
						MA340GrvLg("         ","06","LOCAL: "+cLocal)
						MA340GrvLg("         ","06","LOTECTL: "+cLoteCtl)
						MA340GrvLg("         ","06","NUMLOTE: "+cLote)
	...
	If  Empty(cLog)  
		...
		cAliasQry := oEstEnder:SelectKard(.F.,cLocal,cCod)
		If (cAliasQry)->(!Eof()) .And. (cAliasQry)->NR_COUNT > 0
			/////////////////////////////
			/////LOG INVENTARIO//////////
			MA340GrvLg("1413 -> ","06","PRODUT: "+cCod)
			MA340GrvLg("         ","06","PRDORI: "+cCod)
			MA340GrvLg("         ","06","LOCAL: "+cLocal)
			MA340GrvLg("         ","06","LOTECTL: "+cLoteCtl)
			MA340GrvLg("         ","06","NUMLOTE: "+cLote)
...
/*/
//----------------------------------------
METHOD GeraMovEst(cTipo,dDataMov) CLASS WMSDTCEstoqueEndereco
Local lRet       := .T.
...
Default dDataMov := StoD("")
	...
	oMovEstEnd:SetDtEsto(IIF(IsInCallStack("MATA241"),SD3->D3_EMISSAO,(IIF(!Empty(dDataMov), dDataMov, dDataBase))))
...
Private lDocWms     := Nil
	...
	If lRet
		...
		If lMSErroAuto
			...
			lRet := .F.
			If IsBlind()
				aRetMat := GetAutoGRLog()	 
				//Se ocorreu erro deveria retornar a mensagem. Mas caso isso não ocorra, exibir mensagem genérica.
				If Len(aRetMat) == 0
					Aadd(aRetMat, STR0026) //'Integração com estoque não efetuada. Motivo não identificado.'
				EndIf

				WMSVTAviso('Erro',AllTrim(aRetMat[1])) // Produto [VAR01] não encontrado no endereço [VAR02].
			Else
				MostraErro()
			EndIf
		EndIf
...
METHOD GeraEmpReq(cOrigem,cOp,cTrt,cIdDCF,lEstorno,lCriaSDC,lEmpD14,lOpEncer) CLASS WMSDTCEstoqueEndereco
Local lRet := .T.
...
/*/
//----------------------------------------
METHOD MakePerda(nRecnoSBC,cProg) CLASS WMSDTCEstoqueEndereco
Local lRet       := .T.
...
Default cProg	 := ""
	...
	If lRet
		...
		//Valida SD4 versus SBC, para garantir que o empenho posicionado corresponde a OP referenciada no apontamento de perda
		If Len(aProduto) > 0
			For nProduto := 1 To Len(aProduto)
				nSaldoPrd   := 0
				nSaldoPerda := 0
				...
				BeginSql Alias cAliasSD4
					...
					AND EXISTS(	SELECT 1
								FROM %Table:SDC% SDC
								WHERE SD4.D4_FILIAL = %xFilial:SDC%
								...
								AND SDC.DC_NUMLOTE = SD4.D4_NUMLOTE
								AND SDC.%NotDel%)
					AND SD4.%NotDel%
				EndSql
				while (cAliasSD4)->(!EoF()) .AND. nQtdPerda > 0   
					SD4->(DbGoTo((cAliasSD4)->RECNOSD4))
					//Armazena recno da SD4
					nRecnoAtu  := (cAliasSD4)->RECNOSD4
					lAchouSd4  := .T. 

					//quando é a segunda vez ou mais no while é necessário consultar novamente o saldo
					If !lPrim .AND. !lRastro
						Self:ClearData()
						Self:oProdLote:SetArmazem(oProdLote:GetArmazem())
						Self:oProdLote:SetProduto(aProduto[nProduto][1])	
						Self:oProdLote:SetPrdOri(aProduto[nProduto][3])
					...
					If !Empty(SD4->D4_QUANT) .And. nSaldoPerda > nSaldoEnd //.AND. lAchouSd4
						//Valor que precisará ser descontado da SD4 e SDC por falta de saldo no endereço
						nDiferenca := (nSaldoPerda - nSaldoEnd)

						//--Se a diferença é maior que o empenho, ajusta a variavel para não realizar a quebra da SD4 e assim fazer a liberação somente da quantidade empenhada
						//--A quantidade excedente será baixada fora do while
						If !lRastro .And. nDiferenca > SD4->D4_QUANT
							nDiferenca 	:= SD4->D4_QUANT
						EndIf 
						...
						Else
							...
							SD4->D4_IDDCF := ""
							SD4->(MsUnlock())
							If !Rastro(aProduto[nProduto][1]) .AND. cProg == "MATA685"
								WmsAtuSDC("SC2",;
										SD4->D4_OP,;
										SD4->D4_TRT,;
										Nil,;
					...
					//Baixa saldo
					If !lRastro .AND. nSaldoPerda > nSaldoEnd .AND. nDiferenca >= SD4->D4_QUANT
						Self:SetQuant(QtdComp(nDiferenca))
						nQtdPerda	-= nDiferenca
					else
						Self:SetQuant(QtdComp(nQtdPerda))   //quando controla rastro a baixa e divisão já foi 
						nQtdPerda := 0 //quando controla rastro ja foi dividido a SD4 anteriormente então nao precisa do loop na Sd4. 
					EndIf 
					lRet := Self:UpdSaldo("999",.T. /*lEstoque*/,.F. /*lEntPrev*/,.F. /*lSaiPrev*/,.F./*lEmpenho*/,.F. /*lBloqueio*/,.F. /*lEmpPrev*/,.T./*lMovEstEnd*/,dDataMov)

					//Baixa empenho separadamente, pois nem sempre a quantidade de baixa de estoque é igual a quantidade de baixa de empenho
					If lRet .And. cProg == "MATA685" .And. nDiferenca > 0
						Self:SetQuant(QtdComp(nDiferenca))
				...
				//--Se a perda é maior do que o empenho (saldo livre) realiza a baixa do saldo excedente fora do while
				If lRet .And. !lRastro .And. nDiferenca > 0 .And. cProg == "MATA685"
					If nQtdPerda > 0 
				...
				If !lAchouSd4  //se não achou Sd4 e o saldo livre no endereço for menor que a perda nao processar. 
					// Carrega dados para Estoque por Endereço
					Self:oEndereco:SetArmazem(oProdLote:GetArmazem())
					Self:oEndereco:SetEnder(SBC->BC_LOCALIZ)
					Self:oProdLote:SetArmazem(oProdLote:GetArmazem())
					...
					Self:SetQuant(QtdComp(nQtdPerda))   //nQtdPerda  //quando controla rastro a baixa e divisão já foi 
					lRet := Self:UpdSaldo("999",.T. /*lEstoque*/,.F. /*lEntPrev*/,.F. /*lSaiPrev*/,.F./*lEmpenho*/,.F. /*lBloqueio*/,.F. /*lEmpPrev*/,.T./*lMovEstEnd*/,dDataMov)

					//Baixa empenho separadamente, pois nem sempre a quantidade de baixa de estoque é igual a quantidade de baixa de empenho
					If lRet .And. cProg == "MATA685" .And. nDiferenca > 0
						//--Baixa empenho apenas da quantidade de perda que ultrapassou o saldo disponivel no endereço
						Self:SetQuant(QtdComp(nDiferenca))
						lRet := Self:UpdSaldo("999",.F. /*lEstoque*/,.F. /*lEntPrev*/,.F. /*lSaiPrev*/,.T./*lEmpenho*/,.F. /*lBloqueio*/,.F. /*lEmpPrev*/,.F./*lMovEstEnd*/)
					EndIf
				ENDIF
			...
			Next
		Else
			Self:cErro := STR0028 //Não há saldo disponivel em endereço de produção para apontamento de perda.
			lRet := .F.
		EndIf
	EndIf
	RestArea(aAreaSBC)
	...
	If lRet
		...
		If Len(aProduto) > 0
			For nProduto := 1 To Len(aProduto)
				...
				// Realiza Entrada Armazem Estoque por Endereço
				lRet := Self:UpdSaldo("499",.T. /*lEstoque*/,.F. /*lEntPrev*/,.F. /*lSaiPrev*/,.F. /*lEmpenho*/,.F. /*lBloqueio*/,.F. /*lEmpPrev*/,.T./*lMovEstEnd*/, dDataMov)
				// Verifica os movimentos da ordem de serviço origem para desconsiderar
				// no cálculo de estoque 
				If lRet .And. WmsX312118("D13","D13_USACAL")

					cOpDoc := SubStr( SBC->BC_OP, 1, TamSX3("D13_DOC")[1])
					...
					BeginSql Alias cAliasD13
						...
						FROM %Table:D13% D13
						WHERE D13.D13_FILIAL = %xFilial:D13%
						AND D13.D13_DOC = %Exp:cOpDoc%
						AND D13.D13_NUMSEQ = %Exp:SBC->BC_SEQSD3%
						AND D13.D13_USACAL <> '2'
						AND D13.%NotDel%
					EndSql
					Do While (cAliasD13)->(!Eof())
	...
	BeginSql Alias cAliasQry
		...
		FROM (
			...
			LEFT JOIN (
				...
				FROM %Table:D15% D15
				INNER JOIN 	(SELECT MAX(D15_DATA) D15_DATA,
									D15_LOCAL,
									D15_ENDER,
									D15_PRDORI,
									D15_PRODUT,
									...
									D15_IDUNIT
							FROM %Table:D15% D153
							WHERE D153.D15_FILIAL = %xFilial:D15%
							AND D153.D15_DATA <= %Exp:cUlMes%
				 			AND D153.%NotDel%
			...
			LEFT JOIN (
				...
				AND D13.D13_DTESTO > (SELECT COALESCE(MAX(D151.D15_DATA),' ')
									FROM %Table:D15% D151
			   						WHERE D151.D15_FILIAL = %xFilial:D15%
									AND D151.D15_DATA <=  %Exp:cUlMes%
									AND D151.%NotDel%
		...
		AND TOT.TOT_ENDER <> '  '
		//Trava para garantir que não serão processados produtos com componentes, visto que a rotina não encontra-se preparada para isso
		AND TOT.TOT_PRDORI = TOT.TOT_PRODUT
		%Exp:cWhere%
	EndSql
Return cAliasQry
...
Local cDecimal   := cValtoChar(Self:aD14_QTDEST[1])+","+cValtoChar(Self:aD14_QTDEST[2])
	...
	cCamposAux += " CAST(SUM(SLD.TOT_QTDEMP) AS DECIMAL ("+cDecimal+")) TOT_QTDEMP,"
	...
	cCamposAux += " CAST(SUM(SLD.TOT_MOVEMP) AS DECIMAL ("+cDecimal+")) TOT_MOVEMP"
	cCamposAux := "%"+cCamposAux+"%"
	cAliasQry := GetNextAlias()
	BeginSql Alias cAliasQry
		SELECT %Exp:cCampos%
		FROM (
				SELECT SLD.TOT_LOCAL TOT_LOCAL,
						...
						TRIM(SLD.TOT_NUMSER) TOT_NUMSER,
          				TRIM(SLD.TOT_IDUNIT) TOT_IDUNIT,
						%Exp:cCamposAux%
				FROM (
						...
						SELECT DCF.DCF_LOCAL TOT_LOCAL,
							   ...
							   TRIM('  ') TOT_NUMSER,
							   TRIM(DCF.DCF_UNITIZ) TOT_IDUNIT,
							   0 TOT_QTDEST,
							   0 TOT_QTDEPR,
							   0 TOT_QTDSPR,
							   0 TOT_QTDPEM,
						...
						SELECT DCF.DCF_LOCAL TOT_LOCAL,
						 ...
						 WHERE DCF.DCF_FILIAL = %xFilial:DCF%
						   ...
						   AND DCF.DCF_ENDER <> '  '
						   AND DCF.DCF_ENDDES <> '  '
						   AND ((SB1.B1_RASTRO IN ('L', 'S') AND DCF.DCF_LOTECT <> ' ') OR (SB1.B1_RASTRO ='N' AND DCF.DCF_LOTECT = ' '))
		    			   AND DCF.%NotDel%
						...
						SELECT DCF.DCF_LOCDES TOT_LOCAL,
								...
								DCF.DCF_NUMLOT TOT_NUMLOT,
								TRIM('  ') TOT_NUMSER,
								TRIM(DCF.DCF_UNIDES) TOT_IDUNIT,
								0 TOT_QTDEST,
								0 TOT_QTDEPR,
								0 TOT_QTDSPR,
								0 TOT_QTDPEM,
						...
						SELECT D12.D12_LOCDES TOT_LOCAL,
							   ...
							   TRIM(D12.D12_NUMSER) TOT_NUMSER,
                       		   TRIM(D12.D12_UNIDES) TOT_IDUNIT,
							   0 TOT_QTDEST,
							   0 TOT_QTDEPR,
							   0 TOT_QTDSPR,
							   0 TOT_QTDPEM,
						 ...
						 WHERE D12.D12_FILIAL = %xFilial:D12%
						   AND D12.D12_PRDORI = %Exp:cProduto%
						...
						SELECT D12.D12_LOCORI TOT_LOCAL,
							   ...
							   D12.D12_NUMLOT TOT_NUMLOT,
							   TRIM(D12.D12_NUMSER) TOT_NUMSER,
                       		   TRIM(D12.D12_IDUNIT) TOT_IDUNIT,
							   0 TOT_QTDEST,
							   0 TOT_QTDEPR,
							   0 TOT_QTDSPR,
							   0 TOT_QTDPEM,
						...
						//Movimentos pendentes de finalização de Requisição origem Sd4 e SC9 onde no endereço de origem tem o Qtdpem
		   				SELECT D12.D12_LOCORI TOT_LOCAL
							,D12.D12_ENDORI TOT_ENDER
							,D12.D12_PRDORI TOT_PRDORI
							,D12.D12_PRODUT TOT_PRODUT
						...
						AND DC5.DC5_OPERAC IN ('3','4') //Somente serviços de separação geram previsão de empenho conforme método ChkSepara()
						AND DC5.%NotDel%
						WHERE D12.D12_FILIAL = %xFilial:D12%
						AND D12.D12_PRDORI = %Exp:cProduto%
						AND D12.D12_STATUS NOT IN ('0','1')
					...
					UNION ALL
					   SELECT D14.D14_LOCAL TOT_LOCAL,
							  ...
							  D14.D14_LOTECT TOT_LOTECT,
							  D14.D14_NUMLOT TOT_NUMLOT,
							  TRIM(D14.D14_NUMSER) TOT_NUMSER,
                       		  TRIM(D14.D14_IDUNIT) TOT_IDUNIT,
					   ...
					   SELECT D0V.D0V_LOCAL TOT_LOCAL,
							  ...
							  TRIM('  ') TOT_NUMSER,
							  TRIM(D0V.D0V_IDUNIT) TOT_IDUNIT,
							  0 TOT_QTDEST,
							  0 TOT_QTDEPR,
							  0 TOT_QTDSPR,
							  0 TOT_QTDPEM,
						...
						WHERE D0V.D0V_FILIAL = %xFilial:D0V%
						  AND D0V.D0V_PRDORI = %Exp:cProduto%
						  AND D0V.%NotDel%

						UNION ALL

						//Pedidos que já encontram-se com movimentação finalizada mas não faturados
						SELECT SC9.C9_LOCAL TOT_LOCAL,
							   SC9.C9_ENDPAD TOT_ENDER,
							   SC9.C9_PRODUTO TOT_PRDORI,
							   D12.D12_PRODUT TOT_PRODUT,
							   SC9.C9_LOTECTL TOT_LOTECT,
							   SC9.C9_NUMLOTE TOT_NUMLOT,
							   TRIM('  ') TOT_NUMSER,
							   TRIM(D12.D12_UNIDES) TOT_IDUNIT,
							   0 TOT_QTDEST,
							   0 TOT_QTDEPR,
						...
						INNER JOIN %Table:D12% D12 
						  ...
						  AND D12.D12_LOTECT = SC9.C9_LOTECTL
						  AND D12.D12_NUMLOT = SC9.C9_NUMLOTE
						  AND D12.D12_STATUS = '1'
						  AND D12.D12_ATUEST = '1'
						  AND D12.D12_BXESTO = '2'
						  AND D12.%NotDel%
						INNER JOIN %Table:SDC% SDC 
						  ON SDC.DC_FILIAL = %xFilial:SDC%
						  AND SDC.DC_PRODUTO = SC9.C9_PRODUTO
						  AND SDC.DC_LOCAL = SC9.C9_LOCAL
						  AND SDC.DC_ORIGEM = 'SC6'
						  AND SDC.DC_PEDIDO = SC9.C9_PEDIDO
						  AND SDC.DC_ITEM = SC9.C9_ITEM
						  AND SDC.DC_SEQ = SC9.C9_SEQUEN
						  AND SDC.DC_LOTECTL = SC9.C9_LOTECTL
						  AND SDC.DC_NUMLOTE = SC9.C9_NUMLOTE
						  AND SDC.DC_NUMSERI = SC9.C9_NUMSERI
						  AND SDC.DC_IDDCF = SC9.C9_IDDCF
						  AND SDC.%NotDel%
						...
						// Empenhos de ordem de produção, ou seja, produtos requisitados para ordens de produção, com serviço que não baixa empenho e movimentação realizada
						SELECT SDC.DC_LOCAL TOT_LOCAL,
							   SDC.DC_LOCALIZ TOT_ENDER,
							   SDC.DC_PRODUTO TOT_PRDORI,
							   CASE WHEN D11.D11_PRDCMP IS NULL THEN SDC.DC_PRODUTO ELSE D11.D11_PRDCMP END TOT_PRODUT,
							   SDC.DC_LOTECTL TOT_LOTECT,
							   SDC.DC_NUMLOTE TOT_NUMLOT,
							   TRIM(SDC.DC_NUMSERI) TOT_NUMSER,
							   TRIM(' ') TOT_IDUNIT,
							   0 TOT_QTDEST,
							   0 TOT_QTDEPR,
							   0 TOT_QTDSPR,
							   ...
							   0 TOT_MOVPEM,
							   0 TOT_MOVBLQ,
							   SUM(SDC.DC_QUANT) TOT_MOVEMP
						  FROM %Table:SDC% SDC
						  LEFT JOIN %Table:D11% D11
						    ON D11.D11_FILIAL = %xFilial:D11%
						   AND D11.D11_PRODUT = SDC.DC_PRODUTO
						   AND D11.D11_PRDORI = SDC.DC_PRODUTO
						   AND D11.D11_PRODUT = D11.D11_PRDORI
						   AND D11.%NotDel% 
						 WHERE SDC.DC_FILIAL = %xFilial:SDC%
						   AND SDC.DC_PRODUTO = %Exp:cProduto%
						   AND SDC.DC_ORIGEM NOT IN ('SDD','SC6')
						   AND SDC.%NotDel%
						 GROUP BY SDC.DC_LOCAL,
								  SDC.DC_LOCALIZ,
								  SDC.DC_PRODUTO,
								  D11.D11_PRDCMP,
								  SDC.DC_LOTECTL,
						...
						// Movimentações de unitizadores pendentes de serem executadas
					   SELECT D12.D12_LOCDES TOT_LOCAL,
							  D12.D12_ENDDES TOT_ENDER,
							  D14.D14_PRDORI TOT_PRDORI,
							  D14.D14_PRODUT TOT_PRODUT,
						...
						INNER JOIN %Table:D14% D14
					       ON D14.D14_FILIAL = %xFilial:D14%
						  AND D14.D14_IDUNIT = D12.D12_UNIDES
						  AND D14.D14_LOCAL = D12.D12_LOCORI
						  AND D14.D14_ENDER = D12.D12_ENDORI
						  AND D14.D14_PRDORI = %Exp:cProduto%
						  AND D14.%NotDel% 
						WHERE D12.D12_FILIAL = %xFilial:D12%
						  AND D12.D12_PRODUT = ' '
						...
						// Ordens de serviço de unitizadores não executadas, onde o endereço destino e origem encontram-se preenchidos. 
						// Como é o caso da transferência de unitizador e transferência de devolução
					   SELECT DCF.DCF_LOCDES TOT_LOCAL,
							  DCF.DCF_ENDDES TOT_ENDER,
							  D14.D14_PRDORI TOT_PRDORI,
							  D14.D14_PRODUT TOT_PRODUT,
						...
						INNER JOIN %Table:D14% D14
						   ON D14.D14_FILIAL = %xFilial:D14%
						  AND D14.D14_IDUNIT = DCF.DCF_UNITIZ
						  AND D14.D14_LOCAL = DCF.DCF_LOCAL
						  AND D14.D14_ENDER = DCF.DCF_ENDER
						  AND D14.D14_PRDORI = %Exp:cProduto%
						  AND D14.%NotDel% 
						WHERE DCF.DCF_FILIAL = %xFilial:DCF%
						  AND DCF.DCF_STSERV NOT IN ('0','3')
					   ...
					   // Ordens de serviço de unitizadores não executadas, onde o endereço destino encontra-se vazio e origem encontram-se preenchidos. 
					   // Como é dos endereçamentos, movimentos internos de devolução e transferências sem endereço endereço destino informado.
					   SELECT DCF.DCF_LOCAL TOT_LOCAL,
							  DCF.DCF_ENDER TOT_ENDER,
							  D14.D14_PRDORI TOT_PRDORI,
							  D14.D14_PRODUT TOT_PRODUT,
					   ...
					   //Demanda de unitização, onde os untizadores ainda não foram montados.
					   SELECT D0Q.D0Q_LOCAL TOT_LOCAL,
							  D0Q.D0Q_ENDER TOT_ENDER,
							  D0Q.D0Q_PRDORI TOT_PRDORI,
							  CASE WHEN D11.D11_PRDCMP IS NULL THEN D0Q.D0Q_CODPRO ELSE D11.D11_PRDCMP END TOT_PRODUT,
							  D0Q.D0Q_LOTECT TOT_LOTECT,
							  D0Q.D0Q_NUMLOT TOT_NUMLOT,
							  TRIM('  ') TOT_NUMSER,
							  TRIM(' ') TOT_IDUNIT,
							  0 TOT_QTDEST,
							  0 TOT_QTDEPR,
							  ...
							  0 TOT_QTDEMP,
							  0 TOT_MOVEPR,
							  CASE WHEN D11.D11_QTMULT IS NULL THEN (D0Q.D0Q_QUANT - D0Q.D0Q_QTDUNI) ELSE (D0Q.D0Q_QUANT - D0Q.D0Q_QTDUNI) * D11.D11_QTMULT END TOT_MOVSPR,
							  0 TOT_MOVPEM,
							  0 TOT_MOVBLQ,
							  0 TOT_MOVEMP
						 FROM %Table:D0Q% D0Q
						 LEFT JOIN %Table:D11% D11 
						  ...
						  AND D11.D11_PRDORI = D0Q.D0Q_PRDORI
						  AND D11.D11_PRODUT = D11.D11_PRDORI
						  AND D11.%NotDel%
						WHERE D0Q.D0Q_FILIAL = %xFilial:D0Q%
						  AND D0Q.D0Q_PRDORI = %Exp:cProduto%
					   ...
					   //Unitizadores montados mas que ainda não tiveram ordem de serviço gerada
					   SELECT D0R.D0R_LOCAL TOT_LOCAL,
							  D0R.D0R_ENDER TOT_ENDER,
							  D0S.D0S_PRDORI TOT_PRDORI,
							  D0S.D0S_CODPRO TOT_PRODUT,
							  D0S.D0S_LOTECT TOT_LOTECT,
							  D0S.D0S_NUMLOT TOT_NUMLOT,
							  TRIM('  ') TOT_NUMSER,
							  TRIM(D0S.D0S_IDUNIT) TOT_IDUNIT,
							  0 TOT_QTDEST,
							  0 TOT_QTDEPR,
							  0 TOT_QTDSPR,
							  0 TOT_QTDPEM,
						...
						INNER JOIN %Table:D0S% D0S
						  ...
						  AND D0S.D0S_PRDORI = %Exp:cProduto%
					   ...
					   // Conferências de recebimento em armazém unitizado que ainda não foram finalizadas
					   SELECT D0K.D0K_LOCAL TOT_LOCAL,
							  SD1.D1_ENDER TOT_ENDER,
							  D0K.D0K_PROD TOT_PRDORI,
							  CASE WHEN D11.D11_PRDCMP IS NULL THEN D0K.D0K_PROD ELSE D11.D11_PRDCMP END TOT_PRODUT,
							  D0K.D0K_LOTE TOT_LOTECT,
							  D0K.D0K_SUBLOT TOT_NUMLOT,
							  TRIM('  ') TOT_NUMSER,
							  TRIM(' ') TOT_IDUNIT,
							  0 TOT_QTDEST,
							  0 TOT_QTDEPR,
							  ...
							  0 TOT_QTDEMP,
							  0 TOT_MOVEPR,
							  CASE WHEN D11.D11_QTMULT IS NULL THEN D0K.D0K_QUANT ELSE (D0K.D0K_QUANT * D11.D11_QTMULT) END TOT_MOVSPR,
							  0 TOT_MOVPEM,
							  0 TOT_MOVBLQ,
							  0 TOT_MOVEMP
						 FROM %Table:DCW% DCW
						INNER JOIN %Table:D0K% D0K
						   ON D0K.D0K_FILIAL = %xFilial:D0K%
						  AND D0K.D0K_EMBARQ = DCW.DCW_EMBARQ
						  AND D0K.D0K_PROD = %Exp:cProduto%
						  AND D0K.%NotDel%  
						INNER JOIN %Table:NNR% NNR
						   ON NNR.NNR_FILIAL = %xFilial:NNR%
						...
						INNER JOIN %Table:SD1% SD1
						 ...
						 LEFT JOIN %Table:D11% D11 
						   ON D11.D11_FILIAL = %xFilial:D11%
						  AND D11.D11_PRODUT = D0K.D0K_PROD
						  AND D11.D11_PRDORI = D0K.D0K_PROD
						  AND D11.D11_PRODUT = D11.D11_PRDORI
						  AND D11.%NotDel% 
		...
		WHERE TOT.TOT_LOCAL = %Exp:cArmazem%
		  AND (TOT.TOT_QTDEPR <> TOT.TOT_MOVEPR OR
			   TOT.TOT_QTDSPR <> TOT.TOT_MOVSPR OR
   			   TOT.TOT_QTDPEM <> TOT.TOT_MOVPEM OR
   			   TOT.TOT_QTDBLQ <> TOT.TOT_MOVBLQ OR
   			   TOT.TOT_QTDEMP <> TOT.TOT_MOVEMP)
	EndSql
	If lProcess
		...
		TcSetField(cAliasQry,'TOT_QTDEMP','N',Self:aD14_QTDEST[1],Self:aD14_QTDEST[2])
		...
		TcSetField(cAliasQry,'TOT_MOVEMP','N',Self:aD14_QTDEST[1],Self:aD14_QTDEST[2])
	EndIf
Return cAliasQry
//----------------------------------------
...
Local nMovEm2    := 0
	...
	If lContinua
		...
		Do While (cAliasQry)->(!Eof())
			If lProcess
				oProcess:IncRegua1( WmsFmtMsg(STR0027 + "...",{{"[VAR01]",cValToChar(oProcess:nMeter1+1)},{"[VAR02]",cValToChar(oProcess:oMeter1:nTotal)},{"[VAR03]",cProduto}}) ) // Processando [VAR01]/[VAR02] registro(s) do produto [VAR03]
			...
			nMovEmp    := (cAliasQry)->TOT_MOVEMP
			nMovEm2    := ConvUm(Self:oProdLote:GetProduto(),nMovEmp,0,2)
			...
			D14->D14_QTDEMP := nMovEmp
			D14->D14_QTDEM2 := nMovEm2
			// Verifica se quantidade foram zeradas.
			If QtdComp(D14->D14_QTDEST) == 0 .AND. QtdComp(D14->D14_QTDEPR) == 0 .AND. QtdComp(D14->D14_QTDSPR) == 0 .AND. QtdComp(D14->D14_QTDEMP) == 0 .AND. QtdComp(D14->D14_QTDBLQ) == 0 .AND. QtdComp(D14->D14_QTDPEM) == 0
				D14->(dbDelete())
				lAnalisEnd := .T.
...
Local cEndereco  := ""
Local cIdDCF     := ""
Local nTamDec    := GetWmsSX3("D3_CUSTO1", 2)
Local nHdlPrv    := 0
Local nI         := 0
Local nX         := 0
Local nTotal     := 0
			...
			If	!B2AtuComD3(aCusto)
				...
				For nI := 2	To	Len(aArrSD3)
					...
					If (cAliasSB1)->(!Eof())
						If Rastro(cProduto)
							cAliasSB8 := GetNextAlias()
							If !FwIsInCallStack("WMSA520") //DLOGWMSMSP-15273. Na desmontagem, pegar data de validade de qualquer armazem
								cQryLocal := "% SB8.B8_LOCAL = '"+cLocal+"' AND D_E_L_E_T_ = ' ' %"
							EndIf
							BeginSql Alias cAliasSB8
								SELECT SB8.B8_DTVALID,
										SB8.B8_DFABRIC,
...
/*/{Protheus.doc} WmsEmpPrde
	Metodo utilizado para desconsiderar os empenhos da SB8 quando será utilizado por outra OP .
	@author Equipe WMS
	@since 05/11/2021
	/*/
Method WmsEmpPrde() CLASS WMSDTCEstoqueEndereco
Local cQuery 	:= ""
LOCAL cAliasSB8 :=GetNextAlias()
Local nSaldoEp 	:= 0
Local lConsVec  := SuperGetMV('MV_LOTVENC',.F., 'N') == "S"
Local cIsNull   := ""
Local cTipoDB	:= Alltrim(Upper(TCGetDB()))
Local dDataAtu	:= dDatabase

	//Tratamento para que o valor da consulta não seja "null"
	IIF(("INFORMIX" $ cTipoDB) .Or. ("ORACLE" $ cTipoDB), cIsNull := "NVL",;
	IIF(("DB2" $ cTipoDB) .Or. ("POSTGRES" $ cTipoDB), cIsNull := "COALESCE",;
	cIsNull := "ISNULL" ) )

	cQuery := "%  "+cIsNull+"( SUM( CASE WHEN B8_SALDO > D14_QTDSLD THEN D14_QTDSLD ELSE B8_SALDO END ),0) SLDB8D14 "
	cQuery += "	FROM ( SELECT SB8.B8_LOTECTL, " 
	cQuery += "		SUM(SB8.B8_SALDO - SB8.B8_EMPENHO) B8_SALDO,D14_QTDSLD
	cQuery += "		FROM " + RetSqlName('SB8')+" SB8"
	cQuery += " 	INNER JOIN (SELECT D14.D14_LOTECT,SUM(D14_QTDEST - D14_QTDSPR - D14_QTDEMP - D14_QTDBLQ) D14_QTDSLD "
	cQuery += "					FROM  " + RetSqlName('D14')+" D14"
	cQuery += 				" 	INNER JOIN " + RetSqlName('DC8')+" DC8"
	cQuery += 				" 	ON DC8.DC8_FILIAL =  '"+xFilial("DC8")+"'" 
	cQuery += 				"	AND DC8.DC8_CODEST = D14.D14_ESTFIS "
	...
	If !Empty(Self:oProdLote:GetPrdOri())
		cQuery += 			" AND D14.D14_PRDORI = '"+Self:oProdLote:GetPrdOri()+"'"
	EndIf
	//Verifica se o lote na tabela D14 esta vencido
	If !lConsVec 
		cQuery +=           " AND D14.D14_DTVALD >= '"+DTOS(dDataAtu)+"'"
	EndIf
	cQuery += 				"	AND D14.D_E_L_E_T_ = ' ' "
	cQuery += 				"	GROUP BY D14.D14_LOTECT) D14L "
	cQuery += 		" ON D14L.D14_LOTECT = SB8.B8_LOTECTL "
	cQuery += 		" WHERE SB8.B8_FILIAL = '"+xFilial("SB8")+"'"
	If !Empty(Self:oEndereco:GetArmazem())
		cQuery += 	" AND SB8.B8_LOCAL = '"+Self:oEndereco:GetArmazem()+"'"

	EndIf
	cQuery += 		" AND SB8.B8_PRODUTO = '"+Self:oProdLote:GetProduto()+"'"
	cQuery += 		" AND SB8.D_E_L_E_T_ = ' '"
	cQuery += 		" GROUP BY B8_LOTECTL,D14_QTDSLD) TAB "
	cQuery += "%" 
	BeginSql Alias cAliasSB8
		SELECT %Exp:cQuery%
	EndSql
	nSaldoEp := (cAliasSB8)->SLDB8D14
	(cAliasSB8)->( dbCloseArea())
	
Return nSaldoEp

//----------------------------------------
/*/{Protheus.doc} MakeFtMI
Realisa a retirada da DOCA ao realisar o faturamento de Rmeito de Transferência
...
/*/
//----------------------------------------
METHOD MakeFtMI(nRecnoSD2, cIdDCF, cEnder,lAtuEmp) CLASS WMSDTCEstoqueEndereco
Local lRet       := .T.
Local aProduto   := {}
Local nProduto   := 0
Local oMovEstEnd := WMSDTCMovimentosEstoqueEndereco():New()
		...
		If Len(aProduto) > 0
			For nProduto := 1 To Len(aProduto)
				...
				Self:SetBlkDoc({|oMovEstEnd|;
					oMovEstEnd:SetOrigem("SD2"),;
					oMovEstEnd:SetDocto(SD2->D2_DOC),;
					oMovEstEnd:SetSerie(SD2->D2_Serie),;
					oMovEstEnd:SetNumSeq(SD2->D2_ITEM),;
					oMovEstEnd:SetIdDCF(cIdDCF);
				})
				// Realiza Entrada Armazem Estoque por Endereço
				lRet := Self:UpdSaldo("999",.T. /*lEstoque*/,.F. /*lEntPrev*/,.F. /*lSaiPrev*/,lAtuEmp /*lEmpenho*/,.F. /*lBloqueio*/,.F. /*lEmpPrev*/,.T./*lMovEstEnd*/)
			Next
		Else
			Self:cErro := STR0004 // Produto não encontrado.
			lRet := .F.
...
/*/
//----------------------------------------
METHOD UndoFtMi(nRecno, lEmpenho, lAtuDCF) CLASS WMSDTCEstoqueEndereco
Local lRet       := .T.
Local aProduto   := {}
Local aAreaSC9   := SD2->(GetArea())
Local oProdLote  := WMSDTCProdutoLote():New()
Local oMovEstEnd := WMSDTCMovimentosEstoqueEndereco():New()
Local cAliasD13  := Nil
Local nProduto   := 0
...
Default lAtuDCF  := .T.
	...
	If lRet
		...
		oProdLote:SetLoteCtl(SD2->D2_LOTECTL)
		oProdLote:SetNumLote(SD2->D2_NUMLOTE)
		oProdLote:SetNumSer(SD2->D2_NUMSERIE)
		oProdLote:LoadData()
		// AtualizaSaldo
		If lAtuDCF
			//Fazer um select na DCF para buscar o endereço destino gravado na execução do servico para poder voltar o saldo para o enderco 
			// Parâmetro cWhere
			If !Empty(SD2->D2_LOTECTL)
				cWhere += " AND DCF.DCF_LOTECT = '"+SD2->D2_LOTECTL+"'"
			EndIf
			cWhere := "%"+cWhere+"%"
			...
			BeginSql Alias cAliasDCF
				...
				FROM %Table:DCF% DCF
				WHERE DCF.DCF_FILIAL = %xFilial:DCF%
				AND DCF.DCF_DOCTO = %Exp:SD2->D2_DOC%
				AND DCF.DCF_SERIE = %Exp:SD2->D2_SERIE%
				And DCF.DCF_CODPRO = %Exp:SD2->D2_COD%
				AND DCF.%NotDel%
				%Exp:cWhere%
			EndSql
			If (cAliasDCF)->(!Eof())
			   	cIdDCF  := (cAliasDCF)->DCF_ID
			   cEndDes :=(cAliasDCF)->DCF_ENDDES
			Else 
		    	lRet := .F.
				Self:cErro := "Não eoncontrado dados de endereço para retorno de saldo " // Recno não informado!
			EndIf
			(cAliasDCF)->(dbCloseArea())
		Else
			cEndDes := SD2->D2_LOCALIZ
		EndIf 
		
		// Carrega estrutura do produto x componente
		aProduto := oProdLote:GetArrProd()
		If lRet .And. Len(aProduto) > 0
			For nProduto := 1 To Len(aProduto)
				// Carrega dados para Estoque por Endereço
				Self:oEndereco:SetArmazem(oProdLote:GetArmazem())
				Self:oProdLote:SetArmazem(oProdLote:GetArmazem())
				...
				Self:LoadData()
				Self:SetQuant(QtdComp(SD2->D2_QUANT * aProduto[nProduto][2]) )
				Self:oEndereco:SetEnder(cEndDes)
				// Seta o bloco de código para informações do documento para o Kardex
				Self:SetBlkDoc({|oMovEstEnd|;
					oMovEstEnd:SetOrigem("SD2"),;
					oMovEstEnd:SetDocto(SD2->D2_DOC),;
				...
				If lRet .And. WmsX312118("D13","D13_USACAL")
					...
					BeginSql Alias cAliasD13
						...
						FROM %Table:D13% D13
						WHERE D13.D13_FILIAL = %xFilial:D13%
						AND D13.D13_IDDCF = %Exp:cIdDCF%
						AND D13.D13_DOC =  %Exp:SD2->D2_DOC%
						AND D13.D13_NUMSEQ = %Exp:SD2->D2_ITEM%
						AND D13.D13_USACAL <> '2'
						AND D13.%NotDel%
					EndSql
					Do While (cAliasD13)->(!Eof())
