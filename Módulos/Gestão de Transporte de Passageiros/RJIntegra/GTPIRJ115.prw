					...
					If lContinua
							...
							For nY := 1 To Len(aFldDePara)
								...
								If !Empty(cTagName) .And. !Empty((xValor := oRJIntegra:GetJsonValue(nX, cTagName, cTipoCpo)))
									...
									If lPEIntId .AND. cCampo == "GIC_AGENCI"
							...
							aRequisicao := {}	
							If lIntReq .AND. (/*!EMPTY(oRJIntegra:GetJsonValue(nX, "numeroRequisicao1", "C")) .OR. */;
											  !EMPTY(oRJIntegra:GetJsonValue(nX, "codigoClienteRequisicao1", "C")) .OR.;
								...
								aAdd(aRequisicao, {	cCode,;
													...
													ALLTRIM(cCliReq),; 						//GQW_CODCLI + GQW_CODLOJ
...
If Len(aParam) == 11
	GI115Receb(.F., Nil, aParam[1], aParam[2], aParam[3], aParam[4], aParam[5], aParam[6], aParam[7],,lAuto)
ElseIf Len(aParam) == 9
	GI115Receb(.F., Nil, aParam[1], aParam[2], aParam[3], aParam[4], aParam[5],,,,lAuto)
Else
	GTPIRJ115(.F.,,,lAuto)
EndIf
...
Function GI115Req(aRequisicao, cErro)
	...
	IF lValLj
		...
		aRequisicao[1][3] := padl(LEFT(aRequisicao[1][3],Len(aRequisicao[1][3])-nTamLoj),nTamCod,'0') + RIGHT(aRequisicao[1][3],nTamLoj)
	ELSE
		aRequisicao[1][3] := Padl(aRequisicao[1][3],nTamCod,'0')
	ENDIF 
	
	cCliente := POSICIONE('SA1',1,xFilial('SA1') + aRequisicao[1][3],'A1_COD+A1_LOJA')
	cCliente := IIF(!EMPTY(cCliente),cCliente,POSICIONE('SA1',3,xFilial('SA1') + aRequisicao[1][4],'A1_COD+A1_LOJA'))
	cCliente := IIF(EMPTY(cCliente),cCliente,POSICIONE('GQV',1,xFilial('GQV')+cCliente,'GQV_CODIGO+GQV_CODLOJ'))
