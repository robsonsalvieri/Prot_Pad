    ...
    If !Empty(oModel:GetValue('H7QRECEITA', 'H7Q_NUMTIT'))
        ...
        If SE1->(dbSeek(cChaveSE1))
            ...
            If !Empty(SE1->E1_BAIXA)
                ...
                MsExecAuto({|x,y| Fina070(x,y)}, aTitSE1, 5) // 3-Inclusao,4-Alteração,5-Cancelamento de baixa
        ...
        If SE2->(dbSeek(cChaveSE2))
            ...
            If !Empty(SE2->E2_BAIXA)
             ...
             //   cFilAnt := 	SE2->E2_FILORIG
                ...
                MsExecAuto({|x,y| Fina080(x,y)}, aTitSE2, 5) // 3-Inclusao,4-Alteração,5-Cancelamento de baixa	
...
/*/{Protheus.doc} EstTitDep(oModel)
Função que gera os títulos dos depositos do caixa do urbano
@type Static Function
@author José Carlos
@since 12/03/2024
@version 1.0
/*/
Static Function EstTitDep(oModel)
Local lRet 		:= .T.
Local aTitSE1   := {}
Local cChaveSE1 := ''
Local nX        := 0
...
For nX := 1 To oModel:GetModel('H81DEPOSITO'):Length()
    ...
    If !Empty(oModel:GetValue('H81DEPOSITO', 'H81_NUMTIT'))
        lEstorno := oModel:GetValue('H81DEPOSITO', 'H81_TPMOV') == '2'

        cChaveSE1 := oModel:GetValue('H81DEPOSITO', 'H81_FILTIT')+;
                     oModel:GetValue('H81DEPOSITO', 'H81_PRETIT')+;
                     oModel:GetValue('H81DEPOSITO', 'H81_NUMTIT')+;
                     oModel:GetValue('H81DEPOSITO', 'H81_PARTIT')+; 
                     oModel:GetValue('H81DEPOSITO', 'H81_TIPTIT')
                     

        If !lEstorno .AND. SE1->(dbSeek(cChaveSE1))
            ...
            Else
                ...
                oModel:GetModel('H81DEPOSITO'):ClearField('H81_FILTIT')
                oModel:GetModel('H81DEPOSITO'):ClearField('H81_PRETIT')
                oModel:GetModel('H81DEPOSITO'):ClearField('H81_NUMTIT')
                oModel:GetModel('H81DEPOSITO'):ClearField('H81_PARTIT')
                oModel:GetModel('H81DEPOSITO'):ClearField('H81_TIPTIT')
                oModel:GetModel('H81DEPOSITO'):ClearField('H81_CLIFOR')
                oModel:GetModel('H81DEPOSITO'):ClearField('H81_LOJTIT')

            Endif

        ElseIf lEstorno .AND. SE2->(dbSeek(cChaveSE1))
            ...
            If !Empty(SE2->E2_BAIXA)
                ...
                MsExecAuto({|x,y| Fina080(x,y)}, aTitSE1, 5) // 3-Inclusao,4-Alteração,5-Cancelamento de baixa	
