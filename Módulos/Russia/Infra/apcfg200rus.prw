...
//-------------------------------------------------------------------
/*/{Protheus.doc} ApCfg200RUS
Fonte de cadastro de grupo de empresas localizado para a Rússia
...
If nModulo <> 99
	...
	DEFINE MSDIALOG oDlg FROM aCoors[1],aCoors[2] TO aCoors[3],aCoors[4] STYLE nOR(WS_VISIBLE,WS_POPUP) PIXEL
		...
		// Define o Browse
		//-------------------------------------------------------------------
		DEFINE FWFORMBROWSE oBrowse DATA TABLE ALIAS "XX8"  FILTER FILTERDEFAULT "XX8_TIPO == '0'" DOUBLECLICK { || FWExecView('View','APCFG200',MODEL_OPERATION_VIEW , , { || .T. }) } OF oDlg
	
			//-------------------------------------------------------------------
			// Adiciona os botões do Browse
			//-------------------------------------------------------------------
			ADD BUTTON oButton TITLE STR0045 ACTION 'VIEWDEF.APCFG200' OPERATION 1 OF oBrowse // "Visualizar"
			ADD BUTTON oButton TITLE STR0046 ACTION 'VIEWDEF.APCFG200' OPERATION 4 OF oBrowse // "Alterar"
			ADD BUTTON oButton TITLE STR0047 ACTION "CRMA680RUS('SM0',xFilial('SM0')+ XX8->XX8_CODIGO + XX8->XX8_TIPO,.F.,"+"('"+STR0047 +"' + ' ' + XX8->XX8_DESCRI))" OPERATION 4 OF oBrowse // "Added button Address"
			
			ADD COLUMN oColumn DATA { || XX8->XX8_CODIGO } TITLE STR0041 SIZE 12 OF oBrowse //"Code"
			ADD COLUMN oColumn DATA { || XX8->XX8_DESCRI } TITLE STR0042 SIZE 40 OF oBrowse //"Description"
			ADD COLUMN oColumn DATA { || XX8->XX8_SIZE   } TITLE STR0043 SIZE  2 OF oBrowse //"Size"
			ADD COLUMN oColumn DATA { || XX8->XX8_LEIAUT } TITLE STR0044 SIZE 12 OF oBrowse //"Layout"
	
			oBrowse:SetDBFFilter( .T. )
			oBrowse:SetCacheView( .F. )
Else
	oBrowse := FWLoadBrw( 'APCFG200' )
EndIf
SetKey(K_CTRL_A,{|| CRMA680RUS('SM0',xFilial('SM0')+ XX8->XX8_CODIGO + XX8->XX8_TIPO,.F.,STR0047 + ' ' + XX8->XX8_DESCRI)})
...
aRotina := FWLoadMenuDef( "APCFG200" )
Return aRotina
...
//Modelo
oModel := FWLoadModel( 'APCFG200' )

//Struct
oStrct := FWFormModelStruct():New()

//Table
oStrct:AddTable( C_ALIAS_RUS , { 'CO_COMPGRP' , 'CO_TIPO' } , 'Teste' , { || MPSysTblPrefix() + C_TABLE_RUS } )
...
oStrct:AddField(STR0914,STR0915,'CO_OGRN','C',015,0,{|oModel| VD_ORGN(oModel)},,,,,)
...
//Relation
oModel:SetRelation( 'SIGAMAT_COMP_RUS' , { { 'CO_COMPGRP' , 'XX8->XX8_CODIGO' } , { 'CO_TIPO' , 'XX8->XX8_TIPO' } } , 'CO_COMPGRP+CO_TIPO' )


//SetKey(K_CTRL_A,{|| CRMA680RUS("SM0",xFilial("SM0")+ XX8->XX8_CODIGO + XX8->XX8_TIPO)})
...
oView:AddUserButton(STR0047,'AddrButton', {|| CRMA680RUS('SM0',xFilial('SM0')+ XX8->XX8_CODIGO + XX8->XX8_TIPO,.F.,STR0047+ " " + XX8->XX8_DESCRI)}, /*[cToolTip]*/, K_CTRL_A) // Other Actions - address button in viewdef
//SetKey(K_CTRL_A,{|| CRMA680RUS('SM0',xFilial('SM0')+ XX8->XX8_CODIGO + XX8->XX8_TIPO)})
