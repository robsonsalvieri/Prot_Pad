...
//-------------------------------------------------------------------
/*/{Protheus.doc} ApCfg220RUS
Fonte de cadastro de unidade de negócio localizado para a Rússia
...
If nModulo <> 99
	...
	DEFINE MSDIALOG oDlg FROM aCoors[1],aCoors[2] TO aCoors[3],aCoors[4] STYLE nOR(WS_VISIBLE,WS_POPUP) PIXEL
		...
		// Define o Browse
		//-------------------------------------------------------------------
		DEFINE FWFORMBROWSE oBrowse DATA TABLE ALIAS "XX8" FILTER FILTERDEFAULT "XX8_TIPO == '2'"  DOUBLECLICK { || FWExecView('View','APCFG220', MODEL_OPERATION_VIEW , , { || .T. }) } OF oDlg  
	
			//-------------------------------------------------------------------
			// Adiciona os botões do Browse
			//-------------------------------------------------------------------
			ADD BUTTON oButton TITLE STR0041 ACTION 'VIEWDEF.APCFG220' OPERATION 1 OF oBrowse // "Visualizar"
			ADD BUTTON oButton TITLE STR0042 ACTION 'VIEWDEF.APCFG220' OPERATION 4 OF oBrowse // "Alterar"
			ADD BUTTON oButton TITLE STR0046 ACTION "CRMA680RUS('SM0',xFilial('SM0')+ XX8->XX8_GRPEMP + XX8->XX8_EMPR + XX8->XX8_CODIGO + XX8->XX8_TIPO,.F.,"+"('"+STR0046 +"' + ' ' + XX8->XX8_DESCRI))" OPERATION 4 OF oBrowse // "Added button Address"
			
			//-------------------------------------------------------------------
			// Criação das colunas do Browse
			//-------------------------------------------------------------------
			ADD COLUMN oColumn DATA { || XX8->XX8_GRPEMP } TITLE STR0043 SIZE 4 OF oBrowse // "Grupo"
			...
			ADD COLUMN oColumn DATA { || XX8->XX8_EMPR } TITLE STR0038 SIZE 12 OF oBrowse // "Empresa" 
			//ADD COLUMN oColumn DATA { || FwSkCadFil(XX8->(XX8_GRPEMP+XX8_EMPR)+"1","XX8_DESCRI",2,{{'XX8_GRPEMP',XX8->XX8_GRPEMP},{'XX8_EMPR',XX8->XX8_EMPR}},'1') }  TITLE STR0044 SIZE 20 OF oBrowse 
	
			ADD COLUMN oColumn DATA { || XX8->XX8_CODIGO } TITLE STR0045 SIZE 4 OF oBrowse // "Código"
			ADD COLUMN oColumn DATA { || XX8->XX8_DESCRI } TITLE STR0044 SIZE 30 OF oBrowse // "Descrição"
...
Else
	oBrowse := FWLoadBrw( 'APCFG220' )
EndIf
SetKey(K_CTRL_A,{|| CRMA680RUS('SM0',xFilial('SM0')+ XX8->XX8_GRPEMP + XX8->XX8_EMPR + XX8->XX8_CODIGO + XX8->XX8_TIPO,.F.,STR0046 + ' ' + XX8->XX8_DESCRI)})
...
aRotina := FWLoadMenuDef( 'APCFG220' )
...
//Modelo
oModel := FWLoadModel( 'APCFG220' )

//Struct
oStrct := FWFormModelStruct():New()

//Table
oStrct:AddTable( C_ALIAS_RUS , { 'CO_COMPGRP' , 'CO_COMPEMP' , 'CO_COMPUNI' , 'CO_TIPO' } , 'Teste' , { || MPSysTblPrefix() + C_TABLE_RUS } )
...
//Relation
oModel:SetRelation( 'SIGAMAT_COMP_RUS' , { { 'CO_COMPGRP' , 'XX8->XX8_GRPEMP' } , { 'CO_COMPEMP' , 'XX8->XX8_EMPR' } , { 'CO_COMPUNI' , 'XX8->XX8_CODIGO' } , { 'CO_TIPO' , 'XX8->XX8_TIPO' } } , 'CO_COMPGRP+CO_COMPEMP+CO_COMPUNI+CO_TIPO' )

//SetKey(K_CTRL_A,{|| CRMA680RUS("SM0",xFilial("SM0") + XX8->XX8_GRPEMP + XX8->XX8_EMPR + XX8->XX8_CODIGO + XX8->XX8_TIPO)})
...
oView:AddUserButton(STR0046,'AddrButton', {|| CRMA680RUS('SM0',xFilial('SM0')+ XX8->XX8_GRPEMP + XX8->XX8_EMPR + XX8->XX8_CODIGO + XX8->XX8_TIPO,.F.,STR0046+ " " + XX8->XX8_DESCRI)}, /*[cToolTip]*/, K_CTRL_A) // Other Actions - address button in viewdef
