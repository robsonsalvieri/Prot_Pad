...
PERGUNTE "RU02R01DS" //will be RU04R02DS soon
...
DEFINE COLUMN H07H	TYPE CHARACTER 	SIZE 11		DECIMALS 0 LABEL "H07H" 		//7H
DEFINE COLUMN H08H	TYPE CHARACTER 	SIZE 290	DECIMALS 0 LABEL "H08H" 		//8H
...
static function X60NOT(cAliasMov,cMvpar01)
	...
	cQuery += " AND F42_REPORT IN ('MX-1', 'MX1', 'ALL') "
	...
	cQuery := " SELECT F1_EMISSAO, A2_NREDUZ, A2_END, A2_TEL, A2_FAX,"
	...
	While (cAliasTMP)->(!EOF())
		...
		If !empty(aRet[3]) .AND. !empty(aRet[3][1]) .AND. !empty(aRet[3][1][10])
			...
			+ '.'
		Endif
		H02H	:=alltrim((cAliasTMP)->A2_NREDUZ) + alltrim((cAliasTMP)->A2_END) + alltrim((cAliasTMP)->A2_TEL) + alltrim((cAliasTMP)->A2_FAX)
		H03H	:=aRet[1][12][2]
		H04H	:=(cAliasTMP)->F5Q_NUMBER
		H05H	:=StrTran(DTOC(Stod((cAliasTMP)->F5Q_EDATE)), "/", ".")
		H06H	:=(cAliasTMP)->F1_DOC
		H07H	:=StrTran(DTOC(Stod((cAliasTMP)->F1_EMISSAO)), "/", ".")
		H08H	:=alltrim((cAliasTMP)->A2_NREDUZ) + alltrim((cAliasTMP)->A2_END)
