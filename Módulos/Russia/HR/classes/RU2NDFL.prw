...
/*/
{Protheus.doc} RU2NDFL
    Class for generating a report 2-NDFL.
    ...
    @version 12.1.23
/*/
Class RU2NDFL From LongNameClass
...
{Protheus.doc} New()
    ...
    @since 2021/07/06
    @version 12.1.23
    @return RU2NDFL, Object, RU2NDFL instance.
    @example RU2NDFL():New("000013")
             RU2NDFL():New("000013", "2021", ::oHeader:cCorrectionNumber)
/*/
Method New(cPersonnelNumber, cReportYear, cCorrectionNumber, cReferenceNumber, aPeriods) Class RU2NDFL
...
/*/
Method GetHeaderInfo() Class RU2NDFL
...
{Protheus.doc} GetCompanyInfo(cTypeCompany, cFilCode)
    The method return data about selected company for the 2-NDFL report header.
    Get data from XX8 and CO (SIGACFG) (SYS_COMPANY_L_RUS).

    If cTypeCompany is Empty then using index 1 by default.
    If cFilCode not filled then aCompanyInfo arrya will fill empty character symbols by default.
    ...
    @example aCompanyInfo := o2NDFL:GetCompanyInfo(TYPE_COMPANY_CODE, FwXFilial("SRA"))
/*/
Method GetCompanyInfo(cTypeCompany, cFilCode) Class RU2NDFL
    Local aCompanyInfo    As Array
    Local aGetCoBrRusInfo As Array
    Local aArea           As Array
    Local nInfoIndex      As Numeric
    Local lIsFillFilial   As Logical

    lIsFillFilial := !Empty(cFilCode)
    aArea := GetArea()
    aCompanyInfo := {}
    aGetCoBrRusInfo := {}

    /* 
     * If we using 2-NDFL then get info with index 2. Used table SYS_COMPANY_L_RUS.
    */
    nInfoIndex := 2
    ...
    // Get information about company for report.
    // If the company type is not specified, then fill the array with empty values.
    If lIsFillFilial .And. ValType(nInfoIndex) == "N" .And. nInfoIndex >= COMPANY_INFO_ARRAY_MIN_LENGTH .And. nInfoIndex < COMPANY_INFO_ARRAY_MAX_LENGTH
        aAdd(aCompanyInfo, Alltrim(aGetCoBrRusInfo[CO_INDEX_COMPANY][COMPANY_CO_INN_INDEX][2]))// 1 - INN.
        aAdd(aCompanyInfo, Alltrim(aGetCoBrRusInfo[nInfoIndex][COMPANY_BR_KPP_INDEX][2]))      // 2 - KPP.
        aAdd(aCompanyInfo, Alltrim(aGetCoBrRusInfo[nInfoIndex][COMPANY_BR_FULLNAM_INDEX][2]))  // 3 - Full name.
        aAdd(aCompanyInfo, Alltrim(aGetCoBrRusInfo[nInfoIndex][COMPANY_BR_LOCLTAX_INDEX][2]))  // 4 - IFNS code.
        aAdd(aCompanyInfo, Alltrim(aGetCoBrRusInfo[nInfoIndex][COMPANY_BR_OKTMO_INDEX][2]))    // 5 - OKTMO.
        aAdd(aCompanyInfo, Alltrim(aGetCoBrRusInfo[nInfoIndex][COMPANY_BR_PHONENU_INDEX][2]))  // 6 - Phone number.
        aAdd(aCompanyInfo, Alltrim(aGetCoBrRusInfo[nInfoIndex][COMPANY_BR_SHORTNM_INDEX][2]))  // 7 - Short name.
    Else
        aCompanyInfo := Array(COMPANY_INFO_ARRAY_LENGTH)
        aFill(aCompanyInfo, "", 1, COMPANY_INFO_ARRAY_LENGTH)
    EndIf
...
/*/
Method GetEmployeeInfo(cFil, cPersonnelNumber) Class RU2NDFL
...
/*/
Method GetIncome(cFil, cPersonnelNumber, cReportYear) Class RU2NDFL
    Local oStatement As Object
    Local cQuery     As Character 
    Local aArea      As Array
    Local cTab       As Character
    ...
    cQuery += " WHERE "
    cQuery += " A.RD_FILIAL = ? " // '102030'
    cQuery += " AND A.RD_MAT = ? " // '000013'
    cQuery += " AND LEFT(A.RD_PERIODO,4) = ? " // '2021'
    ...
    oStatement:SetString(1, cFil)
    oStatement:SetString(2, cPersonnelNumber)
    oStatement:SetString(3, ::cReportYear)
...
/*/
Method GetTaxDeductionInfo(cFil, cPersonnelNumber, cReportYear) Class RU2NDFL
    Local oStatement   As Object
    Local cQuery       As Character 
    Local aArea        As Array
    Local cTab         As Character
    ...
    cQuery += " A.RD_FILIAL = ? "
    cQuery += " AND A.RD_MAT = ? "
    cQuery += " AND LEFT(A.RD_PERIODO,4) = ? "
    cQuery += " AND A.RD_CONVOC <> '      ' "
    ...
    While !(cTab)->(Eof())
        ...
        If (::lRate30 .And. !::lRate13 .And. !::lRate15)
            aAdd(::a30TaxPayments, { cRD_PERIODO, cRD_CONVOC, nRD_VALOR })
        ElseIf (::lRate13 .And. !::lRate15 .And. !::lRate30)
        ...
        ElseIf (!::lRate13 .And. ::lRate15 .And. !::lRate30)
            nPostion := Iif(Len(::a15TaxPayments) > 0, aScan(::a15TaxPayments, {|x| x[2] == cRD_CONVOC }), 0)

            If (nPostion > 0)
                ::a15TaxPayments[nPostion][3] += (cTab)->RD_VALOR
            Else
                aAdd(::a15TaxPayments, { cRD_PERIODO, cRD_CONVOC, nRD_VALOR })
            EndIf

        Else // ::lRate13 .And. ::lRate15
            If ((cTab)->RD_PERIODO < ::cDate15)
                nPostion := Iif(Len(::a13TaxPayments) > 0, aScan(::a13TaxPayments, {|x| x[2] == cRD_CONVOC }), 0)
            ...
            ElseIf ((cTab)->RD_PERIODO > ::cDate15)
                nPostion := Iif(Len(::a15TaxPayments) > 0, aScan(::a15TaxPayments, {|x| x[2] == cRD_CONVOC }), 0)
            ...
            ElseIf ((cTab)->RD_PERIODO == ::cDate15)
                nHalfPayment := nRD_VALOR / 2
                nPostion := Iif(Len(::a15TaxPayments) > 0, aScan(::a15TaxPayments, {|x| x[2] == cRD_CONVOC }), 0)

                If (nPostion > 0)
                    ::a15TaxPayments[nPostion][3] += nHalfPayment
                Else
                     // Field 4, value '1' - show what this tax is half of tax by 13%.
                    aAdd(::a15TaxPayments, { cRD_PERIODO, cRD_CONVOC, nHalfPayment, 1})
                EndIf

                nPostion := Iif(Len(::a13TaxPayments) > 0, aScan(::a13TaxPayments, {|x| x[2] == cRD_CONVOC }), 0)

                If (nPostion > 0)
                    ::a13TaxPayments[nPostion][3] += nHalfPayment
                Else
                    aAdd(::a13TaxPayments, { cRD_PERIODO, cRD_CONVOC, nHalfPayment, 1})
                EndIf
    ...
    oStatement:SetString(4, cReportYear)
    ...
    While !Eof()
        ...
        If (::lRate30 .And. !::lRate13 .And. !::lRate15)
            aAdd(::a30TaxInfo, { (cTab)->F5C_DEDCOD, (cTab)->F5C_TYPE, SToD((cTab)->F5C_DATAIN), AllTrim((cTab)->F5C_CARTOR), AllTrim((cTab)->F5C_LOCLTA) })
        ElseIf (::lRate13 .And. !::lRate15 .And. !::lRate30)
            aAdd(::a13TaxInfo, { (cTab)->F5C_DEDCOD, (cTab)->F5C_TYPE, SToD((cTab)->F5C_DATAIN), AllTrim((cTab)->F5C_CARTOR), AllTrim((cTab)->F5C_LOCLTA) })
        ElseIf (!::lRate13 .And. ::lRate15 .And. !::lRate30)
            aAdd(::a15TaxInfo, { (cTab)->F5C_DEDCOD, (cTab)->F5C_TYPE, SToD((cTab)->F5C_DATAIN), AllTrim((cTab)->F5C_CARTOR), AllTrim((cTab)->F5C_LOCLTA) })
        EndIf
...
/*/
Method GetAllSum(cFil, cPersonnelNumber, cReportYear) Class RU2NDFL
    Local oStatement As Object
    Local aTmp       As Array
    Local aTmp1      As Array
    Local aTmp2      As Array
    ...
    cQuery += " WHERE "
    cQuery += " RD_FILIAL = ? " //'102030'
    cQuery += " AND RD_MAT = ? " // '000013'
    cQuery += " AND LEFT(RD_PERIODO,4) = ? " // '2021'
    cQuery += " AND RD_PD IN (?) " // Payments 412 and 413.
    cQuery += " AND D_E_L_E_T_ = ' ' "
...
/*/
Method DefineNdflRates(aPeriod) Class RU2NDFL
    Local oStatement   As Object
    Local cQuery       As Character 
    Local aArea        As Array
    Local cTab         As Character
    ...
    cQuery += " AND LEFT(SRD.RD_PERIODO,4) = ? "
    ...
    oStatement:SetString(2, Self:cReportYear)
...
/*/
Method CollapseIncomeSum(aIncomePayments) Class RU2NDFL
    Local nI As Numeric
    Local nJ As Numeric
