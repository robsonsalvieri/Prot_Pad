...
/*/{Protheus.doc} AfterTTS(oModel, cID)
    This method needs for validation on delete operation in MATA020 
    If customer has already used in contracts (F5Q), delete operation not allowed

    @type method
    @param oModel   = Object with model
    @param cID = String with model id, example "SA2MASTER"
    @return lRet
...
*/
Method AfterTTS(oModel,cID) Class MATA020EVRUS
    
    Local nOperation    := oModel:GetOperation()
    
    If nOperation == MODEL_OPERATION_INSERT
        RU34XREP01("MATA020", .F.)
    EndIf
     
Return Nil 

/*/{Protheus.doc} ModelPreVld(oModel, cID)
    This method needs for validation on delete operation in MATA020 
    If customer has already used in contracts (F5Q), delete operation not allowed
...
*/
Method ModelPreVld(oModel,cID) Class MATA020EVRUS
    Local lValid    := .T.
    If oModel:GetOperation() == MODEL_OPERATION_DELETE
        lValid  := RU34XFUN08(STR0110, STR0111, 'MATA020', 'SA2')
    EndIf
Return (lValid)
//Merge Russia R14                   
