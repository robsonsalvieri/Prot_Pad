...
//data retrieval for report rows from NNT,SB1,SAH,SD2
	...
	Else
		nLenxFil:= Len(Alltrim(xFilial('SB1')))
		cQuery += " AND SB1.B1_FILIAL = LEFT(NNT.NNT_FILORI,"+Str(nLenxFil)+") "
	Endif
	cQuery += "INNER JOIN " + RetSqlName("SAH") + " SAH ON NNT.NNT_UM = SAH.AH_UNIMED AND SAH.D_E_L_E_T_ = '' "
	If Empty(xFilial('SAH'))
		cQuery += " AND SAH.AH_FILIAL = '"+xFilial('SAH')+"' "
	Else
		nLenxFil:= Len(Alltrim(xFilial('SAH')))
		cQuery += " AND SAH.AH_FILIAL = LEFT(NNT.NNT_FILORI,"+Str(nLenxFil)+") "
	Endif
	cQuery += "LEFT JOIN " + RetSqlName("SD2") + " SD2 ON NNT.NNT_DOC = SD2.D2_DOC AND SD2.D_E_L_E_T_ = '' "
	cQuery += " AND SD2.D2_SERIE = NNT_SERIE "
	If Empty(xFilial('SD2'))
		cQuery += " AND SD2.D2_FILIAL = '"+xFilial('SD2')+"' "
	Else
		nLenxFil:= Len(Alltrim(xFilial('SD2')))
		cQuery += " AND SD2.D2_FILIAL =  LEFT(NNT.NNT_FILORI,"+Str(nLenxFil)+") "
	Endif
...
local aSomeone := {}
	...
	cQuery +=  " AND  '" + DTOS(dDatabase) +  "' BETWEEN '''F42.F42_DFROM''' AND F42.F42_DATETO "
...
//Signers name, surname and initials
