...
//-------------------------------------------------------------------
/*/{Protheus.doc} JURR132A(cUser, cThread, lAndam)
...
/*/
//-------------------------------------------------------------------
Function JURR132A(cCodEspec,cDesEspec)
...
JRelatorio(aRelat,aCabec,aSessao,J132AQrPrin(cCodEspec),cCodEspec,cDesEspec) //Chamada da função de impressão do relatório em TMSPrinter
...
If (TMP)->(!EOF())
	...
	ImpCabec(@oPrint, @nLin, aRelat, aCabec,cCodEspec,cDesEspec) // Imprime cabeçalho
	...
	While (TMP)->(!EOF())
		...
		If nLin >= nFimL // Verifica se a linha corrente é maior que linha final permitida por página
			...
			ImpCabec(@oPrint, @nLin, aRelat, aCabec,cCodEspec,cDesEspec) // Cria um novo cabeçalho
			nLinCalc := nLin // Inicia o controle das linhas impressas
