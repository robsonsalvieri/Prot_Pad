		...
		While cAnoMesOHH <= cAnoMesAtu
			...
			For nFat := 1 To Len(aValorFat) // Atualiza a posição histórica para cada fatura vinculada ao título
				...
				If nSaldo > 0
					...
					If lExistOIB .And. lX2UnicoOIB .And. lInclui .And. !Empty(cEscrit) .And. !Empty(cFatura)
						J255VerImp(cEscrit, cFatura, cAnoMesOHH)
					EndIF
...
/*/
//-------------------------------------------------------------------
Static Function J255VerImp(cEscrit, cFatura, cAnoMes)
Local aInfoImp    := {}
Local aImpostos   := {}
Local aImpRetidos := {}
...
Local lFNBusFK4   := FindFunction("JBusFK4Imp") // @12.1.2510
    ...
    For nLoop := 1 To Len(aImpPadrao)
		...
		If nPosImp == 0
			J255GetVlImp(aImpPadrao[nLoop], @nValImp, @nVlBaseImp)
			If nValImp > 0
				aAdd(aImpostos, {;
					aImpPadrao[nLoop],;  // Código
					Iif(Empty(J255SEDAlq(SE1->E1_NATUREZ, aImpPadrao[nLoop])), 0, J255SEDAlq(SE1->E1_NATUREZ, aImpPadrao[nLoop])),;  // Alíquota
					nValImp,;         // Valor
					nVlBaseImp;       // Base de cálculo
				})
			EndIf
...
@param nVlBaseImp Valor base do imposto

@author José Antonio de Oliveira / Abner Fogaça de Oliveira
@since  06/08/2025
/*/
//-------------------------------------------------------------------
Static Function J255GetVlImp(cCampo, nValImp, nVlBaseImp)
    ...
    Do Case
		...
		Case cCampo == "PIS"
			nValImp    := SE1->E1_PIS
			nVlBaseImp := SE1->E1_BASEPIS
		Case cCampo == "COF"
			nValImp    := SE1->E1_COFINS
			nVlBaseImp := SE1->E1_BASECOF
		Case cCampo == "CSL"
			nValImp    := SE1->E1_CSLL
			nVlBaseImp := SE1->E1_BASECSL 
    EndCase
