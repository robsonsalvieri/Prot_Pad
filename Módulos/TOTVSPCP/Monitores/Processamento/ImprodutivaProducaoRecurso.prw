...
Method BuscaDados(oFiltros, cTipo, cSubTipo) Class ImprodutivaProducaoRecurso
    ...
    nHrParada  := PCPMonitorUtils():TransformaMinutosCentesimaisParaTempo(oJsonCalc[dToC(dDatabase)]["I"],2)
    ...
    oGauge:SetLabel(STR0204) //"% Improdutivo"
    oGauge:SetAppend("")
    oGauge:SetThick(20)
    oGauge:SetMargin(15)
    ...
    PCPMonitorUtils():AdicionaTagMonitor(oJsonRet["tags"],@nPosTag,"po-icon-star",STR0106+": "+cValToChar(nHrParada)+" "+STR0054 ) //"Horas Improdutivas" //" horas"
...
Method CargaMonitor() Class ImprodutivaProducaoRecurso
    ...
    If !PCPMonitorCarga():monitorAtualizado("ImprodutivaProducaoRecurso")
        ...
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nPosTag,"po-icon-calendar","01/05/2023 - 31/05/2023")
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nPosTag,"po-icon-manufacture","INJ001 - Injetora 001")
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nPosTag,"po-icon-star",STR0106+": 20 "+STR0054 ) //"Horas Improdutivas" //" horas"
        PCPMonitorUtils():AdicionaTagMonitor(aTags,@nPosTag,"po-icon-star-half",STR0105+": "+"200 "+STR0054) //"Horas Produtivas" //" horas"
        ...
        oGauge:SetLabelStyle("font-weight","bold")
        oGauge:SetThreshold("0",COR_VERDE_FORTE,0.2)
        oGauge:SetThreshold("10",COR_AMARELO_QUEIMADO,0.2)
        oGauge:SetThreshold("25",COR_VERMELHO_FORTE,0.2)
        oGauge:SetMarker("0",COR_PRETO,6,"line")
        oGauge:SetMarker("10",COR_PRETO,6,"line")
        oGauge:SetMarker("25",COR_PRETO,6,"line")
        oGauge:SetMarker("100",COR_PRETO,6,"line")
  
        oCarga:setaTitulo(STR0056) //"Horas Improdutivas X Produção"
        oCarga:setaObjetivo(STR0057) //"Acompanhar o percentual de horas improdutivas em relação às horas produtivas apontadas no recurso no período estabelecido."
        ...
        oCarga:setaPropriedade("03_NIVEIS",STR0205,STR0060,1,30,0,"po-lg-6 po-xl-6 po-md-6 po-sm-12") //"Atenção;Urgente" //"Níveis (% Improd. X Produção)"
...
Method ValidaPropriedades(oFiltros) Class ImprodutivaProducaoRecurso
    ...
    If aRetorno[1] 
        If !oFiltros:HasProperty("03_NIVEIS") .Or. oFiltros["03_NIVEIS"] == Nil .Or. Empty(oFiltros["03_NIVEIS"])
            aRetorno[1] := .F.
            aRetorno[2] := STR0206 //"O campo 'Níveis' deve ser preenchido com 2 valores separados por ponto e vírgula (ex.: '10;25'), definindo o percentual de horas improdutivas em relação às horas produtivas que indicam Atenção e Urgência, respectivamente"
        Else 
            aNiveis := STRTOKARR(Replace(oFiltros["03_NIVEIS"],",","."), ";" )
            If Len(aNiveis) <> 2
                aRetorno[1] := .F.
            ...
            Else
                If Val(aNiveis[1]) >= Val(aNiveis[2])
                    aRetorno[1] := .F.
                    aRetorno[2] := STR0207 //"No campo 'Níveis', o primeiro valor, referente ao status 'Atenção', deve ser menor que o segundo, que representa 'Urgência'"
                EndIf
            EndIf
        EndIf
    EndIf
