...
Function PVGrupo(aGrupo,nGrupo,oBrwProd,nTop,aItePed,lFoco,lCodigo,aObj)
...
if !HB1->(Eof())
	nTop := HB1->(Recno())
	PVListarProd(cUltGrupo,nLinhasMax,oBrwProd,nTop,aItePed,lFoco,aObj)
Else
	Alert(STR0020 + cUltGrupo) //"Não há produtos neste grupo: " ####
	aSize(aProduto,0)
...
Function PVSobe(aGrupo,nGrupo,oBrwProd,nTop,aItePed,lCodigo,aObj)
//Local cGrupo := Substr(aGrupo[nGrupo],1,4)
Local nLinhasMax := GridRows(oBrwProd)
Local nOrder := if(lCodigo,1,2)
...
if ( !HB1->(Bof()) .And. ( Empty(cUltGrupo) .Or. HB1->HB1_GRUPO == cUltGrupo) )
	...
	If Empty(cUltGrupo)
		HB1->( dbSetOrder(nOrder) )
		HB1->(dbSeek(RetFilial("HB1")))//posiciona no 1o. produto
		//HB1->( dbGotop() ) 
	Else 
		HB1->( dbSetOrder(3) )
...
//Listar produtos
PVListarProd(cUltGrupo,nLinhasMax,oBrwProd,nTop,aItePed,.t.,aObj)

Return nil
          

//Paginar arquivo p/ baixo
Function PVDesce(aGrupo,nGrupo,oBrwProd,nTop,aItePed,lCodigo,aObj)
//Local cGrupo := Substr(aGrupo[nGrupo],1,4)
Local nLinhasMax := GridRows(oBrwProd)
Local nOrder := if(lCodigo,1,2)
...
Function PVListarProd(cUltGrupo,nLinhasMax,oBrwProd,nTop,aItePed,lFoco,aObj)
Local i        := 0
Local nPos     := 0                                             
Local nPosProd := 0
Local cPrdDesc := ""
...
// Parametro para opcao de exibir descricao dos produtos em letras minusculas
dbSelectArea("HCF")
dbSetOrder(1)
If dbSeek(RetFilial("HCF") + "MV_SFBRMIN")
	cSfBrMin := AllTrim(HCF->HCF_VALOR)
EndIf
aSize(aProduto,0)
//For i := 1 to nLinhasMax
For i := 1 to nPagProd
   if !HB1->(Eof()) .And. ((AllTrim(HB1->HB1_GRUPO) == AllTrim(cUltGrupo)) .Or. Empty(cUltGrupo))
  		If ExistBlock("SFAPV100")
			lUseProd := ExecBlock("SFAPV100", .F., .F., {HB1->HB1_COD, aObj, aItePed})
			If lUseProd = .F.
				HB1->(dbSkip())
				i := i - 1 // Volta o contador para nao ficar em branco browse de produtos 
				Loop
			EndIf
		EndIf 
      nPos := ScanArray(aItePed,HB1->HB1_COD,,,1)
	  nPosProd := ScanArray(aProduto,HB1->HB1_COD,,,2)
	  If cSfBrMin = "1"
		cPrdDesc := Alltrim(HB1->HB1_DESC)
	  Else
		cPrdDesc := Lower(Alltrim(HB1->HB1_DESC))				
	  EndIf
	  If nPos > 0 //Carrega dados do item localizado no aItePed
		If nPosProd = 0
			AADD(aProduto,{cPrdDesc,HB1->HB1_COD,aItePed[nPos,4],aItePed[nPos,6],aItePed[nPos,7],aItePed[nPos,9]})	
		EndIf
	  Else
		If nPosProd = 0
		  AADD(aProduto,{cPrdDesc,HB1->HB1_COD,0,0,0,0})
		EndIf
	  Endif
		If ExistBlock("SFAPV101")
			ExecBlock("SFAPV101", .F., .F., {aProduto, aObj})
		EndIf
   else
	  break
   endif
...
aSize(aGrupo,0)
AADD(aGrupo, STR0021) //1o. item (padrao)
LoadGrupos(aGrupo)

// Consulta Condicao de Pagamento 
Aadd(aCmpPag,{STR0002,HE4->(FieldPos("HE4_CODIGO")),30}) //"Código"
Aadd(aCmpPag,{STR0003,HE4->(FieldPos("HE4_DESCRI")),70}) //"Descrição"
Aadd(aIndPag,{STR0002,1}) //"Código"

//Consulta Tabela de Preco
Aadd(aCmpTab,{STR0002,HTC->(FieldPos("HTC_TAB")),30}) //"Código"
Aadd(aCmpTab,{STR0003,HTC->(FieldPos("HTC_DESCRI")),100}) //"Descrição"
Aadd(aIndTab,{STR0002,1}) //"Código"

//Consulta Transportadora
Aadd(aCmpTra,{STR0002,HA4->(FieldPos("HA4_COD")),40}) //"Código"
Aadd(aCmpTra,{STR0004,HA4->(FieldPos("HA4_NOME")),100}) //"Nome"
Aadd(aIndTra,{STR0002,1}) //"Código"

//Consulta Forma de Pagamento
Aadd(aCmpFpg,{STR0002,HTP->(FieldPos("HTP_CHAVE")),20}) //"Código"
Aadd(aCmpFpg,{STR0005,HTP->(FieldPos("HTP_DESCRI")),60}) //"Descricao"
Aadd(aIndFpg,{STR0002,1}) //"Código"

//Consulta TES
Aadd(aCmpTes,{STR0002,HF4->(FieldPos("HF4_CODIGO")),30}) //"Código"
Aadd(aCmpTes,{STR0003,HF4->(FieldPos("HF4_TEXTO")),100}) //"Descrição"
...
@ 105,115 GET oCtrl VAR nDescMax READONLY NO UNDERLINE SIZE 40,10 OF oDet
...
ADD COLUMN oCol TO oBrw ARRAY ELEMENT 1 HEADER STR0016 WIDTH 35 //"Tabela"
ADD COLUMN oCol TO oBrw ARRAY ELEMENT 2 HEADER STR0017 WIDTH 32 PICTURE "@E 999,999.99" ALIGN RIGHT //"Valor"
ADD COLUMN oCol TO oBrw ARRAY ELEMENT 3 HEADER STR0014 WIDTH 33 PICTURE "@E 999.99" ALIGN RIGHT //"Desc. Max."
...
SetText(aControls[2,2],HB1->HB1_PRV1)       
...
If HPR->(FieldPos("HPR_DESMAX")) != 0
	lexistecpo := .t.
Endif
While (!HPR->(Eof()) .and. HPR->HPR_PROD == cCod)
...
Function PVSetDetalhes(oBrwProd,aControls,aCabPed,cCod,cDescD,cUN,nQTD,nEnt,nICM,nIPI,cEst,nDescMax)
...
If cNewCod <> cCod
	...
	SetText(aControls[1,15],Str(HB2->HB2_QTD,5,2) + " em " + DtoC(HB2->HB2_DATA)) //Estoque

	// Posiciona produto na tabela
	HPR->(dbSetOrder(1))
	HPR->(dbSeek(RetFilial("HPR") + cCod + aCabPed[8,1]))
...
Function PVFind(cPesq,lCodigo,aGrupo,nGrupo,aPrdPrefix,oBrwProd,aItePed,nTop, oFld, oDlg, aObj)
Local nOrder := if(lCodigo,1,2)
Local cPrefixo := ""
cPesq := Upper(cPesq)    
...
If HB1->(Found()) 
	...
	cUltGrupo:=""
	PVListarProd(cUltGrupo,GridRows(oBrwProd),oBrwProd,@nTop,aItePed,.t.,aObj)
	If oDlg != Nil .And. oFld != Nil
		SET ACTIVE FOLDER oFld OF oDlg
	EndIf
Else
...
Function PvKeyMove(nMove, aObj, aGrupo, nLinhasMax,oBrw,nTop,aItePed,lFoco,lCodigo)
...
Else // Down
	nRow := nRow + 1
EndIf
If nRow > Len(aProduto)
	If Len(aProduto) > nRows
		PVDesce(aGrupo,nGrupo,oBrw,nTop,aItePed,lCodigo,aObj)
	EndIf
ElseIf nRow = 0
	PVSobe(aGrupo,nGrupo,oBrw,nTop,aItePed,lCodigo,aObj)
Else
