...
While !Eof() .And. HU5->HU5_CLIENT == cCodCli .And. HU5->HU5_LOJA == cLojaCli
	...
	dbSelectArea("HX5")
	dbSetOrder(1)
	dbSeek(RetFilial("HX5") + "UM" + HU5->HU5_FUNCAO)
	If !Eof()
		cFuncao	:=HX5->HX5_DESCRI
	Endif      
    
	AADD(aContato,{Alltrim(HU5->HU5_CODCON),AllTrim(HU5->HU5_CONTAT),cFuncao})
...
if lAt
	...
	if HU5->(Found())
	    ...
	    If nOpCon==1 
			AADD(aContato,{Alltrim(HU5->HU5_CODCON),AllTrim(HU5->HU5_CONTAT),cFuncao})
		Else
			aContato[nLinha,2] := AllTrim(HU5->HU5_CONTAT)
