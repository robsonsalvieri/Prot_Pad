...
Function RGAplDescIte(cCliente, cLoja, cCond, cTab, cFormPg, aItePed, nItePed, aColIte, nQtdAnt)
Local I:=0
Local lDesc := .T.
	...
	If ((HCO->ACO_CODCLI = cCliente .Or. Empty(HCO->ACO_CODCLI) ).And.;
		...
		lDesc := .F.
  		While !Eof() .And. HCP->ACP_CODREG = HCO->ACO_CODREG .And. !lDesc
			...
			// -----------------------------------------------------------------------
			Else
				If (HCP->ACP_CODPRO = aItePed[nItePed,1] .Or.	HCP->ACP_GRUPO = aItePed[nItePed,3])
					
					//Aplica a regra de descto quando:
					//for inclusao de um item novo ou 
					//quando for alteracao e a qtde for diferente da anterior
					If aColIte[7,1] == 0
						If ( (nQtdAnt == 0) .Or. ( nQtdAnt > 0 .And. nQtdAnt != aItePed[nItePed,4]) )
							aItePed[nItePed,7] := 0                                                   
							aColIte[7,1] := 0
							
							If ( HCP->ACP_FAIXA >= aItePed[nItePed,4] .And. Empty(aItePed[nItePed,10]) )
