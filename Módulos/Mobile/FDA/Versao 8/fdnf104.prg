...
If dbSeek(RetFilial("HA1")+cCodCli + cLojaCli)
	If HA1->HA1_FLGVIS = "2"  //.OR. HA1->HA1_FLGVIS = "4"
...
Else	// Alteracao da N.F.
	cNovaNF:=aCabNot[1,1]
 	HF2->( dbSetOrder(1) )
 	HF2->( dbSeek(RetFilial("HF2")+cNovaNF+cSerie) )
 	If HF2->(Found())
        ...
        Next
   		...
   		aCabNot[35,1] := Round(HF2->HF2_VALBRUT,2)
...
If aCabNot[2,1] == 1 .Or. aCabNot[2,1] == 4
	dbSelectArea("HF2")
	dbSetOrder(1)
	dbSeek(RetFilial("HF2")+aCabNot[1,1]) 
	If HF2->(Found())
		AADD(aNotas,{aCabNot[1,1],HF2->HF2_EMISSAO,"Nao transmitida" }) 
	...
	If Len(aNotas)>0 
		dbSelectArea("HA1")
		dbSetOrder(1)
		If dbSeek(RetFilial("HA1")+aCabNot[4,1]+aCabNot[5,1])
			HA1->HA1_FLGVIS := "4"
    		dbCommit()
		Endif
		If Empty(aCabNot[5,1])
			dbSelectArea("HD7")
			dbSetOrder(3)
			If dbSeek(RetFilial("HD7")+DtoS(Date()) + aCabNot[4,1]+aCabNot[5,1])
				HD7->HAD7_FLGVIS := "4"
	    		dbCommit()
			Endif
		Else
			dbSelectArea("HD7")
			dbSetOrder(1)
			If dbSeek(RetFilial("HD7")+aCabNot[4,1]+aCabNot[5,1])
				HD7->HAD7_FLGVIS := "4"
	    		dbCommit()
			Endif
		Endif		
...
dbSelectArea("HD2")     
dbSetOrder(1)
If dbSeek(RetFilial("HD2")+cNumNot)
	aSize(aIteNot,0)
...
HF2->( dbSeek( RetFilial("HF2")+cCodCli+cLojaCli,.f. ) )

While !Eof() .And. HF2->HF2_FILIAL == RetFilial("HF2") .and. HF2->HF2_CLIENTE == cCodCli .and. HF2->HF2_LOJA == cLojaCli
	cStnota:=""
	cNumNot:=""
	cNumNot:=HF2->HF2_DOC
	...
	if HF2->HF2_STATUS = "N"      
       ...
       //-> aNotas: Array do Mod. de Notas
    	...
    	AADD(aNotas,{cNumNot,HF2->HF2_EMISSAO,cStatus}) 
