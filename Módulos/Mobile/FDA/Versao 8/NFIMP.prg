...
HA1->( dbSeek(RetFilial("HA1")+HF2->F2_CLIENTE+HF2->F2_LOJA) )	//Cliente
HA3->( dbSeek(RetFilial("HA3")+HF2->F2_VEND1) )				//Vendedor
HE4->( dbSeek(RetFilial("HE4")+HF2->F2_COND) )				//Condicao de Pagto
HA4->( dbSeek(RetFilial("HA4")+HF2->F2_TRANSP) )			//Transportadoras
...
dbSelectArea("HD2")
dbSetOrder(1)
dbSeek(RetFilial("HD2")+cNumNota)   
While HD2->(!Eof()) .And. HD2->HD2_FILIAL == RetFilial("HD2") .And. HD2->D2_DOC == cNumNota
	  HB1->( dbSeek(RetFilial("HB1")+HD2->D2_COD) )
