...
If dbSeek(RetFilial("HIN")+cCodCli + cLoja)
	While !Eof() .And. HIN->HIN_FILIAL == RetFilial("HIN") .And. cCodCli = HIN->IN_CLI  .And. cLoja = HIN->IN_LOJA
...
For i := 1 To Len(aInv)
	...
	If aInv[i,5] = 1
		dbSelectArea("HIN")
		dbSetOrder(1)
		If !dbSeek(RetFilial("HIN")+HB1->B1_GRUPO + HB1->B1_COD + aInv[i,1] + aInv[i,2])
			dbAppend()
		EndIf
...
ElseIf nOp = 2	// Exclusão de item
	...
	dbSelectArea("HIN")
	dbSetOrder(1)
	If dbSeek(RetFilial("HIN")+HB1->B1_GRUPO + HB1->B1_COD + aInv[nPos,1] + aInv[nPos,2])
		dbDelete()
		dbSkip()
	EndIf
...
Else
	...
	While HC5->(!Eof()) .And. HC5->HC5_FILIAL == RetFilial("HC5") .And. HC5->C5_CLI = cCodCli .And. HC5->C5_LOJA = cLojaCli
		If HC5->C5_STATUS = "N" .And. HC5->(IsDirty())
			AADD(aPedido,HC5->C5_NUM)
			AADD(aVlrPed,HC5->C5_VALOR)
		EndIf	
...
While !HE1->(Eof()) .And. HE1->HE1_FILIAL == RetFilial("HE1") .and. (HE1->E1_CLIENTE == cCodCli .and. HE1->E1_LOJA == cLojaCli)
