...
For i := 1 to nPagProd
   if !HB1->(Eof()) .And. ((AllTrim(HB1->HB1_GRUPO) == Alltrim(cUltGrupo)) .Or. Empty(cUltGrupo))
	  ...
	  If nPos > 0 //Carrega dados do item localizado no aItePed
		If nPosProd = 0
		  AADD(aProduto,{HB1->HB1_DESC,HB1->HB1_COD,aItePed[nPos,4],aItePed[nPos,6],aItePed[nPos,7],aItePed[nPos,9]})	
		EndIf
	  Else
		If nPosProd = 0
		  AADD(aProduto,{HB1->HB1_DESC,HB1->HB1_COD,0,0,0,0})
		EndIf
	  Endif
   else
	  break
...
aSize(aPrecos,0)
HPR->(dbSeek(cCod))
While (!HPR->(Eof()) .And. HPR->HPR_FILIAL == RetFilial("HPR") .and. HPR->HPR_PROD == cCod)
