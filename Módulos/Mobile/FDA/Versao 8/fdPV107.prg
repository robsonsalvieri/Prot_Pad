...
Function PVPrepPed(nOperacao,oBrwPedido,aPedido,oBrwUltPed,aUltPed,cNumPed,cCodCli, cLojaCli, cCodRot, cIteRot, aClientes, nCliente, oCliente)
//Variaveis Locais
...
AADD(aCabPed,{space(Len(HC5->HC5_MENNOTA)), HC5->(FieldPos("C5_MENNOTA"))	})
AADD(aCabPed,{Date(),						0								})
AADD(aCabPed,{0.00,		  					0								})  
AADD(aCabPed,{0.00,		  					0								})
AADD(aCabPed,{space(Len(HC5->HC5_TRANSP)),	HC5->(FieldPos("C5_TRANSP"))	})
...
If aCabPed[2,1] == 1 .Or. aCabPed[2,1] == 4
	...
	If HC5->(Found())
		AADD(aPedido,{aCabPed[1,1],HC5->HC5_EMISS,HC5->HC5_COND}) 
	...
	If Len(aPedido)>0 
		...
		Else
			dbSelectArea("HD7")
			dbSetOrder(1)
			If dbSeek(RetFilial("HD7")+aCabPed[3,1]+aCabPed[4,1])
				HD7->HD7_FLGVIS := "1"
	    		dbCommit()
			Endif
		Endif		
