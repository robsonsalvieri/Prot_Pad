...
Else
	...
	dbSetOrder(1)
//	dbSeek(cNomeCli)
	dbSeek(RetFilial("HA1")+aCliente[nCliente,1]+aCliente[nCliente,2])	
	...
	If HA1->(Found())
		...
		cTipo		:= HA1->HA1_TIPO
		cRazao 		:= HA1->HA1_NOME
		cFantasia 	:= HA1->HA1_NREDUZ
		cEndereco 	:= HA1->HA1_END
		cBairro		:= HA1->HA1_BAIRRO
		cCep 		:= HA1->HA1_CEP
		cCidade 	:= HA1->HA1_MUN
		cUF 		:= HA1->HA1_EST
		cTel 		:= HA1->HA1_TEL
...
While !Eof() .And. HX5->HX5_FILIAL == RetFilial("HX5") .And. HX5->HX5_TABELA == "TC" 
