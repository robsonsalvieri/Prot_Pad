...
for nI:=1 to Len(aItems) 
    ...
    If aItems[nI,1]==.t.          
        ...
        cChaveD1 := RetFilial("HD1")+Alltrim( aItems[nI,9] + aItems[nI,2] + cItemOri+"  " + aItems[nI,4] )
        cChaveF1 := RetFilial("HF1")+aItems[nI,9] + cSerie   
        
		if HD1->( dbSeek( cChaveD1 ) )   
	       nQtdEstorno:= HD1->HD1_QUANT 
	       HD1->( dbDelete() ) 
		   HD1->( dbCommit() )	
		   //Devolve a quantidade estornada ao HB6
		   dbSelectArea( "HB6" ) 
		   HB6->( dbGoto(aItems[nI,10]) ) // Usa o Recno do registro para fazer atualizacao 
	       //HB6->HB2_QTD    := HB6->HB2_QTD + nQtdEstorno
           HB6->HB6_STATUS := " "  // Marca " " para devolucao                          
	       HB6->( dbCommit() )	
	       nQtdel := nQtdel + 1 
 	       aItems[nI,8] := " "    
           aItems[nI,9] := " "
