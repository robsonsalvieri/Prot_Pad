...
//Calcula IPI
If cCalcIPI="S"
   aColItenf[9,1]:= aColItenf[8,1] * ( HB1->B1_IPI/100)
   aCabNot[15,1] := aCabNot[15,1] + aColItenf[9,1]          
Endif           
//Calcula ICMS
if cCalcICM="S"
   aColItenf[10,1]:= aColItenf[8,1] * ( HB1->B1_PICM/100)
...
If aCabNot[2,1] == 1 .Or. aCabNot[2,1] == 4
	...
	HF2->F2_DOC     := aCabNot[1,1]          
	HF2->F2_SERIE   := cSerie               
	HF2->F2_DUPL    := aCabNot[1,1]          
	HF2->F2_VEND1   := HA3->A3_COD
	HF2->F2_VALBRUT := aCabNot[15,1]        //Total
	HF2->F2_QTDITE  := len(aIteNot)
	HF2->F2_STATUS  := "N"
	HF2->F2_EST     := HA1->A1_EST
    HF2->F2_TIPOCLI := HA1->A1_TIPO
    	
	HF2->(dbCommit())
Else
	HF2->(dbSeek(aCabNot[1,1]) )
	if HF2->(Found())
		For nI:=1 to Len(aCabNot)
			If aCabNot[nI,2] <> 0  // Se o FIELDPOS for Diferente de Zero    
				HF2->(FieldPut(HF2->(aCabNot[nI,2]), aCabNot[nI,1]))
	...
	dbSetOrder(1)
	dbSeek(aCabNot[1,1])
	While !Eof() .And. HD2->D2_DOC = aCabNot[1,1]
...
For nCont:=1 to len(aIteNot)
	...
	HD2->D2_DOC     := aCabNot[1,1]
	//HD2->D2_DUPL    := aCabNot[1,1]
	HD2->D2_SERIE   := cSerie
	HD2->D2_CLIENTE := aCabNot[4,1]
	HD2->D2_LOJA    := aCabNot[5,1]
	HD2->D2_EST     := HA1->A1_EST
	HD2->D2_EMISSAO := HF2->F2_EMISSAO
	HD2->D2_ITEM    := StrZero(nConT,3)
	HD2->D2_ENTREG  := aCabNot[9,1]
	...
	if HB1->(Found())   
		HD2->D2_UM      := HB1->B1_UM
		HD2->D2_LOCAL   := HB1->B1_LOCPAD
		//HD2->D2_PICMS := HB1->B1_PICM // nao sei porque nao da erro mas tb nao aborta. perguntar para Manoel
		HD2->D2_PICM    := HB1->B1_PICM
	    HD2->D2_IPI     := HB1->B1_IPI
	Endif
...
GrvAtend(5, aCabNot[1,1],, HF2->F2_CLIENTE, HF2->F2_LOJA,)
...
if lPesq
	dbSelectArea("HB1")
	dbSetOrder(1)
	dbSeek(aColIteNf[3,1])
	if Found()
		aColIteNf[1,1]:= HB1->B1_COD
		lEncontrou	 := .T.
	Endif
Else
...
If lEncontrou
	If !Empty(aCabNot[7,1])
		dbSelectArea("HPR")
		dbSetOrder(1)
		dbSeek(aColIteNf[1,1]+aCabNot[7,1])
		If HPR->(Found()) //!Eof()
			aColIteNf[23,1]:=HPR->PR_UNI
		else
			If HB1->B1_PRV1 <> 0
				aColIteNf[23,1]:=HB1->B1_PRV1
			Else
	...
	aColIteNf[43,1]:=HB1->B1_DESC
	aColIteNf[20,1]:=HB1->B1_GRUPO
...
dbSelectArea("HF4")
dbSetorder(1)     
dbSeek( aColiteNF[11,1] )

cCalcIPI:=HF4->F4_IPI  
cCalcICM:=HF4->F4_ICM
cCF     :=HF4->F4_CF
cGeraDup:=HF4->F4_DUPLIC    
...
If dbSeek(cNumNot)
    ...
    dbSetOrder(1)
    dbGoTop()
    dbSeek(cCodCli+cLojaCli+cNumNot)

    While !Eof() .And.(HE1->E1_NUM=cNumNot .And. HE1->E1_CLIENTE=cCodCli .And. HE1->E1_LOJA=cLojaCLi )
		HE1->(dbDelete())	
		dbSkip()			
    EndDo
	...
	While !Eof() .And. HD2->D2_DOC = cNumNot
		// Estornar a qtde dos produtos da NF ao estoque (HB6)
		cProd := HD2->D2_COD
		nQtdEstorna := HD2->D2_QUANT
	...
	GrvAtend(6, cNumNot, , HF2->F2_CLIENTE, HF2->F2_LOJA, dData)
...
While !Eof() .And. HB6->B6_COD == codigo
	if HB6->B6_QTD > 0
		nQtdeAbat:= HD2->D2_QUANT
		If nQtdeAbat <= HB6->B6_QTD
			HB6->B6_QTD := HB6->B6_QTD - nQtdeAbat
			HB6->(dbCommit())
			Exit
		Endif
	endif   
...
While !HB6->(Eof()) .And. HB6->B6_COD == cProd .And. nQtd > 0
	nQtdAbat := HB6->B6_ORI - HB6->B6_QTD

	If nQtd <= nQtdAbat //restaura a qtde em uma unica nota
		HB6->B6_QTD := HB6->B6_QTD + nQtd
		nQtd := 0
	Else 				//restaura a qtde em mais de uma nota
		HB6->B6_QTD := HB6->B6_QTD + nQtdAbat
		nQtd := nQtd - nQtdAbat
	Endif
	HB6->(dbCommit())			   
	HB6->(dbSkip())
