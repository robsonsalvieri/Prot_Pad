...
For i := 1 to nPagProd
   if !HB1->(Eof()) .And. ((HB1->B1_GRUPO == cUltGrupo) .Or. Empty(cUltGrupo))
      nPos := ScanArray(aIteNot,HB1->B1_COD,,,3)
	  If nPos > 0 //Carrega dados do item localizado no aIteNot
		  AADD(aProduto,{HB1->B1_DESC,HB1->B1_COD,aIteNot[nPos,6],aIteNot[nPos,23],aIteNot[nPos,13],aIteNot[nPos,10],aIteNot[nPos,9],aIteNot[nPos,8]})	
	  Else
		  AADD(aProduto,{HB1->B1_DESC,HB1->B1_COD,0,0,0,0,0,0})
	  Endif
   else
	  break
   endif
