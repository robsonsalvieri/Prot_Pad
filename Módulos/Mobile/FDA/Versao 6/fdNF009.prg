...
HB6->( dbSetOrder(1))
if HB6->(dbSeek(aProduto[nLin,2]))
   //nSaldoEst:=HB6->B6_QTD
   //Procura o Proximo Produto que tem quantidade
   While HB6->(!eof()) .And. HB6->B6_COD == aProduto[nLin,2].And. HB6->(!IsDirty())
         if HB6->B6_QTD > 0   
            nRecEst := HB6->(Recno())
            nSaldoEst := nSaldoEst + HB6->B6_QTD
            //Exit                  
         endif           
         HB6->(dbSkip())
   enddo
...
If !Empty(aCabNot[7,1])
	dbSelectArea("HPR")
	dbSetOrder(1)
	dbSeek(aColitenf[3,1]+aCabNot[7,1])
	If HPR->(Found())
...
Else //inicia o campo TES com a 1a.
	HF4->(dbGotop())
	aColitenf[11,1] := Alltrim(HF4->F4_CODIGO)
	//SetText(aObj[3,9],aColitenf[11,1])
Endif
...
For ni:= 1 to nParcelas
    ...
    if !dbSeek( cCodCLi+cLojaCli+cNum+cParcela )   
      
       HE1->( dbAppend() )
       HE1->E1_CLIENTE := cCodCli
       HE1->E1_LOJA    := cLojaCli
       HE1->E1_TIPO    := cTipo
       HE1->E1_SALDO   := nValParc
       HE1->E1_EMISSAO := Date()
       HE1->E1_VENCTO  := Date() + aTitulos[ni,1]
       HE1->E1_NUM     := cNum             
       HE1->E1_PARCELA := cParcela
       HE1->E1_STATUS  := "N"  // Para importar no Protheus
       HE1->(dbCommit()) 
    Endif                                   
Next    
