...
*--------------------------------*
Private lRet :=.T.
If EasyEntryPoint("EICDI410")
   ExecBlock("EICDI410",.F.,.F.)
Else
   Private nOpc
   lTudo:=.F.
...
aAdd( aFixos, { STR0008, {|| Transform(SW6->W6_DI_NUM,IF(SW6->W6_TIPOREG == "2","@R! 99AA9999999999-9","@R 99/9999999-9")) }   } ) //"No. da D.I."
...
Local aRotina  := { { STR0001   ,'AxPesqui'   , 0 , 1},; //"Pesquisar" //LGS-27/04/2015
                    { STR0002   ,'DI410Manut', 0 , 4},; //"Manut. Cont's" DI410Manut
                    { STR0003   ,'DI410Manut', 0 , 2},;//"Estorno Cont's" DI410Manut
                    { "Pequisar Container", 'DI410PsqCnt',0,1}} //Pequisar Container
Private aRotAdic := {}  // GFP - 13/03/2014

If !AvFlags("DEMURRAGE")// FSM -  21/06/11
   aAdd(aRotina,{ "Rel. Demurrage" , "DI410REL" ,0,1}) //Rel. Demurrage
EndIf
...
*--------------------------------*
FUNCTION DI410Manut(cAlias,nReg,nOpcAux)
*--------------------------------*
...
nLin :=5; nColS:=05; nColG:=40; nAltu:=7; nPula:=12
 ...
 @ nLin,nColG msGET SW6->W6_HAWB       WHEN .F. SIZE 60,nAltu of oPanel Pixel
   nLin:=nLin+nPula
 @ nLin,nColG msGET SW6->W6_DT_EMB     WHEN .F. SIZE 60,nAltu of oPanel Pixel
   nLin:=nLin+nPula
 @ nLin,nColG msGET SW6->W6_CHEG       WHEN .F. SIZE 60,nAltu of oPanel Pixel
   nLin:=nLin+nPula
 @ nLin,nColG msGET SW6->W6_DT         WHEN .F. SIZE 60,nAltu of oPanel Pixel
   nLin:=nLin+nPula
 @ nLin,nColG msGET SW6->W6_DI_NUM     PICTURE IF(SW6->W6_TIPOREG == "2","@R! 99AA9999999999-9","@R 99/9999999-9") WHEN .F. SIZE 60,nAltu of oPanel Pixel
 ...
 @ nLin,nColG msGET SW6->W6_DT_DESE    WHEN .F. SIZE 60,nAltu of oPanel Pixel
   nLin:=nLin+nPula
 @ nLin,nColG msGET cArmador F3 "EIA" VALID fArmNome(cArmador) WHEN fArmEmpty(cArmador) SIZE 60,nAltu of oPanel Pixel  // GFP - 14/10/2013
   nLin:=nLin+nPula
...
Function DI410PsqCnt(cAlias,nReg,nOpc)
...
/*
Funcao      : fArmEmpty()
Parametros  : cArmador :: Código Armador
Retorno     : lRet :: .T./.F.
Objetivos   : Validação do When do campo Armador
...
/*
Funcao      : VALDI410
Parametros  : cPrograma
Retorno     : lRet :: .T./.F.
Objetivos   : Validação do valid dos campos da tabela SJD
Autor       : Maurício Frison 
Data/Hora   : 04/08/2020
*/
*------------------------------------*
Function VALDI410(cPrograma)
*------------------------------------*
Local lRet := .T.                                                                                  
   IF cPrograma == 'EICDI410'
         lREt := pertence("1234") .And. If(EasyEntryPoint("EICDI410"),ExecBlock("EICDI410",.F.,.F.,cPrograma),DI410_Val()) 
      ElseIf cPrograma == 'JD_DEVOLUC'
         lRet := if(EasyEntryPoint("EICDI410"),ExecBlock("EICDI410",.F.,.F.,cPrograma),DI410_Devo('JD_DEVOLUC'))
         DI410_Saldo()
      ElseIf cPrograma == 'JD_DTPREVI' //FSM - 17/06/2011
         lRet := if(EasyEntryPoint("EICDI410"),ExecBlock("EICDI410",.F.,.F.,cPrograma),DI410_Devo('JD_DTPREVI'))
      ElseIf cPrograma == 'ATUSALDO'
         lRet := if(EasyEntryPoint("EICDI410"),ExecBlock("EICDI410",.F.,.F.,cPrograma),DI410_Saldo()) .And. Positivo()
      ElseIf cPrograma == 'VERCHAVE'
         lRet := if(EasyEntryPoint("EICDI410"),ExecBlock("EICDI410",.F.,.F.,cPrograma),DI410_ExChave())
   EndIf
Return lRet
...
Parametros  : 
Retorno     : lRet :: .T.
Objetivos   : Relatório demurrage
Autor       : Maurício Frison 
Data/Hora   : 04/08/2020
*/
*------------------------------------*
Function DI410REL(cAlias,nReg,nOpc)
*------------------------------------*
Local lRet := .T.   
   If(EasyEntryPoint("EICDI450"),ExecBlock("EICDI450",.F.,.F.),EICDI450())     
Return lRet
