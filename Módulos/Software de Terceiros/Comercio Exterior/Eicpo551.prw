...
Function Eicpo551()        // incluido pelo assistente de conversao do AP5 IDE em 22/11/99
...
If EasyEntryPoint("EICPO551")
   ExecBlock("EICPO551",.F.,.F.)
Else
...
Local aRotina :=  { { STR0002  ,"AxPesqui"    , 0 , 1},;    //"Pesquisar"
                    { STR0021 , "PO551Altera"   ,0 ,0},;  //"Alterar Texto"      
                    { STR0003  ,"PO551Print", 0 , 0} }    //"Impressao"
                   
// P.E. utilizado para adicionar itens no Menu da mBrowse
If EasyEntryPoint("IPO551MNU")
...
IF EasyEntryPoint("IC195PO2")
   // Rdmake para Klabin
   ExecBlock("IC195PO2",.F.,.F.,Linha)
Else
   oPrn:Say( Linha:=Linha+150, 200 , STR0014 + IF(EasyGParam("MV_ID_CLI")$cSim,SA1->A1_NOME,SY1->Y1_NOME),aFontes:TIMES_NEW_ROMAN_14) //"PURCHASER: "
EndIf
IF !EMPTY(cMemo)          
  Linha := Linha +70
...
oPrn:Say( Linha    , 540 , IF(EasyGParam("MV_ID_CLI")$cSim,SA1->A1_TEL,SY1->Y1_TEL) ,aFontes:COURIER_12 )

oPrn:Say( Linha:=Linha+90, 200 , STR0020,aFontes:COURIER_12 ) //"FAX.....:"
oPrn:Say( Linha    , 540 , IF(EasyGParam("MV_ID_CLI")$cSim,SA1->A1_FAX,SY1->Y1_FAX) ,aFontes:COURIER_12 )


RETURN NIL
*------------------------------*
FUNCTION PO551Altera()
*------------------------------*
