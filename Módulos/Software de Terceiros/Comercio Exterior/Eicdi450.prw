...
*----------------------------------------------------*
Function EICDI450(cPrograma)
*----------------------------------------------------*
Default cPrograma := ''
lRet :=.T.
If EasyEntryPoint("EICDI450")
   lRet := ExecBlock("EICDI450",.F.,.F.,cPrograma)
Else
...
Private nInd, cCond, nBkpLin := 0 // GFP - 11/10/2013
...
Begin Sequence
    ...
    titulo += " - " + If(lDevolvido,STR0032,STR0033)  // GFP - 08/10/2013   "Conteiner Devolvido" ### "Conteiner Não Devolvido"
    ...
    dDtDvIni   := If(!Empty(MV_PAR01),MV_PAR01,CtoD('01/01/1900'))  // GFP - 08/10/2013
    dDtDvFim   := If(!Empty(MV_PAR02),MV_PAR02,CtoD('31/12/2999'))  // GFP - 08/10/2013
	...
	wnrel := SetPrint("SJD",NomeProg,"",@titulo,cDesc1,cDesc2,cDesc3,.F.,aOrd,/*.F.*/.T.,Tamanho,,.F.)  // GFP - 08/10/2013 - Sistema deve permitir ao usuario escolher o formato de impressão (Retrato/Paisagem)
...
Local cBkpArmador := ""  // GFP - 16/10/2013

SetRegua(SJD->(EasyRecCount()))
If !Empty(cArmador)
   SJD->(dbSeek(xFilial("SJD")+cArmador))
Else
   //SJD->(dbGoTop())
...
/*/
	SW6->(dbSetOrder(1))
	If !SW6->(dbSeek(xFilial("SW6")+SJD->JD_HAWB))  // GFP - 08/10/2013
	   SJD->(dbSkip())
	   Loop
	EndIf
	...
	If lDevolvido
		If Empty(SJD->JD_DEVOLUC) .OR. SJD->JD_DEVOLUC < dDtDvIni .Or. SJD->JD_DEVOLUC > dDtDvFim   // GFP - 08/10/2013
			SJD->(dbSkip())
			Loop
		Endif
	Else
	...
	If lImpArmador .OR. cBkpArmador <> SJD->JD_ARMADOR   // GFP - 16/10/2013
		nLin := nLin + 1
		...
		nLin := nLin + 1
		lImpArmador := .f.
		cBkpArmador := SJD->JD_ARMADOR     // GFP - 16/10/2013
	EndIf

	If lImpProc
		nLin := nLin + 1
		@nLin,07  PSAY "***** " + AllTrim(SW6->W6_HAWB) + " *****"  // GFP - 24/10/2013
	...
	nBkpLin := nLin := If(nBkpLin # 0 .AND. nBkpLin == nLin,nLin+1,nLin)  // GFP - 11/10/2013
	...
	If !Empty(SJD->JD_DEVOLUC)
       ...
       dDiaA:= StoD("")
		Do While Type('SJD->JD_ATE'+AllTrim(Str(nInd))) <> "U" .And.;
                 Type('SJD->JD_VAL'+AllTrim(Str(nInd))) <> "U" .And. nDias >= nSaldoDias .And. !Empty(&('SJD->JD_ATE'+AllTrim(Str(nInd))))
...
If lDevolvido
   ...
   cQuery += If(!Empty(dDtDvIni)," And JD_DEVOLUC >= " + DtoS(dDtDvIni)," ") //LGS-24/09/2013 - Incluido validação para as variaveis de data usadas na consulta como filtro.
   cQuery += If(!Empty(dDtDvFim)," And JD_DEVOLUC <= " + DtoS(dDtDvFim)," ")
Else
