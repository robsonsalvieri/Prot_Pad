...
*========================================================================================*
LOCAL oDlg_wk,W
LOCAL bOkGra := {|| IF(MSGYESNO(STR0007),(oDlg_wk:End(),nBotao:=1),) } //"Confirma Gravação dos Valores?"
LOCAL bOkExc := {|| IF(MSGYESNO(STR0009),(oDlg_wk:End(),nBotao:=2),) } // Confirma Exclusão dos Valores?
...
Begin Sequence
	...
	ELSE
	   ...
	   IF SWN->(DBSEEK(CFILSWN+SW6->W6_HAWB+"9")) .AND. SF1->(DBSEEK(CFILSF1+SW6->W6_HAWB+"9"))
         lIntegrouNFT := !lGerouNFT //Tem nota e não tem EIW, entao a nota foi integrada e nao gerada manualmente
         nTela := ESTORNO
	   ENDIF
	ENDIF
   ...
   Private aUsrCpItens := {}//Possibilita incluir campos customizados na WorkItens

   If EasyEntryPoint("EICCO100")
      Execblock("EICCO100",.F.,.F.,"PCOTELA_INICIO")
   EndIf
...
//	EndIf
   ...
   EndIf
	...
	aHeader:={}
   aEval(aUsrCpItens, {|x| aAdd(aSemSX3, x) })
	...
	//ajustado para otimizador de interfaces - campos devem estar disponíveis na work, independente da configuração de uso no SX3
	AAdd(aSemSX3, {"WN_INVOICE" , AVSX3("WN_INVOICE",AV_TIPO), AVSX3("WN_INVOICE", AV_TAMANHO), AVSX3("WN_INVOICE", AV_DECIMAL)})
  	AAdd(aSemSX3, {"W9_MOE_FOB" , AVSX3("W9_MOE_FOB",AV_TIPO), AVSX3("W9_MOE_FOB", AV_TAMANHO), AVSX3("W9_MOE_FOB", AV_DECIMAL)})
  	AAdd(aSemSX3, {"W9_TX_FOB" , AVSX3("W9_TX_FOB",AV_TIPO), AVSX3("W9_TX_FOB", AV_TAMANHO), AVSX3("W9_TX_FOB", AV_DECIMAL)})
...
//	      EndIf
	...
	IF nTela = INCLUSAO .OR. lSoLeNFT
	   ...
	   ENDIF
	ELSE
      If lGerouNFT .Or. (!lGerouNFT .And. nTela <> ESTORNO)
	      Processa( {|| PCOLeGrava(.T.)},,"Lendo dados...")
      Else
         Processa( {|| lGerou:=PCOGerar( .T./*So ler dados da SWN*/)},,"Lendo Dados...")
      EndIf
	ENDIF
	...
	DO WHILE .T.
	   ...
	   DEFINE MSDIALOG oDlg_wk TITLE cCadastro+" - Processo: "+SW6->W6_HAWB;//"Valores Base da Nota Fiscal de Transferencia de Posse" ;
	     ...
	     IF nTela = INCLUSAO .OR. nTela = ALTERACAO
	        ...
	        bOk:={||IF(MSGYESNO(If(PCOAltQtd(),STR0006,STR0007)),(oDlg_wk:End(),nBotao:=5),) } // "Existem itens alterados para os quais não foram gerado notas fiscais, ao confirmar esses dados serão descartados. Deseja prosseguir?" //"Confirma Gravação dos Valores?"   
...
*==============================================*
/* Função para verificar se foi alterado alguma quantidade ainda sem nota
Esta função foi criada para alterar a mensagem caso a quantidade seja alterada ela só será
salva se entrar na opção de Gerar NFT, caso contrário só no botão Salvar a quantidade não é gravada
devido ao controle da variável "lGravaEIW" //LGS-22/10/2014 - Mantem as quantidades originais do processo para controle de saldo NFT Filha.
Author: Maurício Frison
DAta 19/11/2020
*/
FUNCTION PCOAltQtd()
Local nContador := 0
Local cNameEIW := RetSqlName("EIW")
Local cNameWork := TETempName("WorkItens")
Local aArea := GetArea()
Local cQuery
cQuery := "SELECT COUNT(*) CONTADOR FROM " + cNameWork + " TMP INNER JOIN " + cNameEIW + " EIW ON TMP.R_E_C_N_O_ = EIW.R_E_C_N_O_ "
cQuery += " WHERE TMP.D_E_L_E_T_ = ' ' AND EIW.D_E_L_E_T_ = ' ' "
cQuery += " AND TMP.R_E_C_N_O_ <> 0  AND TMP.WN_QUANT <> EIW.EIW_QTDE "
...
ELSE
   ...
   DO WHILE WorkItens->(!EOF())
      IF !EMPTY(WorkItens->WK_FLAG) .AND. WorkItens->WK_FLAG # 'LBNO'
         lTemMarcado:=.T.
         EXIT
      ENDIF
      WorkItens->(DBSKIP())
...
IF lOK
   ...
   ELSE
      lMudouOp := MUDOUOP(cOperacao)
      If lMudouOp
         aDespMarc := DESPMARCADA()
         If !Empty(aDespMarc)
            nRecOld := WorkMarca->(Recno())
            PCOMarca(.T.,.T.,.F.,.F.,.F.)
         EndIf
      EndIf
      WorkItens->(DBGOTOP())
      DO WHILE WorkItens->(!EOF())         
         IF EMPTY(WorkItens->WK_FLAG) .or. WorkItens->WK_FLAG == "LBNO"
            WorkItens->(DBSKIP())
            LOOP
         ENDIF
         ...
         ENDIF
         IF lAlterou
            //RNLP 02/09/2020 ATUALIZAÇÃO VALORES AO MUDAR OPERACAO --------
            PCOCalcImposto()
         ENDIF
      ...
      If lMudouOp
         If !Empty(aDespMarc)
            For nI := 1 To Len(aDespMarc)
               WorkMarca->(dbGoTo(aDespMarc[nI]))
               PCOMarca(.F.,.T.,.F.,.T.,.F.)
            Next
            WorkMarca->(dbGoTo(nRecOld))
         EndIf
      EndIf
   ENDIF
...
RETURN .T.
*===========================================================================*
STATIC FUNCTION PCOMarca(lMarcaALL,lMarcaImpostos,lMensagem,lMarcaDesp,lRefresh)
*===========================================================================*
...
*==============================================*
LOCAL nBase:=nICMS_A:=nBaseICMS:=nValIPI:=nValICM:=nBaseIPI:=nIPI_A:=nRedCte_A:=nIPIBsIcm:=0
LOCAL lTemIPIBase := .F. //IPI da própria ntf incidindo na base do icms
LOCAL lTpCalcIcms := SWZ->(columnPos("WZ_TP_CALC")) > 0
...
//   EndIf
   ...
   nIPI_A    := WorkItens->WKIPITX/100
   iF getConsumo()
      lTemIPIBase := .T.
      nIPIBsIcm := nIPI_A    
   EndIf   
   nBaseICMS := getBaseIcms(nBase,nICMS_A,nIPIBsIcm)   //ICMS integra a própria Base e agrega o IPI da própria NFT na base do ICMS
                                                        // se não entrou no if anterior entao nIPIBsIcm é zero, não vai ter efeito na fórmula  
   nValICM   := nBaseICMS * nICMS_A
      
   nBaseIPI     := WorkItens->WN_FOB_R + WorkItens->WN_FRETE + WorkItens->WN_SEGURO + nValICM + WorkItens->WKDESPBIPI     
   nValIPI   := nBaseIPI * (nIPI_A)

   //Recalculo do ICMS em caso de redução ou carga tributária equivalente
   SWZ->(DBSETORDER(2))
   IF lTemIPIBase .Or. !EMPTY(WorkItens->WKOPERACAO) .AND. SWZ->(DBSEEK(cFilSWZ+WorkItens->WKOPERACAO))//Ordem 2 do SWZ, se lTemIPIBase, já posicionou o registro SWZ

      nAlqReducao := IF(SWZ->WZ_RED_ICM#0,(100-SWZ->WZ_RED_ICM)/100,1)
 
      //não tem o campo WZ_TP_CALC no dicionário; mantém o tratamento atual
      If !lTpCalcIcms

         IF !EMPTY(SWZ->WZ_RED_CTE) .AND. nAlqReducao == 1
            //Carga Tributária Equivalente
            nRedCte_A := SWZ->WZ_RED_CTE/100
            If EasyGParam("MV_EIC0029",,"N") == "R"                                                         //NCF - 22/05/2013 - O valor do ICMS a integrar a própria base de cálculo para cálculo por Carga Tributária
               nBaseNew:= getBaseIcms(nBase,nRedCte_A,nIPIBsIcm )                                  //                   Equivalente deve ser obtido conforme o parâmetro: N=Alíquota Normal; R=Alíquota Reduzida
               nBaseICMS := DITrans( nBaseNew * (SWZ->WZ_RED_CTE/WorkItens->WKICMS_A) ,2)
            Else
               nBaseICMS := DITRANS( nBaseICMS * ((SWZ->WZ_RED_CTE/100) / nICMS_A),2)            
            Endif
         ELSE
         //Redução da base de calculo
            nBaseICMS := DITrans(getBaseIcms((nBase * nAlqReducao),nICMS_A,nIPIBsIcm) ,2)
         ENDIF
      Else
         If lTemIPIBase // Recalculando a base do IPI com o ICMS para o cenário de consumidor final
            nBaseIPI:= DI154CalcICMS(nBase, nAlqReducao, (nICMS_A * (1 + nIPI_A) * 100), (SWZ->WZ_RED_CTE * (1 + nIPI_A)),,,,,,,,,WorkItens->WKOPERACAO)
            nValIPI := nBaseIPI * nIPI_A
            nBaseICMS := DITrans(nBaseIPI * (1 + nIPI_A), 2) // Mesma coisa que somar o nBaseIPI + nValIPI
         Else
            nBaseICMS:= DI154CalcICMS(nBase, nAlqReducao, WorkItens->WKICMS_A, SWZ->WZ_RED_CTE,,,,,,,,,WorkItens->WKOPERACAO)
         EndIF
      EndIf
      ...
      IF lICMSRedIPI .AND. SWZ->WZ_RED_CTE+SWZ->WZ_RED_ICM > 0 .AND. !lTemIPIBase
         nBaseIPI     := WorkItens->WN_FOB_R + WorkItens->WN_FRETE + WorkItens->WN_SEGURO + nValICM + WorkItens->WKDESPBIPI
         nValIPI   := nBaseIPI * nIPI_A
      EndIf
   ENDIF
   ...
   //Possibilita gravar os campos customizados
   If EasyEntryPoint("EICCO100")
      Execblock("EICCO100",.F.,.F.,"PCOCALCIMPOSTO")
   EndIf
...
/*
Programa   : GetBaseIcms(cCampo)
Objetivo   : Retornar a base do ICMS
Parâmetros : nBase = Base principal do icms
             nImcs_A = percentual da alíquota do icms
             nIPI_A = percentual da alíquota do icms
Retorno    : nReturn
Autor      : Maurício Frison
Data/Hora  : Agosto/2022
Obs.       :
*/
STATIC FUNCTION GetBaseIcms(nBase,nIcms_A,nIpi_A)
Local nReturn := nBase/(1-(nICMS_A * (1 + nIPI_A))) * (1 + nIPI_A)  
Return nReturn

/*
Programa   : GetConsumo()
Objetivo   : Verificar se a importação é para consumo para incidir o ipi na base do icms
Retorno    : lReturn
Autor      : Maurício Frison
Data/Hora  : Agosto/2022
Obs.       :
*/
STATIC FUNCTION getConsumo()
Local lReturn
lReturn := !EMPTY(WorkItens->WKOPERACAO) .AND. SWZ->(FieldPos("WZ_IPINFT")) > 0 .And. Posicione("SWZ",2,cFilSWZ+WorkItens->WKOPERACAO,"WZ_IPINFT") == '1'
Return lReturn
...
AADD(aImposto,{"3-PIS"   ,"204"    })
AADD(aImposto,{"4-COFINS","205"    })
AADD(aImposto,{"5-ICMS"  ,"203"    })
...
EndIf
   ...
   If cDespesa=="202" //desp ipi      
      ...
      EndIf 
   EndIf
   lIncideICms := SYB->(DbSeek(xFilial("SYB") + cDespesa)) .and. (SYB->YB_BASEIMP $ cSim .or. SYB->YB_BASEICM $ cSim .AND. SYB->(FIELDPOS("YB_ICMS_"+Alltrim(SYT->YT_ESTADO))) # 0 .AND. SYB->(FIELDGET(FIELDPOS("YB_ICMS_"+Alltrim(SYT->YT_ESTADO)))) $ cSim)
   lIncideIPI  := lCposDspBs .and. SYB->(DbSeek(xFilial("SYB") + cDespesa)) .and. (SYB->YB_BIPINFT $ cSim .or. SYB->YB_BASEIMP $ cSim)
   If  !EasyGParam("MV_EIC0058",,.F.) .or. lIncideIcms .or. (lIncideIPI .And. getConsumo())
      If !lImposto
         IF lMarcando
   ...
   If !EasyGParam("MV_EIC0058",,.F.)  .or. lIncideIPI .or. (lIncideIcmsPI .And. getConsumo())
      If !lImposto
         IF lMarcando
   ...
   IF SYB->(DbSeek(xFilial("SYB") + cDespesa))
      IF !EasyGParam("MV_EIC0058",,.F.) .Or. AvFlags("NFT_DESP_BASE_IMP") .AND. (SYB->YB_TOTNFT $ cSim .or. SYB->YB_BASEIMP $ cSim)
         If !lImposto
            IF lMarcando
...
IF lLer//LEITURA =======================================================================================
   ...
   DO WHILE EIW->(!EOF()) .AND. EIW->EIW_FILIAL == cFilEIW .AND. EIW->EIW_HAWB == SW6->W6_HAWB
      ...
      IF lNftFilha //LGS-22/10/2014
         ...
         IF !lEstFilha .AND. LEN(aItemEIW) > 0 //Inclusão de NFT Filha
            FOR G:=1 TO LEN(aItemEIW)
               IF WorkItens->WN_INVOICE      == aItemEIW[G][4] ;
               .AND. WorkItens->WN_PRODUTO   == aItemEIW[G][1] ;
               .AND. WorkItens->WV_LOTE      == aItemEIW[G][3] ;
               .AND. WorkItens->WN_ITEM      == aItemEIW[G][5] //iif( lNftFilha , aItemEIW[G][6] , aItemEIW[G][5] ) //NCF - 05/03/2021 - Considerar pela EIW_POSICA (posicao gerada para a operação)
      ...
      //Possibilita gravar os campos customizados
      If EasyEntryPoint("EICCO100")
         Execblock("EICCO100",.F.,.F.,"PCOLEGRAVA_LEITURA")
      EndIf
...
IF !DI154NFE("SW6" ,SW6->(RECNO()),nTipoNF, , ,aLocExecAuto,NF_TRANSFERENCIA)//Chama calculando os impostos
   IF EasyGParam("MV_TEM_DI",,.F.)
      E_RESET_554()
   Else
      E_RESET_154()
...
IF lGerar
   ...
   IF EIW->(DBSEEK(cFilEIW+SW6->W6_HAWB))
      ...
      cSerieNota := ""
      dDataNf := ctod("")
      lMVEASY := if(isMemVar("lMV_EASY"), lMV_EASY,(EasyGParam("MV_EASY",,"N") == "S"))
      aAreaSF1 := SF1->(getArea())
      if !lNftFilha .And. !Empty(EIW->EIW_NOTA) .and. !lIntNFTEAI .and. lMVEASY
         SF1->(DBSETORDER(5))
      endif
      ...
      DO WHILE EIW->(!EOF()) .AND. EIW->EIW_FILIAL == cFilEIW .AND. EIW->EIW_HAWB == SW6->W6_HAWB
         ...
         getInfoNFe(@cNrNota, @cSerieNota, @dDataNf, lNftFilha, lIntNFTEAI, lMVEASY)
...
// Work1->WKRATEIO   := Work1->WKFOB_R / DITRANS((SW6->W6_FOB_TOT+MDespesas),2)
   ...
   IF lLote
      ...
      aDtLote := getDTLotes(EIW->EIW_HAWB, EIW->EIW_INVOIC, EIW->EIW_PGI_NUM, EIW->EIW_PO_NUM, EIW->EIW_POSICA, EIW->EIW_LOTECT)
      If !Empty(aDtLote)
         Work1->WKDTVALID  := aDtLote[1] //Dt Validade
         If AvFlags("DATA_FABRIC_LOTE_IMPORTACAO")
            Work1->WKDTFBLT   := aDtLote[2] //Dt Fabricacao
         EndIf
      EndIf
   ENDIF
   IF lExisteSEQ_ADI
...
Private aCAB := {} // Ponto de entrada - INCLUSAO_PV
Private aITENS := {} // Ponto de entrada - INCLUSAO_PV
...
lMSERROAUTO := .F.
IF(EasyEntryPoint("EICCO100"),Execblock("EICCO100",.F.,.F.,'INCLUSAO_PV'),)
...
ELSEIF cP_ACAO == "CLIENTE_PO"
       ...
       ELSE
          ...
          ELSE
             SA1->(DBSETORDER(1))
             IF ! (SA1->(DBSEEK(XFILIAL("SA1")+SW2->W2_CLIENTE+SW2->W2_CLILOJ)))
...
ELSEIF cP_ACAO == "bOK"
       IF !PCOVALPAR("cCONDPAMS") .OR.;
          ...
          lRET := .F.
       ELSEIF !MSGYESNO(STR0008) //Confirma Gravação dos Valores e Geração do PV ?
          lRET := .F.
       ENDIF
...
DO WHILE .T.
   ...
   If !lOldNF .OR. lIntNFTEAI  //LRS - 11/09/2017   //NCF - Refeita as verificações para manter a opção de digitação manual de Nro. da Nota
      If (cTpNrNfs == "1" .OR. cTpNrNfs == "2")
         IF lOK:= SX5NumNota(@cSerie,cTpNrNfs,,,,,,,@cTypeDoc)
            cNota := NxTSx5Nota(cSerie,.T.,cTpNrNfs,,,,,,cTypeDoc)
         Else
            RETURN .F.
         Endif
      ElseIf cTpNrNfs == "3"
	    cNota := Ma461NumNF(.T.,Left(WorkItens->WN_SERIE,SerieNfId("SWN",6,"WN_SERIE")),WorkItens->WN_DOC,,,cTypeDoc) //AAF 18/02/2015
      EndIf
   EndIf
...
If !AvFlags("NFT_DESP_BASE_IMP") .Or. PCOGerado("VERM") .And. nNFT_DESP_BASE_IMP == 0 .Or. EasyGParam("MV_PCOIMPO",,.T.)
   IF lCalcNFT
...
DEFINE MSDIALOG oDlg2 TITLE "TOTAIS" FROM 0,10 TO 15,nTelaLarg //MCF - 12/01/2015
   ...
   IF lCalcNFT
      ...
      @nLinha,nCo3 SAY "Total Mercadoria"                    SIZE nTam,nAlt of oDlg2 PIXEL
      ...
      @nLinha,nCo4 MSGET nVlTotCIPI  WHEN .F. PICTURE PICT15_2 SIZE nTam,nAlt of oDlg2 PIXEL
...
IF EIW->(DBSEEK(cFilEIW+cChaveIT)) .And. !lNfGerou .And. !lMontaTela
	IF !lSoLeNFT
      ...
      WorkItens->WKDESPBICM := ( (EIW->EIW_DBSICM/nQtdOri)* nQtdAlt )
      WorkItens->WKDESPBIPI := ( (EIW->EIW_DBSIPI/nQtdOri)* nQtdAlt )
      WorkItens->EIW_DESPCU := ( (EIW->EIW_DESPCU/nQtdOri)* nQtdAlt )
	ENDIF
ENDIF
...
DO WHILE EIW->(!Eof()) .And. EIW->EIW_HAWB == SW6->W6_HAWB
   AADD(aItemEIW,{EIW->EIW_COD_I, EIW->EIW_QTDE, EIW->EIW_LOTECT, EIW->EIW_INVOIC, EIW->EIW_POSICA, EIW->EIW_POSORI })
   EIW->(DBSKIP())
ENDDO
...
FOR i:=1 TO LEN(aItemSWN)
	aItemSWN[i][2] := 0
	SWN->(dbsetorder(3),DBSEEK(cFilSWN+SW6->W6_HAWB+"9"))
	DO WHILE SWN->(!Eof()) .And. SWN->WN_HAWB == SW6->W6_HAWB
	   IF SWN->WN_INVOICE      == AvKey(aItemSWN[i][4],"WN_INVOICE")    ;
      .AND. SWN->WN_PRODUTO   == AvKey(aItemSWN[i][1],"WN_PRODUTO") ;
      .AND. SWN->WN_LOTECTL   == AvKey(aItemSWN[i][3],"WN_LOTECT")  ;
      .AND. SWN->WN_ITEM      == aItemSWN[i][5] //iif( lNftFilha , aItemSWN[i][6] , aItemSWN[i][5] ) //NCF - 13/01/2021 - Considerar pela EIW_POSICA (posicao gerada para a operação)
...
DO CASE
   CASE cValid == "EIW_QTDE"
		IF NaoVazio(M->WN_QUANT) .AND. Positivo(M->WN_QUANT)
         ...
         IF LEN(aItemEIW)== 0
            MSGINFO(STR0004) //Antes de alterar a quantidade do item, efetue a gravação dos valores
            Return .F.
         EndIf 

		   FOR i:=1 TO LEN(aItemEIW)
            IF WorkItens->WN_INVOICE      == aItemEIW[i][4] ;
            .AND. WorkItens->WN_PRODUTO   == aItemEIW[i][1] ;
            .AND. WorkItens->WV_LOTE      == aItemEIW[i][3] ;
            .AND. WorkItens->WN_ITEM      == aItemEIW[i][5]  //iif( lNftFilha , aItemEIW[i][6] , aItemEIW[i][5] ) - //NCF - 13/01/2021 - Considerar pela EIW_POSICA (posicao gerada para a operação)
               nSaldo := aItemEIW[i][2]
				ENDIF
		   NEXT
		   ...
		   IF M->WN_QUANT > nSaldo
		      ...
		      MSGINFO(STR0005 + Str(nSaldo)) //A quantidade informada é maior que a disponível. Saldo disponível: XX
		   ELSE
...
AADD(aHdrItem  ,{"PO+Posicao+PGI+Lote+Invoice","WK_CHAVE","@!"                 ,50                    ,0                    ,""          ,""      ,"C"    ,""        ,""} )
   ...
   //Permite adicionar campos de usuário no aHdrItem (aHeader)
   If EasyEntryPoint("EICCO100")
      Execblock("EICCO100",.F.,.F.,"PCOGRVHEADER")
   EndIf
...
Local aItensSWN  := {}
Local aRet       := {}
Local cKeyLoopWN := xFilial("SWN") + SW6->W6_HAWB + "9"
Local cKeyLoopW8 := xFilial("SW8") + SW6->W6_HAWB
Local aOrdTab    := SaveOrd({'SW8','SWN'})
Local nQtdeSWN   := 0
Local cItem      := ""
Local cKeyWN_W8  := ""
Local lNftFlh    := EasyGParam("MV_NFT_FLH",,.F.)

SWN->(DbSetOrder(3))
SW8->(DbSetOrder(1))

SWN->(DbSeek( cKeyLoopWN ))
Do While SWN->(!Eof()) .And. Left(  SWN->&(IndexKey())  ,Len(cKeyLoopWN) ) == cKeyLoopWN
   if lNftFlh
      EIW->(dbsetorder(1),msseek( xFilial("EIW")+SWN->WN_HAWB+SWN->WN_PO_EIC+SWN->WN_ITEM+SWN->WN_PGI_NUM+SWN->WN_LOTECTL+SWN->WN_INVOICE ))
      cItem := EIW->EIW_POSORI
   else
      cItem := SWN->WN_ITEM
   endif
   cKeyWN := SWN->WN_FILIAL + SWN->WN_HAWB + SWN->WN_INVOICE + SWN->WN_PO_EIC + cItem + SWN->WN_PGI_NUM + SWN->WN_LOTECTL
   If (nPos := aScan(aItensSWN, {|x| x[1] == cKeyWN   }) ) == 0
      aAdd(aItensSWN, {SWN->WN_FILIAL+SWN->WN_HAWB+SWN->WN_INVOICE+SWN->WN_PO_EIC+cItem+SWN->WN_PGI_NUM+SWN->WN_LOTECTL , SWN->WN_QUANT, SWN->WN_IPIVAL, SWN->WN_VL_ICM } )
   Else
      aItensSWN[nPos][2] += SWN->WN_QUANT
      aItensSWN[nPos][3] += SWN->WN_IPIVAL
      aItensSWN[nPos][4] += SWN->WN_VL_ICM
   EndIf
...
SW8->(DbSeek( cKeyLoopW8 ))
DO WHILE SW8->(!Eof()) .And. SW8->W8_HAWB == SW6->W6_HAWB

   cKeyWN_W8 := xFilial('SWN')+SW8->(W8_HAWB+W8_INVOICE+W8_PO_NUM+W8_POSICAO+W8_PGI_NUM)
   nQtdeSWN := 0
   aEval( aItensSWN, {|x| If(  Left(x[1],Len(cKeyWN_W8))  == cKeyWN_W8  , nQtdeSWN += x[2] , nQtdeSWN += 0  )  } )

   If SW8->W8_QTDE <> nQtdeSWN //aItensSWN[nPos][2]
      lRet := .F.
      EXIT
   EndIf
...
//MFR 05/07/2017  MTRADE-1200 WCC-524950
//Verificar se o Ipi da importação deve ser considerado nas bases do icms, ipi e NFT Total na geração da nota de transf por Conta e Ordem
*==============================================*
STATIC FUNCTION ConsideraIPINFT(cOp, lModoProc)
*==============================================*
Local lSeekSWZ := .F.
Local cRet := "NAO CONFIGURADO"
Local lRet := .F.
LOCAL lSemIPIBase := EasyGParam("MV_EIC0026",,.F.)

Default cOp := ""
Default lModoProc := .F.
SWZ->(DBSETORDER(2))
If Empty(cOp)
   lSeekSWZ := SWZ->(DBSEEK(cFilSWZ+WorkItens->WKOPERACAO))
Else
   lSeekSWZ := SWZ->(DBSEEK(cFilSWZ+cOp))
EndIf

If lSeekSWZ
   IF !lCpoBseNft .OR. (!lModoProc .And. empty(rtrim(WorkItens->WKOPERACAO))) .OR. empty(rtrim(SWZ->WZ_BASENFT)) .OR. SWZ->WZ_BASENFT == '3'
      cRet := "NAO CONFIGURADO"              
...
If cRet == "NAO CONFIGURADO" .And. lSemIPIBase 
	cRet := "NAO"
EndIf

If cRet == "NAO CONFIGURADO" .Or. cRet == "SIM"
	lRet := .T.
EndIf
...
Static Function DESPMARCADA()
Local aRet     := {}
Local cQuery   := ""
Local cWkTmp   := GetNextAlias()
Local aNotSX3 := {{"RECNO","N",10,0}}

cQuery := " SELECT R_E_C_N_O_ RECNO FROM " + TETempName("WorkMarca") + " "
cQuery += " WHERE WK_FLAG <> ' ' "
cQuery += "   AND D_E_L_E_T_ = ' ' "

EasyWkQuery(cQuery,cWkTmp,,aNotSX3)

While (cWkTmp)->(!Eof())
   aAdd(aRet,(cWkTmp)->(RECNO))
   (cWkTmp)->(dbSkip())
End
(cWkTmp)->(dbCloseArea())
...
Static Function MUDOUOP(cOperacao)
Local lRet   := .F.
Local cQuery := ""

cQuery := " SELECT WKOPERACAO FROM " + TETempName("WorkItens") + " "
cQuery += " WHERE WK_FLAG <> ' ' "
cQuery += "   AND WKOPERACAO <> '" + cOperacao + "' "
cQuery += "   AND D_E_L_E_T_ = ' ' "
...
Static Function getDTLotes(cHawb, cInvoice, cPgiNum, cPoNum, cPosica, cLoteCtl)
Local aRet  := {}
Local cQuery:= ""
Local cWkSWV:= GetNextAlias()

cQuery := " SELECT WV_DT_VALI, WV_DFABRI "
cQuery += " FROM " + RetSqlName("SWV") + " "
cQuery += " WHERE WV_FILIAL   = '" + xFilial("SWV") + "' "
cQuery += "   AND WV_HAWB	   = '" + cHawb + "' "
cQuery += "   AND WV_INVOICE  = '" + cInvoice + "' "
cQuery += "   AND WV_PGI_NUM  = '" + cPgiNum + "' "
cQuery += "   AND WV_PO_NUM	= '" + cPoNum + "' "
cQuery += "   AND WV_POSICAO  = '" + cPosica + "' "
cQuery += "   AND WV_LOTE	   = '" + cLoteCtl + "' "
cQuery += "   AND D_E_L_E_T_  = ' ' "

EasyWkQuery(cQuery,cWkSWV)

If (cWkSWV)->(!Eof())
   aAdd(aRet,(cWkSWV)->(WV_DT_VALI))
   aAdd(aRet,(cWkSWV)->(WV_DFABRI))
EndIf
(cWkSWV)->(dbCloseArea())

Return aRet

/*/{Protheus.doc} getInfoNFe
   Retorna os dados da nota fiscal gerada

   @type  Static Function
   @author user
   @since 09/01/2025
   @version version
   @param cNrNota, caractere, Numero da nota
          cSerieNota, caractere, Serie da nota
          dDataNf, data, Data da nota
   @return nil
   @example
   (examples)
   @see (links_or_references)
/*/
static function getInfoNFe(cNrNota, cSerieNota, dDataNf, lFilha, lIntEAI, lMVEASY)
   IF !lFilha .And. Empty(cNrNota) .And. !Empty(EIW->EIW_NOTA)   // By JPP - 27/07/2009 - 12:44 - Pega o numero da nota que será gravado nas despesas para
      If lIntEAI
         cNrNota    := SF1->F1_DOC
      ...
      Else
         ...
         dDataNf    := dDataBase
         if lMVEASY .and. SF1->(dbSeek(xFilial("SF1")+SW6->W6_HAWB+cValToChar(NF_TRANSFERENCIA)))
            dDataNf    := SF1->F1_EMISSAO
         endif
      EndIf
   ELSEIF lFilha .And. Empty(cNrNota) .And. Empty(EIW->EIW_NOTA)
