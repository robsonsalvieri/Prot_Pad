...
CLASS OFIA190EVDF FROM FWModelEvent
...
METHOD FieldPreVld(oModel, cModelID, cAction, cId, xValue) CLASS OFIA190EVDF
...
METHOD InTTS(oModel, cModelId) CLASS OFIA190EVDF

	Local nPos := 0
	Local oVM2 := FWLoadModel( 'OFIA191' )

	nPos := aScan(self:aCpoAlt,{|x| x[1] == "VM0_STATUS"})
	...
	If nPos > 0
		...
		If self:aCpoAlt[nPos,2] <> self:aCpoAlt[nPos,3]
			...
			if lRet
				...
				oVM2:SetValue( "VM2MASTER", "VM2_CODIGO", oModel:GetValue("VM0MASTER","VM0_CODIGO") )
				oVM2:SetValue( "VM2MASTER", "VM2_TIPO"  , "1" )
