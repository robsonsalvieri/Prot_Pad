...
Local nPosCodIss 	:= Ascan(aPosCpoDet,{|x| Alltrim(Upper(x[1])) == "LR_CODISS"}) //Pega a posicao do campo LR_CODISS 

//Se o item que o usuario esta deletando ou restaurando nao eh servico,
//nem deixo entrar no FOR
If !Empty(aColsDet[n][nPosCodIss])
		...
		If !(aCols[n][nPosDtItem] == aCols[nX][nPosDtItem]) .AND. !aCols[nX][Len(aCols[nX])]
			...
			Else
			   	...
			   	If Empty(Lj7GetIss())
			   		lRet := .T.
			   		Exit
			   	ElseIf Empty(aColsDet[nX][nPosCodIss]) .OR. (AllTrim(aColsDet[n][nPosCodIss]) == AllTrim(Lj7GetIss()))
			   		lRet := .T.
			   	Else 
			   		lRet := .F.
			   		Exit
	...
	If !aCols[n][Len(aCols[n])]
		...
		EndIf
	Else
			Lj7SetIss(aColsDet[n][nPosCodIss])			
		EndIf						
	EndIf
...
@param aOrcFilhos, array, Produto/s do/s orcamento/s filho/s

@return array, Retorna array de pedidos formatado no Padrao
/*/
//---------------------------------------------------------------------------
Function Lj7RecupPed( cNumPai , nAuxNcc , aAuxNcc , nNccAux, aOrcFilhos)
Local nX		:= 0
Local nY		:= 0
...
While SL1->(!Eof()) 
	...
	If SL2->( DbSeek(SL1->L1_FILIAL+SL1->L1_NUM) )
		While SL2->(!EOF()) .AND. (SL2->L2_FILIAL+SL2->L2_NUM == SL1->L1_FILIAL+SL1->L1_NUM)
			If !Empty(SL2->L2_ENTREGA) .And. !(SL2->L2_ENTREGA $ "1/2")
				If aScan(aOrcPed ,{ |x| x[1] == SL2->L2_FILRES + SL2->L2_NUM } ) == 0
					Aadd(aOrcPed, {SL2->L2_FILRES + SL2->L2_NUM ,SL2->L2_FILRES , SL2->L2_NUM} )
				EndIf
			EndIf

			Aadd(aOrcFilhos,{SL1->L1_FILIAL, SL1->L1_NUM, SL2->L2_ITESC6, SL2->L2_PRODUTO})
...
For nI := 1 To Len(aPedidos)
	...
	If lRet
        ...
        If SC5->C5_STATUS == "01" .AND. AliasInDic("MHQ").AND. ExistFunc("RmiExeGat") .AND. ExistFunc("SHPStatus") 
            DbSelectArea("MHQ")
        ...
        If lRet .and. FWHasEAI("MATA410B", .T., , .T.) .and. !lShp
...
Local lRet			:= .F.
Local lLjVfNFS		:= SuperGetMV("MV_LJVFNFS",,.F.)
Local nX			:= 0
Local nPos			:= 0
	...
	If Len(aSL2) > 0
		If (nPos := ascan(aSL2[1], {|x| Alltrim(Upper(x[1])) == "L2_ENTREGA"})) > 0
			For nX := 1 To Len(aSL2)
				If aSL2[nX][nPos][2] <> '3' .AND. aSL2[nX][nPos][2] <> '5'				
					lRet := .F.
				Endif
			Next nX
...
If lCentPDV
	...
	lRetorno:= Iif(!lOnline .AND. cMVLjNCCBC == "3", (STFMessage(ProcName(),"STOP",STR0008), STFShowMessage(ProcName()), .F.) , .T.  ) // MV_LJNCCBC = 1 - Online apenas
		
	IIf(!lRetorno .AND. !lOnline .AND. cMVLjNCCBC == "2", aProfile := STFProFile(36) ,Nil) // Solicita senha do supervisor para liberacao de NCC offline.
	...
	If Len(aProfile) == 0 .OR. aProfile[1]
		...
		///BAIXA NCC PARA CENTRAL DE PDV
		STBRemoteExecute("STWBOBxNCC", aParam,,.T., @uResult)
	
		lRet := IIf (VALTYPE( uResult ) == "L", uResult	, .F.)		

		LjGrvLog( "L1_NUM: " + SL1->L1_DOC, "Conseguiu dar baixa na NCC - Central? ", lRet ) //Grava Log =====================================================================		
...
Else 
	...
	//--------------------------------
	lRet := FrtIncluiNCC(aNCCItens		, 	nNccUsada		, nNccGerada		, SL1->L1_DOC		,;
	...
	If lRet
		...
		//------------------------------------------------------------------
		lRet := FrtIncluiCR(SL1->L1_SERIE	, @cDocCred			, @nRecnoSE1	, SL1->L1_DOC		,;
		...
		If lRet
			...
			LjInMovNCC( aNccItens, nMsDecimal	, cDocCred 		, nRecnoSE1	,;
